...
Built with Next.js Static Export, TypeScript, Tailwind CSS, and Shadcn UI. Deployed on Cloudflare Pages, with dynamic APIs served by Cloudflare Workers.
...
5. Preview the Cloudflare Pages output locally:

   ```bash
   pnpm preview
   ```

6. Run the API Worker locally:

   ```bash
   pnpm worker:dev
   ```

7. Edit personal data in [`src/data/resume.tsx`](./src/data/resume.tsx)
...
- Static assets: generated by `scripts/generate-static-assets.mjs` before `next build`

GitHub Actions deploys on every `main` push through `.github/workflows/deploy-cloudflare.yml`.
...
Worker secrets for newsletter subscription:

- `RESEND_API_KEY`
- `RESEND_AUDIENCE_ID`

Configure them in Cloudflare Worker secrets or inject them through your own CI secret workflow. Do not commit token values.

Worker routes:

- `kkdemian.com/api/*`
- `www.kkdemian.com/api/*`

The Worker currently serves:
...
1. Add `kkdemian.com` and `www.kkdemian.com` as Cloudflare Pages custom domains.
2. Point the apex/`www` records to Cloudflare Pages.
3. Keep Cloudflare proxy enabled.
4. Route `/api/*` to the `kkdemian-portfolio-api` Worker.
5. Remove the old Cloudflare -> Vercel path after Pages and Worker routes both pass production checks.
