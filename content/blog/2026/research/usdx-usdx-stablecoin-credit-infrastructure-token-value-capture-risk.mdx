---
title: 'USDX / USDX: stablecoin / credit infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'USDX is treated as a stablecoin / credit infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'usdx-usdx-stablecoin-credit-infrastructure-token-value-capture-risk'
category: 'research'
---

# USDX / USDX: stablecoin / credit infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** USDX appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Arbitrum One, Base, Binance Smart Chain, Ethereum; exchange exposure is not disclosed; category tags are Stablecoins, Yield & Asset Mgmt; current market cap is $5.52M; FDV is $5.52M; and 24h volume is $1.0460. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag USDX before this file was created, and the existing candidate note says: Seeded 2026-06-28 from Surf market-ranking; rank 334, market cap $70,317,755, 24h volume $4,182. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

USDX is best understood as a **stablecoin / credit infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether USDX captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| FDV | $5.52M |
| 24h volume | $1.0460 |
| Circulating supply | 684.56M |
| Total supply | 684.56M |
...
Base case: USDX remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning USDX into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

USDX is a crypto-native synthetic USD stablecoin issued by usdx.money (Stables Labs) that uses a delta-neutral multicoin arbitrage and derivatives hedging strategy to maintain its peg. The protocol also issues sUSDX (a yield-bearing staked USDX) and supports on-chain minting, LP swaps and cross-chain access via its app and docs.

Surf's project profile maps USDX to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://usdx.money/](https://usdx.money/) |
| X handle | [@usdx_money](https://x.com/usdx_money) |
| X followers | 284 |
| Tags | Stablecoins, Yield & Asset Mgmt |
| Chains | Arbitrum One, Base, Binance Smart Chain, Ethereum |
| Exchanges | not disclosed |
| TGE status | not disclosed |
| CoinGecko ID | not disclosed |

The main product question is whether USDX is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For stablecoin / credit infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does USDX convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $1.0460 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 684.56M circulating vs 684.56M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that USDX is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| base | USDX | 0xf3527ef8de265eaa3716fb312c12847bfba66cef | [explorer](https://basescan.org/token/0xf3527ef8de265eaa3716fb312c12847bfba66cef) |
| ethereum | USDX | 0xf3527ef8de265eaa3716fb312c12847bfba66cef | [explorer](https://etherscan.io/token/0xf3527ef8de265eaa3716fb312c12847bfba66cef) |
| arbitrum | USDX | 0xf3527ef8de265eaa3716fb312c12847bfba66cef | [explorer](https://arbiscan.io/token/0xf3527ef8de265eaa3716fb312c12847bfba66cef) |
| bsc | USDX | 0xf3527ef8de265eaa3716fb312c12847bfba66cef | [explorer](https://bscscan.com/token/0xf3527ef8de265eaa3716fb312c12847bfba66cef) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For USDX, the DeFi-oriented backlog note is the first signal: Seeded 2026-06-28 from Surf market-ranking; rank 334, market cap $70,317,755, 24h volume $4,182. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $5.52M, FDV $5.52M, 24h volume $1.0460, and price $0.0081.
...
| 24h volume | $1.0460 | Position sizing and exit liquidity proxy |
| Market cap | $5.52M | Liquid valuation proxy |
| FDV | $5.52M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.00x | Higher spread means more supply risk |
| X followers | 284 | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for USDX | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether USDX is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. USDX currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 5.80% and 30d -5.69% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that USDX's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2024-11-29 | Undisclosed | $45.00M | NGC Ventures, UOB Venture, BAI Capital, Generative Ventures |

| Team member | Role | Link |
|---|---|---|
| Flex Yang | Founder | [X](https://x.com/goXmoonman) |
...
USDX competes in the stablecoin / credit infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether USDX has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: USDX is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | USDX compounds usage, liquidity improves, and USDX gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because USDX has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If USDX does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $1.0460 | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if USDX announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. USDX has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For USDX, the first reconciliation job is identity. The token symbol is USDX, the category is stablecoin / credit infrastructure, chains are Arbitrum One, Base, Binance Smart Chain, Ethereum, exchanges are not disclosed, and tags are Stablecoins, Yield & Asset Mgmt. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If USDX trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $5.52M, FDV is $5.52M, 24h volume is $1.0460, FDV / market cap is 1.00x, 7d price change is 5.80%, and 30d price change is -5.69%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | USDX / USDX | Match official docs, CG, CMC, Surf, contracts |
...
For USDX, the current liquidity read is watchlist first. The market cap is $5.52M, FDV is $5.52M, reported 24h volume is $1.0460, and FDV / market cap is 1.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | USDX gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://usdx.money/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=USDX) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=USDX) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=USDX) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=USDX) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=USDX) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/usdx_money) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/StablesLabsTG) | Identity, market, social, contract, or discovery cross-check |
| 9 | Stables Labs (USDX) to USD Chart - CoinMarketCap | [link](https://coinmarketcap.com/currencies/usdx-money/) | Identity, market, social, contract, or discovery cross-check |
| 10 | Stables Labs USDX Price - CoinGecko | [link](https://www.coingecko.com/en/coins/stables-labs-usdx) | Identity, market, social, contract, or discovery cross-check |
| 11 | USDX/USD Live Price Chart, Market Cap & News Today - CoinGecko | [link](https://www.coingecko.com/en/coins/usdx) | Identity, market, social, contract, or discovery cross-check |
...
| 14 | base USDX | [link](https://basescan.org/token/0xf3527ef8de265eaa3716fb312c12847bfba66cef) | Identity, market, social, contract, or discovery cross-check |
| 15 | ethereum USDX | [link](https://etherscan.io/token/0xf3527ef8de265eaa3716fb312c12847bfba66cef) | Identity, market, social, contract, or discovery cross-check |
| 16 | arbitrum USDX | [link](https://arbiscan.io/token/0xf3527ef8de265eaa3716fb312c12847bfba66cef) | Identity, market, social, contract, or discovery cross-check |
| 17 | bsc USDX | [link](https://bscscan.com/token/0xf3527ef8de265eaa3716fb312c12847bfba66cef) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google project search | [link](https://www.google.com/search?q=USDX%20USDX%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google tokenomics search | [link](https://www.google.com/search?q=USDX%20USDX%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google docs search | [link](https://www.google.com/search?q=USDX%20docs) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google governance search | [link](https://www.google.com/search?q=USDX%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google audit search | [link](https://www.google.com/search?q=USDX%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/usdx/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/usdx/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 25 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/usdx/) | Identity, market, social, contract, or discovery cross-check |
| 26 | Dune discovery | [link](https://dune.com/browse/dashboards?q=USDX) | Identity, market, social, contract, or discovery cross-check |
| 27 | Dune token query | [link](https://dune.com/browse/queries?q=USDX) | Identity, market, social, contract, or discovery cross-check |
| 28 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=USDX) | Identity, market, social, contract, or discovery cross-check |
| 29 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=USDX) | Identity, market, social, contract, or discovery cross-check |
| 30 | TokenUnlocks search | [link](https://token.unlocks.app/?search=USDX) | Identity, market, social, contract, or discovery cross-check |
| 31 | CryptoRank search | [link](https://cryptorank.io/price/usdx) | Identity, market, social, contract, or discovery cross-check |
| 32 | RootData search | [link](https://www.rootdata.com/search?kw=USDX) | Identity, market, social, contract, or discovery cross-check |
| 33 | CoinDesk search | [link](https://www.coindesk.com/search?s=USDX) | Identity, market, social, contract, or discovery cross-check |
| 34 | The Block search | [link](https://www.theblock.co/search?query=USDX) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://usdx.money/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=USDX)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=USDX)
- [DefiLlama protocol discovery](https://defillama.com/search?query=USDX)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=USDX)
- [Messari discovery](https://messari.io/search?q=USDX)
- [X profile](https://x.com/usdx_money)
- [Telegram](https://t.me/StablesLabsTG)
- [Stables Labs (USDX) to USD Chart - CoinMarketCap](https://coinmarketcap.com/currencies/usdx-money/)
- [Stables Labs USDX Price - CoinGecko](https://www.coingecko.com/en/coins/stables-labs-usdx)
- [USDX/USD Live Price Chart, Market Cap & News Today - CoinGecko](https://www.coingecko.com/en/coins/usdx)
...
- [base USDX](https://basescan.org/token/0xf3527ef8de265eaa3716fb312c12847bfba66cef)
- [ethereum USDX](https://etherscan.io/token/0xf3527ef8de265eaa3716fb312c12847bfba66cef)
- [arbitrum USDX](https://arbiscan.io/token/0xf3527ef8de265eaa3716fb312c12847bfba66cef)
- [bsc USDX](https://bscscan.com/token/0xf3527ef8de265eaa3716fb312c12847bfba66cef)
- [Google project search](https://www.google.com/search?q=USDX%20USDX%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=USDX%20USDX%20tokenomics)
- [Google docs search](https://www.google.com/search?q=USDX%20docs)
- [Google governance search](https://www.google.com/search?q=USDX%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=USDX%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/usdx/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/usdx/price-analysis/)

## Final Investment View

USDX should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
