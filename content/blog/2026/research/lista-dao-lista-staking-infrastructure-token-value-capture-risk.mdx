---
title: 'Lista DAO / LISTA: staking infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Lista DAO is treated as a staking infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'lista-dao-lista-staking-infrastructure-token-value-capture-risk'
category: 'research'
---

# Lista DAO / LISTA: staking infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Lista DAO appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Binance Smart Chain; exchange exposure is BINANCE, BITHUMB, BITGET; category tags are Lending & Borrowing, LSD & Restaking, Stablecoins; current market cap is $20.82M; FDV is $39.90M; and 24h volume is $3.51M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Lista DAO before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 812, market cap $20,891,824, 24h volume $3,424,701. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Lista DAO is best understood as a **staking infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether LISTA captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| FDV | $39.90M |
| 24h volume | $3.51M |
| Circulating supply | 415.15M |
| Total supply | 795.59M |
| 7d price change | 8.39% |
| 30d price change | -3.05% |
| Latest TVL sample | $76.23M |
| TVL sample change | 1.82% |
| Primary source posture | Surf + CoinGecko + CMC discovery + official/project links |

Base case: Lista DAO remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning LISTA into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Lista DAO is a decentralized liquidity protocol on the Binance Chain, using cryptocurrencies as collateral for the decentralized stablecoin, lisUSD. It builds on the MakerDAO model, enhancing it with a dual-token system (lisUSD and LISTA) and functionalities like asset collateralization, borrowing, and yield farming. Lista aims to resolve capital efficiency issues in stablecoin markets by using Proof-of-Stake rewards and yield-bearing assets, reducing risks related to overcollateralization and unstable algorithmic models. The protocol plans to expand across multiple blockchain networks, promoting broader adoption and integration into the DeFi ecosystem.

Surf's project profile maps Lista DAO to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://lista.org/](https://lista.org/) |
| X handle | [@lista_dao](https://x.com/lista_dao) |
| X followers | 348.66K |
| Tags | Lending & Borrowing, LSD & Restaking, Stablecoins |
| Chains | Binance Smart Chain |
| Exchanges | BINANCE, BITHUMB, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Lista DAO is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For staking infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Lista DAO convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $3.51M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 415.15M circulating vs 795.59M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Lista DAO is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

Lista DAO's architecture should be evaluated through the job it performs. For a staking infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
| Settlement layer | Chains and contracts that hold user value | Binance Smart Chain |
...
|---|---|---|---|
| bsc | LISTA | 0xfceb31a79f71ac9cbdcf853519c1b12d379edc46 | [explorer](https://bscscan.com/token/0xfceb31a79f71ac9cbdcf853519c1b12d379edc46) |
| bsc | SLISBNBX | 0x4b30fcaa7945fe9fdefd2895aae539ba102ed6f6 | [explorer](https://bscscan.com/token/0x4b30fcaa7945fe9fdefd2895aae539ba102ed6f6) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Lista DAO, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 812, market cap $20,891,824, 24h volume $3,424,701. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $20.82M, FDV $39.90M, 24h volume $3.51M, and price $0.0501.

| Traction metric | Value | Interpretation |
|---|---|---|
| Latest TVL sample | $76.23M | Useful only if sustained without aggressive incentives |
| TVL sample points | 90 | More points improve trend confidence |
| TVL sample change | 1.82% | Positive trend supports usage; negative trend requires explanation |
| 24h volume | $3.51M | Position sizing and exit liquidity proxy |
| Market cap | $20.82M | Liquid valuation proxy |
| FDV | $39.90M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.92x | Higher spread means more supply risk |
| X followers | 348.66K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Lista DAO | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether LISTA is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Lista DAO currently needs to be monitored through that sequence. The available 90-day TVL series has 90 point(s), latest TVL $76.23M, and approximate change 1.82%. Market-price movement shows 7d 8.39% and 30d -3.05% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Lista DAO's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2024-06-19 | Ido | $100.00K | not disclosed |
| 2023-08-11 | Undisclosed | $10.00M | YZi Labs (Prev. Binance Labs) |

| Team member | Role | Link |
|---|---|---|
| Toru Watanabe | Co-Founder, CEO | [X](https://x.com/ToruW1949) |
| Terry Huang | Co-Founder, COO | [X](https://x.com/0xterry2016) |
| Carolina Fan (Lorena) | BD Lead, Core Contributor | [X](https://x.com/lorena_zZZ1) |
| Bob | Product Lead | [X](https://x.com/0xlistaBob) |
| Kay | Marketing Lead | [X](https://x.com/ZKay801) |
| Helen | Core Contributor | [X](https://x.com/Helen_listadao) |
...
Lista DAO competes in the staking infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Lista DAO has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Lista DAO is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Lista DAO compounds usage, liquidity improves, and LISTA gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If LISTA does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
|---|---|---|---|
| TVL | $76.23M | Weekly | >20% drawdown or sustained growth |
| 24h volume | $3.51M | Weekly | Volume below position-size threshold |
| FDV / market cap | 1.92x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Lista DAO announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Lista DAO has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Lista DAO, the first reconciliation job is identity. The token symbol is LISTA, the category is staking infrastructure, chains are Binance Smart Chain, exchanges are BINANCE, BITHUMB, BITGET, and tags are Lending & Borrowing, LSD & Restaking, Stablecoins. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If LISTA trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $20.82M, FDV is $39.90M, 24h volume is $3.51M, FDV / market cap is 1.92x, 7d price change is 8.39%, and 30d price change is -3.05%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.

The third reconciliation job is usage data. The latest TVL sample is $76.23M, TVL sample change is 1.82%, and the sample contains 90 point(s). TVL is useful only when the underlying balances are real, withdrawable, and not double-counted. For staking infrastructure, the memo should identify whether TVL means deposits, staked assets, LP liquidity, stablecoin supply, collateral, locked governance assets, or a derived wrapper. A yield-bearing stablecoin and a validator middleware protocol can both show TVL, but the risk encoded by that TVL is completely different.

| Reconciliation item | Current value | Next check |
|---|---|---|
| Identity | Lista DAO / LISTA | Match official docs, CG, CMC, Surf, contracts |
| Category | staking infrastructure | Confirm product mechanics and revenue model |
| Chains | Binance Smart Chain | Confirm canonical deployments and bridge assumptions |
| Exchanges | BINANCE, BITHUMB, BITGET | Check live order books and inactive pairs |
| Market cap | $20.82M | Compare Surf, CG, CMC, exchange pages |
| FDV | $39.90M | Reconcile total supply and unlock schedule |
| Volume | $3.51M | Check exchange concentration and wash-trading risk |
| TVL | $76.23M | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Lista DAO does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Lista DAO, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does LISTA sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
For Lista DAO, the current liquidity read is watchlist first. The market cap is $20.82M, FDV is $39.90M, reported 24h volume is $3.51M, and FDV / market cap is 1.92x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. Lista DAO can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.

For Lista DAO, the live monitoring stack should include contract events, TVL, volume, governance, documentation changes, social announcements, security disclosures, and exchange-market quality. A single metric is not enough. TVL may stay high while liquidity thins. Volume may rise because of volatility rather than adoption. Social attention may rise around controversy rather than progress. Governance may pass proposals that improve short-term price but add long-term risk. The strongest monitoring dashboard combines on-chain state, off-chain disclosure, and market quality.

Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If Lista DAO suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | Lista DAO gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://lista.org/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Lista%20DAO) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Lista%20DAO) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Lista%20DAO) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Lista%20DAO) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Lista%20DAO) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/lista_dao) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/ListaDAO) | Identity, market, social, contract, or discovery cross-check |
| 9 | Discord | [link](https://discord.com/invite/listadao) | Identity, market, social, contract, or discovery cross-check |
| 10 | Lista DAO Price: LISTA/USD Live Price Chart, Market ... - CoinGecko | [link](https://www.coingecko.com/en/coins/lista-dao) | Identity, market, social, contract, or discovery cross-check |
| 11 | Lista DAO (LISTA) Price Prediction For 2026 & Beyond | [link](https://coinmarketcap.com/cmc-ai/lista-dao/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 12 | What Is Lista DAO (LISTA) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/lista-dao/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 13 | LISTA to BTC: Lista DAO Price in Bitcoin / CoinGecko | [link](https://www.coingecko.com/en/coins/lista-dao/btc) | Identity, market, social, contract, or discovery cross-check |
| 14 | Lista DAO (LISTA) / Tokenomics & Vesting Schedule - Token Unlocks | [link](https://tokenomist.ai/lista) | Identity, market, social, contract, or discovery cross-check |
| 15 | bsc LISTA | [link](https://bscscan.com/token/0xfceb31a79f71ac9cbdcf853519c1b12d379edc46) | Identity, market, social, contract, or discovery cross-check |
| 16 | bsc SLISBNBX | [link](https://bscscan.com/token/0x4b30fcaa7945fe9fdefd2895aae539ba102ed6f6) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google project search | [link](https://www.google.com/search?q=Lista%20DAO%20LISTA%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google tokenomics search | [link](https://www.google.com/search?q=Lista%20DAO%20LISTA%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google docs search | [link](https://www.google.com/search?q=Lista%20DAO%20docs) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google governance search | [link](https://www.google.com/search?q=Lista%20DAO%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google audit search | [link](https://www.google.com/search?q=Lista%20DAO%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/lista-dao/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/lista-dao/) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Lista%20DAO) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune token query | [link](https://dune.com/browse/queries?q=Lista%20DAO) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Lista%20DAO) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=LISTA) | Identity, market, social, contract, or discovery cross-check |
| 28 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Lista%20DAO) | Identity, market, social, contract, or discovery cross-check |
| 29 | CryptoRank search | [link](https://cryptorank.io/price/lista-dao) | Identity, market, social, contract, or discovery cross-check |
| 30 | RootData search | [link](https://www.rootdata.com/search?kw=Lista%20DAO) | Identity, market, social, contract, or discovery cross-check |
| 31 | CoinDesk search | [link](https://www.coindesk.com/search?s=Lista%20DAO) | Identity, market, social, contract, or discovery cross-check |
| 32 | The Block search | [link](https://www.theblock.co/search?query=Lista%20DAO) | Identity, market, social, contract, or discovery cross-check |
| 33 | GitHub search | [link](https://github.com/search?q=Lista%20DAO&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 34 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=LISTA) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://lista.org/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Lista%20DAO)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Lista%20DAO)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Lista%20DAO)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Lista%20DAO)
- [Messari discovery](https://messari.io/search?q=Lista%20DAO)
- [X profile](https://x.com/lista_dao)
- [Telegram](https://t.me/ListaDAO)
- [Discord](https://discord.com/invite/listadao)
- [Lista DAO Price: LISTA/USD Live Price Chart, Market ... - CoinGecko](https://www.coingecko.com/en/coins/lista-dao)
- [Lista DAO (LISTA) Price Prediction For 2026 & Beyond](https://coinmarketcap.com/cmc-ai/lista-dao/price-prediction/)
- [What Is Lista DAO (LISTA) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/lista-dao/what-is/)
- [LISTA to BTC: Lista DAO Price in Bitcoin | CoinGecko](https://www.coingecko.com/en/coins/lista-dao/btc)
- [Lista DAO (LISTA) | Tokenomics & Vesting Schedule - Token Unlocks](https://tokenomist.ai/lista)
- [bsc LISTA](https://bscscan.com/token/0xfceb31a79f71ac9cbdcf853519c1b12d379edc46)
- [bsc SLISBNBX](https://bscscan.com/token/0x4b30fcaa7945fe9fdefd2895aae539ba102ed6f6)
- [Google project search](https://www.google.com/search?q=Lista%20DAO%20LISTA%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Lista%20DAO%20LISTA%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Lista%20DAO%20docs)
- [Google governance search](https://www.google.com/search?q=Lista%20DAO%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Lista%20DAO%20audit%20report)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/lista-dao/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/lista-dao/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Lista%20DAO)

## Final Investment View

Lista DAO should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
