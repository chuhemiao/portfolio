---
title: 'Bonfida / FIDA: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Bonfida is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'bonfida-fida-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# Bonfida / FIDA: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Bonfida appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Solana; exchange exposure is not disclosed; category tags are Developer Tooling, DID & Identity; current market cap is $21.79M; FDV is $21.79M; and 24h volume is $3.12M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Bonfida before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 789, market cap $21,478,920, 24h volume $2,965,766. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Bonfida is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether FIDA captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| FDV | $21.79M |
| 24h volume | $3.12M |
| Circulating supply | 990.91M |
| Total supply | 990.91M |
...
Base case: Bonfida remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning FIDA into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Solana Name Service (SNS) is a decentralized naming protocol on Solana that maps human-readable .sol domains and other identifiers to on‑chain data, enabling unified Web3 identities, address resolution and crypto payments. SNS operates a registry, marketplace, developer SDKs, and an SNS token used for ecosystem utility and distribution.

Surf's project profile maps Bonfida to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://sns.id](https://sns.id) |
| X handle | [@sns](https://x.com/sns) |
| X followers | 145.87K |
...
| Liquidity | $3.12M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 990.91M circulating vs 990.91M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Bonfida is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

Bonfida's architecture should be evaluated through the job it performs. For a DeFi infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
|---|---|---|---|
| solana | FIDA | EchesyfXePKdLtoiZSL8pBe8Myagyy8ZRqsACNCFGnvp | [explorer](https://solscan.io/token/EchesyfXePKdLtoiZSL8pBe8Myagyy8ZRqsACNCFGnvp) |
| solana | SNS | SNS8DJbHc34nKySHVhLGMUUE72ho6igvJaxtq9T3cX3 | [explorer](https://solscan.io/token/SNS8DJbHc34nKySHVhLGMUUE72ho6igvJaxtq9T3cX3) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Bonfida, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 789, market cap $21,478,920, 24h volume $2,965,766. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $21.79M, FDV $21.79M, 24h volume $3.12M, and price $0.0220.
...
| 24h volume | $3.12M | Position sizing and exit liquidity proxy |
| Market cap | $21.79M | Liquid valuation proxy |
| FDV | $21.79M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.00x | Higher spread means more supply risk |
| X followers | 145.87K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Bonfida | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether FIDA is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Bonfida currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 1.18% and 30d 19.19% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Bonfida's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2020-12-22 | Ico | $200.00K | not disclosed |
| 2020-12-17 | Ieo | $200.00K | not disclosed |
| 2020-12-17 | Ieo | $200.00K | not disclosed |
| 2020-12-15 | Seed | $4.50M | GBV Capital, Three Arrows Capital, CMS Holdings, The Spartan Group, Ryze Labs (Sino Global Capital) |

| Team member | Role | Link |
|---|---|---|
| Aunchisa (Aom) Tapanakornvut | CEO | [X](https://x.com/aomdotsol) |
| ProductGal | Head of Product Growth | [X](https://x.com/0xProductGal) |
...
Bonfida competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Bonfida has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Bonfida is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Bonfida compounds usage, liquidity improves, and FIDA gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Bonfida has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If FIDA does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $3.12M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Bonfida announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Bonfida has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Bonfida, the first reconciliation job is identity. The token symbol is FIDA, the category is DeFi infrastructure, chains are Solana, exchanges are not disclosed, and tags are Developer Tooling, DID & Identity. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If FIDA trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $21.79M, FDV is $21.79M, 24h volume is $3.12M, FDV / market cap is 1.00x, 7d price change is 1.18%, and 30d price change is 19.19%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Bonfida / FIDA | Match official docs, CG, CMC, Surf, contracts |
...
For Bonfida, the current liquidity read is watchlist first. The market cap is $21.79M, FDV is $21.79M, reported 24h volume is $3.12M, and FDV / market cap is 1.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Bonfida gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://sns.id) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Bonfida) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Bonfida) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Bonfida) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Bonfida) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Bonfida) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/sns) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/SolanaNameService) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/snsdotsol) | Identity, market, social, contract, or discovery cross-check |
| 10 | Bonfida Price: FIDA/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/bonfida-sns) | Identity, market, social, contract, or discovery cross-check |
| 11 | What Is Bonfida (FIDA) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/bonfida-sns/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 12 | FIDA Deep Dive: Token History, Current State and Future Outlook | [link](https://onekey.so/blog/ecosystem/fida-deep-dive-token-history-current-state-and-future-outlook/?srsltid=AfmBOoqFXn6McLQJPJfxwllHuhv5mJDO4-BKdmple14C3YjLAZO6YZ0v) | Identity, market, social, contract, or discovery cross-check |
| 13 | FIDA to BTC: Bonfida Price in Bitcoin - CoinGecko | [link](https://www.coingecko.com/en/coins/bonfida-sns/btc) | Identity, market, social, contract, or discovery cross-check |
| 14 | Bonfida (FIDA) / Tokenomics & Vesting Schedule - Token Unlocks | [link](https://tokenomist.ai/bonfida) | Identity, market, social, contract, or discovery cross-check |
| 15 | solana FIDA | [link](https://solscan.io/token/EchesyfXePKdLtoiZSL8pBe8Myagyy8ZRqsACNCFGnvp) | Identity, market, social, contract, or discovery cross-check |
| 16 | solana SNS | [link](https://solscan.io/token/SNS8DJbHc34nKySHVhLGMUUE72ho6igvJaxtq9T3cX3) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google project search | [link](https://www.google.com/search?q=Bonfida%20FIDA%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google tokenomics search | [link](https://www.google.com/search?q=Bonfida%20FIDA%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google docs search | [link](https://www.google.com/search?q=Bonfida%20docs) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google governance search | [link](https://www.google.com/search?q=Bonfida%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google audit search | [link](https://www.google.com/search?q=Bonfida%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/bonfida/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/bonfida/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/bonfida/) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Bonfida) | Identity, market, social, contract, or discovery cross-check |
| 26 | Dune token query | [link](https://dune.com/browse/queries?q=Bonfida) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Bonfida) | Identity, market, social, contract, or discovery cross-check |
| 28 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=FIDA) | Identity, market, social, contract, or discovery cross-check |
| 29 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Bonfida) | Identity, market, social, contract, or discovery cross-check |
| 30 | CryptoRank search | [link](https://cryptorank.io/price/bonfida) | Identity, market, social, contract, or discovery cross-check |
| 31 | RootData search | [link](https://www.rootdata.com/search?kw=Bonfida) | Identity, market, social, contract, or discovery cross-check |
| 32 | CoinDesk search | [link](https://www.coindesk.com/search?s=Bonfida) | Identity, market, social, contract, or discovery cross-check |
| 33 | The Block search | [link](https://www.theblock.co/search?query=Bonfida) | Identity, market, social, contract, or discovery cross-check |
| 34 | GitHub search | [link](https://github.com/search?q=Bonfida&type=repositories) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://sns.id)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Bonfida)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Bonfida)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Bonfida)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Bonfida)
- [Messari discovery](https://messari.io/search?q=Bonfida)
- [X profile](https://x.com/sns)
- [GitHub](https://github.com/SolanaNameService)
- [Telegram](https://t.me/snsdotsol)
- [Bonfida Price: FIDA/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/bonfida-sns)
- [What Is Bonfida (FIDA) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/bonfida-sns/what-is/)
- [FIDA Deep Dive: Token History, Current State and Future Outlook](https://onekey.so/blog/ecosystem/fida-deep-dive-token-history-current-state-and-future-outlook/?srsltid=AfmBOoqFXn6McLQJPJfxwllHuhv5mJDO4-BKdmple14C3YjLAZO6YZ0v)
- [FIDA to BTC: Bonfida Price in Bitcoin - CoinGecko](https://www.coingecko.com/en/coins/bonfida-sns/btc)
- [Bonfida (FIDA) | Tokenomics & Vesting Schedule - Token Unlocks](https://tokenomist.ai/bonfida)
- [solana FIDA](https://solscan.io/token/EchesyfXePKdLtoiZSL8pBe8Myagyy8ZRqsACNCFGnvp)
- [solana SNS](https://solscan.io/token/SNS8DJbHc34nKySHVhLGMUUE72ho6igvJaxtq9T3cX3)
- [Google project search](https://www.google.com/search?q=Bonfida%20FIDA%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Bonfida%20FIDA%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Bonfida%20docs)
- [Google governance search](https://www.google.com/search?q=Bonfida%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Bonfida%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/bonfida/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/bonfida/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/bonfida/)

## Final Investment View

Bonfida should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
