---
title: 'Orderly / ORDER: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Orderly is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'orderly-order-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# Orderly / ORDER: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Orderly appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Abstract, Arbitrum One, Avalanche, Base, Binance Smart Chain, Ethereum, Optimistic Ethereum, Polygon Pos, Solana; exchange exposure is UPBIT, BITHUMB, BYBIT, BINANCE ALPHA, BITGET; category tags are Developer Tooling, Derivatives (Perp/Option); current market cap is $14.21M; FDV is $36.11M; and 24h volume is $6.28M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Orderly before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 994, market cap $14,364,024, 24h volume $6,256,938. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Orderly is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether ORDER captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Abstract, Arbitrum One, Avalanche, Base, Binance Smart Chain, Ethereum, Optimistic Ethereum, Polygon Pos, Solana |
| Exchanges | UPBIT, BITHUMB, BYBIT, BINANCE ALPHA, BITGET |
| Price | $0.0361 |
| Market cap | $14.21M |
| FDV | $36.11M |
| 24h volume | $6.28M |
| Circulating supply | 393.63M |
| Total supply | 1.00B |
| 7d price change | 5.71% |
| 30d price change | -4.17% |
...
Base case: Orderly remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning ORDER into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Orderly Network is a permissionless omnichain liquidity layer and orderbook-based trading infrastructure that provides CEX-level spot and perpetual orderbooks via SDKs and APIs for builders. Its ORDER token is the protocol token and the network targets plug-and-play DEX front-ends, omnichain settlement, and integrations across EVM and non-EVM chains.

Surf's project profile maps Orderly to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://orderly.network/](https://orderly.network/) |
| X handle | [@OrderlyNetwork](https://x.com/OrderlyNetwork) |
| X followers | 391.77K |
| Tags | Developer Tooling, Derivatives (Perp/Option) |
| Chains | Abstract, Arbitrum One, Avalanche, Base, Binance Smart Chain, Ethereum, Optimistic Ethereum, Polygon Pos, Solana |
| Exchanges | UPBIT, BITHUMB, BYBIT, BINANCE ALPHA, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Orderly is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DeFi infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Orderly convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $6.28M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 393.63M circulating vs 1.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Orderly is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Abstract, Arbitrum One, Avalanche, Base, Binance Smart Chain, Ethereum, Optimistic Ethereum, Polygon Pos, Solana |
...
|---|---|---|---|
| Abstract | ORDER | 0x9a4c8cd493cf0bf8e14bda5890e3f03a1cded719 | not mapped |
| ethereum | ORDER | 0xabd4c63d2616a5201454168269031355f4764337 | [explorer](https://etherscan.io/token/0xabd4c63d2616a5201454168269031355f4764337) |
| avalanche | ORDER | 0x4e200fe2f3efb977d5fd9c430a41531fb04d97b8 | [explorer](https://snowtrace.io/token/0x4e200fe2f3efb977d5fd9c430a41531fb04d97b8) |
| polygon | ORDER | 0x4e200fe2f3efb977d5fd9c430a41531fb04d97b8 | [explorer](https://polygonscan.com/token/0x4e200fe2f3efb977d5fd9c430a41531fb04d97b8) |
| arbitrum | ORDER | 0x4e200fe2f3efb977d5fd9c430a41531fb04d97b8 | [explorer](https://arbiscan.io/token/0x4e200fe2f3efb977d5fd9c430a41531fb04d97b8) |
| bsc | ORDER | 0x4e200fe2f3efb977d5fd9c430a41531fb04d97b8 | [explorer](https://bscscan.com/token/0x4e200fe2f3efb977d5fd9c430a41531fb04d97b8) |
| optimism | ORDER | 0x4e200fe2f3efb977d5fd9c430a41531fb04d97b8 | [explorer](https://optimistic.etherscan.io/token/0x4e200fe2f3efb977d5fd9c430a41531fb04d97b8) |
| base | ORDER | 0x4e200fe2f3efb977d5fd9c430a41531fb04d97b8 | [explorer](https://basescan.org/token/0x4e200fe2f3efb977d5fd9c430a41531fb04d97b8) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Orderly, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 994, market cap $14,364,024, 24h volume $6,256,938. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $14.21M, FDV $36.11M, 24h volume $6.28M, and price $0.0361.
...
| 24h volume | $6.28M | Position sizing and exit liquidity proxy |
| Market cap | $14.21M | Liquid valuation proxy |
| FDV | $36.11M | Dilution-aware valuation proxy |
| FDV / market cap spread | 2.54x | Higher spread means more supply risk |
| X followers | 391.77K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Orderly | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether ORDER is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. Orderly should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that Orderly belongs to the DeFi infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $14.21M, FDV $36.11M, 24h volume $6.28M, price $0.0361, circulating supply 393.63M, and total supply 1.00B. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For Orderly, the candidate backlog records Seeded 2026-07-06 from Surf market-ranking; rank 994, market cap $14,364,024, 24h volume $6,256,938. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Abstract, Arbitrum One, Avalanche, Base, Binance Smart Chain, Ethereum, Optimistic Ethereum, Polygon Pos, Solana, exchanges as UPBIT, BITHUMB, BYBIT, BINANCE ALPHA, BITGET, and tags as Developer Tooling, Derivatives (Perp/Option). If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Orderly currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 5.71% and 30d -4.17% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Orderly's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2024-08-15 | Strategic | $5.00M | OKX Ventures, Manifold, Origin Protocol, Presto Labs, Nomad Capital, LTP | LiquidityTech |
| 2023-03-28 | Strategic | not disclosed | CoinDCX |
| 2022-11-01 | Undisclosed | not disclosed | Laser Digital |
| 2022-06-09 | Series A | $20.00M | Pantera Capital, Alameda Research, Crypto.com Capital, Three Arrows Capital, Sequoia Capital, LedgerPrime, Dragonfly, Mirana Ventures |

| Team member | Role | Link |
|---|---|---|
| Terence Ng | Co-Founder | [X](https://x.com/TerenceTheNg) |
| Ran Yi | Co-Founder | [X](https://x.com/ranyi1115) |
| Arjun Arora | COO | [X](https://x.com/0x_Arjun) |
| Drew Pierson | Chief Vibes Officer | not disclosed |
| Andrey Lazorenko | Advisor | [X](https://twitter.com/AndreyLazorenko) |
...
Orderly competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Orderly has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Orderly is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Orderly compounds usage, liquidity improves, and ORDER gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Orderly has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If ORDER does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $6.28M | Weekly | Volume below position-size threshold |
| FDV / market cap | 2.54x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Orderly announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Orderly has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Orderly, the first reconciliation job is identity. The token symbol is ORDER, the category is DeFi infrastructure, chains are Abstract, Arbitrum One, Avalanche, Base, Binance Smart Chain, Ethereum, Optimistic Ethereum, Polygon Pos, Solana, exchanges are UPBIT, BITHUMB, BYBIT, BINANCE ALPHA, BITGET, and tags are Developer Tooling, Derivatives (Perp/Option). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If ORDER trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $14.21M, FDV is $36.11M, 24h volume is $6.28M, FDV / market cap is 2.54x, 7d price change is 5.71%, and 30d price change is -4.17%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Orderly / ORDER | Match official docs, CG, CMC, Surf, contracts |
| Category | DeFi infrastructure | Confirm product mechanics and revenue model |
| Chains | Abstract, Arbitrum One, Avalanche, Base, Binance Smart Chain, Ethereum, Optimistic Ethereum, Polygon Pos, Solana | Confirm canonical deployments and bridge assumptions |
| Exchanges | UPBIT, BITHUMB, BYBIT, BINANCE ALPHA, BITGET | Check live order books and inactive pairs |
| Market cap | $14.21M | Compare Surf, CG, CMC, exchange pages |
| FDV | $36.11M | Reconcile total supply and unlock schedule |
| Volume | $6.28M | Check exchange concentration and wash-trading risk |
| TVL | not disclosed | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Orderly does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Orderly, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does ORDER sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
| Peer comparison | Monthly | Orderly gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://orderly.network/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Orderly) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Orderly) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Orderly) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Orderly) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Orderly) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/OrderlyNetwork) | Identity, market, social, contract, or discovery cross-check |
| 8 | ORDER/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/orderly-network) | Identity, market, social, contract, or discovery cross-check |
| 9 | What Is Orderly (ORDER) And How Does It Work? | [link](https://coinmarketcap.com/cmc-ai/orderly-network/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 10 | Overview of $ORDER | [link](https://orderly.network/docs/introduction/tokenomics/overview-of-order) | Identity, market, social, contract, or discovery cross-check |
| 11 | Latest Orderly (ORDER) Price Analysis | [link](https://coinmarketcap.com/cmc-ai/orderly-network/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 12 | CoinGecko Orderly Report | [link](https://www.coingecko.com/research/publications/coingecko-orderly-report) | Identity, market, social, contract, or discovery cross-check |
| 13 | ethereum ORDER | [link](https://etherscan.io/token/0xabd4c63d2616a5201454168269031355f4764337) | Identity, market, social, contract, or discovery cross-check |
| 14 | avalanche ORDER | [link](https://snowtrace.io/token/0x4e200fe2f3efb977d5fd9c430a41531fb04d97b8) | Identity, market, social, contract, or discovery cross-check |
| 15 | polygon ORDER | [link](https://polygonscan.com/token/0x4e200fe2f3efb977d5fd9c430a41531fb04d97b8) | Identity, market, social, contract, or discovery cross-check |
| 16 | arbitrum ORDER | [link](https://arbiscan.io/token/0x4e200fe2f3efb977d5fd9c430a41531fb04d97b8) | Identity, market, social, contract, or discovery cross-check |
| 17 | bsc ORDER | [link](https://bscscan.com/token/0x4e200fe2f3efb977d5fd9c430a41531fb04d97b8) | Identity, market, social, contract, or discovery cross-check |
| 18 | optimism ORDER | [link](https://optimistic.etherscan.io/token/0x4e200fe2f3efb977d5fd9c430a41531fb04d97b8) | Identity, market, social, contract, or discovery cross-check |
| 19 | base ORDER | [link](https://basescan.org/token/0x4e200fe2f3efb977d5fd9c430a41531fb04d97b8) | Identity, market, social, contract, or discovery cross-check |
| 20 | solana ORDER | [link](https://solscan.io/token/ABt79MkRXUsoHuV2CVQT32YMXQhTparKFjmidQxgiQ6E) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google project search | [link](https://www.google.com/search?q=Orderly%20ORDER%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google tokenomics search | [link](https://www.google.com/search?q=Orderly%20ORDER%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 23 | Google docs search | [link](https://www.google.com/search?q=Orderly%20docs) | Identity, market, social, contract, or discovery cross-check |
| 24 | Google governance search | [link](https://www.google.com/search?q=Orderly%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 25 | Google audit search | [link](https://www.google.com/search?q=Orderly%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 26 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/orderly/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 27 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/orderly/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 28 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/orderly/) | Identity, market, social, contract, or discovery cross-check |
| 29 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Orderly) | Identity, market, social, contract, or discovery cross-check |
| 30 | Dune token query | [link](https://dune.com/browse/queries?q=Orderly) | Identity, market, social, contract, or discovery cross-check |
| 31 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Orderly) | Identity, market, social, contract, or discovery cross-check |
| 32 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=ORDER) | Identity, market, social, contract, or discovery cross-check |
| 33 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Orderly) | Identity, market, social, contract, or discovery cross-check |
| 34 | CryptoRank search | [link](https://cryptorank.io/price/orderly) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://orderly.network/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Orderly)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Orderly)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Orderly)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Orderly)
- [Messari discovery](https://messari.io/search?q=Orderly)
- [X profile](https://x.com/OrderlyNetwork)
- [ORDER/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/orderly-network)
- [What Is Orderly (ORDER) And How Does It Work?](https://coinmarketcap.com/cmc-ai/orderly-network/what-is/)
- [Overview of $ORDER](https://orderly.network/docs/introduction/tokenomics/overview-of-order)
- [Latest Orderly (ORDER) Price Analysis](https://coinmarketcap.com/cmc-ai/orderly-network/price-analysis/)
- [CoinGecko Orderly Report](https://www.coingecko.com/research/publications/coingecko-orderly-report)
- [ethereum ORDER](https://etherscan.io/token/0xabd4c63d2616a5201454168269031355f4764337)
- [avalanche ORDER](https://snowtrace.io/token/0x4e200fe2f3efb977d5fd9c430a41531fb04d97b8)
- [polygon ORDER](https://polygonscan.com/token/0x4e200fe2f3efb977d5fd9c430a41531fb04d97b8)
- [arbitrum ORDER](https://arbiscan.io/token/0x4e200fe2f3efb977d5fd9c430a41531fb04d97b8)
- [bsc ORDER](https://bscscan.com/token/0x4e200fe2f3efb977d5fd9c430a41531fb04d97b8)
- [optimism ORDER](https://optimistic.etherscan.io/token/0x4e200fe2f3efb977d5fd9c430a41531fb04d97b8)
- [base ORDER](https://basescan.org/token/0x4e200fe2f3efb977d5fd9c430a41531fb04d97b8)
- [solana ORDER](https://solscan.io/token/ABt79MkRXUsoHuV2CVQT32YMXQhTparKFjmidQxgiQ6E)
- [Google project search](https://www.google.com/search?q=Orderly%20ORDER%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Orderly%20ORDER%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Orderly%20docs)
- [Google governance search](https://www.google.com/search?q=Orderly%20governance%20proposal)

## Final Investment View

Orderly should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
