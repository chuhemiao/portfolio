---
title: 'BOB (Build on Bitcoin) / BOB: bridge / cross-chain infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'BOB (Build on Bitcoin) is treated as a bridge / cross-chain infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'bob-build-on-bitcoin-bob-bridge-cross-chain-infrastructure-token-value-capture-risk'
category: 'research'
---

# BOB (Build on Bitcoin) / BOB: bridge / cross-chain infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** BOB (Build on Bitcoin) appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Binance Smart Chain, Bob Network, Ethereum; exchange exposure is BITHUMB, BINANCE ALPHA, GATE; category tags are Cross-Chain & Bridge, Layer2 & Scaling; current market cap is $14.47M; FDV is $46.40M; and 24h volume is $2.05M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag BOB (Build on Bitcoin) before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 985, market cap $14,567,447, 24h volume $2,073,440. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

BOB (Build on Bitcoin) is best understood as a **bridge / cross-chain infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether BOB captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.

| Metric | Current read |
|---|---|
| Project | BOB (Build on Bitcoin) |
| Token | BOB |
| Category | bridge / cross-chain infrastructure |
| Chains | Binance Smart Chain, Bob Network, Ethereum |
| Exchanges | BITHUMB, BINANCE ALPHA, GATE |
| Price | $0.0046 |
| Market cap | $14.47M |
| FDV | $46.40M |
| 24h volume | $2.05M |
| Circulating supply | 3.12B |
| Total supply | 10.00B |
| 7d price change | -3.56% |
| 30d price change | -22.56% |
...
Base case: BOB (Build on Bitcoin) remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning BOB into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

BOB (Build on Bitcoin) is a hybrid Layer‑2 “Gateway to Bitcoin DeFi” that combines Bitcoin security with an EVM execution layer to enable native BTC in DeFi via BitVM, hybrid ZK/optimistic rollup designs and trust‑minimized BTC bridges. It offers developer tooling, SDKs and a mainnet app for staking, cross‑chain swaps and multi‑chain Bitcoin yield.

Surf's project profile maps BOB (Build on Bitcoin) to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.gobob.xyz/](https://www.gobob.xyz/) |
| X handle | [@build_on_bob](https://x.com/build_on_bob) |
| X followers | 288.85K |
| Tags | Cross-Chain & Bridge, Layer2 & Scaling |
| Chains | Binance Smart Chain, Bob Network, Ethereum |
| Exchanges | BITHUMB, BINANCE ALPHA, GATE |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether BOB (Build on Bitcoin) is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For bridge / cross-chain infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does BOB (Build on Bitcoin) convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $2.05M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 3.12B circulating vs 10.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that BOB (Build on Bitcoin) is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

BOB (Build on Bitcoin)'s architecture should be evaluated through the job it performs. For a bridge / cross-chain infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
| Settlement layer | Chains and contracts that hold user value | Binance Smart Chain, Bob Network, Ethereum |
...
|---|---|---|---|
| ethereum | BOB | 0xc9746f73cc33a36c2cd55b8aefd732586946cedd | [explorer](https://etherscan.io/token/0xc9746f73cc33a36c2cd55b8aefd732586946cedd) |
| Bob Network | BOB | 0xb0bd54846a92b214c04a63b26ad7dc5e19a60808 | not mapped |
| bsc | BOB | 0x52b5fb4b0f6572b8c44d0251cc224513ac5eb7e7 | [explorer](https://bscscan.com/token/0x52b5fb4b0f6572b8c44d0251cc224513ac5eb7e7) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For BOB (Build on Bitcoin), the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 985, market cap $14,567,447, 24h volume $2,073,440. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $14.47M, FDV $46.40M, 24h volume $2.05M, and price $0.0046.
...
| 24h volume | $2.05M | Position sizing and exit liquidity proxy |
| Market cap | $14.47M | Liquid valuation proxy |
| FDV | $46.40M | Dilution-aware valuation proxy |
| FDV / market cap spread | 3.21x | Higher spread means more supply risk |
| X followers | 288.85K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for BOB (Build on Bitcoin) | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether BOB is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. BOB (Build on Bitcoin) should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that BOB (Build on Bitcoin) belongs to the bridge / cross-chain infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $14.47M, FDV $46.40M, 24h volume $2.05M, price $0.0046, circulating supply 3.12B, and total supply 10.00B. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For BOB (Build on Bitcoin), the candidate backlog records Seeded 2026-07-06 from Surf market-ranking; rank 985, market cap $14,567,447, 24h volume $2,073,440. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Binance Smart Chain, Bob Network, Ethereum, exchanges as BITHUMB, BINANCE ALPHA, GATE, and tags as Cross-Chain & Bridge, Layer2 & Scaling. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. BOB (Build on Bitcoin) currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d -3.56% and 30d -22.56% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that BOB (Build on Bitcoin)'s product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-11-10 | Ico | $4.60M | not disclosed |
| 2025-11-10 | Ieo | not disclosed | not disclosed |
| 2025-11-10 | Kol | $3.30M | not disclosed |
| 2025-08-07 | Strategic | $9.50M | CMS Holdings, Zee Prime Capital, IOSG Ventures, Hypersphere Ventures, Amber Group, RockawayX, Asymmetric, Daedalus |
| 2024-07-09 | Strategic | $1.60M | Ledger |
| 2024-04-15 | Seed | not disclosed | Coinbase Ventures |

| Team member | Role | Link |
|---|---|---|
| Alexei Zamyatin | Co-Founder, CEO | [X](https://twitter.com/alexeiZamyatin) |
| Dominik Harz | Co-Founder, CTO | [X](https://twitter.com/dominik0_) |
| David Garai | Advisor | not disclosed |
...
BOB (Build on Bitcoin) competes in the bridge / cross-chain infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether BOB (Build on Bitcoin) has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: BOB (Build on Bitcoin) is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | BOB (Build on Bitcoin) compounds usage, liquidity improves, and BOB gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
| 24h volume | $2.05M | Weekly | Volume below position-size threshold |
| FDV / market cap | 3.21x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if BOB (Build on Bitcoin) announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. BOB (Build on Bitcoin) has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For BOB (Build on Bitcoin), the first reconciliation job is identity. The token symbol is BOB, the category is bridge / cross-chain infrastructure, chains are Binance Smart Chain, Bob Network, Ethereum, exchanges are BITHUMB, BINANCE ALPHA, GATE, and tags are Cross-Chain & Bridge, Layer2 & Scaling. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If BOB trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $14.47M, FDV is $46.40M, 24h volume is $2.05M, FDV / market cap is 3.21x, 7d price change is -3.56%, and 30d price change is -22.56%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | BOB (Build on Bitcoin) / BOB | Match official docs, CG, CMC, Surf, contracts |
| Category | bridge / cross-chain infrastructure | Confirm product mechanics and revenue model |
| Chains | Binance Smart Chain, Bob Network, Ethereum | Confirm canonical deployments and bridge assumptions |
| Exchanges | BITHUMB, BINANCE ALPHA, GATE | Check live order books and inactive pairs |
| Market cap | $14.47M | Compare Surf, CG, CMC, exchange pages |
| FDV | $46.40M | Reconcile total supply and unlock schedule |
| Volume | $2.05M | Check exchange concentration and wash-trading risk |
| TVL | not disclosed | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what BOB (Build on Bitcoin) does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For BOB (Build on Bitcoin), the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does BOB sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
For BOB (Build on Bitcoin), the current liquidity read is watchlist first. The market cap is $14.47M, FDV is $46.40M, reported 24h volume is $2.05M, and FDV / market cap is 3.21x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. BOB (Build on Bitcoin) can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.

For BOB (Build on Bitcoin), the live monitoring stack should include contract events, TVL, volume, governance, documentation changes, social announcements, security disclosures, and exchange-market quality. A single metric is not enough. TVL may stay high while liquidity thins. Volume may rise because of volatility rather than adoption. Social attention may rise around controversy rather than progress. Governance may pass proposals that improve short-term price but add long-term risk. The strongest monitoring dashboard combines on-chain state, off-chain disclosure, and market quality.

Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If BOB (Build on Bitcoin) suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | BOB (Build on Bitcoin) gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.gobob.xyz/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=BOB%20(Build%20on%20Bitcoin) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=BOB%20(Build%20on%20Bitcoin) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=BOB%20(Build%20on%20Bitcoin) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=BOB%20(Build%20on%20Bitcoin) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=BOB%20(Build%20on%20Bitcoin) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/build_on_bob) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/bob-collective/bob) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/gobobxyz) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](https://discord.com/invite/gobob) | Identity, market, social, contract, or discovery cross-check |
| 11 | BOB (Build on Bitcoin) Price Chart (BOB) - CoinGecko | [link](https://www.coingecko.com/en/coins/bob-build-on-bitcoin) | Identity, market, social, contract, or discovery cross-check |
| 12 | BOB Token Price: BOB/USD Live Price Chart, Market ... - CoinGecko | [link](https://www.coingecko.com/en/coins/bob-token) | Identity, market, social, contract, or discovery cross-check |
| 13 | Latest BOB (Build on Bitcoin) (BOB) Price Analysis - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/bob-build-on-bitcoin/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 14 | Build On BNB Price Chart (BOB) - CoinGecko | [link](https://www.coingecko.com/en/coins/build-on-bnb) | Identity, market, social, contract, or discovery cross-check |
| 15 | BOB (Build on Bitcoin) (BOB) Price Prediction - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/bob-build-on-bitcoin/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 16 | ethereum BOB | [link](https://etherscan.io/token/0xc9746f73cc33a36c2cd55b8aefd732586946cedd) | Identity, market, social, contract, or discovery cross-check |
| 17 | bsc BOB | [link](https://bscscan.com/token/0x52b5fb4b0f6572b8c44d0251cc224513ac5eb7e7) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google project search | [link](https://www.google.com/search?q=BOB%20(Build%20on%20Bitcoin)%20BOB%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google tokenomics search | [link](https://www.google.com/search?q=BOB%20(Build%20on%20Bitcoin)%20BOB%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google docs search | [link](https://www.google.com/search?q=BOB%20(Build%20on%20Bitcoin)%20docs) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google governance search | [link](https://www.google.com/search?q=BOB%20(Build%20on%20Bitcoin)%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google audit search | [link](https://www.google.com/search?q=BOB%20(Build%20on%20Bitcoin)%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/bob-build-on-bitcoin/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/bob-build-on-bitcoin/) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune discovery | [link](https://dune.com/browse/dashboards?q=BOB%20(Build%20on%20Bitcoin) | Identity, market, social, contract, or discovery cross-check |
| 26 | Dune token query | [link](https://dune.com/browse/queries?q=BOB%20(Build%20on%20Bitcoin) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=BOB%20(Build%20on%20Bitcoin) | Identity, market, social, contract, or discovery cross-check |
| 28 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=BOB) | Identity, market, social, contract, or discovery cross-check |
| 29 | TokenUnlocks search | [link](https://token.unlocks.app/?search=BOB%20(Build%20on%20Bitcoin) | Identity, market, social, contract, or discovery cross-check |
| 30 | CryptoRank search | [link](https://cryptorank.io/price/bob-build-on-bitcoin) | Identity, market, social, contract, or discovery cross-check |
| 31 | RootData search | [link](https://www.rootdata.com/search?kw=BOB%20(Build%20on%20Bitcoin) | Identity, market, social, contract, or discovery cross-check |
| 32 | CoinDesk search | [link](https://www.coindesk.com/search?s=BOB%20(Build%20on%20Bitcoin) | Identity, market, social, contract, or discovery cross-check |
| 33 | The Block search | [link](https://www.theblock.co/search?query=BOB%20(Build%20on%20Bitcoin) | Identity, market, social, contract, or discovery cross-check |
| 34 | GitHub search | [link](https://github.com/search?q=BOB%20(Build%20on%20Bitcoin)&type=repositories) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.gobob.xyz/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=BOB%20(Build%20on%20Bitcoin)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=BOB%20(Build%20on%20Bitcoin)
- [DefiLlama protocol discovery](https://defillama.com/search?query=BOB%20(Build%20on%20Bitcoin)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=BOB%20(Build%20on%20Bitcoin)
- [Messari discovery](https://messari.io/search?q=BOB%20(Build%20on%20Bitcoin)
- [X profile](https://x.com/build_on_bob)
- [GitHub](https://github.com/bob-collective/bob)
- [Telegram](https://t.me/gobobxyz)
- [Discord](https://discord.com/invite/gobob)
- [BOB (Build on Bitcoin) Price Chart (BOB) - CoinGecko](https://www.coingecko.com/en/coins/bob-build-on-bitcoin)
- [BOB Token Price: BOB/USD Live Price Chart, Market ... - CoinGecko](https://www.coingecko.com/en/coins/bob-token)
- [Latest BOB (Build on Bitcoin) (BOB) Price Analysis - CoinMarketCap](https://coinmarketcap.com/cmc-ai/bob-build-on-bitcoin/price-analysis/)
- [Build On BNB Price Chart (BOB) - CoinGecko](https://www.coingecko.com/en/coins/build-on-bnb)
- [BOB (Build on Bitcoin) (BOB) Price Prediction - CoinMarketCap](https://coinmarketcap.com/cmc-ai/bob-build-on-bitcoin/price-prediction/)
- [ethereum BOB](https://etherscan.io/token/0xc9746f73cc33a36c2cd55b8aefd732586946cedd)
- [bsc BOB](https://bscscan.com/token/0x52b5fb4b0f6572b8c44d0251cc224513ac5eb7e7)
- [Google project search](https://www.google.com/search?q=BOB%20(Build%20on%20Bitcoin)%20BOB%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=BOB%20(Build%20on%20Bitcoin)%20BOB%20tokenomics)
- [Google docs search](https://www.google.com/search?q=BOB%20(Build%20on%20Bitcoin)%20docs)
- [Google governance search](https://www.google.com/search?q=BOB%20(Build%20on%20Bitcoin)%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=BOB%20(Build%20on%20Bitcoin)%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/bob-build-on-bitcoin/what-is/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/bob-build-on-bitcoin/)

## Final Investment View

BOB (Build on Bitcoin) should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
