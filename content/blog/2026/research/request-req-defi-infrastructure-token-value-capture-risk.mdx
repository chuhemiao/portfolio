---
title: 'Request / REQ: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Request is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'request-req-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# Request / REQ: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Request appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Ethereum, Polygon Pos; exchange exposure is BINANCE, COINBASE, BITHUMB; category tags are Developer Tooling, Payments & Gateway; current market cap is $40.49M; FDV is $54.36M; and 24h volume is $1.41M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Request before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 519, market cap $40,646,148, 24h volume $827,304. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Request is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether REQ captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Exchanges | BINANCE, COINBASE, BITHUMB |
| Price | $0.0544 |
| Market cap | $40.49M |
| FDV | $54.36M |
| 24h volume | $1.41M |
...
Base case: Request remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning REQ into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Request Network provides Web3 invoicing and payments infrastructure via a developer API, enabling multi‑chain payments, crypto‑to‑fiat settlement, automated reconciliation, and recurring/batch payment flows for businesses. REQ is the project’s utility token used for protocol fee mechanics and supporting network operations.

Surf's project profile maps Request to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://request.network/](https://request.network/) |
| X handle | [@RequestNetwork](https://x.com/RequestNetwork) |
| X followers | 56.67K |
| Tags | Developer Tooling, Payments & Gateway |
| Chains | Ethereum, Polygon Pos |
| Exchanges | BINANCE, COINBASE, BITHUMB |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Request is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DeFi infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Request convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Supply quality | 744.29M circulating vs 999.42M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Request is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| ethereum | REQ | 0x8f8221afbb33998d8584a2b05749ba73c37a938a | [explorer](https://etherscan.io/token/0x8f8221afbb33998d8584a2b05749ba73c37a938a) |
| polygon | REQ | 0xb25e20de2f2ebb4cffd4d16a55c7b395e8a94762 | [explorer](https://polygonscan.com/token/0xb25e20de2f2ebb4cffd4d16a55c7b395e8a94762) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Request, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 519, market cap $40,646,148, 24h volume $827,304. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $40.49M, FDV $54.36M, 24h volume $1.41M, and price $0.0544.
...
| Market cap | $40.49M | Liquid valuation proxy |
| FDV | $54.36M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.34x | Higher spread means more supply risk |
| X followers | 56.67K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Request | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether REQ is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Request currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 0.70% and 30d 1.37% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Request's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2017-10-13 | Ico | $33.60M | not disclosed |

| Team member | Role | Link |
|---|---|---|
| Christophe Lassuyt | Co-Founder, Board Member | [X](https://x.com/LassuytChristop) |
| Etienne Tatur | Co-Founder, CTO | not disclosed |
...
Request competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Request has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Request is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Request compounds usage, liquidity improves, and REQ gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Request has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If REQ does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
1. Re-run Surf project-detail and project-defi-metrics if Request announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Request has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Request, the first reconciliation job is identity. The token symbol is REQ, the category is DeFi infrastructure, chains are Ethereum, Polygon Pos, exchanges are BINANCE, COINBASE, BITHUMB, and tags are Developer Tooling, Payments & Gateway. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If REQ trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $40.49M, FDV is $54.36M, 24h volume is $1.41M, FDV / market cap is 1.34x, 7d price change is 0.70%, and 30d price change is 1.37%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Request / REQ | Match official docs, CG, CMC, Surf, contracts |
...
| Exchanges | BINANCE, COINBASE, BITHUMB | Check live order books and inactive pairs |
| Market cap | $40.49M | Compare Surf, CG, CMC, exchange pages |
| FDV | $54.36M | Reconcile total supply and unlock schedule |
...
For Request, the current liquidity read is watchlist first. The market cap is $40.49M, FDV is $54.36M, reported 24h volume is $1.41M, and FDV / market cap is 1.34x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Request gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://request.network/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Request) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Request) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Request) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Request) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Request) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/RequestNetwork) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/RequestNetwork/) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/requestnetwork) | Identity, market, social, contract, or discovery cross-check |
| 10 | Request Price: REQ/USD Live Price Chart, Market Cap ... | [link](https://www.coingecko.com/en/coins/request-network) | Identity, market, social, contract, or discovery cross-check |
...
| 12 | What Is Request (REQ) And How Does It Work? | [link](https://coinmarketcap.com/cmc-ai/request/what-is/) | Identity, market, social, contract, or discovery cross-check |
...
| 14 | Latest Request (REQ) Price Analysis | [link](https://coinmarketcap.com/cmc-ai/request/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 15 | ethereum REQ | [link](https://etherscan.io/token/0x8f8221afbb33998d8584a2b05749ba73c37a938a) | Identity, market, social, contract, or discovery cross-check |
| 16 | polygon REQ | [link](https://polygonscan.com/token/0xb25e20de2f2ebb4cffd4d16a55c7b395e8a94762) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google project search | [link](https://www.google.com/search?q=Request%20REQ%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google tokenomics search | [link](https://www.google.com/search?q=Request%20REQ%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google docs search | [link](https://www.google.com/search?q=Request%20docs) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google governance search | [link](https://www.google.com/search?q=Request%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google audit search | [link](https://www.google.com/search?q=Request%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/request/) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Request) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune token query | [link](https://dune.com/browse/queries?q=Request) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Request) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=REQ) | Identity, market, social, contract, or discovery cross-check |
| 27 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Request) | Identity, market, social, contract, or discovery cross-check |
| 28 | CryptoRank search | [link](https://cryptorank.io/price/request) | Identity, market, social, contract, or discovery cross-check |
| 29 | RootData search | [link](https://www.rootdata.com/search?kw=Request) | Identity, market, social, contract, or discovery cross-check |
| 30 | CoinDesk search | [link](https://www.coindesk.com/search?s=Request) | Identity, market, social, contract, or discovery cross-check |
| 31 | The Block search | [link](https://www.theblock.co/search?query=Request) | Identity, market, social, contract, or discovery cross-check |
| 32 | GitHub search | [link](https://github.com/search?q=Request&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 33 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=REQ) | Identity, market, social, contract, or discovery cross-check |
| 34 | BscScan token search | [link](https://bscscan.com/search?f=0&q=REQ) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://request.network/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Request)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Request)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Request)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Request)
- [Messari discovery](https://messari.io/search?q=Request)
- [X profile](https://x.com/RequestNetwork)
- [GitHub](https://github.com/RequestNetwork/)
- [Telegram](https://t.me/requestnetwork)
- [Request Price: REQ/USD Live Price Chart, Market Cap ...](https://www.coingecko.com/en/coins/request-network)
- [Crypto Data API: Most Comprehensive & Reliable ...](https://www.coingecko.com/en/api)
- [What Is Request (REQ) And How Does It Work?](https://coinmarketcap.com/cmc-ai/request/what-is/)
- [CoinGecko: Cryptocurrency Prices, Charts, and Crypto Market ...](https://www.coingecko.com/)
- [Latest Request (REQ) Price Analysis](https://coinmarketcap.com/cmc-ai/request/price-analysis/)
- [ethereum REQ](https://etherscan.io/token/0x8f8221afbb33998d8584a2b05749ba73c37a938a)
- [polygon REQ](https://polygonscan.com/token/0xb25e20de2f2ebb4cffd4d16a55c7b395e8a94762)
- [Google project search](https://www.google.com/search?q=Request%20REQ%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Request%20REQ%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Request%20docs)
- [Google governance search](https://www.google.com/search?q=Request%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Request%20audit%20report)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/request/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Request)
- [Dune token query](https://dune.com/browse/queries?q=Request)

## Final Investment View

Request should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
