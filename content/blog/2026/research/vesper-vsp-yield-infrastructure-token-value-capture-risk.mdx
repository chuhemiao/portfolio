---
title: 'Vesper / VSP: yield infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Vesper is treated as a yield infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'vesper-vsp-yield-infrastructure-token-value-capture-risk'
category: 'research'
---

# Vesper / VSP: yield infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Vesper appears in the local CoinGecko/CMC-oriented candidate pool through [CoinGecko](https://www.coingecko.com/en/coins/vesper-finance) and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Ethereum, Fantom, Polygon Pos; exchange exposure is not disclosed; category tags are Yield & Asset Mgmt; current market cap is $847.46K; FDV is $995.30K; and 24h volume is $87.9000. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Vesper before this file was created, and the existing candidate note says: DefiLlama category Yield Aggregator on Multi-Chain; TVL $40,938,252. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Vesper is best understood as a **yield infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether VSP captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Ethereum, Fantom, Polygon Pos |
| Exchanges | not disclosed |
| Price | $0.0995 |
| Market cap | $847.46K |
| FDV | $995.30K |
| 24h volume | $87.9000 |
| Circulating supply | 8.51M |
| Total supply | 10.00M |
...
Base case: Vesper remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning VSP into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Vesper is a DeFi yield platform offering audited, automated "Grow" pools and modular strategies that optimize passive returns across multiple EVM chains. Its VSP token enables governance and incentives (including a locked esVSP model), and the protocol emphasizes multi-chain deployment, composable strategies, and extensive security audits.

Surf's project profile maps Vesper to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://vesper.finance/](https://vesper.finance/) |
| X handle | [@VesperFi](https://x.com/VesperFi) |
| X followers | 49.46K |
| Tags | Yield & Asset Mgmt |
| Chains | Ethereum, Fantom, Polygon Pos |
| Exchanges | not disclosed |
| TGE status | not disclosed |
| CoinGecko ID | [vesper-finance](https://www.coingecko.com/en/coins/vesper-finance) |

The main product question is whether Vesper is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For yield infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Vesper convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $87.9000 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 8.51M circulating vs 10.00M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Vesper is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Ethereum, Fantom, Polygon Pos |
...
|---|---|---|---|
| fantom | VSP | 0x461d52769884ca6235b685ef2040f47d30c94eb5 | not mapped |
| ethereum | VSP | 0x1b40183efb4dd766f11bda7a7c3ad8982e998421 | [explorer](https://etherscan.io/token/0x1b40183efb4dd766f11bda7a7c3ad8982e998421) |
| polygon | VSP | 0x09c5a4bca808bd1ba2b8e6b3aaf7442046b4ca5b | [explorer](https://polygonscan.com/token/0x09c5a4bca808bd1ba2b8e6b3aaf7442046b4ca5b) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Vesper, the DeFi-oriented backlog note is the first signal: DefiLlama category Yield Aggregator on Multi-Chain; TVL $40,938,252. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $847.46K, FDV $995.30K, 24h volume $87.9000, and price $0.0995.
...
| 24h volume | $87.9000 | Position sizing and exit liquidity proxy |
| Market cap | $847.46K | Liquid valuation proxy |
| FDV | $995.30K | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.17x | Higher spread means more supply risk |
| X followers | 49.46K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Vesper | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether VSP is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Vesper currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 41.86% and 30d 33.33% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Vesper's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2021-02-05 | Strategic | $2.50M | Kenetic Capital, CMT Digital, Digital Strategies, TRGC, A195 Capital, Ryan Selkis, Meltem Demirors, Robert Leshner |

| Team member | Role | Link |
|---|---|---|
| Jeff Garzik | Co-Founder | [X](https://x.com/jgarzik) |
| Jordan Kruger | Cofounder, CEO | [X](https://x.com/JordanJKruger) |
| Matthew Roszak | Co-Founder | [X](https://x.com/MatthewRoszak) |
| Jon Greenwood | Growth Specialist | not disclosed |
...
Vesper competes in the yield infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Vesper has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Vesper is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Vesper compounds usage, liquidity improves, and VSP gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Vesper has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If VSP does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $87.9000 | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Vesper announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Vesper has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Vesper, the first reconciliation job is identity. The token symbol is VSP, the category is yield infrastructure, chains are Ethereum, Fantom, Polygon Pos, exchanges are not disclosed, and tags are Yield & Asset Mgmt. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If VSP trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $847.46K, FDV is $995.30K, 24h volume is $87.9000, FDV / market cap is 1.17x, 7d price change is 41.86%, and 30d price change is 33.33%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
For Vesper, the current liquidity read is watchlist first. The market cap is $847.46K, FDV is $995.30K, reported 24h volume is $87.9000, and FDV / market cap is 1.17x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Vesper gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://vesper.finance/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko profile | [link](https://www.coingecko.com/en/coins/vesper-finance) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Vesper) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Vesper) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Vesper) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Vesper) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/VesperFi) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/vesperfi) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/vesperfinance) | Identity, market, social, contract, or discovery cross-check |
| 10 | Vesper price VSP - CoinMarketCap | [link](https://coinmarketcap.com/currencies/vesper/) | Identity, market, social, contract, or discovery cross-check |
| 11 | VSP to PKR: Vesper Finance Price in Pakistani Rupee / CoinGecko | [link](https://www.coingecko.com/en/coins/vesper-finance/pkr) | Identity, market, social, contract, or discovery cross-check |
| 12 | VSP to TRY: Vesper Finance Price in Turkish Lira / CoinGecko | [link](https://www.coingecko.com/en/coins/vesper-finance/try) | Identity, market, social, contract, or discovery cross-check |
| 13 | Vesper Finance Price History: Download VSP Historical Data | [link](https://www.coingecko.com/en/coins/vesper-finance/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 14 | ethereum VSP | [link](https://etherscan.io/token/0x1b40183efb4dd766f11bda7a7c3ad8982e998421) | Identity, market, social, contract, or discovery cross-check |
| 15 | polygon VSP | [link](https://polygonscan.com/token/0x09c5a4bca808bd1ba2b8e6b3aaf7442046b4ca5b) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google project search | [link](https://www.google.com/search?q=Vesper%20VSP%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google tokenomics search | [link](https://www.google.com/search?q=Vesper%20VSP%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google docs search | [link](https://www.google.com/search?q=Vesper%20docs) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google governance search | [link](https://www.google.com/search?q=Vesper%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google audit search | [link](https://www.google.com/search?q=Vesper%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/vesper/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/vesper/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Vesper) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune token query | [link](https://dune.com/browse/queries?q=Vesper) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Vesper) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=VSP) | Identity, market, social, contract, or discovery cross-check |
| 27 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Vesper) | Identity, market, social, contract, or discovery cross-check |
| 28 | CryptoRank search | [link](https://cryptorank.io/price/vesper) | Identity, market, social, contract, or discovery cross-check |
| 29 | RootData search | [link](https://www.rootdata.com/search?kw=Vesper) | Identity, market, social, contract, or discovery cross-check |
| 30 | CoinDesk search | [link](https://www.coindesk.com/search?s=Vesper) | Identity, market, social, contract, or discovery cross-check |
| 31 | The Block search | [link](https://www.theblock.co/search?query=Vesper) | Identity, market, social, contract, or discovery cross-check |
| 32 | GitHub search | [link](https://github.com/search?q=Vesper&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 33 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=VSP) | Identity, market, social, contract, or discovery cross-check |
| 34 | BscScan token search | [link](https://bscscan.com/search?f=0&q=VSP) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://vesper.finance/)
- [CoinGecko profile](https://www.coingecko.com/en/coins/vesper-finance)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Vesper)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Vesper)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Vesper)
- [Messari discovery](https://messari.io/search?q=Vesper)
- [X profile](https://x.com/VesperFi)
- [GitHub](https://github.com/vesperfi)
- [Telegram](https://t.me/vesperfinance)
- [Vesper price VSP - CoinMarketCap](https://coinmarketcap.com/currencies/vesper/)
- [VSP to PKR: Vesper Finance Price in Pakistani Rupee | CoinGecko](https://www.coingecko.com/en/coins/vesper-finance/pkr)
- [VSP to TRY: Vesper Finance Price in Turkish Lira | CoinGecko](https://www.coingecko.com/en/coins/vesper-finance/try)
- [Vesper Finance Price History: Download VSP Historical Data](https://www.coingecko.com/en/coins/vesper-finance/historical_data)
- [ethereum VSP](https://etherscan.io/token/0x1b40183efb4dd766f11bda7a7c3ad8982e998421)
- [polygon VSP](https://polygonscan.com/token/0x09c5a4bca808bd1ba2b8e6b3aaf7442046b4ca5b)
- [Google project search](https://www.google.com/search?q=Vesper%20VSP%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Vesper%20VSP%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Vesper%20docs)
- [Google governance search](https://www.google.com/search?q=Vesper%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Vesper%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/vesper/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/vesper/price-analysis/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Vesper)
- [Dune token query](https://dune.com/browse/queries?q=Vesper)

## Final Investment View

Vesper should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
