---
title: 'MBG By Multibank Group / MBG: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'MBG By Multibank Group is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'mbg-by-multibank-group-mbg-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# MBG By Multibank Group / MBG: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** MBG By Multibank Group appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Ethereum; exchange exposure is not disclosed; category tags are Exchange (CEX); current market cap is $39.38M; FDV is $155.18M; and 24h volume is $3.55M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag MBG By Multibank Group before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 527, market cap $39,621,548, 24h volume $1,023,649. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

MBG By Multibank Group is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether MBG captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.

| Metric | Current read |
|---|---|
| Project | MBG By Multibank Group |
| Token | MBG |
...
| FDV | $155.18M |
| 24h volume | $3.55M |
| Circulating supply | 248.09M |
| Total supply | 977.51M |
...
Base case: MBG By Multibank Group remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning MBG into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

MultiBank Group is a regulated global financial conglomerate operating TradFi brokerage, the MultiBank.io crypto exchange, and the MEX institutional exchange. Its MBG token is the ecosystem utility token intended for fee discounts, staking, real‑world‑asset access, and other on‑platform utilities across MultiBank’s services.

Surf's project profile maps MBG By Multibank Group to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://token.multibankgroup.com/mbg?utm_source=direct_buys&utm_campaign=MBIO_Global_IC_LeadGen_Web_TGE_Jul-25&utm_medium=CRYPTORANK_MBGLISTINGPAGE_TGE&utm_term=mbio_token_TGE&utm_content=EN](https://token.multibankgroup.com/mbg?utm_source=direct_buys&utm_campaign=MBIO_Global_IC_LeadGen_Web_TGE_Jul-25&utm_medium=CRYPTORANK_MBGLISTINGPAGE_TGE&utm_term=mbio_token_TGE&utm_content=EN) |
| X handle | [@multibankgroup](https://x.com/multibankgroup) |
| X followers | 22.94K |
| Tags | Exchange (CEX) |
| Chains | Ethereum |
| Exchanges | not disclosed |
...
| Liquidity | $3.55M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 248.09M circulating vs 977.51M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that MBG By Multibank Group is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| ethereum | MBG | 0x45e02bc2875a2914c4f585bbf92a6f28bc07cb70 | [explorer](https://etherscan.io/token/0x45e02bc2875a2914c4f585bbf92a6f28bc07cb70) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For MBG By Multibank Group, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 527, market cap $39,621,548, 24h volume $1,023,649. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $39.38M, FDV $155.18M, 24h volume $3.55M, and price $0.1587.
...
| 24h volume | $3.55M | Position sizing and exit liquidity proxy |
| Market cap | $39.38M | Liquid valuation proxy |
| FDV | $155.18M | Dilution-aware valuation proxy |
| FDV / market cap spread | 3.94x | Higher spread means more supply risk |
| X followers | 22.94K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for MBG By Multibank Group | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether MBG is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. MBG By Multibank Group currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d -7.44% and 30d -44.69% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that MBG By Multibank Group's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-07-20 | Pre-Sale | $1.05M | not disclosed |
| 2025-07-15 | Pre-Sale | $2.45M | not disclosed |

| Team member | Role | Link |
|---|---|---|
| Naser Taher | Founder | not disclosed |
| Zak Taher | CEO | [X](https://x.com/Zak_MBG) |
...
MBG By Multibank Group competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether MBG By Multibank Group has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: MBG By Multibank Group is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | MBG By Multibank Group compounds usage, liquidity improves, and MBG gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If MBG does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $3.55M | Weekly | Volume below position-size threshold |
| FDV / market cap | 3.94x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if MBG By Multibank Group announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. MBG By Multibank Group has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For MBG By Multibank Group, the first reconciliation job is identity. The token symbol is MBG, the category is DeFi infrastructure, chains are Ethereum, exchanges are not disclosed, and tags are Exchange (CEX). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If MBG trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $39.38M, FDV is $155.18M, 24h volume is $3.55M, FDV / market cap is 3.94x, 7d price change is -7.44%, and 30d price change is -44.69%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | MBG By Multibank Group / MBG | Match official docs, CG, CMC, Surf, contracts |
...
For MBG By Multibank Group, the current liquidity read is watchlist first. The market cap is $39.38M, FDV is $155.18M, reported 24h volume is $3.55M, and FDV / market cap is 3.94x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. MBG By Multibank Group can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.

For MBG By Multibank Group, the live monitoring stack should include contract events, TVL, volume, governance, documentation changes, social announcements, security disclosures, and exchange-market quality. A single metric is not enough. TVL may stay high while liquidity thins. Volume may rise because of volatility rather than adoption. Social attention may rise around controversy rather than progress. Governance may pass proposals that improve short-term price but add long-term risk. The strongest monitoring dashboard combines on-chain state, off-chain disclosure, and market quality.

Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If MBG By Multibank Group suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | MBG By Multibank Group gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://token.multibankgroup.com/mbg?utm_source=direct_buys&utm_campaign=MBIO_Global_IC_LeadGen_Web_TGE_Jul-25&utm_medium=CRYPTORANK_MBGLISTINGPAGE_TGE&utm_term=mbio_token_TGE&utm_content=EN) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=MBG%20By%20Multibank%20Group) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=MBG%20By%20Multibank%20Group) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=MBG%20By%20Multibank%20Group) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=MBG%20By%20Multibank%20Group) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=MBG%20By%20Multibank%20Group) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/multibankgroup) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/MultiBank_io) | Identity, market, social, contract, or discovery cross-check |
| 9 | MBG By Multibank Group Price | [link](https://www.coingecko.com/en/coins/mbg-by-multibank-group) | Identity, market, social, contract, or discovery cross-check |
| 10 | What Is MultiBank Group (MBG) And How Does It Work? | [link](https://coinmarketcap.com/cmc-ai/multibank-group/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 11 | MultiBank Group (MBG) Price Prediction For 2026 & Beyond | [link](https://coinmarketcap.com/cmc-ai/multibank-group/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 12 | Introducing the $MBG Token / MultiBank Group | [link](https://token.multibankgroup.com/) | Identity, market, social, contract, or discovery cross-check |
| 13 | Latest MultiBank Group (MBG) News Update | [link](https://coinmarketcap.com/cmc-ai/multibank-group/latest-updates/) | Identity, market, social, contract, or discovery cross-check |
| 14 | ethereum MBG | [link](https://etherscan.io/token/0x45e02bc2875a2914c4f585bbf92a6f28bc07cb70) | Identity, market, social, contract, or discovery cross-check |
| 15 | Google project search | [link](https://www.google.com/search?q=MBG%20By%20Multibank%20Group%20MBG%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google tokenomics search | [link](https://www.google.com/search?q=MBG%20By%20Multibank%20Group%20MBG%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google docs search | [link](https://www.google.com/search?q=MBG%20By%20Multibank%20Group%20docs) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google governance search | [link](https://www.google.com/search?q=MBG%20By%20Multibank%20Group%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google audit search | [link](https://www.google.com/search?q=MBG%20By%20Multibank%20Group%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 20 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/mbg-by-multibank-group/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/mbg-by-multibank-group/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/mbg-by-multibank-group/) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune discovery | [link](https://dune.com/browse/dashboards?q=MBG%20By%20Multibank%20Group) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune token query | [link](https://dune.com/browse/queries?q=MBG%20By%20Multibank%20Group) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=MBG%20By%20Multibank%20Group) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=MBG) | Identity, market, social, contract, or discovery cross-check |
| 27 | TokenUnlocks search | [link](https://token.unlocks.app/?search=MBG%20By%20Multibank%20Group) | Identity, market, social, contract, or discovery cross-check |
| 28 | CryptoRank search | [link](https://cryptorank.io/price/mbg-by-multibank-group) | Identity, market, social, contract, or discovery cross-check |
| 29 | RootData search | [link](https://www.rootdata.com/search?kw=MBG%20By%20Multibank%20Group) | Identity, market, social, contract, or discovery cross-check |
| 30 | CoinDesk search | [link](https://www.coindesk.com/search?s=MBG%20By%20Multibank%20Group) | Identity, market, social, contract, or discovery cross-check |
| 31 | The Block search | [link](https://www.theblock.co/search?query=MBG%20By%20Multibank%20Group) | Identity, market, social, contract, or discovery cross-check |
| 32 | GitHub search | [link](https://github.com/search?q=MBG%20By%20Multibank%20Group&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 33 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=MBG) | Identity, market, social, contract, or discovery cross-check |
| 34 | BscScan token search | [link](https://bscscan.com/search?f=0&q=MBG) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://token.multibankgroup.com/mbg?utm_source=direct_buys&utm_campaign=MBIO_Global_IC_LeadGen_Web_TGE_Jul-25&utm_medium=CRYPTORANK_MBGLISTINGPAGE_TGE&utm_term=mbio_token_TGE&utm_content=EN)
- [CoinGecko search](https://www.coingecko.com/en/search?query=MBG%20By%20Multibank%20Group)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=MBG%20By%20Multibank%20Group)
- [DefiLlama protocol discovery](https://defillama.com/search?query=MBG%20By%20Multibank%20Group)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=MBG%20By%20Multibank%20Group)
- [Messari discovery](https://messari.io/search?q=MBG%20By%20Multibank%20Group)
- [X profile](https://x.com/multibankgroup)
- [Telegram](https://t.me/MultiBank_io)
- [MBG By Multibank Group Price](https://www.coingecko.com/en/coins/mbg-by-multibank-group)
- [What Is MultiBank Group (MBG) And How Does It Work?](https://coinmarketcap.com/cmc-ai/multibank-group/what-is/)
- [MultiBank Group (MBG) Price Prediction For 2026 & Beyond](https://coinmarketcap.com/cmc-ai/multibank-group/price-prediction/)
- [Introducing the $MBG Token | MultiBank Group](https://token.multibankgroup.com/)
- [Latest MultiBank Group (MBG) News Update](https://coinmarketcap.com/cmc-ai/multibank-group/latest-updates/)
- [ethereum MBG](https://etherscan.io/token/0x45e02bc2875a2914c4f585bbf92a6f28bc07cb70)
- [Google project search](https://www.google.com/search?q=MBG%20By%20Multibank%20Group%20MBG%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=MBG%20By%20Multibank%20Group%20MBG%20tokenomics)
- [Google docs search](https://www.google.com/search?q=MBG%20By%20Multibank%20Group%20docs)
- [Google governance search](https://www.google.com/search?q=MBG%20By%20Multibank%20Group%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=MBG%20By%20Multibank%20Group%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/mbg-by-multibank-group/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/mbg-by-multibank-group/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/mbg-by-multibank-group/)
- [Dune discovery](https://dune.com/browse/dashboards?q=MBG%20By%20Multibank%20Group)
- [Dune token query](https://dune.com/browse/queries?q=MBG%20By%20Multibank%20Group)

## Final Investment View

MBG By Multibank Group should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
