---
title: 'ConstitutionDAO / PEOPLE: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'ConstitutionDAO is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'constitutiondao-people-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# ConstitutionDAO / PEOPLE: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** ConstitutionDAO appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Ethereum; exchange exposure is BINANCE, OKX, BYBIT, BITGET; category tags are Meme; current market cap is $27.48M; FDV is $27.48M; and 24h volume is $3.82M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag ConstitutionDAO before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 691, market cap $26,913,292, 24h volume $3,506,286. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

ConstitutionDAO is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether PEOPLE captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.

| Metric | Current read |
|---|---|
| Project | ConstitutionDAO |
| Token | PEOPLE |
...
Base case: ConstitutionDAO remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning PEOPLE into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

ConstitutionDAO was a 2021 single‑purpose DAO that crowdfunded via PEOPLE tokens to attempt to buy an original U.S. Constitution copy. After raising millions and losing the Sotheby’s auction, the core team wound down, issued refunds, and PEOPLE tokens remain tradable with ongoing community activity.

Surf's project profile maps ConstitutionDAO to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.constitutiondao.com/](https://www.constitutiondao.com/) |
| X handle | [@ConstitutionDAO](https://x.com/ConstitutionDAO) |
| X followers | 72.94K |
| Tags | Meme |
| Chains | Ethereum |
| Exchanges | BINANCE, OKX, BYBIT, BITGET |
...
| Liquidity | $3.82M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 5.07B circulating vs 5.07B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that ConstitutionDAO is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

ConstitutionDAO's architecture should be evaluated through the job it performs. For a DeFi infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
|---|---|---|---|
| ethereum | PEOPLE | 0x7a58c0be72be218b41c608b7fe7c5bb630736c71 | [explorer](https://etherscan.io/token/0x7a58c0be72be218b41c608b7fe7c5bb630736c71) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For ConstitutionDAO, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 691, market cap $26,913,292, 24h volume $3,506,286. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $27.48M, FDV $27.48M, 24h volume $3.82M, and price $0.0054.
...
| 24h volume | $3.82M | Position sizing and exit liquidity proxy |
| Market cap | $27.48M | Liquid valuation proxy |
| FDV | $27.48M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.00x | Higher spread means more supply risk |
| X followers | 72.94K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for ConstitutionDAO | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether PEOPLE is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. ConstitutionDAO currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 8.00% and 30d 3.70% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that ConstitutionDAO's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
ConstitutionDAO competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether ConstitutionDAO has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: ConstitutionDAO is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | ConstitutionDAO compounds usage, liquidity improves, and PEOPLE gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If PEOPLE does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $3.82M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if ConstitutionDAO announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. ConstitutionDAO has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For ConstitutionDAO, the first reconciliation job is identity. The token symbol is PEOPLE, the category is DeFi infrastructure, chains are Ethereum, exchanges are BINANCE, OKX, BYBIT, BITGET, and tags are Meme. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If PEOPLE trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $27.48M, FDV is $27.48M, 24h volume is $3.82M, FDV / market cap is 1.00x, 7d price change is 8.00%, and 30d price change is 3.70%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | ConstitutionDAO / PEOPLE | Match official docs, CG, CMC, Surf, contracts |
...
For ConstitutionDAO, the current liquidity read is watchlist first. The market cap is $27.48M, FDV is $27.48M, reported 24h volume is $3.82M, and FDV / market cap is 1.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. ConstitutionDAO can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.

For ConstitutionDAO, the live monitoring stack should include contract events, TVL, volume, governance, documentation changes, social announcements, security disclosures, and exchange-market quality. A single metric is not enough. TVL may stay high while liquidity thins. Volume may rise because of volatility rather than adoption. Social attention may rise around controversy rather than progress. Governance may pass proposals that improve short-term price but add long-term risk. The strongest monitoring dashboard combines on-chain state, off-chain disclosure, and market quality.

Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If ConstitutionDAO suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | ConstitutionDAO gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.constitutiondao.com/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=ConstitutionDAO) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=ConstitutionDAO) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=ConstitutionDAO) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=ConstitutionDAO) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=ConstitutionDAO) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/ConstitutionDAO) | Identity, market, social, contract, or discovery cross-check |
| 8 | Discord | [link](https://discord.com/invite/p2jdESeVfy) | Identity, market, social, contract, or discovery cross-check |
| 9 | ConstitutionDAO Price Chart (PEOPLE) | [link](https://www.coingecko.com/en/coins/constitutiondao) | Identity, market, social, contract, or discovery cross-check |
| 10 | PEOPLE to ETH: ConstitutionDAO Price in Ether | [link](https://www.coingecko.com/en/coins/constitutiondao/eth) | Identity, market, social, contract, or discovery cross-check |
| 11 | PEOPLE to BTC: ConstitutionDAO Price in Bitcoin | [link](https://www.coingecko.com/en/coins/constitutiondao/btc) | Identity, market, social, contract, or discovery cross-check |
| 12 | ConstitutionDAO (PEOPLE) Price Prediction | [link](https://coinmarketcap.com/cmc-ai/constitutiondao/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 13 | ConstitutionDAO price today, PEOPLE to USD live ... | [link](https://coinmarketcap.com/currencies/constitutiondao/) | Identity, market, social, contract, or discovery cross-check |
| 14 | ethereum PEOPLE | [link](https://etherscan.io/token/0x7a58c0be72be218b41c608b7fe7c5bb630736c71) | Identity, market, social, contract, or discovery cross-check |
| 15 | Google project search | [link](https://www.google.com/search?q=ConstitutionDAO%20PEOPLE%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google tokenomics search | [link](https://www.google.com/search?q=ConstitutionDAO%20PEOPLE%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google docs search | [link](https://www.google.com/search?q=ConstitutionDAO%20docs) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google governance search | [link](https://www.google.com/search?q=ConstitutionDAO%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google audit search | [link](https://www.google.com/search?q=ConstitutionDAO%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 20 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/constitutiondao/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/constitutiondao/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 22 | Dune discovery | [link](https://dune.com/browse/dashboards?q=ConstitutionDAO) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune token query | [link](https://dune.com/browse/queries?q=ConstitutionDAO) | Identity, market, social, contract, or discovery cross-check |
| 24 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=ConstitutionDAO) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=PEOPLE) | Identity, market, social, contract, or discovery cross-check |
| 26 | TokenUnlocks search | [link](https://token.unlocks.app/?search=ConstitutionDAO) | Identity, market, social, contract, or discovery cross-check |
| 27 | CryptoRank search | [link](https://cryptorank.io/price/constitutiondao) | Identity, market, social, contract, or discovery cross-check |
| 28 | RootData search | [link](https://www.rootdata.com/search?kw=ConstitutionDAO) | Identity, market, social, contract, or discovery cross-check |
| 29 | CoinDesk search | [link](https://www.coindesk.com/search?s=ConstitutionDAO) | Identity, market, social, contract, or discovery cross-check |
| 30 | The Block search | [link](https://www.theblock.co/search?query=ConstitutionDAO) | Identity, market, social, contract, or discovery cross-check |
| 31 | GitHub search | [link](https://github.com/search?q=ConstitutionDAO&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 32 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=PEOPLE) | Identity, market, social, contract, or discovery cross-check |
| 33 | BscScan token search | [link](https://bscscan.com/search?f=0&q=PEOPLE) | Identity, market, social, contract, or discovery cross-check |
| 34 | Arbiscan token search | [link](https://arbiscan.io/search?f=0&q=PEOPLE) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.constitutiondao.com/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=ConstitutionDAO)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=ConstitutionDAO)
- [DefiLlama protocol discovery](https://defillama.com/search?query=ConstitutionDAO)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=ConstitutionDAO)
- [Messari discovery](https://messari.io/search?q=ConstitutionDAO)
- [X profile](https://x.com/ConstitutionDAO)
- [Discord](https://discord.com/invite/p2jdESeVfy)
- [ConstitutionDAO Price Chart (PEOPLE)](https://www.coingecko.com/en/coins/constitutiondao)
- [PEOPLE to ETH: ConstitutionDAO Price in Ether](https://www.coingecko.com/en/coins/constitutiondao/eth)
- [PEOPLE to BTC: ConstitutionDAO Price in Bitcoin](https://www.coingecko.com/en/coins/constitutiondao/btc)
- [ConstitutionDAO (PEOPLE) Price Prediction](https://coinmarketcap.com/cmc-ai/constitutiondao/price-prediction/)
- [ConstitutionDAO price today, PEOPLE to USD live ...](https://coinmarketcap.com/currencies/constitutiondao/)
- [ethereum PEOPLE](https://etherscan.io/token/0x7a58c0be72be218b41c608b7fe7c5bb630736c71)
- [Google project search](https://www.google.com/search?q=ConstitutionDAO%20PEOPLE%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=ConstitutionDAO%20PEOPLE%20tokenomics)
- [Google docs search](https://www.google.com/search?q=ConstitutionDAO%20docs)
- [Google governance search](https://www.google.com/search?q=ConstitutionDAO%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=ConstitutionDAO%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/constitutiondao/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/constitutiondao/price-analysis/)
- [Dune discovery](https://dune.com/browse/dashboards?q=ConstitutionDAO)
- [Dune token query](https://dune.com/browse/queries?q=ConstitutionDAO)
- [DeFiLlama fees discovery](https://defillama.com/fees?chain=All&search=ConstitutionDAO)
