---
title: 'Threshold Network / T: privacy infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Threshold Network is treated as a privacy infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'threshold-network-t-privacy-infrastructure-token-value-capture-risk'
category: 'research'
---

# Threshold Network / T: privacy infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Threshold Network appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Arbitrum One, Base, Bob Network, Ethereum, Hydration, Optimistic Ethereum, Polygon Pos, Solana, Starknet, Sui; exchange exposure is BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BITGET; category tags are Cross-Chain & Bridge, Privacy & ZK; current market cap is $39.71M; FDV is $39.71M; and 24h volume is $4.38M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Threshold Network before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 535, market cap $39,048,924, 24h volume $4,531,510. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Threshold Network is best understood as a **privacy infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether T captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Arbitrum One, Base, Bob Network, Ethereum, Hydration, Optimistic Ethereum, Polygon Pos, Solana, Starknet, Sui |
| Exchanges | BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BITGET |
| Price | $0.0036 |
| Market cap | $39.71M |
| FDV | $39.71M |
| 24h volume | $4.38M |
| Circulating supply | 11.15B |
| Total supply | 11.15B |
...
Base case: Threshold Network remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning T into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Threshold is a decentralized threshold-cryptography network powering tBTC, a trust-minimized bridge that brings native Bitcoin on-chain for permissionless DeFi across multiple chains. The T token captures protocol value and governance rights in the Threshold DAO, while the network provides cryptographic primitives (proxy re-encryption, threshold signatures, DKG, random beacon) and developer tooling.

Surf's project profile maps Threshold Network to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://threshold.network/](https://threshold.network/) |
| X handle | [@TheTNetwork](https://x.com/TheTNetwork) |
| X followers | 45.38K |
| Tags | Cross-Chain & Bridge, Privacy & ZK |
| Chains | Arbitrum One, Base, Bob Network, Ethereum, Hydration, Optimistic Ethereum, Polygon Pos, Solana, Starknet, Sui |
| Exchanges | BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Threshold Network is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For privacy infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Threshold Network convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $4.38M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 11.15B circulating vs 11.15B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Threshold Network is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

Threshold Network's architecture should be evaluated through the job it performs. For a privacy infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
| Settlement layer | Chains and contracts that hold user value | Arbitrum One, Base, Bob Network, Ethereum, Hydration, Optimistic Ethereum, Polygon Pos, Solana, Starknet, Sui |
...
|---|---|---|---|
| base | T | 0x26f3901ac8a79c50fb0d8289c74f0d09adc42e29 | [explorer](https://basescan.org/token/0x26f3901ac8a79c50fb0d8289c74f0d09adc42e29) |
| solana | T | 4Njvi3928U3figEF5tf8xvjLC5GqUN33oe4XTJNe7xXC | [explorer](https://solscan.io/token/4Njvi3928U3figEF5tf8xvjLC5GqUN33oe4XTJNe7xXC) |
| ethereum | T | 0xcdf7028ceab81fa0c6971208e83fa7872994bee5 | [explorer](https://etherscan.io/token/0xcdf7028ceab81fa0c6971208e83fa7872994bee5) |
| optimism | T | 0x747e42eb0591547a0ab429b3627816208c734ea7 | [explorer](https://optimistic.etherscan.io/token/0x747e42eb0591547a0ab429b3627816208c734ea7) |
| arbitrum | TBTC | 0x6c84a8f1c29108f47a79964b5fe888d4f4d0de40 | [explorer](https://arbiscan.io/token/0x6c84a8f1c29108f47a79964b5fe888d4f4d0de40) |
| Sui | TBTC | 0x77045f1b9f811a7a8fb9ebd085b5b0c55c5cb0d1520ff55f7037f89b5da9f5f1::TBTC::TBTC | not mapped |
| base | TBTC | 0x236aa50979d5f3de3bd1eeb40e81137f22ab794b | [explorer](https://basescan.org/token/0x236aa50979d5f3de3bd1eeb40e81137f22ab794b) |
| solana | TBTC | 6DNSN2BJsaPFdFFc1zP37kkeNe4Usc1Sqkzr9C9vPWcU | [explorer](https://solscan.io/token/6DNSN2BJsaPFdFFc1zP37kkeNe4Usc1Sqkzr9C9vPWcU) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Threshold Network, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 535, market cap $39,048,924, 24h volume $4,531,510. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $39.71M, FDV $39.71M, 24h volume $4.38M, and price $0.0036.
...
| 24h volume | $4.38M | Position sizing and exit liquidity proxy |
| Market cap | $39.71M | Liquid valuation proxy |
| FDV | $39.71M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.00x | Higher spread means more supply risk |
| X followers | 45.38K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Threshold Network | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether T is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. Threshold Network should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that Threshold Network belongs to the privacy infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $39.71M, FDV $39.71M, 24h volume $4.38M, price $0.0036, circulating supply 11.15B, and total supply 11.15B. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For Threshold Network, the candidate backlog records Seeded 2026-07-06 from Surf market-ranking; rank 535, market cap $39,048,924, 24h volume $4,531,510. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Arbitrum One, Base, Bob Network, Ethereum, Hydration, Optimistic Ethereum, Polygon Pos, Solana, Starknet, Sui, exchanges as BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BITGET, and tags as Cross-Chain & Bridge, Privacy & ZK. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.

The fourth frame is value capture. A privacy infrastructure asset can be useful while the token remains weak. The memo therefore separates product-market fit from token-market fit. Product-market fit asks whether users need the protocol even when incentives fade. Token-market fit asks whether the token absorbs value through staking, fee routing, collateral, governance constraints, emissions control, slashing insurance, or privileged access. If the token is mostly governance, the base case assigns lower confidence. If the token is used for security or fee capture but the mechanism is not yet activated, the base case treats value capture as an option rather than as current cash flow.

The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Threshold Network currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 3.19% and 30d -5.66% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Threshold Network's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|
| MacLane Wilkison | Co-Founder | [X](https://x.com/MacLaneWilkison) |
| Callan Sarre | Co-Founder | [X](https://x.com/saprophetik) |
| John Packel | Head of Operations | not disclosed |
| Ethan Hassall | Head of BD | not disclosed |
| Rizza Carla R. | Head of Marketing | not disclosed |
...
Threshold Network competes in the privacy infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Threshold Network has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Threshold Network is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Threshold Network compounds usage, liquidity improves, and T gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If T does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $4.38M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Threshold Network announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Threshold Network has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Threshold Network, the first reconciliation job is identity. The token symbol is T, the category is privacy infrastructure, chains are Arbitrum One, Base, Bob Network, Ethereum, Hydration, Optimistic Ethereum, Polygon Pos, Solana, Starknet, Sui, exchanges are BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BITGET, and tags are Cross-Chain & Bridge, Privacy & ZK. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If T trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $39.71M, FDV is $39.71M, 24h volume is $4.38M, FDV / market cap is 1.00x, 7d price change is 3.19%, and 30d price change is -5.66%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.

The third reconciliation job is usage data. The latest TVL sample is not disclosed, TVL sample change is not disclosed, and the sample contains 0 point(s). TVL is useful only when the underlying balances are real, withdrawable, and not double-counted. For privacy infrastructure, the memo should identify whether TVL means deposits, staked assets, LP liquidity, stablecoin supply, collateral, locked governance assets, or a derived wrapper. A yield-bearing stablecoin and a validator middleware protocol can both show TVL, but the risk encoded by that TVL is completely different.

| Reconciliation item | Current value | Next check |
|---|---|---|
| Identity | Threshold Network / T | Match official docs, CG, CMC, Surf, contracts |
| Category | privacy infrastructure | Confirm product mechanics and revenue model |
| Chains | Arbitrum One, Base, Bob Network, Ethereum, Hydration, Optimistic Ethereum, Polygon Pos, Solana, Starknet, Sui | Confirm canonical deployments and bridge assumptions |
| Exchanges | BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BITGET | Check live order books and inactive pairs |
| Market cap | $39.71M | Compare Surf, CG, CMC, exchange pages |
| FDV | $39.71M | Reconcile total supply and unlock schedule |
| Volume | $4.38M | Check exchange concentration and wash-trading risk |
| TVL | not disclosed | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Threshold Network does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Threshold Network, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does T sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
| Correlation check | Count exposure to related privacy infrastructure assets | Category beta can hide concentration |
...
The failure-mode view is deliberately harsher than the base case. Threshold Network can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.

For Threshold Network, the live monitoring stack should include contract events, TVL, volume, governance, documentation changes, social announcements, security disclosures, and exchange-market quality. A single metric is not enough. TVL may stay high while liquidity thins. Volume may rise because of volatility rather than adoption. Social attention may rise around controversy rather than progress. Governance may pass proposals that improve short-term price but add long-term risk. The strongest monitoring dashboard combines on-chain state, off-chain disclosure, and market quality.

Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If Threshold Network suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | Threshold Network gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://threshold.network/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Threshold%20Network) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Threshold%20Network) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Threshold%20Network) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Threshold%20Network) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Threshold%20Network) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/TheTNetwork) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/threshold-network) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/thresholdnetwork) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](https://discord.gg/threshold) | Identity, market, social, contract, or discovery cross-check |
| 11 | Threshold Network Price: T/USD Live Price Chart, Market ... | [link](https://www.coingecko.com/en/coins/threshold-network-token) | Identity, market, social, contract, or discovery cross-check |
| 12 | CoinGecko: Cryptocurrency Prices, Charts, and Crypto Market ... | [link](https://www.coingecko.com/) | Identity, market, social, contract, or discovery cross-check |
| 13 | What Is Threshold (T) And How Does It Work? | [link](https://coinmarketcap.com/cmc-ai/threshold/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 14 | Threshold Network Price History: Download T Historical Data | [link](https://www.coingecko.com/en/coins/threshold-network-token/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 15 | Threshold Network: The Standard for Bitcoin Finance | [link](https://www.threshold.network/) | Identity, market, social, contract, or discovery cross-check |
| 16 | base T | [link](https://basescan.org/token/0x26f3901ac8a79c50fb0d8289c74f0d09adc42e29) | Identity, market, social, contract, or discovery cross-check |
| 17 | solana T | [link](https://solscan.io/token/4Njvi3928U3figEF5tf8xvjLC5GqUN33oe4XTJNe7xXC) | Identity, market, social, contract, or discovery cross-check |
| 18 | ethereum T | [link](https://etherscan.io/token/0xcdf7028ceab81fa0c6971208e83fa7872994bee5) | Identity, market, social, contract, or discovery cross-check |
| 19 | optimism T | [link](https://optimistic.etherscan.io/token/0x747e42eb0591547a0ab429b3627816208c734ea7) | Identity, market, social, contract, or discovery cross-check |
| 20 | arbitrum TBTC | [link](https://arbiscan.io/token/0x6c84a8f1c29108f47a79964b5fe888d4f4d0de40) | Identity, market, social, contract, or discovery cross-check |
| 21 | base TBTC | [link](https://basescan.org/token/0x236aa50979d5f3de3bd1eeb40e81137f22ab794b) | Identity, market, social, contract, or discovery cross-check |
| 22 | solana TBTC | [link](https://solscan.io/token/6DNSN2BJsaPFdFFc1zP37kkeNe4Usc1Sqkzr9C9vPWcU) | Identity, market, social, contract, or discovery cross-check |
| 23 | ethereum TBTC | [link](https://etherscan.io/token/0x18084fba666a33d37592fa2633fd49a74dd93a88) | Identity, market, social, contract, or discovery cross-check |
| 24 | Google project search | [link](https://www.google.com/search?q=Threshold%20Network%20T%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 25 | Google tokenomics search | [link](https://www.google.com/search?q=Threshold%20Network%20T%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 26 | Google docs search | [link](https://www.google.com/search?q=Threshold%20Network%20docs) | Identity, market, social, contract, or discovery cross-check |
| 27 | Google governance search | [link](https://www.google.com/search?q=Threshold%20Network%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 28 | Google audit search | [link](https://www.google.com/search?q=Threshold%20Network%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 29 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/threshold-network/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 30 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/threshold-network/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 31 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/threshold-network/) | Identity, market, social, contract, or discovery cross-check |
| 32 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Threshold%20Network) | Identity, market, social, contract, or discovery cross-check |
| 33 | Dune token query | [link](https://dune.com/browse/queries?q=Threshold%20Network) | Identity, market, social, contract, or discovery cross-check |
| 34 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Threshold%20Network) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://threshold.network/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Threshold%20Network)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Threshold%20Network)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Threshold%20Network)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Threshold%20Network)
- [Messari discovery](https://messari.io/search?q=Threshold%20Network)
- [X profile](https://x.com/TheTNetwork)
- [GitHub](https://github.com/threshold-network)
- [Telegram](https://t.me/thresholdnetwork)
- [Discord](https://discord.gg/threshold)
- [Threshold Network Price: T/USD Live Price Chart, Market ...](https://www.coingecko.com/en/coins/threshold-network-token)
- [CoinGecko: Cryptocurrency Prices, Charts, and Crypto Market ...](https://www.coingecko.com/)
- [What Is Threshold (T) And How Does It Work?](https://coinmarketcap.com/cmc-ai/threshold/what-is/)
- [Threshold Network Price History: Download T Historical Data](https://www.coingecko.com/en/coins/threshold-network-token/historical_data)
- [Threshold Network: The Standard for Bitcoin Finance](https://www.threshold.network/)
- [base T](https://basescan.org/token/0x26f3901ac8a79c50fb0d8289c74f0d09adc42e29)
- [solana T](https://solscan.io/token/4Njvi3928U3figEF5tf8xvjLC5GqUN33oe4XTJNe7xXC)
- [ethereum T](https://etherscan.io/token/0xcdf7028ceab81fa0c6971208e83fa7872994bee5)
- [optimism T](https://optimistic.etherscan.io/token/0x747e42eb0591547a0ab429b3627816208c734ea7)
- [arbitrum TBTC](https://arbiscan.io/token/0x6c84a8f1c29108f47a79964b5fe888d4f4d0de40)
- [base TBTC](https://basescan.org/token/0x236aa50979d5f3de3bd1eeb40e81137f22ab794b)
- [solana TBTC](https://solscan.io/token/6DNSN2BJsaPFdFFc1zP37kkeNe4Usc1Sqkzr9C9vPWcU)
- [ethereum TBTC](https://etherscan.io/token/0x18084fba666a33d37592fa2633fd49a74dd93a88)
- [Google project search](https://www.google.com/search?q=Threshold%20Network%20T%20crypto)

## Final Investment View

Threshold Network should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
