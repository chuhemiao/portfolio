---
title: 'SingularityNET / AGIX: bridge / cross-chain infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'SingularityNET is treated as a bridge / cross-chain infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'singularitynet-agix-bridge-cross-chain-infrastructure-token-value-capture-risk'
category: 'research'
---

# SingularityNET / AGIX: bridge / cross-chain infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** SingularityNET appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Cardano, Ethereum, SORA; exchange exposure is not disclosed; category tags are Cross-Chain & Bridge, Storage & Compute, Web3 AI (Agent/AIGC); current market cap is $18.41M; FDV is $29.94M; and 24h volume is $6.12K. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag SingularityNET before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 865, market cap $18,498,853, 24h volume $8,071. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

SingularityNET is best understood as a **bridge / cross-chain infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether AGIX captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Cardano, Ethereum, SORA |
| Exchanges | not disclosed |
| Price | $0.0767 |
| Market cap | $18.41M |
| FDV | $29.94M |
| 24h volume | $6.12K |
| Circulating supply | 240.02M |
| Total supply | 390.31M |
| 7d price change | -0.16% |
| 30d price change | -8.95% |
...
Base case: SingularityNET remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning AGIX into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.
...
Surf's project profile maps SingularityNET to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://singularitynet.io/](https://singularitynet.io/) |
| X handle | [@SingularityNET](https://x.com/SingularityNET) |
| X followers | 154.03K |
| Tags | Cross-Chain & Bridge, Storage & Compute, Web3 AI (Agent/AIGC) |
| Chains | Cardano, Ethereum, SORA |
| Exchanges | not disclosed |
| TGE status | not disclosed |
| CoinGecko ID | not disclosed |

The main product question is whether SingularityNET is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For bridge / cross-chain infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does SingularityNET convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $6.12K 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 240.02M circulating vs 390.31M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that SingularityNET is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Cardano, Ethereum, SORA |
...
|---|---|---|---|
| SORA | AGIX | 0x005e152271f8816d76221c7a0b5c6cafcb54fdfb6954dd8812f0158bfeac900d | not mapped |
| Cardano | AGIX | f43a62fdc3965df486de8a0d32fe800963589c41b38946602a0dc53541474958 | not mapped |
| ethereum | AGIX | 0x5b7533812759b45c2b44c19e320ba2cd2681b542 | [explorer](https://etherscan.io/token/0x5b7533812759b45c2b44c19e320ba2cd2681b542) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For SingularityNET, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 865, market cap $18,498,853, 24h volume $8,071. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $18.41M, FDV $29.94M, 24h volume $6.12K, and price $0.0767.
...
| 24h volume | $6.12K | Position sizing and exit liquidity proxy |
| Market cap | $18.41M | Liquid valuation proxy |
| FDV | $29.94M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.63x | Higher spread means more supply risk |
| X followers | 154.03K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for SingularityNET | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether AGIX is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. SingularityNET currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d -0.16% and 30d -8.95% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that SingularityNET's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2017-12-21 | Ico | $32.85M | not disclosed |

| Team member | Role | Link |
|---|---|---|
| Ben Goertzel | Founder, CEO | [X](https://twitter.com/bengoertzel) |
| David Hanson | Co-Founder | [X](https://twitter.com/HansonRobo) |
| Sergey Shalyapin | CTO | not disclosed |
| Janet Adams | COO | [X](https://twitter.com/JanetAdamsAI) |
| Matthew Ikle | Chief Science Officer | not disclosed |
| Alexey Potapov | Chief AGI Officer | not disclosed |
| Mihaela Ulieru | Chief AI Alchemist | not disclosed |
| Jan Horlings | CPO | [X](https://twitter.com/jan_horlings) |
...
SingularityNET competes in the bridge / cross-chain infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether SingularityNET has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: SingularityNET is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | SingularityNET compounds usage, liquidity improves, and AGIX gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If AGIX does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $6.12K | Weekly | Volume below position-size threshold |
| FDV / market cap | 1.63x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if SingularityNET announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. SingularityNET has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For SingularityNET, the first reconciliation job is identity. The token symbol is AGIX, the category is bridge / cross-chain infrastructure, chains are Cardano, Ethereum, SORA, exchanges are not disclosed, and tags are Cross-Chain & Bridge, Storage & Compute, Web3 AI (Agent/AIGC). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If AGIX trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $18.41M, FDV is $29.94M, 24h volume is $6.12K, FDV / market cap is 1.63x, 7d price change is -0.16%, and 30d price change is -8.95%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
For SingularityNET, the current liquidity read is watchlist first. The market cap is $18.41M, FDV is $29.94M, reported 24h volume is $6.12K, and FDV / market cap is 1.63x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. SingularityNET can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.
...
Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If SingularityNET suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | SingularityNET gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://singularitynet.io/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=SingularityNET) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=SingularityNET) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=SingularityNET) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=SingularityNET) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=SingularityNET) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/SingularityNET) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/singnet) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/singularitynet) | Identity, market, social, contract, or discovery cross-check |
| 10 | SingularityNET AGIX Price - CoinGecko | [link](https://www.coingecko.com/en/coins/singularitynet) | Identity, market, social, contract, or discovery cross-check |
| 11 | SingularityNET price AGIX #4723 - CoinMarketCap | [link](https://coinmarketcap.com/currencies/singularitynet/) | Identity, market, social, contract, or discovery cross-check |
| 12 | AGIX to BTC: SingularityNET Price in Bitcoin - CoinGecko | [link](https://www.coingecko.com/en/coins/singularitynet/btc) | Identity, market, social, contract, or discovery cross-check |
| 13 | AGIX to PKR: SingularityNET Price in Pakistani Rupee / CoinGecko | [link](https://www.coingecko.com/en/coins/singularitynet/pkr) | Identity, market, social, contract, or discovery cross-check |
| 14 | AGIX to INR: SingularityNET Price in Indian Rupee - CoinGecko | [link](https://www.coingecko.com/en/coins/singularitynet/inr) | Identity, market, social, contract, or discovery cross-check |
| 15 | ethereum AGIX | [link](https://etherscan.io/token/0x5b7533812759b45c2b44c19e320ba2cd2681b542) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google project search | [link](https://www.google.com/search?q=SingularityNET%20AGIX%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google tokenomics search | [link](https://www.google.com/search?q=SingularityNET%20AGIX%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google docs search | [link](https://www.google.com/search?q=SingularityNET%20docs) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google governance search | [link](https://www.google.com/search?q=SingularityNET%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google audit search | [link](https://www.google.com/search?q=SingularityNET%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/singularitynet/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/singularitynet/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune discovery | [link](https://dune.com/browse/dashboards?q=SingularityNET) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune token query | [link](https://dune.com/browse/queries?q=SingularityNET) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=SingularityNET) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=AGIX) | Identity, market, social, contract, or discovery cross-check |
| 27 | TokenUnlocks search | [link](https://token.unlocks.app/?search=SingularityNET) | Identity, market, social, contract, or discovery cross-check |
| 28 | CryptoRank search | [link](https://cryptorank.io/price/singularitynet) | Identity, market, social, contract, or discovery cross-check |
| 29 | RootData search | [link](https://www.rootdata.com/search?kw=SingularityNET) | Identity, market, social, contract, or discovery cross-check |
| 30 | CoinDesk search | [link](https://www.coindesk.com/search?s=SingularityNET) | Identity, market, social, contract, or discovery cross-check |
| 31 | The Block search | [link](https://www.theblock.co/search?query=SingularityNET) | Identity, market, social, contract, or discovery cross-check |
| 32 | GitHub search | [link](https://github.com/search?q=SingularityNET&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 33 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=AGIX) | Identity, market, social, contract, or discovery cross-check |
| 34 | BscScan token search | [link](https://bscscan.com/search?f=0&q=AGIX) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://singularitynet.io/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=SingularityNET)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=SingularityNET)
- [DefiLlama protocol discovery](https://defillama.com/search?query=SingularityNET)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=SingularityNET)
- [Messari discovery](https://messari.io/search?q=SingularityNET)
- [X profile](https://x.com/SingularityNET)
- [GitHub](https://github.com/singnet)
- [Telegram](https://t.me/singularitynet)
- [SingularityNET AGIX Price - CoinGecko](https://www.coingecko.com/en/coins/singularitynet)
- [SingularityNET price AGIX #4723 - CoinMarketCap](https://coinmarketcap.com/currencies/singularitynet/)
- [AGIX to BTC: SingularityNET Price in Bitcoin - CoinGecko](https://www.coingecko.com/en/coins/singularitynet/btc)
- [AGIX to PKR: SingularityNET Price in Pakistani Rupee | CoinGecko](https://www.coingecko.com/en/coins/singularitynet/pkr)
- [AGIX to INR: SingularityNET Price in Indian Rupee - CoinGecko](https://www.coingecko.com/en/coins/singularitynet/inr)
- [ethereum AGIX](https://etherscan.io/token/0x5b7533812759b45c2b44c19e320ba2cd2681b542)
- [Google project search](https://www.google.com/search?q=SingularityNET%20AGIX%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=SingularityNET%20AGIX%20tokenomics)
- [Google docs search](https://www.google.com/search?q=SingularityNET%20docs)
- [Google governance search](https://www.google.com/search?q=SingularityNET%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=SingularityNET%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/singularitynet/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/singularitynet/price-analysis/)
- [Dune discovery](https://dune.com/browse/dashboards?q=SingularityNET)
- [Dune token query](https://dune.com/browse/queries?q=SingularityNET)

## Final Investment View

SingularityNET should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
