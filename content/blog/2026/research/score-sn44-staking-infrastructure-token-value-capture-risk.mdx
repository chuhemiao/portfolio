---
title: 'Score / SN44: staking infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Score is treated as a staking infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'score-sn44-staking-infrastructure-token-value-capture-risk'
category: 'research'
---

# Score / SN44: staking infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Score appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is not disclosed; exchange exposure is not disclosed; category tags are Data & Analytics, Node & Validator, Web3 AI (Agent/AIGC); current market cap is $40.53M; FDV is $40.53M; and 24h volume is $396.06K. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Score before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 523, market cap $39,960,229, 24h volume $388,171. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Score is best understood as a **staking infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether SN44 captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
Base case: Score remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning SN44 into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Score is building vision AI models that make cameras "intelligent" by processing raw video into structured data on movement, context, and predictions — starting with football (soccer) analysis.It operates as Bittensor Subnet 44 (SN44), where miners run computer vision models to annotate match footage: detecting players, tracking ball positions, identifying game events, and more using object detection and keypoint analysis on short video clips. Validators assess the accuracy and quality of these annotations against benchmarks or human-level labels to fairly distribute rewards. This decentralized approach makes high-quality video analysis dramatically cheaper (e.g., $10 per game) and faster (e.g., minutes instead of hours) than traditional centralized services. The system begins in sports for its complex motion dynamics but is designed to expand to robotics, manufacturing, retail, healthcare, security, and other video-heavy fields via Vision AutoML for self-improving models. Public models from Bittensor miners are shared on Hugging Face, with a live console tracking performance gains over time.

Surf's project profile maps Score to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.wearescore.com/](https://www.wearescore.com/) |
| X handle | [@webuildscore](https://x.com/webuildscore) |
| X followers | 9.09K |
| Tags | Data & Analytics, Node & Validator, Web3 AI (Agent/AIGC) |
| Chains | not disclosed |
| Exchanges | not disclosed |
| TGE status | not disclosed |
| CoinGecko ID | not disclosed |

The main product question is whether Score is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For staking infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Score convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $396.06K 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 4.89M circulating vs 4.89M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Score is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Score, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 523, market cap $39,960,229, 24h volume $388,171. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $40.53M, FDV $40.53M, 24h volume $396.06K, and price $8.3000.
...
| 24h volume | $396.06K | Position sizing and exit liquidity proxy |
| Market cap | $40.53M | Liquid valuation proxy |
| FDV | $40.53M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.00x | Higher spread means more supply risk |
| X followers | 9.09K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Score | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether SN44 is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Score currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 3.39% and 30d 11.01% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Score's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-06-19 | Otc Purchase | $100.00K | not disclosed |

| Team member | Role | Link |
|---|---|---|
| Nigel Grant | Core Contributor | not disclosed |
| Max Sebti | Core Contributor | not disclosed |
| Tim Kalic | Core Contributor | not disclosed |
...
Score competes in the staking infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Score has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Score is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Score compounds usage, liquidity improves, and SN44 gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Score has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If SN44 does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $396.06K | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Score announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Score has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Score, the first reconciliation job is identity. The token symbol is SN44, the category is staking infrastructure, chains are not disclosed, exchanges are not disclosed, and tags are Data & Analytics, Node & Validator, Web3 AI (Agent/AIGC). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If SN44 trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $40.53M, FDV is $40.53M, 24h volume is $396.06K, FDV / market cap is 1.00x, 7d price change is 3.39%, and 30d price change is 11.01%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Score / SN44 | Match official docs, CG, CMC, Surf, contracts |
...
For Score, the current liquidity read is watchlist first. The market cap is $40.53M, FDV is $40.53M, reported 24h volume is $396.06K, and FDV / market cap is 1.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Score gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.wearescore.com/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Score) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Score) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Score) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Score) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Score) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/webuildscore) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/score-technologies/score-vision) | Identity, market, social, contract, or discovery cross-check |
| 9 | Score Price: SN44/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/score) | Identity, market, social, contract, or discovery cross-check |
...
| 12 | Score price today, SN44 to USD live price, marketcap and chart | [link](https://coinmarketcap.com/currencies/score/) | Identity, market, social, contract, or discovery cross-check |
| 13 | How CoinMarketCap and CoinGecko Validate the Numbers They ... | [link](https://coinmarketcap.com/academy/article/how-coinmarketcap-and-coingecko-validate-the-numbers-they-show-you) | Identity, market, social, contract, or discovery cross-check |
| 14 | Google project search | [link](https://www.google.com/search?q=Score%20SN44%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 15 | Google tokenomics search | [link](https://www.google.com/search?q=Score%20SN44%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google docs search | [link](https://www.google.com/search?q=Score%20docs) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google governance search | [link](https://www.google.com/search?q=Score%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google audit search | [link](https://www.google.com/search?q=Score%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 19 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/score/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 20 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/score/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 21 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Score) | Identity, market, social, contract, or discovery cross-check |
| 22 | Dune token query | [link](https://dune.com/browse/queries?q=Score) | Identity, market, social, contract, or discovery cross-check |
| 23 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Score) | Identity, market, social, contract, or discovery cross-check |
| 24 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=SN44) | Identity, market, social, contract, or discovery cross-check |
| 25 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Score) | Identity, market, social, contract, or discovery cross-check |
| 26 | CryptoRank search | [link](https://cryptorank.io/price/score) | Identity, market, social, contract, or discovery cross-check |
| 27 | RootData search | [link](https://www.rootdata.com/search?kw=Score) | Identity, market, social, contract, or discovery cross-check |
| 28 | CoinDesk search | [link](https://www.coindesk.com/search?s=Score) | Identity, market, social, contract, or discovery cross-check |
| 29 | The Block search | [link](https://www.theblock.co/search?query=Score) | Identity, market, social, contract, or discovery cross-check |
| 30 | GitHub search | [link](https://github.com/search?q=Score&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 31 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=SN44) | Identity, market, social, contract, or discovery cross-check |
| 32 | BscScan token search | [link](https://bscscan.com/search?f=0&q=SN44) | Identity, market, social, contract, or discovery cross-check |
| 33 | Arbiscan token search | [link](https://arbiscan.io/search?f=0&q=SN44) | Identity, market, social, contract, or discovery cross-check |
| 34 | BaseScan token search | [link](https://basescan.org/search?f=0&q=SN44) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.wearescore.com/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Score)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Score)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Score)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Score)
- [Messari discovery](https://messari.io/search?q=Score)
- [X profile](https://x.com/webuildscore)
- [GitHub](https://github.com/score-technologies/score-vision)
- [Score Price: SN44/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/score)
...
- [Score price today, SN44 to USD live price, marketcap and chart](https://coinmarketcap.com/currencies/score/)
- [How CoinMarketCap and CoinGecko Validate the Numbers They ...](https://coinmarketcap.com/academy/article/how-coinmarketcap-and-coingecko-validate-the-numbers-they-show-you)
- [Google project search](https://www.google.com/search?q=Score%20SN44%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Score%20SN44%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Score%20docs)
- [Google governance search](https://www.google.com/search?q=Score%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Score%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/score/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/score/price-analysis/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Score)
- [Dune token query](https://dune.com/browse/queries?q=Score)
- [DeFiLlama fees discovery](https://defillama.com/fees?chain=All&search=Score)
- [DeFiLlama yield discovery](https://defillama.com/yields?token=SN44)

## Final Investment View

Score should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
