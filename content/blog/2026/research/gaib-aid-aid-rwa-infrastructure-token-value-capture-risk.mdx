---
title: 'GAIB AID / AID: RWA infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'GAIB AID is treated as a RWA infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'gaib-aid-aid-rwa-infrastructure-token-value-capture-risk'
category: 'research'
---

# GAIB AID / AID: RWA infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** GAIB AID appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Binance Smart Chain, Ethereum; exchange exposure is BYBIT, BINANCE ALPHA, BITGET; category tags are DePIN (Physical Infra), RWA (Real World Assets), Storage & Compute; current market cap is $19.98M; FDV is $19.98M; and 24h volume is $0.1534. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag GAIB AID before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 831, market cap $19,977,312, 24h volume $0. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

GAIB AID is best understood as a **RWA infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether AID captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
Base case: GAIB AID remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning AID into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

GAIB builds an economic layer for AI infrastructure by tokenizing enterprise-grade GPUs and converting compute yields into tradable, yield-generating assets. Its protocol issues AID (an AI synthetic dollar) and staking (sAID) to provide liquidity and passive income while integrating AI compute financing with DeFi.

Surf's project profile maps GAIB AID to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.gaib.ai/](https://www.gaib.ai/) |
| X handle | [@gaib_ai](https://x.com/gaib_ai) |
| X followers | 196.10K |
| Tags | DePIN (Physical Infra), RWA (Real World Assets), Storage & Compute |
| Chains | Binance Smart Chain, Ethereum |
| Exchanges | BYBIT, BINANCE ALPHA, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether GAIB AID is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For RWA infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does GAIB AID convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $0.1534 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 20.01M circulating vs 20.01M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that GAIB AID is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

GAIB AID's architecture should be evaluated through the job it performs. For a RWA infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
|---|---|---|---|
| ethereum | AID | 0x18f52b3fb465118731d9e0d276d4eb3599d57596 | [explorer](https://etherscan.io/token/0x18f52b3fb465118731d9e0d276d4eb3599d57596) |
| ethereum | GAIB | 0xc19d38925f9f645337b1d1f37baf3c0647a48e50 | [explorer](https://etherscan.io/token/0xc19d38925f9f645337b1d1f37baf3c0647a48e50) |
| bsc | GAIB | 0xc19d38925f9f645337b1d1f37baf3c0647a48e50 | [explorer](https://bscscan.com/token/0xc19d38925f9f645337b1d1f37baf3c0647a48e50) |
| ethereum | SAID | 0xb3b3c527ba57cd61648e2ec2f5e006a0b390a9f8 | [explorer](https://etherscan.io/token/0xb3b3c527ba57cd61648e2ec2f5e006a0b390a9f8) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For GAIB AID, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 831, market cap $19,977,312, 24h volume $0. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $19.98M, FDV $19.98M, 24h volume $0.1534, and price $0.9982.
...
| 24h volume | $0.1534 | Position sizing and exit liquidity proxy |
| Market cap | $19.98M | Liquid valuation proxy |
| FDV | $19.98M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.00x | Higher spread means more supply risk |
| X followers | 196.10K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for GAIB AID | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether AID is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. GAIB AID currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 0.01% and 30d 0.17% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that GAIB AID's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-07-29 | Undisclosed | $10.00M | Amber Group |
| 2024-12-03 | Pre Seed | $5.00M | Hashed, Animoca Brands, The Spartan Group, Infinity Ventures Crypto (IVC), Hack VC, MH Ventures, Near Foundation, Faction |

| Team member | Role | Link |
|---|---|---|
| Kony Kwong | CEO, Co-Founder | [X](https://x.com/konyk001) |
| Alex Yeh | Co-Founder | [X](https://x.com/alex_yehya) |
| Mathilda Sun | Head of Strategy | [X](https://x.com/Mathilda_Sun_) |
...
GAIB AID competes in the RWA infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether GAIB AID has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: GAIB AID is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | GAIB AID compounds usage, liquidity improves, and AID gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If AID does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $0.1534 | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if GAIB AID announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. GAIB AID has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For GAIB AID, the first reconciliation job is identity. The token symbol is AID, the category is RWA infrastructure, chains are Binance Smart Chain, Ethereum, exchanges are BYBIT, BINANCE ALPHA, BITGET, and tags are DePIN (Physical Infra), RWA (Real World Assets), Storage & Compute. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If AID trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $19.98M, FDV is $19.98M, 24h volume is $0.1534, FDV / market cap is 1.00x, 7d price change is 0.01%, and 30d price change is 0.17%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | GAIB AID / AID | Match official docs, CG, CMC, Surf, contracts |
...
For GAIB AID, the current liquidity read is watchlist first. The market cap is $19.98M, FDV is $19.98M, reported 24h volume is $0.1534, and FDV / market cap is 1.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | GAIB AID gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.gaib.ai/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=GAIB%20AID) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=GAIB%20AID) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=GAIB%20AID) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=GAIB%20AID) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=GAIB%20AID) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/gaib_ai) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/gaib_ai_official) | Identity, market, social, contract, or discovery cross-check |
| 9 | GAIB AID Price: AID/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/gaib-aid) | Identity, market, social, contract, or discovery cross-check |
| 10 | GAIB/USD Live Price Chart, Market Cap & News Today - CoinGecko | [link](https://www.coingecko.com/en/coins/gaib) | Identity, market, social, contract, or discovery cross-check |
| 11 | GAIB sAID Price: SAID/USD Live Price Chart, Market ... - CoinGecko | [link](https://www.coingecko.com/en/coins/gaib-said) | Identity, market, social, contract, or discovery cross-check |
| 12 | GAIB price today, GAIB to USD live price, marketcap and chart | [link](https://coinmarketcap.com/currencies/gaib/) | Identity, market, social, contract, or discovery cross-check |
| 13 | GAIB AID Price History: Download AID Historical Data - CoinGecko | [link](https://www.coingecko.com/en/coins/gaib-aid/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 14 | ethereum AID | [link](https://etherscan.io/token/0x18f52b3fb465118731d9e0d276d4eb3599d57596) | Identity, market, social, contract, or discovery cross-check |
| 15 | ethereum GAIB | [link](https://etherscan.io/token/0xc19d38925f9f645337b1d1f37baf3c0647a48e50) | Identity, market, social, contract, or discovery cross-check |
| 16 | bsc GAIB | [link](https://bscscan.com/token/0xc19d38925f9f645337b1d1f37baf3c0647a48e50) | Identity, market, social, contract, or discovery cross-check |
| 17 | ethereum SAID | [link](https://etherscan.io/token/0xb3b3c527ba57cd61648e2ec2f5e006a0b390a9f8) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google project search | [link](https://www.google.com/search?q=GAIB%20AID%20AID%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google tokenomics search | [link](https://www.google.com/search?q=GAIB%20AID%20AID%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google docs search | [link](https://www.google.com/search?q=GAIB%20AID%20docs) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google governance search | [link](https://www.google.com/search?q=GAIB%20AID%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google audit search | [link](https://www.google.com/search?q=GAIB%20AID%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/gaib-aid/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/gaib-aid/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 25 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/gaib-aid/) | Identity, market, social, contract, or discovery cross-check |
| 26 | Dune discovery | [link](https://dune.com/browse/dashboards?q=GAIB%20AID) | Identity, market, social, contract, or discovery cross-check |
| 27 | Dune token query | [link](https://dune.com/browse/queries?q=GAIB%20AID) | Identity, market, social, contract, or discovery cross-check |
| 28 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=GAIB%20AID) | Identity, market, social, contract, or discovery cross-check |
| 29 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=AID) | Identity, market, social, contract, or discovery cross-check |
| 30 | TokenUnlocks search | [link](https://token.unlocks.app/?search=GAIB%20AID) | Identity, market, social, contract, or discovery cross-check |
| 31 | CryptoRank search | [link](https://cryptorank.io/price/gaib-aid) | Identity, market, social, contract, or discovery cross-check |
| 32 | RootData search | [link](https://www.rootdata.com/search?kw=GAIB%20AID) | Identity, market, social, contract, or discovery cross-check |
| 33 | CoinDesk search | [link](https://www.coindesk.com/search?s=GAIB%20AID) | Identity, market, social, contract, or discovery cross-check |
| 34 | The Block search | [link](https://www.theblock.co/search?query=GAIB%20AID) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.gaib.ai/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=GAIB%20AID)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=GAIB%20AID)
- [DefiLlama protocol discovery](https://defillama.com/search?query=GAIB%20AID)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=GAIB%20AID)
- [Messari discovery](https://messari.io/search?q=GAIB%20AID)
- [X profile](https://x.com/gaib_ai)
- [Telegram](https://t.me/gaib_ai_official)
- [GAIB AID Price: AID/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/gaib-aid)
- [GAIB/USD Live Price Chart, Market Cap & News Today - CoinGecko](https://www.coingecko.com/en/coins/gaib)
- [GAIB sAID Price: SAID/USD Live Price Chart, Market ... - CoinGecko](https://www.coingecko.com/en/coins/gaib-said)
- [GAIB price today, GAIB to USD live price, marketcap and chart](https://coinmarketcap.com/currencies/gaib/)
- [GAIB AID Price History: Download AID Historical Data - CoinGecko](https://www.coingecko.com/en/coins/gaib-aid/historical_data)
- [ethereum AID](https://etherscan.io/token/0x18f52b3fb465118731d9e0d276d4eb3599d57596)
- [ethereum GAIB](https://etherscan.io/token/0xc19d38925f9f645337b1d1f37baf3c0647a48e50)
- [bsc GAIB](https://bscscan.com/token/0xc19d38925f9f645337b1d1f37baf3c0647a48e50)
- [ethereum SAID](https://etherscan.io/token/0xb3b3c527ba57cd61648e2ec2f5e006a0b390a9f8)
- [Google project search](https://www.google.com/search?q=GAIB%20AID%20AID%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=GAIB%20AID%20AID%20tokenomics)
- [Google docs search](https://www.google.com/search?q=GAIB%20AID%20docs)
- [Google governance search](https://www.google.com/search?q=GAIB%20AID%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=GAIB%20AID%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/gaib-aid/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/gaib-aid/price-analysis/)

## Final Investment View

GAIB AID should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
