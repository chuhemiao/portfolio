---
title: 'Bio Protocol / BIO: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Bio Protocol is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'bio-protocol-bio-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# Bio Protocol / BIO: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Bio Protocol appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Base, Binance Smart Chain, Ethereum, Solana; exchange exposure is BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BITGET, ASTER; category tags are DeSci (Decentralized Science), Web3 AI (Agent/AIGC), DAO; current market cap is $62.04M; FDV is $96.02M; and 24h volume is $15.54M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Bio Protocol before this file was created, and the existing candidate note says: Seeded 2026-06-28 from Surf market-ranking; rank 371, market cap $62,012,499, 24h volume $18,922,597. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Bio Protocol is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether BIO captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.

| Metric | Current read |
|---|---|
| Project | Bio Protocol |
| Token | BIO |
| Category | DeFi infrastructure |
| Chains | Base, Binance Smart Chain, Ethereum, Solana |
| Exchanges | BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BITGET, ASTER |
| Price | $0.0289 |
| Market cap | $62.04M |
| FDV | $96.02M |
| 24h volume | $15.54M |
...
Base case: Bio Protocol remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning BIO into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

BIO Protocol is a financial layer for decentralized science (DeSci) designed to accelerate the flow of capital and talent into on-chain scientific research. It enables BioDAOs to fundraise through auction contracts and direct funding towards initiatives in biotech, including research, intellectual property, and other organizations.

Surf's project profile maps Bio Protocol to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://ai.bio.xyz/](https://ai.bio.xyz/) |
| X handle | [@BioProtocol](https://x.com/BioProtocol) |
| X followers | 116.84K |
| Tags | DeSci (Decentralized Science), Web3 AI (Agent/AIGC), DAO |
| Chains | Base, Binance Smart Chain, Ethereum, Solana |
| Exchanges | BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BITGET, ASTER |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Bio Protocol is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DeFi infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Bio Protocol convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $15.54M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 2.15B circulating vs 3.32B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Bio Protocol is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| base | BIO | 0x226a2fa2556c48245e57cd1cba4c6c9e67077dd2 | [explorer](https://basescan.org/token/0x226a2fa2556c48245e57cd1cba4c6c9e67077dd2) |
| solana | BIO | bioJ9JTqW62MLz7UKHU69gtKhPpGi1BQhccj2kmSvUJ | [explorer](https://solscan.io/token/bioJ9JTqW62MLz7UKHU69gtKhPpGi1BQhccj2kmSvUJ) |
| ethereum | BIO | 0xcb1592591996765ec0efc1f92599a19767ee5ffa | [explorer](https://etherscan.io/token/0xcb1592591996765ec0efc1f92599a19767ee5ffa) |
| bsc | BIO | 0x226a2fa2556c48245e57cd1cba4c6c9e67077dd2 | [explorer](https://bscscan.com/token/0x226a2fa2556c48245e57cd1cba4c6c9e67077dd2) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Bio Protocol, the DeFi-oriented backlog note is the first signal: Seeded 2026-06-28 from Surf market-ranking; rank 371, market cap $62,012,499, 24h volume $18,922,597. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $62.04M, FDV $96.02M, 24h volume $15.54M, and price $0.0289.
...
| 24h volume | $15.54M | Position sizing and exit liquidity proxy |
| Market cap | $62.04M | Liquid valuation proxy |
| FDV | $96.02M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.55x | Higher spread means more supply risk |
| X followers | 116.84K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Bio Protocol | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether BIO is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. Bio Protocol should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that Bio Protocol belongs to the DeFi infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $62.04M, FDV $96.02M, 24h volume $15.54M, price $0.0289, circulating supply 2.15B, and total supply 3.32B. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For Bio Protocol, the candidate backlog records Seeded 2026-06-28 from Surf market-ranking; rank 371, market cap $62,012,499, 24h volume $18,922,597. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Base, Binance Smart Chain, Ethereum, Solana, exchanges as BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BITGET, ASTER, and tags as DeSci (Decentralized Science), Web3 AI (Agent/AIGC), DAO. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Bio Protocol currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 3.89% and 30d 11.23% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Bio Protocol's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-09-17 | Seed | $6.90M | Mechanism Capital, Animoca Brands, Mirana Ventures, Zee Prime Capital, Big Brain Holdings, Foresight Ventures, Maelstrom, Panga Capital |
| 2024-11-08 | Undisclosed | not disclosed | YZi Labs (Prev. Binance Labs) |

| Team member | Role | Link |
|---|---|---|
| Paul Kohlhaas | Founder, Co-founder, CEO | [X](https://x.com/paulkhls) |
| Clemens Ortlepp | Co-Founder, CPO | [X](https://x.com/cl2pp) |
| James Sinka | Co-Founder, Science Lead | [X](https://x.com/jamessinka) |
| Tyler Golato | Co-Initiator | [X](https://x.com/GolatoTyler) |
...
Bio Protocol competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Bio Protocol has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Bio Protocol is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Bio Protocol compounds usage, liquidity improves, and BIO gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If BIO does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $15.54M | Weekly | Volume below position-size threshold |
| FDV / market cap | 1.55x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Bio Protocol announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Bio Protocol has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Bio Protocol, the first reconciliation job is identity. The token symbol is BIO, the category is DeFi infrastructure, chains are Base, Binance Smart Chain, Ethereum, Solana, exchanges are BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BITGET, ASTER, and tags are DeSci (Decentralized Science), Web3 AI (Agent/AIGC), DAO. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If BIO trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $62.04M, FDV is $96.02M, 24h volume is $15.54M, FDV / market cap is 1.55x, 7d price change is 3.89%, and 30d price change is 11.23%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Bio Protocol / BIO | Match official docs, CG, CMC, Surf, contracts |
...
For Bio Protocol, the current liquidity read is watchlist first. The market cap is $62.04M, FDV is $96.02M, reported 24h volume is $15.54M, and FDV / market cap is 1.55x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Bio Protocol gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://ai.bio.xyz/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Bio%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Bio%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Bio%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Bio%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Bio%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/BioProtocol) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/bio-xyz) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/bio_protocol) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](https://discord.com/invite/bioxyz) | Identity, market, social, contract, or discovery cross-check |
| 11 | Bio Protocol price today, BIO to USD live price, marketcap ... | [link](https://coinmarketcap.com/currencies/bio/) | Identity, market, social, contract, or discovery cross-check |
| 12 | Bio Protocol Price: BIO/USD Live Price Chart, Market Cap ... | [link](https://www.coingecko.com/en/coins/bio-protocol) | Identity, market, social, contract, or discovery cross-check |
| 13 | What Is Bio Protocol Crypto? Why This AI-Powered DeSci ... | [link](https://bitcoinfoundation.org/news/ai-news/what-is-bio-protocol-crypto-why-this-ai-powered-desci-token-is-exploding-in-2026/) | Identity, market, social, contract, or discovery cross-check |
| 14 | Bio • BIO Token | [link](https://www.bio.xyz/bio-token) | Identity, market, social, contract, or discovery cross-check |
| 15 | Latest Bio Protocol (BIO) News Update | [link](https://coinmarketcap.com/cmc-ai/bio/latest-updates/) | Identity, market, social, contract, or discovery cross-check |
| 16 | base BIO | [link](https://basescan.org/token/0x226a2fa2556c48245e57cd1cba4c6c9e67077dd2) | Identity, market, social, contract, or discovery cross-check |
| 17 | solana BIO | [link](https://solscan.io/token/bioJ9JTqW62MLz7UKHU69gtKhPpGi1BQhccj2kmSvUJ) | Identity, market, social, contract, or discovery cross-check |
| 18 | ethereum BIO | [link](https://etherscan.io/token/0xcb1592591996765ec0efc1f92599a19767ee5ffa) | Identity, market, social, contract, or discovery cross-check |
| 19 | bsc BIO | [link](https://bscscan.com/token/0x226a2fa2556c48245e57cd1cba4c6c9e67077dd2) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google project search | [link](https://www.google.com/search?q=Bio%20Protocol%20BIO%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google tokenomics search | [link](https://www.google.com/search?q=Bio%20Protocol%20BIO%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google docs search | [link](https://www.google.com/search?q=Bio%20Protocol%20docs) | Identity, market, social, contract, or discovery cross-check |
| 23 | Google governance search | [link](https://www.google.com/search?q=Bio%20Protocol%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 24 | Google audit search | [link](https://www.google.com/search?q=Bio%20Protocol%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 25 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/bio-protocol/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 26 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/bio-protocol/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 27 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/bio-protocol/) | Identity, market, social, contract, or discovery cross-check |
| 28 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Bio%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 29 | Dune token query | [link](https://dune.com/browse/queries?q=Bio%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 30 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Bio%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 31 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=BIO) | Identity, market, social, contract, or discovery cross-check |
| 32 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Bio%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 33 | CryptoRank search | [link](https://cryptorank.io/price/bio-protocol) | Identity, market, social, contract, or discovery cross-check |
| 34 | RootData search | [link](https://www.rootdata.com/search?kw=Bio%20Protocol) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://ai.bio.xyz/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Bio%20Protocol)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Bio%20Protocol)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Bio%20Protocol)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Bio%20Protocol)
- [Messari discovery](https://messari.io/search?q=Bio%20Protocol)
- [X profile](https://x.com/BioProtocol)
- [GitHub](https://github.com/bio-xyz)
- [Telegram](https://t.me/bio_protocol)
- [Discord](https://discord.com/invite/bioxyz)
- [Bio Protocol price today, BIO to USD live price, marketcap ...](https://coinmarketcap.com/currencies/bio/)
- [Bio Protocol Price: BIO/USD Live Price Chart, Market Cap ...](https://www.coingecko.com/en/coins/bio-protocol)
- [What Is Bio Protocol Crypto? Why This AI-Powered DeSci ...](https://bitcoinfoundation.org/news/ai-news/what-is-bio-protocol-crypto-why-this-ai-powered-desci-token-is-exploding-in-2026/)
- [Bio • BIO Token](https://www.bio.xyz/bio-token)
- [Latest Bio Protocol (BIO) News Update](https://coinmarketcap.com/cmc-ai/bio/latest-updates/)
- [base BIO](https://basescan.org/token/0x226a2fa2556c48245e57cd1cba4c6c9e67077dd2)
- [solana BIO](https://solscan.io/token/bioJ9JTqW62MLz7UKHU69gtKhPpGi1BQhccj2kmSvUJ)
- [ethereum BIO](https://etherscan.io/token/0xcb1592591996765ec0efc1f92599a19767ee5ffa)
- [bsc BIO](https://bscscan.com/token/0x226a2fa2556c48245e57cd1cba4c6c9e67077dd2)
- [Google project search](https://www.google.com/search?q=Bio%20Protocol%20BIO%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Bio%20Protocol%20BIO%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Bio%20Protocol%20docs)
- [Google governance search](https://www.google.com/search?q=Bio%20Protocol%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Bio%20Protocol%20audit%20report)

## Final Investment View

Bio Protocol should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
