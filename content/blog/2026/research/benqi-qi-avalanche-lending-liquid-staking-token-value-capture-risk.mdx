---
title: 'BENQI / QI: Avalanche lending / liquid staking value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'BENQI is treated as a Avalanche lending / liquid staking watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'benqi-qi-avalanche-lending-liquid-staking-token-value-capture-risk'
category: 'research'
---

# BENQI / QI: Avalanche lending / liquid staking value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** BENQI appears in the local CoinGecko/CMC-oriented candidate pool through [CoinGecko](https://www.coingecko.com/en/coins/benqi) and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Avalanche; exchange exposure is BINANCE, COINBASE; category tags are Lending & Borrowing, Node & Validator; current market cap is $8.74M; FDV is $8.74M; and 24h volume is $575.89K. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag BENQI before this file was created, and the existing candidate note says: DefiLlama category Lending on Avalanche; TVL $83,120,390. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

BENQI is best understood as a **Avalanche lending / liquid staking** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether QI captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.

| Metric | Current read |
|---|---|
| Project | BENQI |
| Token | QI |
| Category | Avalanche lending / liquid staking |
| Chains | Avalanche |
...
Base case: BENQI remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning QI into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

BENQI is a suite of Avalanche-native DeFi protocols providing non-custodial lending and borrowing markets, liquid staking (sAVAX), and tools to deploy validators (Ignite). Its native QI token governs the protocol and coordinates incentives across BENQI’s markets and staking services.

Surf's project profile maps BENQI to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://benqi.fi/](https://benqi.fi/) |
| X handle | [@BenqiFinance](https://x.com/BenqiFinance) |
| X followers | 79.11K |
| Tags | Lending & Borrowing, Node & Validator |
| Chains | Avalanche |
| Exchanges | BINANCE, COINBASE |
| TGE status | post |
| CoinGecko ID | [benqi](https://www.coingecko.com/en/coins/benqi) |

The main product question is whether BENQI is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For Avalanche lending / liquid staking projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does BENQI convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $575.89K 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 7.20B circulating vs 7.20B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that BENQI is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

BENQI's architecture should be evaluated through the job it performs. For a Avalanche lending / liquid staking project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
|---|---|---|---|
| avalanche | QI | 0x8729438eb15e2c8b576fcc6aecda6a148776c0f5 | [explorer](https://snowtrace.io/token/0x8729438eb15e2c8b576fcc6aecda6a148776c0f5) |
| avalanche | SAVAX | 0x2b2c81e08f1af8835a78bb2a90ae924ace0ea4be | [explorer](https://snowtrace.io/token/0x2b2c81e08f1af8835a78bb2a90ae924ace0ea4be) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For BENQI, the DeFi-oriented backlog note is the first signal: DefiLlama category Lending on Avalanche; TVL $83,120,390. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $8.74M, FDV $8.74M, 24h volume $575.89K, and price $0.0012.
...
| 24h volume | $575.89K | Position sizing and exit liquidity proxy |
| Market cap | $8.74M | Liquid valuation proxy |
| FDV | $8.74M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.00x | Higher spread means more supply risk |
| X followers | 79.11K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for BENQI | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether QI is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. BENQI should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that BENQI belongs to the Avalanche lending / liquid staking bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $8.74M, FDV $8.74M, 24h volume $575.89K, price $0.0012, circulating supply 7.20B, and total supply 7.20B. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For BENQI, the candidate backlog records DefiLlama category Lending on Avalanche; TVL $83,120,390. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Avalanche, exchanges as BINANCE, COINBASE, and tags as Lending & Borrowing, Node & Validator. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.

The fourth frame is value capture. A Avalanche lending / liquid staking asset can be useful while the token remains weak. The memo therefore separates product-market fit from token-market fit. Product-market fit asks whether users need the protocol even when incentives fade. Token-market fit asks whether the token absorbs value through staking, fee routing, collateral, governance constraints, emissions control, slashing insurance, or privileged access. If the token is mostly governance, the base case assigns lower confidence. If the token is used for security or fee capture but the mechanism is not yet activated, the base case treats value capture as an option rather than as current cash flow.

The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. BENQI currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 8.07% and 30d -9.81% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that BENQI's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2021-04-28 | Ico | $10.89M | not disclosed |
| 2021-04-28 | Ico | $583.20K | not disclosed |
| 2021-04-14 | Strategic | $6.00M | Mechanism Capital, Arrington Capital, Ava Labs, Genblock Capital, Ascensive Assets, Rarestone Capital, Woodstock Fund, Skynet Trading |
| 2021-03-01 | Seed | $1.08M | not disclosed |

| Team member | Role | Link |
|---|---|---|
| Dan Mgbor | Co-Founder | [X](https://twitter.com/born2hodl) |
| Hannu Kuusi | Co-Founder | [X](https://twitter.com/BaldSpruce) |
| JD Gagnon | Co-Founder | [X](https://twitter.com/the_diamondrock) |
| Alex N | Advisor | not disclosed |
| Alexander Szul | CIO | [X](https://twitter.com/alex_szul) |
...
BENQI competes in the Avalanche lending / liquid staking bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether BENQI has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: BENQI is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | BENQI compounds usage, liquidity improves, and QI gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because BENQI has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If QI does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $575.89K | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if BENQI announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. BENQI has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For BENQI, the first reconciliation job is identity. The token symbol is QI, the category is Avalanche lending / liquid staking, chains are Avalanche, exchanges are BINANCE, COINBASE, and tags are Lending & Borrowing, Node & Validator. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If QI trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $8.74M, FDV is $8.74M, 24h volume is $575.89K, FDV / market cap is 1.00x, 7d price change is 8.07%, and 30d price change is -9.81%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.

The third reconciliation job is usage data. The latest TVL sample is not disclosed, TVL sample change is not disclosed, and the sample contains 0 point(s). TVL is useful only when the underlying balances are real, withdrawable, and not double-counted. For Avalanche lending / liquid staking, the memo should identify whether TVL means deposits, staked assets, LP liquidity, stablecoin supply, collateral, locked governance assets, or a derived wrapper. A yield-bearing stablecoin and a validator middleware protocol can both show TVL, but the risk encoded by that TVL is completely different.

| Reconciliation item | Current value | Next check |
|---|---|---|
| Identity | BENQI / QI | Match official docs, CG, CMC, Surf, contracts |
| Category | Avalanche lending / liquid staking | Confirm product mechanics and revenue model |
...
| Correlation check | Count exposure to related Avalanche lending / liquid staking assets | Category beta can hide concentration |
...
| Peer comparison | Monthly | BENQI gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://benqi.fi/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko profile | [link](https://www.coingecko.com/en/coins/benqi) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=BENQI) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=BENQI) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=BENQI) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=BENQI) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/BenqiFinance) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/Benqi-fi) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/BenqiFinance) | Identity, market, social, contract, or discovery cross-check |
| 10 | BENQI price today, QI to USD live price, marketcap and chart | [link](https://coinmarketcap.com/currencies/benqi/) | Identity, market, social, contract, or discovery cross-check |
| 11 | What Is BENQI (QI) And How Does It Work? | [link](https://coinmarketcap.com/cmc-ai/benqi/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 12 | Latest BENQI (QI) Price Analysis | [link](https://coinmarketcap.com/cmc-ai/benqi/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 13 | avalanche QI | [link](https://snowtrace.io/token/0x8729438eb15e2c8b576fcc6aecda6a148776c0f5) | Identity, market, social, contract, or discovery cross-check |
| 14 | avalanche SAVAX | [link](https://snowtrace.io/token/0x2b2c81e08f1af8835a78bb2a90ae924ace0ea4be) | Identity, market, social, contract, or discovery cross-check |
| 15 | Google project search | [link](https://www.google.com/search?q=BENQI%20QI%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google tokenomics search | [link](https://www.google.com/search?q=BENQI%20QI%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google docs search | [link](https://www.google.com/search?q=BENQI%20docs) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google governance search | [link](https://www.google.com/search?q=BENQI%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google audit search | [link](https://www.google.com/search?q=BENQI%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 20 | CoinGecko historical data | [link](https://www.coingecko.com/en/coins/benqi/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 21 | Dune discovery | [link](https://dune.com/browse/dashboards?q=BENQI) | Identity, market, social, contract, or discovery cross-check |
| 22 | Dune token query | [link](https://dune.com/browse/queries?q=BENQI) | Identity, market, social, contract, or discovery cross-check |
| 23 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=BENQI) | Identity, market, social, contract, or discovery cross-check |
| 24 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=QI) | Identity, market, social, contract, or discovery cross-check |
| 25 | TokenUnlocks search | [link](https://token.unlocks.app/?search=BENQI) | Identity, market, social, contract, or discovery cross-check |
| 26 | CryptoRank search | [link](https://cryptorank.io/price/benqi) | Identity, market, social, contract, or discovery cross-check |
| 27 | RootData search | [link](https://www.rootdata.com/search?kw=BENQI) | Identity, market, social, contract, or discovery cross-check |
| 28 | CoinDesk search | [link](https://www.coindesk.com/search?s=BENQI) | Identity, market, social, contract, or discovery cross-check |
| 29 | The Block search | [link](https://www.theblock.co/search?query=BENQI) | Identity, market, social, contract, or discovery cross-check |
| 30 | GitHub search | [link](https://github.com/search?q=BENQI&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 31 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=QI) | Identity, market, social, contract, or discovery cross-check |
| 32 | BscScan token search | [link](https://bscscan.com/search?f=0&q=QI) | Identity, market, social, contract, or discovery cross-check |
| 33 | Arbiscan token search | [link](https://arbiscan.io/search?f=0&q=QI) | Identity, market, social, contract, or discovery cross-check |
| 34 | BaseScan token search | [link](https://basescan.org/search?f=0&q=QI) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://benqi.fi/)
- [CoinGecko profile](https://www.coingecko.com/en/coins/benqi)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=BENQI)
- [DefiLlama protocol discovery](https://defillama.com/search?query=BENQI)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=BENQI)
- [Messari discovery](https://messari.io/search?q=BENQI)
- [X profile](https://x.com/BenqiFinance)
- [GitHub](https://github.com/Benqi-fi)
- [Telegram](https://t.me/BenqiFinance)
- [BENQI price today, QI to USD live price, marketcap and chart](https://coinmarketcap.com/currencies/benqi/)
- [What Is BENQI (QI) And How Does It Work?](https://coinmarketcap.com/cmc-ai/benqi/what-is/)
- [Latest BENQI (QI) Price Analysis](https://coinmarketcap.com/cmc-ai/benqi/price-analysis/)
- [avalanche QI](https://snowtrace.io/token/0x8729438eb15e2c8b576fcc6aecda6a148776c0f5)
- [avalanche SAVAX](https://snowtrace.io/token/0x2b2c81e08f1af8835a78bb2a90ae924ace0ea4be)
- [Google project search](https://www.google.com/search?q=BENQI%20QI%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=BENQI%20QI%20tokenomics)
- [Google docs search](https://www.google.com/search?q=BENQI%20docs)
- [Google governance search](https://www.google.com/search?q=BENQI%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=BENQI%20audit%20report)
- [CoinGecko historical data](https://www.coingecko.com/en/coins/benqi/historical_data)
- [Dune discovery](https://dune.com/browse/dashboards?q=BENQI)
- [Dune token query](https://dune.com/browse/queries?q=BENQI)
- [DeFiLlama fees discovery](https://defillama.com/fees?chain=All&search=BENQI)
- [DeFiLlama yield discovery](https://defillama.com/yields?token=QI)

## Final Investment View

BENQI should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
