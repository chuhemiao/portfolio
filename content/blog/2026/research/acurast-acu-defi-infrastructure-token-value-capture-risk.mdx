---
title: 'Acurast / ACU: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Acurast is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'acurast-acu-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# Acurast / ACU: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Acurast appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Base, Binance Smart Chain, Ethereum; exchange exposure is BINANCE ALPHA, GATE; category tags are DePIN (Physical Infra); current market cap is $25.81M; FDV is $84.45M; and 24h volume is $3.01M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Acurast before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 710, market cap $25,747,239, 24h volume $2,865,215. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Acurast is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether ACU captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Exchanges | BINANCE ALPHA, GATE |
| Price | $0.0827 |
| Market cap | $25.81M |
| FDV | $84.45M |
| 24h volume | $3.01M |
| Circulating supply | 311.96M |
| Total supply | 1.02B |
| 7d price change | 2.82% |
| 30d price change | 21.56% |
...
Base case: Acurast remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning ACU into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Acurast is a decentralized compute network that leverages smartphones and their Trusted Execution Environments (TEEs) to provide verifiable, confidential, and scalable off-chain compute for Web3 and AI workloads. Developers deploy privacy-preserving serverless applications while device operators contribute compute and earn network rewards.

Surf's project profile maps Acurast to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://acurast.com/](https://acurast.com/) |
| X handle | [@Acurast](https://x.com/Acurast) |
| X followers | 150.45K |
...
| Liquidity | $3.01M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 311.96M circulating vs 1.02B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Acurast is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| base | ACU | 0xc5fed7c8ccc75d8a72b601a66dffd7a489073f0b | [explorer](https://basescan.org/token/0xc5fed7c8ccc75d8a72b601a66dffd7a489073f0b) |
| ethereum | ACU | 0x216b3643ff8b7bb30d8a48e9f1bd550126202add | [explorer](https://etherscan.io/token/0x216b3643ff8b7bb30d8a48e9f1bd550126202add) |
| bsc | ACU | 0x6ef2ffb38d64afe18ce782da280b300e358cfeaf | [explorer](https://bscscan.com/token/0x6ef2ffb38d64afe18ce782da280b300e358cfeaf) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Acurast, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 710, market cap $25,747,239, 24h volume $2,865,215. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $25.81M, FDV $84.45M, 24h volume $3.01M, and price $0.0827.
...
| 24h volume | $3.01M | Position sizing and exit liquidity proxy |
| Market cap | $25.81M | Liquid valuation proxy |
| FDV | $84.45M | Dilution-aware valuation proxy |
| FDV / market cap spread | 3.27x | Higher spread means more supply risk |
| X followers | 150.45K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Acurast | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether ACU is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Acurast currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 2.82% and 30d 21.56% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Acurast's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-11-13 | Undisclosed | $11.00M | CoinList, Tezos Foundation, Gavin Wood, Scytale Digital, Web3 Foundation, Michael van de Poppe, MN Capital, ogle |
| 2025-05-15 | Ico | $10.80M | not disclosed |
| 2024-01-12 | Strategic | $2.00M | not disclosed |
| 2023-02-01 | Seed | $2.00M | not disclosed |

| Team member | Role | Link |
|---|---|---|
| Alessandro De Carli | Co-Founder | [X](https://x.com/a_d_c_) |
| Pascal Brun | Co-Founder | not disclosed |
| Mike Godenzi | CTO | not disclosed |
...
Acurast competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Acurast has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Acurast is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Acurast compounds usage, liquidity improves, and ACU gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Acurast has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If ACU does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $3.01M | Weekly | Volume below position-size threshold |
| FDV / market cap | 3.27x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Acurast announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Acurast has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Acurast, the first reconciliation job is identity. The token symbol is ACU, the category is DeFi infrastructure, chains are Base, Binance Smart Chain, Ethereum, exchanges are BINANCE ALPHA, GATE, and tags are DePIN (Physical Infra). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If ACU trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $25.81M, FDV is $84.45M, 24h volume is $3.01M, FDV / market cap is 3.27x, 7d price change is 2.82%, and 30d price change is 21.56%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Acurast / ACU | Match official docs, CG, CMC, Surf, contracts |
...
| Exchanges | BINANCE ALPHA, GATE | Check live order books and inactive pairs |
| Market cap | $25.81M | Compare Surf, CG, CMC, exchange pages |
| FDV | $84.45M | Reconcile total supply and unlock schedule |
| Volume | $3.01M | Check exchange concentration and wash-trading risk |
| TVL | not disclosed | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Acurast does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Acurast, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does ACU sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
For Acurast, the current liquidity read is watchlist first. The market cap is $25.81M, FDV is $84.45M, reported 24h volume is $3.01M, and FDV / market cap is 3.27x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Acurast gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://acurast.com/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Acurast) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Acurast) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Acurast) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Acurast) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Acurast) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/Acurast) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/Acurast) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/acurastnetwork) | Identity, market, social, contract, or discovery cross-check |
| 10 | Acurast Price: ACU/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/acurast) | Identity, market, social, contract, or discovery cross-check |
| 11 | Tokenomics / Acurast Docs | [link](https://docs.acurast.com/discover/tokenomics/) | Identity, market, social, contract, or discovery cross-check |
| 12 | Token / Acurast Docs | [link](https://docs.acurast.com/discover/acurast-token/) | Identity, market, social, contract, or discovery cross-check |
| 13 | Latest Acurast (ACU) Price Analysis - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/acurast/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 14 | Updated Acurast Tokenomics — Decentralized Compute, Powered ... | [link](https://acurast.com/blog/network-and-tech/updated-acurast-tokenomics-decentralized-compute-powered-by-acu/) | Identity, market, social, contract, or discovery cross-check |
| 15 | base ACU | [link](https://basescan.org/token/0xc5fed7c8ccc75d8a72b601a66dffd7a489073f0b) | Identity, market, social, contract, or discovery cross-check |
| 16 | ethereum ACU | [link](https://etherscan.io/token/0x216b3643ff8b7bb30d8a48e9f1bd550126202add) | Identity, market, social, contract, or discovery cross-check |
| 17 | bsc ACU | [link](https://bscscan.com/token/0x6ef2ffb38d64afe18ce782da280b300e358cfeaf) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google project search | [link](https://www.google.com/search?q=Acurast%20ACU%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google tokenomics search | [link](https://www.google.com/search?q=Acurast%20ACU%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google docs search | [link](https://www.google.com/search?q=Acurast%20docs) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google governance search | [link](https://www.google.com/search?q=Acurast%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google audit search | [link](https://www.google.com/search?q=Acurast%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/acurast/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/acurast/) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Acurast) | Identity, market, social, contract, or discovery cross-check |
| 26 | Dune token query | [link](https://dune.com/browse/queries?q=Acurast) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Acurast) | Identity, market, social, contract, or discovery cross-check |
| 28 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=ACU) | Identity, market, social, contract, or discovery cross-check |
| 29 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Acurast) | Identity, market, social, contract, or discovery cross-check |
| 30 | CryptoRank search | [link](https://cryptorank.io/price/acurast) | Identity, market, social, contract, or discovery cross-check |
| 31 | RootData search | [link](https://www.rootdata.com/search?kw=Acurast) | Identity, market, social, contract, or discovery cross-check |
| 32 | CoinDesk search | [link](https://www.coindesk.com/search?s=Acurast) | Identity, market, social, contract, or discovery cross-check |
| 33 | The Block search | [link](https://www.theblock.co/search?query=Acurast) | Identity, market, social, contract, or discovery cross-check |
| 34 | GitHub search | [link](https://github.com/search?q=Acurast&type=repositories) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://acurast.com/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Acurast)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Acurast)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Acurast)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Acurast)
- [Messari discovery](https://messari.io/search?q=Acurast)
- [X profile](https://x.com/Acurast)
- [GitHub](https://github.com/Acurast)
- [Telegram](https://t.me/acurastnetwork)
- [Acurast Price: ACU/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/acurast)
- [Tokenomics | Acurast Docs](https://docs.acurast.com/discover/tokenomics/)
- [Token | Acurast Docs](https://docs.acurast.com/discover/acurast-token/)
- [Latest Acurast (ACU) Price Analysis - CoinMarketCap](https://coinmarketcap.com/cmc-ai/acurast/price-analysis/)
- [Updated Acurast Tokenomics — Decentralized Compute, Powered ...](https://acurast.com/blog/network-and-tech/updated-acurast-tokenomics-decentralized-compute-powered-by-acu/)
- [base ACU](https://basescan.org/token/0xc5fed7c8ccc75d8a72b601a66dffd7a489073f0b)
- [ethereum ACU](https://etherscan.io/token/0x216b3643ff8b7bb30d8a48e9f1bd550126202add)
- [bsc ACU](https://bscscan.com/token/0x6ef2ffb38d64afe18ce782da280b300e358cfeaf)
- [Google project search](https://www.google.com/search?q=Acurast%20ACU%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Acurast%20ACU%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Acurast%20docs)
- [Google governance search](https://www.google.com/search?q=Acurast%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Acurast%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/acurast/what-is/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/acurast/)
