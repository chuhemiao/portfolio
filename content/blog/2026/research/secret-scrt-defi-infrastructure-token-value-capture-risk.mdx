---
title: 'Secret / SCRT: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Secret is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'secret-scrt-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# Secret / SCRT: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Secret appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Ethereum, Osmosis, Secret; exchange exposure is BINANCE, BYBIT; category tags are Layer1; current market cap is $20.70M; FDV is $21.29M; and 24h volume is $14.01M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Secret before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 787, market cap $21,603,006, 24h volume $14,175,169. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Secret is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether SCRT captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Ethereum, Osmosis, Secret |
| Exchanges | BINANCE, BYBIT |
| Price | $0.0591 |
| Market cap | $20.70M |
| FDV | $21.29M |
| 24h volume | $14.01M |
| Circulating supply | 350.13M |
| Total supply | 359.99M |
...
Base case: Secret remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning SCRT into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Secret Network is a privacy-focused blockchain platform launched in 2020, enabling customizable privacy for decentralized applications through encrypted smart contracts known as secret contracts. It uses Trusted Execution Environment (TEE) technology to ensure data privacy, allowing secure computation without exposing sensitive information. The network supports interoperability and is designed for industries handling confidential data, such as finance and healthcare.

Surf's project profile maps Secret to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://scrt.network/](https://scrt.network/) |
| X handle | [@SecretNetwork](https://x.com/SecretNetwork) |
| X followers | 184.64K |
| Tags | Layer1 |
| Chains | Ethereum, Osmosis, Secret |
| Exchanges | BINANCE, BYBIT |
...
| Liquidity | $14.01M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 350.13M circulating vs 359.99M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Secret is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Ethereum, Osmosis, Secret |
...
|---|---|---|---|
| Secret | SCRT | secret1k0jntykt7e4g3y88ltc60czgjuqdy4c9e8fzek | not mapped |
| Osmosis | SCRT | ibc/0954E1C28EB7AF5B72D24F3BC2B47BBB2FDF91BDDFD57B74B99E133AED40972A | not mapped |
| ethereum | WSCRT | 0x2b89bf8ba858cd2fcee1fada378d5cd6936968be | [explorer](https://etherscan.io/token/0x2b89bf8ba858cd2fcee1fada378d5cd6936968be) |
| Secret | STKD | secret1k6u0cy4feepm6pehnz804zmwakuwdapm69tuc4 | not mapped |
| Osmosis | STKD | ibc/D0E5BF2940FB58D9B283A339032DE88111407AAD7D94A7F1F3EB78874F8616D4 | not mapped |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Secret, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 787, market cap $21,603,006, 24h volume $14,175,169. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $20.70M, FDV $21.29M, 24h volume $14.01M, and price $0.0591.
...
| 24h volume | $14.01M | Position sizing and exit liquidity proxy |
| Market cap | $20.70M | Liquid valuation proxy |
| FDV | $21.29M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.03x | Higher spread means more supply risk |
| X followers | 184.64K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Secret | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether SCRT is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Secret currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 18.20% and 30d 0.03% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Secret's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2022-02-01 | Undisclosed | $400.00M | Alameda Research, HTX Ventures, NGC Ventures, Fenbushi Capital, Hashed, Arrington Capital, Dragonfly, DeFiance Capital |
| 2021-05-03 | Undisclosed | $11.50M | Arrington Capital, The Spartan Group, Strobe Ventures (Prev. BlockTower Capital), Skynet Trading |

| Team member | Role | Link |
|---|---|---|
| Tor Bair | Founder | [X](https://x.com/torbair) |
...
Secret competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Secret has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Secret is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Secret compounds usage, liquidity improves, and SCRT gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Secret has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
| 24h volume | $14.01M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Secret announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
For Secret, the current liquidity read is watchlist first. The market cap is $20.70M, FDV is $21.29M, reported 24h volume is $14.01M, and FDV / market cap is 1.03x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Secret gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://scrt.network/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Secret) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Secret) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Secret) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Secret) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Secret) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/SecretNetwork) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/SecretFoundation/SecretWebsite) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/SCRTcommunity) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](https://discord.com/invite/SJK32GY) | Identity, market, social, contract, or discovery cross-check |
| 11 | Secret Price: SCRT/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/secret) | Identity, market, social, contract, or discovery cross-check |
| 12 | What Is Secret (SCRT) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/secret/what-is/) | Identity, market, social, contract, or discovery cross-check |
...
| 14 | Secret (SCRT) Price Prediction For 2026 & Beyond - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/secret/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 15 | SCRT to ETH: Secret Price in Ether - CoinGecko | [link](https://www.coingecko.com/en/coins/secret/eth) | Identity, market, social, contract, or discovery cross-check |
| 16 | ethereum WSCRT | [link](https://etherscan.io/token/0x2b89bf8ba858cd2fcee1fada378d5cd6936968be) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google project search | [link](https://www.google.com/search?q=Secret%20SCRT%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google tokenomics search | [link](https://www.google.com/search?q=Secret%20SCRT%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google docs search | [link](https://www.google.com/search?q=Secret%20docs) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google governance search | [link](https://www.google.com/search?q=Secret%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google audit search | [link](https://www.google.com/search?q=Secret%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/secret/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/secret/) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Secret) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune token query | [link](https://dune.com/browse/queries?q=Secret) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Secret) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=SCRT) | Identity, market, social, contract, or discovery cross-check |
| 28 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Secret) | Identity, market, social, contract, or discovery cross-check |
| 29 | CryptoRank search | [link](https://cryptorank.io/price/secret) | Identity, market, social, contract, or discovery cross-check |
| 30 | RootData search | [link](https://www.rootdata.com/search?kw=Secret) | Identity, market, social, contract, or discovery cross-check |
| 31 | CoinDesk search | [link](https://www.coindesk.com/search?s=Secret) | Identity, market, social, contract, or discovery cross-check |
| 32 | The Block search | [link](https://www.theblock.co/search?query=Secret) | Identity, market, social, contract, or discovery cross-check |
| 33 | GitHub search | [link](https://github.com/search?q=Secret&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 34 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=SCRT) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://scrt.network/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Secret)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Secret)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Secret)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Secret)
- [Messari discovery](https://messari.io/search?q=Secret)
- [X profile](https://x.com/SecretNetwork)
...
- [Secret Price: SCRT/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/secret)
- [What Is Secret (SCRT) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/secret/what-is/)
- [CoinGecko: Cryptocurrency Prices, Charts, and Crypto Market Cap](https://www.coingecko.com/)
- [Secret (SCRT) Price Prediction For 2026 & Beyond - CoinMarketCap](https://coinmarketcap.com/cmc-ai/secret/price-prediction/)
- [SCRT to ETH: Secret Price in Ether - CoinGecko](https://www.coingecko.com/en/coins/secret/eth)
- [ethereum WSCRT](https://etherscan.io/token/0x2b89bf8ba858cd2fcee1fada378d5cd6936968be)
- [Google project search](https://www.google.com/search?q=Secret%20SCRT%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Secret%20SCRT%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Secret%20docs)
- [Google governance search](https://www.google.com/search?q=Secret%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Secret%20audit%20report)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/secret/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/secret/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Secret)

## Final Investment View

Secret should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
