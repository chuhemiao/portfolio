---
title: 'Coin98 / C98: DEX / liquidity infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Coin98 is treated as a DEX / liquidity infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'coin98-c98-dex-liquidity-infrastructure-token-value-capture-risk'
category: 'research'
---

# Coin98 / C98: DEX / liquidity infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Coin98 appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Binance Smart Chain, Ethereum, Polygon Pos, Solana, Viction (TomoChain); exchange exposure is BINANCE, COINBASE, BITHUMB, BYBIT, BITGET; category tags are Cross-Chain & Bridge, DEX & AMM, Wallets & AA; current market cap is $13.20M; FDV is $13.20M; and 24h volume is $9.05M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Coin98 before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 1020, market cap $13,332,192, 24h volume $9,110,727. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Coin98 is best understood as a **DEX / liquidity infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether C98 captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Binance Smart Chain, Ethereum, Polygon Pos, Solana, Viction (TomoChain) |
| Exchanges | BINANCE, COINBASE, BITHUMB, BYBIT, BITGET |
| Price | $0.0132 |
| Market cap | $13.20M |
| FDV | $13.20M |
| 24h volume | $9.05M |
...
Base case: Coin98 remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning C98 into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Ninety Eight, formerly Coin98, is a multi-chain Web3 ecosystem providing Super Wallet, Exchange, and cross-chain tools across 100+ blockchains. It supports builders through funding, accelerators, and ecosystem development. The C98 token enables governance, staking rewards, and fee payments.

Surf's project profile maps Coin98 to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.coin98.com/](https://www.coin98.com/) |
| X handle | [@coin98_wallet](https://x.com/coin98_wallet) |
| X followers | 556.57K |
| Tags | Cross-Chain & Bridge, DEX & AMM, Wallets & AA |
| Chains | Binance Smart Chain, Ethereum, Polygon Pos, Solana, Viction (TomoChain) |
| Exchanges | BINANCE, COINBASE, BITHUMB, BYBIT, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Coin98 is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DEX / liquidity infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Coin98 convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $9.05M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 1000.00M circulating vs 1.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Coin98 is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Binance Smart Chain, Ethereum, Polygon Pos, Solana, Viction (TomoChain) |
...
|---|---|---|---|
| solana | C98 | C98A4nkJXhpVZNAZdHUA95RpTF3T4whtQubL3YobiUX9 | [explorer](https://solscan.io/token/C98A4nkJXhpVZNAZdHUA95RpTF3T4whtQubL3YobiUX9) |
| ethereum | C98 | 0xae12c5930881c53715b369cec7606b70d8eb229f | [explorer](https://etherscan.io/token/0xae12c5930881c53715b369cec7606b70d8eb229f) |
| Viction (TomoChain) | C98 | 0x0fd0288aaae91eaf935e2ec14b23486f86516c8c | not mapped |
| polygon | C98 | 0x77f56cf9365955486b12c4816992388ee8606f0e | [explorer](https://polygonscan.com/token/0x77f56cf9365955486b12c4816992388ee8606f0e) |
| bsc | C98 | 0xaec945e04baf28b135fa7c640f624f8d90f1c3a6 | [explorer](https://bscscan.com/token/0xaec945e04baf28b135fa7c640f624f8d90f1c3a6) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Coin98, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 1020, market cap $13,332,192, 24h volume $9,110,727. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $13.20M, FDV $13.20M, 24h volume $9.05M, and price $0.0132.
...
| 24h volume | $9.05M | Position sizing and exit liquidity proxy |
| Market cap | $13.20M | Liquid valuation proxy |
| FDV | $13.20M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.00x | Higher spread means more supply risk |
| X followers | 556.57K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Coin98 | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether C98 is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. Coin98 should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that Coin98 belongs to the DEX / liquidity infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $13.20M, FDV $13.20M, 24h volume $9.05M, price $0.0132, circulating supply 1000.00M, and total supply 1.00B. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For Coin98, the candidate backlog records Seeded 2026-07-06 from Surf market-ranking; rank 1020, market cap $13,332,192, 24h volume $9,110,727. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Binance Smart Chain, Ethereum, Polygon Pos, Solana, Viction (TomoChain), exchanges as BINANCE, COINBASE, BITHUMB, BYBIT, BITGET, and tags as Cross-Chain & Bridge, DEX & AMM, Wallets & AA. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Coin98 currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 4.12% and 30d -7.14% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Coin98's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2023-08-08 | Undisclosed | not disclosed | DWF Labs |
| 2022-01-05 | Strategic | not disclosed | YZi Labs (Prev. Binance Labs) |
| 2021-07-20 | Strategic | $11.25M | NGC Ventures, GBV Capital, Hashed, Moonwhale Ventures, The Spartan Group, AU21 Capital, Kyros Ventures, Magnus Capital |
| 2021-07-16 | Ieo | $3.75M | not disclosed |
| 2021-04-13 | Seed | $1.25M | Hashed, The Spartan Group, Multicoin Capital, ParaFi Capital, Kyber Ventures |
| 2021-04-06 | Strategic | $4.00M | Alameda Research |

| Team member | Role | Link |
|---|---|---|
| Thanh Le | Founder | [X](https://twitter.com/imlethanh98) |
| Vinh The Nguyen | Co-Founder, CEO | [X](https://twitter.com/drofin69) |
| Pham Phuoc Nguyen | Head of Digital | not disclosed |
| Minh Ho | Head of BD | [X](https://x.com/hominh95) |
| Tris Vuong | Head of Brand | [X](https://x.com/thattris0x) |
| Uyen Pham | CMO | not disclosed |
...
Coin98 competes in the DEX / liquidity infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Coin98 has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Coin98 is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Coin98 compounds usage, liquidity improves, and C98 gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Coin98 has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If C98 does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $9.05M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Coin98 announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Coin98 has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Coin98, the first reconciliation job is identity. The token symbol is C98, the category is DEX / liquidity infrastructure, chains are Binance Smart Chain, Ethereum, Polygon Pos, Solana, Viction (TomoChain), exchanges are BINANCE, COINBASE, BITHUMB, BYBIT, BITGET, and tags are Cross-Chain & Bridge, DEX & AMM, Wallets & AA. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If C98 trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $13.20M, FDV is $13.20M, 24h volume is $9.05M, FDV / market cap is 1.00x, 7d price change is 4.12%, and 30d price change is -7.14%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Coin98 / C98 | Match official docs, CG, CMC, Surf, contracts |
| Category | DEX / liquidity infrastructure | Confirm product mechanics and revenue model |
| Chains | Binance Smart Chain, Ethereum, Polygon Pos, Solana, Viction (TomoChain) | Confirm canonical deployments and bridge assumptions |
| Exchanges | BINANCE, COINBASE, BITHUMB, BYBIT, BITGET | Check live order books and inactive pairs |
| Market cap | $13.20M | Compare Surf, CG, CMC, exchange pages |
| FDV | $13.20M | Reconcile total supply and unlock schedule |
| Volume | $9.05M | Check exchange concentration and wash-trading risk |
| TVL | not disclosed | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Coin98 does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Coin98, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does C98 sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
For Coin98, the current liquidity read is watchlist first. The market cap is $13.20M, FDV is $13.20M, reported 24h volume is $9.05M, and FDV / market cap is 1.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Coin98 gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.coin98.com/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Coin98) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Coin98) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Coin98) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Coin98) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Coin98) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/coin98_wallet) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/coin98) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/coin98_wallet) | Identity, market, social, contract, or discovery cross-check |
| 10 | Coin98 Price Chart (C98) - CoinGecko | [link](https://www.coingecko.com/en/coins/coin98) | Identity, market, social, contract, or discovery cross-check |
| 11 | Coin98 Token | [link](https://docs.coin98.com/products/coin98-token) | Identity, market, social, contract, or discovery cross-check |
| 12 | C98 Deep Dive: Token Utility, Recent Developments, and Outlook ... | [link](https://onekey.so/blog/ecosystem/c98-deep-dive-token-utility-recent-developments-and-outlook-for-the-future/?srsltid=AfmBOoqlo13cQtzJaKKW9ZMqxna-3SmFO_Es6rruHX0cGOiGZeJL7-yI) | Identity, market, social, contract, or discovery cross-check |
| 13 | C98 to INR: Coin98 Price in Indian Rupee - CoinGecko | [link](https://www.coingecko.com/en/coins/coin98/inr) | Identity, market, social, contract, or discovery cross-check |
| 14 | What Is Coin98 (C98) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/coin98/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 15 | solana C98 | [link](https://solscan.io/token/C98A4nkJXhpVZNAZdHUA95RpTF3T4whtQubL3YobiUX9) | Identity, market, social, contract, or discovery cross-check |
| 16 | ethereum C98 | [link](https://etherscan.io/token/0xae12c5930881c53715b369cec7606b70d8eb229f) | Identity, market, social, contract, or discovery cross-check |
| 17 | polygon C98 | [link](https://polygonscan.com/token/0x77f56cf9365955486b12c4816992388ee8606f0e) | Identity, market, social, contract, or discovery cross-check |
| 18 | bsc C98 | [link](https://bscscan.com/token/0xaec945e04baf28b135fa7c640f624f8d90f1c3a6) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google project search | [link](https://www.google.com/search?q=Coin98%20C98%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google tokenomics search | [link](https://www.google.com/search?q=Coin98%20C98%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google docs search | [link](https://www.google.com/search?q=Coin98%20docs) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google governance search | [link](https://www.google.com/search?q=Coin98%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 23 | Google audit search | [link](https://www.google.com/search?q=Coin98%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/coin98/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 25 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/coin98/) | Identity, market, social, contract, or discovery cross-check |
| 26 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Coin98) | Identity, market, social, contract, or discovery cross-check |
| 27 | Dune token query | [link](https://dune.com/browse/queries?q=Coin98) | Identity, market, social, contract, or discovery cross-check |
| 28 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Coin98) | Identity, market, social, contract, or discovery cross-check |
| 29 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=C98) | Identity, market, social, contract, or discovery cross-check |
| 30 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Coin98) | Identity, market, social, contract, or discovery cross-check |
| 31 | CryptoRank search | [link](https://cryptorank.io/price/coin98) | Identity, market, social, contract, or discovery cross-check |
| 32 | RootData search | [link](https://www.rootdata.com/search?kw=Coin98) | Identity, market, social, contract, or discovery cross-check |
| 33 | CoinDesk search | [link](https://www.coindesk.com/search?s=Coin98) | Identity, market, social, contract, or discovery cross-check |
| 34 | The Block search | [link](https://www.theblock.co/search?query=Coin98) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.coin98.com/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Coin98)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Coin98)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Coin98)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Coin98)
- [Messari discovery](https://messari.io/search?q=Coin98)
- [X profile](https://x.com/coin98_wallet)
- [GitHub](https://github.com/coin98)
- [Telegram](https://t.me/coin98_wallet)
- [Coin98 Price Chart (C98) - CoinGecko](https://www.coingecko.com/en/coins/coin98)
- [Coin98 Token](https://docs.coin98.com/products/coin98-token)
- [C98 Deep Dive: Token Utility, Recent Developments, and Outlook ...](https://onekey.so/blog/ecosystem/c98-deep-dive-token-utility-recent-developments-and-outlook-for-the-future/?srsltid=AfmBOoqlo13cQtzJaKKW9ZMqxna-3SmFO_Es6rruHX0cGOiGZeJL7-yI)
- [C98 to INR: Coin98 Price in Indian Rupee - CoinGecko](https://www.coingecko.com/en/coins/coin98/inr)
- [What Is Coin98 (C98) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/coin98/what-is/)
- [solana C98](https://solscan.io/token/C98A4nkJXhpVZNAZdHUA95RpTF3T4whtQubL3YobiUX9)
- [ethereum C98](https://etherscan.io/token/0xae12c5930881c53715b369cec7606b70d8eb229f)
- [polygon C98](https://polygonscan.com/token/0x77f56cf9365955486b12c4816992388ee8606f0e)
- [bsc C98](https://bscscan.com/token/0xaec945e04baf28b135fa7c640f624f8d90f1c3a6)
- [Google project search](https://www.google.com/search?q=Coin98%20C98%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Coin98%20C98%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Coin98%20docs)
- [Google governance search](https://www.google.com/search?q=Coin98%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Coin98%20audit%20report)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/coin98/price-analysis/)

## Final Investment View

Coin98 should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
