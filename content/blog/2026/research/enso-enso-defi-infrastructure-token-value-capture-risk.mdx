---
title: 'Enso / ENSO: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Enso is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'enso-enso-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# Enso / ENSO: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Enso appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Binance Smart Chain, Ethereum; exchange exposure is BINANCE, UPBIT, BITHUMB, BYBIT, BITGET; category tags are Developer Tooling, MEV & Intent; current market cap is $13.77M; FDV is $66.87M; and 24h volume is $9.02M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Enso before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 1006, market cap $13,885,107, 24h volume $9,326,879. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Enso is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether ENSO captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
Base case: Enso remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning ENSO into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Enso is a chain-abstraction network unifying blockchains via intents, modular Actions, and reusable Shortcuts for seamless cross-chain smart contract interactions. The live ENSO token enables fees, staking, and governance. Backed by Polychain and Multicoin, it's used by 100+ apps.

Surf's project profile maps Enso to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.enso.build/](https://www.enso.build/) |
| X handle | [@EnsoBuild](https://x.com/EnsoBuild) |
| X followers | 213.56K |
| Tags | Developer Tooling, MEV & Intent |
| Chains | Binance Smart Chain, Ethereum |
| Exchanges | BINANCE, UPBIT, BITHUMB, BYBIT, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Enso is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DeFi infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Enso convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Supply quality | 20.59M circulating vs 100.00M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Enso is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| ethereum | ENSO | 0x699f088b5dddcafb7c4824db5b10b57b37cb0c66 | [explorer](https://etherscan.io/token/0x699f088b5dddcafb7c4824db5b10b57b37cb0c66) |
| bsc | ENSO | 0xfeb339236d25d3e415f280189bc7c2fbab6ae9ef | [explorer](https://bscscan.com/token/0xfeb339236d25d3e415f280189bc7c2fbab6ae9ef) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Enso, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 1006, market cap $13,885,107, 24h volume $9,326,879. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $13.77M, FDV $66.87M, 24h volume $9.02M, and price $0.6687.
...
| Market cap | $13.77M | Liquid valuation proxy |
| FDV | $66.87M | Dilution-aware valuation proxy |
| FDV / market cap spread | 4.86x | Higher spread means more supply risk |
| X followers | 213.56K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Enso | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether ENSO is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Enso currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 1.21% and 30d 20.31% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Enso's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-06-12 | Ico | $5.00M | not disclosed |
| 2024-06-25 | Undisclosed | $4.20M | IDEO CoLab Ventures, Hypersphere Ventures |
| 2021-04-16 | Strategic | $5.00M | The Spartan Group, Multicoin Capital, Polychain Capital, The LAO, P2P Capital, Zora |

| Team member | Role | Link |
|---|---|---|
| Connor Howe | Founder, CEO | [X](https://x.com/connor_enso) |
| Milos Costantini | Co-Founder, Solidity Developer | [X](https://x.com/MilosCostantini) |
| Peter Phillips | Co-Founder, Software Engineer | not disclosed |
...
Enso competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Enso has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Enso is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Enso compounds usage, liquidity improves, and ENSO gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Enso has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If ENSO does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| FDV / market cap | 4.86x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Enso announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Enso has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Enso, the first reconciliation job is identity. The token symbol is ENSO, the category is DeFi infrastructure, chains are Binance Smart Chain, Ethereum, exchanges are BINANCE, UPBIT, BITHUMB, BYBIT, BITGET, and tags are Developer Tooling, MEV & Intent. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If ENSO trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $13.77M, FDV is $66.87M, 24h volume is $9.02M, FDV / market cap is 4.86x, 7d price change is 1.21%, and 30d price change is 20.31%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Enso / ENSO | Match official docs, CG, CMC, Surf, contracts |
...
| Market cap | $13.77M | Compare Surf, CG, CMC, exchange pages |
| FDV | $66.87M | Reconcile total supply and unlock schedule |
...
For Enso, the current liquidity read is watchlist first. The market cap is $13.77M, FDV is $66.87M, reported 24h volume is $9.02M, and FDV / market cap is 4.86x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Enso gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.enso.build/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Enso) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Enso) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Enso) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Enso) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Enso) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/EnsoBuild) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/EnsoBuild) | Identity, market, social, contract, or discovery cross-check |
| 9 | Discord | [link](https://discord.com/invite/enso-build) | Identity, market, social, contract, or discovery cross-check |
| 10 | ENSO/USD Live Price Chart, Market Cap & News Today - CoinGecko | [link](https://www.coingecko.com/en/coins/enso) | Identity, market, social, contract, or discovery cross-check |
| 11 | Enso Finance Price - CoinGecko | [link](https://www.coingecko.com/en/coins/enso-finance) | Identity, market, social, contract, or discovery cross-check |
| 12 | Enso (ENSO) Price Prediction For 2026 & Beyond - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/enso/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 13 | What Is Enso (ENSO) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/enso/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 14 | Tokenomics - Enso Developer Documentation | [link](https://docs.enso.build/pages/network/token) | Identity, market, social, contract, or discovery cross-check |
| 15 | ethereum ENSO | [link](https://etherscan.io/token/0x699f088b5dddcafb7c4824db5b10b57b37cb0c66) | Identity, market, social, contract, or discovery cross-check |
| 16 | bsc ENSO | [link](https://bscscan.com/token/0xfeb339236d25d3e415f280189bc7c2fbab6ae9ef) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google project search | [link](https://www.google.com/search?q=Enso%20ENSO%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google tokenomics search | [link](https://www.google.com/search?q=Enso%20ENSO%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google docs search | [link](https://www.google.com/search?q=Enso%20docs) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google governance search | [link](https://www.google.com/search?q=Enso%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google audit search | [link](https://www.google.com/search?q=Enso%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/enso/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/enso/) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Enso) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune token query | [link](https://dune.com/browse/queries?q=Enso) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Enso) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=ENSO) | Identity, market, social, contract, or discovery cross-check |
| 28 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Enso) | Identity, market, social, contract, or discovery cross-check |
| 29 | CryptoRank search | [link](https://cryptorank.io/price/enso) | Identity, market, social, contract, or discovery cross-check |
| 30 | RootData search | [link](https://www.rootdata.com/search?kw=Enso) | Identity, market, social, contract, or discovery cross-check |
| 31 | CoinDesk search | [link](https://www.coindesk.com/search?s=Enso) | Identity, market, social, contract, or discovery cross-check |
| 32 | The Block search | [link](https://www.theblock.co/search?query=Enso) | Identity, market, social, contract, or discovery cross-check |
| 33 | GitHub search | [link](https://github.com/search?q=Enso&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 34 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=ENSO) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.enso.build/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Enso)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Enso)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Enso)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Enso)
- [Messari discovery](https://messari.io/search?q=Enso)
- [X profile](https://x.com/EnsoBuild)
- [GitHub](https://github.com/EnsoBuild)
- [Discord](https://discord.com/invite/enso-build)
- [ENSO/USD Live Price Chart, Market Cap & News Today - CoinGecko](https://www.coingecko.com/en/coins/enso)
- [Enso Finance Price - CoinGecko](https://www.coingecko.com/en/coins/enso-finance)
- [Enso (ENSO) Price Prediction For 2026 & Beyond - CoinMarketCap](https://coinmarketcap.com/cmc-ai/enso/price-prediction/)
- [What Is Enso (ENSO) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/enso/what-is/)
- [Tokenomics - Enso Developer Documentation](https://docs.enso.build/pages/network/token)
- [ethereum ENSO](https://etherscan.io/token/0x699f088b5dddcafb7c4824db5b10b57b37cb0c66)
- [bsc ENSO](https://bscscan.com/token/0xfeb339236d25d3e415f280189bc7c2fbab6ae9ef)
- [Google project search](https://www.google.com/search?q=Enso%20ENSO%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Enso%20ENSO%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Enso%20docs)
- [Google governance search](https://www.google.com/search?q=Enso%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Enso%20audit%20report)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/enso/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/enso/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Enso)

## Final Investment View

Enso should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
