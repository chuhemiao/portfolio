---
title: 'iExec RLC / RLC: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'iExec RLC is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'iexec-rlc-rlc-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# iExec RLC / RLC: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** iExec RLC appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Arbitrum One, Energi, Ethereum, SORA; exchange exposure is BINANCE, UPBIT, COINBASE, BITHUMB, BITGET; category tags are DePIN (Physical Infra), Storage & Compute; current market cap is $21.91M; FDV is $26.33M; and 24h volume is $1.16M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag iExec RLC before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 788, market cap $21,506,513, 24h volume $1,101,325. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

iExec RLC is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether RLC captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Arbitrum One, Energi, Ethereum, SORA |
...
Base case: iExec RLC remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning RLC into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

iExec (RLC) is a decentralized cloud-computing protocol offering confidential off‑chain execution via trusted execution environments (TEEs) and a decentralized CPU/GPU marketplace for privacy-preserving compute. Its RLC token powers an asset-based economy connecting buyers and sellers of compute, enabling developers to build and monetize privacy-first dApps across AI, DePIN, DeFi, and RWA.

Surf's project profile maps iExec RLC to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [http://iex.ec/](http://iex.ec/) |
| X handle | [@iEx_ec](https://x.com/iEx_ec) |
| X followers | 94.53K |
...
| Supply quality | 72.38M circulating vs 87.00M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that iExec RLC is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

iExec RLC's architecture should be evaluated through the job it performs. For a DeFi infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
| Settlement layer | Chains and contracts that hold user value | Arbitrum One, Energi, Ethereum, SORA |
...
|---|---|---|---|
| SORA | RLC | 0x008294f7b08f568a661de2b248c34fc574e7e0012a12ef7959eb1a5c6b349e09 | not mapped |
| Energi | RLC | 0xb4ff17b5e93c40ff09326b0d538118022f02dc2b | not mapped |
| ethereum | RLC | 0x607f4c5bb672230e8672085532f7e901544a7375 | [explorer](https://etherscan.io/token/0x607f4c5bb672230e8672085532f7e901544a7375) |
| arbitrum | RLC | 0xe649e6a1f2afc63ca268c2363691cecaf75cf47c | [explorer](https://arbiscan.io/token/0xe649e6a1f2afc63ca268c2363691cecaf75cf47c) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For iExec RLC, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 788, market cap $21,506,513, 24h volume $1,101,325. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $21.91M, FDV $26.33M, 24h volume $1.16M, and price $0.3027.
...
| Market cap | $21.91M | Liquid valuation proxy |
| FDV | $26.33M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.20x | Higher spread means more supply risk |
| X followers | 94.53K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for iExec RLC | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether RLC is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. iExec RLC should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that iExec RLC belongs to the DeFi infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $21.91M, FDV $26.33M, 24h volume $1.16M, price $0.3027, circulating supply 72.38M, and total supply 87.00M. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For iExec RLC, the candidate backlog records Seeded 2026-07-06 from Surf market-ranking; rank 788, market cap $21,506,513, 24h volume $1,101,325. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Arbitrum One, Energi, Ethereum, SORA, exchanges as BINANCE, UPBIT, COINBASE, BITHUMB, BITGET, and tags as DePIN (Physical Infra), Storage & Compute. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. iExec RLC currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 3.69% and 30d -10.62% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that iExec RLC's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2017-04-25 | Undisclosed | $12.00M | not disclosed |

| Team member | Role | Link |
|---|---|---|
| Haiwu He | Co Founder | [X](https://x.com/hehaiwu) |
| Gilles Fedak | Co-Founder, CEO | [X](https://x.com/gilfedak) |
...
iExec RLC competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether iExec RLC has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: iExec RLC is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | iExec RLC compounds usage, liquidity improves, and RLC gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If RLC does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
1. Re-run Surf project-detail and project-defi-metrics if iExec RLC announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. iExec RLC has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For iExec RLC, the first reconciliation job is identity. The token symbol is RLC, the category is DeFi infrastructure, chains are Arbitrum One, Energi, Ethereum, SORA, exchanges are BINANCE, UPBIT, COINBASE, BITHUMB, BITGET, and tags are DePIN (Physical Infra), Storage & Compute. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If RLC trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $21.91M, FDV is $26.33M, 24h volume is $1.16M, FDV / market cap is 1.20x, 7d price change is 3.69%, and 30d price change is -10.62%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | iExec RLC / RLC | Match official docs, CG, CMC, Surf, contracts |
| Category | DeFi infrastructure | Confirm product mechanics and revenue model |
| Chains | Arbitrum One, Energi, Ethereum, SORA | Confirm canonical deployments and bridge assumptions |
| Exchanges | BINANCE, UPBIT, COINBASE, BITHUMB, BITGET | Check live order books and inactive pairs |
| Market cap | $21.91M | Compare Surf, CG, CMC, exchange pages |
| FDV | $26.33M | Reconcile total supply and unlock schedule |
...
For iExec RLC, the current liquidity read is watchlist first. The market cap is $21.91M, FDV is $26.33M, reported 24h volume is $1.16M, and FDV / market cap is 1.20x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. iExec RLC can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.

For iExec RLC, the live monitoring stack should include contract events, TVL, volume, governance, documentation changes, social announcements, security disclosures, and exchange-market quality. A single metric is not enough. TVL may stay high while liquidity thins. Volume may rise because of volatility rather than adoption. Social attention may rise around controversy rather than progress. Governance may pass proposals that improve short-term price but add long-term risk. The strongest monitoring dashboard combines on-chain state, off-chain disclosure, and market quality.

Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If iExec RLC suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | iExec RLC gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](http://iex.ec/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=iExec%20RLC) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=iExec%20RLC) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=iExec%20RLC) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=iExec%20RLC) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=iExec%20RLC) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/iEx_ec) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/iExecBlockchainComputing) | Identity, market, social, contract, or discovery cross-check |
| 9 | iExec RLC Price: RLC/USD Live Price Chart, Market Cap & ... | [link](https://www.coingecko.com/en/coins/iexec-rlc) | Identity, market, social, contract, or discovery cross-check |
| 10 | What Is iExec RLC (RLC) And How Does It Work? | [link](https://coinmarketcap.com/cmc-ai/rlc/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 11 | iExec RLC (RLC) Price Prediction For 2026 & Beyond | [link](https://coinmarketcap.com/cmc-ai/rlc/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 12 | RLC to EUR: iExec RLC Price in Euro | [link](https://www.coingecko.com/en/coins/iexec-rlc/eur) | Identity, market, social, contract, or discovery cross-check |
| 13 | iExec RLC Token Price is $0.3 today. See ... | [link](https://metamask.io/price/iexec-rlc) | Identity, market, social, contract, or discovery cross-check |
| 14 | ethereum RLC | [link](https://etherscan.io/token/0x607f4c5bb672230e8672085532f7e901544a7375) | Identity, market, social, contract, or discovery cross-check |
| 15 | arbitrum RLC | [link](https://arbiscan.io/token/0xe649e6a1f2afc63ca268c2363691cecaf75cf47c) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google project search | [link](https://www.google.com/search?q=iExec%20RLC%20RLC%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google tokenomics search | [link](https://www.google.com/search?q=iExec%20RLC%20RLC%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google docs search | [link](https://www.google.com/search?q=iExec%20RLC%20docs) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google governance search | [link](https://www.google.com/search?q=iExec%20RLC%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google audit search | [link](https://www.google.com/search?q=iExec%20RLC%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/iexec-rlc/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/iexec-rlc/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/iexec-rlc/) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune discovery | [link](https://dune.com/browse/dashboards?q=iExec%20RLC) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune token query | [link](https://dune.com/browse/queries?q=iExec%20RLC) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=iExec%20RLC) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=RLC) | Identity, market, social, contract, or discovery cross-check |
| 28 | TokenUnlocks search | [link](https://token.unlocks.app/?search=iExec%20RLC) | Identity, market, social, contract, or discovery cross-check |
| 29 | CryptoRank search | [link](https://cryptorank.io/price/iexec-rlc) | Identity, market, social, contract, or discovery cross-check |
| 30 | RootData search | [link](https://www.rootdata.com/search?kw=iExec%20RLC) | Identity, market, social, contract, or discovery cross-check |
| 31 | CoinDesk search | [link](https://www.coindesk.com/search?s=iExec%20RLC) | Identity, market, social, contract, or discovery cross-check |
| 32 | The Block search | [link](https://www.theblock.co/search?query=iExec%20RLC) | Identity, market, social, contract, or discovery cross-check |
| 33 | GitHub search | [link](https://github.com/search?q=iExec%20RLC&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 34 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=RLC) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](http://iex.ec/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=iExec%20RLC)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=iExec%20RLC)
- [DefiLlama protocol discovery](https://defillama.com/search?query=iExec%20RLC)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=iExec%20RLC)
- [Messari discovery](https://messari.io/search?q=iExec%20RLC)
- [X profile](https://x.com/iEx_ec)
- [GitHub](https://github.com/iExecBlockchainComputing)
- [iExec RLC Price: RLC/USD Live Price Chart, Market Cap & ...](https://www.coingecko.com/en/coins/iexec-rlc)
- [What Is iExec RLC (RLC) And How Does It Work?](https://coinmarketcap.com/cmc-ai/rlc/what-is/)
- [iExec RLC (RLC) Price Prediction For 2026 & Beyond](https://coinmarketcap.com/cmc-ai/rlc/price-prediction/)
- [RLC to EUR: iExec RLC Price in Euro](https://www.coingecko.com/en/coins/iexec-rlc/eur)
- [iExec RLC Token Price is $0.3 today. See ...](https://metamask.io/price/iexec-rlc)
- [ethereum RLC](https://etherscan.io/token/0x607f4c5bb672230e8672085532f7e901544a7375)
- [arbitrum RLC](https://arbiscan.io/token/0xe649e6a1f2afc63ca268c2363691cecaf75cf47c)
- [Google project search](https://www.google.com/search?q=iExec%20RLC%20RLC%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=iExec%20RLC%20RLC%20tokenomics)
- [Google docs search](https://www.google.com/search?q=iExec%20RLC%20docs)
- [Google governance search](https://www.google.com/search?q=iExec%20RLC%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=iExec%20RLC%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/iexec-rlc/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/iexec-rlc/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/iexec-rlc/)
- [Dune discovery](https://dune.com/browse/dashboards?q=iExec%20RLC)

## Final Investment View

iExec RLC should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
