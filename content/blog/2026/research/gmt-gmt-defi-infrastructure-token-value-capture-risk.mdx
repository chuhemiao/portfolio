---
title: 'GMT / GMT: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'GMT is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'gmt-gmt-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# GMT / GMT: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** GMT appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Binance Smart Chain, Ethereum, Polygon Pos, Solana; exchange exposure is BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BYBIT, BITGET; category tags are GameFi, SocialFi; current market cap is $24.83M; FDV is $40.48M; and 24h volume is $11.67M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag GMT before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 737, market cap $24,335,593, 24h volume $11,312,129. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

GMT is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether GMT captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Binance Smart Chain, Ethereum, Polygon Pos, Solana |
| Exchanges | BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BYBIT, BITGET |
| Price | $0.0080 |
| Market cap | $24.83M |
| FDV | $40.48M |
| 24h volume | $11.67M |
| Circulating supply | 3.11B |
| Total supply | 5.07B |
...
Base case: GMT remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning GMT into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

STEPN is a move-to-earn Web3 fitness app that rewards walking, jogging and running through NFT “sneakers” and token rewards. Its primary governance token is GMT (with in-app utility token GST), and the platform includes a mobile app, NFT marketplace, and play-to-earn mechanics developed by Find Satoshi Lab.

Surf's project profile maps GMT to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.stepn.com](https://www.stepn.com) |
| X handle | [@Stepnofficial](https://x.com/Stepnofficial) |
| X followers | 572.66K |
| Tags | GameFi, SocialFi |
| Chains | Binance Smart Chain, Ethereum, Polygon Pos, Solana |
| Exchanges | BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BYBIT, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether GMT is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DeFi infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does GMT convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $11.67M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 3.11B circulating vs 5.07B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that GMT is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Binance Smart Chain, Ethereum, Polygon Pos, Solana |
...
|---|---|---|---|
| polygon | GMT | 0x714db550b574b3e927af3d93e26127d15721d4c2 | [explorer](https://polygonscan.com/token/0x714db550b574b3e927af3d93e26127d15721d4c2) |
| bsc | GMT | 0x3019bf2a2ef8040c242c9a4c5c4bd4c81678b2a1 | [explorer](https://bscscan.com/token/0x3019bf2a2ef8040c242c9a4c5c4bd4c81678b2a1) |
| solana | GMT | 7i5KKsX2weiTkry7jA4ZwSuXGhs5eJBEjY8vVxR4pfRx | [explorer](https://solscan.io/token/7i5KKsX2weiTkry7jA4ZwSuXGhs5eJBEjY8vVxR4pfRx) |
| ethereum | GMT | 0xe3c408bd53c31c085a1746af401a4042954ff740 | [explorer](https://etherscan.io/token/0xe3c408bd53c31c085a1746af401a4042954ff740) |
| solana | GST-SOL | AFbX8oGjGpmVFywbVouvhQSRmiW2aR1mohfahi4Y2AdB | [explorer](https://solscan.io/token/AFbX8oGjGpmVFywbVouvhQSRmiW2aR1mohfahi4Y2AdB) |
| bsc | GST-BSC | 0x4a2c860cec6471b9f5f5a336eb4f38bb21683c98 | [explorer](https://bscscan.com/token/0x4a2c860cec6471b9f5f5a336eb4f38bb21683c98) |
| polygon | GGT | 0x8349314651ede274f8c5fef01aa65ff8da75e57c | [explorer](https://polygonscan.com/token/0x8349314651ede274f8c5fef01aa65ff8da75e57c) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For GMT, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 737, market cap $24,335,593, 24h volume $11,312,129. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $24.83M, FDV $40.48M, 24h volume $11.67M, and price $0.0080.
...
| 24h volume | $11.67M | Position sizing and exit liquidity proxy |
| Market cap | $24.83M | Liquid valuation proxy |
| FDV | $40.48M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.63x | Higher spread means more supply risk |
| X followers | 572.66K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for GMT | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether GMT is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. GMT currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 4.70% and 30d 0.92% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that GMT's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2022-03-02 | Ieo | $4.20M | not disclosed |
| 2022-01-20 | Seed | $5.00M | Morningstar Ventures, Spark Digital Capital, Sequoia Capital, Solana Ventures, Zee Prime Capital, Lemniscap, Sfermion, 6th Man Ventures |

| Team member | Role | Link |
|---|---|---|
| Jerry Huang | Founder | not disclosed |
| Yawn Rong | Co-Founder | [X](https://twitter.com/yawn_rong) |
| Shiti Rastogi Manghani | CEO | not disclosed |
...
GMT competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether GMT has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: GMT is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | GMT compounds usage, liquidity improves, and GMT gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because GMT has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If GMT does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $11.67M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if GMT announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
For GMT, the current liquidity read is watchlist first. The market cap is $24.83M, FDV is $40.48M, reported 24h volume is $11.67M, and FDV / market cap is 1.63x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | GMT gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.stepn.com) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=GMT) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=GMT) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=GMT) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=GMT) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=GMT) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/Stepnofficial) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/STEPNofficial) | Identity, market, social, contract, or discovery cross-check |
| 9 | Discord | [link](https://discord.com/invite/stepn) | Identity, market, social, contract, or discovery cross-check |
| 10 | GMT/USD Live Price Chart, Market Cap & News Today - CoinGecko | [link](https://www.coingecko.com/en/coins/stepn) | Identity, market, social, contract, or discovery cross-check |
| 11 | What Is GMT (GMT) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/green-metaverse-token/what-is/) | Identity, market, social, contract, or discovery cross-check |
...
| 13 | GMT (GMT) Price Prediction For 2026 & Beyond - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/green-metaverse-token/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 14 | Latest GMT (GMT) Price Analysis - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/green-metaverse-token/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 15 | polygon GMT | [link](https://polygonscan.com/token/0x714db550b574b3e927af3d93e26127d15721d4c2) | Identity, market, social, contract, or discovery cross-check |
| 16 | bsc GMT | [link](https://bscscan.com/token/0x3019bf2a2ef8040c242c9a4c5c4bd4c81678b2a1) | Identity, market, social, contract, or discovery cross-check |
| 17 | solana GMT | [link](https://solscan.io/token/7i5KKsX2weiTkry7jA4ZwSuXGhs5eJBEjY8vVxR4pfRx) | Identity, market, social, contract, or discovery cross-check |
| 18 | ethereum GMT | [link](https://etherscan.io/token/0xe3c408bd53c31c085a1746af401a4042954ff740) | Identity, market, social, contract, or discovery cross-check |
| 19 | solana GST-SOL | [link](https://solscan.io/token/AFbX8oGjGpmVFywbVouvhQSRmiW2aR1mohfahi4Y2AdB) | Identity, market, social, contract, or discovery cross-check |
| 20 | bsc GST-BSC | [link](https://bscscan.com/token/0x4a2c860cec6471b9f5f5a336eb4f38bb21683c98) | Identity, market, social, contract, or discovery cross-check |
| 21 | polygon GGT | [link](https://polygonscan.com/token/0x8349314651ede274f8c5fef01aa65ff8da75e57c) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google project search | [link](https://www.google.com/search?q=GMT%20GMT%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 23 | Google tokenomics search | [link](https://www.google.com/search?q=GMT%20GMT%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 24 | Google docs search | [link](https://www.google.com/search?q=GMT%20docs) | Identity, market, social, contract, or discovery cross-check |
| 25 | Google governance search | [link](https://www.google.com/search?q=GMT%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 26 | Google audit search | [link](https://www.google.com/search?q=GMT%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 27 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/gmt/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 28 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/gmt/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 29 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/gmt/) | Identity, market, social, contract, or discovery cross-check |
| 30 | Dune discovery | [link](https://dune.com/browse/dashboards?q=GMT) | Identity, market, social, contract, or discovery cross-check |
| 31 | Dune token query | [link](https://dune.com/browse/queries?q=GMT) | Identity, market, social, contract, or discovery cross-check |
| 32 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=GMT) | Identity, market, social, contract, or discovery cross-check |
| 33 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=GMT) | Identity, market, social, contract, or discovery cross-check |
| 34 | TokenUnlocks search | [link](https://token.unlocks.app/?search=GMT) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.stepn.com)
- [CoinGecko search](https://www.coingecko.com/en/search?query=GMT)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=GMT)
- [DefiLlama protocol discovery](https://defillama.com/search?query=GMT)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=GMT)
- [Messari discovery](https://messari.io/search?q=GMT)
- [X profile](https://x.com/Stepnofficial)
- [Telegram](https://t.me/STEPNofficial)
- [Discord](https://discord.com/invite/stepn)
- [GMT/USD Live Price Chart, Market Cap & News Today - CoinGecko](https://www.coingecko.com/en/coins/stepn)
- [What Is GMT (GMT) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/green-metaverse-token/what-is/)
- [CoinGecko: Cryptocurrency Prices, Charts, and Crypto Market Cap](https://www.coingecko.com/)
- [GMT (GMT) Price Prediction For 2026 & Beyond - CoinMarketCap](https://coinmarketcap.com/cmc-ai/green-metaverse-token/price-prediction/)
- [Latest GMT (GMT) Price Analysis - CoinMarketCap](https://coinmarketcap.com/cmc-ai/green-metaverse-token/price-analysis/)
- [polygon GMT](https://polygonscan.com/token/0x714db550b574b3e927af3d93e26127d15721d4c2)
- [bsc GMT](https://bscscan.com/token/0x3019bf2a2ef8040c242c9a4c5c4bd4c81678b2a1)
- [solana GMT](https://solscan.io/token/7i5KKsX2weiTkry7jA4ZwSuXGhs5eJBEjY8vVxR4pfRx)
- [ethereum GMT](https://etherscan.io/token/0xe3c408bd53c31c085a1746af401a4042954ff740)
- [solana GST-SOL](https://solscan.io/token/AFbX8oGjGpmVFywbVouvhQSRmiW2aR1mohfahi4Y2AdB)
- [bsc GST-BSC](https://bscscan.com/token/0x4a2c860cec6471b9f5f5a336eb4f38bb21683c98)
- [polygon GGT](https://polygonscan.com/token/0x8349314651ede274f8c5fef01aa65ff8da75e57c)
- [Google project search](https://www.google.com/search?q=GMT%20GMT%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=GMT%20GMT%20tokenomics)
- [Google docs search](https://www.google.com/search?q=GMT%20docs)

## Final Investment View

GMT should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
