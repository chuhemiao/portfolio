---
title: 'Delysium / AGI: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Delysium is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'delysium-agi-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# Delysium / AGI: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Delysium appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Binance Smart Chain, Ethereum, Solana; exchange exposure is BINANCE, BITHUMB, BYBIT; category tags are Developer Tooling, Web3 AI (Agent/AIGC); current market cap is $11.05M; FDV is $12.82M; and 24h volume is $685.28K. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Delysium before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 1117, market cap $11,055,364, 24h volume $643,327. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Delysium is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether AGI captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Exchanges | BINANCE, BITHUMB, BYBIT |
| Price | $0.0043 |
| Market cap | $11.05M |
| FDV | $12.82M |
| 24h volume | $685.28K |
...
Base case: Delysium remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning AGI into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Delysium builds a blockchain-based AI Agent Network (YKILY) and the Lucy AI OS to enable autonomous agent-to-agent interactions, identity (Agent‑ID) and an immutable decision log (Chronicle). Its native token AGI powers staking, incentives, governance and in-network/game economies, with multi-chain token contracts and a published GitBook whitepaper.

Surf's project profile maps Delysium to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.delysium.com/](https://www.delysium.com/) |
| X handle | [@The_Delysium](https://x.com/The_Delysium) |
| X followers | 329.90K |
...
| Liquidity | $685.28K 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 2.59B circulating vs 3.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Delysium is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

Delysium's architecture should be evaluated through the job it performs. For a DeFi infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
|---|---|---|---|
| solana | AGI | 8bUbe1ujsM1G3JEbBWVVCXa2widmuPdKUB2rGKMYFw7R | [explorer](https://solscan.io/token/8bUbe1ujsM1G3JEbBWVVCXa2widmuPdKUB2rGKMYFw7R) |
| ethereum | AGI | 0x7da2641000cbb407c329310c461b2cb9c70c3046 | [explorer](https://etherscan.io/token/0x7da2641000cbb407c329310c461b2cb9c70c3046) |
| bsc | AGI | 0x818835503f55283cd51a4399f595e295a9338753 | [explorer](https://bscscan.com/token/0x818835503f55283cd51a4399f595e295a9338753) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Delysium, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 1117, market cap $11,055,364, 24h volume $643,327. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $11.05M, FDV $12.82M, 24h volume $685.28K, and price $0.0043.
...
| 24h volume | $685.28K | Position sizing and exit liquidity proxy |
| Market cap | $11.05M | Liquid valuation proxy |
| FDV | $12.82M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.16x | Higher spread means more supply risk |
| X followers | 329.90K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Delysium | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether AGI is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Delysium currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 2.75% and 30d -20.59% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Delysium's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2023-06-27 | Strategic | not disclosed | P2 Ventures (Polygon Ventures) |
| 2023-04-10 | Ieo | $20.00K | not disclosed |
| 2022-10-17 | Strategic | $10.00M | GSR, Formless Capital, Antalpha Global, Anthos Capital, Immutable, Blockchain Coinvestors, Leonis Capital, Perion |
| 2022-03-14 | Undisclosed | $4.00M | Alameda Research, Infinity Ventures Crypto (IVC), Republic, Galaxy Interactive, LucidBlue Ventures, Tess Ventures, Mask Network, Ancient8 |
| not disclosed | Strategic | not disclosed | not disclosed |

| Team member | Role | Link |
|---|---|---|
| Wu Xiankun | Co-Founder | not disclosed |
| Yan Zhang | Co-Founder & CTO | not disclosed |
...
Delysium competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Delysium has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Delysium is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Delysium compounds usage, liquidity improves, and AGI gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If AGI does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $685.28K | Weekly | Volume below position-size threshold |
| FDV / market cap | 1.16x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Delysium announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Delysium has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Delysium, the first reconciliation job is identity. The token symbol is AGI, the category is DeFi infrastructure, chains are Binance Smart Chain, Ethereum, Solana, exchanges are BINANCE, BITHUMB, BYBIT, and tags are Developer Tooling, Web3 AI (Agent/AIGC). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If AGI trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $11.05M, FDV is $12.82M, 24h volume is $685.28K, FDV / market cap is 1.16x, 7d price change is 2.75%, and 30d price change is -20.59%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Delysium / AGI | Match official docs, CG, CMC, Surf, contracts |
...
| Exchanges | BINANCE, BITHUMB, BYBIT | Check live order books and inactive pairs |
| Market cap | $11.05M | Compare Surf, CG, CMC, exchange pages |
| FDV | $12.82M | Reconcile total supply and unlock schedule |
| Volume | $685.28K | Check exchange concentration and wash-trading risk |
| TVL | not disclosed | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Delysium does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Delysium, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does AGI sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
For Delysium, the current liquidity read is watchlist first. The market cap is $11.05M, FDV is $12.82M, reported 24h volume is $685.28K, and FDV / market cap is 1.16x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. Delysium can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.
...
| Peer comparison | Monthly | Delysium gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.delysium.com/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Delysium) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Delysium) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Delysium) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Delysium) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Delysium) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/The_Delysium) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/TheDelysium) | Identity, market, social, contract, or discovery cross-check |
| 9 | Discord | [link](https://discord.com/invite/YQtKDGTU8U) | Identity, market, social, contract, or discovery cross-check |
| 10 | Delysium Price: AGI/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/delysium) | Identity, market, social, contract, or discovery cross-check |
| 11 | Latest Delysium (AGI) Price Analysis - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/delysium/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 12 | AGI to BTC: Delysium Price in Bitcoin - CoinGecko | [link](https://www.coingecko.com/en/coins/delysium/btc) | Identity, market, social, contract, or discovery cross-check |
| 13 | AGI to LKR: Delysium Price in Sri Lankan Rupee / CoinGecko | [link](https://www.coingecko.com/en/coins/delysium/lkr) | Identity, market, social, contract, or discovery cross-check |
| 14 | Delysium (AGI) Price Prediction For 2026 & Beyond - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/delysium/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 15 | solana AGI | [link](https://solscan.io/token/8bUbe1ujsM1G3JEbBWVVCXa2widmuPdKUB2rGKMYFw7R) | Identity, market, social, contract, or discovery cross-check |
| 16 | ethereum AGI | [link](https://etherscan.io/token/0x7da2641000cbb407c329310c461b2cb9c70c3046) | Identity, market, social, contract, or discovery cross-check |
| 17 | bsc AGI | [link](https://bscscan.com/token/0x818835503f55283cd51a4399f595e295a9338753) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google project search | [link](https://www.google.com/search?q=Delysium%20AGI%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google tokenomics search | [link](https://www.google.com/search?q=Delysium%20AGI%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google docs search | [link](https://www.google.com/search?q=Delysium%20docs) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google governance search | [link](https://www.google.com/search?q=Delysium%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google audit search | [link](https://www.google.com/search?q=Delysium%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/delysium/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/delysium/) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Delysium) | Identity, market, social, contract, or discovery cross-check |
| 26 | Dune token query | [link](https://dune.com/browse/queries?q=Delysium) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Delysium) | Identity, market, social, contract, or discovery cross-check |
| 28 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=AGI) | Identity, market, social, contract, or discovery cross-check |
| 29 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Delysium) | Identity, market, social, contract, or discovery cross-check |
| 30 | CryptoRank search | [link](https://cryptorank.io/price/delysium) | Identity, market, social, contract, or discovery cross-check |
| 31 | RootData search | [link](https://www.rootdata.com/search?kw=Delysium) | Identity, market, social, contract, or discovery cross-check |
| 32 | CoinDesk search | [link](https://www.coindesk.com/search?s=Delysium) | Identity, market, social, contract, or discovery cross-check |
| 33 | The Block search | [link](https://www.theblock.co/search?query=Delysium) | Identity, market, social, contract, or discovery cross-check |
| 34 | GitHub search | [link](https://github.com/search?q=Delysium&type=repositories) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.delysium.com/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Delysium)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Delysium)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Delysium)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Delysium)
- [Messari discovery](https://messari.io/search?q=Delysium)
- [X profile](https://x.com/The_Delysium)
- [Telegram](https://t.me/TheDelysium)
- [Discord](https://discord.com/invite/YQtKDGTU8U)
- [Delysium Price: AGI/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/delysium)
- [Latest Delysium (AGI) Price Analysis - CoinMarketCap](https://coinmarketcap.com/cmc-ai/delysium/price-analysis/)
- [AGI to BTC: Delysium Price in Bitcoin - CoinGecko](https://www.coingecko.com/en/coins/delysium/btc)
- [AGI to LKR: Delysium Price in Sri Lankan Rupee | CoinGecko](https://www.coingecko.com/en/coins/delysium/lkr)
- [Delysium (AGI) Price Prediction For 2026 & Beyond - CoinMarketCap](https://coinmarketcap.com/cmc-ai/delysium/price-prediction/)
- [solana AGI](https://solscan.io/token/8bUbe1ujsM1G3JEbBWVVCXa2widmuPdKUB2rGKMYFw7R)
- [ethereum AGI](https://etherscan.io/token/0x7da2641000cbb407c329310c461b2cb9c70c3046)
- [bsc AGI](https://bscscan.com/token/0x818835503f55283cd51a4399f595e295a9338753)
- [Google project search](https://www.google.com/search?q=Delysium%20AGI%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Delysium%20AGI%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Delysium%20docs)
- [Google governance search](https://www.google.com/search?q=Delysium%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Delysium%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/delysium/what-is/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/delysium/)

## Final Investment View

Delysium should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
