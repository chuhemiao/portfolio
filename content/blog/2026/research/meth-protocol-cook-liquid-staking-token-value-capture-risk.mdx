---
title: 'mETH Protocol / COOK: Liquid staking value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'mETH Protocol is treated as a Liquid staking watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'meth-protocol-cook-liquid-staking-token-value-capture-risk'
category: 'research'
---

# mETH Protocol / COOK: Liquid staking value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** mETH Protocol appears in the local CoinGecko/CMC-oriented candidate pool through [CoinGecko](https://www.coingecko.com/en/coins/meth-protocol) and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Ethereum, HyperEVM, Mantle; exchange exposure is BYBIT, HYPERLIQUID; category tags are LSD & Restaking; current market cap is $2.55M; FDV is $13.26M; and 24h volume is $110.11K. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag mETH Protocol before this file was created, and the existing candidate note says: DefiLlama category Liquid Staking on Ethereum; TVL $360,958,377. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

mETH Protocol is best understood as a **Liquid staking** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether COOK captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Ethereum, HyperEVM, Mantle |
| Exchanges | BYBIT, HYPERLIQUID |
| Price | $0.0027 |
| Market cap | $2.55M |
| FDV | $13.26M |
| 24h volume | $110.11K |
| Circulating supply | 960.00M |
| Total supply | 5.00B |
...
Base case: mETH Protocol remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning COOK into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

mETH Protocol is a permissionless, non-custodial ETH liquid staking and liquid restaking protocol built under Mantle. It issues mETH (value-accumulating ERC-20 receipt token), cmETH (1:1 restaking receipt), and COOK governance, enabling composable, omnichain staking and restaking across EigenLayer, Symbiotic, Karak and DeFi integrations.

Surf's project profile maps mETH Protocol to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.methprotocol.xyz/](https://www.methprotocol.xyz/) |
| X handle | [@mETHProtocol](https://x.com/mETHProtocol) |
| X followers | 30.41K |
| Tags | LSD & Restaking |
| Chains | Ethereum, HyperEVM, Mantle |
| Exchanges | BYBIT, HYPERLIQUID |
| TGE status | post |
| CoinGecko ID | [meth-protocol](https://www.coingecko.com/en/coins/meth-protocol) |

The main product question is whether mETH Protocol is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For Liquid staking projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does mETH Protocol convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $110.11K 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 960.00M circulating vs 5.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that mETH Protocol is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

mETH Protocol's architecture should be evaluated through the job it performs. For a Liquid staking project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
| Settlement layer | Chains and contracts that hold user value | Ethereum, HyperEVM, Mantle |
...
|---|---|---|---|
| Mantle | COOK | 0x9f0c013016e8656bc256f948cd4b79ab25c7b94d | not mapped |
| ethereum | COOK | 0x9f0c013016e8656bc256f948cd4b79ab25c7b94d | [explorer](https://etherscan.io/token/0x9f0c013016e8656bc256f948cd4b79ab25c7b94d) |
| ethereum | CMETH | 0xe6829d9a7ee3040e1276fa75293bde931859e8fa | [explorer](https://etherscan.io/token/0xe6829d9a7ee3040e1276fa75293bde931859e8fa) |
| HyperEVM | CMETH | 0xe6829d9a7ee3040e1276fa75293bde931859e8fa | not mapped |
| Mantle | CMETH | 0xe6829d9a7ee3040e1276fa75293bde931859e8fa | not mapped |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For mETH Protocol, the DeFi-oriented backlog note is the first signal: DefiLlama category Liquid Staking on Ethereum; TVL $360,958,377. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $2.55M, FDV $13.26M, 24h volume $110.11K, and price $0.0027.
...
| 24h volume | $110.11K | Position sizing and exit liquidity proxy |
| Market cap | $2.55M | Liquid valuation proxy |
| FDV | $13.26M | Dilution-aware valuation proxy |
| FDV / market cap spread | 5.21x | Higher spread means more supply risk |
| X followers | 30.41K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for mETH Protocol | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether COOK is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. mETH Protocol currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d -0.28% and 30d -2.92% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that mETH Protocol's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
mETH Protocol competes in the Liquid staking bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether mETH Protocol has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: mETH Protocol is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | mETH Protocol compounds usage, liquidity improves, and COOK gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If COOK does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $110.11K | Weekly | Volume below position-size threshold |
| FDV / market cap | 5.21x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if mETH Protocol announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. mETH Protocol has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For mETH Protocol, the first reconciliation job is identity. The token symbol is COOK, the category is Liquid staking, chains are Ethereum, HyperEVM, Mantle, exchanges are BYBIT, HYPERLIQUID, and tags are LSD & Restaking. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If COOK trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $2.55M, FDV is $13.26M, 24h volume is $110.11K, FDV / market cap is 5.21x, 7d price change is -0.28%, and 30d price change is -2.92%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | mETH Protocol / COOK | Match official docs, CG, CMC, Surf, contracts |
| Category | Liquid staking | Confirm product mechanics and revenue model |
| Chains | Ethereum, HyperEVM, Mantle | Confirm canonical deployments and bridge assumptions |
| Exchanges | BYBIT, HYPERLIQUID | Check live order books and inactive pairs |
| Market cap | $2.55M | Compare Surf, CG, CMC, exchange pages |
| FDV | $13.26M | Reconcile total supply and unlock schedule |
| Volume | $110.11K | Check exchange concentration and wash-trading risk |
| TVL | not disclosed | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what mETH Protocol does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For mETH Protocol, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does COOK sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
For mETH Protocol, the current liquidity read is watchlist first. The market cap is $2.55M, FDV is $13.26M, reported 24h volume is $110.11K, and FDV / market cap is 5.21x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | mETH Protocol gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.methprotocol.xyz/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko profile | [link](https://www.coingecko.com/en/coins/meth-protocol) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=mETH%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=mETH%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=mETH%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=mETH%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/mETHProtocol) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/methprotocol_official) | Identity, market, social, contract, or discovery cross-check |
| 9 | Discord | [link](https://discord.com/invite/methprotocol) | Identity, market, social, contract, or discovery cross-check |
| 10 | mETH Protocol Price (COOK) - Coinbase | [link](https://www.coinbase.com/price/meth-protocol) | Identity, market, social, contract, or discovery cross-check |
...
| 12 | Cook Finance to USD Chart - CoinMarketCap | [link](https://coinmarketcap.com/currencies/cook-protocol/) | Identity, market, social, contract, or discovery cross-check |
| 13 | Overview of $COOK: mETH Protocol's Governance Token | [link](https://www.mantle.xyz/blog/announcements/overview-of-cook-meth-protocols-governance-token) | Identity, market, social, contract, or discovery cross-check |
| 14 | ethereum COOK | [link](https://etherscan.io/token/0x9f0c013016e8656bc256f948cd4b79ab25c7b94d) | Identity, market, social, contract, or discovery cross-check |
| 15 | ethereum CMETH | [link](https://etherscan.io/token/0xe6829d9a7ee3040e1276fa75293bde931859e8fa) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google project search | [link](https://www.google.com/search?q=mETH%20Protocol%20COOK%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google tokenomics search | [link](https://www.google.com/search?q=mETH%20Protocol%20COOK%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google docs search | [link](https://www.google.com/search?q=mETH%20Protocol%20docs) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google governance search | [link](https://www.google.com/search?q=mETH%20Protocol%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google audit search | [link](https://www.google.com/search?q=mETH%20Protocol%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinGecko historical data | [link](https://www.coingecko.com/en/coins/meth-protocol/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/meth-protocol/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/meth-protocol/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/meth-protocol/) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune discovery | [link](https://dune.com/browse/dashboards?q=mETH%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 26 | Dune token query | [link](https://dune.com/browse/queries?q=mETH%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=mETH%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 28 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=COOK) | Identity, market, social, contract, or discovery cross-check |
| 29 | TokenUnlocks search | [link](https://token.unlocks.app/?search=mETH%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 30 | CryptoRank search | [link](https://cryptorank.io/price/meth-protocol) | Identity, market, social, contract, or discovery cross-check |
| 31 | RootData search | [link](https://www.rootdata.com/search?kw=mETH%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 32 | CoinDesk search | [link](https://www.coindesk.com/search?s=mETH%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 33 | The Block search | [link](https://www.theblock.co/search?query=mETH%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 34 | GitHub search | [link](https://github.com/search?q=mETH%20Protocol&type=repositories) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.methprotocol.xyz/)
- [CoinGecko profile](https://www.coingecko.com/en/coins/meth-protocol)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=mETH%20Protocol)
- [DefiLlama protocol discovery](https://defillama.com/search?query=mETH%20Protocol)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=mETH%20Protocol)
- [Messari discovery](https://messari.io/search?q=mETH%20Protocol)
- [X profile](https://x.com/mETHProtocol)
- [Telegram](https://t.me/methprotocol_official)
- [Discord](https://discord.com/invite/methprotocol)
- [mETH Protocol Price (COOK) - Coinbase](https://www.coinbase.com/price/meth-protocol)
- [CoinGecko: Cryptocurrency Prices, Charts, and Crypto Market Cap](https://www.coingecko.com/)
- [Cook Finance to USD Chart - CoinMarketCap](https://coinmarketcap.com/currencies/cook-protocol/)
- [Overview of $COOK: mETH Protocol's Governance Token](https://www.mantle.xyz/blog/announcements/overview-of-cook-meth-protocols-governance-token)
- [ethereum COOK](https://etherscan.io/token/0x9f0c013016e8656bc256f948cd4b79ab25c7b94d)
- [ethereum CMETH](https://etherscan.io/token/0xe6829d9a7ee3040e1276fa75293bde931859e8fa)
- [Google project search](https://www.google.com/search?q=mETH%20Protocol%20COOK%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=mETH%20Protocol%20COOK%20tokenomics)
- [Google docs search](https://www.google.com/search?q=mETH%20Protocol%20docs)
- [Google governance search](https://www.google.com/search?q=mETH%20Protocol%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=mETH%20Protocol%20audit%20report)
- [CoinGecko historical data](https://www.coingecko.com/en/coins/meth-protocol/historical_data)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/meth-protocol/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/meth-protocol/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/meth-protocol/)

## Final Investment View

mETH Protocol should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
