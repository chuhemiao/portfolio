---
title: 'ResearchCoin / RSC: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'ResearchCoin is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'researchcoin-rsc-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# ResearchCoin / RSC: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** ResearchCoin appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Base, Ethereum; exchange exposure is COINBASE; category tags are DAO, DeSci (Decentralized Science); current market cap is $17.60M; FDV is $79.55M; and 24h volume is $390.47K. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag ResearchCoin before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 883, market cap $17,735,796, 24h volume $390,341. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

ResearchCoin is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether RSC captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.

| Metric | Current read |
|---|---|
| Project | ResearchCoin |
| Token | RSC |
...
| FDV | $79.55M |
| 24h volume | $390.47K |
| Circulating supply | 221.23M |
| Total supply | 999.84M |
| 7d price change | 2.02% |
| 30d price change | -28.74% |
...
| Liquidity | $390.47K 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 221.23M circulating vs 999.84M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that ResearchCoin is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

ResearchCoin's architecture should be evaluated through the job it performs. For a DeFi infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
|---|---|---|---|
| base | RSC | 0xfbb75a59193a3525a8825bebe7d4b56899e2f7e1 | [explorer](https://basescan.org/token/0xfbb75a59193a3525a8825bebe7d4b56899e2f7e1) |
| ethereum | RSC | 0xd101dcc414f310268c37eeb4cd376ccfa507f571 | [explorer](https://etherscan.io/token/0xd101dcc414f310268c37eeb4cd376ccfa507f571) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For ResearchCoin, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 883, market cap $17,735,796, 24h volume $390,341. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $17.60M, FDV $79.55M, 24h volume $390.47K, and price $0.0796.
...
| 24h volume | $390.47K | Position sizing and exit liquidity proxy |
| Market cap | $17.60M | Liquid valuation proxy |
| FDV | $79.55M | Dilution-aware valuation proxy |
| FDV / market cap spread | 4.52x | Higher spread means more supply risk |
| X followers | 43.28K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for ResearchCoin | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether RSC is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. ResearchCoin currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 2.02% and 30d -28.74% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that ResearchCoin's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-02-19 | Undisclosed | $2.00M | Boost VC |
| 2023-08-14 | Undisclosed | not disclosed | Sora Ventures |
| 2023-06-15 | Seed | $5.00M | Boost VC, Amjad Masad, Guillermo Rauch, Garry Tan, Bob Young, OSS Capital |

| Team member | Role | Link |
|---|---|---|
| Brian Armstrong | CEO | [X](https://twitter.com/brian_armstrong) |
| Tyler Diorio | Chief of Staff | [X](https://x.com/TylerDiorio) |
| Kobe Attias | Founding Engineer | not disclosed |
| Patrick Joyce | CCO | [X](https://x.com/joycesticks) |
...
ResearchCoin competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
Weighted conclusion: ResearchCoin is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | ResearchCoin compounds usage, liquidity improves, and RSC gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If RSC does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $390.47K | Weekly | Volume below position-size threshold |
| FDV / market cap | 4.52x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if ResearchCoin announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. ResearchCoin has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For ResearchCoin, the first reconciliation job is identity. The token symbol is RSC, the category is DeFi infrastructure, chains are Base, Ethereum, exchanges are COINBASE, and tags are DAO, DeSci (Decentralized Science). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If RSC trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $17.60M, FDV is $79.55M, 24h volume is $390.47K, FDV / market cap is 4.52x, 7d price change is 2.02%, and 30d price change is -28.74%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | ResearchCoin / RSC | Match official docs, CG, CMC, Surf, contracts |
...
For ResearchCoin, the current liquidity read is watchlist first. The market cap is $17.60M, FDV is $79.55M, reported 24h volume is $390.47K, and FDV / market cap is 4.52x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. ResearchCoin can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.

For ResearchCoin, the live monitoring stack should include contract events, TVL, volume, governance, documentation changes, social announcements, security disclosures, and exchange-market quality. A single metric is not enough. TVL may stay high while liquidity thins. Volume may rise because of volatility rather than adoption. Social attention may rise around controversy rather than progress. Governance may pass proposals that improve short-term price but add long-term risk. The strongest monitoring dashboard combines on-chain state, off-chain disclosure, and market quality.

Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If ResearchCoin suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | ResearchCoin gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.researchhub.com/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=ResearchCoin) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=ResearchCoin) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=ResearchCoin) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=ResearchCoin) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=ResearchCoin) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/ResearchHub) | Identity, market, social, contract, or discovery cross-check |
| 8 | Discord | [link](https://discord.com/invite/ZcCYgcnUp5) | Identity, market, social, contract, or discovery cross-check |
| 9 | ResearchCoin Price Chart (RSC) - CoinGecko | [link](https://www.coingecko.com/en/coins/researchcoin) | Identity, market, social, contract, or discovery cross-check |
| 10 | What Is ResearchCoin (RSC) And How Does It Work? | [link](https://coinmarketcap.com/cmc-ai/researchcoin/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 11 | RSC Tokenomics - ResearchHub Docs | [link](https://docs.researchhub.com/researchcoin/rsc-tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 12 | RSC to TRY: ResearchCoin Price in Turkish Lira - CoinGecko | [link](https://www.coingecko.com/en/coins/researchcoin/try) | Identity, market, social, contract, or discovery cross-check |
| 13 | RSC to PLN: ResearchCoin Price in Polish Zloty - CoinGecko | [link](https://www.coingecko.com/en/coins/researchcoin/pln) | Identity, market, social, contract, or discovery cross-check |
| 14 | base RSC | [link](https://basescan.org/token/0xfbb75a59193a3525a8825bebe7d4b56899e2f7e1) | Identity, market, social, contract, or discovery cross-check |
| 15 | ethereum RSC | [link](https://etherscan.io/token/0xd101dcc414f310268c37eeb4cd376ccfa507f571) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google project search | [link](https://www.google.com/search?q=ResearchCoin%20RSC%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google tokenomics search | [link](https://www.google.com/search?q=ResearchCoin%20RSC%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google docs search | [link](https://www.google.com/search?q=ResearchCoin%20docs) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google governance search | [link](https://www.google.com/search?q=ResearchCoin%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google audit search | [link](https://www.google.com/search?q=ResearchCoin%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/researchcoin/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/researchcoin/) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune discovery | [link](https://dune.com/browse/dashboards?q=ResearchCoin) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune token query | [link](https://dune.com/browse/queries?q=ResearchCoin) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=ResearchCoin) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=RSC) | Identity, market, social, contract, or discovery cross-check |
| 27 | TokenUnlocks search | [link](https://token.unlocks.app/?search=ResearchCoin) | Identity, market, social, contract, or discovery cross-check |
| 28 | CryptoRank search | [link](https://cryptorank.io/price/researchcoin) | Identity, market, social, contract, or discovery cross-check |
| 29 | RootData search | [link](https://www.rootdata.com/search?kw=ResearchCoin) | Identity, market, social, contract, or discovery cross-check |
| 30 | CoinDesk search | [link](https://www.coindesk.com/search?s=ResearchCoin) | Identity, market, social, contract, or discovery cross-check |
| 31 | The Block search | [link](https://www.theblock.co/search?query=ResearchCoin) | Identity, market, social, contract, or discovery cross-check |
| 32 | GitHub search | [link](https://github.com/search?q=ResearchCoin&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 33 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=RSC) | Identity, market, social, contract, or discovery cross-check |
| 34 | BscScan token search | [link](https://bscscan.com/search?f=0&q=RSC) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.researchhub.com/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=ResearchCoin)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=ResearchCoin)
- [DefiLlama protocol discovery](https://defillama.com/search?query=ResearchCoin)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=ResearchCoin)
- [Messari discovery](https://messari.io/search?q=ResearchCoin)
- [X profile](https://x.com/ResearchHub)
- [Discord](https://discord.com/invite/ZcCYgcnUp5)
- [ResearchCoin Price Chart (RSC) - CoinGecko](https://www.coingecko.com/en/coins/researchcoin)
- [What Is ResearchCoin (RSC) And How Does It Work?](https://coinmarketcap.com/cmc-ai/researchcoin/what-is/)
- [RSC Tokenomics - ResearchHub Docs](https://docs.researchhub.com/researchcoin/rsc-tokenomics)
- [RSC to TRY: ResearchCoin Price in Turkish Lira - CoinGecko](https://www.coingecko.com/en/coins/researchcoin/try)
- [RSC to PLN: ResearchCoin Price in Polish Zloty - CoinGecko](https://www.coingecko.com/en/coins/researchcoin/pln)
- [base RSC](https://basescan.org/token/0xfbb75a59193a3525a8825bebe7d4b56899e2f7e1)
- [ethereum RSC](https://etherscan.io/token/0xd101dcc414f310268c37eeb4cd376ccfa507f571)
- [Google project search](https://www.google.com/search?q=ResearchCoin%20RSC%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=ResearchCoin%20RSC%20tokenomics)
- [Google docs search](https://www.google.com/search?q=ResearchCoin%20docs)
- [Google governance search](https://www.google.com/search?q=ResearchCoin%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=ResearchCoin%20audit%20report)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/researchcoin/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/researchcoin/)
- [Dune discovery](https://dune.com/browse/dashboards?q=ResearchCoin)
- [Dune token query](https://dune.com/browse/queries?q=ResearchCoin)

## Final Investment View

ResearchCoin should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
