---
title: 'Origin Token / OGN: staking infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Origin Token is treated as a staking infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'origin-token-ogn-staking-infrastructure-token-value-capture-risk'
category: 'research'
---

# Origin Token / OGN: staking infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Origin Token appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Arbitrum One, Astar, Base, Ethereum, Plume Network, Sonic; exchange exposure is BINANCE, UPBIT, COINBASE, BITHUMB, BITGET; category tags are LSD & Restaking, Stablecoins, Yield & Asset Mgmt; current market cap is $11.37M; FDV is $23.84M; and 24h volume is $5.21M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Origin Token before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 1092, market cap $11,524,609, 24h volume $5,754,159. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Origin Token is best understood as a **staking infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether OGN captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.

| Metric | Current read |
|---|---|
| Project | Origin Token |
| Token | OGN |
| Category | staking infrastructure |
| Chains | Arbitrum One, Astar, Base, Ethereum, Plume Network, Sonic |
| Exchanges | BINANCE, UPBIT, COINBASE, BITHUMB, BITGET |
| Price | $0.0169 |
| Market cap | $11.37M |
| FDV | $23.84M |
| 24h volume | $5.21M |
| Circulating supply | 672.42M |
| Total supply | 1.41B |
| 7d price change | 11.16% |
| 30d price change | -0.05% |
| Latest TVL sample | $167.05M |
| TVL sample change | 51.71% |
| Primary source posture | Surf + CoinGecko + CMC discovery + official/project links |

Base case: Origin Token remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning OGN into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Origin Protocol is a multichain DeFi platform offering liquid staking, yield-bearing stablecoins, and composable vaults. Its ecosystem includes OETH/Super OETH, OUSD, Origin Sonic (OS), and the OGN governance/value-accrual token, with protocol fees routed to OGN buybacks and staker rewards.

Surf's project profile maps Origin Token to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.originprotocol.com/](https://www.originprotocol.com/) |
| X handle | [@OriginProtocol](https://x.com/OriginProtocol) |
| X followers | 167.19K |
| Tags | LSD & Restaking, Stablecoins, Yield & Asset Mgmt |
| Chains | Arbitrum One, Astar, Base, Ethereum, Plume Network, Sonic |
| Exchanges | BINANCE, UPBIT, COINBASE, BITHUMB, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Origin Token is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For staking infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Origin Token convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $5.21M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 672.42M circulating vs 1.41B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Origin Token is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

Origin Token's architecture should be evaluated through the job it performs. For a staking infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
| Settlement layer | Chains and contracts that hold user value | Arbitrum One, Astar, Base, Ethereum, Plume Network, Sonic |
...
|---|---|---|---|
| base | OGN | 0x7002458b1df59eccb57387bc79ffc7c29e22e6f7 | [explorer](https://basescan.org/token/0x7002458b1df59eccb57387bc79ffc7c29e22e6f7) |
| Sonic | OGN | 0x61647882acf21792e577a963d320d51e2c8f6194 | not mapped |
| ethereum | OGN | 0x8207c1ffc5b6804f6024322ccf34f29c3541ae26 | [explorer](https://etherscan.io/token/0x8207c1ffc5b6804f6024322ccf34f29c3541ae26) |
| ethereum | ARM-WETH-EETH | 0xfb0a3cf9b019bfd8827443d131b235b3e0fc58d2 | [explorer](https://etherscan.io/token/0xfb0a3cf9b019bfd8827443d131b235b3e0fc58d2) |
| Sonic | OS | 0xb1e25689d55734fd3fffc939c4c3eb52dff8a794 | not mapped |
| Sonic | ARM-WS-OS | 0x2f872623d1e1af5835b08b0e49aad2d81d649d30 | not mapped |
| ethereum | XOGN | 0x63898b3b6ef3d39332082178656e9862bee45c57 | [explorer](https://etherscan.io/token/0x63898b3b6ef3d39332082178656e9862bee45c57) |
| ethereum | ARM-WETH-STETH | 0x85b78aca6deae198fbf201c82daf6ca21942acc6 | [explorer](https://etherscan.io/token/0x85b78aca6deae198fbf201c82daf6ca21942acc6) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Origin Token, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 1092, market cap $11,524,609, 24h volume $5,754,159. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $11.37M, FDV $23.84M, 24h volume $5.21M, and price $0.0169.

| Traction metric | Value | Interpretation |
|---|---|---|
| Latest TVL sample | $167.05M | Useful only if sustained without aggressive incentives |
| TVL sample points | 90 | More points improve trend confidence |
| TVL sample change | 51.71% | Positive trend supports usage; negative trend requires explanation |
| 24h volume | $5.21M | Position sizing and exit liquidity proxy |
| Market cap | $11.37M | Liquid valuation proxy |
| FDV | $23.84M | Dilution-aware valuation proxy |
| FDV / market cap spread | 2.10x | Higher spread means more supply risk |
| X followers | 167.19K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Origin Token | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether OGN is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. Origin Token should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that Origin Token belongs to the staking infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $11.37M, FDV $23.84M, 24h volume $5.21M, price $0.0169, circulating supply 672.42M, and total supply 1.41B. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For Origin Token, the candidate backlog records Seeded 2026-07-06 from Surf market-ranking; rank 1092, market cap $11,524,609, 24h volume $5,754,159. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Arbitrum One, Astar, Base, Ethereum, Plume Network, Sonic, exchanges as BINANCE, UPBIT, COINBASE, BITHUMB, BITGET, and tags as LSD & Restaking, Stablecoins, Yield & Asset Mgmt. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Origin Token currently needs to be monitored through that sequence. The available 90-day TVL series has 90 point(s), latest TVL $167.05M, and approximate change 51.71%. Market-price movement shows 7d 11.16% and 30d -0.05% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Origin Token's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2018-06-18 | Ico | $6.60M | not disclosed |
| 2018-04-17 | Strategic | $28.50M | gumi Cryptos, One Block Capital, Randall Kaplan, Sora Ventures, PreAngel Fund, Gil Penchina, Oyster Ventures, Kommune |
| 2017-12-11 | Seed | $3.00M | Pantera Capital, Visary Capital |

| Team member | Role | Link |
|---|---|---|
| Matthew Liu | Co-Founder | [X](https://twitter.com/matthewliu) |
| Josh Fraser | Co-Founder | [X](https://twitter.com/joshfraser) |
| Alex Yopp | Talent Advisor | not disclosed |
| Paul Veradittakit | Advisor | [X](https://twitter.com/veradittakit) |
| Joey Krug | Advisor | [X](https://twitter.com/joeykrug) |
...
Origin Token competes in the staking infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Origin Token has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Origin Token is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Origin Token compounds usage, liquidity improves, and OGN gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If OGN does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
|---|---|---|---|
| TVL | $167.05M | Weekly | >20% drawdown or sustained growth |
| 24h volume | $5.21M | Weekly | Volume below position-size threshold |
| FDV / market cap | 2.10x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Origin Token announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Origin Token has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Origin Token, the first reconciliation job is identity. The token symbol is OGN, the category is staking infrastructure, chains are Arbitrum One, Astar, Base, Ethereum, Plume Network, Sonic, exchanges are BINANCE, UPBIT, COINBASE, BITHUMB, BITGET, and tags are LSD & Restaking, Stablecoins, Yield & Asset Mgmt. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If OGN trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $11.37M, FDV is $23.84M, 24h volume is $5.21M, FDV / market cap is 2.10x, 7d price change is 11.16%, and 30d price change is -0.05%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.

The third reconciliation job is usage data. The latest TVL sample is $167.05M, TVL sample change is 51.71%, and the sample contains 90 point(s). TVL is useful only when the underlying balances are real, withdrawable, and not double-counted. For staking infrastructure, the memo should identify whether TVL means deposits, staked assets, LP liquidity, stablecoin supply, collateral, locked governance assets, or a derived wrapper. A yield-bearing stablecoin and a validator middleware protocol can both show TVL, but the risk encoded by that TVL is completely different.

| Reconciliation item | Current value | Next check |
|---|---|---|
| Identity | Origin Token / OGN | Match official docs, CG, CMC, Surf, contracts |
| Category | staking infrastructure | Confirm product mechanics and revenue model |
| Chains | Arbitrum One, Astar, Base, Ethereum, Plume Network, Sonic | Confirm canonical deployments and bridge assumptions |
| Exchanges | BINANCE, UPBIT, COINBASE, BITHUMB, BITGET | Check live order books and inactive pairs |
| Market cap | $11.37M | Compare Surf, CG, CMC, exchange pages |
| FDV | $23.84M | Reconcile total supply and unlock schedule |
| Volume | $5.21M | Check exchange concentration and wash-trading risk |
| TVL | $167.05M | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Origin Token does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Origin Token, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does OGN sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
For Origin Token, the current liquidity read is watchlist first. The market cap is $11.37M, FDV is $23.84M, reported 24h volume is $5.21M, and FDV / market cap is 2.10x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. Origin Token can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.

For Origin Token, the live monitoring stack should include contract events, TVL, volume, governance, documentation changes, social announcements, security disclosures, and exchange-market quality. A single metric is not enough. TVL may stay high while liquidity thins. Volume may rise because of volatility rather than adoption. Social attention may rise around controversy rather than progress. Governance may pass proposals that improve short-term price but add long-term risk. The strongest monitoring dashboard combines on-chain state, off-chain disclosure, and market quality.

Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If Origin Token suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | Origin Token gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.originprotocol.com/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Origin%20Token) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Origin%20Token) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Origin%20Token) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Origin%20Token) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Origin%20Token) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/OriginProtocol) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/OriginProtocol) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/originprotocol) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](https://discord.com/invite/ogn) | Identity, market, social, contract, or discovery cross-check |
| 11 | Origin Token Price Chart (OGN) - CoinGecko | [link](https://www.coingecko.com/en/coins/origin-protocol) | Identity, market, social, contract, or discovery cross-check |
| 12 | Origin Protocol price OGN #904 - CoinMarketCap | [link](https://coinmarketcap.com/currencies/origin-protocol/) | Identity, market, social, contract, or discovery cross-check |
| 13 | OGN to INR: Origin Token Price in Indian Rupee - CoinGecko | [link](https://www.coingecko.com/en/coins/origin-protocol/inr) | Identity, market, social, contract, or discovery cross-check |
| 14 | OGN to BTC: Origin Token Price in Bitcoin - CoinGecko | [link](https://www.coingecko.com/en/coins/origin-protocol/btc) | Identity, market, social, contract, or discovery cross-check |
| 15 | Latest Origin Protocol (OGN) Price Analysis - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/origin-protocol/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 16 | base OGN | [link](https://basescan.org/token/0x7002458b1df59eccb57387bc79ffc7c29e22e6f7) | Identity, market, social, contract, or discovery cross-check |
| 17 | ethereum OGN | [link](https://etherscan.io/token/0x8207c1ffc5b6804f6024322ccf34f29c3541ae26) | Identity, market, social, contract, or discovery cross-check |
| 18 | ethereum ARM-WETH-EETH | [link](https://etherscan.io/token/0xfb0a3cf9b019bfd8827443d131b235b3e0fc58d2) | Identity, market, social, contract, or discovery cross-check |
| 19 | ethereum XOGN | [link](https://etherscan.io/token/0x63898b3b6ef3d39332082178656e9862bee45c57) | Identity, market, social, contract, or discovery cross-check |
| 20 | ethereum ARM-WETH-STETH | [link](https://etherscan.io/token/0x85b78aca6deae198fbf201c82daf6ca21942acc6) | Identity, market, social, contract, or discovery cross-check |
| 21 | ethereum WOETH | [link](https://etherscan.io/token/0xdcee70654261af21c44c093c300ed3bb97b78192) | Identity, market, social, contract, or discovery cross-check |
| 22 | arbitrum WOETH | [link](https://arbiscan.io/token/0xd8724322f44e5c58d7a815f542036fb17dbbf839) | Identity, market, social, contract, or discovery cross-check |
| 23 | ethereum WOUSD | [link](https://etherscan.io/token/0xd2af830e8cbdfed6cc11bab697bb25496ed6fa62) | Identity, market, social, contract, or discovery cross-check |
| 24 | ethereum ARM-SUSDE-USDE | [link](https://etherscan.io/token/0xceda2d856238aa0d12f6329de20b9115f07c366d) | Identity, market, social, contract, or discovery cross-check |
| 25 | Google project search | [link](https://www.google.com/search?q=Origin%20Token%20OGN%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 26 | Google tokenomics search | [link](https://www.google.com/search?q=Origin%20Token%20OGN%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 27 | Google docs search | [link](https://www.google.com/search?q=Origin%20Token%20docs) | Identity, market, social, contract, or discovery cross-check |
| 28 | Google governance search | [link](https://www.google.com/search?q=Origin%20Token%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 29 | Google audit search | [link](https://www.google.com/search?q=Origin%20Token%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 30 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/origin-token/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 31 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/origin-token/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 32 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/origin-token/) | Identity, market, social, contract, or discovery cross-check |
| 33 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Origin%20Token) | Identity, market, social, contract, or discovery cross-check |
| 34 | Dune token query | [link](https://dune.com/browse/queries?q=Origin%20Token) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.originprotocol.com/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Origin%20Token)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Origin%20Token)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Origin%20Token)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Origin%20Token)
- [Messari discovery](https://messari.io/search?q=Origin%20Token)
- [X profile](https://x.com/OriginProtocol)
- [GitHub](https://github.com/OriginProtocol)
- [Telegram](https://t.me/originprotocol)
- [Discord](https://discord.com/invite/ogn)
- [Origin Token Price Chart (OGN) - CoinGecko](https://www.coingecko.com/en/coins/origin-protocol)
- [Origin Protocol price OGN #904 - CoinMarketCap](https://coinmarketcap.com/currencies/origin-protocol/)
- [OGN to INR: Origin Token Price in Indian Rupee - CoinGecko](https://www.coingecko.com/en/coins/origin-protocol/inr)
- [OGN to BTC: Origin Token Price in Bitcoin - CoinGecko](https://www.coingecko.com/en/coins/origin-protocol/btc)
- [Latest Origin Protocol (OGN) Price Analysis - CoinMarketCap](https://coinmarketcap.com/cmc-ai/origin-protocol/price-analysis/)
- [base OGN](https://basescan.org/token/0x7002458b1df59eccb57387bc79ffc7c29e22e6f7)
- [ethereum OGN](https://etherscan.io/token/0x8207c1ffc5b6804f6024322ccf34f29c3541ae26)
- [ethereum ARM-WETH-EETH](https://etherscan.io/token/0xfb0a3cf9b019bfd8827443d131b235b3e0fc58d2)
- [ethereum XOGN](https://etherscan.io/token/0x63898b3b6ef3d39332082178656e9862bee45c57)
- [ethereum ARM-WETH-STETH](https://etherscan.io/token/0x85b78aca6deae198fbf201c82daf6ca21942acc6)
- [ethereum WOETH](https://etherscan.io/token/0xdcee70654261af21c44c093c300ed3bb97b78192)
- [arbitrum WOETH](https://arbiscan.io/token/0xd8724322f44e5c58d7a815f542036fb17dbbf839)
- [ethereum WOUSD](https://etherscan.io/token/0xd2af830e8cbdfed6cc11bab697bb25496ed6fa62)
- [ethereum ARM-SUSDE-USDE](https://etherscan.io/token/0xceda2d856238aa0d12f6329de20b9115f07c366d)

## Final Investment View

Origin Token should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
