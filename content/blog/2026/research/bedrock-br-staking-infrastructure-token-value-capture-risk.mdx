---
title: 'Bedrock / BR: staking infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Bedrock is treated as a staking infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'bedrock-br-staking-infrastructure-token-value-capture-risk'
category: 'research'
---

# Bedrock / BR: staking infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Bedrock appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Aptos, Arbitrum One, Base, Berachain, Binance Smart Chain, Bitlayer, Bob Network, Bsquared Network, Corn, DuckChain, Ethereum, Hemi, HyperEVM, Ink, IoTeX, Mantle, Merlin Chain, Mode, Optimistic Ethereum, RSK, Sei V2, Solana, Sonic, Tac, Taiko, Unichain, X Layer, ZetaChain; exchange exposure is not disclosed; category tags are LSD & Restaking, Yield & Asset Mgmt; current market cap is $41.10M; FDV is $140.90M; and 24h volume is $9.76M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Bedrock before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 516, market cap $40,715,172, 24h volume $9,696,060. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Bedrock is best understood as a **staking infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether BR captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Aptos, Arbitrum One, Base, Berachain, Binance Smart Chain, Bitlayer, Bob Network, Bsquared Network, Corn, DuckChain, Ethereum, Hemi, HyperEVM, Ink, IoTeX, Mantle, Merlin Chain, Mode, Optimistic Ethereum, RSK, Sei V2, Solana, Sonic, Tac, Taiko, Unichain, X Layer, ZetaChain |
| Exchanges | not disclosed |
| Price | $0.1409 |
| Market cap | $41.10M |
| FDV | $140.90M |
| 24h volume | $9.76M |
| Circulating supply | 291.67M |
| Total supply | 1.00B |
...
Base case: Bedrock remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning BR into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Bedrock is a multi‑asset liquid restaking protocol that lets users restake native assets (including Bitcoin and Ethereum) to mint liquid restaked tokens and capture cross‑chain DeFi yield. It offers non‑custodial restaking, liquidity incentives and on‑chain governance through the BR token.

Surf's project profile maps Bedrock to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.bedrock.technology/](https://www.bedrock.technology/) |
| X handle | [@Bedrock_DeFi](https://x.com/Bedrock_DeFi) |
| X followers | 248.38K |
...
| Liquidity | $9.76M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 291.67M circulating vs 1.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Bedrock is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Aptos, Arbitrum One, Base, Berachain, Binance Smart Chain, Bitlayer, Bob Network, Bsquared Network, Corn, DuckChain, Ethereum, Hemi, HyperEVM, Ink, IoTeX, Mantle, Merlin Chain, Mode, Optimistic Ethereum, RSK, Sei V2, Solana, Sonic, Tac, Taiko, Unichain, X Layer, ZetaChain |
...
|---|---|---|---|
| base | BR | 0xd6122ddada244913521f3d62006eaf756c157660 | [explorer](https://basescan.org/token/0xd6122ddada244913521f3d62006eaf756c157660) |
| ethereum | BR | 0x9b61879e91a0b1322f3d61c23aaf936231882096 | [explorer](https://etherscan.io/token/0x9b61879e91a0b1322f3d61c23aaf936231882096) |
| Berachain | BR | 0xd352dc6e5f0c45e2f2b38eb5565eb286a1ea4087 | not mapped |
| bsc | BR | 0xff7d6a96ae471bbcd7713af9cb1feeb16cf56b41 | [explorer](https://bscscan.com/token/0xff7d6a96ae471bbcd7713af9cb1feeb16cf56b41) |
| Unichain | BRBTC | 0xa161132371c94299d215915d4cbc3b629e2059be | not mapped |
| Berachain | BRBTC | 0x93919784c523f39cacaa98ee0a9d96c3f32b593e | not mapped |
| RSK | BRBTC | 0xf1376bcef0f78459c0ed0ba5ddce976f1ddf51f4 | not mapped |
| Ink | BRBTC | 0xa161132371c94299d215915d4cbc3b629e2059be | not mapped |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Bedrock, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 516, market cap $40,715,172, 24h volume $9,696,060. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $41.10M, FDV $140.90M, 24h volume $9.76M, and price $0.1409.
...
| 24h volume | $9.76M | Position sizing and exit liquidity proxy |
| Market cap | $41.10M | Liquid valuation proxy |
| FDV | $140.90M | Dilution-aware valuation proxy |
| FDV / market cap spread | 3.43x | Higher spread means more supply risk |
| X followers | 248.38K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Bedrock | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether BR is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. Bedrock should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that Bedrock belongs to the staking infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $41.10M, FDV $140.90M, 24h volume $9.76M, price $0.1409, circulating supply 291.67M, and total supply 1.00B. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For Bedrock, the candidate backlog records Seeded 2026-07-06 from Surf market-ranking; rank 516, market cap $40,715,172, 24h volume $9,696,060. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Aptos, Arbitrum One, Base, Berachain, Binance Smart Chain, Bitlayer, Bob Network, Bsquared Network, Corn, DuckChain, Ethereum, Hemi, HyperEVM, Ink, IoTeX, Mantle, Merlin Chain, Mode, Optimistic Ethereum, RSK, Sei V2, Solana, Sonic, Tac, Taiko, Unichain, X Layer, ZetaChain, exchanges as not disclosed, and tags as LSD & Restaking, Yield & Asset Mgmt. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Bedrock currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d -4.17% and 30d 41.39% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Bedrock's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-03-20 | Ido | $1.25M | not disclosed |
| 2024-05-02 | Seed | not disclosed | OKX Ventures, Arche Fund, LongHash Ventures, Waterdrip Capital, Amber Group, LBank, Comma3 Ventures, Whale Ground |

| Team member | Role | Link |
|---|---|---|
| Chen Zhuling | CEO | [X](https://x.com/czhuling) |
| KasperStone | Head of Growth | [X](https://x.com/KasperStone1) |
| Adam Wong | Head of Marketing | [X](https://x.com/frostyfoxes) |
| Calvin | Core Contributor | [X](https://x.com/calvincrypto7) |
...
Bedrock competes in the staking infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
Weighted conclusion: Bedrock is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Bedrock compounds usage, liquidity improves, and BR gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Bedrock has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If BR does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $9.76M | Weekly | Volume below position-size threshold |
| FDV / market cap | 3.43x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Bedrock announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Bedrock has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Bedrock, the first reconciliation job is identity. The token symbol is BR, the category is staking infrastructure, chains are Aptos, Arbitrum One, Base, Berachain, Binance Smart Chain, Bitlayer, Bob Network, Bsquared Network, Corn, DuckChain, Ethereum, Hemi, HyperEVM, Ink, IoTeX, Mantle, Merlin Chain, Mode, Optimistic Ethereum, RSK, Sei V2, Solana, Sonic, Tac, Taiko, Unichain, X Layer, ZetaChain, exchanges are not disclosed, and tags are LSD & Restaking, Yield & Asset Mgmt. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If BR trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $41.10M, FDV is $140.90M, 24h volume is $9.76M, FDV / market cap is 3.43x, 7d price change is -4.17%, and 30d price change is 41.39%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Bedrock / BR | Match official docs, CG, CMC, Surf, contracts |
| Category | staking infrastructure | Confirm product mechanics and revenue model |
| Chains | Aptos, Arbitrum One, Base, Berachain, Binance Smart Chain, Bitlayer, Bob Network, Bsquared Network, Corn, DuckChain, Ethereum, Hemi, HyperEVM, Ink, IoTeX, Mantle, Merlin Chain, Mode, Optimistic Ethereum, RSK, Sei V2, Solana, Sonic, Tac, Taiko, Unichain, X Layer, ZetaChain | Confirm canonical deployments and bridge assumptions |
| Exchanges | not disclosed | Check live order books and inactive pairs |
| Market cap | $41.10M | Compare Surf, CG, CMC, exchange pages |
| FDV | $140.90M | Reconcile total supply and unlock schedule |
| Volume | $9.76M | Check exchange concentration and wash-trading risk |
| TVL | not disclosed | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Bedrock does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Bedrock, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does BR sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
For Bedrock, the current liquidity read is watchlist first. The market cap is $41.10M, FDV is $140.90M, reported 24h volume is $9.76M, and FDV / market cap is 3.43x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Bedrock gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.bedrock.technology/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Bedrock) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Bedrock) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Bedrock) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Bedrock) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Bedrock) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/Bedrock_DeFi) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/Bedrock-Technology) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/Bedrock_Official) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](https://discord.com/invite/ctXrm6wfeg) | Identity, market, social, contract, or discovery cross-check |
| 11 | Bedrock Price: BR/USD Live Price Chart, Market Cap ... | [link](https://www.coingecko.com/en/coins/bedrock) | Identity, market, social, contract, or discovery cross-check |
| 12 | Bedrock price today, BR to USD live ... | [link](https://coinmarketcap.com/currencies/bedrock-dao/) | Identity, market, social, contract, or discovery cross-check |
| 13 | What Is Bedrock (BR) And How Does It Work? | [link](https://coinmarketcap.com/cmc-ai/bedrock-dao/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 14 | Bedrock (BR) Price Prediction For 2026 & Beyond | [link](https://coinmarketcap.com/cmc-ai/bedrock-dao/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 15 | Bedrock BTC Price Chart (BRBTC) | [link](https://www.coingecko.com/en/coins/bedrock-btc) | Identity, market, social, contract, or discovery cross-check |
| 16 | base BR | [link](https://basescan.org/token/0xd6122ddada244913521f3d62006eaf756c157660) | Identity, market, social, contract, or discovery cross-check |
| 17 | ethereum BR | [link](https://etherscan.io/token/0x9b61879e91a0b1322f3d61c23aaf936231882096) | Identity, market, social, contract, or discovery cross-check |
| 18 | bsc BR | [link](https://bscscan.com/token/0xff7d6a96ae471bbcd7713af9cb1feeb16cf56b41) | Identity, market, social, contract, or discovery cross-check |
| 19 | base BRBTC | [link](https://basescan.org/token/0x3376ebca0a85fc8d791b1001a571c41fdd61514a) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google project search | [link](https://www.google.com/search?q=Bedrock%20BR%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google tokenomics search | [link](https://www.google.com/search?q=Bedrock%20BR%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google docs search | [link](https://www.google.com/search?q=Bedrock%20docs) | Identity, market, social, contract, or discovery cross-check |
| 23 | Google governance search | [link](https://www.google.com/search?q=Bedrock%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 24 | Google audit search | [link](https://www.google.com/search?q=Bedrock%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 25 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/bedrock/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 26 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/bedrock/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 27 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/bedrock/) | Identity, market, social, contract, or discovery cross-check |
| 28 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Bedrock) | Identity, market, social, contract, or discovery cross-check |
| 29 | Dune token query | [link](https://dune.com/browse/queries?q=Bedrock) | Identity, market, social, contract, or discovery cross-check |
| 30 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Bedrock) | Identity, market, social, contract, or discovery cross-check |
| 31 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=BR) | Identity, market, social, contract, or discovery cross-check |
| 32 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Bedrock) | Identity, market, social, contract, or discovery cross-check |
| 33 | CryptoRank search | [link](https://cryptorank.io/price/bedrock) | Identity, market, social, contract, or discovery cross-check |
| 34 | RootData search | [link](https://www.rootdata.com/search?kw=Bedrock) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.bedrock.technology/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Bedrock)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Bedrock)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Bedrock)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Bedrock)
- [Messari discovery](https://messari.io/search?q=Bedrock)
- [X profile](https://x.com/Bedrock_DeFi)
...
- [Bedrock Price: BR/USD Live Price Chart, Market Cap ...](https://www.coingecko.com/en/coins/bedrock)
- [Bedrock price today, BR to USD live ...](https://coinmarketcap.com/currencies/bedrock-dao/)
- [What Is Bedrock (BR) And How Does It Work?](https://coinmarketcap.com/cmc-ai/bedrock-dao/what-is/)
- [Bedrock (BR) Price Prediction For 2026 & Beyond](https://coinmarketcap.com/cmc-ai/bedrock-dao/price-prediction/)
- [Bedrock BTC Price Chart (BRBTC)](https://www.coingecko.com/en/coins/bedrock-btc)
- [base BR](https://basescan.org/token/0xd6122ddada244913521f3d62006eaf756c157660)
- [ethereum BR](https://etherscan.io/token/0x9b61879e91a0b1322f3d61c23aaf936231882096)
- [bsc BR](https://bscscan.com/token/0xff7d6a96ae471bbcd7713af9cb1feeb16cf56b41)
- [base BRBTC](https://basescan.org/token/0x3376ebca0a85fc8d791b1001a571c41fdd61514a)
- [Google project search](https://www.google.com/search?q=Bedrock%20BR%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Bedrock%20BR%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Bedrock%20docs)
- [Google governance search](https://www.google.com/search?q=Bedrock%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Bedrock%20audit%20report)

## Final Investment View

Bedrock should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
