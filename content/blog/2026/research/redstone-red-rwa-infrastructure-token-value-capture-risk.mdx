---
title: 'RedStone / RED: RWA infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'RedStone is treated as a RWA infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'redstone-red-rwa-infrastructure-token-value-capture-risk'
category: 'research'
---

# RedStone / RED: RWA infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** RedStone appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Base, Ethereum; exchange exposure is BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET; category tags are Oracle, RWA (Real World Assets); current market cap is $44.55M; FDV is $101.33M; and 24h volume is $4.45M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag RedStone before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 488, market cap $44,145,816, 24h volume $3,925,633. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

RedStone is best understood as a **RWA infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether RED captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
Base case: RedStone remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning RED into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

RedStone is a modular blockchain oracle platform that delivers fast, secure, and customizable multi-chain data feeds for DeFi protocols and institutions. It offers price feeds, proof-of-reserve, and specialized asset pricing (e.g., LSTs, BTCfi, RWAs) via an architecture separating data collection from delivery. The RED token powers staking and network incentives.

Surf's project profile maps RedStone to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://redstone.finance/](https://redstone.finance/) |
| X handle | [@redstone_defi](https://x.com/redstone_defi) |
| X followers | 248.26K |
| Tags | Oracle, RWA (Real World Assets) |
| Chains | Base, Ethereum |
| Exchanges | BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether RedStone is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For RWA infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does RedStone convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $4.45M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 439.60M circulating vs 1.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that RedStone is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| base | RED | 0x4eb92702ba4cfbf80561bad64d89c706ac824960 | [explorer](https://basescan.org/token/0x4eb92702ba4cfbf80561bad64d89c706ac824960) |
| ethereum | RED | 0xc43c6bfeda065fe2c4c11765bf838789bd0bb5de | [explorer](https://etherscan.io/token/0xc43c6bfeda065fe2c4c11765bf838789bd0bb5de) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For RedStone, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 488, market cap $44,145,816, 24h volume $3,925,633. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $44.55M, FDV $101.33M, 24h volume $4.45M, and price $0.1014.
...
| 24h volume | $4.45M | Position sizing and exit liquidity proxy |
| Market cap | $44.55M | Liquid valuation proxy |
| FDV | $101.33M | Dilution-aware valuation proxy |
| FDV / market cap spread | 2.27x | Higher spread means more supply risk |
| X followers | 248.26K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for RedStone | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether RED is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. RedStone currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 11.56% and 30d 5.39% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that RedStone's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2024-07-02 | Series A | $15.00M | Kenetic Capital, HTX Ventures, Arrington Capital, The Spartan Group, SevenX Ventures, IOSG Ventures, gumi Cryptos, Amber Group |
| 2023-05-22 | Angel | not disclosed | Sandeep Nailwal, Stani Kulechov, Alex Gluchowski, Richard Ma, Emin Gun Sirer, Marco Cora, Jacob Blish, Patrick Dai |
| 2022-08-30 | Seed | $7.00M | Coinbase Ventures, Blockchain Capital, SevenX Ventures, Lemniscap, Maven 11 Capital, Distributed Global, Lattice, Folius Ventures |

| Team member | Role | Link |
|---|---|---|
| Jakub Wojciechowski | Founder, CEO and Founder, CEO | [X](https://x.com/kuba_eth) |
| Marcin Kazmierczak | Co-Founder, COO | [X](https://x.com/MarcinRedStone) |
| Alex Suvorov | Co-Founder | [X](https://x.com/hatskier_me/) |
| Matt Gurbiel | Head of BD | [X](https://x.com/mattgurbiel) |
...
RedStone competes in the RWA infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether RedStone has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: RedStone is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | RedStone compounds usage, liquidity improves, and RED gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If RED does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $4.45M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if RedStone announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. RedStone has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For RedStone, the first reconciliation job is identity. The token symbol is RED, the category is RWA infrastructure, chains are Base, Ethereum, exchanges are BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET, and tags are Oracle, RWA (Real World Assets). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If RED trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $44.55M, FDV is $101.33M, 24h volume is $4.45M, FDV / market cap is 2.27x, 7d price change is 11.56%, and 30d price change is 5.39%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | RedStone / RED | Match official docs, CG, CMC, Surf, contracts |
...
For RedStone, the current liquidity read is watchlist first. The market cap is $44.55M, FDV is $101.33M, reported 24h volume is $4.45M, and FDV / market cap is 2.27x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | RedStone gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://redstone.finance/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=RedStone) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=RedStone) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=RedStone) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=RedStone) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=RedStone) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/redstone_defi) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/redstone-finance) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/redstonefinance/) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](https://discord.gg/redstonedefi) | Identity, market, social, contract, or discovery cross-check |
| 11 | RedStone Price Chart (RED) - CoinGecko | [link](https://www.coingecko.com/en/coins/redstone-oracles) | Identity, market, social, contract, or discovery cross-check |
| 12 | What Is RedStone (RED) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/redstone/what-is/) | Identity, market, social, contract, or discovery cross-check |
...
| 14 | Latest RedStone (RED) Price Analysis - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/redstone/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 15 | RED TOKEN Price: RED/USD Live Price Chart, Market ... - CoinGecko | [link](https://www.coingecko.com/en/coins/red-token) | Identity, market, social, contract, or discovery cross-check |
| 16 | base RED | [link](https://basescan.org/token/0x4eb92702ba4cfbf80561bad64d89c706ac824960) | Identity, market, social, contract, or discovery cross-check |
| 17 | ethereum RED | [link](https://etherscan.io/token/0xc43c6bfeda065fe2c4c11765bf838789bd0bb5de) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google project search | [link](https://www.google.com/search?q=RedStone%20RED%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google tokenomics search | [link](https://www.google.com/search?q=RedStone%20RED%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google docs search | [link](https://www.google.com/search?q=RedStone%20docs) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google governance search | [link](https://www.google.com/search?q=RedStone%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google audit search | [link](https://www.google.com/search?q=RedStone%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/redstone/) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune discovery | [link](https://dune.com/browse/dashboards?q=RedStone) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune token query | [link](https://dune.com/browse/queries?q=RedStone) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=RedStone) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=RED) | Identity, market, social, contract, or discovery cross-check |
| 28 | TokenUnlocks search | [link](https://token.unlocks.app/?search=RedStone) | Identity, market, social, contract, or discovery cross-check |
| 29 | CryptoRank search | [link](https://cryptorank.io/price/redstone) | Identity, market, social, contract, or discovery cross-check |
| 30 | RootData search | [link](https://www.rootdata.com/search?kw=RedStone) | Identity, market, social, contract, or discovery cross-check |
| 31 | CoinDesk search | [link](https://www.coindesk.com/search?s=RedStone) | Identity, market, social, contract, or discovery cross-check |
| 32 | The Block search | [link](https://www.theblock.co/search?query=RedStone) | Identity, market, social, contract, or discovery cross-check |
| 33 | GitHub search | [link](https://github.com/search?q=RedStone&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 34 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=RED) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://redstone.finance/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=RedStone)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=RedStone)
- [DefiLlama protocol discovery](https://defillama.com/search?query=RedStone)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=RedStone)
- [Messari discovery](https://messari.io/search?q=RedStone)
- [X profile](https://x.com/redstone_defi)
- [GitHub](https://github.com/redstone-finance)
- [Telegram](https://t.me/redstonefinance/)
- [Discord](https://discord.gg/redstonedefi)
- [RedStone Price Chart (RED) - CoinGecko](https://www.coingecko.com/en/coins/redstone-oracles)
- [What Is RedStone (RED) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/redstone/what-is/)
- [CoinGecko: Cryptocurrency Prices, Charts, and Crypto Market Cap](https://www.coingecko.com/)
- [Latest RedStone (RED) Price Analysis - CoinMarketCap](https://coinmarketcap.com/cmc-ai/redstone/price-analysis/)
- [RED TOKEN Price: RED/USD Live Price Chart, Market ... - CoinGecko](https://www.coingecko.com/en/coins/red-token)
- [base RED](https://basescan.org/token/0x4eb92702ba4cfbf80561bad64d89c706ac824960)
- [ethereum RED](https://etherscan.io/token/0xc43c6bfeda065fe2c4c11765bf838789bd0bb5de)
- [Google project search](https://www.google.com/search?q=RedStone%20RED%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=RedStone%20RED%20tokenomics)
- [Google docs search](https://www.google.com/search?q=RedStone%20docs)
- [Google governance search](https://www.google.com/search?q=RedStone%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=RedStone%20audit%20report)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/redstone/)
- [Dune discovery](https://dune.com/browse/dashboards?q=RedStone)

## Final Investment View

RedStone should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
