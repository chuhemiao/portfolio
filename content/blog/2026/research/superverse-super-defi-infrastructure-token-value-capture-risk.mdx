---
title: 'SuperVerse / SUPER: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'SuperVerse is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'superverse-super-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# SuperVerse / SUPER: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** SuperVerse appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Avalanche, Base, Ethereum, Polygon Pos, Soneium; exchange exposure is BINANCE, UPBIT, COINBASE, BITHUMB, BITGET; category tags are GameFi, Gaming Infra, NFT Marketplace; current market cap is $56.58M; FDV is $88.38M; and 24h volume is $3.64M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag SuperVerse before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 408, market cap $56,533,737, 24h volume $3,620,416. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

SuperVerse is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether SUPER captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Avalanche, Base, Ethereum, Polygon Pos, Soneium |
| Exchanges | BINANCE, UPBIT, COINBASE, BITHUMB, BITGET |
| Price | $0.0884 |
| Market cap | $56.58M |
| FDV | $88.38M |
| 24h volume | $3.64M |
| Circulating supply | 640.16M |
| Total supply | 1.00B |
...
Base case: SuperVerse remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning SUPER into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

SuperVerse is a Web3 gaming ecosystem and NFT marketplace that unifies multiple crypto games and tools under a single SUPER ERC‑20 token. The project runs a DAO with staking and governance features, integrates marketplace and game products (e.g., GigaMart and Impostors), and publishes technical documentation and multi-chain token deployments.

Surf's project profile maps SuperVerse to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://superverse.cool/](https://superverse.cool/) |
| X handle | [@SuperVerse](https://x.com/SuperVerse) |
| X followers | 708.89K |
| Tags | GameFi, Gaming Infra, NFT Marketplace |
| Chains | Avalanche, Base, Ethereum, Polygon Pos, Soneium |
| Exchanges | BINANCE, UPBIT, COINBASE, BITHUMB, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether SuperVerse is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DeFi infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does SuperVerse convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Supply quality | 640.16M circulating vs 1.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that SuperVerse is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

SuperVerse's architecture should be evaluated through the job it performs. For a DeFi infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
| Settlement layer | Chains and contracts that hold user value | Avalanche, Base, Ethereum, Polygon Pos, Soneium |
...
|---|---|---|---|
| base | SUPER | 0x391359ab0ccef572dcac78f74e47d7c06db0b982 | [explorer](https://basescan.org/token/0x391359ab0ccef572dcac78f74e47d7c06db0b982) |
| Soneium | SUPER | 0xf765356c901a064a74fa4560622f21c91048c3d7 | not mapped |
| ethereum | SUPER | 0xe53ec727dbdeb9e2d5456c3be40cff031ab40a55 | [explorer](https://etherscan.io/token/0xe53ec727dbdeb9e2d5456c3be40cff031ab40a55) |
| avalanche | SUPER | 0x09fa58228bb791ea355c90da1e4783452b9bd8c3 | [explorer](https://snowtrace.io/token/0x09fa58228bb791ea355c90da1e4783452b9bd8c3) |
| polygon | SUPER | 0xa1428174f516f527fafdd146b883bb4428682737 | [explorer](https://polygonscan.com/token/0xa1428174f516f527fafdd146b883bb4428682737) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For SuperVerse, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 408, market cap $56,533,737, 24h volume $3,620,416. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $56.58M, FDV $88.38M, 24h volume $3.64M, and price $0.0884.
...
| Market cap | $56.58M | Liquid valuation proxy |
| FDV | $88.38M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.56x | Higher spread means more supply risk |
| X followers | 708.89K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for SuperVerse | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether SUPER is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. SuperVerse currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 3.83% and 30d -3.82% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that SuperVerse's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2021-02-22 | Ido | $250.00K | not disclosed |
| 2021-02-09 | Strategic | $1.20M | GBV Capital, Spark Digital Capital, Animoca Brands, AU21 Capital, Woodstock Fund, BlackEdge Capital, Solidity Ventures, Bitcoin.com |
| not disclosed | Private | $405.00K | not disclosed |
| not disclosed | Private | $1.00M | not disclosed |
| not disclosed | Private | $1.00M | not disclosed |
| not disclosed | Seed | $150.00K | not disclosed |

| Team member | Role | Link |
|---|---|---|
| Dave Murray | Co-Founder | not disclosed |
| Elliot Wainman | Co-Founder | [X](https://x.com/elliotrades) |
| Jeff Solomon  | COO | [X](https://x.com/sersolly) |
| Eric Rosloff  | Head of Product | [X](https://x.com/ericrosloff) |
| Rick Ellis | Head of Gaming | not disclosed |
...
SuperVerse competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether SuperVerse has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: SuperVerse is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | SuperVerse compounds usage, liquidity improves, and SUPER gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If SUPER does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| FDV / market cap | 1.56x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if SuperVerse announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. SuperVerse has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For SuperVerse, the first reconciliation job is identity. The token symbol is SUPER, the category is DeFi infrastructure, chains are Avalanche, Base, Ethereum, Polygon Pos, Soneium, exchanges are BINANCE, UPBIT, COINBASE, BITHUMB, BITGET, and tags are GameFi, Gaming Infra, NFT Marketplace. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If SUPER trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $56.58M, FDV is $88.38M, 24h volume is $3.64M, FDV / market cap is 1.56x, 7d price change is 3.83%, and 30d price change is -3.82%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | SuperVerse / SUPER | Match official docs, CG, CMC, Surf, contracts |
| Category | DeFi infrastructure | Confirm product mechanics and revenue model |
| Chains | Avalanche, Base, Ethereum, Polygon Pos, Soneium | Confirm canonical deployments and bridge assumptions |
| Exchanges | BINANCE, UPBIT, COINBASE, BITHUMB, BITGET | Check live order books and inactive pairs |
| Market cap | $56.58M | Compare Surf, CG, CMC, exchange pages |
| FDV | $88.38M | Reconcile total supply and unlock schedule |
...
For SuperVerse, the current liquidity read is watchlist first. The market cap is $56.58M, FDV is $88.38M, reported 24h volume is $3.64M, and FDV / market cap is 1.56x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. SuperVerse can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.

For SuperVerse, the live monitoring stack should include contract events, TVL, volume, governance, documentation changes, social announcements, security disclosures, and exchange-market quality. A single metric is not enough. TVL may stay high while liquidity thins. Volume may rise because of volatility rather than adoption. Social attention may rise around controversy rather than progress. Governance may pass proposals that improve short-term price but add long-term risk. The strongest monitoring dashboard combines on-chain state, off-chain disclosure, and market quality.

Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If SuperVerse suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | SuperVerse gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://superverse.cool/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=SuperVerse) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=SuperVerse) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=SuperVerse) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=SuperVerse) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=SuperVerse) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/SuperVerse) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/SuperVerseDAO) | Identity, market, social, contract, or discovery cross-check |
| 9 | SuperVerse Price Chart (SUPER) - CoinGecko | [link](https://www.coingecko.com/en/coins/superverse) | Identity, market, social, contract, or discovery cross-check |
| 10 | SuperVerse price SUPER #353 - CoinMarketCap | [link](https://coinmarketcap.com/currencies/superverse/) | Identity, market, social, contract, or discovery cross-check |
| 11 | SuperVerse Price History: Download SUPER Historical Data | [link](https://www.coingecko.com/en/coins/superverse/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 12 | SuperVerse (SUPER) / Tokenomics & Vesting Schedule | [link](https://tokenomist.ai/superfarm) | Identity, market, social, contract, or discovery cross-check |
| 13 | What Is SuperVerse (SUPER) And How Does It Work? | [link](https://coinmarketcap.com/cmc-ai/superverse/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 14 | base SUPER | [link](https://basescan.org/token/0x391359ab0ccef572dcac78f74e47d7c06db0b982) | Identity, market, social, contract, or discovery cross-check |
| 15 | ethereum SUPER | [link](https://etherscan.io/token/0xe53ec727dbdeb9e2d5456c3be40cff031ab40a55) | Identity, market, social, contract, or discovery cross-check |
| 16 | avalanche SUPER | [link](https://snowtrace.io/token/0x09fa58228bb791ea355c90da1e4783452b9bd8c3) | Identity, market, social, contract, or discovery cross-check |
| 17 | polygon SUPER | [link](https://polygonscan.com/token/0xa1428174f516f527fafdd146b883bb4428682737) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google project search | [link](https://www.google.com/search?q=SuperVerse%20SUPER%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google tokenomics search | [link](https://www.google.com/search?q=SuperVerse%20SUPER%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google docs search | [link](https://www.google.com/search?q=SuperVerse%20docs) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google governance search | [link](https://www.google.com/search?q=SuperVerse%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google audit search | [link](https://www.google.com/search?q=SuperVerse%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/superverse/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune discovery | [link](https://dune.com/browse/dashboards?q=SuperVerse) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune token query | [link](https://dune.com/browse/queries?q=SuperVerse) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=SuperVerse) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=SUPER) | Identity, market, social, contract, or discovery cross-check |
| 28 | TokenUnlocks search | [link](https://token.unlocks.app/?search=SuperVerse) | Identity, market, social, contract, or discovery cross-check |
| 29 | CryptoRank search | [link](https://cryptorank.io/price/superverse) | Identity, market, social, contract, or discovery cross-check |
| 30 | RootData search | [link](https://www.rootdata.com/search?kw=SuperVerse) | Identity, market, social, contract, or discovery cross-check |
| 31 | CoinDesk search | [link](https://www.coindesk.com/search?s=SuperVerse) | Identity, market, social, contract, or discovery cross-check |
| 32 | The Block search | [link](https://www.theblock.co/search?query=SuperVerse) | Identity, market, social, contract, or discovery cross-check |
| 33 | GitHub search | [link](https://github.com/search?q=SuperVerse&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 34 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=SUPER) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://superverse.cool/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=SuperVerse)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=SuperVerse)
- [DefiLlama protocol discovery](https://defillama.com/search?query=SuperVerse)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=SuperVerse)
- [Messari discovery](https://messari.io/search?q=SuperVerse)
- [X profile](https://x.com/SuperVerse)
- [Telegram](https://t.me/SuperVerseDAO)
- [SuperVerse Price Chart (SUPER) - CoinGecko](https://www.coingecko.com/en/coins/superverse)
- [SuperVerse price SUPER #353 - CoinMarketCap](https://coinmarketcap.com/currencies/superverse/)
- [SuperVerse Price History: Download SUPER Historical Data](https://www.coingecko.com/en/coins/superverse/historical_data)
- [SuperVerse (SUPER) | Tokenomics & Vesting Schedule](https://tokenomist.ai/superfarm)
- [What Is SuperVerse (SUPER) And How Does It Work?](https://coinmarketcap.com/cmc-ai/superverse/what-is/)
- [base SUPER](https://basescan.org/token/0x391359ab0ccef572dcac78f74e47d7c06db0b982)
- [ethereum SUPER](https://etherscan.io/token/0xe53ec727dbdeb9e2d5456c3be40cff031ab40a55)
- [avalanche SUPER](https://snowtrace.io/token/0x09fa58228bb791ea355c90da1e4783452b9bd8c3)
- [polygon SUPER](https://polygonscan.com/token/0xa1428174f516f527fafdd146b883bb4428682737)
- [Google project search](https://www.google.com/search?q=SuperVerse%20SUPER%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=SuperVerse%20SUPER%20tokenomics)
- [Google docs search](https://www.google.com/search?q=SuperVerse%20docs)
- [Google governance search](https://www.google.com/search?q=SuperVerse%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=SuperVerse%20audit%20report)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/superverse/price-analysis/)
- [Dune discovery](https://dune.com/browse/dashboards?q=SuperVerse)

## Final Investment View

SuperVerse should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
