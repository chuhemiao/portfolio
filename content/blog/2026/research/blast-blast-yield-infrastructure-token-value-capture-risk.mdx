---
title: 'Blast / BLAST: yield infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Blast is treated as a yield infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'blast-blast-yield-infrastructure-token-value-capture-risk'
category: 'research'
---

# Blast / BLAST: yield infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Blast appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Blast; exchange exposure is UPBIT, COINBASE, BITHUMB, BYBIT, BITGET; category tags are Layer2 & Scaling, RWA (Real World Assets), Yield & Asset Mgmt; current market cap is $18.79M; FDV is $28.98M; and 24h volume is $1.82M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Blast before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 861, market cap $18,624,322, 24h volume $8,195,958. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Blast is best understood as a **yield infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether BLAST captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Blast |
| Exchanges | UPBIT, COINBASE, BITHUMB, BYBIT, BITGET |
| Price | $0.0003 |
| Market cap | $18.79M |
| FDV | $28.98M |
| 24h volume | $1.82M |
| Circulating supply | 64.82B |
...
Base case: Blast remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning BLAST into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Blast is an EVM-compatible optimistic rollup Layer‑2 for Ethereum that distributes native yield to users via L1 ETH staking and on‑chain RWA/T‑bill protocols. Balances (ETH and the USDB stablecoin) are auto-rebasing, and the chain shares net gas revenue with dapps, enabling new yield-aware dapp business models. BLAST is the platform token.

Surf's project profile maps Blast to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://blast.io/](https://blast.io/) |
| X handle | [@blast](https://x.com/blast) |
| X followers | 708.89K |
| Tags | Layer2 & Scaling, RWA (Real World Assets), Yield & Asset Mgmt |
| Chains | Blast |
| Exchanges | UPBIT, COINBASE, BITHUMB, BYBIT, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Blast is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For yield infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Blast convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Supply quality | 64.82B circulating vs 100.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Blast is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Blast |
...
|---|---|---|---|
| Blast | BLAST | 0xb1a5700fa2358173fe465e6ea4ff52e36e88e2ad | not mapped |
| Blast | USDB | 0x4300000000000000000000000000000000000003 | not mapped |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Blast, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 861, market cap $18,624,322, 24h volume $8,195,958. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $18.79M, FDV $28.98M, 24h volume $1.82M, and price $0.0003.
...
| Market cap | $18.79M | Liquid valuation proxy |
| FDV | $28.98M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.54x | Higher spread means more supply risk |
...
| Accrual channel | Strength for Blast | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether BLAST is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Blast currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 6.51% and 30d -8.75% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Blast's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2023-11-21 | Undisclosed | $20.00M | Paradigm, eGirl Capital, Standard Crypto, Santiago Roel Santos, Larry Cermak, Hasu, CL207, Andrew Kang |

| Team member | Role | Link |
|---|---|---|
| Tieshun (Pacman) Roquerre | Founder | [X](https://twitter.com/PacmanBlur) |
...
Blast competes in the yield infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Blast has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Blast is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Blast compounds usage, liquidity improves, and BLAST gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Blast has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If BLAST does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| FDV / market cap | 1.54x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Blast announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Blast has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Blast, the first reconciliation job is identity. The token symbol is BLAST, the category is yield infrastructure, chains are Blast, exchanges are UPBIT, COINBASE, BITHUMB, BYBIT, BITGET, and tags are Layer2 & Scaling, RWA (Real World Assets), Yield & Asset Mgmt. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If BLAST trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $18.79M, FDV is $28.98M, 24h volume is $1.82M, FDV / market cap is 1.54x, 7d price change is 6.51%, and 30d price change is -8.75%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Blast / BLAST | Match official docs, CG, CMC, Surf, contracts |
| Category | yield infrastructure | Confirm product mechanics and revenue model |
| Chains | Blast | Confirm canonical deployments and bridge assumptions |
| Exchanges | UPBIT, COINBASE, BITHUMB, BYBIT, BITGET | Check live order books and inactive pairs |
| Market cap | $18.79M | Compare Surf, CG, CMC, exchange pages |
| FDV | $28.98M | Reconcile total supply and unlock schedule |
...
For Blast, the current liquidity read is watchlist first. The market cap is $18.79M, FDV is $28.98M, reported 24h volume is $1.82M, and FDV / market cap is 1.54x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Blast gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://blast.io/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Blast) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Blast) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Blast) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Blast) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Blast) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/blast) | Identity, market, social, contract, or discovery cross-check |
| 8 | Discord | [link](https://discord.gg/blast-l2) | Identity, market, social, contract, or discovery cross-check |
| 9 | BLAST/USD Live Price Chart, Market Cap & News Today - CoinGecko | [link](https://www.coingecko.com/en/coins/blast) | Identity, market, social, contract, or discovery cross-check |
| 10 | Blast price today, BLAST to USD live price, marketcap and chart | [link](https://coinmarketcap.com/currencies/blast/) | Identity, market, social, contract, or discovery cross-check |
| 11 | Blast Price & Market Data API for Tokens & NFTs - CoinGecko | [link](https://www.coingecko.com/en/api/blast) | Identity, market, social, contract, or discovery cross-check |
| 12 | Top Blast Ecosystem Coins by Market Cap - CoinGecko | [link](https://www.coingecko.com/en/categories/blast-ecosystem) | Identity, market, social, contract, or discovery cross-check |
...
| 14 | Google project search | [link](https://www.google.com/search?q=Blast%20BLAST%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 15 | Google tokenomics search | [link](https://www.google.com/search?q=Blast%20BLAST%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google docs search | [link](https://www.google.com/search?q=Blast%20docs) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google governance search | [link](https://www.google.com/search?q=Blast%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google audit search | [link](https://www.google.com/search?q=Blast%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 19 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/blast/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 20 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/blast/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 21 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Blast) | Identity, market, social, contract, or discovery cross-check |
| 22 | Dune token query | [link](https://dune.com/browse/queries?q=Blast) | Identity, market, social, contract, or discovery cross-check |
| 23 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Blast) | Identity, market, social, contract, or discovery cross-check |
| 24 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=BLAST) | Identity, market, social, contract, or discovery cross-check |
| 25 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Blast) | Identity, market, social, contract, or discovery cross-check |
| 26 | CryptoRank search | [link](https://cryptorank.io/price/blast) | Identity, market, social, contract, or discovery cross-check |
| 27 | RootData search | [link](https://www.rootdata.com/search?kw=Blast) | Identity, market, social, contract, or discovery cross-check |
| 28 | CoinDesk search | [link](https://www.coindesk.com/search?s=Blast) | Identity, market, social, contract, or discovery cross-check |
| 29 | The Block search | [link](https://www.theblock.co/search?query=Blast) | Identity, market, social, contract, or discovery cross-check |
| 30 | GitHub search | [link](https://github.com/search?q=Blast&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 31 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=BLAST) | Identity, market, social, contract, or discovery cross-check |
| 32 | BscScan token search | [link](https://bscscan.com/search?f=0&q=BLAST) | Identity, market, social, contract, or discovery cross-check |
| 33 | Arbiscan token search | [link](https://arbiscan.io/search?f=0&q=BLAST) | Identity, market, social, contract, or discovery cross-check |
| 34 | BaseScan token search | [link](https://basescan.org/search?f=0&q=BLAST) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://blast.io/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Blast)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Blast)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Blast)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Blast)
- [Messari discovery](https://messari.io/search?q=Blast)
- [X profile](https://x.com/blast)
- [Discord](https://discord.gg/blast-l2)
- [BLAST/USD Live Price Chart, Market Cap & News Today - CoinGecko](https://www.coingecko.com/en/coins/blast)
- [Blast price today, BLAST to USD live price, marketcap and chart](https://coinmarketcap.com/currencies/blast/)
- [Blast Price & Market Data API for Tokens & NFTs - CoinGecko](https://www.coingecko.com/en/api/blast)
- [Top Blast Ecosystem Coins by Market Cap - CoinGecko](https://www.coingecko.com/en/categories/blast-ecosystem)
- [CoinMarketCap: Cryptocurrency Prices, Charts And Market ...](https://coinmarketcap.com/)
- [Google project search](https://www.google.com/search?q=Blast%20BLAST%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Blast%20BLAST%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Blast%20docs)
- [Google governance search](https://www.google.com/search?q=Blast%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Blast%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/blast/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/blast/price-analysis/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Blast)
- [Dune token query](https://dune.com/browse/queries?q=Blast)
- [DeFiLlama fees discovery](https://defillama.com/fees?chain=All&search=Blast)
- [DeFiLlama yield discovery](https://defillama.com/yields?token=BLAST)
