---
title: 'B3 (Base) / B3: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'B3 (Base) is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'b3-base-b3-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# B3 (Base) / B3: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** B3 (Base) appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Base; exchange exposure is UPBIT, COINBASE, BITHUMB, BYBIT, BINANCE ALPHA; category tags are Developer Tooling, Gaming Infra, Modular & DA; current market cap is $23.55M; FDV is $50.72M; and 24h volume is $3.77M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag B3 (Base) before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 760, market cap $23,235,654, 24h volume $3,709,282. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

B3 (Base) is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether B3 captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
Base case: B3 (Base) remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning B3 into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

B3 (B3 fun) is a horizontally‑scaled Layer‑3 gaming ecosystem built on Base that enables modular appchains, developer SDKs, and consumer gaming apps with shared liquidity, identity, and incentives. The native $B3 token powers governance, staking, and in‑game transactions across the B3 network.

Surf's project profile maps B3 (Base) to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://b3.fun/](https://b3.fun/) |
| X handle | [@b3dotfun](https://x.com/b3dotfun) |
| X followers | 153.39K |
| Tags | Developer Tooling, Gaming Infra, Modular & DA |
| Chains | Base |
| Exchanges | UPBIT, COINBASE, BITHUMB, BYBIT, BINANCE ALPHA |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether B3 (Base) is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DeFi infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does B3 (Base) convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $3.77M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 46.43B circulating vs 100.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that B3 (Base) is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

B3 (Base)'s architecture should be evaluated through the job it performs. For a DeFi infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
|---|---|---|---|
| base | B3 | 0xb3b32f9f8827d4634fe7d973fa1034ec9fddb3b3 | [explorer](https://basescan.org/token/0xb3b32f9f8827d4634fe7d973fa1034ec9fddb3b3) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For B3 (Base), the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 760, market cap $23,235,654, 24h volume $3,709,282. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $23.55M, FDV $50.72M, 24h volume $3.77M, and price $0.0005.
...
| 24h volume | $3.77M | Position sizing and exit liquidity proxy |
| Market cap | $23.55M | Liquid valuation proxy |
| FDV | $50.72M | Dilution-aware valuation proxy |
| FDV / market cap spread | 2.15x | Higher spread means more supply risk |
| X followers | 153.39K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for B3 (Base) | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether B3 is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. B3 (Base) currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 0.48% and 30d -8.24% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that B3 (Base)'s product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-02-09 | Ieo | $50.00K | not disclosed |
| 2024-07-22 | Seed | $21.00M | Pantera Capital, Hashed, Bitscale Capital, Mirana Ventures, Sfermion, Makers Fund, Collab+Currency, Mantle |

| Team member | Role | Link |
|---|---|---|
| Daryl Xu | Co-Founder, CEO | [X](https://x.com/darylX24) |
| Sean Geng | Co-Founder | not disclosed |
| Viktoriya Hying | Co-Founder | not disclosed |
...
B3 (Base) competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether B3 (Base) has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: B3 (Base) is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | B3 (Base) compounds usage, liquidity improves, and B3 gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because B3 (Base) has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If B3 does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $3.77M | Weekly | Volume below position-size threshold |
| FDV / market cap | 2.15x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if B3 (Base) announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. B3 (Base) has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For B3 (Base), the first reconciliation job is identity. The token symbol is B3, the category is DeFi infrastructure, chains are Base, exchanges are UPBIT, COINBASE, BITHUMB, BYBIT, BINANCE ALPHA, and tags are Developer Tooling, Gaming Infra, Modular & DA. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If B3 trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $23.55M, FDV is $50.72M, 24h volume is $3.77M, FDV / market cap is 2.15x, 7d price change is 0.48%, and 30d price change is -8.24%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | B3 (Base) / B3 | Match official docs, CG, CMC, Surf, contracts |
...
For B3 (Base), the current liquidity read is watchlist first. The market cap is $23.55M, FDV is $50.72M, reported 24h volume is $3.77M, and FDV / market cap is 2.15x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | B3 (Base) gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://b3.fun/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=B3%20(Base) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=B3%20(Base) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=B3%20(Base) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=B3%20(Base) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=B3%20(Base) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/b3dotfun) | Identity, market, social, contract, or discovery cross-check |
| 8 | B3 (Base) Price: B3/USD Live Price Chart, Market Cap ... | [link](https://www.coingecko.com/en/coins/b3) | Identity, market, social, contract, or discovery cross-check |
| 9 | B3 (Base) (B3) Price Prediction For 2026 & Beyond | [link](https://coinmarketcap.com/cmc-ai/b3/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 10 | B3 (Base) Price History: Download B3 Historical Data | [link](https://www.coingecko.com/en/coins/b3/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 11 | What Is B3 (Base) (B3) And How Does It Work? | [link](https://coinmarketcap.com/cmc-ai/b3/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 12 | B3 to ETH: B3 (Base) Price in Ether | [link](https://www.coingecko.com/en/coins/b3/eth) | Identity, market, social, contract, or discovery cross-check |
| 13 | base B3 | [link](https://basescan.org/token/0xb3b32f9f8827d4634fe7d973fa1034ec9fddb3b3) | Identity, market, social, contract, or discovery cross-check |
| 14 | Google project search | [link](https://www.google.com/search?q=B3%20(Base)%20B3%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 15 | Google tokenomics search | [link](https://www.google.com/search?q=B3%20(Base)%20B3%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google docs search | [link](https://www.google.com/search?q=B3%20(Base)%20docs) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google governance search | [link](https://www.google.com/search?q=B3%20(Base)%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google audit search | [link](https://www.google.com/search?q=B3%20(Base)%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 19 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/b3-base/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 20 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/b3-base/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/b3-base/) | Identity, market, social, contract, or discovery cross-check |
| 22 | Dune discovery | [link](https://dune.com/browse/dashboards?q=B3%20(Base) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune token query | [link](https://dune.com/browse/queries?q=B3%20(Base) | Identity, market, social, contract, or discovery cross-check |
| 24 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=B3%20(Base) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=B3) | Identity, market, social, contract, or discovery cross-check |
| 26 | TokenUnlocks search | [link](https://token.unlocks.app/?search=B3%20(Base) | Identity, market, social, contract, or discovery cross-check |
| 27 | CryptoRank search | [link](https://cryptorank.io/price/b3-base) | Identity, market, social, contract, or discovery cross-check |
| 28 | RootData search | [link](https://www.rootdata.com/search?kw=B3%20(Base) | Identity, market, social, contract, or discovery cross-check |
| 29 | CoinDesk search | [link](https://www.coindesk.com/search?s=B3%20(Base) | Identity, market, social, contract, or discovery cross-check |
| 30 | The Block search | [link](https://www.theblock.co/search?query=B3%20(Base) | Identity, market, social, contract, or discovery cross-check |
| 31 | GitHub search | [link](https://github.com/search?q=B3%20(Base)&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 32 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=B3) | Identity, market, social, contract, or discovery cross-check |
| 33 | BscScan token search | [link](https://bscscan.com/search?f=0&q=B3) | Identity, market, social, contract, or discovery cross-check |
| 34 | Arbiscan token search | [link](https://arbiscan.io/search?f=0&q=B3) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://b3.fun/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=B3%20(Base)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=B3%20(Base)
- [DefiLlama protocol discovery](https://defillama.com/search?query=B3%20(Base)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=B3%20(Base)
- [Messari discovery](https://messari.io/search?q=B3%20(Base)
- [X profile](https://x.com/b3dotfun)
- [B3 (Base) Price: B3/USD Live Price Chart, Market Cap ...](https://www.coingecko.com/en/coins/b3)
- [B3 (Base) (B3) Price Prediction For 2026 & Beyond](https://coinmarketcap.com/cmc-ai/b3/price-prediction/)
- [B3 (Base) Price History: Download B3 Historical Data](https://www.coingecko.com/en/coins/b3/historical_data)
- [What Is B3 (Base) (B3) And How Does It Work?](https://coinmarketcap.com/cmc-ai/b3/what-is/)
- [B3 to ETH: B3 (Base) Price in Ether](https://www.coingecko.com/en/coins/b3/eth)
- [base B3](https://basescan.org/token/0xb3b32f9f8827d4634fe7d973fa1034ec9fddb3b3)
- [Google project search](https://www.google.com/search?q=B3%20(Base)%20B3%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=B3%20(Base)%20B3%20tokenomics)
- [Google docs search](https://www.google.com/search?q=B3%20(Base)%20docs)
- [Google governance search](https://www.google.com/search?q=B3%20(Base)%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=B3%20(Base)%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/b3-base/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/b3-base/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/b3-base/)
- [Dune discovery](https://dune.com/browse/dashboards?q=B3%20(Base)
- [Dune token query](https://dune.com/browse/queries?q=B3%20(Base)
- [DeFiLlama fees discovery](https://defillama.com/fees?chain=All&search=B3%20(Base)

## Final Investment View

B3 (Base) should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
