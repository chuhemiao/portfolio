---
title: 'Ekubo / EKUBO: DEX / liquidity infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Ekubo is treated as a DEX / liquidity infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'ekubo-ekubo-dex-liquidity-infrastructure-token-value-capture-risk'
category: 'research'
---

# Ekubo / EKUBO: DEX / liquidity infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Ekubo appears in the local CoinGecko/CMC-oriented candidate pool through [CoinGecko](https://www.coingecko.com/en/coins/ekubo-protocol) and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Ethereum, Starknet; exchange exposure is not disclosed; category tags are DEX & AMM; current market cap is $5.32M; FDV is $5.32M; and 24h volume is $7.74K. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Ekubo before this file was created, and the existing candidate note says: DefiLlama category Dexs on Multi-Chain; TVL $20,689,446. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Ekubo is best understood as a **DEX / liquidity infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether EKUBO captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
Base case: Ekubo remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning EKUBO into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Ekubo Protocol is an advanced AMM with concentrated liquidity, singleton architecture, and extensible features for optimal efficiency. V3 enables multichain deployment on Starknet and EVM chains with open-source contracts. The EKUBO token facilitates governance and revenue sharing.

Surf's project profile maps Ekubo to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://ekubo.org/](https://ekubo.org/) |
| X handle | [@EkuboProtocol](https://x.com/EkuboProtocol) |
| X followers | 22.69K |
| Tags | DEX & AMM |
| Chains | Ethereum, Starknet |
| Exchanges | not disclosed |
| TGE status | not disclosed |
| CoinGecko ID | [ekubo-protocol](https://www.coingecko.com/en/coins/ekubo-protocol) |

The main product question is whether Ekubo is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DEX / liquidity infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Ekubo convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $7.74K 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 10.00M circulating vs 10.00M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Ekubo is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Ethereum, Starknet |
...
|---|---|---|---|
| ethereum | EKUBO | 0x04c46e830bb56ce22735d5d8fc9cb90309317d0f | [explorer](https://etherscan.io/token/0x04c46e830bb56ce22735d5d8fc9cb90309317d0f) |
| Starknet | EKUBO | 0x75afe6402ad5a5c20dd25e10ec3b3986acaa647b77e4ae24b0cbc9a54a27a87 | not mapped |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Ekubo, the DeFi-oriented backlog note is the first signal: DefiLlama category Dexs on Multi-Chain; TVL $20,689,446. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $5.32M, FDV $5.32M, 24h volume $7.74K, and price $0.5320.
...
| 24h volume | $7.74K | Position sizing and exit liquidity proxy |
| Market cap | $5.32M | Liquid valuation proxy |
| FDV | $5.32M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.00x | Higher spread means more supply risk |
| X followers | 22.69K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Ekubo | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether EKUBO is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Ekubo currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 10.41% and 30d 22.58% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Ekubo's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-03-18 | Undisclosed | not disclosed | Delphi Ventures |
| 2023-10-29 | Undisclosed | $12.00M | Uniswap Labs Ventures |

| Team member | Role | Link |
|---|---|---|
| Moody Salem | Founder | [X](https://twitter.com/sendmoodz) |
| Yohan | Software Engineer | [X](https://x.com/aholouu) |
...
Ekubo competes in the DEX / liquidity infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Ekubo has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Ekubo is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Ekubo compounds usage, liquidity improves, and EKUBO gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Ekubo has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
| 24h volume | $7.74K | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Ekubo announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
For Ekubo, the current liquidity read is watchlist first. The market cap is $5.32M, FDV is $5.32M, reported 24h volume is $7.74K, and FDV / market cap is 1.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Ekubo gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://ekubo.org/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko profile | [link](https://www.coingecko.com/en/coins/ekubo-protocol) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Ekubo) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Ekubo) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Ekubo) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Ekubo) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/EkuboProtocol) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/EkuboProtocol) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/Ekubo_Protocol) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](https://discord.com/invite/RFbSXxtqUG) | Identity, market, social, contract, or discovery cross-check |
...
| 12 | Ekubo V3 (Ethereum) Statistics: Markets, Trading Volume & ... | [link](https://www.coingecko.com/en/exchanges/ekubo-v3-ethereum) | Identity, market, social, contract, or discovery cross-check |
| 13 | Governance info - Ekubo Interface | [link](https://ekubo.org/governance/info) | Identity, market, social, contract, or discovery cross-check |
| 14 | EKUBO to ETH: Ekubo Protocol Price in Ether | [link](https://www.coingecko.com/en/coins/ekubo-protocol/eth) | Identity, market, social, contract, or discovery cross-check |
| 15 | ethereum EKUBO | [link](https://etherscan.io/token/0x04c46e830bb56ce22735d5d8fc9cb90309317d0f) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google project search | [link](https://www.google.com/search?q=Ekubo%20EKUBO%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google tokenomics search | [link](https://www.google.com/search?q=Ekubo%20EKUBO%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google docs search | [link](https://www.google.com/search?q=Ekubo%20docs) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google governance search | [link](https://www.google.com/search?q=Ekubo%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google audit search | [link](https://www.google.com/search?q=Ekubo%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinGecko historical data | [link](https://www.coingecko.com/en/coins/ekubo-protocol/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/ekubo/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/ekubo/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/ekubo/) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Ekubo) | Identity, market, social, contract, or discovery cross-check |
| 26 | Dune token query | [link](https://dune.com/browse/queries?q=Ekubo) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Ekubo) | Identity, market, social, contract, or discovery cross-check |
| 28 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=EKUBO) | Identity, market, social, contract, or discovery cross-check |
| 29 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Ekubo) | Identity, market, social, contract, or discovery cross-check |
| 30 | CryptoRank search | [link](https://cryptorank.io/price/ekubo) | Identity, market, social, contract, or discovery cross-check |
| 31 | RootData search | [link](https://www.rootdata.com/search?kw=Ekubo) | Identity, market, social, contract, or discovery cross-check |
| 32 | CoinDesk search | [link](https://www.coindesk.com/search?s=Ekubo) | Identity, market, social, contract, or discovery cross-check |
| 33 | The Block search | [link](https://www.theblock.co/search?query=Ekubo) | Identity, market, social, contract, or discovery cross-check |
| 34 | GitHub search | [link](https://github.com/search?q=Ekubo&type=repositories) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://ekubo.org/)
- [CoinGecko profile](https://www.coingecko.com/en/coins/ekubo-protocol)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Ekubo)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Ekubo)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Ekubo)
- [Messari discovery](https://messari.io/search?q=Ekubo)
- [X profile](https://x.com/EkuboProtocol)
...
- [Ekubo V3 (Ethereum) Statistics: Markets, Trading Volume & ...](https://www.coingecko.com/en/exchanges/ekubo-v3-ethereum)
- [Governance info - Ekubo Interface](https://ekubo.org/governance/info)
- [EKUBO to ETH: Ekubo Protocol Price in Ether](https://www.coingecko.com/en/coins/ekubo-protocol/eth)
- [ethereum EKUBO](https://etherscan.io/token/0x04c46e830bb56ce22735d5d8fc9cb90309317d0f)
- [Google project search](https://www.google.com/search?q=Ekubo%20EKUBO%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Ekubo%20EKUBO%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Ekubo%20docs)
- [Google governance search](https://www.google.com/search?q=Ekubo%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Ekubo%20audit%20report)
- [CoinGecko historical data](https://www.coingecko.com/en/coins/ekubo-protocol/historical_data)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/ekubo/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/ekubo/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/ekubo/)

## Final Investment View

Ekubo should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
