---
title: 'Propy / PRO: RWA infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Propy is treated as a RWA infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'propy-pro-rwa-infrastructure-token-value-capture-risk'
category: 'research'
---

# Propy / PRO: RWA infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Propy appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Base, Ethereum; exchange exposure is COINBASE; category tags are NFT Collections, Payments & Gateway, RWA (Real World Assets), Web3 AI (Agent/AIGC); current market cap is $23.20M; FDV is $40.07M; and 24h volume is $4.88M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Propy before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 773, market cap $22,058,601, 24h volume $7,494,653. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Propy is best understood as a **RWA infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether PRO captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| FDV | $40.07M |
| 24h volume | $4.88M |
| Circulating supply | 57.90M |
| Total supply | 100.00M |
| 7d price change | 13.62% |
| 30d price change | 47.21% |
...
Base case: Propy remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning PRO into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Propy is a blockchain and AI-powered platform automating real estate transactions, title, and escrow services. It offers onchain deed recording, AI assistants for efficiency, and supports crypto/NFT property transfers. PRO token enables platform utilities and settlements.

Surf's project profile maps Propy to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://propy.com/](https://propy.com/) |
| X handle | [@PropyInc](https://x.com/PropyInc) |
| X followers | 62.01K |
| Tags | NFT Collections, Payments & Gateway, RWA (Real World Assets), Web3 AI (Agent/AIGC) |
| Chains | Base, Ethereum |
| Exchanges | COINBASE |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Propy is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For RWA infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Propy convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $4.88M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 57.90M circulating vs 100.00M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Propy is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

Propy's architecture should be evaluated through the job it performs. For a RWA infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
|---|---|---|---|
| base | PRO | 0x18dd5b087bca9920562aff7a0199b96b9230438b | [explorer](https://basescan.org/token/0x18dd5b087bca9920562aff7a0199b96b9230438b) |
| ethereum | PRO | 0x226bb599a12c826476e3a771454697ea52e9e220 | [explorer](https://etherscan.io/token/0x226bb599a12c826476e3a771454697ea52e9e220) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Propy, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 773, market cap $22,058,601, 24h volume $7,494,653. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $23.20M, FDV $40.07M, 24h volume $4.88M, and price $0.4007.
...
| 24h volume | $4.88M | Position sizing and exit liquidity proxy |
| Market cap | $23.20M | Liquid valuation proxy |
| FDV | $40.07M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.73x | Higher spread means more supply risk |
| X followers | 62.01K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Propy | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether PRO is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Propy currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 13.62% and 30d 47.21% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Propy's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2026-01-29 | Debt Financing | $100.00M | not disclosed |
| 2022-11-08 | Undisclosed | $1.50M | not disclosed |
| 2020-10-03 | Strategic | $1.20M | Tim Draper |
| 2019-06-10 | Undisclosed | not disclosed | Second Century Ventures |
| 2017-09-18 | Private Token Sale | $15.00M | not disclosed |

| Team member | Role | Link |
|---|---|---|
| Natalia Karayaneva | Founder, CEO | [X](https://x.com/NataliePropy) |
| Maria Angelova | Co-Founder, CFO | not disclosed |
| Denitza Tyufekchieva | Co-Founder, VP, BD | [X](https://x.com/DenitzaPropy) |
| Eric L. Cruz | CRO | [X](https://x.com/realEricCruz) |
...
Propy competes in the RWA infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Propy has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Propy is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Propy compounds usage, liquidity improves, and PRO gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Propy has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If PRO does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $4.88M | Weekly | Volume below position-size threshold |
| FDV / market cap | 1.73x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Propy announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Propy has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Propy, the first reconciliation job is identity. The token symbol is PRO, the category is RWA infrastructure, chains are Base, Ethereum, exchanges are COINBASE, and tags are NFT Collections, Payments & Gateway, RWA (Real World Assets), Web3 AI (Agent/AIGC). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If PRO trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $23.20M, FDV is $40.07M, 24h volume is $4.88M, FDV / market cap is 1.73x, 7d price change is 13.62%, and 30d price change is 47.21%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Propy / PRO | Match official docs, CG, CMC, Surf, contracts |
...
For Propy, the current liquidity read is watchlist first. The market cap is $23.20M, FDV is $40.07M, reported 24h volume is $4.88M, and FDV / market cap is 1.73x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Propy gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://propy.com/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Propy) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Propy) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Propy) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Propy) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Propy) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/PropyInc) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/propy) | Identity, market, social, contract, or discovery cross-check |
| 9 | Propy Price: PRO/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/propy) | Identity, market, social, contract, or discovery cross-check |
| 10 | What Is Propy (PRO) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/propy/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 11 | Propy Price History: Download PRO Historical Data / CoinGecko | [link](https://www.coingecko.com/en/coins/propy/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 12 | PRO to INR: Propy Price in Indian Rupee - CoinGecko | [link](https://www.coingecko.com/en/coins/propy/inr) | Identity, market, social, contract, or discovery cross-check |
| 13 | Latest Propy (PRO) Price Analysis - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/propy/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 14 | base PRO | [link](https://basescan.org/token/0x18dd5b087bca9920562aff7a0199b96b9230438b) | Identity, market, social, contract, or discovery cross-check |
| 15 | ethereum PRO | [link](https://etherscan.io/token/0x226bb599a12c826476e3a771454697ea52e9e220) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google project search | [link](https://www.google.com/search?q=Propy%20PRO%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google tokenomics search | [link](https://www.google.com/search?q=Propy%20PRO%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google docs search | [link](https://www.google.com/search?q=Propy%20docs) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google governance search | [link](https://www.google.com/search?q=Propy%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google audit search | [link](https://www.google.com/search?q=Propy%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/propy/) | Identity, market, social, contract, or discovery cross-check |
| 22 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Propy) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune token query | [link](https://dune.com/browse/queries?q=Propy) | Identity, market, social, contract, or discovery cross-check |
| 24 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Propy) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=PRO) | Identity, market, social, contract, or discovery cross-check |
| 26 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Propy) | Identity, market, social, contract, or discovery cross-check |
| 27 | CryptoRank search | [link](https://cryptorank.io/price/propy) | Identity, market, social, contract, or discovery cross-check |
| 28 | RootData search | [link](https://www.rootdata.com/search?kw=Propy) | Identity, market, social, contract, or discovery cross-check |
| 29 | CoinDesk search | [link](https://www.coindesk.com/search?s=Propy) | Identity, market, social, contract, or discovery cross-check |
| 30 | The Block search | [link](https://www.theblock.co/search?query=Propy) | Identity, market, social, contract, or discovery cross-check |
| 31 | GitHub search | [link](https://github.com/search?q=Propy&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 32 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=PRO) | Identity, market, social, contract, or discovery cross-check |
| 33 | BscScan token search | [link](https://bscscan.com/search?f=0&q=PRO) | Identity, market, social, contract, or discovery cross-check |
| 34 | Arbiscan token search | [link](https://arbiscan.io/search?f=0&q=PRO) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://propy.com/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Propy)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Propy)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Propy)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Propy)
- [Messari discovery](https://messari.io/search?q=Propy)
- [X profile](https://x.com/PropyInc)
- [Telegram](https://t.me/propy)
- [Propy Price: PRO/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/propy)
- [What Is Propy (PRO) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/propy/what-is/)
- [Propy Price History: Download PRO Historical Data | CoinGecko](https://www.coingecko.com/en/coins/propy/historical_data)
- [PRO to INR: Propy Price in Indian Rupee - CoinGecko](https://www.coingecko.com/en/coins/propy/inr)
- [Latest Propy (PRO) Price Analysis - CoinMarketCap](https://coinmarketcap.com/cmc-ai/propy/price-analysis/)
- [base PRO](https://basescan.org/token/0x18dd5b087bca9920562aff7a0199b96b9230438b)
- [ethereum PRO](https://etherscan.io/token/0x226bb599a12c826476e3a771454697ea52e9e220)
- [Google project search](https://www.google.com/search?q=Propy%20PRO%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Propy%20PRO%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Propy%20docs)
- [Google governance search](https://www.google.com/search?q=Propy%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Propy%20audit%20report)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/propy/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Propy)
- [Dune token query](https://dune.com/browse/queries?q=Propy)
- [DeFiLlama fees discovery](https://defillama.com/fees?chain=All&search=Propy)

## Final Investment View

Propy should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
