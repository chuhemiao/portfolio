---
title: 'Avalon / AVL: stablecoin / credit infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Avalon is treated as a stablecoin / credit infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'avalon-avl-stablecoin-credit-infrastructure-token-value-capture-risk'
category: 'research'
---

# Avalon / AVL: stablecoin / credit infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Avalon appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Binance Smart Chain, Bitlayer, Ethereum, Mantle, Merlin Chain; exchange exposure is BITHUMB, BYBIT, BINANCE ALPHA, ASTER; category tags are Lending & Borrowing, Yield & Asset Mgmt, Stablecoins; current market cap is $7.59M; FDV is $22.95M; and 24h volume is $379.38K. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Avalon before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 1322, market cap $7,575,361, 24h volume $387,593. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Avalon is best understood as a **stablecoin / credit infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether AVL captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Binance Smart Chain, Bitlayer, Ethereum, Mantle, Merlin Chain |
| Exchanges | BITHUMB, BYBIT, BINANCE ALPHA, ASTER |
| Price | $0.0229 |
| Market cap | $7.59M |
| FDV | $22.95M |
| 24h volume | $379.38K |
| Circulating supply | 330.89M |
| Total supply | 1.00B |
...
Base case: Avalon remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning AVL into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Avalon Labs builds an on-chain financial center for Bitcoin, offering BTC-backed lending, a Bitcoin-backed stablecoin (USDa), yield accounts, and a governance token (AVL). Its hybrid CeDeFi model connects institutional liquidity with DeFi lending (USDaLend, CeDeFi CDP), supports omnichain interoperability, and emphasizes fixed-rate borrowing and risk-managed isolated pools.

Surf's project profile maps Avalon to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.avalonfinance.xyz/](https://www.avalonfinance.xyz/) |
| X handle | [@avalonfinance_](https://x.com/avalonfinance_) |
| X followers | 287.50K |
| Tags | Lending & Borrowing, Yield & Asset Mgmt, Stablecoins |
| Chains | Binance Smart Chain, Bitlayer, Ethereum, Mantle, Merlin Chain |
| Exchanges | BITHUMB, BYBIT, BINANCE ALPHA, ASTER |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Avalon is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For stablecoin / credit infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Avalon convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $379.38K 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 330.89M circulating vs 1.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Avalon is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Binance Smart Chain, Bitlayer, Ethereum, Mantle, Merlin Chain |
...
|---|---|---|---|
| Bitlayer | AVL | 0x3228995749610bea00b59c44f8d1df21c14027f1 | not mapped |
| ethereum | AVL | 0x5c8d0c48810fd37a0a824d074ee290e64f7a8fa2 | [explorer](https://etherscan.io/token/0x5c8d0c48810fd37a0a824d074ee290e64f7a8fa2) |
| Merlin Chain | AVL | 0x916addd975718d307868b814e0a9bbbedbd7ab17 | not mapped |
| bsc | AVL | 0x9beee89723ceec27d7c2834bec6834208ffdc202 | [explorer](https://bscscan.com/token/0x9beee89723ceec27d7c2834bec6834208ffdc202) |
| Mantle | USDA | 0x075df695b8e7f4361fa7f8c1426c63f11b06e326 | not mapped |
| ethereum | USDA | 0x8a60e489004ca22d775c5f2c657598278d17d9c2 | [explorer](https://etherscan.io/token/0x8a60e489004ca22d775c5f2c657598278d17d9c2) |
| bsc | USDA | 0x9356086146be5158e98ad827e21b5cf944699894 | [explorer](https://bscscan.com/token/0x9356086146be5158e98ad827e21b5cf944699894) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Avalon, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 1322, market cap $7,575,361, 24h volume $387,593. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $7.59M, FDV $22.95M, 24h volume $379.38K, and price $0.0229.
...
| 24h volume | $379.38K | Position sizing and exit liquidity proxy |
| Market cap | $7.59M | Liquid valuation proxy |
| FDV | $22.95M | Dilution-aware valuation proxy |
| FDV / market cap spread | 3.02x | Higher spread means more supply risk |
| X followers | 287.50K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Avalon | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether AVL is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Avalon currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 7.44% and 30d -16.96% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Avalon's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-05-26 | Undisclosed | not disclosed | YZi Labs (Prev. Binance Labs) |
| 2024-12-23 | Series A | $10.00M | Kenetic Capital, Framework Ventures, SNZ Holding, Antalpha Global, UTXO Management, Presto Labs, GeekCartel, Marblex |
| 2024-03-15 | Seed | $1.50M | Spark Digital Capital, Matrixport, SNZ Holding, Marblex, Summer Ventures, Web3Port |

| Team member | Role | Link |
|---|---|---|
| Venus Li | Co-Founder | [X](https://x.com/0xvebb) |
...
Avalon competes in the stablecoin / credit infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Avalon has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Avalon is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Avalon compounds usage, liquidity improves, and AVL gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Avalon has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If AVL does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $379.38K | Weekly | Volume below position-size threshold |
| FDV / market cap | 3.02x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Avalon announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Avalon has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Avalon, the first reconciliation job is identity. The token symbol is AVL, the category is stablecoin / credit infrastructure, chains are Binance Smart Chain, Bitlayer, Ethereum, Mantle, Merlin Chain, exchanges are BITHUMB, BYBIT, BINANCE ALPHA, ASTER, and tags are Lending & Borrowing, Yield & Asset Mgmt, Stablecoins. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If AVL trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $7.59M, FDV is $22.95M, 24h volume is $379.38K, FDV / market cap is 3.02x, 7d price change is 7.44%, and 30d price change is -16.96%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Avalon / AVL | Match official docs, CG, CMC, Surf, contracts |
| Category | stablecoin / credit infrastructure | Confirm product mechanics and revenue model |
| Chains | Binance Smart Chain, Bitlayer, Ethereum, Mantle, Merlin Chain | Confirm canonical deployments and bridge assumptions |
| Exchanges | BITHUMB, BYBIT, BINANCE ALPHA, ASTER | Check live order books and inactive pairs |
| Market cap | $7.59M | Compare Surf, CG, CMC, exchange pages |
| FDV | $22.95M | Reconcile total supply and unlock schedule |
| Volume | $379.38K | Check exchange concentration and wash-trading risk |
| TVL | not disclosed | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Avalon does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Avalon, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does AVL sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
For Avalon, the current liquidity read is watchlist first. The market cap is $7.59M, FDV is $22.95M, reported 24h volume is $379.38K, and FDV / market cap is 3.02x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Avalon gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.avalonfinance.xyz/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Avalon) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Avalon) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Avalon) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Avalon) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Avalon) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/avalonfinance_) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/avalonfinancexyz) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/AvalonLabs_Official) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](https://discord.com/invite/theofficialavalonlabs) | Identity, market, social, contract, or discovery cross-check |
| 11 | Avalon Price: AVL/USD Live Price Chart, Market Cap ... | [link](https://www.coingecko.com/en/coins/avalon-2) | Identity, market, social, contract, or discovery cross-check |
| 12 | What Is Avalon Labs (AVL) (AVL) And How Does It Work? | [link](https://coinmarketcap.com/cmc-ai/avalon-labs-avl/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 13 | AVL to INR: Avalon Price in Indian Rupee | [link](https://www.coingecko.com/en/coins/avalon-2/inr) | Identity, market, social, contract, or discovery cross-check |
| 14 | AVL Tokenomics | [link](https://docs.avalonfinance.xyz/avl-governance-token/avl-tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 15 | AVL to BTC: Avalon Price in Bitcoin | [link](https://www.coingecko.com/en/coins/avalon-2/btc) | Identity, market, social, contract, or discovery cross-check |
| 16 | ethereum AVL | [link](https://etherscan.io/token/0x5c8d0c48810fd37a0a824d074ee290e64f7a8fa2) | Identity, market, social, contract, or discovery cross-check |
| 17 | bsc AVL | [link](https://bscscan.com/token/0x9beee89723ceec27d7c2834bec6834208ffdc202) | Identity, market, social, contract, or discovery cross-check |
| 18 | ethereum USDA | [link](https://etherscan.io/token/0x8a60e489004ca22d775c5f2c657598278d17d9c2) | Identity, market, social, contract, or discovery cross-check |
| 19 | bsc USDA | [link](https://bscscan.com/token/0x9356086146be5158e98ad827e21b5cf944699894) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google project search | [link](https://www.google.com/search?q=Avalon%20AVL%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google tokenomics search | [link](https://www.google.com/search?q=Avalon%20AVL%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google docs search | [link](https://www.google.com/search?q=Avalon%20docs) | Identity, market, social, contract, or discovery cross-check |
| 23 | Google governance search | [link](https://www.google.com/search?q=Avalon%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 24 | Google audit search | [link](https://www.google.com/search?q=Avalon%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 25 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/avalon/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 26 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/avalon/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 27 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/avalon/) | Identity, market, social, contract, or discovery cross-check |
| 28 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Avalon) | Identity, market, social, contract, or discovery cross-check |
| 29 | Dune token query | [link](https://dune.com/browse/queries?q=Avalon) | Identity, market, social, contract, or discovery cross-check |
| 30 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Avalon) | Identity, market, social, contract, or discovery cross-check |
| 31 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=AVL) | Identity, market, social, contract, or discovery cross-check |
| 32 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Avalon) | Identity, market, social, contract, or discovery cross-check |
| 33 | CryptoRank search | [link](https://cryptorank.io/price/avalon) | Identity, market, social, contract, or discovery cross-check |
| 34 | RootData search | [link](https://www.rootdata.com/search?kw=Avalon) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.avalonfinance.xyz/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Avalon)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Avalon)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Avalon)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Avalon)
- [Messari discovery](https://messari.io/search?q=Avalon)
- [X profile](https://x.com/avalonfinance_)
- [GitHub](https://github.com/avalonfinancexyz)
- [Telegram](https://t.me/AvalonLabs_Official)
- [Discord](https://discord.com/invite/theofficialavalonlabs)
- [Avalon Price: AVL/USD Live Price Chart, Market Cap ...](https://www.coingecko.com/en/coins/avalon-2)
- [What Is Avalon Labs (AVL) (AVL) And How Does It Work?](https://coinmarketcap.com/cmc-ai/avalon-labs-avl/what-is/)
- [AVL to INR: Avalon Price in Indian Rupee](https://www.coingecko.com/en/coins/avalon-2/inr)
- [AVL Tokenomics](https://docs.avalonfinance.xyz/avl-governance-token/avl-tokenomics)
- [AVL to BTC: Avalon Price in Bitcoin](https://www.coingecko.com/en/coins/avalon-2/btc)
- [ethereum AVL](https://etherscan.io/token/0x5c8d0c48810fd37a0a824d074ee290e64f7a8fa2)
- [bsc AVL](https://bscscan.com/token/0x9beee89723ceec27d7c2834bec6834208ffdc202)
- [ethereum USDA](https://etherscan.io/token/0x8a60e489004ca22d775c5f2c657598278d17d9c2)
- [bsc USDA](https://bscscan.com/token/0x9356086146be5158e98ad827e21b5cf944699894)
- [Google project search](https://www.google.com/search?q=Avalon%20AVL%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Avalon%20AVL%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Avalon%20docs)
- [Google governance search](https://www.google.com/search?q=Avalon%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Avalon%20audit%20report)

## Final Investment View

Avalon should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
