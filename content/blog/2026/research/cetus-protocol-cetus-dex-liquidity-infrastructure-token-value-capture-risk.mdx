---
title: 'Cetus Protocol / CETUS: DEX / liquidity infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Cetus Protocol is treated as a DEX / liquidity infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'cetus-protocol-cetus-dex-liquidity-infrastructure-token-value-capture-risk'
category: 'research'
---

# Cetus Protocol / CETUS: DEX / liquidity infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Cetus Protocol appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Sui; exchange exposure is BINANCE, OKX, BITGET; category tags are DEX & AMM, MEV & Intent, Yield & Asset Mgmt; current market cap is $17.61M; FDV is $18.57M; and 24h volume is $1.94M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Cetus Protocol before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 885, market cap $17,677,647, 24h volume $1,794,242. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Cetus Protocol is best understood as a **DEX / liquidity infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether CETUS captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
Base case: Cetus Protocol remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning CETUS into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Cetus Protocol is a concentrated‑liquidity automated market maker (AMM) and decentralized exchange built for Sui and Aptos, offering swaps, liquidity pools and an aggregator to access deep on‑chain liquidity. It uses a dual‑token model (CETUS and xCETUS) for governance and rewards and provides intent orders, vaults, liquidity mining, and developer integration tools.

Surf's project profile maps Cetus Protocol to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.cetus.zone/](https://www.cetus.zone/) |
| X handle | [@CetusProtocol](https://x.com/CetusProtocol) |
| X followers | 477.30K |
| Tags | DEX & AMM, MEV & Intent, Yield & Asset Mgmt |
| Chains | Sui |
| Exchanges | BINANCE, OKX, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Cetus Protocol is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DEX / liquidity infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Cetus Protocol convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $1.94M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 947.83M circulating vs 1.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Cetus Protocol is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

Cetus Protocol's architecture should be evaluated through the job it performs. For a DEX / liquidity infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
| Settlement layer | Chains and contracts that hold user value | Sui |
...
|---|---|---|---|
| Sui | CETUS | 0x06864a6f921804860930db6ddbe2e16acdf8504495ea7481637a1c8b9a8fe54b::cetus::CETUS | not mapped |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Cetus Protocol, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 885, market cap $17,677,647, 24h volume $1,794,242. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $17.61M, FDV $18.57M, 24h volume $1.94M, and price $0.0186.
...
| 24h volume | $1.94M | Position sizing and exit liquidity proxy |
| Market cap | $17.61M | Liquid valuation proxy |
| FDV | $18.57M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.06x | Higher spread means more supply risk |
| X followers | 477.30K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Cetus Protocol | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether CETUS is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Cetus Protocol currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 7.00% and 30d 1.57% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Cetus Protocol's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2024-02-29 | Strategic | not disclosed | Sui Foundation |
| 2023-05-02 | Seed | not disclosed | NGC Ventures, Animoca Brands, OKX Ventures, Arche Fund, Jump Crypto, Adaverse, AC Capital, IDG Capital |

| Team member | Role | Link |
|---|---|---|
| Amy Huang | Head of BD | [X](https://x.com/amywph) |
| Pika | Core Contributor | [X](https://x.com/pikapikasui) |
...
Cetus Protocol competes in the DEX / liquidity infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Cetus Protocol has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Cetus Protocol is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Cetus Protocol compounds usage, liquidity improves, and CETUS gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If CETUS does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $1.94M | Weekly | Volume below position-size threshold |
| FDV / market cap | 1.06x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Cetus Protocol announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Cetus Protocol has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Cetus Protocol, the first reconciliation job is identity. The token symbol is CETUS, the category is DEX / liquidity infrastructure, chains are Sui, exchanges are BINANCE, OKX, BITGET, and tags are DEX & AMM, MEV & Intent, Yield & Asset Mgmt. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If CETUS trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $17.61M, FDV is $18.57M, 24h volume is $1.94M, FDV / market cap is 1.06x, 7d price change is 7.00%, and 30d price change is 1.57%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Cetus Protocol / CETUS | Match official docs, CG, CMC, Surf, contracts |
| Category | DEX / liquidity infrastructure | Confirm product mechanics and revenue model |
| Chains | Sui | Confirm canonical deployments and bridge assumptions |
| Exchanges | BINANCE, OKX, BITGET | Check live order books and inactive pairs |
| Market cap | $17.61M | Compare Surf, CG, CMC, exchange pages |
| FDV | $18.57M | Reconcile total supply and unlock schedule |
| Volume | $1.94M | Check exchange concentration and wash-trading risk |
| TVL | not disclosed | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Cetus Protocol does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Cetus Protocol, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does CETUS sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
For Cetus Protocol, the current liquidity read is watchlist first. The market cap is $17.61M, FDV is $18.57M, reported 24h volume is $1.94M, and FDV / market cap is 1.06x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Cetus Protocol gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.cetus.zone/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Cetus%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Cetus%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Cetus%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Cetus%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Cetus%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/CetusProtocol) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/cetuscommunity) | Identity, market, social, contract, or discovery cross-check |
| 9 | Discord | [link](https://discord.gg/cetusprotocol) | Identity, market, social, contract, or discovery cross-check |
| 10 | Cetus Protocol Price - CoinGecko | [link](https://www.coingecko.com/en/coins/cetus-protocol) | Identity, market, social, contract, or discovery cross-check |
| 11 | Cetus Statistics: Markets, Trading Volume & Trust Score / CoinGecko | [link](https://www.coingecko.com/en/exchanges/cetus) | Identity, market, social, contract, or discovery cross-check |
| 12 | What Is Cetus Protocol (CETUS) And How Does It Work? | [link](https://coinmarketcap.com/cmc-ai/cetus-protocol/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 13 | Cetus Protocol Price History and Historical Data / CoinMarketCap | [link](https://coinmarketcap.com/currencies/cetus-protocol/historical-data/) | Identity, market, social, contract, or discovery cross-check |
| 14 | Cetus Protocol (CETUS) / Tokenomics & Vesting Schedule | [link](https://tokenomist.ai/cetus-protocol) | Identity, market, social, contract, or discovery cross-check |
| 15 | Google project search | [link](https://www.google.com/search?q=Cetus%20Protocol%20CETUS%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google tokenomics search | [link](https://www.google.com/search?q=Cetus%20Protocol%20CETUS%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google docs search | [link](https://www.google.com/search?q=Cetus%20Protocol%20docs) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google governance search | [link](https://www.google.com/search?q=Cetus%20Protocol%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google audit search | [link](https://www.google.com/search?q=Cetus%20Protocol%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 20 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/cetus-protocol/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/cetus-protocol/) | Identity, market, social, contract, or discovery cross-check |
| 22 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Cetus%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune token query | [link](https://dune.com/browse/queries?q=Cetus%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 24 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Cetus%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=CETUS) | Identity, market, social, contract, or discovery cross-check |
| 26 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Cetus%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 27 | CryptoRank search | [link](https://cryptorank.io/price/cetus-protocol) | Identity, market, social, contract, or discovery cross-check |
| 28 | RootData search | [link](https://www.rootdata.com/search?kw=Cetus%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 29 | CoinDesk search | [link](https://www.coindesk.com/search?s=Cetus%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 30 | The Block search | [link](https://www.theblock.co/search?query=Cetus%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 31 | GitHub search | [link](https://github.com/search?q=Cetus%20Protocol&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 32 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=CETUS) | Identity, market, social, contract, or discovery cross-check |
| 33 | BscScan token search | [link](https://bscscan.com/search?f=0&q=CETUS) | Identity, market, social, contract, or discovery cross-check |
| 34 | Arbiscan token search | [link](https://arbiscan.io/search?f=0&q=CETUS) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.cetus.zone/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Cetus%20Protocol)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Cetus%20Protocol)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Cetus%20Protocol)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Cetus%20Protocol)
- [Messari discovery](https://messari.io/search?q=Cetus%20Protocol)
- [X profile](https://x.com/CetusProtocol)
- [Telegram](https://t.me/cetuscommunity)
- [Discord](https://discord.gg/cetusprotocol)
- [Cetus Protocol Price - CoinGecko](https://www.coingecko.com/en/coins/cetus-protocol)
- [Cetus Statistics: Markets, Trading Volume & Trust Score | CoinGecko](https://www.coingecko.com/en/exchanges/cetus)
- [What Is Cetus Protocol (CETUS) And How Does It Work?](https://coinmarketcap.com/cmc-ai/cetus-protocol/what-is/)
- [Cetus Protocol Price History and Historical Data | CoinMarketCap](https://coinmarketcap.com/currencies/cetus-protocol/historical-data/)
- [Cetus Protocol (CETUS) | Tokenomics & Vesting Schedule](https://tokenomist.ai/cetus-protocol)
- [Google project search](https://www.google.com/search?q=Cetus%20Protocol%20CETUS%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Cetus%20Protocol%20CETUS%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Cetus%20Protocol%20docs)
- [Google governance search](https://www.google.com/search?q=Cetus%20Protocol%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Cetus%20Protocol%20audit%20report)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/cetus-protocol/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/cetus-protocol/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Cetus%20Protocol)
- [Dune token query](https://dune.com/browse/queries?q=Cetus%20Protocol)
- [DeFiLlama fees discovery](https://defillama.com/fees?chain=All&search=Cetus%20Protocol)

## Final Investment View

Cetus Protocol should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
