---
title: 'Clearpool / CPOOL: stablecoin / credit infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Clearpool is treated as a stablecoin / credit infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'clearpool-cpool-stablecoin-credit-infrastructure-token-value-capture-risk'
category: 'research'
---

# Clearpool / CPOOL: stablecoin / credit infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Clearpool appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Ethereum, Solana; exchange exposure is UPBIT, BITHUMB, BYBIT, BINANCE ALPHA, BITGET; category tags are Lending & Borrowing, RWA (Real World Assets), Stablecoins; current market cap is $19.29M; FDV is $19.29M; and 24h volume is $738.48K. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Clearpool before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 840, market cap $19,480,351, 24h volume $337,908. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Clearpool is best understood as a **stablecoin / credit infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether CPOOL captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
Base case: Clearpool remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning CPOOL into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Clearpool is a decentralized credit marketplace that enables institutions to raise unsecured liquidity from DeFi and lets liquidity providers earn yield via tokenized credit pools (cpTokens). The CPOOL token powers governance, staking and buyback-funded rewards; Clearpool also runs a permissioned Prime product, PayFi/RWA vaults and is expanding RWA yield via the Ozean L2.

Surf's project profile maps Clearpool to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://clearpool.finance](https://clearpool.finance) |
| X handle | [@ClearpoolFin](https://x.com/ClearpoolFin) |
| X followers | 90.42K |
| Tags | Lending & Borrowing, RWA (Real World Assets), Stablecoins |
| Chains | Ethereum, Solana |
| Exchanges | UPBIT, BITHUMB, BYBIT, BINANCE ALPHA, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Clearpool is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For stablecoin / credit infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Clearpool convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $738.48K 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
...
The strongest reason to add the report is not that Clearpool is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| solana | CPOOL | AeXrLftu8chuY4ctc6oDeG4dUx6Yr4aqeakUMFNvACdg | [explorer](https://solscan.io/token/AeXrLftu8chuY4ctc6oDeG4dUx6Yr4aqeakUMFNvACdg) |
| ethereum | CPOOL | 0x66761fa41377003622aee3c7675fc7b5c1c2fac5 | [explorer](https://etherscan.io/token/0x66761fa41377003622aee3c7675fc7b5c1c2fac5) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Clearpool, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 840, market cap $19,480,351, 24h volume $337,908. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $19.29M, FDV $19.29M, 24h volume $738.48K, and price $0.0193.
...
| 24h volume | $738.48K | Position sizing and exit liquidity proxy |
| Market cap | $19.29M | Liquid valuation proxy |
| FDV | $19.29M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.00x | Higher spread means more supply risk |
| X followers | 90.42K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Clearpool | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether CPOOL is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Clearpool currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 1.88% and 30d -12.52% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Clearpool's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-03-19 | Incubation | not disclosed | Plug and Play, XDC Foundation |
| 2024-02-06 | Grant | $150.00K | Mantle |
| 2021-10-28 | Ido | $40.00K | not disclosed |
| 2021-10-27 | Ieo | $30.00K | not disclosed |
| 2021-10-26 | Ido | $100.00K | not disclosed |
| 2021-09-27 | Seed | $3.00M | Kenetic Capital, HTX Ventures, GBV Capital, Sequoia Capital, Arrington Capital, HashKey Capital, Ryze Labs (Sino Global Capital), FBG Capital |

| Team member | Role | Link |
|---|---|---|
| Robert Alcorn | Co-Founder, Chairman | [X](https://twitter.com/Rob8830) |
| Jakob Kronbichler | Co-Founder, CEO | [X](https://twitter.com/JKronbichler) |
| Alessio Quaglini | Co-Founder, Advisor | [X](https://twitter.com/onchainpanini) |
| Varinder Singh | COO | [X](https://twitter.com/Va_rinder) |
| Vadim Zolotokrylin | Chief of Product, Chief of Technology | [X](https://twitter.com/zolotokrylin) |
| Dan Kaizer | Blockchain Advisor | not disclosed |
| Jennifer Ng | Advisor | not disclosed |
| Drew Turchin | Advisor | not disclosed |
...
Clearpool competes in the stablecoin / credit infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Clearpool has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Clearpool is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Clearpool compounds usage, liquidity improves, and CPOOL gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If CPOOL does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $738.48K | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Clearpool announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Clearpool has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Clearpool, the first reconciliation job is identity. The token symbol is CPOOL, the category is stablecoin / credit infrastructure, chains are Ethereum, Solana, exchanges are UPBIT, BITHUMB, BYBIT, BINANCE ALPHA, BITGET, and tags are Lending & Borrowing, RWA (Real World Assets), Stablecoins. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If CPOOL trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $19.29M, FDV is $19.29M, 24h volume is $738.48K, FDV / market cap is 1.00x, 7d price change is 1.88%, and 30d price change is -12.52%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Clearpool / CPOOL | Match official docs, CG, CMC, Surf, contracts |
...
For Clearpool, the current liquidity read is watchlist first. The market cap is $19.29M, FDV is $19.29M, reported 24h volume is $738.48K, and FDV / market cap is 1.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. Clearpool can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.

For Clearpool, the live monitoring stack should include contract events, TVL, volume, governance, documentation changes, social announcements, security disclosures, and exchange-market quality. A single metric is not enough. TVL may stay high while liquidity thins. Volume may rise because of volatility rather than adoption. Social attention may rise around controversy rather than progress. Governance may pass proposals that improve short-term price but add long-term risk. The strongest monitoring dashboard combines on-chain state, off-chain disclosure, and market quality.

Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If Clearpool suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | Clearpool gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://clearpool.finance) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Clearpool) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Clearpool) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Clearpool) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Clearpool) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Clearpool) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/ClearpoolFin) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/clearpool-finance) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/clearpoolofficial) | Identity, market, social, contract, or discovery cross-check |
| 10 | Clearpool Price Chart (CPOOL) - CoinGecko | [link](https://www.coingecko.com/en/coins/clearpool) | Identity, market, social, contract, or discovery cross-check |
| 11 | What Is Clearpool (CPOOL) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/clearpool/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 12 | CPOOL to ETH: Clearpool Price in Ether - CoinGecko | [link](https://www.coingecko.com/en/coins/clearpool/eth) | Identity, market, social, contract, or discovery cross-check |
| 13 | Clearpool Price History: Download CPOOL Historical Data | [link](https://www.coingecko.com/en/coins/clearpool/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 14 | Clearpool (CPOOL) / Tokenomics & Vesting Schedule - Token Unlocks | [link](https://tokenomist.ai/clearpool) | Identity, market, social, contract, or discovery cross-check |
| 15 | solana CPOOL | [link](https://solscan.io/token/AeXrLftu8chuY4ctc6oDeG4dUx6Yr4aqeakUMFNvACdg) | Identity, market, social, contract, or discovery cross-check |
| 16 | ethereum CPOOL | [link](https://etherscan.io/token/0x66761fa41377003622aee3c7675fc7b5c1c2fac5) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google project search | [link](https://www.google.com/search?q=Clearpool%20CPOOL%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google tokenomics search | [link](https://www.google.com/search?q=Clearpool%20CPOOL%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google docs search | [link](https://www.google.com/search?q=Clearpool%20docs) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google governance search | [link](https://www.google.com/search?q=Clearpool%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google audit search | [link](https://www.google.com/search?q=Clearpool%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/clearpool/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/clearpool/) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Clearpool) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune token query | [link](https://dune.com/browse/queries?q=Clearpool) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Clearpool) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=CPOOL) | Identity, market, social, contract, or discovery cross-check |
| 28 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Clearpool) | Identity, market, social, contract, or discovery cross-check |
| 29 | CryptoRank search | [link](https://cryptorank.io/price/clearpool) | Identity, market, social, contract, or discovery cross-check |
| 30 | RootData search | [link](https://www.rootdata.com/search?kw=Clearpool) | Identity, market, social, contract, or discovery cross-check |
| 31 | CoinDesk search | [link](https://www.coindesk.com/search?s=Clearpool) | Identity, market, social, contract, or discovery cross-check |
| 32 | The Block search | [link](https://www.theblock.co/search?query=Clearpool) | Identity, market, social, contract, or discovery cross-check |
| 33 | GitHub search | [link](https://github.com/search?q=Clearpool&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 34 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=CPOOL) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://clearpool.finance)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Clearpool)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Clearpool)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Clearpool)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Clearpool)
- [Messari discovery](https://messari.io/search?q=Clearpool)
- [X profile](https://x.com/ClearpoolFin)
- [GitHub](https://github.com/clearpool-finance)
- [Telegram](https://t.me/clearpoolofficial)
- [Clearpool Price Chart (CPOOL) - CoinGecko](https://www.coingecko.com/en/coins/clearpool)
- [What Is Clearpool (CPOOL) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/clearpool/what-is/)
- [CPOOL to ETH: Clearpool Price in Ether - CoinGecko](https://www.coingecko.com/en/coins/clearpool/eth)
- [Clearpool Price History: Download CPOOL Historical Data](https://www.coingecko.com/en/coins/clearpool/historical_data)
- [Clearpool (CPOOL) | Tokenomics & Vesting Schedule - Token Unlocks](https://tokenomist.ai/clearpool)
- [solana CPOOL](https://solscan.io/token/AeXrLftu8chuY4ctc6oDeG4dUx6Yr4aqeakUMFNvACdg)
- [ethereum CPOOL](https://etherscan.io/token/0x66761fa41377003622aee3c7675fc7b5c1c2fac5)
- [Google project search](https://www.google.com/search?q=Clearpool%20CPOOL%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Clearpool%20CPOOL%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Clearpool%20docs)
- [Google governance search](https://www.google.com/search?q=Clearpool%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Clearpool%20audit%20report)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/clearpool/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/clearpool/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Clearpool)

## Final Investment View

Clearpool should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
