---
title: 'io.net / IO: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'io.net is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'io-net-io-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# io.net / IO: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** io.net appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Solana; exchange exposure is BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET; category tags are DePIN (Physical Infra), Web3 AI (Agent/AIGC); current market cap is $63.29M; FDV is $139.56M; and 24h volume is $11.07M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag io.net before this file was created, and the existing candidate note says: Seeded 2026-06-28 from Surf market-ranking; rank 373, market cap $61,559,121, 24h volume $12,901,336. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

io.net is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether IO captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| FDV | $139.56M |
| 24h volume | $11.07M |
| Circulating supply | 362.45M |
| Total supply | 799.21M |
...
Base case: io.net remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning IO into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

io.net is a decentralized GPU infrastructure network (DePIN) that provides scalable, low-cost GPU clusters for AI/ML training and inference. The platform uses a native IO token for on-chain payments and compensation across its distributed compute network.

Surf's project profile maps io.net to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://io.net/](https://io.net/) |
| X handle | [@ionet](https://x.com/ionet) |
| X followers | 454.87K |
...
| Liquidity | $11.07M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 362.45M circulating vs 799.21M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that io.net is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| solana | IO | BZLbGTNCSFfoth2GYDtwr7e4imWzpR5jqcUuGEwr646K | [explorer](https://solscan.io/token/BZLbGTNCSFfoth2GYDtwr7e4imWzpR5jqcUuGEwr646K) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For io.net, the DeFi-oriented backlog note is the first signal: Seeded 2026-06-28 from Surf market-ranking; rank 373, market cap $61,559,121, 24h volume $12,901,336. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $63.29M, FDV $139.56M, 24h volume $11.07M, and price $0.1746.
...
| 24h volume | $11.07M | Position sizing and exit liquidity proxy |
| Market cap | $63.29M | Liquid valuation proxy |
| FDV | $139.56M | Dilution-aware valuation proxy |
| FDV / market cap spread | 2.21x | Higher spread means more supply risk |
| X followers | 454.87K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for io.net | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether IO is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. io.net currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 1.63% and 30d 25.77% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that io.net's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2024-03-05 | Series A | $30.00M | Avery Ching, Animoca Brands, OKX Ventures, Multicoin Capital, Delphi Ventures, Solana Ventures, LongHash Ventures, SevenX Ventures |

| Team member | Role | Link |
|---|---|---|
| Ahmad Shadid | Founder & CEO | [X](https://twitter.com/shadid_io) |
| Basem Oubah | Co-Founder, COO | [X](https://twitter.com/Basem_IO) |
| Saad Mohammed Alenezi | Co-Founder, Chief Customer Officer | [X](https://x.com/xxivk_io) |
| Tory Green | Co-Founder, CEO | [X](https://twitter.com/MTorygreen) |
| Gaurav Sharma | CEO, CTO | [X](https://x.com/gaurav_ionet) |
| Mark Roszak | CLO, Chief Compliance Officer | not disclosed |
| Maher Jilani | Chief Experience Officer | [X](https://twitter.com/wagieeacc) |
| Lord Ed Vaizey | Advisor | not disclosed |
...
io.net competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether io.net has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: io.net is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | io.net compounds usage, liquidity improves, and IO gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because io.net has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If IO does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $11.07M | Weekly | Volume below position-size threshold |
| FDV / market cap | 2.21x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if io.net announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. io.net has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For io.net, the first reconciliation job is identity. The token symbol is IO, the category is DeFi infrastructure, chains are Solana, exchanges are BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET, and tags are DePIN (Physical Infra), Web3 AI (Agent/AIGC). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If IO trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $63.29M, FDV is $139.56M, 24h volume is $11.07M, FDV / market cap is 2.21x, 7d price change is 1.63%, and 30d price change is 25.77%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | io.net / IO | Match official docs, CG, CMC, Surf, contracts |
...
For io.net, the current liquidity read is watchlist first. The market cap is $63.29M, FDV is $139.56M, reported 24h volume is $11.07M, and FDV / market cap is 2.21x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | io.net gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://io.net/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=io.net) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=io.net) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=io.net) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=io.net) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=io.net) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/ionet) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/io_net) | Identity, market, social, contract, or discovery cross-check |
| 9 | io.net Price: IO/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/io-net) | Identity, market, social, contract, or discovery cross-check |
| 10 | Company Origins - io.net | [link](https://io.net/docs/guides/inception) | Identity, market, social, contract, or discovery cross-check |
| 11 | Overview - io.net | [link](https://io.net/docs/guides/coin/io-coin) | Identity, market, social, contract, or discovery cross-check |
| 12 | io.net (IO) Price Prediction For 2026 & Beyond - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/io-net/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 13 | IO to INR: io.net Price in Indian Rupee - CoinGecko | [link](https://www.coingecko.com/en/coins/io-net/inr) | Identity, market, social, contract, or discovery cross-check |
| 14 | solana IO | [link](https://solscan.io/token/BZLbGTNCSFfoth2GYDtwr7e4imWzpR5jqcUuGEwr646K) | Identity, market, social, contract, or discovery cross-check |
| 15 | Google project search | [link](https://www.google.com/search?q=io.net%20IO%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google tokenomics search | [link](https://www.google.com/search?q=io.net%20IO%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google docs search | [link](https://www.google.com/search?q=io.net%20docs) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google governance search | [link](https://www.google.com/search?q=io.net%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google audit search | [link](https://www.google.com/search?q=io.net%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 20 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/io-net/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/io-net/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/io-net/) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune discovery | [link](https://dune.com/browse/dashboards?q=io.net) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune token query | [link](https://dune.com/browse/queries?q=io.net) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=io.net) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=IO) | Identity, market, social, contract, or discovery cross-check |
| 27 | TokenUnlocks search | [link](https://token.unlocks.app/?search=io.net) | Identity, market, social, contract, or discovery cross-check |
| 28 | CryptoRank search | [link](https://cryptorank.io/price/io-net) | Identity, market, social, contract, or discovery cross-check |
| 29 | RootData search | [link](https://www.rootdata.com/search?kw=io.net) | Identity, market, social, contract, or discovery cross-check |
| 30 | CoinDesk search | [link](https://www.coindesk.com/search?s=io.net) | Identity, market, social, contract, or discovery cross-check |
| 31 | The Block search | [link](https://www.theblock.co/search?query=io.net) | Identity, market, social, contract, or discovery cross-check |
| 32 | GitHub search | [link](https://github.com/search?q=io.net&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 33 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=IO) | Identity, market, social, contract, or discovery cross-check |
| 34 | BscScan token search | [link](https://bscscan.com/search?f=0&q=IO) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://io.net/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=io.net)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=io.net)
- [DefiLlama protocol discovery](https://defillama.com/search?query=io.net)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=io.net)
- [Messari discovery](https://messari.io/search?q=io.net)
- [X profile](https://x.com/ionet)
- [Telegram](https://t.me/io_net)
- [io.net Price: IO/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/io-net)
- [Company Origins - io.net](https://io.net/docs/guides/inception)
- [Overview - io.net](https://io.net/docs/guides/coin/io-coin)
- [io.net (IO) Price Prediction For 2026 & Beyond - CoinMarketCap](https://coinmarketcap.com/cmc-ai/io-net/price-prediction/)
- [IO to INR: io.net Price in Indian Rupee - CoinGecko](https://www.coingecko.com/en/coins/io-net/inr)
- [solana IO](https://solscan.io/token/BZLbGTNCSFfoth2GYDtwr7e4imWzpR5jqcUuGEwr646K)
- [Google project search](https://www.google.com/search?q=io.net%20IO%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=io.net%20IO%20tokenomics)
- [Google docs search](https://www.google.com/search?q=io.net%20docs)
- [Google governance search](https://www.google.com/search?q=io.net%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=io.net%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/io-net/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/io-net/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/io-net/)
- [Dune discovery](https://dune.com/browse/dashboards?q=io.net)
- [Dune token query](https://dune.com/browse/queries?q=io.net)

## Final Investment View

io.net should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
