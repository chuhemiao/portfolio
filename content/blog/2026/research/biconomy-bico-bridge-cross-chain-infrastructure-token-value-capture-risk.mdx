---
title: 'Biconomy / BICO: bridge / cross-chain infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Biconomy is treated as a bridge / cross-chain infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'biconomy-bico-bridge-cross-chain-infrastructure-token-value-capture-risk'
category: 'research'
---

# Biconomy / BICO: bridge / cross-chain infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Biconomy appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Arbitrum One, Ethereum; exchange exposure is BINANCE, COINBASE, OKX, BITHUMB, BYBIT, BITGET; category tags are Cross-Chain & Bridge, Developer Tooling, Wallets & AA (Account Abstraction); current market cap is $11.01M; FDV is $15.34M; and 24h volume is $7.92M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Biconomy before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 1102, market cap $11,389,021, 24h volume $8,134,119. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Biconomy is best understood as a **bridge / cross-chain infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether BICO captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Total supply | 1.00B |
| 7d price change | -18.70% |
| 30d price change | -15.93% |
...
Base case: Biconomy remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning BICO into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Biconomy is a multi-chain transaction infrastructure that provides APIs and SDKs to enable gasless meta-transactions, account abstraction, and seamless cross-chain transaction execution for Web3 dApps. It offers relayer services, a transaction-orchestration stack (MEE), and developer tools to simplify UX across EVM chains and rollups.

Surf's project profile maps Biconomy to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.biconomy.io](https://www.biconomy.io) |
| X handle | [@biconomy](https://x.com/biconomy) |
| X followers | 119.50K |
| Tags | Cross-Chain & Bridge, Developer Tooling, Wallets & AA (Account Abstraction) |
| Chains | Arbitrum One, Ethereum |
| Exchanges | BINANCE, COINBASE, OKX, BITHUMB, BYBIT, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Biconomy is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For bridge / cross-chain infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Biconomy convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $7.92M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 718.05M circulating vs 1.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Biconomy is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Arbitrum One, Ethereum |
...
|---|---|---|---|
| ethereum | BICO | 0xf17e65822b568b3903685a7c9f496cf7656cc6c2 | [explorer](https://etherscan.io/token/0xf17e65822b568b3903685a7c9f496cf7656cc6c2) |
| arbitrum | BICO | 0xa68ec98d7ca870cf1dd0b00ebbb7c4bf60a8e74d | [explorer](https://arbiscan.io/token/0xa68ec98d7ca870cf1dd0b00ebbb7c4bf60a8e74d) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Biconomy, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 1102, market cap $11,389,021, 24h volume $8,134,119. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $11.01M, FDV $15.34M, 24h volume $7.92M, and price $0.0153.
...
| 24h volume | $7.92M | Position sizing and exit liquidity proxy |
| Market cap | $11.01M | Liquid valuation proxy |
| FDV | $15.34M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.39x | Higher spread means more supply risk |
| X followers | 119.50K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Biconomy | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether BICO is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Biconomy currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d -18.70% and 30d -15.93% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Biconomy's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2024-03-28 | Strategic | not disclosed | Jump Capital, Borderless Capital, ConsenSys, Balaji Srinivasan, Manifold, Side Door Ventures, Blockchain Founders Fund, Taisu Ventures |
| 2021-12-01 | Ieo | $600.00K | not disclosed |
| 2021-10-14 | Ico | $10.00M | not disclosed |
| 2021-10-14 | Ico | $1.50M | not disclosed |
| 2021-09-01 | Strategic | $625.00K | not disclosed |
| 2021-07-28 | Private Token Sale | $9.00M | Coinbase Ventures, HTX Ventures, Mechanism Capital, LedgerPrime, CoinFund, Genblock Capital, Rarestone Capital, True Ventures |

| Team member | Role | Link |
|---|---|---|
| Ahmed Al-Balaghi | Co-Founder, CEO | [X](https://twitter.com/AraBalaghi) |
| Sachin Tomar | Co-Founder, CTO | [X](https://twitter.com/schin_tomar) |
| Aniket Jindal | Co-Founder, COO | [X](https://twitter.com/aniket_jindal08) |
| Purva Gera | Chief People Officer | [X](https://twitter.com/PurvaGera) |
| Sandeep Nailwal | Advisor | [X](https://twitter.com/sandeepnailwal) |
...
Biconomy competes in the bridge / cross-chain infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Biconomy has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Biconomy is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Biconomy compounds usage, liquidity improves, and BICO gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If BICO does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $7.92M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Biconomy announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Biconomy has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Biconomy, the first reconciliation job is identity. The token symbol is BICO, the category is bridge / cross-chain infrastructure, chains are Arbitrum One, Ethereum, exchanges are BINANCE, COINBASE, OKX, BITHUMB, BYBIT, BITGET, and tags are Cross-Chain & Bridge, Developer Tooling, Wallets & AA (Account Abstraction). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If BICO trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $11.01M, FDV is $15.34M, 24h volume is $7.92M, FDV / market cap is 1.39x, 7d price change is -18.70%, and 30d price change is -15.93%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
| Peer comparison | Monthly | Biconomy gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.biconomy.io) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Biconomy) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Biconomy) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Biconomy) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Biconomy) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Biconomy) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/biconomy) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/biconomy) | Identity, market, social, contract, or discovery cross-check |
| 9 | Biconomy Price Chart (BICO) - CoinGecko | [link](https://www.coingecko.com/en/coins/biconomy) | Identity, market, social, contract, or discovery cross-check |
| 10 | What Is Biconomy (BICO)? Features, Tokenomics, and Price Prediction | [link](https://coinmarketcap.com/academy/article/what-is-biconomy-bico-features-tokenomics-and-price-prediction) | Identity, market, social, contract, or discovery cross-check |
| 11 | Biconomy (BICO) Price Prediction For 2026 & Beyond | [link](https://coinmarketcap.com/cmc-ai/biconomy/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 12 | Biconomy Exchange Token Price Chart (BIT) - CoinGecko | [link](https://www.coingecko.com/en/coins/biconomy-exchange-token) | Identity, market, social, contract, or discovery cross-check |
| 13 | Biconomy price today, BICO to USD live price, marketcap and chart | [link](https://coinmarketcap.com/currencies/biconomy/) | Identity, market, social, contract, or discovery cross-check |
| 14 | ethereum BICO | [link](https://etherscan.io/token/0xf17e65822b568b3903685a7c9f496cf7656cc6c2) | Identity, market, social, contract, or discovery cross-check |
| 15 | arbitrum BICO | [link](https://arbiscan.io/token/0xa68ec98d7ca870cf1dd0b00ebbb7c4bf60a8e74d) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google project search | [link](https://www.google.com/search?q=Biconomy%20BICO%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google tokenomics search | [link](https://www.google.com/search?q=Biconomy%20BICO%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google docs search | [link](https://www.google.com/search?q=Biconomy%20docs) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google governance search | [link](https://www.google.com/search?q=Biconomy%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google audit search | [link](https://www.google.com/search?q=Biconomy%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/biconomy/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/biconomy/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Biconomy) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune token query | [link](https://dune.com/browse/queries?q=Biconomy) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Biconomy) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=BICO) | Identity, market, social, contract, or discovery cross-check |
| 27 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Biconomy) | Identity, market, social, contract, or discovery cross-check |
| 28 | CryptoRank search | [link](https://cryptorank.io/price/biconomy) | Identity, market, social, contract, or discovery cross-check |
| 29 | RootData search | [link](https://www.rootdata.com/search?kw=Biconomy) | Identity, market, social, contract, or discovery cross-check |
| 30 | CoinDesk search | [link](https://www.coindesk.com/search?s=Biconomy) | Identity, market, social, contract, or discovery cross-check |
| 31 | The Block search | [link](https://www.theblock.co/search?query=Biconomy) | Identity, market, social, contract, or discovery cross-check |
| 32 | GitHub search | [link](https://github.com/search?q=Biconomy&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 33 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=BICO) | Identity, market, social, contract, or discovery cross-check |
| 34 | BscScan token search | [link](https://bscscan.com/search?f=0&q=BICO) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.biconomy.io)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Biconomy)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Biconomy)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Biconomy)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Biconomy)
- [Messari discovery](https://messari.io/search?q=Biconomy)
- [X profile](https://x.com/biconomy)
- [Telegram](https://t.me/biconomy)
- [Biconomy Price Chart (BICO) - CoinGecko](https://www.coingecko.com/en/coins/biconomy)
- [What Is Biconomy (BICO)? Features, Tokenomics, and Price Prediction](https://coinmarketcap.com/academy/article/what-is-biconomy-bico-features-tokenomics-and-price-prediction)
- [Biconomy (BICO) Price Prediction For 2026 & Beyond](https://coinmarketcap.com/cmc-ai/biconomy/price-prediction/)
- [Biconomy Exchange Token Price Chart (BIT) - CoinGecko](https://www.coingecko.com/en/coins/biconomy-exchange-token)
- [Biconomy price today, BICO to USD live price, marketcap and chart](https://coinmarketcap.com/currencies/biconomy/)
- [ethereum BICO](https://etherscan.io/token/0xf17e65822b568b3903685a7c9f496cf7656cc6c2)
- [arbitrum BICO](https://arbiscan.io/token/0xa68ec98d7ca870cf1dd0b00ebbb7c4bf60a8e74d)
- [Google project search](https://www.google.com/search?q=Biconomy%20BICO%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Biconomy%20BICO%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Biconomy%20docs)
- [Google governance search](https://www.google.com/search?q=Biconomy%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Biconomy%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/biconomy/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/biconomy/price-analysis/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Biconomy)
- [Dune token query](https://dune.com/browse/queries?q=Biconomy)

## Final Investment View

Biconomy should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
