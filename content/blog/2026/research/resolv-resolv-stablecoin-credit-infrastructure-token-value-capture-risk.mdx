---
title: 'Resolv / RESOLV: stablecoin / credit infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Resolv is treated as a stablecoin / credit infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'resolv-resolv-stablecoin-credit-infrastructure-token-value-capture-risk'
category: 'research'
---

# Resolv / RESOLV: stablecoin / credit infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Resolv appears in the local CoinGecko/CMC-oriented candidate pool through [CoinGecko](https://www.coingecko.com/en/coins/resolv) and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Arbitrum One, Base, Berachain, Binance Smart Chain, Ethereum, HyperEVM, Plasma, Soneium, Tac; exchange exposure is BINANCE, UPBIT, OKX, BITHUMB, BYBIT, BITGET, HYPERLIQUID; category tags are Insurance, Stablecoins, Yield & Asset Mgmt; current market cap is $9.67M; FDV is $22.30M; and 24h volume is $20.69M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Resolv before this file was created, and the existing candidate note says: DefiLlama category Basis Trading on Ethereum; TVL $13,213,695. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Resolv is best understood as a **stablecoin / credit infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether RESOLV captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Arbitrum One, Base, Berachain, Binance Smart Chain, Ethereum, HyperEVM, Plasma, Soneium, Tac |
| Exchanges | BINANCE, UPBIT, OKX, BITHUMB, BYBIT, BITGET, HYPERLIQUID |
| Price | $0.0222 |
| Market cap | $9.67M |
| FDV | $22.30M |
| 24h volume | $20.69M |
| Circulating supply | 433.69M |
| Total supply | 1.00B |
| 7d price change | 12.81% |
| 30d price change | 35.81% |
...
Base case: Resolv remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning RESOLV into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Resolv USR (USR) is a crypto-native, overcollateralized stablecoin pegged to the US dollar and natively backed by ETH and BTC using a delta-neutral hedging architecture. The protocol isolates strategy risk into a tokenized protection layer (RLP), supports mint/redemptions, and offers yield-bearing variants such as stUSR via Resolv Labs.

Surf's project profile maps Resolv to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://resolv.xyz/](https://resolv.xyz/) |
| X handle | [@ResolvLabs](https://x.com/ResolvLabs) |
| X followers | 41.62K |
| Tags | Insurance, Stablecoins, Yield & Asset Mgmt |
| Chains | Arbitrum One, Base, Berachain, Binance Smart Chain, Ethereum, HyperEVM, Plasma, Soneium, Tac |
| Exchanges | BINANCE, UPBIT, OKX, BITHUMB, BYBIT, BITGET, HYPERLIQUID |
| TGE status | post |
| CoinGecko ID | [resolv](https://www.coingecko.com/en/coins/resolv) |

The main product question is whether Resolv is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For stablecoin / credit infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Resolv convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $20.69M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 433.69M circulating vs 1.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Resolv is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Arbitrum One, Base, Berachain, Binance Smart Chain, Ethereum, HyperEVM, Plasma, Soneium, Tac |
...
|---|---|---|---|
| ethereum | RESOLV | 0x259338656198ec7a76c729514d3cb45dfbf768a1 | [explorer](https://etherscan.io/token/0x259338656198ec7a76c729514d3cb45dfbf768a1) |
| bsc | RESOLV | 0xda6cef7f667d992a60eb823ab215493aa0c6b360 | [explorer](https://bscscan.com/token/0xda6cef7f667d992a60eb823ab215493aa0c6b360) |
| Tac | RLP | 0x35533f54740f1f1aa4179e57ba37039dfa16868b | not mapped |
| base | RLP | 0xc31389794ffac23331e0d9f611b7953f90aa5fdc | [explorer](https://basescan.org/token/0xc31389794ffac23331e0d9f611b7953f90aa5fdc) |
| Plasma | RLP | 0x35533f54740f1f1aa4179e57ba37039dfa16868b | not mapped |
| Soneium | RLP | 0x35533f54740f1f1aa4179e57ba37039dfa16868b | not mapped |
| ethereum | RLP | 0x4956b52ae2ff65d74ca2d61207523288e4528f96 | [explorer](https://etherscan.io/token/0x4956b52ae2ff65d74ca2d61207523288e4528f96) |
| HyperEVM | RLP | 0x0a3d8466f5de586fa5f6de117301e2f90bcc5c48 | not mapped |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Resolv, the DeFi-oriented backlog note is the first signal: DefiLlama category Basis Trading on Ethereum; TVL $13,213,695. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $9.67M, FDV $22.30M, 24h volume $20.69M, and price $0.0222.
...
| 24h volume | $20.69M | Position sizing and exit liquidity proxy |
| Market cap | $9.67M | Liquid valuation proxy |
| FDV | $22.30M | Dilution-aware valuation proxy |
| FDV / market cap spread | 2.31x | Higher spread means more supply risk |
| X followers | 41.62K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Resolv | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether RESOLV is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Resolv currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 12.81% and 30d 35.81% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Resolv's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-04-16 | Seed | $10.00M | Coinbase Ventures, Arrington Capital, Maven 11 Capital, Robot Ventures, gumi Cryptos, No Limit Holdings, cyber Fund, Flowdesk |

| Team member | Role | Link |
|---|---|---|
| Fedor Chmilevfa | Co-Founder | [X](https://x.com/chmilevfa) |
| Tim Shekikhachev | Co-Founder | [X](https://x.com/timsheki) |
| Ivan Kozlov | Co-Founder | [X](https://x.com/Iv4n_Ko) |
...
Resolv competes in the stablecoin / credit infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Resolv has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Resolv is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Resolv compounds usage, liquidity improves, and RESOLV gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Resolv has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If RESOLV does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $20.69M | Weekly | Volume below position-size threshold |
| FDV / market cap | 2.31x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Resolv announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Resolv has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Resolv, the first reconciliation job is identity. The token symbol is RESOLV, the category is stablecoin / credit infrastructure, chains are Arbitrum One, Base, Berachain, Binance Smart Chain, Ethereum, HyperEVM, Plasma, Soneium, Tac, exchanges are BINANCE, UPBIT, OKX, BITHUMB, BYBIT, BITGET, HYPERLIQUID, and tags are Insurance, Stablecoins, Yield & Asset Mgmt. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If RESOLV trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $9.67M, FDV is $22.30M, 24h volume is $20.69M, FDV / market cap is 2.31x, 7d price change is 12.81%, and 30d price change is 35.81%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Resolv / RESOLV | Match official docs, CG, CMC, Surf, contracts |
| Category | stablecoin / credit infrastructure | Confirm product mechanics and revenue model |
| Chains | Arbitrum One, Base, Berachain, Binance Smart Chain, Ethereum, HyperEVM, Plasma, Soneium, Tac | Confirm canonical deployments and bridge assumptions |
| Exchanges | BINANCE, UPBIT, OKX, BITHUMB, BYBIT, BITGET, HYPERLIQUID | Check live order books and inactive pairs |
| Market cap | $9.67M | Compare Surf, CG, CMC, exchange pages |
| FDV | $22.30M | Reconcile total supply and unlock schedule |
| Volume | $20.69M | Check exchange concentration and wash-trading risk |
| TVL | not disclosed | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Resolv does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Resolv, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does RESOLV sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
For Resolv, the current liquidity read is watchlist first. The market cap is $9.67M, FDV is $22.30M, reported 24h volume is $20.69M, and FDV / market cap is 2.31x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Resolv gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://resolv.xyz/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko profile | [link](https://www.coingecko.com/en/coins/resolv) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Resolv) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Resolv) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Resolv) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Resolv) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/ResolvLabs) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/Resolv_Community) | Identity, market, social, contract, or discovery cross-check |
| 9 | Discord | [link](https://discord.com/invite/resolvcommunity) | Identity, market, social, contract, or discovery cross-check |
| 10 | Resolv USR Price: USR/USD Live Price Chart, Market ... - CoinGecko | [link](https://www.coingecko.com/en/coins/resolv-usr) | Identity, market, social, contract, or discovery cross-check |
| 11 | Resolv price today, RESOLV to USD live price, marketcap and chart | [link](https://coinmarketcap.com/currencies/resolv/) | Identity, market, social, contract, or discovery cross-check |
| 12 | What Is Resolv (RESOLV) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/resolv/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 13 | Resolv Liquidity Provider Token Price Chart (RLP) - CoinGecko | [link](https://www.coingecko.com/en/coins/resolv-liquidity-provider-token) | Identity, market, social, contract, or discovery cross-check |
| 14 | ethereum RESOLV | [link](https://etherscan.io/token/0x259338656198ec7a76c729514d3cb45dfbf768a1) | Identity, market, social, contract, or discovery cross-check |
| 15 | bsc RESOLV | [link](https://bscscan.com/token/0xda6cef7f667d992a60eb823ab215493aa0c6b360) | Identity, market, social, contract, or discovery cross-check |
| 16 | base RLP | [link](https://basescan.org/token/0xc31389794ffac23331e0d9f611b7953f90aa5fdc) | Identity, market, social, contract, or discovery cross-check |
| 17 | ethereum RLP | [link](https://etherscan.io/token/0x4956b52ae2ff65d74ca2d61207523288e4528f96) | Identity, market, social, contract, or discovery cross-check |
| 18 | arbitrum RLP | [link](https://arbiscan.io/token/0x35e5db674d8e93a03d814fa0ada70731efe8a4b9) | Identity, market, social, contract, or discovery cross-check |
| 19 | bsc RLP | [link](https://bscscan.com/token/0x35e5db674d8e93a03d814fa0ada70731efe8a4b9) | Identity, market, social, contract, or discovery cross-check |
| 20 | arbitrum WSTUSR | [link](https://arbiscan.io/token/0x66cfbd79257dc5217903a36293120282548e2254) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google project search | [link](https://www.google.com/search?q=Resolv%20RESOLV%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google tokenomics search | [link](https://www.google.com/search?q=Resolv%20RESOLV%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 23 | Google docs search | [link](https://www.google.com/search?q=Resolv%20docs) | Identity, market, social, contract, or discovery cross-check |
| 24 | Google governance search | [link](https://www.google.com/search?q=Resolv%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 25 | Google audit search | [link](https://www.google.com/search?q=Resolv%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 26 | CoinGecko historical data | [link](https://www.coingecko.com/en/coins/resolv/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 27 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/resolv/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 28 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Resolv) | Identity, market, social, contract, or discovery cross-check |
| 29 | Dune token query | [link](https://dune.com/browse/queries?q=Resolv) | Identity, market, social, contract, or discovery cross-check |
| 30 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Resolv) | Identity, market, social, contract, or discovery cross-check |
| 31 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=RESOLV) | Identity, market, social, contract, or discovery cross-check |
| 32 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Resolv) | Identity, market, social, contract, or discovery cross-check |
| 33 | CryptoRank search | [link](https://cryptorank.io/price/resolv) | Identity, market, social, contract, or discovery cross-check |
| 34 | RootData search | [link](https://www.rootdata.com/search?kw=Resolv) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://resolv.xyz/)
- [CoinGecko profile](https://www.coingecko.com/en/coins/resolv)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Resolv)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Resolv)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Resolv)
- [Messari discovery](https://messari.io/search?q=Resolv)
- [X profile](https://x.com/ResolvLabs)
- [Telegram](https://t.me/Resolv_Community)
- [Discord](https://discord.com/invite/resolvcommunity)
- [Resolv USR Price: USR/USD Live Price Chart, Market ... - CoinGecko](https://www.coingecko.com/en/coins/resolv-usr)
- [Resolv price today, RESOLV to USD live price, marketcap and chart](https://coinmarketcap.com/currencies/resolv/)
- [What Is Resolv (RESOLV) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/resolv/what-is/)
- [Resolv Liquidity Provider Token Price Chart (RLP) - CoinGecko](https://www.coingecko.com/en/coins/resolv-liquidity-provider-token)
- [ethereum RESOLV](https://etherscan.io/token/0x259338656198ec7a76c729514d3cb45dfbf768a1)
- [bsc RESOLV](https://bscscan.com/token/0xda6cef7f667d992a60eb823ab215493aa0c6b360)
- [base RLP](https://basescan.org/token/0xc31389794ffac23331e0d9f611b7953f90aa5fdc)
- [ethereum RLP](https://etherscan.io/token/0x4956b52ae2ff65d74ca2d61207523288e4528f96)
- [arbitrum RLP](https://arbiscan.io/token/0x35e5db674d8e93a03d814fa0ada70731efe8a4b9)
- [bsc RLP](https://bscscan.com/token/0x35e5db674d8e93a03d814fa0ada70731efe8a4b9)
- [arbitrum WSTUSR](https://arbiscan.io/token/0x66cfbd79257dc5217903a36293120282548e2254)
- [Google project search](https://www.google.com/search?q=Resolv%20RESOLV%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Resolv%20RESOLV%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Resolv%20docs)
- [Google governance search](https://www.google.com/search?q=Resolv%20governance%20proposal)

## Final Investment View

Resolv should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
