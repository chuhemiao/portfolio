---
title: 'Api3 / API3: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Api3 is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'api3-api3-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# Api3 / API3: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Api3 appears in the local CoinGecko/CMC-oriented candidate pool through [CoinGecko](https://www.coingecko.com/en/coins/api3) and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Ethereum; exchange exposure is BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BITGET; category tags are DAO, Oracle; current market cap is $32.72M; FDV is $39.78M; and 24h volume is $5.71M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Api3 before this file was created, and the existing candidate note says: DefiLlama category Risk Curators on Ethereum; TVL $15,626,756. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Api3 is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether API3 captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| FDV | $39.78M |
| 24h volume | $5.71M |
| Circulating supply | 143.32M |
| Total supply | 174.26M |
...
Base case: Api3 remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning API3 into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Api3 is a decentralized oracle network (API3 DAO) providing first-party oracles and Airnode technology to connect web APIs directly to smart contracts. Its native API3 token supports governance, staking, and bonding, while Api3 offers an Api3 Market and mechanisms to recapture Oracle Extractable Value (OEV).

Surf's project profile maps Api3 to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://api3.org/](https://api3.org/) |
| X handle | [@Api3DAO](https://x.com/Api3DAO) |
| X followers | 69.95K |
| Tags | DAO, Oracle |
| Chains | Ethereum |
| Exchanges | BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BITGET |
| TGE status | post |
| CoinGecko ID | [api3](https://www.coingecko.com/en/coins/api3) |

The main product question is whether Api3 is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DeFi infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Api3 convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $5.71M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 143.32M circulating vs 174.26M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Api3 is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| ethereum | API3 | 0x0b38210ea11411557c13457d4da7dc6ea731b88a | [explorer](https://etherscan.io/token/0x0b38210ea11411557c13457d4da7dc6ea731b88a) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Api3, the DeFi-oriented backlog note is the first signal: DefiLlama category Risk Curators on Ethereum; TVL $15,626,756. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $32.72M, FDV $39.78M, 24h volume $5.71M, and price $0.2283.
...
| 24h volume | $5.71M | Position sizing and exit liquidity proxy |
| Market cap | $32.72M | Liquid valuation proxy |
| FDV | $39.78M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.22x | Higher spread means more supply risk |
| X followers | 69.95K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Api3 | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether API3 is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Api3 currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 3.02% and 30d -9.71% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Api3's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2024-05-29 | Treasury Diversification | $4.00M | The Spartan Group, Caladan, DWF Labs, Nomura |
| 2020-11-30 | Ico | $23.00M | not disclosed |
| 2020-11-12 | Seed | $3.00M | Pantera Capital, Placeholder Ventures, Hashed, CoinFund, Digital Currency Group (DCG), Accomplice, Block0 |
| 2019-09-01 | Pre Seed | not disclosed | not disclosed |

| Team member | Role | Link |
|---|---|---|
| Burak Benligiray | Co-Founder | [X](https://twitter.com/bbenligiray) |
| Heikki Vanttinen | Co-Founder | not disclosed |
| Sasa Milic | Co-Founder | not disclosed |
| Joe Grech | Head of Marketing | not disclosed |
...
Api3 competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
Weighted conclusion: Api3 is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Api3 compounds usage, liquidity improves, and API3 gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Api3 has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If API3 does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $5.71M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Api3 announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
|---|---|---|
| Identity | Api3 / API3 | Match official docs, CG, CMC, Surf, contracts |
...
For Api3, the current liquidity read is watchlist first. The market cap is $32.72M, FDV is $39.78M, reported 24h volume is $5.71M, and FDV / market cap is 1.22x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Api3 gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://api3.org/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko profile | [link](https://www.coingecko.com/en/coins/api3) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Api3) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Api3) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Api3) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Api3) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/Api3DAO) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/api3dao) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/API3DAO) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](https://discord.gg/api3dao) | Identity, market, social, contract, or discovery cross-check |
| 11 | API3 price today, API3 to USD live price, marketcap and chart | [link](https://coinmarketcap.com/currencies/api3/) | Identity, market, social, contract, or discovery cross-check |
| 12 | Crypto Data API: Most Comprehensive & Reliable ... | [link](https://www.coingecko.com/en/api) | Identity, market, social, contract, or discovery cross-check |
| 13 | Api3 Price History: Download API3 Historical Data | [link](https://www.coingecko.com/en/coins/api3/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 14 | API3 to EUR: Api3 Price in Euro | [link](https://www.coingecko.com/en/coins/api3/eur) | Identity, market, social, contract, or discovery cross-check |
| 15 | ethereum API3 | [link](https://etherscan.io/token/0x0b38210ea11411557c13457d4da7dc6ea731b88a) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google project search | [link](https://www.google.com/search?q=Api3%20API3%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google tokenomics search | [link](https://www.google.com/search?q=Api3%20API3%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google docs search | [link](https://www.google.com/search?q=Api3%20docs) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google governance search | [link](https://www.google.com/search?q=Api3%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google audit search | [link](https://www.google.com/search?q=Api3%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/api3/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/api3/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Api3) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune token query | [link](https://dune.com/browse/queries?q=Api3) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Api3) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=API3) | Identity, market, social, contract, or discovery cross-check |
| 27 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Api3) | Identity, market, social, contract, or discovery cross-check |
| 28 | CryptoRank search | [link](https://cryptorank.io/price/api3) | Identity, market, social, contract, or discovery cross-check |
| 29 | RootData search | [link](https://www.rootdata.com/search?kw=Api3) | Identity, market, social, contract, or discovery cross-check |
| 30 | CoinDesk search | [link](https://www.coindesk.com/search?s=Api3) | Identity, market, social, contract, or discovery cross-check |
| 31 | The Block search | [link](https://www.theblock.co/search?query=Api3) | Identity, market, social, contract, or discovery cross-check |
| 32 | GitHub search | [link](https://github.com/search?q=Api3&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 33 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=API3) | Identity, market, social, contract, or discovery cross-check |
| 34 | BscScan token search | [link](https://bscscan.com/search?f=0&q=API3) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://api3.org/)
- [CoinGecko profile](https://www.coingecko.com/en/coins/api3)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Api3)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Api3)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Api3)
- [Messari discovery](https://messari.io/search?q=Api3)
- [X profile](https://x.com/Api3DAO)
- [GitHub](https://github.com/api3dao)
- [Telegram](https://t.me/API3DAO)
- [Discord](https://discord.gg/api3dao)
- [API3 price today, API3 to USD live price, marketcap and chart](https://coinmarketcap.com/currencies/api3/)
- [Crypto Data API: Most Comprehensive & Reliable ...](https://www.coingecko.com/en/api)
- [Api3 Price History: Download API3 Historical Data](https://www.coingecko.com/en/coins/api3/historical_data)
- [API3 to EUR: Api3 Price in Euro](https://www.coingecko.com/en/coins/api3/eur)
- [ethereum API3](https://etherscan.io/token/0x0b38210ea11411557c13457d4da7dc6ea731b88a)
- [Google project search](https://www.google.com/search?q=Api3%20API3%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Api3%20API3%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Api3%20docs)
- [Google governance search](https://www.google.com/search?q=Api3%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Api3%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/api3/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/api3/price-analysis/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Api3)
- [Dune token query](https://dune.com/browse/queries?q=Api3)

## Final Investment View

Api3 should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
