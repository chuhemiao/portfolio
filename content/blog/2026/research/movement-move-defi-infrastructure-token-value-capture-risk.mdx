---
title: 'Movement / MOVE: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Movement is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'movement-move-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# Movement / MOVE: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Movement appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Base, Ethereum, HyperEVM, Hyperliquid, Movement; exchange exposure is BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BYBIT, BITGET; category tags are Layer1; current market cap is $48.10M; FDV is $120.19M; and 24h volume is $9.07M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Movement before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 458, market cap $47,729,542, 24h volume $8,797,345. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Movement is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether MOVE captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Base, Ethereum, HyperEVM, Hyperliquid, Movement |
...
Base case: Movement remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning MOVE into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Movement (Movement Labs) is a modular, Move-based blockchain network built on MoveVM/EigenDA to deliver resource-oriented, secure and high-performance smart contracts. It provides developer tooling, bridging to Ethereum, and an ecosystem token (MOVE) used for DeFi, governance and application coordination across the Movement ecosystem.

Surf's project profile maps Movement to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.movementnetwork.xyz/](https://www.movementnetwork.xyz/) |
| X handle | [@movement_xyz](https://x.com/movement_xyz) |
| X followers | 686.21K |
| Tags | Layer1 |
| Chains | Base, Ethereum, HyperEVM, Hyperliquid, Movement |
| Exchanges | BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BYBIT, BITGET |
...
| Liquidity | $9.07M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 4.00B circulating vs 10.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Movement is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Base, Ethereum, HyperEVM, Hyperliquid, Movement |
...
|---|---|---|---|
| base | MOVE | 0x3073f7aaa4db83f95e9fff17424f71d4751a3073 | [explorer](https://basescan.org/token/0x3073f7aaa4db83f95e9fff17424f71d4751a3073) |
| ethereum | MOVE | 0x3073f7aaa4db83f95e9fff17424f71d4751a3073 | [explorer](https://etherscan.io/token/0x3073f7aaa4db83f95e9fff17424f71d4751a3073) |
| HyperEVM | MOVE | 0x3073f7aaa4db83f95e9fff17424f71d4751a3073 | not mapped |
| Movement | MOVE | 0xa | not mapped |
| Hyperliquid | MOVE | 0xedfb8d993ee8cb935338601d8673dfcf | not mapped |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Movement, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 458, market cap $47,729,542, 24h volume $8,797,345. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $48.10M, FDV $120.19M, 24h volume $9.07M, and price $0.0120.
...
| 24h volume | $9.07M | Position sizing and exit liquidity proxy |
| Market cap | $48.10M | Liquid valuation proxy |
| FDV | $120.19M | Dilution-aware valuation proxy |
| FDV / market cap spread | 2.50x | Higher spread means more supply risk |
| X followers | 686.21K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Movement | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether MOVE is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. Movement should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that Movement belongs to the DeFi infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $48.10M, FDV $120.19M, 24h volume $9.07M, price $0.0120, circulating supply 4.00B, and total supply 10.00B. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For Movement, the candidate backlog records Seeded 2026-07-06 from Surf market-ranking; rank 458, market cap $47,729,542, 24h volume $8,797,345. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Base, Ethereum, HyperEVM, Hyperliquid, Movement, exchanges as BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BYBIT, BITGET, and tags as Layer1. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Movement currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 8.58% and 30d 4.32% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Movement's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2024-05-01 | Undisclosed | not disclosed | YZi Labs (Prev. Binance Labs) |
| 2024-04-25 | Series A | $38.00M | Placeholder Ventures, OKX Ventures, Polychain Capital, Maven 11 Capital, Robot Ventures, Figment Capital, Hack VC, Santiago Roel Santos |
| 2023-09-13 | Pre Seed | $3.40M | BENQI Finance, Colony Lab, Blizzard Fund, Borderless Capital, Taureon Capital, Serafund, MH Ventures, Varys Capital |

| Team member | Role | Link |
|---|---|---|
| Rushi Manche | Co-Founder | [X](https://x.com/rushimanche) |
| Cooper Scanlon | Co-Founder | [X](https://x.com/coopsmoves) |
| Franck Cassez | Head of Research | [X](https://x.com/franckdownunder) |
| Brian Henhsi | Head of Strategy | not disclosed |
...
Movement competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Movement has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Movement is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Movement compounds usage, liquidity improves, and MOVE gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If MOVE does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $9.07M | Weekly | Volume below position-size threshold |
| FDV / market cap | 2.50x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Movement announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Movement has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Movement, the first reconciliation job is identity. The token symbol is MOVE, the category is DeFi infrastructure, chains are Base, Ethereum, HyperEVM, Hyperliquid, Movement, exchanges are BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BYBIT, BITGET, and tags are Layer1. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If MOVE trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $48.10M, FDV is $120.19M, 24h volume is $9.07M, FDV / market cap is 2.50x, 7d price change is 8.58%, and 30d price change is 4.32%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
For Movement, the current liquidity read is watchlist first. The market cap is $48.10M, FDV is $120.19M, reported 24h volume is $9.07M, and FDV / market cap is 2.50x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Movement gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.movementnetwork.xyz/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Movement) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Movement) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Movement) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Movement) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Movement) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/movement_xyz) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/movementlabsxyz/movement-docs) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/movementlabsxyz) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](https://discord.gg/moveindustries) | Identity, market, social, contract, or discovery cross-check |
| 11 | Movement Price: MOVE/USD Live Price Chart, Market Cap ... | [link](https://www.coingecko.com/en/coins/movement) | Identity, market, social, contract, or discovery cross-check |
...
| 13 | Crypto Data API: Most Comprehensive & Reliable ... | [link](https://www.coingecko.com/en/api) | Identity, market, social, contract, or discovery cross-check |
| 14 | Top Movement Ecosystem Coins by Market Cap | [link](https://www.coingecko.com/en/categories/movement-ecosystem) | Identity, market, social, contract, or discovery cross-check |
| 15 | Movement (MOVE) Price Prediction For 2026 & Beyond | [link](https://coinmarketcap.com/cmc-ai/movement/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 16 | base MOVE | [link](https://basescan.org/token/0x3073f7aaa4db83f95e9fff17424f71d4751a3073) | Identity, market, social, contract, or discovery cross-check |
| 17 | ethereum MOVE | [link](https://etherscan.io/token/0x3073f7aaa4db83f95e9fff17424f71d4751a3073) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google project search | [link](https://www.google.com/search?q=Movement%20MOVE%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google tokenomics search | [link](https://www.google.com/search?q=Movement%20MOVE%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google docs search | [link](https://www.google.com/search?q=Movement%20docs) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google governance search | [link](https://www.google.com/search?q=Movement%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google audit search | [link](https://www.google.com/search?q=Movement%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/movement/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/movement/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 25 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/movement/) | Identity, market, social, contract, or discovery cross-check |
| 26 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Movement) | Identity, market, social, contract, or discovery cross-check |
| 27 | Dune token query | [link](https://dune.com/browse/queries?q=Movement) | Identity, market, social, contract, or discovery cross-check |
| 28 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Movement) | Identity, market, social, contract, or discovery cross-check |
| 29 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=MOVE) | Identity, market, social, contract, or discovery cross-check |
| 30 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Movement) | Identity, market, social, contract, or discovery cross-check |
| 31 | CryptoRank search | [link](https://cryptorank.io/price/movement) | Identity, market, social, contract, or discovery cross-check |
| 32 | RootData search | [link](https://www.rootdata.com/search?kw=Movement) | Identity, market, social, contract, or discovery cross-check |
| 33 | CoinDesk search | [link](https://www.coindesk.com/search?s=Movement) | Identity, market, social, contract, or discovery cross-check |
| 34 | The Block search | [link](https://www.theblock.co/search?query=Movement) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.movementnetwork.xyz/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Movement)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Movement)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Movement)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Movement)
- [Messari discovery](https://messari.io/search?q=Movement)
- [X profile](https://x.com/movement_xyz)
- [GitHub](https://github.com/movementlabsxyz/movement-docs)
- [Telegram](https://t.me/movementlabsxyz)
- [Discord](https://discord.gg/moveindustries)
- [Movement Price: MOVE/USD Live Price Chart, Market Cap ...](https://www.coingecko.com/en/coins/movement)
...
- [Top Movement Ecosystem Coins by Market Cap](https://www.coingecko.com/en/categories/movement-ecosystem)
- [Movement (MOVE) Price Prediction For 2026 & Beyond](https://coinmarketcap.com/cmc-ai/movement/price-prediction/)
- [base MOVE](https://basescan.org/token/0x3073f7aaa4db83f95e9fff17424f71d4751a3073)
- [ethereum MOVE](https://etherscan.io/token/0x3073f7aaa4db83f95e9fff17424f71d4751a3073)
- [Google project search](https://www.google.com/search?q=Movement%20MOVE%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Movement%20MOVE%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Movement%20docs)
- [Google governance search](https://www.google.com/search?q=Movement%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Movement%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/movement/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/movement/price-analysis/)

## Final Investment View

Movement should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
