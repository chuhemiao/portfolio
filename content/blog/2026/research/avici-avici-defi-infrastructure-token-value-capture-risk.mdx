---
title: 'Avici / AVICI: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Avici is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'avici-avici-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# Avici / AVICI: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Avici appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Solana; exchange exposure is not disclosed; category tags are Payments & Gateway, Wallets & AA (Account Abstraction); current market cap is $10.76M; FDV is $10.76M; and 24h volume is $664.79K. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Avici before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 1141, market cap $10,607,015, 24h volume $602,388. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Avici is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether AVICI captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| FDV | $10.76M |
| 24h volume | $664.79K |
| Circulating supply | 12.90M |
| Total supply | 12.90M |
| 7d price change | -20.73% |
...
Base case: Avici remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning AVICI into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Avici is a crypto neobank that issues Visa cards and self-custodial wallets, letting users top up cards with cryptocurrency to spend in fiat. It provides USD/EUR currency accounts, virtual and physical cards, mobile apps, and emphasizes low fees and multi-chain support.

Surf's project profile maps Avici to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://avici.money/](https://avici.money/) |
| X handle | [@AviciMoney](https://x.com/AviciMoney) |
| X followers | 47.47K |
| Tags | Payments & Gateway, Wallets & AA (Account Abstraction) |
| Chains | Solana |
| Exchanges | not disclosed |
| TGE status | not disclosed |
| CoinGecko ID | not disclosed |

The main product question is whether Avici is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DeFi infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Avici convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $664.79K 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 12.90M circulating vs 12.90M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Avici is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

Avici's architecture should be evaluated through the job it performs. For a DeFi infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
|---|---|---|---|
| solana | AVICI | BANKJmvhT8tiJRsBSS1n2HryMBPvT5Ze4HU95DUAmeta | [explorer](https://solscan.io/token/BANKJmvhT8tiJRsBSS1n2HryMBPvT5Ze4HU95DUAmeta) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Avici, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 1141, market cap $10,607,015, 24h volume $602,388. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $10.76M, FDV $10.76M, 24h volume $664.79K, and price $0.8342.
...
| 24h volume | $664.79K | Position sizing and exit liquidity proxy |
...
| Accrual channel | Strength for Avici | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether AVICI is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Avici currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d -20.73% and 30d 15.98% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Avici's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-10-14 | Ido | $3.50M | not disclosed |
...
Avici competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Avici has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Avici is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Avici compounds usage, liquidity improves, and AVICI gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Avici has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
| 24h volume | $664.79K | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Avici announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
|---|---|---|
| Identity | Avici / AVICI | Match official docs, CG, CMC, Surf, contracts |
...
For Avici, the current liquidity read is watchlist first. The market cap is $10.76M, FDV is $10.76M, reported 24h volume is $664.79K, and FDV / market cap is 1.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Avici gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://avici.money/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Avici) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Avici) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Avici) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Avici) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Avici) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/AviciMoney) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/Aviciclub) | Identity, market, social, contract, or discovery cross-check |
| 9 | AVICI/USD Live Price Chart, Market Cap & News Today - CoinGecko | [link](https://www.coingecko.com/en/coins/avici) | Identity, market, social, contract, or discovery cross-check |
| 10 | Avici price today, AVICI to USD live price, marketcap and chart | [link](https://coinmarketcap.com/currencies/avici/) | Identity, market, social, contract, or discovery cross-check |
| 11 | What Is Avici (AVICI) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/avici/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 12 | AVICI to INR: Avici Price in Indian Rupee - CoinGecko | [link](https://www.coingecko.com/en/coins/avici/inr) | Identity, market, social, contract, or discovery cross-check |
| 13 | Avici (AVICI) Price Prediction For 2026 & Beyond - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/avici/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 14 | solana AVICI | [link](https://solscan.io/token/BANKJmvhT8tiJRsBSS1n2HryMBPvT5Ze4HU95DUAmeta) | Identity, market, social, contract, or discovery cross-check |
| 15 | Google project search | [link](https://www.google.com/search?q=Avici%20AVICI%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google tokenomics search | [link](https://www.google.com/search?q=Avici%20AVICI%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google docs search | [link](https://www.google.com/search?q=Avici%20docs) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google governance search | [link](https://www.google.com/search?q=Avici%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google audit search | [link](https://www.google.com/search?q=Avici%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 20 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/avici/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 21 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Avici) | Identity, market, social, contract, or discovery cross-check |
| 22 | Dune token query | [link](https://dune.com/browse/queries?q=Avici) | Identity, market, social, contract, or discovery cross-check |
| 23 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Avici) | Identity, market, social, contract, or discovery cross-check |
| 24 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=AVICI) | Identity, market, social, contract, or discovery cross-check |
| 25 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Avici) | Identity, market, social, contract, or discovery cross-check |
| 26 | CryptoRank search | [link](https://cryptorank.io/price/avici) | Identity, market, social, contract, or discovery cross-check |
| 27 | RootData search | [link](https://www.rootdata.com/search?kw=Avici) | Identity, market, social, contract, or discovery cross-check |
| 28 | CoinDesk search | [link](https://www.coindesk.com/search?s=Avici) | Identity, market, social, contract, or discovery cross-check |
| 29 | The Block search | [link](https://www.theblock.co/search?query=Avici) | Identity, market, social, contract, or discovery cross-check |
| 30 | GitHub search | [link](https://github.com/search?q=Avici&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 31 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=AVICI) | Identity, market, social, contract, or discovery cross-check |
| 32 | BscScan token search | [link](https://bscscan.com/search?f=0&q=AVICI) | Identity, market, social, contract, or discovery cross-check |
| 33 | Arbiscan token search | [link](https://arbiscan.io/search?f=0&q=AVICI) | Identity, market, social, contract, or discovery cross-check |
| 34 | BaseScan token search | [link](https://basescan.org/search?f=0&q=AVICI) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://avici.money/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Avici)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Avici)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Avici)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Avici)
- [Messari discovery](https://messari.io/search?q=Avici)
- [X profile](https://x.com/AviciMoney)
- [Telegram](https://t.me/Aviciclub)
- [AVICI/USD Live Price Chart, Market Cap & News Today - CoinGecko](https://www.coingecko.com/en/coins/avici)
- [Avici price today, AVICI to USD live price, marketcap and chart](https://coinmarketcap.com/currencies/avici/)
- [What Is Avici (AVICI) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/avici/what-is/)
- [AVICI to INR: Avici Price in Indian Rupee - CoinGecko](https://www.coingecko.com/en/coins/avici/inr)
- [Avici (AVICI) Price Prediction For 2026 & Beyond - CoinMarketCap](https://coinmarketcap.com/cmc-ai/avici/price-prediction/)
- [solana AVICI](https://solscan.io/token/BANKJmvhT8tiJRsBSS1n2HryMBPvT5Ze4HU95DUAmeta)
- [Google project search](https://www.google.com/search?q=Avici%20AVICI%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Avici%20AVICI%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Avici%20docs)
- [Google governance search](https://www.google.com/search?q=Avici%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Avici%20audit%20report)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/avici/price-analysis/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Avici)
- [Dune token query](https://dune.com/browse/queries?q=Avici)
- [DeFiLlama fees discovery](https://defillama.com/fees?chain=All&search=Avici)
- [DeFiLlama yield discovery](https://defillama.com/yields?token=AVICI)

## Final Investment View

Avici should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
