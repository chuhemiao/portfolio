---
title: 'Space and Time / SXT: privacy infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Space and Time is treated as a privacy infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'space-and-time-sxt-privacy-infrastructure-token-value-capture-risk'
category: 'research'
---

# Space and Time / SXT: privacy infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Space and Time appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Ethereum; exchange exposure is BINANCE, COINBASE, BITHUMB, BYBIT, BITGET; category tags are Privacy & ZK, Layer1, Storage & Compute; current market cap is $17.96M; FDV is $34.55M; and 24h volume is $3.13M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Space and Time before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 873, market cap $18,214,824, 24h volume $2,878,614. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Space and Time is best understood as a **privacy infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether SXT captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
Base case: Space and Time remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning SXT into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Space and Time is a decentralized blockchain platform that provides a ZK‑proven relational database, indexed Web3 data, and APIs enabling sub‑second Proof of SQL queries over on‑chain and off‑chain data. Its native token SXT secures the validator network via staking/slashing and powers protocol payments, data incentives, and query/insert fees.

Surf's project profile maps Space and Time to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.spaceandtime.io/](https://www.spaceandtime.io/) |
| X handle | [@spaceandtime](https://x.com/spaceandtime) |
| X followers | 353.75K |
| Tags | Privacy & ZK, Layer1, Storage & Compute |
| Chains | Ethereum |
| Exchanges | BINANCE, COINBASE, BITHUMB, BYBIT, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Space and Time is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For privacy infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Space and Time convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $3.13M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 2.60B circulating vs 5.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Space and Time is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| ethereum | SXT | 0xe6bfd33f52d82ccb5b37e16d3dd81f9ffdabb195 | [explorer](https://etherscan.io/token/0xe6bfd33f52d82ccb5b37e16d3dd81f9ffdabb195) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Space and Time, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 873, market cap $18,214,824, 24h volume $2,878,614. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $17.96M, FDV $34.55M, 24h volume $3.13M, and price $0.0069.
...
| 24h volume | $3.13M | Position sizing and exit liquidity proxy |
| Market cap | $17.96M | Liquid valuation proxy |
| FDV | $34.55M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.92x | Higher spread means more supply risk |
| X followers | 353.75K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Space and Time | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether SXT is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. Space and Time should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that Space and Time belongs to the privacy infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $17.96M, FDV $34.55M, 24h volume $3.13M, price $0.0069, circulating supply 2.60B, and total supply 5.00B. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For Space and Time, the candidate backlog records Seeded 2026-07-06 from Surf market-ranking; rank 873, market cap $18,214,824, 24h volume $2,878,614. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Ethereum, exchanges as BINANCE, COINBASE, BITHUMB, BYBIT, BITGET, and tags as Privacy & ZK, Layer1, Storage & Compute. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Space and Time currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 3.47% and 30d -16.71% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Space and Time's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2024-08-27 | Series A | $20.00M | Arrington Capital, OKX Ventures, Framework Ventures, Digital Currency Group (DCG), Alumni Ventures, M12, Faction, F-Prime Capital |
| 2022-09-27 | Strategic | $20.00M | Framework Ventures, HashKey Capital, Blizzard Fund, SevenX Ventures, P2 Ventures (Polygon Ventures), Foresight Ventures, M12 |
| 2022-07-28 | Seed | $10.00M | Framework Ventures, Digital Currency Group (DCG), HashKey Capital, Bitscale Capital, IOSG Ventures, GSR, Alliance DAO, SamsungNext |

| Team member | Role | Link |
|---|---|---|
| Nate Holiday | Co-Founder, CEO | [X](https://x.com/holiday_nate) |
| Scott Dykstra | Co-Founder, CTO | [X](https://x.com/chiefbuidl) |
| Jay White | Co-Founder, Head of Research | not disclosed |
| Craig Holiday | Co-Founder, CFO | not disclosed |
| Rika Khurdayan | CLO | [X](https://x.com/rkhurdayan) |
...
Space and Time competes in the privacy infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Space and Time has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Space and Time is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Space and Time compounds usage, liquidity improves, and SXT gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If SXT does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $3.13M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Space and Time announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
|---|---|---|
| Identity | Space and Time / SXT | Match official docs, CG, CMC, Surf, contracts |
...
For Space and Time, the current liquidity read is watchlist first. The market cap is $17.96M, FDV is $34.55M, reported 24h volume is $3.13M, and FDV / market cap is 1.92x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. Space and Time can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.

For Space and Time, the live monitoring stack should include contract events, TVL, volume, governance, documentation changes, social announcements, security disclosures, and exchange-market quality. A single metric is not enough. TVL may stay high while liquidity thins. Volume may rise because of volatility rather than adoption. Social attention may rise around controversy rather than progress. Governance may pass proposals that improve short-term price but add long-term risk. The strongest monitoring dashboard combines on-chain state, off-chain disclosure, and market quality.

Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If Space and Time suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | Space and Time gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.spaceandtime.io/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Space%20and%20Time) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Space%20and%20Time) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Space%20and%20Time) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Space%20and%20Time) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Space%20and%20Time) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/spaceandtime) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/spaceandtimefdn) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/spaceandtimedb) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](http://discord.gg/spaceandtimeDB) | Identity, market, social, contract, or discovery cross-check |
| 11 | Space and Time Price Chart (SXT) - CoinGecko | [link](https://www.coingecko.com/en/coins/space-and-time) | Identity, market, social, contract, or discovery cross-check |
| 12 | Space and Time / The Data Blockchain Securing Onchain Finance | [link](https://spaceandtime.io/) | Identity, market, social, contract, or discovery cross-check |
| 13 | What is Space and Time (SXT)? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/space-and-time/what-is/) | Identity, market, social, contract, or discovery cross-check |
...
| 15 | Space and Time (SXT) Price Prediction - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/space-and-time/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 16 | ethereum SXT | [link](https://etherscan.io/token/0xe6bfd33f52d82ccb5b37e16d3dd81f9ffdabb195) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google project search | [link](https://www.google.com/search?q=Space%20and%20Time%20SXT%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google tokenomics search | [link](https://www.google.com/search?q=Space%20and%20Time%20SXT%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google docs search | [link](https://www.google.com/search?q=Space%20and%20Time%20docs) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google governance search | [link](https://www.google.com/search?q=Space%20and%20Time%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google audit search | [link](https://www.google.com/search?q=Space%20and%20Time%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/space-and-time/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/space-and-time/) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Space%20and%20Time) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune token query | [link](https://dune.com/browse/queries?q=Space%20and%20Time) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Space%20and%20Time) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=SXT) | Identity, market, social, contract, or discovery cross-check |
| 28 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Space%20and%20Time) | Identity, market, social, contract, or discovery cross-check |
| 29 | CryptoRank search | [link](https://cryptorank.io/price/space-and-time) | Identity, market, social, contract, or discovery cross-check |
| 30 | RootData search | [link](https://www.rootdata.com/search?kw=Space%20and%20Time) | Identity, market, social, contract, or discovery cross-check |
| 31 | CoinDesk search | [link](https://www.coindesk.com/search?s=Space%20and%20Time) | Identity, market, social, contract, or discovery cross-check |
| 32 | The Block search | [link](https://www.theblock.co/search?query=Space%20and%20Time) | Identity, market, social, contract, or discovery cross-check |
| 33 | GitHub search | [link](https://github.com/search?q=Space%20and%20Time&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 34 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=SXT) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.spaceandtime.io/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Space%20and%20Time)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Space%20and%20Time)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Space%20and%20Time)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Space%20and%20Time)
- [Messari discovery](https://messari.io/search?q=Space%20and%20Time)
- [X profile](https://x.com/spaceandtime)
- [GitHub](https://github.com/spaceandtimefdn)
- [Telegram](https://t.me/spaceandtimedb)
- [Discord](http://discord.gg/spaceandtimeDB)
- [Space and Time Price Chart (SXT) - CoinGecko](https://www.coingecko.com/en/coins/space-and-time)
- [Space and Time | The Data Blockchain Securing Onchain Finance](https://spaceandtime.io/)
- [What is Space and Time (SXT)? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/space-and-time/what-is/)
- [CoinGecko: Cryptocurrency Prices, Charts, and Crypto Market Cap](https://www.coingecko.com/)
- [Space and Time (SXT) Price Prediction - CoinMarketCap](https://coinmarketcap.com/cmc-ai/space-and-time/price-prediction/)
- [ethereum SXT](https://etherscan.io/token/0xe6bfd33f52d82ccb5b37e16d3dd81f9ffdabb195)
- [Google project search](https://www.google.com/search?q=Space%20and%20Time%20SXT%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Space%20and%20Time%20SXT%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Space%20and%20Time%20docs)
- [Google governance search](https://www.google.com/search?q=Space%20and%20Time%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Space%20and%20Time%20audit%20report)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/space-and-time/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/space-and-time/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Space%20and%20Time)

## Final Investment View

Space and Time should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
