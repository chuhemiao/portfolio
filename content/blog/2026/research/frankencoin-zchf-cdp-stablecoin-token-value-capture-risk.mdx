---
title: 'Frankencoin / ZCHF: CDP stablecoin value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Frankencoin is treated as a CDP stablecoin watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'frankencoin-zchf-cdp-stablecoin-token-value-capture-risk'
category: 'research'
---

# Frankencoin / ZCHF: CDP stablecoin value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Frankencoin appears in the local CoinGecko/CMC-oriented candidate pool through [CoinGecko](https://www.coingecko.com/en/coins/frankencoin) and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Arbitrum One, Avalanche, Base, Ethereum, Optimistic Ethereum, Polygon Pos, Sonic, Xdai; exchange exposure is not disclosed; category tags are Lending & Borrowing, Stablecoins, Yield & Asset Mgmt; current market cap is $42.29M; FDV is $42.29M; and 24h volume is $217.79K. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Frankencoin before this file was created, and the existing candidate note says: DefiLlama category CDP on Ethereum; TVL $66,893,567. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Frankencoin is best understood as a **CDP stablecoin** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether ZCHF captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Arbitrum One, Avalanche, Base, Ethereum, Optimistic Ethereum, Polygon Pos, Sonic, Xdai |
| Exchanges | not disclosed |
| Price | $1.2400 |
| Market cap | $42.29M |
| FDV | $42.29M |
| 24h volume | $217.79K |
| Circulating supply | 34.03M |
| Total supply | 34.03M |
...
Base case: Frankencoin remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning ZCHF into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Frankencoin is a decentralized, collateral-backed stablecoin (ZCHF) that tracks the Swiss Franc using an oracle-free mechanism and community governance via FPS tokens. It enables users to mint ZCHF against crypto collateral for borrowing, payments, and earning yields in DeFi.

Surf's project profile maps Frankencoin to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | not disclosed |
| X handle | [@frankencoinzchf](https://x.com/frankencoinzchf) |
| X followers | 4.73K |
| Tags | Lending & Borrowing, Stablecoins, Yield & Asset Mgmt |
| Chains | Arbitrum One, Avalanche, Base, Ethereum, Optimistic Ethereum, Polygon Pos, Sonic, Xdai |
| Exchanges | not disclosed |
| TGE status | not disclosed |
| CoinGecko ID | [frankencoin](https://www.coingecko.com/en/coins/frankencoin) |

The main product question is whether Frankencoin is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For CDP stablecoin projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Frankencoin convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $217.79K 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 34.03M circulating vs 34.03M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Frankencoin is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

Frankencoin's architecture should be evaluated through the job it performs. For a CDP stablecoin project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
| Settlement layer | Chains and contracts that hold user value | Arbitrum One, Avalanche, Base, Ethereum, Optimistic Ethereum, Polygon Pos, Sonic, Xdai |
...
|---|---|---|---|
| arbitrum | ZCHF | 0xd4dd9e2f021bb459d5a5f6c24c12fe09c5d45553 | [explorer](https://arbiscan.io/token/0xd4dd9e2f021bb459d5a5f6c24c12fe09c5d45553) |
| Xdai | ZCHF | 0xd4dd9e2f021bb459d5a5f6c24c12fe09c5d45553 | not mapped |
| Sonic | ZCHF | 0xd4dd9e2f021bb459d5a5f6c24c12fe09c5d45553 | not mapped |
| ethereum | ZCHF | 0xb58e61c3098d85632df34eecfb899a1ed80921cb | [explorer](https://etherscan.io/token/0xb58e61c3098d85632df34eecfb899a1ed80921cb) |
| avalanche | ZCHF | 0xd4dd9e2f021bb459d5a5f6c24c12fe09c5d45553 | [explorer](https://snowtrace.io/token/0xd4dd9e2f021bb459d5a5f6c24c12fe09c5d45553) |
| polygon | ZCHF | 0xd4dd9e2f021bb459d5a5f6c24c12fe09c5d45553 | [explorer](https://polygonscan.com/token/0xd4dd9e2f021bb459d5a5f6c24c12fe09c5d45553) |
| optimism | ZCHF | 0xd4dd9e2f021bb459d5a5f6c24c12fe09c5d45553 | [explorer](https://optimistic.etherscan.io/token/0xd4dd9e2f021bb459d5a5f6c24c12fe09c5d45553) |
| base | ZCHF | 0xd4dd9e2f021bb459d5a5f6c24c12fe09c5d45553 | [explorer](https://basescan.org/token/0xd4dd9e2f021bb459d5a5f6c24c12fe09c5d45553) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Frankencoin, the DeFi-oriented backlog note is the first signal: DefiLlama category CDP on Ethereum; TVL $66,893,567. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $42.29M, FDV $42.29M, 24h volume $217.79K, and price $1.2400.
...
| 24h volume | $217.79K | Position sizing and exit liquidity proxy |
| Market cap | $42.29M | Liquid valuation proxy |
| FDV | $42.29M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.00x | Higher spread means more supply risk |
| X followers | 4.73K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Frankencoin | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether ZCHF is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. Frankencoin should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that Frankencoin belongs to the CDP stablecoin bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $42.29M, FDV $42.29M, 24h volume $217.79K, price $1.2400, circulating supply 34.03M, and total supply 34.03M. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For Frankencoin, the candidate backlog records DefiLlama category CDP on Ethereum; TVL $66,893,567. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Arbitrum One, Avalanche, Base, Ethereum, Optimistic Ethereum, Polygon Pos, Sonic, Xdai, exchanges as not disclosed, and tags as Lending & Borrowing, Stablecoins, Yield & Asset Mgmt. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.

The fourth frame is value capture. A CDP stablecoin asset can be useful while the token remains weak. The memo therefore separates product-market fit from token-market fit. Product-market fit asks whether users need the protocol even when incentives fade. Token-market fit asks whether the token absorbs value through staking, fee routing, collateral, governance constraints, emissions control, slashing insurance, or privileged access. If the token is mostly governance, the base case assigns lower confidence. If the token is used for security or fee capture but the mechanism is not yet activated, the base case treats value capture as an option rather than as current cash flow.

The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Frankencoin currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 0.66% and 30d -1.00% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Frankencoin's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
Frankencoin competes in the CDP stablecoin bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Frankencoin has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Frankencoin is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Frankencoin compounds usage, liquidity improves, and ZCHF gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If ZCHF does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $217.79K | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Frankencoin announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Frankencoin has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Frankencoin, the first reconciliation job is identity. The token symbol is ZCHF, the category is CDP stablecoin, chains are Arbitrum One, Avalanche, Base, Ethereum, Optimistic Ethereum, Polygon Pos, Sonic, Xdai, exchanges are not disclosed, and tags are Lending & Borrowing, Stablecoins, Yield & Asset Mgmt. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If ZCHF trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $42.29M, FDV is $42.29M, 24h volume is $217.79K, FDV / market cap is 1.00x, 7d price change is 0.66%, and 30d price change is -1.00%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.

The third reconciliation job is usage data. The latest TVL sample is not disclosed, TVL sample change is not disclosed, and the sample contains 0 point(s). TVL is useful only when the underlying balances are real, withdrawable, and not double-counted. For CDP stablecoin, the memo should identify whether TVL means deposits, staked assets, LP liquidity, stablecoin supply, collateral, locked governance assets, or a derived wrapper. A yield-bearing stablecoin and a validator middleware protocol can both show TVL, but the risk encoded by that TVL is completely different.

| Reconciliation item | Current value | Next check |
|---|---|---|
| Identity | Frankencoin / ZCHF | Match official docs, CG, CMC, Surf, contracts |
| Category | CDP stablecoin | Confirm product mechanics and revenue model |
| Chains | Arbitrum One, Avalanche, Base, Ethereum, Optimistic Ethereum, Polygon Pos, Sonic, Xdai | Confirm canonical deployments and bridge assumptions |
| Exchanges | not disclosed | Check live order books and inactive pairs |
| Market cap | $42.29M | Compare Surf, CG, CMC, exchange pages |
| FDV | $42.29M | Reconcile total supply and unlock schedule |
| Volume | $217.79K | Check exchange concentration and wash-trading risk |
| TVL | not disclosed | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Frankencoin does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Frankencoin, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does ZCHF sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
| Correlation check | Count exposure to related CDP stablecoin assets | Category beta can hide concentration |
...
The failure-mode view is deliberately harsher than the base case. Frankencoin can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.

For Frankencoin, the live monitoring stack should include contract events, TVL, volume, governance, documentation changes, social announcements, security disclosures, and exchange-market quality. A single metric is not enough. TVL may stay high while liquidity thins. Volume may rise because of volatility rather than adoption. Social attention may rise around controversy rather than progress. Governance may pass proposals that improve short-term price but add long-term risk. The strongest monitoring dashboard combines on-chain state, off-chain disclosure, and market quality.

Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If Frankencoin suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | Frankencoin gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | CoinGecko profile | [link](https://www.coingecko.com/en/coins/frankencoin) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Frankencoin) | Identity, market, social, contract, or discovery cross-check |
| 3 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Frankencoin) | Identity, market, social, contract, or discovery cross-check |
| 4 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Frankencoin) | Identity, market, social, contract, or discovery cross-check |
| 5 | Messari discovery | [link](https://messari.io/search?q=Frankencoin) | Identity, market, social, contract, or discovery cross-check |
| 6 | X profile | [link](https://x.com/frankencoinzchf) | Identity, market, social, contract, or discovery cross-check |
| 7 | What Is Frankencoin (ZCHF) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/frankencoin/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 8 | Frankencoin - Swiss Franc Stablecoin (ZCHF) | [link](https://frankencoin.com/) | Identity, market, social, contract, or discovery cross-check |
| 9 | ZCHF to EUR: Frankencoin Price in Euro - CoinGecko | [link](https://www.coingecko.com/en/coins/frankencoin/eur) | Identity, market, social, contract, or discovery cross-check |
| 10 | ZCHF to BTC: Frankencoin Price in Bitcoin - CoinGecko | [link](https://www.coingecko.com/en/coins/frankencoin/btc) | Identity, market, social, contract, or discovery cross-check |
| 11 | arbitrum ZCHF | [link](https://arbiscan.io/token/0xd4dd9e2f021bb459d5a5f6c24c12fe09c5d45553) | Identity, market, social, contract, or discovery cross-check |
| 12 | ethereum ZCHF | [link](https://etherscan.io/token/0xb58e61c3098d85632df34eecfb899a1ed80921cb) | Identity, market, social, contract, or discovery cross-check |
| 13 | avalanche ZCHF | [link](https://snowtrace.io/token/0xd4dd9e2f021bb459d5a5f6c24c12fe09c5d45553) | Identity, market, social, contract, or discovery cross-check |
| 14 | polygon ZCHF | [link](https://polygonscan.com/token/0xd4dd9e2f021bb459d5a5f6c24c12fe09c5d45553) | Identity, market, social, contract, or discovery cross-check |
| 15 | optimism ZCHF | [link](https://optimistic.etherscan.io/token/0xd4dd9e2f021bb459d5a5f6c24c12fe09c5d45553) | Identity, market, social, contract, or discovery cross-check |
| 16 | base ZCHF | [link](https://basescan.org/token/0xd4dd9e2f021bb459d5a5f6c24c12fe09c5d45553) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google project search | [link](https://www.google.com/search?q=Frankencoin%20ZCHF%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google tokenomics search | [link](https://www.google.com/search?q=Frankencoin%20ZCHF%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google docs search | [link](https://www.google.com/search?q=Frankencoin%20docs) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google governance search | [link](https://www.google.com/search?q=Frankencoin%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google audit search | [link](https://www.google.com/search?q=Frankencoin%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinGecko historical data | [link](https://www.coingecko.com/en/coins/frankencoin/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/frankencoin/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/frankencoin/) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Frankencoin) | Identity, market, social, contract, or discovery cross-check |
| 26 | Dune token query | [link](https://dune.com/browse/queries?q=Frankencoin) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Frankencoin) | Identity, market, social, contract, or discovery cross-check |
| 28 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=ZCHF) | Identity, market, social, contract, or discovery cross-check |
| 29 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Frankencoin) | Identity, market, social, contract, or discovery cross-check |
| 30 | CryptoRank search | [link](https://cryptorank.io/price/frankencoin) | Identity, market, social, contract, or discovery cross-check |
| 31 | RootData search | [link](https://www.rootdata.com/search?kw=Frankencoin) | Identity, market, social, contract, or discovery cross-check |
| 32 | CoinDesk search | [link](https://www.coindesk.com/search?s=Frankencoin) | Identity, market, social, contract, or discovery cross-check |
| 33 | The Block search | [link](https://www.theblock.co/search?query=Frankencoin) | Identity, market, social, contract, or discovery cross-check |
| 34 | GitHub search | [link](https://github.com/search?q=Frankencoin&type=repositories) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [CoinGecko profile](https://www.coingecko.com/en/coins/frankencoin)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Frankencoin)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Frankencoin)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Frankencoin)
- [Messari discovery](https://messari.io/search?q=Frankencoin)
- [X profile](https://x.com/frankencoinzchf)
- [What Is Frankencoin (ZCHF) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/frankencoin/what-is/)
- [Frankencoin - Swiss Franc Stablecoin (ZCHF)](https://frankencoin.com/)
- [ZCHF to EUR: Frankencoin Price in Euro - CoinGecko](https://www.coingecko.com/en/coins/frankencoin/eur)
- [ZCHF to BTC: Frankencoin Price in Bitcoin - CoinGecko](https://www.coingecko.com/en/coins/frankencoin/btc)
- [arbitrum ZCHF](https://arbiscan.io/token/0xd4dd9e2f021bb459d5a5f6c24c12fe09c5d45553)
- [ethereum ZCHF](https://etherscan.io/token/0xb58e61c3098d85632df34eecfb899a1ed80921cb)
- [avalanche ZCHF](https://snowtrace.io/token/0xd4dd9e2f021bb459d5a5f6c24c12fe09c5d45553)
- [polygon ZCHF](https://polygonscan.com/token/0xd4dd9e2f021bb459d5a5f6c24c12fe09c5d45553)
- [optimism ZCHF](https://optimistic.etherscan.io/token/0xd4dd9e2f021bb459d5a5f6c24c12fe09c5d45553)
- [base ZCHF](https://basescan.org/token/0xd4dd9e2f021bb459d5a5f6c24c12fe09c5d45553)
- [Google project search](https://www.google.com/search?q=Frankencoin%20ZCHF%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Frankencoin%20ZCHF%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Frankencoin%20docs)
- [Google governance search](https://www.google.com/search?q=Frankencoin%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Frankencoin%20audit%20report)
- [CoinGecko historical data](https://www.coingecko.com/en/coins/frankencoin/historical_data)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/frankencoin/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/frankencoin/)

## Final Investment View

Frankencoin should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
