---
title: 'Solayer / LAYER: staking infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Solayer is treated as a staking infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'solayer-layer-staking-infrastructure-token-value-capture-risk'
category: 'research'
---

# Solayer / LAYER: staking infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Solayer appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Solana; exchange exposure is BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BYBIT, BITGET; category tags are Layer1, LSD & Restaking; current market cap is $14.09M; FDV is $67.11M; and 24h volume is $12.15M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Solayer before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 996, market cap $14,194,411, 24h volume $12,394,287. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Solayer is best understood as a **staking infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether LAYER captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
Base case: Solayer remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning LAYER into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Solayer is a hardware‑accelerated Layer‑1 SVM blockchain (InfiniSVM) leveraging RDMA/InfiniBand and a multi‑executor model to target >1M TPS and ultra‑low latency. It offers a vertically integrated finance stack on Solana—liquid restaking, native assets (sSOL, sUSD), on‑chain payments (Emerald Card) and the LAYER governance token.

Surf's project profile maps Solayer to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://solayer.org/](https://solayer.org/) |
| X handle | [@solayer_labs](https://x.com/solayer_labs) |
| X followers | 182.19K |
| Tags | Layer1, LSD & Restaking |
| Chains | Solana |
| Exchanges | BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BYBIT, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Solayer is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For staking infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Solayer convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $12.15M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 210.00M circulating vs 1.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Solayer is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| solana | LAYER | LAYER4xPpTCb3QL8S9u41EAhAX7mhBn8Q6xMTwY2Yzc | [explorer](https://solscan.io/token/LAYER4xPpTCb3QL8S9u41EAhAX7mhBn8Q6xMTwY2Yzc) |
| solana | SUSD | susdabGDNbhrnCa6ncrYo81u4s9GM8ecK2UwMyZiq4X | [explorer](https://solscan.io/token/susdabGDNbhrnCa6ncrYo81u4s9GM8ecK2UwMyZiq4X) |
| solana | SSOL | sSo14endRuUbvQaJS3dq36Q829a3A6BEfoeeRGJywEh | [explorer](https://solscan.io/token/sSo14endRuUbvQaJS3dq36Q829a3A6BEfoeeRGJywEh) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Solayer, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 996, market cap $14,194,411, 24h volume $12,394,287. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $14.09M, FDV $67.11M, 24h volume $12.15M, and price $0.0671.
...
| 24h volume | $12.15M | Position sizing and exit liquidity proxy |
| Market cap | $14.09M | Liquid valuation proxy |
| FDV | $67.11M | Dilution-aware valuation proxy |
| FDV / market cap spread | 4.76x | Higher spread means more supply risk |
| X followers | 182.19K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Solayer | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether LAYER is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Solayer currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 0.54% and 30d 7.64% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Solayer's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-01-16 | Ido | $10.50M | not disclosed |
| 2024-08-27 | Seed | $12.00M | YZi Labs (Prev. Binance Labs), Polychain Capital, Bitscale Capital, Borderless Capital, Big Brain Holdings, Hack VC, Race Capital, Road Capital |
| 2024-08-07 | Undisclosed | not disclosed | YZi Labs (Prev. Binance Labs) |
| 2024-07-01 | Undisclosed | not disclosed | Sandeep Nailwal, DCF God, Anatoly Yakovenko, Joe Lallouz, Xinshu Dong, Stanford Liu, WSB Mod, 0xRooter |

| Team member | Role | Link |
|---|---|---|
| Jason Li | Co-Founder, Head of Engineering | [X](https://x.com/lxjhk) |
| Rachel Chu | Co-Founder | [X](https://x.com/0xchu) |
| Joshua Sum | Co-Founder | [X](https://x.com/joshuasum_eth) |
...
Solayer competes in the staking infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Solayer has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Solayer is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Solayer compounds usage, liquidity improves, and LAYER gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Solayer has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If LAYER does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $12.15M | Weekly | Volume below position-size threshold |
| FDV / market cap | 4.76x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Solayer announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Solayer has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Solayer, the first reconciliation job is identity. The token symbol is LAYER, the category is staking infrastructure, chains are Solana, exchanges are BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BYBIT, BITGET, and tags are Layer1, LSD & Restaking. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If LAYER trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $14.09M, FDV is $67.11M, 24h volume is $12.15M, FDV / market cap is 4.76x, 7d price change is 0.54%, and 30d price change is 7.64%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Solayer / LAYER | Match official docs, CG, CMC, Surf, contracts |
...
For Solayer, the current liquidity read is watchlist first. The market cap is $14.09M, FDV is $67.11M, reported 24h volume is $12.15M, and FDV / market cap is 4.76x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Solayer gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://solayer.org/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Solayer) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Solayer) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Solayer) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Solayer) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Solayer) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/solayer_labs) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/solayer-labs) | Identity, market, social, contract, or discovery cross-check |
| 9 | Discord | [link](https://discord.com/invite/solayer-labs) | Identity, market, social, contract, or discovery cross-check |
| 10 | Solayer Price Chart (LAYER) - CoinGecko | [link](https://www.coingecko.com/en/coins/solayer) | Identity, market, social, contract, or discovery cross-check |
| 11 | Tokenomics - Solayer Docs V2 | [link](https://docs.solayer.org/documentation/Tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 12 | LAYER to INR: Solayer Price in Indian Rupee - CoinGecko | [link](https://www.coingecko.com/en/coins/solayer/inr) | Identity, market, social, contract, or discovery cross-check |
| 13 | What Is Solayer (LAYER) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/solayer/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 14 | Solayer (LAYER) / Tokenomics & Vesting Schedule - Token Unlocks | [link](https://tokenomist.ai/solayer) | Identity, market, social, contract, or discovery cross-check |
| 15 | solana LAYER | [link](https://solscan.io/token/LAYER4xPpTCb3QL8S9u41EAhAX7mhBn8Q6xMTwY2Yzc) | Identity, market, social, contract, or discovery cross-check |
| 16 | solana SUSD | [link](https://solscan.io/token/susdabGDNbhrnCa6ncrYo81u4s9GM8ecK2UwMyZiq4X) | Identity, market, social, contract, or discovery cross-check |
| 17 | solana SSOL | [link](https://solscan.io/token/sSo14endRuUbvQaJS3dq36Q829a3A6BEfoeeRGJywEh) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google project search | [link](https://www.google.com/search?q=Solayer%20LAYER%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google tokenomics search | [link](https://www.google.com/search?q=Solayer%20LAYER%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google docs search | [link](https://www.google.com/search?q=Solayer%20docs) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google governance search | [link](https://www.google.com/search?q=Solayer%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google audit search | [link](https://www.google.com/search?q=Solayer%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/solayer/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/solayer/) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Solayer) | Identity, market, social, contract, or discovery cross-check |
| 26 | Dune token query | [link](https://dune.com/browse/queries?q=Solayer) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Solayer) | Identity, market, social, contract, or discovery cross-check |
| 28 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=LAYER) | Identity, market, social, contract, or discovery cross-check |
| 29 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Solayer) | Identity, market, social, contract, or discovery cross-check |
| 30 | CryptoRank search | [link](https://cryptorank.io/price/solayer) | Identity, market, social, contract, or discovery cross-check |
| 31 | RootData search | [link](https://www.rootdata.com/search?kw=Solayer) | Identity, market, social, contract, or discovery cross-check |
| 32 | CoinDesk search | [link](https://www.coindesk.com/search?s=Solayer) | Identity, market, social, contract, or discovery cross-check |
| 33 | The Block search | [link](https://www.theblock.co/search?query=Solayer) | Identity, market, social, contract, or discovery cross-check |
| 34 | GitHub search | [link](https://github.com/search?q=Solayer&type=repositories) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://solayer.org/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Solayer)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Solayer)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Solayer)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Solayer)
- [Messari discovery](https://messari.io/search?q=Solayer)
- [X profile](https://x.com/solayer_labs)
- [GitHub](https://github.com/solayer-labs)
- [Discord](https://discord.com/invite/solayer-labs)
- [Solayer Price Chart (LAYER) - CoinGecko](https://www.coingecko.com/en/coins/solayer)
- [Tokenomics - Solayer Docs V2](https://docs.solayer.org/documentation/Tokenomics)
- [LAYER to INR: Solayer Price in Indian Rupee - CoinGecko](https://www.coingecko.com/en/coins/solayer/inr)
- [What Is Solayer (LAYER) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/solayer/what-is/)
- [Solayer (LAYER) | Tokenomics & Vesting Schedule - Token Unlocks](https://tokenomist.ai/solayer)
- [solana LAYER](https://solscan.io/token/LAYER4xPpTCb3QL8S9u41EAhAX7mhBn8Q6xMTwY2Yzc)
- [solana SUSD](https://solscan.io/token/susdabGDNbhrnCa6ncrYo81u4s9GM8ecK2UwMyZiq4X)
- [solana SSOL](https://solscan.io/token/sSo14endRuUbvQaJS3dq36Q829a3A6BEfoeeRGJywEh)
- [Google project search](https://www.google.com/search?q=Solayer%20LAYER%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Solayer%20LAYER%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Solayer%20docs)
- [Google governance search](https://www.google.com/search?q=Solayer%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Solayer%20audit%20report)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/solayer/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/solayer/)

## Final Investment View

Solayer should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
