---
title: 'Fragmetric / FRAG: staking infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Fragmetric is treated as a staking infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'fragmetric-frag-staking-infrastructure-token-value-capture-risk'
category: 'research'
---

# Fragmetric / FRAG: staking infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Fragmetric appears in the local CoinGecko/CMC-oriented candidate pool through [CoinGecko](https://www.coingecko.com/en/coins/fragmetric) and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Solana; exchange exposure is BYBIT, BITGET; category tags are LSD & Restaking, Yield & Asset Mgmt; current market cap is $2.59M; FDV is $2.59M; and 24h volume is $1.47K. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Fragmetric before this file was created, and the existing candidate note says: DefiLlama category Liquid Restaking on Solana; TVL $10,540,554. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Fragmetric is best understood as a **staking infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether FRAG captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| FDV | $2.59M |
| 24h volume | $1.47K |
| Circulating supply | 28.68K |
| Total supply | 28.68K |
| 7d price change | 10.39% |
| 30d price change | 29.43% |
...
Base case: Fragmetric remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning FRAG into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Fragmetric is a Solana-native liquid restaking protocol and asset-management stack that issues fragAssets (e.g., fragSOL) via its FRAG-22 token standard to normalize LSTs and enable seamless restaking into additional yield sources. It launched the FRAG governance token for staking and governance and integrates with Jito and ecosystem partners for precision multi-reward distribution.

Surf's project profile maps Fragmetric to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://fragmetric.xyz/](https://fragmetric.xyz/) |
| X handle | [@fragmetric](https://x.com/fragmetric) |
| X followers | 51.57K |
| Tags | LSD & Restaking, Yield & Asset Mgmt |
| Chains | Solana |
| Exchanges | BYBIT, BITGET |
| TGE status | post |
| CoinGecko ID | [fragmetric](https://www.coingecko.com/en/coins/fragmetric) |

The main product question is whether Fragmetric is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For staking infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Fragmetric convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $1.47K 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 28.68K circulating vs 28.68K total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Fragmetric is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| solana | WFRAGSOL | WFRGSWjaz8tbAxsJitmbfRuFV2mSNwy7BMWcCwaA28U | [explorer](https://solscan.io/token/WFRGSWjaz8tbAxsJitmbfRuFV2mSNwy7BMWcCwaA28U) |
| solana | FRAG | FRAGMEWj2z65qM62zqKhNtwNFskdfKs4ekDUDX3b4VD5 | [explorer](https://solscan.io/token/FRAGMEWj2z65qM62zqKhNtwNFskdfKs4ekDUDX3b4VD5) |
| solana | WFRAGJTO | WFRGJnQt5pK8Dv4cDAbrSsgPcmboysrmX3RYhmRRyTR | [explorer](https://solscan.io/token/WFRGJnQt5pK8Dv4cDAbrSsgPcmboysrmX3RYhmRRyTR) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Fragmetric, the DeFi-oriented backlog note is the first signal: DefiLlama category Liquid Restaking on Solana; TVL $10,540,554. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $2.59M, FDV $2.59M, 24h volume $1.47K, and price $90.2500.
...
| 24h volume | $1.47K | Position sizing and exit liquidity proxy |
| Market cap | $2.59M | Liquid valuation proxy |
| FDV | $2.59M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.00x | Higher spread means more supply risk |
| X followers | 51.57K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Fragmetric | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether FRAG is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Fragmetric currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 10.39% and 30d 29.43% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Fragmetric's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-06-27 | Ido | $360.00K | not disclosed |
| 2025-04-03 | Ico | $1.00M | not disclosed |
| 2025-03-24 | Strategic | $5.00M | Hypersphere Ventures, Robot Ventures, Amber Group, RockawayX, BitGo |
| 2025-02-07 | Seed | $7.00M | Hashed, Bitscale Capital, Hypersphere Ventures, Finality Capital Partners, Presto Labs, Flowdesk, Halo Capital, Legends Group |
| 2024-11-04 | Angel | not disclosed | Lily Liu, Anatoly Yakovenko, Richard Ma, Yoann Turpin, James Zhang, Alan Curtis, Chris Hermida, Robert Chen |

| Team member | Role | Link |
|---|---|---|
| SolZac | Co-Founder, CEO | [X](https://x.com/SolZacque) |
| Sang Kim | Co-Founder, COO | [X](https://x.com/sangdotsol) |
| Daniel | Co-Founder, CTO | [X](https://x.com/daniel_ssol) |
...
Fragmetric competes in the staking infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Fragmetric has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Fragmetric is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Fragmetric compounds usage, liquidity improves, and FRAG gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If FRAG does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $1.47K | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Fragmetric announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Fragmetric has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Fragmetric, the first reconciliation job is identity. The token symbol is FRAG, the category is staking infrastructure, chains are Solana, exchanges are BYBIT, BITGET, and tags are LSD & Restaking, Yield & Asset Mgmt. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If FRAG trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $2.59M, FDV is $2.59M, 24h volume is $1.47K, FDV / market cap is 1.00x, 7d price change is 10.39%, and 30d price change is 29.43%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Fragmetric / FRAG | Match official docs, CG, CMC, Surf, contracts |
...
| Exchanges | BYBIT, BITGET | Check live order books and inactive pairs |
| Market cap | $2.59M | Compare Surf, CG, CMC, exchange pages |
| FDV | $2.59M | Reconcile total supply and unlock schedule |
| Volume | $1.47K | Check exchange concentration and wash-trading risk |
| TVL | not disclosed | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Fragmetric does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Fragmetric, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does FRAG sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
For Fragmetric, the current liquidity read is watchlist first. The market cap is $2.59M, FDV is $2.59M, reported 24h volume is $1.47K, and FDV / market cap is 1.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. Fragmetric can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.

For Fragmetric, the live monitoring stack should include contract events, TVL, volume, governance, documentation changes, social announcements, security disclosures, and exchange-market quality. A single metric is not enough. TVL may stay high while liquidity thins. Volume may rise because of volatility rather than adoption. Social attention may rise around controversy rather than progress. Governance may pass proposals that improve short-term price but add long-term risk. The strongest monitoring dashboard combines on-chain state, off-chain disclosure, and market quality.

Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If Fragmetric suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | Fragmetric gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://fragmetric.xyz/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko profile | [link](https://www.coingecko.com/en/coins/fragmetric) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Fragmetric) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Fragmetric) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Fragmetric) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Fragmetric) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/fragmetric) | Identity, market, social, contract, or discovery cross-check |
| 8 | Discord | [link](https://discord.com/invite/fragmetric) | Identity, market, social, contract, or discovery cross-check |
| 9 | Fragmetric Price / FRAG Price Today, Live Chart, USD converter ... | [link](https://cryptorank.io/price/fragmetric) | Identity, market, social, contract, or discovery cross-check |
| 10 | FRAG to BTC: Fragmetric Price in Bitcoin - CoinGecko | [link](https://www.coingecko.com/en/coins/fragmetric/btc) | Identity, market, social, contract, or discovery cross-check |
| 11 | Fragmetric Price History: Download FRAG Historical Data / CoinGecko | [link](https://www.coingecko.com/en/coins/fragmetric/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 12 | solana WFRAGSOL | [link](https://solscan.io/token/WFRGSWjaz8tbAxsJitmbfRuFV2mSNwy7BMWcCwaA28U) | Identity, market, social, contract, or discovery cross-check |
| 13 | solana FRAG | [link](https://solscan.io/token/FRAGMEWj2z65qM62zqKhNtwNFskdfKs4ekDUDX3b4VD5) | Identity, market, social, contract, or discovery cross-check |
| 14 | solana WFRAGJTO | [link](https://solscan.io/token/WFRGJnQt5pK8Dv4cDAbrSsgPcmboysrmX3RYhmRRyTR) | Identity, market, social, contract, or discovery cross-check |
| 15 | Google project search | [link](https://www.google.com/search?q=Fragmetric%20FRAG%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google tokenomics search | [link](https://www.google.com/search?q=Fragmetric%20FRAG%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google docs search | [link](https://www.google.com/search?q=Fragmetric%20docs) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google governance search | [link](https://www.google.com/search?q=Fragmetric%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google audit search | [link](https://www.google.com/search?q=Fragmetric%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 20 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/fragmetric/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/fragmetric/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/fragmetric/) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Fragmetric) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune token query | [link](https://dune.com/browse/queries?q=Fragmetric) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Fragmetric) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=FRAG) | Identity, market, social, contract, or discovery cross-check |
| 27 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Fragmetric) | Identity, market, social, contract, or discovery cross-check |
| 28 | RootData search | [link](https://www.rootdata.com/search?kw=Fragmetric) | Identity, market, social, contract, or discovery cross-check |
| 29 | CoinDesk search | [link](https://www.coindesk.com/search?s=Fragmetric) | Identity, market, social, contract, or discovery cross-check |
| 30 | The Block search | [link](https://www.theblock.co/search?query=Fragmetric) | Identity, market, social, contract, or discovery cross-check |
| 31 | GitHub search | [link](https://github.com/search?q=Fragmetric&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 32 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=FRAG) | Identity, market, social, contract, or discovery cross-check |
| 33 | BscScan token search | [link](https://bscscan.com/search?f=0&q=FRAG) | Identity, market, social, contract, or discovery cross-check |
| 34 | Arbiscan token search | [link](https://arbiscan.io/search?f=0&q=FRAG) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://fragmetric.xyz/)
- [CoinGecko profile](https://www.coingecko.com/en/coins/fragmetric)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Fragmetric)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Fragmetric)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Fragmetric)
- [Messari discovery](https://messari.io/search?q=Fragmetric)
- [X profile](https://x.com/fragmetric)
- [Discord](https://discord.com/invite/fragmetric)
- [Fragmetric Price | FRAG Price Today, Live Chart, USD converter ...](https://cryptorank.io/price/fragmetric)
- [FRAG to BTC: Fragmetric Price in Bitcoin - CoinGecko](https://www.coingecko.com/en/coins/fragmetric/btc)
- [Fragmetric Price History: Download FRAG Historical Data | CoinGecko](https://www.coingecko.com/en/coins/fragmetric/historical_data)
- [solana WFRAGSOL](https://solscan.io/token/WFRGSWjaz8tbAxsJitmbfRuFV2mSNwy7BMWcCwaA28U)
- [solana FRAG](https://solscan.io/token/FRAGMEWj2z65qM62zqKhNtwNFskdfKs4ekDUDX3b4VD5)
- [solana WFRAGJTO](https://solscan.io/token/WFRGJnQt5pK8Dv4cDAbrSsgPcmboysrmX3RYhmRRyTR)
- [Google project search](https://www.google.com/search?q=Fragmetric%20FRAG%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Fragmetric%20FRAG%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Fragmetric%20docs)
- [Google governance search](https://www.google.com/search?q=Fragmetric%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Fragmetric%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/fragmetric/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/fragmetric/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/fragmetric/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Fragmetric)
- [Dune token query](https://dune.com/browse/queries?q=Fragmetric)
