---
title: 'FOLKS / FOLKS: lending market value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'FOLKS is treated as a lending market watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'folks-folks-lending-market-token-value-capture-risk'
category: 'research'
---

# FOLKS / FOLKS: lending market value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** FOLKS appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Algorand, Arbitrum One, Avalanche, Base, Binance Smart Chain, Ethereum, Monad, Polygon Pos, Sei V2; exchange exposure is BINANCE ALPHA; category tags are Cross-Chain & Bridge, Lending & Borrowing; current market cap is $34.81M; FDV is $104.24M; and 24h volume is $3.08M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag FOLKS before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 578, market cap $34,461,329, 24h volume $2,182,469. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

FOLKS is best understood as a **lending market** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether FOLKS captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Algorand, Arbitrum One, Avalanche, Base, Binance Smart Chain, Ethereum, Monad, Polygon Pos, Sei V2 |
| Exchanges | BINANCE ALPHA |
| Price | $2.0900 |
| Market cap | $34.81M |
| FDV | $104.24M |
| 24h volume | $3.08M |
| Circulating supply | 16.70M |
| Total supply | 50.00M |
| 7d price change | 0.72% |
| 30d price change | 44.52% |
...
Base case: FOLKS remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning FOLKS into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Folks Finance is a community-driven, non-custodial DeFi protocol that provides cross-chain lending, borrowing, liquid staking (xALGO), a DEX router, and developer SDKs. Launched on Algorand and expanded into a multichain xChain hub, it offers unified liquidity, audited smart contracts, and a documented governance/tokenomics framework (FOLKS).

Surf's project profile maps FOLKS to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://folks.finance/](https://folks.finance/) |
| X handle | [@FolksFinance](https://x.com/FolksFinance) |
| X followers | 132.30K |
| Tags | Cross-Chain & Bridge, Lending & Borrowing |
| Chains | Algorand, Arbitrum One, Avalanche, Base, Binance Smart Chain, Ethereum, Monad, Polygon Pos, Sei V2 |
| Exchanges | BINANCE ALPHA |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether FOLKS is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For lending market projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does FOLKS convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $3.08M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 16.70M circulating vs 50.00M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that FOLKS is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

FOLKS's architecture should be evaluated through the job it performs. For a lending market project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
| Settlement layer | Chains and contracts that hold user value | Algorand, Arbitrum One, Avalanche, Base, Binance Smart Chain, Ethereum, Monad, Polygon Pos, Sei V2 |
...
|---|---|---|---|
| polygon | FOLKS | 0xff7f8f301f7a706e3cfd3d2275f5dc0b9ee8009b | [explorer](https://polygonscan.com/token/0xff7f8f301f7a706e3cfd3d2275f5dc0b9ee8009b) |
| arbitrum | FOLKS | 0xff7f8f301f7a706e3cfd3d2275f5dc0b9ee8009b | [explorer](https://arbiscan.io/token/0xff7f8f301f7a706e3cfd3d2275f5dc0b9ee8009b) |
| bsc | FOLKS | 0xff7f8f301f7a706e3cfd3d2275f5dc0b9ee8009b | [explorer](https://bscscan.com/token/0xff7f8f301f7a706e3cfd3d2275f5dc0b9ee8009b) |
| Algorand | FOLKS | 3203964481 | not mapped |
| base | FOLKS | 0xff7f8f301f7a706e3cfd3d2275f5dc0b9ee8009b | [explorer](https://basescan.org/token/0xff7f8f301f7a706e3cfd3d2275f5dc0b9ee8009b) |
| Monad | FOLKS | 0xff7f8f301f7a706e3cfd3d2275f5dc0b9ee8009b | not mapped |
| Sei V2 | FOLKS | 0xff7f8f301f7a706e3cfd3d2275f5dc0b9ee8009b | not mapped |
| ethereum | FOLKS | 0xff7f8f301f7a706e3cfd3d2275f5dc0b9ee8009b | [explorer](https://etherscan.io/token/0xff7f8f301f7a706e3cfd3d2275f5dc0b9ee8009b) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For FOLKS, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 578, market cap $34,461,329, 24h volume $2,182,469. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $34.81M, FDV $104.24M, 24h volume $3.08M, and price $2.0900.
...
| 24h volume | $3.08M | Position sizing and exit liquidity proxy |
| Market cap | $34.81M | Liquid valuation proxy |
| FDV | $104.24M | Dilution-aware valuation proxy |
| FDV / market cap spread | 2.99x | Higher spread means more supply risk |
| X followers | 132.30K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for FOLKS | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether FOLKS is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. FOLKS should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that FOLKS belongs to the lending market bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $34.81M, FDV $104.24M, 24h volume $3.08M, price $2.0900, circulating supply 16.70M, and total supply 50.00M. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For FOLKS, the candidate backlog records Seeded 2026-07-06 from Surf market-ranking; rank 578, market cap $34,461,329, 24h volume $2,182,469. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Algorand, Arbitrum One, Avalanche, Base, Binance Smart Chain, Ethereum, Monad, Polygon Pos, Sei V2, exchanges as BINANCE ALPHA, and tags as Cross-Chain & Bridge, Lending & Borrowing. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.

The fourth frame is value capture. A lending market asset can be useful while the token remains weak. The memo therefore separates product-market fit from token-market fit. Product-market fit asks whether users need the protocol even when incentives fade. Token-market fit asks whether the token absorbs value through staking, fee routing, collateral, governance constraints, emissions control, slashing insurance, or privileged access. If the token is mostly governance, the base case assigns lower confidence. If the token is used for security or fee capture but the mechanism is not yet activated, the base case treats value capture as an option rather than as current cash flow.

The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. FOLKS currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 0.72% and 30d 44.52% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that FOLKS's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2024-11-14 | Series A | $3.20M | Dutch Crypto Investors, Titans Ventures, Borderless Capital, Mapleblock Capital, Algorand Foundation, Roham Gharegozlou, Stakez Capital, DuckDAO |
| 2022-01-20 | Seed | $3.00M | ZBS CAPITAL, Coinbase Ventures, OKX Ventures, Youbi Capital, 0xVentures, ParaFi Capital, Borderless Capital, Valhalla Capital |

| Team member | Role | Link |
|---|---|---|
| Benedetto Biondi | Founder, CEO | [X](https://x.com/benedettobio) |
| Zeno Auersperg | Partner, Head of DeFi | [X](https://x.com/ZenzaF1) |
| Roman Klymonchuk | Head of Partnerships | [X](https://x.com/remp1re) |
| Ibu Karel | CMO | [X](https://x.com/ibuprod) |
...
FOLKS competes in the lending market bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether FOLKS has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: FOLKS is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | FOLKS compounds usage, liquidity improves, and FOLKS gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because FOLKS has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If FOLKS does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $3.08M | Weekly | Volume below position-size threshold |
| FDV / market cap | 2.99x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if FOLKS announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. FOLKS has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For FOLKS, the first reconciliation job is identity. The token symbol is FOLKS, the category is lending market, chains are Algorand, Arbitrum One, Avalanche, Base, Binance Smart Chain, Ethereum, Monad, Polygon Pos, Sei V2, exchanges are BINANCE ALPHA, and tags are Cross-Chain & Bridge, Lending & Borrowing. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If FOLKS trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $34.81M, FDV is $104.24M, 24h volume is $3.08M, FDV / market cap is 2.99x, 7d price change is 0.72%, and 30d price change is 44.52%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.

The third reconciliation job is usage data. The latest TVL sample is not disclosed, TVL sample change is not disclosed, and the sample contains 0 point(s). TVL is useful only when the underlying balances are real, withdrawable, and not double-counted. For lending market, the memo should identify whether TVL means deposits, staked assets, LP liquidity, stablecoin supply, collateral, locked governance assets, or a derived wrapper. A yield-bearing stablecoin and a validator middleware protocol can both show TVL, but the risk encoded by that TVL is completely different.

| Reconciliation item | Current value | Next check |
|---|---|---|
| Identity | FOLKS / FOLKS | Match official docs, CG, CMC, Surf, contracts |
| Category | lending market | Confirm product mechanics and revenue model |
| Chains | Algorand, Arbitrum One, Avalanche, Base, Binance Smart Chain, Ethereum, Monad, Polygon Pos, Sei V2 | Confirm canonical deployments and bridge assumptions |
| Exchanges | BINANCE ALPHA | Check live order books and inactive pairs |
| Market cap | $34.81M | Compare Surf, CG, CMC, exchange pages |
| FDV | $104.24M | Reconcile total supply and unlock schedule |
| Volume | $3.08M | Check exchange concentration and wash-trading risk |
| TVL | not disclosed | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what FOLKS does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For FOLKS, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does FOLKS sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
| Correlation check | Count exposure to related lending market assets | Category beta can hide concentration |
...
| Peer comparison | Monthly | FOLKS gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://folks.finance/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=FOLKS) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=FOLKS) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=FOLKS) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=FOLKS) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=FOLKS) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/FolksFinance) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/Folks-Finance) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/FolksfinanceOfficial) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](https://discord.gg/folksfinance) | Identity, market, social, contract, or discovery cross-check |
| 11 | FOLKS/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/folks) | Identity, market, social, contract, or discovery cross-check |
| 12 | $folks tokenomics question : r/algorand | [link](https://www.reddit.com/r/algorand/comments/1o4d8nm/folks_tokenomics_question/) | Identity, market, social, contract, or discovery cross-check |
| 13 | What Is Tokenomics? A Complete Guide to Crypto Token ... | [link](https://www.coingecko.com/learn/what-is-tokenomics-understanding-crypto-fundamentals) | Identity, market, social, contract, or discovery cross-check |
| 14 | Tokenomics | [link](https://docs.folks.finance/folks-token/tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 15 | ConstitutionDAO Price Chart (PEOPLE) | [link](https://www.coingecko.com/en/coins/constitutiondao) | Identity, market, social, contract, or discovery cross-check |
| 16 | polygon FOLKS | [link](https://polygonscan.com/token/0xff7f8f301f7a706e3cfd3d2275f5dc0b9ee8009b) | Identity, market, social, contract, or discovery cross-check |
| 17 | arbitrum FOLKS | [link](https://arbiscan.io/token/0xff7f8f301f7a706e3cfd3d2275f5dc0b9ee8009b) | Identity, market, social, contract, or discovery cross-check |
| 18 | bsc FOLKS | [link](https://bscscan.com/token/0xff7f8f301f7a706e3cfd3d2275f5dc0b9ee8009b) | Identity, market, social, contract, or discovery cross-check |
| 19 | base FOLKS | [link](https://basescan.org/token/0xff7f8f301f7a706e3cfd3d2275f5dc0b9ee8009b) | Identity, market, social, contract, or discovery cross-check |
| 20 | ethereum FOLKS | [link](https://etherscan.io/token/0xff7f8f301f7a706e3cfd3d2275f5dc0b9ee8009b) | Identity, market, social, contract, or discovery cross-check |
| 21 | avalanche FOLKS | [link](https://snowtrace.io/token/0xff7f8f301f7a706e3cfd3d2275f5dc0b9ee8009b) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google project search | [link](https://www.google.com/search?q=FOLKS%20FOLKS%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 23 | Google tokenomics search | [link](https://www.google.com/search?q=FOLKS%20FOLKS%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 24 | Google docs search | [link](https://www.google.com/search?q=FOLKS%20docs) | Identity, market, social, contract, or discovery cross-check |
| 25 | Google governance search | [link](https://www.google.com/search?q=FOLKS%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 26 | Google audit search | [link](https://www.google.com/search?q=FOLKS%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 27 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/folks/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 28 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/folks/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 29 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/folks/) | Identity, market, social, contract, or discovery cross-check |
| 30 | Dune discovery | [link](https://dune.com/browse/dashboards?q=FOLKS) | Identity, market, social, contract, or discovery cross-check |
| 31 | Dune token query | [link](https://dune.com/browse/queries?q=FOLKS) | Identity, market, social, contract, or discovery cross-check |
| 32 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=FOLKS) | Identity, market, social, contract, or discovery cross-check |
| 33 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=FOLKS) | Identity, market, social, contract, or discovery cross-check |
| 34 | TokenUnlocks search | [link](https://token.unlocks.app/?search=FOLKS) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://folks.finance/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=FOLKS)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=FOLKS)
- [DefiLlama protocol discovery](https://defillama.com/search?query=FOLKS)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=FOLKS)
- [Messari discovery](https://messari.io/search?q=FOLKS)
- [X profile](https://x.com/FolksFinance)
- [GitHub](https://github.com/Folks-Finance)
- [Telegram](https://t.me/FolksfinanceOfficial)
- [Discord](https://discord.gg/folksfinance)
- [FOLKS/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/folks)
- [$folks tokenomics question : r/algorand](https://www.reddit.com/r/algorand/comments/1o4d8nm/folks_tokenomics_question/)
...
- [Tokenomics](https://docs.folks.finance/folks-token/tokenomics)
- [ConstitutionDAO Price Chart (PEOPLE)](https://www.coingecko.com/en/coins/constitutiondao)
- [polygon FOLKS](https://polygonscan.com/token/0xff7f8f301f7a706e3cfd3d2275f5dc0b9ee8009b)
- [arbitrum FOLKS](https://arbiscan.io/token/0xff7f8f301f7a706e3cfd3d2275f5dc0b9ee8009b)
- [bsc FOLKS](https://bscscan.com/token/0xff7f8f301f7a706e3cfd3d2275f5dc0b9ee8009b)
- [base FOLKS](https://basescan.org/token/0xff7f8f301f7a706e3cfd3d2275f5dc0b9ee8009b)
- [ethereum FOLKS](https://etherscan.io/token/0xff7f8f301f7a706e3cfd3d2275f5dc0b9ee8009b)
- [avalanche FOLKS](https://snowtrace.io/token/0xff7f8f301f7a706e3cfd3d2275f5dc0b9ee8009b)
- [Google project search](https://www.google.com/search?q=FOLKS%20FOLKS%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=FOLKS%20FOLKS%20tokenomics)
- [Google docs search](https://www.google.com/search?q=FOLKS%20docs)

## Final Investment View

FOLKS should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
