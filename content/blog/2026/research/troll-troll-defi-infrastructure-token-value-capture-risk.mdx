...
| FDV | $57.88M |
| 24h volume | $3.68M |
| Circulating supply | 998.77M |
| Total supply | 998.77M |
...
Base case: TROLL remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning TROLL into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

TROLL is a Solana-based (SPL) meme token inspired by internet trolling culture, designed for entertainment and community engagement. The project is community-driven, listed on DEXs and CEXs, and maintains an official website and active Twitter presence (@trololol_io).

Surf's project profile maps TROLL to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://trololol.io/](https://trololol.io/) |
| X handle | [@Troll_](https://x.com/Troll_) |
| X followers | 38.46K |
| Tags | Meme |
| Chains | Solana |
| Exchanges | COINBASE, BINANCE ALPHA |
...
| Supply quality | 998.77M circulating vs 998.77M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that TROLL is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| solana | TROLL | 5UUH9RTDiSpq6HKS6bp4NdU9PNJpXRXuiw6ShBTBhgH2 | [explorer](https://solscan.io/token/5UUH9RTDiSpq6HKS6bp4NdU9PNJpXRXuiw6ShBTBhgH2) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For TROLL, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 401, market cap $57,548,296, 24h volume $3,666,542. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $57.88M, FDV $57.88M, 24h volume $3.68M, and price $0.0580.
...
| Market cap | $57.88M | Liquid valuation proxy |
| FDV | $57.88M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.00x | Higher spread means more supply risk |
| X followers | 38.46K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for TROLL | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether TROLL is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. TROLL currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 20.45% and 30d -16.74% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that TROLL's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
TROLL competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether TROLL has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: TROLL is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | TROLL compounds usage, liquidity improves, and TROLL gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because TROLL has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If TROLL does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
1. Re-run Surf project-detail and project-defi-metrics if TROLL announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. TROLL has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For TROLL, the first reconciliation job is identity. The token symbol is TROLL, the category is DeFi infrastructure, chains are Solana, exchanges are COINBASE, BINANCE ALPHA, and tags are Meme. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If TROLL trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $57.88M, FDV is $57.88M, 24h volume is $3.68M, FDV / market cap is 1.00x, 7d price change is 20.45%, and 30d price change is -16.74%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | TROLL / TROLL | Match official docs, CG, CMC, Surf, contracts |
...
| Market cap | $57.88M | Compare Surf, CG, CMC, exchange pages |
| FDV | $57.88M | Reconcile total supply and unlock schedule |
...
For TROLL, the current liquidity read is watchlist first. The market cap is $57.88M, FDV is $57.88M, reported 24h volume is $3.68M, and FDV / market cap is 1.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | TROLL gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://trololol.io/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=TROLL) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=TROLL) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=TROLL) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=TROLL) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=TROLL) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/Troll_) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/trollsolog) | Identity, market, social, contract, or discovery cross-check |
| 9 | TROLL/USD Live Price Chart, Market Cap & News Today - CoinGecko | [link](https://www.coingecko.com/en/coins/troll-2) | Identity, market, social, contract, or discovery cross-check |
| 10 | TROLL/USD Live Price Chart, Market Cap & News Today - CoinGecko | [link](https://www.coingecko.com/en/coins/troll) | Identity, market, social, contract, or discovery cross-check |
| 11 | Troll Coin to USD Chart - CoinMarketCap | [link](https://coinmarketcap.com/currencies/troll-coin/) | Identity, market, social, contract, or discovery cross-check |
| 12 | Based Troll Price: TROLL/USD Live Price Chart, Market ... - CoinGecko | [link](https://www.coingecko.com/en/coins/based-troll) | Identity, market, social, contract, or discovery cross-check |
| 13 | TROLL (SOL) (TROLL) Price Prediction For 2026 & Beyond | [link](https://coinmarketcap.com/cmc-ai/troll-sol/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 14 | solana TROLL | [link](https://solscan.io/token/5UUH9RTDiSpq6HKS6bp4NdU9PNJpXRXuiw6ShBTBhgH2) | Identity, market, social, contract, or discovery cross-check |
| 15 | Google project search | [link](https://www.google.com/search?q=TROLL%20TROLL%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google tokenomics search | [link](https://www.google.com/search?q=TROLL%20TROLL%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google docs search | [link](https://www.google.com/search?q=TROLL%20docs) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google governance search | [link](https://www.google.com/search?q=TROLL%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google audit search | [link](https://www.google.com/search?q=TROLL%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 20 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/troll/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/troll/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/troll/) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune discovery | [link](https://dune.com/browse/dashboards?q=TROLL) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune token query | [link](https://dune.com/browse/queries?q=TROLL) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=TROLL) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=TROLL) | Identity, market, social, contract, or discovery cross-check |
| 27 | TokenUnlocks search | [link](https://token.unlocks.app/?search=TROLL) | Identity, market, social, contract, or discovery cross-check |
| 28 | CryptoRank search | [link](https://cryptorank.io/price/troll) | Identity, market, social, contract, or discovery cross-check |
| 29 | RootData search | [link](https://www.rootdata.com/search?kw=TROLL) | Identity, market, social, contract, or discovery cross-check |
| 30 | CoinDesk search | [link](https://www.coindesk.com/search?s=TROLL) | Identity, market, social, contract, or discovery cross-check |
| 31 | The Block search | [link](https://www.theblock.co/search?query=TROLL) | Identity, market, social, contract, or discovery cross-check |
| 32 | GitHub search | [link](https://github.com/search?q=TROLL&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 33 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=TROLL) | Identity, market, social, contract, or discovery cross-check |
| 34 | BscScan token search | [link](https://bscscan.com/search?f=0&q=TROLL) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://trololol.io/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=TROLL)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=TROLL)
- [DefiLlama protocol discovery](https://defillama.com/search?query=TROLL)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=TROLL)
- [Messari discovery](https://messari.io/search?q=TROLL)
- [X profile](https://x.com/Troll_)
- [Telegram](https://t.me/trollsolog)
- [TROLL/USD Live Price Chart, Market Cap & News Today - CoinGecko](https://www.coingecko.com/en/coins/troll-2)
- [TROLL/USD Live Price Chart, Market Cap & News Today - CoinGecko](https://www.coingecko.com/en/coins/troll)
- [Troll Coin to USD Chart - CoinMarketCap](https://coinmarketcap.com/currencies/troll-coin/)
- [Based Troll Price: TROLL/USD Live Price Chart, Market ... - CoinGecko](https://www.coingecko.com/en/coins/based-troll)
- [TROLL (SOL) (TROLL) Price Prediction For 2026 & Beyond](https://coinmarketcap.com/cmc-ai/troll-sol/price-prediction/)
- [solana TROLL](https://solscan.io/token/5UUH9RTDiSpq6HKS6bp4NdU9PNJpXRXuiw6ShBTBhgH2)
- [Google project search](https://www.google.com/search?q=TROLL%20TROLL%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=TROLL%20TROLL%20tokenomics)
- [Google docs search](https://www.google.com/search?q=TROLL%20docs)
- [Google governance search](https://www.google.com/search?q=TROLL%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=TROLL%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/troll/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/troll/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/troll/)
- [Dune discovery](https://dune.com/browse/dashboards?q=TROLL)
- [Dune token query](https://dune.com/browse/queries?q=TROLL)

## Final Investment View

TROLL should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
