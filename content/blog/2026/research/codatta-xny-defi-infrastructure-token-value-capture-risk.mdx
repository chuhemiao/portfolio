---
title: 'Codatta / XNY: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Codatta is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'codatta-xny-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# Codatta / XNY: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Codatta appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Binance Smart Chain; exchange exposure is BINANCE ALPHA; category tags are Data & Analytics, DeSci (Decentralized Science), Web3 AI (Agent/AIGC); current market cap is $16.72M; FDV is $66.86M; and 24h volume is $2.11M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Codatta before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 910, market cap $17,053,294, 24h volume $1,764,620. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Codatta is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether XNY captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
Base case: Codatta remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning XNY into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Codatta is an open, decentralized multi-chain protocol that converts expert human knowledge into blockchain-based, royalty-bearing data assets used to power AI. A permissionless contributor network (providers, verifiers, backers) curates and verifies data with staking, on-chain commitments and hybrid private storage, and its native token XNY funds rewards and economic security.

Surf's project profile maps Codatta to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://codatta.io/](https://codatta.io/) |
| X handle | [@codatta_io](https://x.com/codatta_io) |
| X followers | 227.93K |
| Tags | Data & Analytics, DeSci (Decentralized Science), Web3 AI (Agent/AIGC) |
| Chains | Binance Smart Chain |
| Exchanges | BINANCE ALPHA |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Codatta is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DeFi infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Codatta convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $2.11M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
...
The strongest reason to add the report is not that Codatta is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| bsc | XNY | 0xe3225e11cab122f1a126a28997788e5230838ab9 | [explorer](https://bscscan.com/token/0xe3225e11cab122f1a126a28997788e5230838ab9) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Codatta, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 910, market cap $17,053,294, 24h volume $1,764,620. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $16.72M, FDV $66.86M, 24h volume $2.11M, and price $0.0067.
...
| 24h volume | $2.11M | Position sizing and exit liquidity proxy |
| Market cap | $16.72M | Liquid valuation proxy |
| FDV | $66.86M | Dilution-aware valuation proxy |
| FDV / market cap spread | 4.00x | Higher spread means more supply risk |
| X followers | 227.93K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Codatta | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether XNY is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Codatta currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 34.31% and 30d 37.29% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Codatta's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2024-06-12 | Seed | $2.50M | OKX Ventures, Mask Network, CGV FoF, CatcherVC, Comma3 Ventures, GoPlus Security, Oak Grove Ventures, Paramita Venture |

| Team member | Role | Link |
|---|---|---|
| Yi Zhang | Co-Founder, CEO | [X](https://x.com/drtwo101) |
| Kevin Wang | Co-Founder, CPO | [X](https://x.com/qiw) |
| Paul Pang | Co-Founder | not disclosed |
...
Codatta competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Codatta has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Codatta is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Codatta compounds usage, liquidity improves, and XNY gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Codatta has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If XNY does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $2.11M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Codatta announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Codatta has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Codatta, the first reconciliation job is identity. The token symbol is XNY, the category is DeFi infrastructure, chains are Binance Smart Chain, exchanges are BINANCE ALPHA, and tags are Data & Analytics, DeSci (Decentralized Science), Web3 AI (Agent/AIGC). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If XNY trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $16.72M, FDV is $66.86M, 24h volume is $2.11M, FDV / market cap is 4.00x, 7d price change is 34.31%, and 30d price change is 37.29%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Codatta / XNY | Match official docs, CG, CMC, Surf, contracts |
...
For Codatta, the current liquidity read is watchlist first. The market cap is $16.72M, FDV is $66.86M, reported 24h volume is $2.11M, and FDV / market cap is 4.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Codatta gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://codatta.io/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Codatta) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Codatta) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Codatta) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Codatta) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Codatta) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/codatta_io) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/codatta/) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/codatta_io) | Identity, market, social, contract, or discovery cross-check |
| 10 | Codatta Price: XNY/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/codatta) | Identity, market, social, contract, or discovery cross-check |
| 11 | Codatta price today, XNY to USD live price, marketcap and chart | [link](https://coinmarketcap.com/currencies/codatta/) | Identity, market, social, contract, or discovery cross-check |
| 12 | What Is Codatta (XNY) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/codatta/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 13 | Codatta (XNY) Price Prediction For 2026 & Beyond - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/codatta/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
...
| 15 | bsc XNY | [link](https://bscscan.com/token/0xe3225e11cab122f1a126a28997788e5230838ab9) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google project search | [link](https://www.google.com/search?q=Codatta%20XNY%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google tokenomics search | [link](https://www.google.com/search?q=Codatta%20XNY%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google docs search | [link](https://www.google.com/search?q=Codatta%20docs) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google governance search | [link](https://www.google.com/search?q=Codatta%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google audit search | [link](https://www.google.com/search?q=Codatta%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/codatta/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 22 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Codatta) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune token query | [link](https://dune.com/browse/queries?q=Codatta) | Identity, market, social, contract, or discovery cross-check |
| 24 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Codatta) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=XNY) | Identity, market, social, contract, or discovery cross-check |
| 26 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Codatta) | Identity, market, social, contract, or discovery cross-check |
| 27 | CryptoRank search | [link](https://cryptorank.io/price/codatta) | Identity, market, social, contract, or discovery cross-check |
| 28 | RootData search | [link](https://www.rootdata.com/search?kw=Codatta) | Identity, market, social, contract, or discovery cross-check |
| 29 | CoinDesk search | [link](https://www.coindesk.com/search?s=Codatta) | Identity, market, social, contract, or discovery cross-check |
| 30 | The Block search | [link](https://www.theblock.co/search?query=Codatta) | Identity, market, social, contract, or discovery cross-check |
| 31 | GitHub search | [link](https://github.com/search?q=Codatta&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 32 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=XNY) | Identity, market, social, contract, or discovery cross-check |
| 33 | BscScan token search | [link](https://bscscan.com/search?f=0&q=XNY) | Identity, market, social, contract, or discovery cross-check |
| 34 | Arbiscan token search | [link](https://arbiscan.io/search?f=0&q=XNY) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://codatta.io/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Codatta)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Codatta)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Codatta)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Codatta)
- [Messari discovery](https://messari.io/search?q=Codatta)
- [X profile](https://x.com/codatta_io)
- [GitHub](https://github.com/codatta/)
- [Telegram](https://t.me/codatta_io)
- [Codatta Price: XNY/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/codatta)
- [Codatta price today, XNY to USD live price, marketcap and chart](https://coinmarketcap.com/currencies/codatta/)
- [What Is Codatta (XNY) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/codatta/what-is/)
- [Codatta (XNY) Price Prediction For 2026 & Beyond - CoinMarketCap](https://coinmarketcap.com/cmc-ai/codatta/price-prediction/)
- [CoinGecko: Cryptocurrency Prices, Charts, and Crypto Market Cap](https://www.coingecko.com/)
- [bsc XNY](https://bscscan.com/token/0xe3225e11cab122f1a126a28997788e5230838ab9)
- [Google project search](https://www.google.com/search?q=Codatta%20XNY%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Codatta%20XNY%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Codatta%20docs)
- [Google governance search](https://www.google.com/search?q=Codatta%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Codatta%20audit%20report)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/codatta/price-analysis/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Codatta)
- [Dune token query](https://dune.com/browse/queries?q=Codatta)
- [DeFiLlama fees discovery](https://defillama.com/fees?chain=All&search=Codatta)

## Final Investment View

Codatta should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
