---
title: 'Plume / PLUME: yield infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Plume is treated as a yield infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'plume-plume-yield-infrastructure-token-value-capture-risk'
category: 'research'
---

# Plume / PLUME: yield infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Plume appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Binance Smart Chain, Ethereum, Plume Network; exchange exposure is BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET; category tags are Layer1, RWA (Real World Assets), Yield & Asset Mgmt; current market cap is $64.28M; FDV is $107.68M; and 24h volume is $8.88M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Plume before this file was created, and the existing candidate note says: Seeded 2026-06-28 from Surf market-ranking; rank 391, market cap $57,060,394, 24h volume $3,510,432. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Plume is best understood as a **yield infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether PLUME captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Binance Smart Chain, Ethereum, Plume Network |
| Exchanges | BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET |
| Price | $0.0108 |
| Market cap | $64.28M |
| FDV | $107.68M |
| 24h volume | $8.88M |
| Circulating supply | 5.97B |
| Total supply | 10.00B |
...
Base case: Plume remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning PLUME into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Plume Network is an EVM-compatible Layer‑1 blockchain focused on tokenizing real‑world assets and enabling RWA-native DeFi. It offers a vertically integrated stack (tokenization engine, compliance and data tooling, pUSD stablecoin) and developer tools for issuing, trading, lending, and composing on‑chain RWAs.

Surf's project profile maps Plume to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://plume.org/](https://plume.org/) |
| X handle | [@plumenetwork](https://x.com/plumenetwork) |
| X followers | 949.33K |
...
| Liquidity | $8.88M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 5.97B circulating vs 10.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Plume is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Binance Smart Chain, Ethereum, Plume Network |
...
|---|---|---|---|
| ethereum | PLUME | 0x4c1746a800d224393fe2470c70a35717ed4ea5f1 | [explorer](https://etherscan.io/token/0x4c1746a800d224393fe2470c70a35717ed4ea5f1) |
| bsc | PLUME | 0x5afadcd1e8e3ca78ee2d37100102f2aec8bc0aa8 | [explorer](https://bscscan.com/token/0x5afadcd1e8e3ca78ee2d37100102f2aec8bc0aa8) |
| ethereum | PUSD | 0xdddd73f5df1f0dc31373357beac77545dc5a6f3f | [explorer](https://etherscan.io/token/0xdddd73f5df1f0dc31373357beac77545dc5a6f3f) |
| Plume Network | PUSD | 0xdddd73f5df1f0dc31373357beac77545dc5a6f3f | not mapped |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Plume, the DeFi-oriented backlog note is the first signal: Seeded 2026-06-28 from Surf market-ranking; rank 391, market cap $57,060,394, 24h volume $3,510,432. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $64.28M, FDV $107.68M, 24h volume $8.88M, and price $0.0108.
...
| 24h volume | $8.88M | Position sizing and exit liquidity proxy |
| Market cap | $64.28M | Liquid valuation proxy |
| FDV | $107.68M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.68x | Higher spread means more supply risk |
| X followers | 949.33K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Plume | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether PLUME is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. Plume should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that Plume belongs to the yield infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $64.28M, FDV $107.68M, 24h volume $8.88M, price $0.0108, circulating supply 5.97B, and total supply 10.00B. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For Plume, the candidate backlog records Seeded 2026-06-28 from Surf market-ranking; rank 391, market cap $57,060,394, 24h volume $3,510,432. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Binance Smart Chain, Ethereum, Plume Network, exchanges as BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET, and tags as Layer1, RWA (Real World Assets), Yield & Asset Mgmt. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Plume currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 9.52% and 30d -4.49% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Plume's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-09-16 | Incubation | not disclosed | Mastercard |
| 2025-04-08 | Strategic | not disclosed | Apollo Global Management |
| 2025-03-17 | Undisclosed | not disclosed | YZi Labs (Prev. Binance Labs) |
| 2025-01-19 | Ieo | $30.00K | not disclosed |
| 2024-12-18 | Series A | $20.00M | Galaxy, HashKey Capital, a_capital, SV Angel, Haun Ventures, Brevan Howard Digital, Faction, Reciprocal Ventures |
| 2024-05-23 | Seed | $10.00M | Galaxy, a_capital, SV Angel, Haun Ventures, Calvin Liu, Reciprocal Ventures, Superscrypt, Jasper de Gooijer |

| Team member | Role | Link |
|---|---|---|
| Chris Yin | Co-Founder, CEO | [X](https://x.com/chriseyin) |
| Teddy Pornprinya | Co-Founder, CBO | [X](https://x.com/teddyP_xyz) |
| Eugene Y. Q. Shen | Co-Founder | [X](https://x.com/eyqshen) |
| Shukyee Ma | Chief Strategy Officer | [X](https://x.com/shukyeerwa) |
| Jason Meng | Head of Business Development | not disclosed |
| Ivy Kang | Head of Capital Markets | not disclosed |
| Alex Palmer | Head of Product | [X](https://x.com/ThatAlexPalmer) |
| James Friel | Head of Ecosystem | [X](https://twitter.com/James__Friel) |
...
Plume competes in the yield infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Plume has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Plume is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Plume compounds usage, liquidity improves, and PLUME gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Plume has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If PLUME does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $8.88M | Weekly | Volume below position-size threshold |
| FDV / market cap | 1.68x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Plume announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Plume has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Plume, the first reconciliation job is identity. The token symbol is PLUME, the category is yield infrastructure, chains are Binance Smart Chain, Ethereum, Plume Network, exchanges are BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET, and tags are Layer1, RWA (Real World Assets), Yield & Asset Mgmt. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If PLUME trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $64.28M, FDV is $107.68M, 24h volume is $8.88M, FDV / market cap is 1.68x, 7d price change is 9.52%, and 30d price change is -4.49%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
For Plume, the current liquidity read is watchlist first. The market cap is $64.28M, FDV is $107.68M, reported 24h volume is $8.88M, and FDV / market cap is 1.68x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Plume gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://plume.org/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Plume) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Plume) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Plume) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Plume) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Plume) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/plumenetwork) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/plumenetwork_community) | Identity, market, social, contract, or discovery cross-check |
| 9 | Discord | [link](https://discord.com/invite/plume-network) | Identity, market, social, contract, or discovery cross-check |
| 10 | PLUME/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/plume) | Identity, market, social, contract, or discovery cross-check |
| 11 | Plume Network Token Price & Market Data API | [link](https://www.coingecko.com/en/api/plume-network) | Identity, market, social, contract, or discovery cross-check |
| 12 | Plume USD Price Chart (PUSD) | [link](https://www.coingecko.com/en/coins/plume-usd) | Identity, market, social, contract, or discovery cross-check |
| 13 | Top Plume Network Ecosystem Coins by Market Cap | [link](https://www.coingecko.com/en/categories/plume-network-ecosystem) | Identity, market, social, contract, or discovery cross-check |
| 14 | Plume (PLUME) Price Prediction For 2026 & Beyond | [link](https://coinmarketcap.com/cmc-ai/plume/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 15 | ethereum PLUME | [link](https://etherscan.io/token/0x4c1746a800d224393fe2470c70a35717ed4ea5f1) | Identity, market, social, contract, or discovery cross-check |
| 16 | bsc PLUME | [link](https://bscscan.com/token/0x5afadcd1e8e3ca78ee2d37100102f2aec8bc0aa8) | Identity, market, social, contract, or discovery cross-check |
| 17 | ethereum PUSD | [link](https://etherscan.io/token/0xdddd73f5df1f0dc31373357beac77545dc5a6f3f) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google project search | [link](https://www.google.com/search?q=Plume%20PLUME%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google tokenomics search | [link](https://www.google.com/search?q=Plume%20PLUME%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google docs search | [link](https://www.google.com/search?q=Plume%20docs) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google governance search | [link](https://www.google.com/search?q=Plume%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google audit search | [link](https://www.google.com/search?q=Plume%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/plume/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/plume/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 25 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/plume/) | Identity, market, social, contract, or discovery cross-check |
| 26 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Plume) | Identity, market, social, contract, or discovery cross-check |
| 27 | Dune token query | [link](https://dune.com/browse/queries?q=Plume) | Identity, market, social, contract, or discovery cross-check |
| 28 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Plume) | Identity, market, social, contract, or discovery cross-check |
| 29 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=PLUME) | Identity, market, social, contract, or discovery cross-check |
| 30 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Plume) | Identity, market, social, contract, or discovery cross-check |
| 31 | CryptoRank search | [link](https://cryptorank.io/price/plume) | Identity, market, social, contract, or discovery cross-check |
| 32 | RootData search | [link](https://www.rootdata.com/search?kw=Plume) | Identity, market, social, contract, or discovery cross-check |
| 33 | CoinDesk search | [link](https://www.coindesk.com/search?s=Plume) | Identity, market, social, contract, or discovery cross-check |
| 34 | The Block search | [link](https://www.theblock.co/search?query=Plume) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://plume.org/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Plume)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Plume)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Plume)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Plume)
- [Messari discovery](https://messari.io/search?q=Plume)
- [X profile](https://x.com/plumenetwork)
- [Telegram](https://t.me/plumenetwork_community)
- [Discord](https://discord.com/invite/plume-network)
- [PLUME/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/plume)
- [Plume Network Token Price & Market Data API](https://www.coingecko.com/en/api/plume-network)
- [Plume USD Price Chart (PUSD)](https://www.coingecko.com/en/coins/plume-usd)
- [Top Plume Network Ecosystem Coins by Market Cap](https://www.coingecko.com/en/categories/plume-network-ecosystem)
- [Plume (PLUME) Price Prediction For 2026 & Beyond](https://coinmarketcap.com/cmc-ai/plume/price-prediction/)
- [ethereum PLUME](https://etherscan.io/token/0x4c1746a800d224393fe2470c70a35717ed4ea5f1)
- [bsc PLUME](https://bscscan.com/token/0x5afadcd1e8e3ca78ee2d37100102f2aec8bc0aa8)
- [ethereum PUSD](https://etherscan.io/token/0xdddd73f5df1f0dc31373357beac77545dc5a6f3f)
- [Google project search](https://www.google.com/search?q=Plume%20PLUME%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Plume%20PLUME%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Plume%20docs)
- [Google governance search](https://www.google.com/search?q=Plume%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Plume%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/plume/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/plume/price-analysis/)

## Final Investment View

Plume should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
