---
title: 'Pax Dollar / USDP: stablecoin / credit infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Pax Dollar is treated as a stablecoin / credit infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'pax-dollar-usdp-stablecoin-credit-infrastructure-token-value-capture-risk'
category: 'research'
---

# Pax Dollar / USDP: stablecoin / credit infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Pax Dollar appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Ethereum, Solana; exchange exposure is not disclosed; category tags are Payments & Gateway, RWA (Real World Assets), Stablecoins; current market cap is $1.88B; FDV is $1.88B; and 24h volume is $113.06M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Pax Dollar before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 614, market cap $31,935,589, 24h volume $5,082,492. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Pax Dollar is best understood as a **stablecoin / credit infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether USDP captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| FDV | $1.88B |
| 24h volume | $113.06M |
| Circulating supply | 452.15K |
| Total supply | 452.15K |
| 7d price change | 3.29% |
| 30d price change | -3.30% |
| Latest TVL sample | $2.55B |
| TVL sample change | 30.37% |
| Primary source posture | Surf + CoinGecko + CMC discovery + official/project links |

Base case: Pax Dollar remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning USDP into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Pax Dollar (USDP) is a regulated U.S. dollar stablecoin issued by Paxos, backed 1:1 by cash and cash equivalents with always-available redemption. It operates under OCC oversight and is available on multiple blockchains like Ethereum and Solana. Reserves are held in segregated accounts for customer protection.

Surf's project profile maps Pax Dollar to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.paxos.com/](https://www.paxos.com/) |
| X handle | [@paxosglobal](https://x.com/paxosglobal) |
| X followers | 4.23K |
| Tags | Payments & Gateway, RWA (Real World Assets), Stablecoins |
| Chains | Ethereum, Solana |
| Exchanges | not disclosed |
| TGE status | not disclosed |
| CoinGecko ID | not disclosed |

The main product question is whether Pax Dollar is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For stablecoin / credit infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Pax Dollar convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $113.06M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 452.15K circulating vs 452.15K total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Pax Dollar is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

Pax Dollar's architecture should be evaluated through the job it performs. For a stablecoin / credit infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
| Settlement layer | Chains and contracts that hold user value | Ethereum, Solana |
...
|---|---|---|---|
| ethereum | PAXG | 0x45804880de22913dafe09f4980848ece6ecbaf78 | [explorer](https://etherscan.io/token/0x45804880de22913dafe09f4980848ece6ecbaf78) |
| solana | USDP | HVbpJAQGNpkgBaYBZQBR1t7yFdvaYVp2vCQQfKKEN4tM | [explorer](https://solscan.io/token/HVbpJAQGNpkgBaYBZQBR1t7yFdvaYVp2vCQQfKKEN4tM) |
| ethereum | USDP | 0x8e870d67f660d95d5be530380d0ec0bd388289e1 | [explorer](https://etherscan.io/token/0x8e870d67f660d95d5be530380d0ec0bd388289e1) |
| ethereum | BUSD | 0x4fabb145d64652a948d72533023f6e7a623c7c53 | [explorer](https://etherscan.io/token/0x4fabb145d64652a948d72533023f6e7a623c7c53) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Pax Dollar, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 614, market cap $31,935,589, 24h volume $5,082,492. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $1.88B, FDV $1.88B, 24h volume $113.06M, and price $4.15K.

| Traction metric | Value | Interpretation |
|---|---|---|
| Latest TVL sample | $2.55B | Useful only if sustained without aggressive incentives |
| TVL sample points | 90 | More points improve trend confidence |
| TVL sample change | 30.37% | Positive trend supports usage; negative trend requires explanation |
| 24h volume | $113.06M | Position sizing and exit liquidity proxy |
| Market cap | $1.88B | Liquid valuation proxy |
| FDV | $1.88B | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.00x | Higher spread means more supply risk |
| X followers | 4.23K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Pax Dollar | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether USDP is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Pax Dollar currently needs to be monitored through that sequence. The available 90-day TVL series has 90 point(s), latest TVL $2.55B, and approximate change 30.37%. Market-price movement shows 7d 3.29% and 30d -3.30% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Pax Dollar's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2021-07-29 | Extended Series D | not disclosed | Coinbase Ventures, FTX Ventures, Founders Fund |
| 2021-04-29 | Series D | $300.00M | Oak HC/FT, Declaration Partners, Senator Investment Group, PayPal Ventures, Mithril Capital Management |
| 2020-12-17 | Series C | $142.00M | Declaration Partners, RIT Capital Partners, PayPal Ventures, Mithril Capital Management, Ken Moelis |
| 2018-05-31 | Series B | $65.00M | Liberty City Ventures, RRE Ventures |
| 2015-05-07 | Series A | $25.00M | Raptor Capital, Liberty City Ventures, RRE Ventures |

| Team member | Role | Link |
|---|---|---|
| Charles Cascarilla | Co-Founder, CEO | [X](https://x.com/chadcascarilla) |
| Rich Teo | Co-Founder | [X](https://x.com/rich_teo) |
| Emma Tupa | Chief of Staff | [X](https://x.com/emma_tupa) |
| Derek Gottfrid | Global Head of Engineering | [X](https://x.com/derekg) |
| Ronak Daya | Head of Product | [X](https://x.com/ronakdaya) |
| Peter Jonas | CRO | [X](https://x.com/pbjonas) |
| Sarah Breslow | Associate General Counsel | not disclosed |
| Jimmy Song | Principal Architect | [X](https://x.com/jimmysong) |
...
Pax Dollar competes in the stablecoin / credit infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Pax Dollar has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Pax Dollar is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Pax Dollar compounds usage, liquidity improves, and USDP gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If USDP does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
|---|---|---|---|
| TVL | $2.55B | Weekly | >20% drawdown or sustained growth |
| 24h volume | $113.06M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Pax Dollar announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Pax Dollar has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Pax Dollar, the first reconciliation job is identity. The token symbol is USDP, the category is stablecoin / credit infrastructure, chains are Ethereum, Solana, exchanges are not disclosed, and tags are Payments & Gateway, RWA (Real World Assets), Stablecoins. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If USDP trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $1.88B, FDV is $1.88B, 24h volume is $113.06M, FDV / market cap is 1.00x, 7d price change is 3.29%, and 30d price change is -3.30%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.

The third reconciliation job is usage data. The latest TVL sample is $2.55B, TVL sample change is 30.37%, and the sample contains 90 point(s). TVL is useful only when the underlying balances are real, withdrawable, and not double-counted. For stablecoin / credit infrastructure, the memo should identify whether TVL means deposits, staked assets, LP liquidity, stablecoin supply, collateral, locked governance assets, or a derived wrapper. A yield-bearing stablecoin and a validator middleware protocol can both show TVL, but the risk encoded by that TVL is completely different.

| Reconciliation item | Current value | Next check |
|---|---|---|
| Identity | Pax Dollar / USDP | Match official docs, CG, CMC, Surf, contracts |
| Category | stablecoin / credit infrastructure | Confirm product mechanics and revenue model |
| Chains | Ethereum, Solana | Confirm canonical deployments and bridge assumptions |
| Exchanges | not disclosed | Check live order books and inactive pairs |
| Market cap | $1.88B | Compare Surf, CG, CMC, exchange pages |
| FDV | $1.88B | Reconcile total supply and unlock schedule |
| Volume | $113.06M | Check exchange concentration and wash-trading risk |
| TVL | $2.55B | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Pax Dollar does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Pax Dollar, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does USDP sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
For Pax Dollar, the current liquidity read is watchlist first. The market cap is $1.88B, FDV is $1.88B, reported 24h volume is $113.06M, and FDV / market cap is 1.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. Pax Dollar can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.

For Pax Dollar, the live monitoring stack should include contract events, TVL, volume, governance, documentation changes, social announcements, security disclosures, and exchange-market quality. A single metric is not enough. TVL may stay high while liquidity thins. Volume may rise because of volatility rather than adoption. Social attention may rise around controversy rather than progress. Governance may pass proposals that improve short-term price but add long-term risk. The strongest monitoring dashboard combines on-chain state, off-chain disclosure, and market quality.

Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If Pax Dollar suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | Pax Dollar gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.paxos.com/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Pax%20Dollar) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Pax%20Dollar) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Pax%20Dollar) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Pax%20Dollar) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Pax%20Dollar) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/paxosglobal) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/paxosglobal/pax-contracts) | Identity, market, social, contract, or discovery cross-check |
| 9 | Pax Dollar USDP Price - CoinGecko | [link](https://www.coingecko.com/en/coins/pax-dollar) | Identity, market, social, contract, or discovery cross-check |
| 10 | What Is Pax Dollar (USDP) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/paxos-standard/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 11 | Top Stablecoins by Market Cap - CoinGecko | [link](https://www.coingecko.com/en/categories/stablecoins) | Identity, market, social, contract, or discovery cross-check |
| 12 | Pax Dollar (USDP) Price Prediction For 2026 & Beyond | [link](https://coinmarketcap.com/cmc-ai/paxos-standard/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 13 | USDP Overview - Paxos Documentation | [link](https://docs.paxos.com/guides/stablecoin/usdp) | Identity, market, social, contract, or discovery cross-check |
| 14 | ethereum PAXG | [link](https://etherscan.io/token/0x45804880de22913dafe09f4980848ece6ecbaf78) | Identity, market, social, contract, or discovery cross-check |
| 15 | solana USDP | [link](https://solscan.io/token/HVbpJAQGNpkgBaYBZQBR1t7yFdvaYVp2vCQQfKKEN4tM) | Identity, market, social, contract, or discovery cross-check |
| 16 | ethereum USDP | [link](https://etherscan.io/token/0x8e870d67f660d95d5be530380d0ec0bd388289e1) | Identity, market, social, contract, or discovery cross-check |
| 17 | ethereum BUSD | [link](https://etherscan.io/token/0x4fabb145d64652a948d72533023f6e7a623c7c53) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google project search | [link](https://www.google.com/search?q=Pax%20Dollar%20USDP%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google tokenomics search | [link](https://www.google.com/search?q=Pax%20Dollar%20USDP%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google docs search | [link](https://www.google.com/search?q=Pax%20Dollar%20docs) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google governance search | [link](https://www.google.com/search?q=Pax%20Dollar%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google audit search | [link](https://www.google.com/search?q=Pax%20Dollar%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/pax-dollar/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/pax-dollar/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 25 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/pax-dollar/) | Identity, market, social, contract, or discovery cross-check |
| 26 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Pax%20Dollar) | Identity, market, social, contract, or discovery cross-check |
| 27 | Dune token query | [link](https://dune.com/browse/queries?q=Pax%20Dollar) | Identity, market, social, contract, or discovery cross-check |
| 28 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Pax%20Dollar) | Identity, market, social, contract, or discovery cross-check |
| 29 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=USDP) | Identity, market, social, contract, or discovery cross-check |
| 30 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Pax%20Dollar) | Identity, market, social, contract, or discovery cross-check |
| 31 | CryptoRank search | [link](https://cryptorank.io/price/pax-dollar) | Identity, market, social, contract, or discovery cross-check |
| 32 | RootData search | [link](https://www.rootdata.com/search?kw=Pax%20Dollar) | Identity, market, social, contract, or discovery cross-check |
| 33 | CoinDesk search | [link](https://www.coindesk.com/search?s=Pax%20Dollar) | Identity, market, social, contract, or discovery cross-check |
| 34 | The Block search | [link](https://www.theblock.co/search?query=Pax%20Dollar) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.paxos.com/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Pax%20Dollar)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Pax%20Dollar)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Pax%20Dollar)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Pax%20Dollar)
- [Messari discovery](https://messari.io/search?q=Pax%20Dollar)
- [X profile](https://x.com/paxosglobal)
- [GitHub](https://github.com/paxosglobal/pax-contracts)
- [Pax Dollar USDP Price - CoinGecko](https://www.coingecko.com/en/coins/pax-dollar)
- [What Is Pax Dollar (USDP) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/paxos-standard/what-is/)
- [Top Stablecoins by Market Cap - CoinGecko](https://www.coingecko.com/en/categories/stablecoins)
- [Pax Dollar (USDP) Price Prediction For 2026 & Beyond](https://coinmarketcap.com/cmc-ai/paxos-standard/price-prediction/)
- [USDP Overview - Paxos Documentation](https://docs.paxos.com/guides/stablecoin/usdp)
- [ethereum PAXG](https://etherscan.io/token/0x45804880de22913dafe09f4980848ece6ecbaf78)
- [solana USDP](https://solscan.io/token/HVbpJAQGNpkgBaYBZQBR1t7yFdvaYVp2vCQQfKKEN4tM)
- [ethereum USDP](https://etherscan.io/token/0x8e870d67f660d95d5be530380d0ec0bd388289e1)
- [ethereum BUSD](https://etherscan.io/token/0x4fabb145d64652a948d72533023f6e7a623c7c53)
- [Google project search](https://www.google.com/search?q=Pax%20Dollar%20USDP%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Pax%20Dollar%20USDP%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Pax%20Dollar%20docs)
- [Google governance search](https://www.google.com/search?q=Pax%20Dollar%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Pax%20Dollar%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/pax-dollar/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/pax-dollar/price-analysis/)

## Final Investment View

Pax Dollar should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
