---
title: 'Audius / AUDIO: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Audius is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'audius-audio-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# Audius / AUDIO: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Audius appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Energi, Ethereum, Solana; exchange exposure is BINANCE, UPBIT, COINBASE, BITHUMB, BITGET; category tags are Creator Economy, Messaging & Media, SocialFi; current market cap is $19.51M; FDV is $19.51M; and 24h volume is $2.79M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Audius before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 838, market cap $19,552,935, 24h volume $2,790,400. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Audius is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether AUDIO captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Energi, Ethereum, Solana |
| Exchanges | BINANCE, UPBIT, COINBASE, BITHUMB, BITGET |
| Price | $0.0136 |
| Market cap | $19.51M |
| FDV | $19.51M |
| 24h volume | $2.79M |
| Circulating supply | 1.44B |
| Total supply | 1.44B |
...
Base case: Audius remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning AUDIO into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Audius is a decentralized, community-run music streaming protocol and platform that lets artists publish, monetize, and distribute audio directly to fans. Its native AUDIO token powers staking, governance, and incentive rewards for artists, node operators, and fans, while open-source APIs and SDKs enable third-party apps and integrations.

Surf's project profile maps Audius to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://audius.co/](https://audius.co/) |
| X handle | [@audius](https://x.com/audius) |
| X followers | 134.32K |
| Tags | Creator Economy, Messaging & Media, SocialFi |
| Chains | Energi, Ethereum, Solana |
| Exchanges | BINANCE, UPBIT, COINBASE, BITHUMB, BITGET |
| TGE status | post |
...
| Liquidity | $2.79M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 1.44B circulating vs 1.44B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Audius is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

Audius's architecture should be evaluated through the job it performs. For a DeFi infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
| Settlement layer | Chains and contracts that hold user value | Energi, Ethereum, Solana |
...
|---|---|---|---|
| Energi | AUDIO | 0x2c25972d4adb478773be354a09e4596f29e31fb3 | not mapped |
| solana | AUDIO | 9LzCMqDgTKYz9Drzqnpgee3SGa89up3a247ypMj2xrqM | [explorer](https://solscan.io/token/9LzCMqDgTKYz9Drzqnpgee3SGa89up3a247ypMj2xrqM) |
| ethereum | AUDIO | 0x18aaa7115705e8be94bffebde57af9bfc265b998 | [explorer](https://etherscan.io/token/0x18aaa7115705e8be94bffebde57af9bfc265b998) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Audius, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 838, market cap $19,552,935, 24h volume $2,790,400. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $19.51M, FDV $19.51M, 24h volume $2.79M, and price $0.0136.
...
| 24h volume | $2.79M | Position sizing and exit liquidity proxy |
| Market cap | $19.51M | Liquid valuation proxy |
| FDV | $19.51M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.00x | Higher spread means more supply risk |
| X followers | 134.32K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Audius | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether AUDIO is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. Audius should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that Audius belongs to the DeFi infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $19.51M, FDV $19.51M, 24h volume $2.79M, price $0.0136, circulating supply 1.44B, and total supply 1.44B. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For Audius, the candidate backlog records Seeded 2026-07-06 from Surf market-ranking; rank 838, market cap $19,552,935, 24h volume $2,790,400. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Energi, Ethereum, Solana, exchanges as BINANCE, UPBIT, COINBASE, BITHUMB, BITGET, and tags as Creator Economy, Messaging & Media, SocialFi. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Audius currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 2.22% and 30d -7.70% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Audius's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2021-09-16 | Undisclosed | $5.00M | Steve Aoki, Nas, Katy Perry, Pusha T, The Chainsmokers, Jason Derulo |
| 2020-10-23 | Undisclosed | $1.25M | YZi Labs (Prev. Binance Labs) |
| 2020-07-31 | Strategic | $3.10M | Coinbase Ventures, Pantera Capital, Multicoin Capital, Blockchange Ventures, Collab+Currency |

| Team member | Role | Link |
|---|---|---|
| Roneil Rumburg | Co-Founder, CEO | [X](https://twitter.com/roneilr) |
| Forrest Browning | Co-Founder, CPO | [X](https://twitter.com/ForrestBrowning) |
| Raymond Jacobson | CTO | not disclosed |
| Julian Baker | Head of Design | [X](https://twitter.com/notjulianbaker) |
| Christina Rowland | Head of Operations | not disclosed |
| Clayton Blaha | Head of Partnerships | [X](https://twitter.com/FerrariJetpack) |
...
Audius competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
Weighted conclusion: Audius is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Audius compounds usage, liquidity improves, and AUDIO gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Audius has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If AUDIO does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $2.79M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Audius announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
For Audius, the current liquidity read is watchlist first. The market cap is $19.51M, FDV is $19.51M, reported 24h volume is $2.79M, and FDV / market cap is 1.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Audius gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://audius.co/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Audius) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Audius) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Audius) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Audius) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Audius) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/audius) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/AudiusProject) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/Audius) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](https://discord.gg/audius) | Identity, market, social, contract, or discovery cross-check |
| 11 | Audius Price: AUDIO/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/audius) | Identity, market, social, contract, or discovery cross-check |
| 12 | Audius price today, AUDIO to USD live price, marketcap and chart | [link](https://coinmarketcap.com/currencies/audius/) | Identity, market, social, contract, or discovery cross-check |
| 13 | Audius Price History: Download AUDIO Historical Data / CoinGecko | [link](https://www.coingecko.com/en/coins/audius/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 14 | Latest Audius (AUDIO) Price Analysis - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/audius/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 15 | AUDIO to BTC: Audius Price in Bitcoin - CoinGecko | [link](https://www.coingecko.com/en/coins/audius/btc) | Identity, market, social, contract, or discovery cross-check |
| 16 | solana AUDIO | [link](https://solscan.io/token/9LzCMqDgTKYz9Drzqnpgee3SGa89up3a247ypMj2xrqM) | Identity, market, social, contract, or discovery cross-check |
| 17 | ethereum AUDIO | [link](https://etherscan.io/token/0x18aaa7115705e8be94bffebde57af9bfc265b998) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google project search | [link](https://www.google.com/search?q=Audius%20AUDIO%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google tokenomics search | [link](https://www.google.com/search?q=Audius%20AUDIO%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google docs search | [link](https://www.google.com/search?q=Audius%20docs) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google governance search | [link](https://www.google.com/search?q=Audius%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google audit search | [link](https://www.google.com/search?q=Audius%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/audius/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Audius) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune token query | [link](https://dune.com/browse/queries?q=Audius) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Audius) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=AUDIO) | Identity, market, social, contract, or discovery cross-check |
| 28 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Audius) | Identity, market, social, contract, or discovery cross-check |
| 29 | CryptoRank search | [link](https://cryptorank.io/price/audius) | Identity, market, social, contract, or discovery cross-check |
| 30 | RootData search | [link](https://www.rootdata.com/search?kw=Audius) | Identity, market, social, contract, or discovery cross-check |
| 31 | CoinDesk search | [link](https://www.coindesk.com/search?s=Audius) | Identity, market, social, contract, or discovery cross-check |
| 32 | The Block search | [link](https://www.theblock.co/search?query=Audius) | Identity, market, social, contract, or discovery cross-check |
| 33 | GitHub search | [link](https://github.com/search?q=Audius&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 34 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=AUDIO) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://audius.co/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Audius)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Audius)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Audius)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Audius)
- [Messari discovery](https://messari.io/search?q=Audius)
- [X profile](https://x.com/audius)
- [GitHub](https://github.com/AudiusProject)
- [Telegram](https://t.me/Audius)
- [Discord](https://discord.gg/audius)
- [Audius Price: AUDIO/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/audius)
- [Audius price today, AUDIO to USD live price, marketcap and chart](https://coinmarketcap.com/currencies/audius/)
- [Audius Price History: Download AUDIO Historical Data | CoinGecko](https://www.coingecko.com/en/coins/audius/historical_data)
- [Latest Audius (AUDIO) Price Analysis - CoinMarketCap](https://coinmarketcap.com/cmc-ai/audius/price-analysis/)
- [AUDIO to BTC: Audius Price in Bitcoin - CoinGecko](https://www.coingecko.com/en/coins/audius/btc)
- [solana AUDIO](https://solscan.io/token/9LzCMqDgTKYz9Drzqnpgee3SGa89up3a247ypMj2xrqM)
- [ethereum AUDIO](https://etherscan.io/token/0x18aaa7115705e8be94bffebde57af9bfc265b998)
- [Google project search](https://www.google.com/search?q=Audius%20AUDIO%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Audius%20AUDIO%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Audius%20docs)
- [Google governance search](https://www.google.com/search?q=Audius%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Audius%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/audius/what-is/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Audius)

## Final Investment View

Audius should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
