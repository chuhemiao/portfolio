---
title: 'AIOZ Network / AIOZ: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'AIOZ Network is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'aioz-network-aioz-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# AIOZ Network / AIOZ: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** AIOZ Network appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Binance Smart Chain, Ethereum, Osmosis; exchange exposure is COINBASE, BITHUMB, BYBIT; category tags are DePIN (Physical Infra), Layer1, Storage & Compute; current market cap is $67.53M; FDV is $67.59M; and 24h volume is $5.11M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag AIOZ Network before this file was created, and the existing candidate note says: Seeded 2026-06-28 from Surf market-ranking; rank 347, market cap $67,090,990, 24h volume $2,917,124. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

AIOZ Network is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether AIOZ captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Binance Smart Chain, Ethereum, Osmosis |
| Exchanges | COINBASE, BITHUMB, BYBIT |
| Price | $0.0535 |
| Market cap | $67.53M |
| FDV | $67.59M |
| 24h volume | $5.11M |
...
Base case: AIOZ Network remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning AIOZ into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

AIOZ Network is a DePIN-based Web3 infrastructure platform that combines a Layer‑1 blockchain (EVM + Cosmos‑compatible) with decentralized content delivery, P2P media streaming, AI compute marketplace, and S3‑compatible distributed storage. The native AIOZ token incentivizes node operators and powers ecosystem services such as storage, streaming, pinning, and AI model hosting.

Surf's project profile maps AIOZ Network to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://aioz.network/](https://aioz.network/) |
| X handle | [@AIOZNetwork](https://x.com/AIOZNetwork) |
| X followers | 106.12K |
| Tags | DePIN (Physical Infra), Layer1, Storage & Compute |
| Chains | Binance Smart Chain, Ethereum, Osmosis |
| Exchanges | COINBASE, BITHUMB, BYBIT |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether AIOZ Network is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DeFi infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does AIOZ Network convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $5.11M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 1.26B circulating vs 1.26B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that AIOZ Network is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

AIOZ Network's architecture should be evaluated through the job it performs. For a DeFi infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
| Settlement layer | Chains and contracts that hold user value | Binance Smart Chain, Ethereum, Osmosis |
...
|---|---|---|---|
| Osmosis | AIOZ | ibc/BB0AFE2AFBD6E883690DAE4B9168EAC2B306BCC9C9292DACBB4152BBB08DB25F | not mapped |
| ethereum | AIOZ | 0x626e8036deb333b408be468f951bdb42433cbf18 | [explorer](https://etherscan.io/token/0x626e8036deb333b408be468f951bdb42433cbf18) |
| bsc | AIOZ | 0x33d08d8c7a168333a85285a68c0042b39fc3741d | [explorer](https://bscscan.com/token/0x33d08d8c7a168333a85285a68c0042b39fc3741d) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For AIOZ Network, the DeFi-oriented backlog note is the first signal: Seeded 2026-06-28 from Surf market-ranking; rank 347, market cap $67,090,990, 24h volume $2,917,124. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $67.53M, FDV $67.59M, 24h volume $5.11M, and price $0.0535.
...
| 24h volume | $5.11M | Position sizing and exit liquidity proxy |
| Market cap | $67.53M | Liquid valuation proxy |
| FDV | $67.59M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.00x | Higher spread means more supply risk |
| X followers | 106.12K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for AIOZ Network | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether AIOZ is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. AIOZ Network currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 1.98% and 30d 1.29% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that AIOZ Network's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2021-04-02 | Ido | $100.00K | not disclosed |
| 2021-04-02 | Ido | $155.00K | not disclosed |
| not disclosed | Private | $1.09M | not disclosed |

| Team member | Role | Link |
|---|---|---|
| Erman Tjiputra | Founder | [X](https://twitter.com/ErmanTjiputra) |
| Trieu Nguyen | CTO | not disclosed |
| Hien Nguyen | Head of Blockchain | not disclosed |
| Quang Tran | Head of AI | not disclosed |
| Dave Peacock | Head of Design | not disclosed |
| Laura Toma | Advisor | [X](https://twitter.com/lor2005) |
| Joshua Toh | Advisor | not disclosed |
| Dejan Horvat | Advisor | [X](https://twitter.com/horvat_de) |
...
AIOZ Network competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether AIOZ Network has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: AIOZ Network is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | AIOZ Network compounds usage, liquidity improves, and AIOZ gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
| 24h volume | $5.11M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if AIOZ Network announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
For AIOZ Network, the current liquidity read is watchlist first. The market cap is $67.53M, FDV is $67.59M, reported 24h volume is $5.11M, and FDV / market cap is 1.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | AIOZ Network gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://aioz.network/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=AIOZ%20Network) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=AIOZ%20Network) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=AIOZ%20Network) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=AIOZ%20Network) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=AIOZ%20Network) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/AIOZNetwork) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/aioznetwork) | Identity, market, social, contract, or discovery cross-check |
| 9 | AIOZ/USD Live Price Chart, Market Cap & News Today - CoinGecko | [link](https://www.coingecko.com/en/coins/aioz-network) | Identity, market, social, contract, or discovery cross-check |
| 10 | AIOZ Network price today, AIOZ to USD live price, marketcap and chart | [link](https://coinmarketcap.com/currencies/aioz-network/) | Identity, market, social, contract, or discovery cross-check |
| 11 | AIOZ Network Tokenomics: Supply, Staking, Incentives | [link](https://www.findas.org/tokenomics-review/coins/the-tokenomics-of-aioz-network-aioz/r/UCDuZ8XbpX4rXBPf6NyTfs) | Identity, market, social, contract, or discovery cross-check |
| 12 | AIOZ to BTC: AIOZ Network Price in Bitcoin - CoinGecko | [link](https://www.coingecko.com/en/coins/aioz-network/btc) | Identity, market, social, contract, or discovery cross-check |
| 13 | Tokenomics 2.0 - aioz network docs | [link](https://docs.aioz.network/overview/aioz-tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 14 | ethereum AIOZ | [link](https://etherscan.io/token/0x626e8036deb333b408be468f951bdb42433cbf18) | Identity, market, social, contract, or discovery cross-check |
| 15 | bsc AIOZ | [link](https://bscscan.com/token/0x33d08d8c7a168333a85285a68c0042b39fc3741d) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google project search | [link](https://www.google.com/search?q=AIOZ%20Network%20AIOZ%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google tokenomics search | [link](https://www.google.com/search?q=AIOZ%20Network%20AIOZ%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google docs search | [link](https://www.google.com/search?q=AIOZ%20Network%20docs) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google governance search | [link](https://www.google.com/search?q=AIOZ%20Network%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google audit search | [link](https://www.google.com/search?q=AIOZ%20Network%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/aioz-network/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/aioz-network/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune discovery | [link](https://dune.com/browse/dashboards?q=AIOZ%20Network) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune token query | [link](https://dune.com/browse/queries?q=AIOZ%20Network) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=AIOZ%20Network) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=AIOZ) | Identity, market, social, contract, or discovery cross-check |
| 27 | TokenUnlocks search | [link](https://token.unlocks.app/?search=AIOZ%20Network) | Identity, market, social, contract, or discovery cross-check |
| 28 | CryptoRank search | [link](https://cryptorank.io/price/aioz-network) | Identity, market, social, contract, or discovery cross-check |
| 29 | RootData search | [link](https://www.rootdata.com/search?kw=AIOZ%20Network) | Identity, market, social, contract, or discovery cross-check |
| 30 | CoinDesk search | [link](https://www.coindesk.com/search?s=AIOZ%20Network) | Identity, market, social, contract, or discovery cross-check |
| 31 | The Block search | [link](https://www.theblock.co/search?query=AIOZ%20Network) | Identity, market, social, contract, or discovery cross-check |
| 32 | GitHub search | [link](https://github.com/search?q=AIOZ%20Network&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 33 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=AIOZ) | Identity, market, social, contract, or discovery cross-check |
| 34 | BscScan token search | [link](https://bscscan.com/search?f=0&q=AIOZ) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://aioz.network/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=AIOZ%20Network)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=AIOZ%20Network)
- [DefiLlama protocol discovery](https://defillama.com/search?query=AIOZ%20Network)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=AIOZ%20Network)
- [Messari discovery](https://messari.io/search?q=AIOZ%20Network)
- [X profile](https://x.com/AIOZNetwork)
- [Telegram](https://t.me/aioznetwork)
- [AIOZ/USD Live Price Chart, Market Cap & News Today - CoinGecko](https://www.coingecko.com/en/coins/aioz-network)
- [AIOZ Network price today, AIOZ to USD live price, marketcap and chart](https://coinmarketcap.com/currencies/aioz-network/)
- [AIOZ Network Tokenomics: Supply, Staking, Incentives](https://www.findas.org/tokenomics-review/coins/the-tokenomics-of-aioz-network-aioz/r/UCDuZ8XbpX4rXBPf6NyTfs)
- [AIOZ to BTC: AIOZ Network Price in Bitcoin - CoinGecko](https://www.coingecko.com/en/coins/aioz-network/btc)
- [Tokenomics 2.0 - aioz network docs](https://docs.aioz.network/overview/aioz-tokenomics)
- [ethereum AIOZ](https://etherscan.io/token/0x626e8036deb333b408be468f951bdb42433cbf18)
- [bsc AIOZ](https://bscscan.com/token/0x33d08d8c7a168333a85285a68c0042b39fc3741d)
- [Google project search](https://www.google.com/search?q=AIOZ%20Network%20AIOZ%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=AIOZ%20Network%20AIOZ%20tokenomics)
- [Google docs search](https://www.google.com/search?q=AIOZ%20Network%20docs)
- [Google governance search](https://www.google.com/search?q=AIOZ%20Network%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=AIOZ%20Network%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/aioz-network/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/aioz-network/price-analysis/)
- [Dune discovery](https://dune.com/browse/dashboards?q=AIOZ%20Network)
- [Dune token query](https://dune.com/browse/queries?q=AIOZ%20Network)

## Final Investment View

AIOZ Network should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
