---
title: 'Wilder World / WILD: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Wilder World is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'wilder-world-wild-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# Wilder World / WILD: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Wilder World appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Avalanche, Base, Binance Smart Chain, Ethereum, Polygon Pos, Solana; exchange exposure is BINANCE ALPHA, BITGET; category tags are GameFi, Metaverse, Web3 AI (Agent/AIGC); current market cap is $9.35M; FDV is $9.76M; and 24h volume is $117.54K. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Wilder World before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 1209, market cap $9,307,644, 24h volume $117,577. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Wilder World is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether WILD captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Avalanche, Base, Binance Smart Chain, Ethereum, Polygon Pos, Solana |
| Exchanges | BINANCE ALPHA, BITGET |
| Price | $0.0195 |
| Market cap | $9.35M |
| FDV | $9.76M |
| 24h volume | $117.54K |
| Circulating supply | 479.23M |
| Total supply | 499.97M |
...
Base case: Wilder World remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning WILD into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Wilder World is a photorealistic, open-world metaverse built with Unreal Engine 5 that combines multiplayer gaming, NFT assets and an on-chain economy. $WILD (ERC-20) powers in-game transactions, governance, staking and a marketplace (WWMM); the project integrates Ethereum and ZERO technologies and emphasizes community-driven governance.

Surf's project profile maps Wilder World to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.wilderworld.com/](https://www.wilderworld.com/) |
| X handle | [@WilderWorld](https://x.com/WilderWorld) |
| X followers | 255.88K |
| Tags | GameFi, Metaverse, Web3 AI (Agent/AIGC) |
| Chains | Avalanche, Base, Binance Smart Chain, Ethereum, Polygon Pos, Solana |
| Exchanges | BINANCE ALPHA, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Wilder World is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DeFi infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Wilder World convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $117.54K 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 479.23M circulating vs 499.97M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Wilder World is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

Wilder World's architecture should be evaluated through the job it performs. For a DeFi infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
| Settlement layer | Chains and contracts that hold user value | Avalanche, Base, Binance Smart Chain, Ethereum, Polygon Pos, Solana |
...
|---|---|---|---|
| base | WILD | 0xafde2490236bc64950def5472296aa0d9758db0d | [explorer](https://basescan.org/token/0xafde2490236bc64950def5472296aa0d9758db0d) |
| solana | WILD | FVvd3s9dZYzsgitkJyWbmycSc8MkYZjyF7oqAEvmSxTZ | [explorer](https://solscan.io/token/FVvd3s9dZYzsgitkJyWbmycSc8MkYZjyF7oqAEvmSxTZ) |
| ethereum | WILD | 0x2a3bff78b79a009976eea096a51a948a3dc00e34 | [explorer](https://etherscan.io/token/0x2a3bff78b79a009976eea096a51a948a3dc00e34) |
| avalanche | WILD | 0x153374c6d6786b6ca2c4bc96f9c3a471428f2bc7 | [explorer](https://snowtrace.io/token/0x153374c6d6786b6ca2c4bc96f9c3a471428f2bc7) |
| polygon | WILD | 0xafde2490236bc64950def5472296aa0d9758db0d | [explorer](https://polygonscan.com/token/0xafde2490236bc64950def5472296aa0d9758db0d) |
| bsc | WILD | 0x6685906b75c61c57772c335402f594f855c1b0e3 | [explorer](https://bscscan.com/token/0x6685906b75c61c57772c335402f594f855c1b0e3) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Wilder World, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 1209, market cap $9,307,644, 24h volume $117,577. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $9.35M, FDV $9.76M, 24h volume $117.54K, and price $0.0195.
...
| 24h volume | $117.54K | Position sizing and exit liquidity proxy |
| Market cap | $9.35M | Liquid valuation proxy |
| FDV | $9.76M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.04x | Higher spread means more supply risk |
| X followers | 255.88K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Wilder World | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether WILD is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. Wilder World should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that Wilder World belongs to the DeFi infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $9.35M, FDV $9.76M, 24h volume $117.54K, price $0.0195, circulating supply 479.23M, and total supply 499.97M. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For Wilder World, the candidate backlog records Seeded 2026-07-06 from Surf market-ranking; rank 1209, market cap $9,307,644, 24h volume $117,577. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Avalanche, Base, Binance Smart Chain, Ethereum, Polygon Pos, Solana, exchanges as BINANCE ALPHA, BITGET, and tags as GameFi, Metaverse, Web3 AI (Agent/AIGC). If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Wilder World currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 6.19% and 30d -3.09% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Wilder World's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2022-01-17 | Undisclosed | $30.00M | Animoca Brands, The Spartan Group, Republic, Red Beard Ventures, Anthony Pompliano, Metapurse, Magdalena Kala, Nexo |
| 2021-05-11 | Ido | $250.00K | not disclosed |
| 2021-05-10 | Undisclosed | $3.00M | Animoca Brands, The Spartan Group, Digital Currency Group (DCG), Signum Capital, Republic |
| not disclosed | Private | $2.75M | not disclosed |

| Team member | Role | Link |
|---|---|---|
| Andy Lee | Co-Founder | not disclosed |
| Dave Waslen | Cofounder, CEO | [X](https://x.com/DaveWaslen) |
...
Wilder World competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Wilder World has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Wilder World is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Wilder World compounds usage, liquidity improves, and WILD gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If WILD does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $117.54K | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Wilder World announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Wilder World has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Wilder World, the first reconciliation job is identity. The token symbol is WILD, the category is DeFi infrastructure, chains are Avalanche, Base, Binance Smart Chain, Ethereum, Polygon Pos, Solana, exchanges are BINANCE ALPHA, BITGET, and tags are GameFi, Metaverse, Web3 AI (Agent/AIGC). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If WILD trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $9.35M, FDV is $9.76M, 24h volume is $117.54K, FDV / market cap is 1.04x, 7d price change is 6.19%, and 30d price change is -3.09%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
For Wilder World, the current liquidity read is watchlist first. The market cap is $9.35M, FDV is $9.76M, reported 24h volume is $117.54K, and FDV / market cap is 1.04x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. Wilder World can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.

For Wilder World, the live monitoring stack should include contract events, TVL, volume, governance, documentation changes, social announcements, security disclosures, and exchange-market quality. A single metric is not enough. TVL may stay high while liquidity thins. Volume may rise because of volatility rather than adoption. Social attention may rise around controversy rather than progress. Governance may pass proposals that improve short-term price but add long-term risk. The strongest monitoring dashboard combines on-chain state, off-chain disclosure, and market quality.

Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If Wilder World suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | Wilder World gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.wilderworld.com/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Wilder%20World) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Wilder%20World) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Wilder%20World) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Wilder%20World) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Wilder%20World) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/WilderWorld) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/wilder_world) | Identity, market, social, contract, or discovery cross-check |
| 9 | Discord | [link](https://discord.com/invite/wilderworld) | Identity, market, social, contract, or discovery cross-check |
| 10 | Wilder World Price Chart (WILD) - CoinGecko | [link](https://www.coingecko.com/en/coins/wilder-world) | Identity, market, social, contract, or discovery cross-check |
| 11 | What Is Wilder World (WILD) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/wilder-world/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 12 | WILD to ETH: Wilder World Price in Ether / CoinGecko | [link](https://www.coingecko.com/en/coins/wilder-world/eth) | Identity, market, social, contract, or discovery cross-check |
| 13 | Wilder World (WILD) Price Prediction - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/wilder-world/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 14 | WILD to BTC: Wilder World Price in Bitcoin - CoinGecko | [link](https://www.coingecko.com/en/coins/wilder-world/btc) | Identity, market, social, contract, or discovery cross-check |
| 15 | base WILD | [link](https://basescan.org/token/0xafde2490236bc64950def5472296aa0d9758db0d) | Identity, market, social, contract, or discovery cross-check |
| 16 | solana WILD | [link](https://solscan.io/token/FVvd3s9dZYzsgitkJyWbmycSc8MkYZjyF7oqAEvmSxTZ) | Identity, market, social, contract, or discovery cross-check |
| 17 | ethereum WILD | [link](https://etherscan.io/token/0x2a3bff78b79a009976eea096a51a948a3dc00e34) | Identity, market, social, contract, or discovery cross-check |
| 18 | avalanche WILD | [link](https://snowtrace.io/token/0x153374c6d6786b6ca2c4bc96f9c3a471428f2bc7) | Identity, market, social, contract, or discovery cross-check |
| 19 | polygon WILD | [link](https://polygonscan.com/token/0xafde2490236bc64950def5472296aa0d9758db0d) | Identity, market, social, contract, or discovery cross-check |
| 20 | bsc WILD | [link](https://bscscan.com/token/0x6685906b75c61c57772c335402f594f855c1b0e3) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google project search | [link](https://www.google.com/search?q=Wilder%20World%20WILD%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google tokenomics search | [link](https://www.google.com/search?q=Wilder%20World%20WILD%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 23 | Google docs search | [link](https://www.google.com/search?q=Wilder%20World%20docs) | Identity, market, social, contract, or discovery cross-check |
| 24 | Google governance search | [link](https://www.google.com/search?q=Wilder%20World%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 25 | Google audit search | [link](https://www.google.com/search?q=Wilder%20World%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 26 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/wilder-world/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 27 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/wilder-world/) | Identity, market, social, contract, or discovery cross-check |
| 28 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Wilder%20World) | Identity, market, social, contract, or discovery cross-check |
| 29 | Dune token query | [link](https://dune.com/browse/queries?q=Wilder%20World) | Identity, market, social, contract, or discovery cross-check |
| 30 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Wilder%20World) | Identity, market, social, contract, or discovery cross-check |
| 31 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=WILD) | Identity, market, social, contract, or discovery cross-check |
| 32 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Wilder%20World) | Identity, market, social, contract, or discovery cross-check |
| 33 | CryptoRank search | [link](https://cryptorank.io/price/wilder-world) | Identity, market, social, contract, or discovery cross-check |
| 34 | RootData search | [link](https://www.rootdata.com/search?kw=Wilder%20World) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.wilderworld.com/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Wilder%20World)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Wilder%20World)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Wilder%20World)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Wilder%20World)
- [Messari discovery](https://messari.io/search?q=Wilder%20World)
- [X profile](https://x.com/WilderWorld)
- [Telegram](https://t.me/wilder_world)
- [Discord](https://discord.com/invite/wilderworld)
- [Wilder World Price Chart (WILD) - CoinGecko](https://www.coingecko.com/en/coins/wilder-world)
- [What Is Wilder World (WILD) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/wilder-world/what-is/)
- [WILD to ETH: Wilder World Price in Ether | CoinGecko](https://www.coingecko.com/en/coins/wilder-world/eth)
- [Wilder World (WILD) Price Prediction - CoinMarketCap](https://coinmarketcap.com/cmc-ai/wilder-world/price-prediction/)
- [WILD to BTC: Wilder World Price in Bitcoin - CoinGecko](https://www.coingecko.com/en/coins/wilder-world/btc)
- [base WILD](https://basescan.org/token/0xafde2490236bc64950def5472296aa0d9758db0d)
- [solana WILD](https://solscan.io/token/FVvd3s9dZYzsgitkJyWbmycSc8MkYZjyF7oqAEvmSxTZ)
- [ethereum WILD](https://etherscan.io/token/0x2a3bff78b79a009976eea096a51a948a3dc00e34)
- [avalanche WILD](https://snowtrace.io/token/0x153374c6d6786b6ca2c4bc96f9c3a471428f2bc7)
- [polygon WILD](https://polygonscan.com/token/0xafde2490236bc64950def5472296aa0d9758db0d)
- [bsc WILD](https://bscscan.com/token/0x6685906b75c61c57772c335402f594f855c1b0e3)
- [Google project search](https://www.google.com/search?q=Wilder%20World%20WILD%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Wilder%20World%20WILD%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Wilder%20World%20docs)
- [Google governance search](https://www.google.com/search?q=Wilder%20World%20governance%20proposal)

## Final Investment View

Wilder World should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
