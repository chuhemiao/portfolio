---
title: 'FLOCK / FLOCK: privacy infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'FLOCK is treated as a privacy infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'flock-flock-privacy-infrastructure-token-value-capture-risk'
category: 'research'
---

# FLOCK / FLOCK: privacy infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** FLOCK appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Base; exchange exposure is UPBIT, COINBASE, BITHUMB, BYBIT, BINANCE ALPHA, BITGET; category tags are Privacy & ZK, Web3 AI (Agent/AIGC); current market cap is $14.02M; FDV is $34.10M; and 24h volume is $1.76M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag FLOCK before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 997, market cap $14,104,403, 24h volume $1,708,045. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

FLOCK is best understood as a **privacy infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether FLOCK captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
Base case: FLOCK remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning FLOCK into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

FLock.io is a decentralized AI platform that focuses on privacy-preserving federated learning, enabling secure and collaborative AI model training without sharing sensitive data. It utilizes blockchain technology to coordinate data holders and has partnerships, such as with CIMG Inc., for developing AI wellness products like 'LifeNode'. The platform aims to advance AI applications in sectors requiring high privacy, such as healthcare and public services.

Surf's project profile maps FLOCK to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.flock.io/](https://www.flock.io/) |
| X handle | [@flock_io](https://x.com/flock_io) |
| X followers | 183.45K |
...
| Supply quality | 411.17M circulating vs 1.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that FLOCK is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| base | FLOCK | 0x5ab3d4c385b400f3abb49e80de2faf6a88a7b691 | [explorer](https://basescan.org/token/0x5ab3d4c385b400f3abb49e80de2faf6a88a7b691) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For FLOCK, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 997, market cap $14,104,403, 24h volume $1,708,045. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $14.02M, FDV $34.10M, 24h volume $1.76M, and price $0.0341.
...
| Market cap | $14.02M | Liquid valuation proxy |
| FDV | $34.10M | Dilution-aware valuation proxy |
| FDV / market cap spread | 2.43x | Higher spread means more supply risk |
| X followers | 183.45K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for FLOCK | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether FLOCK is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. FLOCK currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 2.39% and 30d -12.31% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that FLOCK's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2024-12-19 | Undisclosed | $3.00M | Fenbushi Capital, Animoca Brands, OKX Ventures, Digital Currency Group (DCG), GSR, Gnosis, Faction, Bas1s Ventures |
| 2024-03-28 | Seed | $6.00M | OKX Ventures, Digital Currency Group (DCG), Inception capital, Volt Capital, Faction, Tagus Capital |

| Team member | Role | Link |
|---|---|---|
| Jiahao Sun | Founder, CEO | [X](https://x.com/0x7SUN) |
| Vincent Z. Wang | CFO | not disclosed |
| Sameeha Rehman | Chief of Staff | not disclosed |
...
FLOCK competes in the privacy infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether FLOCK has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: FLOCK is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | FLOCK compounds usage, liquidity improves, and FLOCK gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because FLOCK has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If FLOCK does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| FDV / market cap | 2.43x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if FLOCK announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. FLOCK has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For FLOCK, the first reconciliation job is identity. The token symbol is FLOCK, the category is privacy infrastructure, chains are Base, exchanges are UPBIT, COINBASE, BITHUMB, BYBIT, BINANCE ALPHA, BITGET, and tags are Privacy & ZK, Web3 AI (Agent/AIGC). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If FLOCK trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $14.02M, FDV is $34.10M, 24h volume is $1.76M, FDV / market cap is 2.43x, 7d price change is 2.39%, and 30d price change is -12.31%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | FLOCK / FLOCK | Match official docs, CG, CMC, Surf, contracts |
...
| Market cap | $14.02M | Compare Surf, CG, CMC, exchange pages |
| FDV | $34.10M | Reconcile total supply and unlock schedule |
...
For FLOCK, the current liquidity read is watchlist first. The market cap is $14.02M, FDV is $34.10M, reported 24h volume is $1.76M, and FDV / market cap is 2.43x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | FLOCK gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.flock.io/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=FLOCK) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=FLOCK) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=FLOCK) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=FLOCK) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=FLOCK) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/flock_io) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/flock_io_community) | Identity, market, social, contract, or discovery cross-check |
| 9 | Discord | [link](https://discord.gg/ay8MnJCg2W) | Identity, market, social, contract, or discovery cross-check |
| 10 | FLOCK/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/flock-2) | Identity, market, social, contract, or discovery cross-check |
| 11 | FLOCK price today, FLOCK to USD live price, marketcap ... | [link](https://coinmarketcap.com/currencies/flock/) | Identity, market, social, contract, or discovery cross-check |
| 12 | What Is FLock.io (FLOCK) And How Does It Work? | [link](https://coinmarketcap.com/cmc-ai/flock-io/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 13 | Flockerz Price: FLOCK/USD Live Price Chart, Market Cap ... | [link](https://www.coingecko.com/en/coins/flockerz) | Identity, market, social, contract, or discovery cross-check |
| 14 | Token Allocations - FLock.io | [link](https://docs.flock.io/flock-tokenomics/token-allocations) | Identity, market, social, contract, or discovery cross-check |
| 15 | base FLOCK | [link](https://basescan.org/token/0x5ab3d4c385b400f3abb49e80de2faf6a88a7b691) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google project search | [link](https://www.google.com/search?q=FLOCK%20FLOCK%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google tokenomics search | [link](https://www.google.com/search?q=FLOCK%20FLOCK%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google docs search | [link](https://www.google.com/search?q=FLOCK%20docs) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google governance search | [link](https://www.google.com/search?q=FLOCK%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google audit search | [link](https://www.google.com/search?q=FLOCK%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/flock/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/flock/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune discovery | [link](https://dune.com/browse/dashboards?q=FLOCK) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune token query | [link](https://dune.com/browse/queries?q=FLOCK) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=FLOCK) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=FLOCK) | Identity, market, social, contract, or discovery cross-check |
| 27 | TokenUnlocks search | [link](https://token.unlocks.app/?search=FLOCK) | Identity, market, social, contract, or discovery cross-check |
| 28 | CryptoRank search | [link](https://cryptorank.io/price/flock) | Identity, market, social, contract, or discovery cross-check |
| 29 | RootData search | [link](https://www.rootdata.com/search?kw=FLOCK) | Identity, market, social, contract, or discovery cross-check |
| 30 | CoinDesk search | [link](https://www.coindesk.com/search?s=FLOCK) | Identity, market, social, contract, or discovery cross-check |
| 31 | The Block search | [link](https://www.theblock.co/search?query=FLOCK) | Identity, market, social, contract, or discovery cross-check |
| 32 | GitHub search | [link](https://github.com/search?q=FLOCK&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 33 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=FLOCK) | Identity, market, social, contract, or discovery cross-check |
| 34 | BscScan token search | [link](https://bscscan.com/search?f=0&q=FLOCK) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.flock.io/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=FLOCK)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=FLOCK)
- [DefiLlama protocol discovery](https://defillama.com/search?query=FLOCK)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=FLOCK)
- [Messari discovery](https://messari.io/search?q=FLOCK)
- [X profile](https://x.com/flock_io)
- [Telegram](https://t.me/flock_io_community)
- [Discord](https://discord.gg/ay8MnJCg2W)
- [FLOCK/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/flock-2)
- [FLOCK price today, FLOCK to USD live price, marketcap ...](https://coinmarketcap.com/currencies/flock/)
- [What Is FLock.io (FLOCK) And How Does It Work?](https://coinmarketcap.com/cmc-ai/flock-io/what-is/)
- [Flockerz Price: FLOCK/USD Live Price Chart, Market Cap ...](https://www.coingecko.com/en/coins/flockerz)
- [Token Allocations - FLock.io](https://docs.flock.io/flock-tokenomics/token-allocations)
- [base FLOCK](https://basescan.org/token/0x5ab3d4c385b400f3abb49e80de2faf6a88a7b691)
- [Google project search](https://www.google.com/search?q=FLOCK%20FLOCK%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=FLOCK%20FLOCK%20tokenomics)
- [Google docs search](https://www.google.com/search?q=FLOCK%20docs)
- [Google governance search](https://www.google.com/search?q=FLOCK%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=FLOCK%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/flock/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/flock/price-analysis/)
- [Dune discovery](https://dune.com/browse/dashboards?q=FLOCK)
- [Dune token query](https://dune.com/browse/queries?q=FLOCK)

## Final Investment View

FLOCK should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
