---
title: 'DeepBook / DEEP: DEX / liquidity infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'DeepBook is treated as a DEX / liquidity infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'deepbook-deep-dex-liquidity-infrastructure-token-value-capture-risk'
category: 'research'
---

# DeepBook / DEEP: DEX / liquidity infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** DeepBook appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Sui; exchange exposure is UPBIT, BITHUMB, BYBIT, BITGET; category tags are DEX & AMM; current market cap is $46.97M; FDV is $187.89M; and 24h volume is $5.94M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag DeepBook before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 472, market cap $45,947,325, 24h volume $5,712,157. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

DeepBook is best understood as a **DEX / liquidity infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether DEEP captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
Base case: DeepBook remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning DEEP into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Fully on-chain central limit order book (CLOB) and liquidity layer on Sui blockchain for high-throughput trading; DEEP token used for fees, staking, incentives, and governance.

Surf's project profile maps DeepBook to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://deepbook.tech/](https://deepbook.tech/) |
| X handle | [@DeepBookonSui](https://x.com/DeepBookonSui) |
| X followers | 64.61K |
| Tags | DEX & AMM |
| Chains | Sui |
| Exchanges | UPBIT, BITHUMB, BYBIT, BITGET |
...
| Liquidity | $5.94M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
...
The strongest reason to add the report is not that DeepBook is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| Sui | DEEP | 0xdeeb7a4662eec9f2f3def03fb937a663dddaa2e215b8078a284d026b7946c270::deep::DEEP | not mapped |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For DeepBook, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 472, market cap $45,947,325, 24h volume $5,712,157. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $46.97M, FDV $187.89M, 24h volume $5.94M, and price $0.0188.
...
| 24h volume | $5.94M | Position sizing and exit liquidity proxy |
| Market cap | $46.97M | Liquid valuation proxy |
| FDV | $187.89M | Dilution-aware valuation proxy |
| FDV / market cap spread | 4.00x | Higher spread means more supply risk |
| X followers | 64.61K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for DeepBook | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether DEEP is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. DeepBook currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 16.86% and 30d 15.20% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that DeepBook's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2024-10-12 | Ieo | $30.00K | not disclosed |
...
DeepBook competes in the DEX / liquidity infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether DeepBook has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: DeepBook is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | DeepBook compounds usage, liquidity improves, and DEEP gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
| 24h volume | $5.94M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if DeepBook announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
|---|---|---|
| Identity | DeepBook / DEEP | Match official docs, CG, CMC, Surf, contracts |
...
For DeepBook, the current liquidity read is watchlist first. The market cap is $46.97M, FDV is $187.89M, reported 24h volume is $5.94M, and FDV / market cap is 4.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | DeepBook gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://deepbook.tech/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=DeepBook) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=DeepBook) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=DeepBook) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=DeepBook) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=DeepBook) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/DeepBookonSui) | Identity, market, social, contract, or discovery cross-check |
| 8 | Google project search | [link](https://www.google.com/search?q=DeepBook%20DEEP%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 9 | Google tokenomics search | [link](https://www.google.com/search?q=DeepBook%20DEEP%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 10 | Google docs search | [link](https://www.google.com/search?q=DeepBook%20docs) | Identity, market, social, contract, or discovery cross-check |
| 11 | Google governance search | [link](https://www.google.com/search?q=DeepBook%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 12 | Google audit search | [link](https://www.google.com/search?q=DeepBook%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 13 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/deepbook/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 14 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/deepbook/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 15 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/deepbook/) | Identity, market, social, contract, or discovery cross-check |
| 16 | Dune discovery | [link](https://dune.com/browse/dashboards?q=DeepBook) | Identity, market, social, contract, or discovery cross-check |
| 17 | Dune token query | [link](https://dune.com/browse/queries?q=DeepBook) | Identity, market, social, contract, or discovery cross-check |
| 18 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=DeepBook) | Identity, market, social, contract, or discovery cross-check |
| 19 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=DEEP) | Identity, market, social, contract, or discovery cross-check |
| 20 | TokenUnlocks search | [link](https://token.unlocks.app/?search=DeepBook) | Identity, market, social, contract, or discovery cross-check |
| 21 | CryptoRank search | [link](https://cryptorank.io/price/deepbook) | Identity, market, social, contract, or discovery cross-check |
| 22 | RootData search | [link](https://www.rootdata.com/search?kw=DeepBook) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinDesk search | [link](https://www.coindesk.com/search?s=DeepBook) | Identity, market, social, contract, or discovery cross-check |
| 24 | The Block search | [link](https://www.theblock.co/search?query=DeepBook) | Identity, market, social, contract, or discovery cross-check |
| 25 | GitHub search | [link](https://github.com/search?q=DeepBook&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 26 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=DEEP) | Identity, market, social, contract, or discovery cross-check |
| 27 | BscScan token search | [link](https://bscscan.com/search?f=0&q=DEEP) | Identity, market, social, contract, or discovery cross-check |
| 28 | Arbiscan token search | [link](https://arbiscan.io/search?f=0&q=DEEP) | Identity, market, social, contract, or discovery cross-check |
| 29 | BaseScan token search | [link](https://basescan.org/search?f=0&q=DEEP) | Identity, market, social, contract, or discovery cross-check |
| 30 | RWA.xyz search | [link](https://app.rwa.xyz/search?query=DeepBook) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://deepbook.tech/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=DeepBook)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=DeepBook)
- [DefiLlama protocol discovery](https://defillama.com/search?query=DeepBook)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=DeepBook)
- [Messari discovery](https://messari.io/search?q=DeepBook)
- [X profile](https://x.com/DeepBookonSui)
- [Google project search](https://www.google.com/search?q=DeepBook%20DEEP%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=DeepBook%20DEEP%20tokenomics)
- [Google docs search](https://www.google.com/search?q=DeepBook%20docs)
- [Google governance search](https://www.google.com/search?q=DeepBook%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=DeepBook%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/deepbook/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/deepbook/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/deepbook/)
- [Dune discovery](https://dune.com/browse/dashboards?q=DeepBook)
- [Dune token query](https://dune.com/browse/queries?q=DeepBook)
- [DeFiLlama fees discovery](https://defillama.com/fees?chain=All&search=DeepBook)
- [DeFiLlama yield discovery](https://defillama.com/yields?token=DEEP)
- [TokenUnlocks search](https://token.unlocks.app/?search=DeepBook)
- [CryptoRank search](https://cryptorank.io/price/deepbook)
- [RootData search](https://www.rootdata.com/search?kw=DeepBook)
- [CoinDesk search](https://www.coindesk.com/search?s=DeepBook)
- [The Block search](https://www.theblock.co/search?query=DeepBook)

## Final Investment View

DeepBook should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
