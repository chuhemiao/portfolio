---
title: 'Avantis / AVNT: DEX / liquidity infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Avantis is treated as a DEX / liquidity infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'avantis-avnt-dex-liquidity-infrastructure-token-value-capture-risk'
category: 'research'
---

# Avantis / AVNT: DEX / liquidity infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Avantis appears in the local CoinGecko/CMC-oriented candidate pool through [CoinGecko](https://www.coingecko.com/en/coins/avantis) and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Base; exchange exposure is BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BYBIT, BITGET; category tags are Derivatives (Perp/Option), DEX & AMM, RWA (Real World Assets); current market cap is $30.65M; FDV is $93.14M; and 24h volume is $6.11M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Avantis before this file was created, and the existing candidate note says: DefiLlama category Derivatives on Base; TVL $31,447,237. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Avantis is best understood as a **DEX / liquidity infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether AVNT captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
Base case: Avantis remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning AVNT into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Avantis is a decentralized perpetuals DEX providing cross-asset leveraged trading (crypto, forex, commodities, indices) with up to 500x leverage, zero-fee perps, and tranche-based LP risk management. Built on Base, it offers an SDK and docs and features loss-rebates and positive-slippage mechanisms to incentivize liquidity and balanced open interest.

Surf's project profile maps Avantis to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.avantisfi.com/trade](https://www.avantisfi.com/trade) |
| X handle | [@avantisfi](https://x.com/avantisfi) |
| X followers | 106.38K |
| Tags | Derivatives (Perp/Option), DEX & AMM, RWA (Real World Assets) |
| Chains | Base |
| Exchanges | BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BYBIT, BITGET |
| TGE status | post |
| CoinGecko ID | [avantis](https://www.coingecko.com/en/coins/avantis) |

The main product question is whether Avantis is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DEX / liquidity infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Avantis convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $6.11M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 329.11M circulating vs 1.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Avantis is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| base | AVNT | 0x696f9436b67233384889472cd7cd58a6fb5df4f1 | [explorer](https://basescan.org/token/0x696f9436b67233384889472cd7cd58a6fb5df4f1) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Avantis, the DeFi-oriented backlog note is the first signal: DefiLlama category Derivatives on Base; TVL $31,447,237. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $30.65M, FDV $93.14M, 24h volume $6.11M, and price $0.0932.
...
| 24h volume | $6.11M | Position sizing and exit liquidity proxy |
| Market cap | $30.65M | Liquid valuation proxy |
| FDV | $93.14M | Dilution-aware valuation proxy |
| FDV / market cap spread | 3.04x | Higher spread means more supply risk |
| X followers | 106.38K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Avantis | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether AVNT is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. Avantis should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that Avantis belongs to the DEX / liquidity infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $30.65M, FDV $93.14M, 24h volume $6.11M, price $0.0932, circulating supply 329.11M, and total supply 1.00B. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For Avantis, the candidate backlog records DefiLlama category Derivatives on Base; TVL $31,447,237. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Base, exchanges as BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BYBIT, BITGET, and tags as Derivatives (Perp/Option), DEX & AMM, RWA (Real World Assets). If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Avantis currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d -0.21% and 30d -9.10% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Avantis's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-06-03 | Series A | $8.00M | Pantera Capital, Founders Fund, Symbolic Capital, Flowdesk, SALT |
| 2023-09-26 | Seed | $4.00M | Pantera Capital, Galaxy, Founders Fund, Modular Capital, Base Ecosystem Fund |
| 2023-09-07 | Undisclosed | not disclosed | Base Ecosystem Fund |

| Team member | Role | Link |
|---|---|---|
| Harsehaj Singh | Co-Founder, CEO | [X](https://x.com/0xSehaj) |
| Brank D. | Co-Founder, CTO | [X](https://x.com/brank_dev) |
...
Avantis competes in the DEX / liquidity infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Avantis has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Avantis is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Avantis compounds usage, liquidity improves, and AVNT gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Avantis has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If AVNT does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $6.11M | Weekly | Volume below position-size threshold |
| FDV / market cap | 3.04x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Avantis announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Avantis has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Avantis, the first reconciliation job is identity. The token symbol is AVNT, the category is DEX / liquidity infrastructure, chains are Base, exchanges are BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BYBIT, BITGET, and tags are Derivatives (Perp/Option), DEX & AMM, RWA (Real World Assets). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If AVNT trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $30.65M, FDV is $93.14M, 24h volume is $6.11M, FDV / market cap is 3.04x, 7d price change is -0.21%, and 30d price change is -9.10%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Avantis / AVNT | Match official docs, CG, CMC, Surf, contracts |
...
For Avantis, the current liquidity read is watchlist first. The market cap is $30.65M, FDV is $93.14M, reported 24h volume is $6.11M, and FDV / market cap is 3.04x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Avantis gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.avantisfi.com/trade) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko profile | [link](https://www.coingecko.com/en/coins/avantis) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Avantis) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Avantis) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Avantis) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Avantis) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/avantisfi) | Identity, market, social, contract, or discovery cross-check |
| 8 | Discord | [link](https://discord.gg/avantisfi) | Identity, market, social, contract, or discovery cross-check |
| 9 | Avantis price today, AVNT to USD live price, marketcap ... | [link](https://coinmarketcap.com/currencies/avantis/) | Identity, market, social, contract, or discovery cross-check |
| 10 | What Is Avantis (AVNT) And How Does It Work? | [link](https://coinmarketcap.com/cmc-ai/avantis/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 11 | Latest Avantis (AVNT) News Update | [link](https://coinmarketcap.com/cmc-ai/avantis/latest-updates/) | Identity, market, social, contract, or discovery cross-check |
| 12 | Avantis (AVNT) / Tokenomics & Vesting Schedule | [link](https://tokenomist.ai/avantis) | Identity, market, social, contract, or discovery cross-check |
| 13 | base AVNT | [link](https://basescan.org/token/0x696f9436b67233384889472cd7cd58a6fb5df4f1) | Identity, market, social, contract, or discovery cross-check |
| 14 | Google project search | [link](https://www.google.com/search?q=Avantis%20AVNT%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 15 | Google tokenomics search | [link](https://www.google.com/search?q=Avantis%20AVNT%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google docs search | [link](https://www.google.com/search?q=Avantis%20docs) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google governance search | [link](https://www.google.com/search?q=Avantis%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google audit search | [link](https://www.google.com/search?q=Avantis%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 19 | CoinGecko historical data | [link](https://www.coingecko.com/en/coins/avantis/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 20 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/avantis/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 21 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Avantis) | Identity, market, social, contract, or discovery cross-check |
| 22 | Dune token query | [link](https://dune.com/browse/queries?q=Avantis) | Identity, market, social, contract, or discovery cross-check |
| 23 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Avantis) | Identity, market, social, contract, or discovery cross-check |
| 24 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=AVNT) | Identity, market, social, contract, or discovery cross-check |
| 25 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Avantis) | Identity, market, social, contract, or discovery cross-check |
| 26 | CryptoRank search | [link](https://cryptorank.io/price/avantis) | Identity, market, social, contract, or discovery cross-check |
| 27 | RootData search | [link](https://www.rootdata.com/search?kw=Avantis) | Identity, market, social, contract, or discovery cross-check |
| 28 | CoinDesk search | [link](https://www.coindesk.com/search?s=Avantis) | Identity, market, social, contract, or discovery cross-check |
| 29 | The Block search | [link](https://www.theblock.co/search?query=Avantis) | Identity, market, social, contract, or discovery cross-check |
| 30 | GitHub search | [link](https://github.com/search?q=Avantis&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 31 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=AVNT) | Identity, market, social, contract, or discovery cross-check |
| 32 | BscScan token search | [link](https://bscscan.com/search?f=0&q=AVNT) | Identity, market, social, contract, or discovery cross-check |
| 33 | Arbiscan token search | [link](https://arbiscan.io/search?f=0&q=AVNT) | Identity, market, social, contract, or discovery cross-check |
| 34 | BaseScan token search | [link](https://basescan.org/search?f=0&q=AVNT) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.avantisfi.com/trade)
- [CoinGecko profile](https://www.coingecko.com/en/coins/avantis)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Avantis)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Avantis)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Avantis)
- [Messari discovery](https://messari.io/search?q=Avantis)
- [X profile](https://x.com/avantisfi)
- [Discord](https://discord.gg/avantisfi)
- [Avantis price today, AVNT to USD live price, marketcap ...](https://coinmarketcap.com/currencies/avantis/)
- [What Is Avantis (AVNT) And How Does It Work?](https://coinmarketcap.com/cmc-ai/avantis/what-is/)
- [Latest Avantis (AVNT) News Update](https://coinmarketcap.com/cmc-ai/avantis/latest-updates/)
- [Avantis (AVNT) | Tokenomics & Vesting Schedule](https://tokenomist.ai/avantis)
- [base AVNT](https://basescan.org/token/0x696f9436b67233384889472cd7cd58a6fb5df4f1)
- [Google project search](https://www.google.com/search?q=Avantis%20AVNT%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Avantis%20AVNT%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Avantis%20docs)
- [Google governance search](https://www.google.com/search?q=Avantis%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Avantis%20audit%20report)
- [CoinGecko historical data](https://www.coingecko.com/en/coins/avantis/historical_data)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/avantis/price-analysis/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Avantis)
- [Dune token query](https://dune.com/browse/queries?q=Avantis)
- [DeFiLlama fees discovery](https://defillama.com/fees?chain=All&search=Avantis)
- [DeFiLlama yield discovery](https://defillama.com/yields?token=AVNT)
