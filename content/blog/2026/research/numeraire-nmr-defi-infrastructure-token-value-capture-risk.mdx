---
title: 'Numeraire / NMR: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Numeraire is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'numeraire-nmr-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# Numeraire / NMR: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Numeraire appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Energi, Ethereum; exchange exposure is BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BITGET; category tags are Data & Analytics, Quant & Trading Tools, DeSci (Decentralized Science); current market cap is $66.71M; FDV is $100.81M; and 24h volume is $3.63M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Numeraire before this file was created, and the existing candidate note says: Seeded 2026-06-28 from Surf market-ranking; rank 376, market cap $60,897,998, 24h volume $4,008,494. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Numeraire is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether NMR captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| FDV | $100.81M |
| 24h volume | $3.63M |
| Circulating supply | 7.04M |
| Total supply | 10.63M |
| 7d price change | 12.33% |
| 30d price change | 24.01% |
...
Base case: Numeraire remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning NMR into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Numeraire (NMR) is the ERC-20 utility token for Numerai, a crowdsourced AI hedge fund that runs machine-learning tournaments where data scientists submit stock-prediction models. NMR is used to stake model predictions and distribute rewards to top performers, aligning incentives between contributors and Numerai’s trading platform.

Surf's project profile maps Numeraire to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://numer.ai/](https://numer.ai/) |
| X handle | [@numerai](https://x.com/numerai) |
| X followers | 56.72K |
| Tags | Data & Analytics, Quant & Trading Tools, DeSci (Decentralized Science) |
| Chains | Energi, Ethereum |
| Exchanges | BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Numeraire is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DeFi infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Numeraire convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Supply quality | 7.04M circulating vs 10.63M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Numeraire is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

Numeraire's architecture should be evaluated through the job it performs. For a DeFi infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
|---|---|---|---|
| ethereum | NMR | 0x1776e1f26f98b1a5df9cd347953a26dd3cb46671 | [explorer](https://etherscan.io/token/0x1776e1f26f98b1a5df9cd347953a26dd3cb46671) |
| Energi | NMR | 0xd72922e849477a042a7e6dc84309f4bc1c1227a2 | not mapped |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Numeraire, the DeFi-oriented backlog note is the first signal: Seeded 2026-06-28 from Surf market-ranking; rank 376, market cap $60,897,998, 24h volume $4,008,494. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $66.71M, FDV $100.81M, 24h volume $3.63M, and price $9.4800.
...
| Market cap | $66.71M | Liquid valuation proxy |
| FDV | $100.81M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.51x | Higher spread means more supply risk |
| X followers | 56.72K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Numeraire | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether NMR is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. Numeraire should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that Numeraire belongs to the DeFi infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $66.71M, FDV $100.81M, 24h volume $3.63M, price $9.4800, circulating supply 7.04M, and total supply 10.63M. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For Numeraire, the candidate backlog records Seeded 2026-06-28 from Surf market-ranking; rank 376, market cap $60,897,998, 24h volume $4,008,494. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Energi, Ethereum, exchanges as BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BITGET, and tags as Data & Analytics, Quant & Trading Tools, DeSci (Decentralized Science). If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Numeraire currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 12.33% and 30d 24.01% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Numeraire's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-11-20 | Series C | $30.00M | Union Square Ventures (USV), Shine Capital, Paul Tudor Jones |
| 2020-03-06 | Undisclosed | $3.00M | Placeholder Ventures, Dragonfly, CoinFund, Union Square Ventures (USV) |
| 2019-03-21 | Undisclosed | $11.00M | Placeholder Ventures, Paradigm |
| 2016-12-12 | Undisclosed | $6.00M | Union Square Ventures (USV), First Round Capital |
| 2016-04-18 | Undisclosed | $1.50M | Naval Ravikant, Playfair Capital, Howard Morgan |

| Team member | Role | Link |
|---|---|---|
| Richard Craib | Founder, CEO | [X](https://twitter.com/richardcraib) |
| James Elford | CTO | not disclosed |
| Dan Spier | COO | not disclosed |
| Michael Oliver | Chief Scientist | [X](https://twitter.com/the_moliver) |
| Michael P. | Chief Data Officer | not disclosed |
| Joey Krug | Technical Advisor | [X](https://twitter.com/joeykrug) |
| Ash F. | Advisor | not disclosed |
| Matt Boyd | Advisor | not disclosed |
...
Numeraire competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Numeraire has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Numeraire is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Numeraire compounds usage, liquidity improves, and NMR gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If NMR does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| FDV / market cap | 1.51x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Numeraire announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Numeraire has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Numeraire, the first reconciliation job is identity. The token symbol is NMR, the category is DeFi infrastructure, chains are Energi, Ethereum, exchanges are BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BITGET, and tags are Data & Analytics, Quant & Trading Tools, DeSci (Decentralized Science). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If NMR trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $66.71M, FDV is $100.81M, 24h volume is $3.63M, FDV / market cap is 1.51x, 7d price change is 12.33%, and 30d price change is 24.01%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Numeraire / NMR | Match official docs, CG, CMC, Surf, contracts |
...
| Market cap | $66.71M | Compare Surf, CG, CMC, exchange pages |
| FDV | $100.81M | Reconcile total supply and unlock schedule |
...
For Numeraire, the current liquidity read is watchlist first. The market cap is $66.71M, FDV is $100.81M, reported 24h volume is $3.63M, and FDV / market cap is 1.51x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Numeraire gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://numer.ai/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Numeraire) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Numeraire) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Numeraire) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Numeraire) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Numeraire) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/numerai) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/numerai) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/NMR_Official) | Identity, market, social, contract, or discovery cross-check |
| 10 | Numeraire Price Chart (NMR) - CoinGecko | [link](https://www.coingecko.com/en/coins/numeraire) | Identity, market, social, contract, or discovery cross-check |
| 11 | What Is Numeraire (NMR) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/numeraire/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 12 | NMR to ETH: Numeraire Price in Ether - CoinGecko | [link](https://www.coingecko.com/en/coins/numeraire/eth) | Identity, market, social, contract, or discovery cross-check |
| 13 | NMR Deep Dive: Token Fundamentals, Recent Developments, and ... | [link](https://onekey.so/blog/ecosystem/nmr-deep-dive-token-fundamentals-recent-developments-and-possible-future-trajectories/?srsltid=AfmBOopcmBXwC4ah5vsamfF_yExxO5hmZE9blwnoxCkDWyJDQXWWEq9u) | Identity, market, social, contract, or discovery cross-check |
| 14 | Numeraire (NMR) Price Prediction For 2026 & Beyond | [link](https://coinmarketcap.com/cmc-ai/numeraire/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 15 | ethereum NMR | [link](https://etherscan.io/token/0x1776e1f26f98b1a5df9cd347953a26dd3cb46671) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google project search | [link](https://www.google.com/search?q=Numeraire%20NMR%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google tokenomics search | [link](https://www.google.com/search?q=Numeraire%20NMR%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google docs search | [link](https://www.google.com/search?q=Numeraire%20docs) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google governance search | [link](https://www.google.com/search?q=Numeraire%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google audit search | [link](https://www.google.com/search?q=Numeraire%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/numeraire/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/numeraire/) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Numeraire) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune token query | [link](https://dune.com/browse/queries?q=Numeraire) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Numeraire) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=NMR) | Identity, market, social, contract, or discovery cross-check |
| 27 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Numeraire) | Identity, market, social, contract, or discovery cross-check |
| 28 | CryptoRank search | [link](https://cryptorank.io/price/numeraire) | Identity, market, social, contract, or discovery cross-check |
| 29 | RootData search | [link](https://www.rootdata.com/search?kw=Numeraire) | Identity, market, social, contract, or discovery cross-check |
| 30 | CoinDesk search | [link](https://www.coindesk.com/search?s=Numeraire) | Identity, market, social, contract, or discovery cross-check |
| 31 | The Block search | [link](https://www.theblock.co/search?query=Numeraire) | Identity, market, social, contract, or discovery cross-check |
| 32 | GitHub search | [link](https://github.com/search?q=Numeraire&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 33 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=NMR) | Identity, market, social, contract, or discovery cross-check |
| 34 | BscScan token search | [link](https://bscscan.com/search?f=0&q=NMR) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://numer.ai/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Numeraire)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Numeraire)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Numeraire)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Numeraire)
- [Messari discovery](https://messari.io/search?q=Numeraire)
- [X profile](https://x.com/numerai)
- [GitHub](https://github.com/numerai)
- [Telegram](https://t.me/NMR_Official)
- [Numeraire Price Chart (NMR) - CoinGecko](https://www.coingecko.com/en/coins/numeraire)
- [What Is Numeraire (NMR) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/numeraire/what-is/)
- [NMR to ETH: Numeraire Price in Ether - CoinGecko](https://www.coingecko.com/en/coins/numeraire/eth)
- [NMR Deep Dive: Token Fundamentals, Recent Developments, and ...](https://onekey.so/blog/ecosystem/nmr-deep-dive-token-fundamentals-recent-developments-and-possible-future-trajectories/?srsltid=AfmBOopcmBXwC4ah5vsamfF_yExxO5hmZE9blwnoxCkDWyJDQXWWEq9u)
- [Numeraire (NMR) Price Prediction For 2026 & Beyond](https://coinmarketcap.com/cmc-ai/numeraire/price-prediction/)
- [ethereum NMR](https://etherscan.io/token/0x1776e1f26f98b1a5df9cd347953a26dd3cb46671)
- [Google project search](https://www.google.com/search?q=Numeraire%20NMR%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Numeraire%20NMR%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Numeraire%20docs)
- [Google governance search](https://www.google.com/search?q=Numeraire%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Numeraire%20audit%20report)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/numeraire/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/numeraire/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Numeraire)
- [Dune token query](https://dune.com/browse/queries?q=Numeraire)

## Final Investment View

Numeraire should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
