---
title: 'Vana / VANA: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Vana is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'vana-vana-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# Vana / VANA: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Vana appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Arbitrum One, Base, Binance Smart Chain, Ethereum, Optimistic Ethereum, Polygon Pos; exchange exposure is BINANCE, UPBIT, BITHUMB, BYBIT, BITGET; category tags are Layer1, Storage & Compute, Web3 AI (Agent/AIGC); current market cap is $37.31M; FDV is $145.37M; and 24h volume is $1.79M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Vana before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 550, market cap $37,360,013, 24h volume $1,587,079. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Vana is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether VANA captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Arbitrum One, Base, Binance Smart Chain, Ethereum, Optimistic Ethereum, Polygon Pos |
| Exchanges | BINANCE, UPBIT, BITHUMB, BYBIT, BITGET |
| Price | $1.2100 |
| Market cap | $37.31M |
| FDV | $145.37M |
| 24h volume | $1.79M |
| Circulating supply | 30.80M |
| Total supply | 120.00M |
| 7d price change | 7.05% |
| 30d price change | 11.70% |
...
Base case: Vana remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning VANA into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

EVM-compatible L1 for user-owned data and AI

Surf's project profile maps Vana to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.vana.org/](https://www.vana.org/) |
| X handle | [@vana](https://x.com/vana) |
| X followers | 290.58K |
| Tags | Layer1, Storage & Compute, Web3 AI (Agent/AIGC) |
| Chains | Arbitrum One, Base, Binance Smart Chain, Ethereum, Optimistic Ethereum, Polygon Pos |
| Exchanges | BINANCE, UPBIT, BITHUMB, BYBIT, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Vana is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DeFi infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Vana convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Supply quality | 30.80M circulating vs 120.00M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Vana is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Arbitrum One, Base, Binance Smart Chain, Ethereum, Optimistic Ethereum, Polygon Pos |
...
|---|---|---|---|
| base | VANA | 0x7ff7fa94b8b66ef313f7970d4eebd2cb3103a2c0 | [explorer](https://basescan.org/token/0x7ff7fa94b8b66ef313f7970d4eebd2cb3103a2c0) |
| ethereum | VANA | 0x7ff7fa94b8b66ef313f7970d4eebd2cb3103a2c0 | [explorer](https://etherscan.io/token/0x7ff7fa94b8b66ef313f7970d4eebd2cb3103a2c0) |
| polygon | VANA | 0x7ff7fa94b8b66ef313f7970d4eebd2cb3103a2c0 | [explorer](https://polygonscan.com/token/0x7ff7fa94b8b66ef313f7970d4eebd2cb3103a2c0) |
| arbitrum | VANA | 0x7ff7fa94b8b66ef313f7970d4eebd2cb3103a2c0 | [explorer](https://arbiscan.io/token/0x7ff7fa94b8b66ef313f7970d4eebd2cb3103a2c0) |
| bsc | VANA | 0x7ff7fa94b8b66ef313f7970d4eebd2cb3103a2c0 | [explorer](https://bscscan.com/token/0x7ff7fa94b8b66ef313f7970d4eebd2cb3103a2c0) |
| optimism | VANA | 0x7ff7fa94b8b66ef313f7970d4eebd2cb3103a2c0 | [explorer](https://optimistic.etherscan.io/token/0x7ff7fa94b8b66ef313f7970d4eebd2cb3103a2c0) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Vana, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 550, market cap $37,360,013, 24h volume $1,587,079. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $37.31M, FDV $145.37M, 24h volume $1.79M, and price $1.2100.
...
| Market cap | $37.31M | Liquid valuation proxy |
| FDV | $145.37M | Dilution-aware valuation proxy |
| FDV / market cap spread | 3.90x | Higher spread means more supply risk |
| X followers | 290.58K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Vana | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether VANA is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. Vana should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that Vana belongs to the DeFi infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $37.31M, FDV $145.37M, 24h volume $1.79M, price $1.2100, circulating supply 30.80M, and total supply 120.00M. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For Vana, the candidate backlog records Seeded 2026-07-06 from Surf market-ranking; rank 550, market cap $37,360,013, 24h volume $1,587,079. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Arbitrum One, Base, Binance Smart Chain, Ethereum, Optimistic Ethereum, Polygon Pos, exchanges as BINANCE, UPBIT, BITHUMB, BYBIT, BITGET, and tags as Layer1, Storage & Compute, Web3 AI (Agent/AIGC). If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Vana currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 7.05% and 30d 11.70% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Vana's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-02-24 | Strategic | not disclosed | YZi Labs (Prev. Binance Labs) |
| 2024-09-18 | Strategic | $5.00M | Coinbase Ventures |
| 2022-12-01 | Series A | $18.00M | Paradigm, Polychain Capital, Packy McCormick, Casey Caruso |
| 2021-12-01 | Seed | $2.00M | Polychain Capital, MH Ventures |

| Team member | Role | Link |
|---|---|---|
| Anna Kazlauskas | Founder, Core Contributor | [X](https://x.com/anna_kazlauskas) |
| Arthur Abal | Co-Founder, Managing Director | [X](https://x.com/artieart88) |
| 0xconfused | CTO | [X](https://x.com/0xconfused) |
| Colin Stevenson | Chief of Staff | not disclosed |
| Shannon Wells | Vice President | [X](https://x.com/shanvasion) |
...
Vana competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Vana has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Vana is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Vana compounds usage, liquidity improves, and VANA gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Vana has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If VANA does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| FDV / market cap | 3.90x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Vana announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Vana has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Vana, the first reconciliation job is identity. The token symbol is VANA, the category is DeFi infrastructure, chains are Arbitrum One, Base, Binance Smart Chain, Ethereum, Optimistic Ethereum, Polygon Pos, exchanges are BINANCE, UPBIT, BITHUMB, BYBIT, BITGET, and tags are Layer1, Storage & Compute, Web3 AI (Agent/AIGC). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If VANA trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $37.31M, FDV is $145.37M, 24h volume is $1.79M, FDV / market cap is 3.90x, 7d price change is 7.05%, and 30d price change is 11.70%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Vana / VANA | Match official docs, CG, CMC, Surf, contracts |
| Category | DeFi infrastructure | Confirm product mechanics and revenue model |
| Chains | Arbitrum One, Base, Binance Smart Chain, Ethereum, Optimistic Ethereum, Polygon Pos | Confirm canonical deployments and bridge assumptions |
| Exchanges | BINANCE, UPBIT, BITHUMB, BYBIT, BITGET | Check live order books and inactive pairs |
| Market cap | $37.31M | Compare Surf, CG, CMC, exchange pages |
| FDV | $145.37M | Reconcile total supply and unlock schedule |
...
For Vana, the current liquidity read is watchlist first. The market cap is $37.31M, FDV is $145.37M, reported 24h volume is $1.79M, and FDV / market cap is 3.90x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Vana gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.vana.org/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Vana) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Vana) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Vana) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Vana) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Vana) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/vana) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/vana-com) | Identity, market, social, contract, or discovery cross-check |
| 9 | Discord | [link](https://discord.gg/withvana) | Identity, market, social, contract, or discovery cross-check |
| 10 | VANA/USD Live Price Chart, Market Cap & News Today - CoinGecko | [link](https://www.coingecko.com/en/coins/vana) | Identity, market, social, contract, or discovery cross-check |
| 11 | Top Vana Ecosystem Coins by Market Cap / CoinGecko | [link](https://www.coingecko.com/en/categories/vana-ecosystem) | Identity, market, social, contract, or discovery cross-check |
| 12 | Latest Vana (VANA) Price Analysis - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/vana/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 13 | Vana Price, VANA Price, Live Charts, and Marketcap - Coinbase | [link](https://www.coinbase.com/price/vana) | Identity, market, social, contract, or discovery cross-check |
| 14 | Vana Blockchain: Coins, NFTs, Exchanges & More / CoinGecko | [link](https://www.coingecko.com/en/chains/vana) | Identity, market, social, contract, or discovery cross-check |
| 15 | base VANA | [link](https://basescan.org/token/0x7ff7fa94b8b66ef313f7970d4eebd2cb3103a2c0) | Identity, market, social, contract, or discovery cross-check |
| 16 | ethereum VANA | [link](https://etherscan.io/token/0x7ff7fa94b8b66ef313f7970d4eebd2cb3103a2c0) | Identity, market, social, contract, or discovery cross-check |
| 17 | polygon VANA | [link](https://polygonscan.com/token/0x7ff7fa94b8b66ef313f7970d4eebd2cb3103a2c0) | Identity, market, social, contract, or discovery cross-check |
| 18 | arbitrum VANA | [link](https://arbiscan.io/token/0x7ff7fa94b8b66ef313f7970d4eebd2cb3103a2c0) | Identity, market, social, contract, or discovery cross-check |
| 19 | bsc VANA | [link](https://bscscan.com/token/0x7ff7fa94b8b66ef313f7970d4eebd2cb3103a2c0) | Identity, market, social, contract, or discovery cross-check |
| 20 | optimism VANA | [link](https://optimistic.etherscan.io/token/0x7ff7fa94b8b66ef313f7970d4eebd2cb3103a2c0) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google project search | [link](https://www.google.com/search?q=Vana%20VANA%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google tokenomics search | [link](https://www.google.com/search?q=Vana%20VANA%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 23 | Google docs search | [link](https://www.google.com/search?q=Vana%20docs) | Identity, market, social, contract, or discovery cross-check |
| 24 | Google governance search | [link](https://www.google.com/search?q=Vana%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 25 | Google audit search | [link](https://www.google.com/search?q=Vana%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 26 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/vana/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 27 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/vana/) | Identity, market, social, contract, or discovery cross-check |
| 28 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Vana) | Identity, market, social, contract, or discovery cross-check |
| 29 | Dune token query | [link](https://dune.com/browse/queries?q=Vana) | Identity, market, social, contract, or discovery cross-check |
| 30 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Vana) | Identity, market, social, contract, or discovery cross-check |
| 31 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=VANA) | Identity, market, social, contract, or discovery cross-check |
| 32 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Vana) | Identity, market, social, contract, or discovery cross-check |
| 33 | CryptoRank search | [link](https://cryptorank.io/price/vana) | Identity, market, social, contract, or discovery cross-check |
| 34 | RootData search | [link](https://www.rootdata.com/search?kw=Vana) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.vana.org/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Vana)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Vana)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Vana)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Vana)
- [Messari discovery](https://messari.io/search?q=Vana)
- [X profile](https://x.com/vana)
- [GitHub](https://github.com/vana-com)
- [Discord](https://discord.gg/withvana)
- [VANA/USD Live Price Chart, Market Cap & News Today - CoinGecko](https://www.coingecko.com/en/coins/vana)
- [Top Vana Ecosystem Coins by Market Cap | CoinGecko](https://www.coingecko.com/en/categories/vana-ecosystem)
- [Latest Vana (VANA) Price Analysis - CoinMarketCap](https://coinmarketcap.com/cmc-ai/vana/price-analysis/)
- [Vana Price, VANA Price, Live Charts, and Marketcap - Coinbase](https://www.coinbase.com/price/vana)
- [Vana Blockchain: Coins, NFTs, Exchanges & More | CoinGecko](https://www.coingecko.com/en/chains/vana)
- [base VANA](https://basescan.org/token/0x7ff7fa94b8b66ef313f7970d4eebd2cb3103a2c0)
- [ethereum VANA](https://etherscan.io/token/0x7ff7fa94b8b66ef313f7970d4eebd2cb3103a2c0)
- [polygon VANA](https://polygonscan.com/token/0x7ff7fa94b8b66ef313f7970d4eebd2cb3103a2c0)
- [arbitrum VANA](https://arbiscan.io/token/0x7ff7fa94b8b66ef313f7970d4eebd2cb3103a2c0)
- [bsc VANA](https://bscscan.com/token/0x7ff7fa94b8b66ef313f7970d4eebd2cb3103a2c0)
- [optimism VANA](https://optimistic.etherscan.io/token/0x7ff7fa94b8b66ef313f7970d4eebd2cb3103a2c0)
- [Google project search](https://www.google.com/search?q=Vana%20VANA%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Vana%20VANA%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Vana%20docs)
- [Google governance search](https://www.google.com/search?q=Vana%20governance%20proposal)

## Final Investment View

Vana should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
