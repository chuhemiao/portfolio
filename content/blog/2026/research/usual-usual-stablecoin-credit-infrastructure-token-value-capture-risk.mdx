---
title: 'Usual / USUAL: stablecoin / credit infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Usual is treated as a stablecoin / credit infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'usual-usual-stablecoin-credit-infrastructure-token-value-capture-risk'
category: 'research'
---

# Usual / USUAL: stablecoin / credit infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Usual appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Base, Binance Smart Chain, Ethereum; exchange exposure is BINANCE, BITGET; category tags are RWA (Real World Assets), Stablecoins, Yield & Asset Mgmt; current market cap is $16.63M; FDV is $17.24M; and 24h volume is $2.75M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Usual before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 918, market cap $16,766,371, 24h volume $2,904,329. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Usual is best understood as a **stablecoin / credit infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether USUAL captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| FDV | $17.24M |
| 24h volume | $2.75M |
| Circulating supply | 1.79B |
| Total supply | 1.86B |
| 7d price change | -1.67% |
| 30d price change | -0.19% |
| Latest TVL sample | $658.69M |
| TVL sample change | 2.55% |
| Primary source posture | Surf + CoinGecko + CMC discovery + official/project links |

Base case: Usual remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning USUAL into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Usual is a multi-chain RWA stablecoin protocol that issues USD0 (a permissionless, on‑chain fiat stablecoin) and the governance token USUAL. It aggregates tokenized real‑world assets to collateralize stablecoins and redistributes ownership and governance to users via the USUAL token, enabling composability and yield-oriented products.

Surf's project profile maps Usual to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://usual.money/](https://usual.money/) |
| X handle | [@usualmoney](https://x.com/usualmoney) |
| X followers | 115.30K |
| Tags | RWA (Real World Assets), Stablecoins, Yield & Asset Mgmt |
| Chains | Base, Binance Smart Chain, Ethereum |
| Exchanges | BINANCE, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Usual is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For stablecoin / credit infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Usual convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $2.75M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 1.79B circulating vs 1.86B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Usual is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

Usual's architecture should be evaluated through the job it performs. For a stablecoin / credit infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
| Settlement layer | Chains and contracts that hold user value | Base, Binance Smart Chain, Ethereum |
...
|---|---|---|---|
| base | USUAL | 0x4acd4d03af6f9cc0fb7c5f0868b7b6287d7969c5 | [explorer](https://basescan.org/token/0x4acd4d03af6f9cc0fb7c5f0868b7b6287d7969c5) |
| ethereum | USUAL | 0xc4441c2be5d8fa8126822b9929ca0b81ea0de38e | [explorer](https://etherscan.io/token/0xc4441c2be5d8fa8126822b9929ca0b81ea0de38e) |
| bsc | USUAL | 0x4acd4d03af6f9cc0fb7c5f0868b7b6287d7969c5 | [explorer](https://bscscan.com/token/0x4acd4d03af6f9cc0fb7c5f0868b7b6287d7969c5) |
| ethereum | ETH0 | 0x734eec7930bc84ec5732022b9eb949a81fb89abe | [explorer](https://etherscan.io/token/0x734eec7930bc84ec5732022b9eb949a81fb89abe) |
| ethereum | EUR0A | 0x8a90058b22e4411b44c67e0795ca4bcb2c6b2f21 | [explorer](https://etherscan.io/token/0x8a90058b22e4411b44c67e0795ca4bcb2c6b2f21) |
| ethereum | USD0A | 0x2e7fc02be94bc7f0cd69dcab572f64bcc173cd81 | [explorer](https://etherscan.io/token/0x2e7fc02be94bc7f0cd69dcab572f64bcc173cd81) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Usual, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 918, market cap $16,766,371, 24h volume $2,904,329. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $16.63M, FDV $17.24M, 24h volume $2.75M, and price $0.0093.

| Traction metric | Value | Interpretation |
|---|---|---|
| Latest TVL sample | $658.69M | Useful only if sustained without aggressive incentives |
| TVL sample points | 90 | More points improve trend confidence |
| TVL sample change | 2.55% | Positive trend supports usage; negative trend requires explanation |
| 24h volume | $2.75M | Position sizing and exit liquidity proxy |
| Market cap | $16.63M | Liquid valuation proxy |
| FDV | $17.24M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.04x | Higher spread means more supply risk |
| X followers | 115.30K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Usual | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether USUAL is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Usual currently needs to be monitored through that sequence. The available 90-day TVL series has 90 point(s), latest TVL $658.69M, and approximate change 2.55%. Market-price movement shows 7d -1.67% and 30d -0.19% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Usual's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2024-12-23 | Series A | $10.00M | Coinbase Ventures, YZi Labs (Prev. Binance Labs), OKX Ventures, Galaxy, IOSG Ventures, Hypersphere Ventures, GSR, Amber Group |
| 2024-11-06 | Undisclosed | $1.50M | Breed VC, echo, Alpen Capital (Prev. Comfy Capital) |
| 2024-04-17 | Strategic | $7.00M | X Ventures, IOSG Ventures, Hypersphere Ventures, TRGC, StarkWare, GSR Ventures, DCF God, Triton Capital (Prev. Kraken Ventures) |

| Team member | Role | Link |
|---|---|---|
| Adli Takkal Bataille | Design Director, Founder | [X](https://x.com/adlitb) |
| Hugo Salle de Chou | Co-Founder, COO | [X](https://x.com/hugosdc) |
| Pierre Person | CEO | [X](https://x.com/Pierr_Person) |
| Allan Floury | Vice President | [X](https://x.com/AllanFloury) |
...
Usual competes in the stablecoin / credit infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Usual has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Usual is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Usual compounds usage, liquidity improves, and USUAL gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Usual has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If USUAL does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
|---|---|---|---|
| TVL | $658.69M | Weekly | >20% drawdown or sustained growth |
| 24h volume | $2.75M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Usual announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Usual has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Usual, the first reconciliation job is identity. The token symbol is USUAL, the category is stablecoin / credit infrastructure, chains are Base, Binance Smart Chain, Ethereum, exchanges are BINANCE, BITGET, and tags are RWA (Real World Assets), Stablecoins, Yield & Asset Mgmt. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If USUAL trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $16.63M, FDV is $17.24M, 24h volume is $2.75M, FDV / market cap is 1.04x, 7d price change is -1.67%, and 30d price change is -0.19%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.

The third reconciliation job is usage data. The latest TVL sample is $658.69M, TVL sample change is 2.55%, and the sample contains 90 point(s). TVL is useful only when the underlying balances are real, withdrawable, and not double-counted. For stablecoin / credit infrastructure, the memo should identify whether TVL means deposits, staked assets, LP liquidity, stablecoin supply, collateral, locked governance assets, or a derived wrapper. A yield-bearing stablecoin and a validator middleware protocol can both show TVL, but the risk encoded by that TVL is completely different.

| Reconciliation item | Current value | Next check |
|---|---|---|
| Identity | Usual / USUAL | Match official docs, CG, CMC, Surf, contracts |
| Category | stablecoin / credit infrastructure | Confirm product mechanics and revenue model |
| Chains | Base, Binance Smart Chain, Ethereum | Confirm canonical deployments and bridge assumptions |
| Exchanges | BINANCE, BITGET | Check live order books and inactive pairs |
| Market cap | $16.63M | Compare Surf, CG, CMC, exchange pages |
| FDV | $17.24M | Reconcile total supply and unlock schedule |
| Volume | $2.75M | Check exchange concentration and wash-trading risk |
| TVL | $658.69M | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Usual does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Usual, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does USUAL sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
For Usual, the current liquidity read is watchlist first. The market cap is $16.63M, FDV is $17.24M, reported 24h volume is $2.75M, and FDV / market cap is 1.04x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Usual gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://usual.money/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Usual) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Usual) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Usual) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Usual) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Usual) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/usualmoney) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/UsualCommunity) | Identity, market, social, contract, or discovery cross-check |
| 9 | USUAL/USD Live Price Chart, Market Cap & News Today - CoinGecko | [link](https://www.coingecko.com/en/coins/usual) | Identity, market, social, contract, or discovery cross-check |
| 10 | CoinGecko: Cryptocurrency Prices, Charts, and Crypto Market Cap | [link](https://www.coingecko.com/) | Identity, market, social, contract, or discovery cross-check |
| 11 | What Is Usual (USUAL) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/usual/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 12 | Usual USD Price Chart (USD0) - CoinGecko | [link](https://www.coingecko.com/en/coins/usual-usd) | Identity, market, social, contract, or discovery cross-check |
| 13 | CoinMarketCap API Documentation | [link](https://coinmarketcap.com/api/documentation) | Identity, market, social, contract, or discovery cross-check |
| 14 | base USUAL | [link](https://basescan.org/token/0x4acd4d03af6f9cc0fb7c5f0868b7b6287d7969c5) | Identity, market, social, contract, or discovery cross-check |
| 15 | ethereum USUAL | [link](https://etherscan.io/token/0xc4441c2be5d8fa8126822b9929ca0b81ea0de38e) | Identity, market, social, contract, or discovery cross-check |
| 16 | bsc USUAL | [link](https://bscscan.com/token/0x4acd4d03af6f9cc0fb7c5f0868b7b6287d7969c5) | Identity, market, social, contract, or discovery cross-check |
| 17 | ethereum ETH0 | [link](https://etherscan.io/token/0x734eec7930bc84ec5732022b9eb949a81fb89abe) | Identity, market, social, contract, or discovery cross-check |
| 18 | ethereum EUR0A | [link](https://etherscan.io/token/0x8a90058b22e4411b44c67e0795ca4bcb2c6b2f21) | Identity, market, social, contract, or discovery cross-check |
| 19 | ethereum USD0A | [link](https://etherscan.io/token/0x2e7fc02be94bc7f0cd69dcab572f64bcc173cd81) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google project search | [link](https://www.google.com/search?q=Usual%20USUAL%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google tokenomics search | [link](https://www.google.com/search?q=Usual%20USUAL%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google docs search | [link](https://www.google.com/search?q=Usual%20docs) | Identity, market, social, contract, or discovery cross-check |
| 23 | Google governance search | [link](https://www.google.com/search?q=Usual%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 24 | Google audit search | [link](https://www.google.com/search?q=Usual%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 25 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/usual/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 26 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/usual/) | Identity, market, social, contract, or discovery cross-check |
| 27 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Usual) | Identity, market, social, contract, or discovery cross-check |
| 28 | Dune token query | [link](https://dune.com/browse/queries?q=Usual) | Identity, market, social, contract, or discovery cross-check |
| 29 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Usual) | Identity, market, social, contract, or discovery cross-check |
| 30 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=USUAL) | Identity, market, social, contract, or discovery cross-check |
| 31 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Usual) | Identity, market, social, contract, or discovery cross-check |
| 32 | CryptoRank search | [link](https://cryptorank.io/price/usual) | Identity, market, social, contract, or discovery cross-check |
| 33 | RootData search | [link](https://www.rootdata.com/search?kw=Usual) | Identity, market, social, contract, or discovery cross-check |
| 34 | CoinDesk search | [link](https://www.coindesk.com/search?s=Usual) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://usual.money/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Usual)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Usual)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Usual)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Usual)
- [Messari discovery](https://messari.io/search?q=Usual)
- [X profile](https://x.com/usualmoney)
- [Telegram](https://t.me/UsualCommunity)
- [USUAL/USD Live Price Chart, Market Cap & News Today - CoinGecko](https://www.coingecko.com/en/coins/usual)
- [CoinGecko: Cryptocurrency Prices, Charts, and Crypto Market Cap](https://www.coingecko.com/)
- [What Is Usual (USUAL) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/usual/what-is/)
- [Usual USD Price Chart (USD0) - CoinGecko](https://www.coingecko.com/en/coins/usual-usd)
- [CoinMarketCap API Documentation](https://coinmarketcap.com/api/documentation)
- [base USUAL](https://basescan.org/token/0x4acd4d03af6f9cc0fb7c5f0868b7b6287d7969c5)
- [ethereum USUAL](https://etherscan.io/token/0xc4441c2be5d8fa8126822b9929ca0b81ea0de38e)
- [bsc USUAL](https://bscscan.com/token/0x4acd4d03af6f9cc0fb7c5f0868b7b6287d7969c5)
- [ethereum ETH0](https://etherscan.io/token/0x734eec7930bc84ec5732022b9eb949a81fb89abe)
- [ethereum EUR0A](https://etherscan.io/token/0x8a90058b22e4411b44c67e0795ca4bcb2c6b2f21)
- [ethereum USD0A](https://etherscan.io/token/0x2e7fc02be94bc7f0cd69dcab572f64bcc173cd81)
- [Google project search](https://www.google.com/search?q=Usual%20USUAL%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Usual%20USUAL%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Usual%20docs)
- [Google governance search](https://www.google.com/search?q=Usual%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Usual%20audit%20report)

## Final Investment View

Usual should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
