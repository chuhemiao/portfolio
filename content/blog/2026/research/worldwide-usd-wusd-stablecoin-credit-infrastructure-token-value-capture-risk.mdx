---
title: 'Worldwide USD / WUSD: stablecoin / credit infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Worldwide USD is treated as a stablecoin / credit infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'worldwide-usd-wusd-stablecoin-credit-infrastructure-token-value-capture-risk'
category: 'research'
---

# Worldwide USD / WUSD: stablecoin / credit infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Worldwide USD appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Ethereum, Polygon Pos, Viction (TomoChain); exchange exposure is not disclosed; category tags are Payments & Gateway, Stablecoins, Wallets & AA; current market cap is $10.76M; FDV is $10.76M; and 24h volume is $127.76K. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Worldwide USD before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 1130, market cap $10,768,750, 24h volume $123,097. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Worldwide USD is best understood as a **stablecoin / credit infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether WUSD captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.

| Metric | Current read |
|---|---|
| Project | Worldwide USD |
| Token | WUSD |
| Category | stablecoin / credit infrastructure |
| Chains | Ethereum, Polygon Pos, Viction (TomoChain) |
| Exchanges | not disclosed |
| Price | $0.9970 |
| Market cap | $10.76M |
| FDV | $10.76M |
| 24h volume | $127.76K |
| Circulating supply | 10.80M |
| Total supply | 10.80M |
| 7d price change | 0.22% |
| 30d price change | -0.13% |
...
Base case: Worldwide USD remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning WUSD into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Worldwide USD (WUSD) is a fiat-collateralized stablecoin issued by Worldwide Stablecoin Payment Network (WSPN), pegged 1:1 to the US dollar and deployed across multiple blockchains. WSPN provides compliant on/off-ramps, multichain conversion, APIs and developer tools to enable enterprise-grade cross-border payments, remittances and programmable settlement.

Surf's project profile maps Worldwide USD to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://wspn.io/](https://wspn.io/) |
| X handle | [@WSPNpayment](https://x.com/WSPNpayment) |
| X followers | 24.32K |
| Tags | Payments & Gateway, Stablecoins, Wallets & AA |
| Chains | Ethereum, Polygon Pos, Viction (TomoChain) |
| Exchanges | not disclosed |
| TGE status | not disclosed |
| CoinGecko ID | not disclosed |

The main product question is whether Worldwide USD is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For stablecoin / credit infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Worldwide USD convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $127.76K 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 10.80M circulating vs 10.80M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Worldwide USD is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Ethereum, Polygon Pos, Viction (TomoChain) |
...
|---|---|---|---|
| ethereum | WUSD | 0x7cd017ca5ddb86861fa983a34b5f495c6f898c41 | [explorer](https://etherscan.io/token/0x7cd017ca5ddb86861fa983a34b5f495c6f898c41) |
| Viction (TomoChain) | WUSD | 0xba73e59f11597c1c13b0d9114688efb6a6d430f6 | not mapped |
| polygon | WUSD | 0x7cd017ca5ddb86861fa983a34b5f495c6f898c41 | [explorer](https://polygonscan.com/token/0x7cd017ca5ddb86861fa983a34b5f495c6f898c41) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Worldwide USD, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 1130, market cap $10,768,750, 24h volume $123,097. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $10.76M, FDV $10.76M, 24h volume $127.76K, and price $0.9970.
...
| 24h volume | $127.76K | Position sizing and exit liquidity proxy |
| Market cap | $10.76M | Liquid valuation proxy |
| FDV | $10.76M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.00x | Higher spread means more supply risk |
| X followers | 24.32K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Worldwide USD | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether WUSD is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Worldwide USD currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 0.22% and 30d -0.13% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Worldwide USD's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2024-08-16 | Seed | $30.00M | Folius Ventures, Foresight Ventures, Hash Global, Yunqi Partners, Redpoint China Ventures, Generative Ventures |

| Team member | Role | Link |
|---|---|---|
| Raymond Yuan | Founder, CEO | [X](https://x.com/raymondcthgroup) |
| Austin Campbell | US CEO | [X](https://x.com/CampbellJAustin) |
| John Partridge | members of the board of directors | not disclosed |
...
Worldwide USD competes in the stablecoin / credit infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Worldwide USD has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Worldwide USD is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Worldwide USD compounds usage, liquidity improves, and WUSD gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If WUSD does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $127.76K | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Worldwide USD announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Worldwide USD has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Worldwide USD, the first reconciliation job is identity. The token symbol is WUSD, the category is stablecoin / credit infrastructure, chains are Ethereum, Polygon Pos, Viction (TomoChain), exchanges are not disclosed, and tags are Payments & Gateway, Stablecoins, Wallets & AA. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If WUSD trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $10.76M, FDV is $10.76M, 24h volume is $127.76K, FDV / market cap is 1.00x, 7d price change is 0.22%, and 30d price change is -0.13%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
For Worldwide USD, the current liquidity read is watchlist first. The market cap is $10.76M, FDV is $10.76M, reported 24h volume is $127.76K, and FDV / market cap is 1.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. Worldwide USD can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.

For Worldwide USD, the live monitoring stack should include contract events, TVL, volume, governance, documentation changes, social announcements, security disclosures, and exchange-market quality. A single metric is not enough. TVL may stay high while liquidity thins. Volume may rise because of volatility rather than adoption. Social attention may rise around controversy rather than progress. Governance may pass proposals that improve short-term price but add long-term risk. The strongest monitoring dashboard combines on-chain state, off-chain disclosure, and market quality.

Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If Worldwide USD suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | Worldwide USD gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://wspn.io/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Worldwide%20USD) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Worldwide%20USD) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Worldwide%20USD) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Worldwide%20USD) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Worldwide%20USD) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/WSPNpayment) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/+-jaR0fB20zI1MzNl) | Identity, market, social, contract, or discovery cross-check |
| 9 | Worldwide USD Price Chart (WUSD) | [link](https://www.coingecko.com/en/coins/worldwide-usd) | Identity, market, social, contract, or discovery cross-check |
...
| 11 | Coins List with Market Data | [link](https://docs.coingecko.com/reference/coins-markets) | Identity, market, social, contract, or discovery cross-check |
| 12 | CoinMarketCap API Documentation | [link](https://coinmarketcap.com/api/documentation) | Identity, market, social, contract, or discovery cross-check |
| 13 | Worldwide USD (WUSD) Price, Coin Market Cap, & Token ... | [link](https://de.fi/token/worldwide-usd) | Identity, market, social, contract, or discovery cross-check |
| 14 | ethereum WUSD | [link](https://etherscan.io/token/0x7cd017ca5ddb86861fa983a34b5f495c6f898c41) | Identity, market, social, contract, or discovery cross-check |
| 15 | polygon WUSD | [link](https://polygonscan.com/token/0x7cd017ca5ddb86861fa983a34b5f495c6f898c41) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google project search | [link](https://www.google.com/search?q=Worldwide%20USD%20WUSD%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google tokenomics search | [link](https://www.google.com/search?q=Worldwide%20USD%20WUSD%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google docs search | [link](https://www.google.com/search?q=Worldwide%20USD%20docs) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google governance search | [link](https://www.google.com/search?q=Worldwide%20USD%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google audit search | [link](https://www.google.com/search?q=Worldwide%20USD%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/worldwide-usd/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/worldwide-usd/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/worldwide-usd/) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Worldwide%20USD) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune token query | [link](https://dune.com/browse/queries?q=Worldwide%20USD) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Worldwide%20USD) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=WUSD) | Identity, market, social, contract, or discovery cross-check |
| 28 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Worldwide%20USD) | Identity, market, social, contract, or discovery cross-check |
| 29 | CryptoRank search | [link](https://cryptorank.io/price/worldwide-usd) | Identity, market, social, contract, or discovery cross-check |
| 30 | RootData search | [link](https://www.rootdata.com/search?kw=Worldwide%20USD) | Identity, market, social, contract, or discovery cross-check |
| 31 | CoinDesk search | [link](https://www.coindesk.com/search?s=Worldwide%20USD) | Identity, market, social, contract, or discovery cross-check |
| 32 | The Block search | [link](https://www.theblock.co/search?query=Worldwide%20USD) | Identity, market, social, contract, or discovery cross-check |
| 33 | GitHub search | [link](https://github.com/search?q=Worldwide%20USD&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 34 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=WUSD) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://wspn.io/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Worldwide%20USD)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Worldwide%20USD)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Worldwide%20USD)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Worldwide%20USD)
- [Messari discovery](https://messari.io/search?q=Worldwide%20USD)
- [X profile](https://x.com/WSPNpayment)
- [Telegram](https://t.me/+-jaR0fB20zI1MzNl)
- [Worldwide USD Price Chart (WUSD)](https://www.coingecko.com/en/coins/worldwide-usd)
...
- [Worldwide USD (WUSD) Price, Coin Market Cap, & Token ...](https://de.fi/token/worldwide-usd)
- [ethereum WUSD](https://etherscan.io/token/0x7cd017ca5ddb86861fa983a34b5f495c6f898c41)
- [polygon WUSD](https://polygonscan.com/token/0x7cd017ca5ddb86861fa983a34b5f495c6f898c41)
- [Google project search](https://www.google.com/search?q=Worldwide%20USD%20WUSD%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Worldwide%20USD%20WUSD%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Worldwide%20USD%20docs)
- [Google governance search](https://www.google.com/search?q=Worldwide%20USD%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Worldwide%20USD%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/worldwide-usd/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/worldwide-usd/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/worldwide-usd/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Worldwide%20USD)

## Final Investment View

Worldwide USD should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
