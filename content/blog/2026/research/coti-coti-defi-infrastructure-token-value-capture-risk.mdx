---
title: 'COTI / COTI: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'COTI is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'coti-coti-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# COTI / COTI: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** COTI appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Cardano, Ethereum; exchange exposure is BINANCE, COINBASE, BITHUMB, BITGET; category tags are Layer2 & Scaling, Payments & Gateway; current market cap is $24.74M; FDV is $24.75M; and 24h volume is $4.60M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag COTI before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 739, market cap $24,258,042, 24h volume $4,585,594. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

COTI is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether COTI captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
Base case: COTI remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning COTI into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

COTI V2 is a privacy-focused Ethereum Layer 2 utilizing Garbled Circuits for fast, scalable confidential computation and private smart contracts. It enables privacy-preserving DeFi, transactions, and applications with auditability. The COTI token powers gas fees, staking, and governance.

Surf's project profile maps COTI to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://coti.io/](https://coti.io/) |
| X handle | [@COTInetwork](https://x.com/COTInetwork) |
| X followers | 264.45K |
| Tags | Layer2 & Scaling, Payments & Gateway |
| Chains | Cardano, Ethereum |
| Exchanges | BINANCE, COINBASE, BITHUMB, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether COTI is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DeFi infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does COTI convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $4.60M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 2.92B circulating vs 2.92B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that COTI is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Cardano, Ethereum |
...
|---|---|---|---|
| ethereum | COTI | 0xddb3422497e61e13543bea06989c0789117555c5 | [explorer](https://etherscan.io/token/0xddb3422497e61e13543bea06989c0789117555c5) |
| ethereum | GCOTI | 0xaf2ca40d3fc4459436d11b94d21fa4b8a89fb51d | [explorer](https://etherscan.io/token/0xaf2ca40d3fc4459436d11b94d21fa4b8a89fb51d) |
| Cardano | DJED | 8db269c3ec630e06ae29f74bc39edd1f87c819f1056206e879a1cd61446a65644d6963726f555344 | not mapped |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For COTI, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 739, market cap $24,258,042, 24h volume $4,585,594. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $24.74M, FDV $24.75M, 24h volume $4.60M, and price $0.0085.
...
| 24h volume | $4.60M | Position sizing and exit liquidity proxy |
| Market cap | $24.74M | Liquid valuation proxy |
| FDV | $24.75M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.00x | Higher spread means more supply risk |
| X followers | 264.45K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for COTI | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether COTI is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. COTI currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 1.69% and 30d -12.22% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that COTI's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2024-03-08 | Strategic | not disclosed | Morningstar Ventures |
| 2019-06-04 | Ieo | $3.00M | not disclosed |
| 2018-06-05 | Private/Pre-Sale | $16.02M | not disclosed |
| not disclosed | Strategic | not disclosed | not disclosed |
| not disclosed | Seed | $3.44M | not disclosed |

| Team member | Role | Link |
|---|---|---|
| Shahaf Bar-Geffen | CEO | [X](https://x.com/shahafbg) |
...
COTI competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether COTI has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: COTI is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | COTI compounds usage, liquidity improves, and COTI gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because COTI has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If COTI does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $4.60M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if COTI announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
For COTI, the current liquidity read is watchlist first. The market cap is $24.74M, FDV is $24.75M, reported 24h volume is $4.60M, and FDV / market cap is 1.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. COTI can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.
...
Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If COTI suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | COTI gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://coti.io/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=COTI) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=COTI) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=COTI) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=COTI) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=COTI) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/COTInetwork) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/coti-io) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/COTInetwork) | Identity, market, social, contract, or discovery cross-check |
| 10 | COTI/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/coti) | Identity, market, social, contract, or discovery cross-check |
| 11 | What Is COTI? Features, Tokenomics and Price Prediction | [link](https://coinmarketcap.com/academy/article/what-is-coti-features-tokenomics-and-price-prediction) | Identity, market, social, contract, or discovery cross-check |
| 12 | COTI Governance Token Price Chart (GCOTI) | [link](https://www.coingecko.com/en/coins/coti-governance-token) | Identity, market, social, contract, or discovery cross-check |
| 13 | COTI (COTI) Price Prediction For 2026 & Beyond | [link](https://coinmarketcap.com/cmc-ai/coti/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
...
| 15 | ethereum COTI | [link](https://etherscan.io/token/0xddb3422497e61e13543bea06989c0789117555c5) | Identity, market, social, contract, or discovery cross-check |
| 16 | ethereum GCOTI | [link](https://etherscan.io/token/0xaf2ca40d3fc4459436d11b94d21fa4b8a89fb51d) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google project search | [link](https://www.google.com/search?q=COTI%20COTI%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google tokenomics search | [link](https://www.google.com/search?q=COTI%20COTI%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google docs search | [link](https://www.google.com/search?q=COTI%20docs) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google governance search | [link](https://www.google.com/search?q=COTI%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google audit search | [link](https://www.google.com/search?q=COTI%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/coti/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/coti/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/coti/) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune discovery | [link](https://dune.com/browse/dashboards?q=COTI) | Identity, market, social, contract, or discovery cross-check |
| 26 | Dune token query | [link](https://dune.com/browse/queries?q=COTI) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=COTI) | Identity, market, social, contract, or discovery cross-check |
| 28 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=COTI) | Identity, market, social, contract, or discovery cross-check |
| 29 | TokenUnlocks search | [link](https://token.unlocks.app/?search=COTI) | Identity, market, social, contract, or discovery cross-check |
| 30 | CryptoRank search | [link](https://cryptorank.io/price/coti) | Identity, market, social, contract, or discovery cross-check |
| 31 | RootData search | [link](https://www.rootdata.com/search?kw=COTI) | Identity, market, social, contract, or discovery cross-check |
| 32 | CoinDesk search | [link](https://www.coindesk.com/search?s=COTI) | Identity, market, social, contract, or discovery cross-check |
| 33 | The Block search | [link](https://www.theblock.co/search?query=COTI) | Identity, market, social, contract, or discovery cross-check |
| 34 | GitHub search | [link](https://github.com/search?q=COTI&type=repositories) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://coti.io/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=COTI)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=COTI)
- [DefiLlama protocol discovery](https://defillama.com/search?query=COTI)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=COTI)
- [Messari discovery](https://messari.io/search?q=COTI)
- [X profile](https://x.com/COTInetwork)
- [GitHub](https://github.com/coti-io)
- [Telegram](https://t.me/COTInetwork)
- [COTI/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/coti)
- [What Is COTI? Features, Tokenomics and Price Prediction](https://coinmarketcap.com/academy/article/what-is-coti-features-tokenomics-and-price-prediction)
- [COTI Governance Token Price Chart (GCOTI)](https://www.coingecko.com/en/coins/coti-governance-token)
- [COTI (COTI) Price Prediction For 2026 & Beyond](https://coinmarketcap.com/cmc-ai/coti/price-prediction/)
- [CoinGecko: Cryptocurrency Prices, Charts, and Crypto Market ...](https://www.coingecko.com/)
- [ethereum COTI](https://etherscan.io/token/0xddb3422497e61e13543bea06989c0789117555c5)
- [ethereum GCOTI](https://etherscan.io/token/0xaf2ca40d3fc4459436d11b94d21fa4b8a89fb51d)
- [Google project search](https://www.google.com/search?q=COTI%20COTI%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=COTI%20COTI%20tokenomics)
- [Google docs search](https://www.google.com/search?q=COTI%20docs)
- [Google governance search](https://www.google.com/search?q=COTI%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=COTI%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/coti/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/coti/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/coti/)

## Final Investment View

COTI should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
