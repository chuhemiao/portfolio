---
title: 'Alien Worlds / TLM: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Alien Worlds is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'alien-worlds-tlm-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# Alien Worlds / TLM: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Alien Worlds appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Binance Smart Chain, Ethereum, WAX; exchange exposure is BINANCE, BITGET; category tags are DAO, GameFi, Metaverse; current market cap is $24.17M; FDV is $24.39M; and 24h volume is $165.78M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Alien Worlds before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 688, market cap $27,060,937, 24h volume $179,166,855. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Alien Worlds is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether TLM captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.

| Metric | Current read |
|---|---|
| Project | Alien Worlds |
| Token | TLM |
| Category | DeFi infrastructure |
| Chains | Binance Smart Chain, Ethereum, WAX |
| Exchanges | BINANCE, BITGET |
| Price | $0.0035 |
| Market cap | $24.17M |
| FDV | $24.39M |
| 24h volume | $165.78M |
| Circulating supply | 6.97B |
| Total supply | 7.03B |
| 7d price change | 293.34% |
| 30d price change | 222.05% |
...
Base case: Alien Worlds remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning TLM into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Alien Worlds is a community-built NFT metaverse and play-to-earn GameFi ecosystem where players mine, stake, trade NFTs and compete for the native token Trilium (TLM). Planets operate as DAOs for governance, and the project supports cross-chain assets (WAX, Ethereum, BSC) and funds third-party builders via its Galactic Hubs grants program.

Surf's project profile maps Alien Worlds to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://alienworlds.io/](https://alienworlds.io/) |
| X handle | [@AlienWorlds](https://x.com/AlienWorlds) |
| X followers | 108.25K |
| Tags | DAO, GameFi, Metaverse |
| Chains | Binance Smart Chain, Ethereum, WAX |
| Exchanges | BINANCE, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Alien Worlds is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DeFi infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Alien Worlds convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $165.78M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 6.97B circulating vs 7.03B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Alien Worlds is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Binance Smart Chain, Ethereum, WAX |
...
|---|---|---|---|
| WAX | TLM | TLM-wax-alien.worlds | not mapped |
| ethereum | TLM | 0x888888848b652b3e3a0f34c96e00eec0f3a23f72 | [explorer](https://etherscan.io/token/0x888888848b652b3e3a0f34c96e00eec0f3a23f72) |
| bsc | TLM | 0x2222227e22102fe3322098e4cbfe18cfebd57c95 | [explorer](https://bscscan.com/token/0x2222227e22102fe3322098e4cbfe18cfebd57c95) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Alien Worlds, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 688, market cap $27,060,937, 24h volume $179,166,855. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $24.17M, FDV $24.39M, 24h volume $165.78M, and price $0.0035.
...
| 24h volume | $165.78M | Position sizing and exit liquidity proxy |
| Market cap | $24.17M | Liquid valuation proxy |
| FDV | $24.39M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.01x | Higher spread means more supply risk |
| X followers | 108.25K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Alien Worlds | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether TLM is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Alien Worlds currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 293.34% and 30d 222.05% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Alien Worlds's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2021-03-26 | Strategic | $2.00M | LD Capital, Animoca Brands, 3Commas, LongLing Capital |
| not disclosed | Seed | $1.48M | not disclosed |
| not disclosed | Private | $1.03M | not disclosed |

| Team member | Role | Link |
|---|---|---|
| Rob Allen | Co-Founder | not disclosed |
| Saro Mckenna | Co-Founder | not disclosed |
| Brendan Sullivan | Chief Product Officer | not disclosed |
...
Alien Worlds competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Alien Worlds has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Alien Worlds is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Alien Worlds compounds usage, liquidity improves, and TLM gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If TLM does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $165.78M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Alien Worlds announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Alien Worlds has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Alien Worlds, the first reconciliation job is identity. The token symbol is TLM, the category is DeFi infrastructure, chains are Binance Smart Chain, Ethereum, WAX, exchanges are BINANCE, BITGET, and tags are DAO, GameFi, Metaverse. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If TLM trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $24.17M, FDV is $24.39M, 24h volume is $165.78M, FDV / market cap is 1.01x, 7d price change is 293.34%, and 30d price change is 222.05%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
For Alien Worlds, the current liquidity read is watchlist first. The market cap is $24.17M, FDV is $24.39M, reported 24h volume is $165.78M, and FDV / market cap is 1.01x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. Alien Worlds can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.
...
Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If Alien Worlds suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | Alien Worlds gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://alienworlds.io/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Alien%20Worlds) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Alien%20Worlds) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Alien%20Worlds) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Alien%20Worlds) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Alien%20Worlds) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/AlienWorlds) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/AlienWorldsOfficial) | Identity, market, social, contract, or discovery cross-check |
| 9 | Discord | [link](https://discord.com/invite/alien-worlds-official-733591843568222298) | Identity, market, social, contract, or discovery cross-check |
| 10 | Alien Worlds Price Chart (TLM) - CoinGecko | [link](https://www.coingecko.com/en/coins/alien-worlds) | Identity, market, social, contract, or discovery cross-check |
| 11 | What Is Alien Worlds (TLM) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/alien-worlds/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 12 | What Is Alien Worlds (TLM)? Features, Tokenomics and Price ... | [link](https://coinmarketcap.com/academy/article/what-is-alien-worlds-tlm-features-tokenomics-and-price-prediction) | Identity, market, social, contract, or discovery cross-check |
| 13 | Alien Worlds Price History: Download TLM Historical Data / CoinGecko | [link](https://www.coingecko.com/en/coins/alien-worlds/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 14 | TLM to INR: Alien Worlds Price in Indian Rupee - CoinGecko | [link](https://www.coingecko.com/en/coins/alien-worlds/inr) | Identity, market, social, contract, or discovery cross-check |
| 15 | ethereum TLM | [link](https://etherscan.io/token/0x888888848b652b3e3a0f34c96e00eec0f3a23f72) | Identity, market, social, contract, or discovery cross-check |
| 16 | bsc TLM | [link](https://bscscan.com/token/0x2222227e22102fe3322098e4cbfe18cfebd57c95) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google project search | [link](https://www.google.com/search?q=Alien%20Worlds%20TLM%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google tokenomics search | [link](https://www.google.com/search?q=Alien%20Worlds%20TLM%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google docs search | [link](https://www.google.com/search?q=Alien%20Worlds%20docs) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google governance search | [link](https://www.google.com/search?q=Alien%20Worlds%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google audit search | [link](https://www.google.com/search?q=Alien%20Worlds%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/alien-worlds/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/alien-worlds/) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Alien%20Worlds) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune token query | [link](https://dune.com/browse/queries?q=Alien%20Worlds) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Alien%20Worlds) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=TLM) | Identity, market, social, contract, or discovery cross-check |
| 28 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Alien%20Worlds) | Identity, market, social, contract, or discovery cross-check |
| 29 | CryptoRank search | [link](https://cryptorank.io/price/alien-worlds) | Identity, market, social, contract, or discovery cross-check |
| 30 | RootData search | [link](https://www.rootdata.com/search?kw=Alien%20Worlds) | Identity, market, social, contract, or discovery cross-check |
| 31 | CoinDesk search | [link](https://www.coindesk.com/search?s=Alien%20Worlds) | Identity, market, social, contract, or discovery cross-check |
| 32 | The Block search | [link](https://www.theblock.co/search?query=Alien%20Worlds) | Identity, market, social, contract, or discovery cross-check |
| 33 | GitHub search | [link](https://github.com/search?q=Alien%20Worlds&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 34 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=TLM) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://alienworlds.io/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Alien%20Worlds)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Alien%20Worlds)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Alien%20Worlds)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Alien%20Worlds)
- [Messari discovery](https://messari.io/search?q=Alien%20Worlds)
- [X profile](https://x.com/AlienWorlds)
- [Telegram](https://t.me/AlienWorldsOfficial)
- [Discord](https://discord.com/invite/alien-worlds-official-733591843568222298)
- [Alien Worlds Price Chart (TLM) - CoinGecko](https://www.coingecko.com/en/coins/alien-worlds)
- [What Is Alien Worlds (TLM) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/alien-worlds/what-is/)
- [What Is Alien Worlds (TLM)? Features, Tokenomics and Price ...](https://coinmarketcap.com/academy/article/what-is-alien-worlds-tlm-features-tokenomics-and-price-prediction)
- [Alien Worlds Price History: Download TLM Historical Data | CoinGecko](https://www.coingecko.com/en/coins/alien-worlds/historical_data)
- [TLM to INR: Alien Worlds Price in Indian Rupee - CoinGecko](https://www.coingecko.com/en/coins/alien-worlds/inr)
- [ethereum TLM](https://etherscan.io/token/0x888888848b652b3e3a0f34c96e00eec0f3a23f72)
- [bsc TLM](https://bscscan.com/token/0x2222227e22102fe3322098e4cbfe18cfebd57c95)
- [Google project search](https://www.google.com/search?q=Alien%20Worlds%20TLM%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Alien%20Worlds%20TLM%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Alien%20Worlds%20docs)
- [Google governance search](https://www.google.com/search?q=Alien%20Worlds%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Alien%20Worlds%20audit%20report)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/alien-worlds/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/alien-worlds/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Alien%20Worlds)

## Final Investment View

Alien Worlds should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
