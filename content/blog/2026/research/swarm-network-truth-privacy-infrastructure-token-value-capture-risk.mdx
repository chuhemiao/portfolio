---
title: 'Swarm Network / TRUTH: privacy infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Swarm Network is treated as a privacy infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'swarm-network-truth-privacy-infrastructure-token-value-capture-risk'
category: 'research'
---

# Swarm Network / TRUTH: privacy infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Swarm Network appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Sui; exchange exposure is BINANCE ALPHA; category tags are Oracle, Privacy & ZK, Web3 AI (Agent/AIGC); current market cap is $23.97M; FDV is $114.95M; and 24h volume is $2.96M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Swarm Network before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 746, market cap $23,931,007, 24h volume $3,120,013. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Swarm Network is best understood as a **privacy infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether TRUTH captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.

| Metric | Current read |
|---|---|
| Project | Swarm Network |
| Token | TRUTH |
...
Base case: Swarm Network remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning TRUTH into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Swarm Network's Truth Protocol uses AI-agent swarms, human validators, and ZK proofs to verify real-time data on-chain, powering dApps. It provides no-code tools to create agent clusters and issues TRUTH token for governance and incentives.

Surf's project profile maps Swarm Network to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://swarmnetwork.ai/](https://swarmnetwork.ai/) |
| X handle | [@GetSwarmed](https://x.com/GetSwarmed) |
| X followers | 283.66K |
| Tags | Oracle, Privacy & ZK, Web3 AI (Agent/AIGC) |
| Chains | Sui |
| Exchanges | BINANCE ALPHA |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Swarm Network is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For privacy infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Swarm Network convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $2.96M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 2.09B circulating vs 10.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Swarm Network is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

Swarm Network's architecture should be evaluated through the job it performs. For a privacy infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
|---|---|---|---|
| Sui | TRUTH | 0x0a48f85a3905cfa49a652bdb074d9e9fabad27892d54afaa5c9e0adeb7ac3cdf::swarm_network_token::SWARM_NETWORK_TOKEN | not mapped |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Swarm Network, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 746, market cap $23,931,007, 24h volume $3,120,013. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $23.97M, FDV $114.95M, 24h volume $2.96M, and price $0.0115.
...
| 24h volume | $2.96M | Position sizing and exit liquidity proxy |
| Market cap | $23.97M | Liquid valuation proxy |
| FDV | $114.95M | Dilution-aware valuation proxy |
| FDV / market cap spread | 4.80x | Higher spread means more supply risk |
| X followers | 283.66K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Swarm Network | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether TRUTH is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Swarm Network currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d -27.93% and 30d 8.80% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Swarm Network's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-08-27 | Pre-Sale | $10.00M | not disclosed |
| 2025-01-23 | Seed | $3.00M | GHAF Capital Partners, y2z Ventures, Brinc, Sui Foundation, ZeroStage |

| Team member | Role | Link |
|---|---|---|
| Yannick Myson | Founder, CEO | [X](https://x.com/YannickMyson) |
| Queena Tsai | Co-Founder, COO | [X](https://x.com/queenatsai_ai) |
| Morty Tabibi | Co-Founder, CTO | not disclosed |
| Fuad Fatu | Co-Founder, CMO | not disclosed |
| Andrew Durgee | Advisor | not disclosed |
| Kevin O'Hara | Advisor | [X](https://twitter.com/Kevinohara95) |
| Mark Klein | Advisor | not disclosed |
| Robin Janaway | CBDO | not disclosed |
...
Swarm Network competes in the privacy infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Swarm Network has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Swarm Network is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Swarm Network compounds usage, liquidity improves, and TRUTH gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If TRUTH does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $2.96M | Weekly | Volume below position-size threshold |
| FDV / market cap | 4.80x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Swarm Network announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Swarm Network has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Swarm Network, the first reconciliation job is identity. The token symbol is TRUTH, the category is privacy infrastructure, chains are Sui, exchanges are BINANCE ALPHA, and tags are Oracle, Privacy & ZK, Web3 AI (Agent/AIGC). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If TRUTH trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $23.97M, FDV is $114.95M, 24h volume is $2.96M, FDV / market cap is 4.80x, 7d price change is -27.93%, and 30d price change is 8.80%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Swarm Network / TRUTH | Match official docs, CG, CMC, Surf, contracts |
...
For Swarm Network, the current liquidity read is watchlist first. The market cap is $23.97M, FDV is $114.95M, reported 24h volume is $2.96M, and FDV / market cap is 4.80x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Swarm Network gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://swarmnetwork.ai/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Swarm%20Network) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Swarm%20Network) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Swarm%20Network) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Swarm%20Network) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Swarm%20Network) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/GetSwarmed) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/SwarmNews) | Identity, market, social, contract, or discovery cross-check |
| 9 | Swarm Network Price Chart (TRUTH) | [link](https://www.coingecko.com/en/coins/swarm-network-2) | Identity, market, social, contract, or discovery cross-check |
| 10 | What Is Swarm Network (TRUTH) And How Does It Work? | [link](https://coinmarketcap.com/cmc-ai/swarm-network-ai/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 11 | TRUTH to EUR: Swarm Network Price in Euro | [link](https://www.coingecko.com/en/coins/swarm-network-2/eur) | Identity, market, social, contract, or discovery cross-check |
| 12 | Swarm Network (TRUTH) Price Prediction | [link](https://coinmarketcap.com/cmc-ai/swarm-network-ai/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 13 | SWARM/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/the-swarm) | Identity, market, social, contract, or discovery cross-check |
| 14 | Google project search | [link](https://www.google.com/search?q=Swarm%20Network%20TRUTH%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 15 | Google tokenomics search | [link](https://www.google.com/search?q=Swarm%20Network%20TRUTH%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google docs search | [link](https://www.google.com/search?q=Swarm%20Network%20docs) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google governance search | [link](https://www.google.com/search?q=Swarm%20Network%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google audit search | [link](https://www.google.com/search?q=Swarm%20Network%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 19 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/swarm-network/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 20 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/swarm-network/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/swarm-network/) | Identity, market, social, contract, or discovery cross-check |
| 22 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Swarm%20Network) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune token query | [link](https://dune.com/browse/queries?q=Swarm%20Network) | Identity, market, social, contract, or discovery cross-check |
| 24 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Swarm%20Network) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=TRUTH) | Identity, market, social, contract, or discovery cross-check |
| 26 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Swarm%20Network) | Identity, market, social, contract, or discovery cross-check |
| 27 | CryptoRank search | [link](https://cryptorank.io/price/swarm-network) | Identity, market, social, contract, or discovery cross-check |
| 28 | RootData search | [link](https://www.rootdata.com/search?kw=Swarm%20Network) | Identity, market, social, contract, or discovery cross-check |
| 29 | CoinDesk search | [link](https://www.coindesk.com/search?s=Swarm%20Network) | Identity, market, social, contract, or discovery cross-check |
| 30 | The Block search | [link](https://www.theblock.co/search?query=Swarm%20Network) | Identity, market, social, contract, or discovery cross-check |
| 31 | GitHub search | [link](https://github.com/search?q=Swarm%20Network&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 32 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=TRUTH) | Identity, market, social, contract, or discovery cross-check |
| 33 | BscScan token search | [link](https://bscscan.com/search?f=0&q=TRUTH) | Identity, market, social, contract, or discovery cross-check |
| 34 | Arbiscan token search | [link](https://arbiscan.io/search?f=0&q=TRUTH) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://swarmnetwork.ai/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Swarm%20Network)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Swarm%20Network)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Swarm%20Network)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Swarm%20Network)
- [Messari discovery](https://messari.io/search?q=Swarm%20Network)
- [X profile](https://x.com/GetSwarmed)
- [Telegram](https://t.me/SwarmNews)
- [Swarm Network Price Chart (TRUTH)](https://www.coingecko.com/en/coins/swarm-network-2)
- [What Is Swarm Network (TRUTH) And How Does It Work?](https://coinmarketcap.com/cmc-ai/swarm-network-ai/what-is/)
- [TRUTH to EUR: Swarm Network Price in Euro](https://www.coingecko.com/en/coins/swarm-network-2/eur)
- [Swarm Network (TRUTH) Price Prediction](https://coinmarketcap.com/cmc-ai/swarm-network-ai/price-prediction/)
- [SWARM/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/the-swarm)
- [Google project search](https://www.google.com/search?q=Swarm%20Network%20TRUTH%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Swarm%20Network%20TRUTH%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Swarm%20Network%20docs)
- [Google governance search](https://www.google.com/search?q=Swarm%20Network%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Swarm%20Network%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/swarm-network/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/swarm-network/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/swarm-network/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Swarm%20Network)
- [Dune token query](https://dune.com/browse/queries?q=Swarm%20Network)
- [DeFiLlama fees discovery](https://defillama.com/fees?chain=All&search=Swarm%20Network)

## Final Investment View

Swarm Network should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
