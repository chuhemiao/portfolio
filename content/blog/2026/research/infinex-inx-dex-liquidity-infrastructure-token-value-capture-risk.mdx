---
title: 'Infinex / INX: DEX / liquidity infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Infinex is treated as a DEX / liquidity infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'infinex-inx-dex-liquidity-infrastructure-token-value-capture-risk'
category: 'research'
---

# Infinex / INX: DEX / liquidity infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Infinex appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Binance Smart Chain, Ethereum, Solana; exchange exposure is BINANCE ALPHA; category tags are DEX & AMM, Derivatives (Perp/Option), Wallets & AA (Account Abstraction); current market cap is $15.77M; FDV is $79.07M; and 24h volume is $3.76M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Infinex before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 942, market cap $15,736,818, 24h volume $3,694,720. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Infinex is best understood as a **DEX / liquidity infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether INX captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
Base case: Infinex remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning INX into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Infinex is a non-custodial, multi-chain crypto app and decentralized exchange that brings centralized-exchange UX on-chain via smart accounts, passkeys and a recoverable Vault for swaps, bridging, staking and (planned) derivatives. It issues Patron NFTs for governance and is led by Kain Warwick.

Surf's project profile maps Infinex to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://infinex.xyz/](https://infinex.xyz/) |
| X handle | [@infinex](https://x.com/infinex) |
| X followers | 91.24K |
| Tags | DEX & AMM, Derivatives (Perp/Option), Wallets & AA (Account Abstraction) |
| Chains | Binance Smart Chain, Ethereum, Solana |
| Exchanges | BINANCE ALPHA |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Infinex is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DEX / liquidity infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Infinex convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $3.76M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 1.99B circulating vs 10.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Infinex is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

Infinex's architecture should be evaluated through the job it performs. For a DEX / liquidity infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
|---|---|---|---|
| solana | INX | inxKXw9V2NDZE7hDijzpJaKKUb97NEPJDTCEEiYg4yY | [explorer](https://solscan.io/token/inxKXw9V2NDZE7hDijzpJaKKUb97NEPJDTCEEiYg4yY) |
| ethereum | INX | 0xdef1b2d939edc0e4d35806c59b3166f790175afe | [explorer](https://etherscan.io/token/0xdef1b2d939edc0e4d35806c59b3166f790175afe) |
| bsc | INX | 0x45f55b46689402583073ff227b6ac20520052a24 | [explorer](https://bscscan.com/token/0x45f55b46689402583073ff227b6ac20520052a24) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Infinex, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 942, market cap $15,736,818, 24h volume $3,694,720. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $15.77M, FDV $79.07M, 24h volume $3.76M, and price $0.0079.
...
| 24h volume | $3.76M | Position sizing and exit liquidity proxy |
| Market cap | $15.77M | Liquid valuation proxy |
| FDV | $79.07M | Dilution-aware valuation proxy |
| FDV / market cap spread | 5.01x | Higher spread means more supply risk |
| X followers | 91.24K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Infinex | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether INX is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Infinex currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d -3.99% and 30d 14.81% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Infinex's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-12-27 | Ico | $7.20M | not disclosed |
| 2024-11-11 | Private Token Sale | not disclosed | Near Foundation |
| 2024-09-25 | Undisclosed | $65.29M | Framework Ventures, Solana Ventures, Eden Block, Wintermute, Everest Ventures Group (EVG), Moonrock Capital, Alex Svanevik, Anatoly Yakovenko |

| Team member | Role | Link |
|---|---|---|
| Kain Warwick | Founder | [X](https://twitter.com/kaiynne) |
...
Infinex competes in the DEX / liquidity infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Infinex has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Infinex is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Infinex compounds usage, liquidity improves, and INX gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Infinex has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If INX does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $3.76M | Weekly | Volume below position-size threshold |
| FDV / market cap | 5.01x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Infinex announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Infinex has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Infinex, the first reconciliation job is identity. The token symbol is INX, the category is DEX / liquidity infrastructure, chains are Binance Smart Chain, Ethereum, Solana, exchanges are BINANCE ALPHA, and tags are DEX & AMM, Derivatives (Perp/Option), Wallets & AA (Account Abstraction). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If INX trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $15.77M, FDV is $79.07M, 24h volume is $3.76M, FDV / market cap is 5.01x, 7d price change is -3.99%, and 30d price change is 14.81%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Infinex / INX | Match official docs, CG, CMC, Surf, contracts |
...
For Infinex, the current liquidity read is watchlist first. The market cap is $15.77M, FDV is $79.07M, reported 24h volume is $3.76M, and FDV / market cap is 5.01x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Infinex gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://infinex.xyz/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Infinex) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Infinex) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Infinex) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Infinex) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Infinex) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/infinex) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/infinex-xyz) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/infinexbroadcast) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](https://discord.gg/infinex) | Identity, market, social, contract, or discovery cross-check |
| 11 | Infinex Price: INX/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/infinex) | Identity, market, social, contract, or discovery cross-check |
| 12 | INX to ETH: Infinex Price in Ether - CoinGecko | [link](https://www.coingecko.com/en/coins/infinex/eth) | Identity, market, social, contract, or discovery cross-check |
| 13 | What Is Infinex (INX) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/infinex/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 14 | INX to AUD: Infinex Price in Australian Dollar / CoinGecko | [link](https://www.coingecko.com/en/coins/infinex/aud) | Identity, market, social, contract, or discovery cross-check |
| 15 | Road to TGE: The Complete Guide - Infinex | [link](https://infinex.xyz/news/road-to-inx-tge) | Identity, market, social, contract, or discovery cross-check |
| 16 | solana INX | [link](https://solscan.io/token/inxKXw9V2NDZE7hDijzpJaKKUb97NEPJDTCEEiYg4yY) | Identity, market, social, contract, or discovery cross-check |
| 17 | ethereum INX | [link](https://etherscan.io/token/0xdef1b2d939edc0e4d35806c59b3166f790175afe) | Identity, market, social, contract, or discovery cross-check |
| 18 | bsc INX | [link](https://bscscan.com/token/0x45f55b46689402583073ff227b6ac20520052a24) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google project search | [link](https://www.google.com/search?q=Infinex%20INX%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google tokenomics search | [link](https://www.google.com/search?q=Infinex%20INX%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google docs search | [link](https://www.google.com/search?q=Infinex%20docs) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google governance search | [link](https://www.google.com/search?q=Infinex%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 23 | Google audit search | [link](https://www.google.com/search?q=Infinex%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/infinex/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 25 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/infinex/) | Identity, market, social, contract, or discovery cross-check |
| 26 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Infinex) | Identity, market, social, contract, or discovery cross-check |
| 27 | Dune token query | [link](https://dune.com/browse/queries?q=Infinex) | Identity, market, social, contract, or discovery cross-check |
| 28 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Infinex) | Identity, market, social, contract, or discovery cross-check |
| 29 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=INX) | Identity, market, social, contract, or discovery cross-check |
| 30 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Infinex) | Identity, market, social, contract, or discovery cross-check |
| 31 | CryptoRank search | [link](https://cryptorank.io/price/infinex) | Identity, market, social, contract, or discovery cross-check |
| 32 | RootData search | [link](https://www.rootdata.com/search?kw=Infinex) | Identity, market, social, contract, or discovery cross-check |
| 33 | CoinDesk search | [link](https://www.coindesk.com/search?s=Infinex) | Identity, market, social, contract, or discovery cross-check |
| 34 | The Block search | [link](https://www.theblock.co/search?query=Infinex) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://infinex.xyz/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Infinex)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Infinex)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Infinex)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Infinex)
- [Messari discovery](https://messari.io/search?q=Infinex)
- [X profile](https://x.com/infinex)
- [GitHub](https://github.com/infinex-xyz)
- [Telegram](https://t.me/infinexbroadcast)
- [Discord](https://discord.gg/infinex)
- [Infinex Price: INX/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/infinex)
- [INX to ETH: Infinex Price in Ether - CoinGecko](https://www.coingecko.com/en/coins/infinex/eth)
- [What Is Infinex (INX) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/infinex/what-is/)
- [INX to AUD: Infinex Price in Australian Dollar | CoinGecko](https://www.coingecko.com/en/coins/infinex/aud)
- [Road to TGE: The Complete Guide - Infinex](https://infinex.xyz/news/road-to-inx-tge)
- [solana INX](https://solscan.io/token/inxKXw9V2NDZE7hDijzpJaKKUb97NEPJDTCEEiYg4yY)
- [ethereum INX](https://etherscan.io/token/0xdef1b2d939edc0e4d35806c59b3166f790175afe)
- [bsc INX](https://bscscan.com/token/0x45f55b46689402583073ff227b6ac20520052a24)
- [Google project search](https://www.google.com/search?q=Infinex%20INX%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Infinex%20INX%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Infinex%20docs)
- [Google governance search](https://www.google.com/search?q=Infinex%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Infinex%20audit%20report)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/infinex/price-analysis/)

## Final Investment View

Infinex should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
