---
title: 'GoPlus Security / GPS: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'GoPlus Security is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'goplus-security-gps-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# GoPlus Security / GPS: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** GoPlus Security appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Base, Binance Smart Chain; exchange exposure is BINANCE, BITHUMB, BYBIT, BITGET; category tags are Developer Tooling, Security & Audit; current market cap is $42.39M; FDV is $96.14M; and 24h volume is $4.04M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag GoPlus Security before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 504, market cap $42,284,984, 24h volume $4,019,230. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

GoPlus Security is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether GPS captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.

| Metric | Current read |
|---|---|
| Project | GoPlus Security |
| Token | GPS |
...
Base case: GoPlus Security remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning GPS into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

GoPlus (GoPlus Security) builds a permissionless, user-driven Web3 security layer providing real-time scam, phishing and token risk detection via APIs, a browser extension, and apps. It delivers Token Security, Malicious Address, Transaction Simulation and SafeToken Locker/Protocol services across multiple chains and has an associated GPS token for ecosystem utility.

Surf's project profile maps GoPlus Security to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://gopluslabs.io/](https://gopluslabs.io/) |
| X handle | [@GoPlusSecurity](https://x.com/GoPlusSecurity) |
| X followers | 464.84K |
| Tags | Developer Tooling, Security & Audit |
| Chains | Base, Binance Smart Chain |
| Exchanges | BINANCE, BITHUMB, BYBIT, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether GoPlus Security is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DeFi infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does GoPlus Security convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $4.04M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 4.41B circulating vs 10.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that GoPlus Security is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

GoPlus Security's architecture should be evaluated through the job it performs. For a DeFi infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
|---|---|---|---|
| bsc | GPS | 0x9a4a67721573f2c9209dfff972c52be4e3f6642e | [explorer](https://bscscan.com/token/0x9a4a67721573f2c9209dfff972c52be4e3f6642e) |
| base | GPS | 0x0c1dc73159e30c4b06170f2593d3118968a0dca5 | [explorer](https://basescan.org/token/0x0c1dc73159e30c4b06170f2593d3118968a0dca5) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For GoPlus Security, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 504, market cap $42,284,984, 24h volume $4,019,230. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $42.39M, FDV $96.14M, 24h volume $4.04M, and price $0.0096.
...
| 24h volume | $4.04M | Position sizing and exit liquidity proxy |
| Market cap | $42.39M | Liquid valuation proxy |
| FDV | $96.14M | Dilution-aware valuation proxy |
| FDV / market cap spread | 2.27x | Higher spread means more supply risk |
| X followers | 464.84K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for GoPlus Security | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether GPS is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. GoPlus Security currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 6.13% and 30d 37.10% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that GoPlus Security's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-01-15 | Ieo | $50.00K | not disclosed |
| 2024-06-05 | Undisclosed | $10.00M | Fenbushi Capital, Animoca Brands, OKX Ventures, HashKey Capital, Quantstamp, Richard Ma, 7UpDAO, Redpoint China Ventures |
| 2022-12-08 | Strategic | not disclosed | YZi Labs (Prev. Binance Labs) |
| 2022-05-18 | Undisclosed | not disclosed | HTX Ventures, Crypto.com Capital, SevenX Ventures, GSR, Arweave, KuCoin Ventures, Avatar, Neo |
| 2021-08-01 | Angel | not disclosed | SevenX Ventures, Youbi Capital, LucidBlue Ventures, Puzzle Ventures, Mask Network, Incuba Alpha, Delta Blockchain Fund, Richard Ma |

| Team member | Role | Link |
|---|---|---|
| Mike | Founder | [X](https://x.com/mikelee205) |
| Eskil Xu | Co-Founder | not disclosed |
| Allen Zhang | Co-Founder, CTO | not disclosed |
...
GoPlus Security competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether GoPlus Security has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: GoPlus Security is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | GoPlus Security compounds usage, liquidity improves, and GPS gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If GPS does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $4.04M | Weekly | Volume below position-size threshold |
| FDV / market cap | 2.27x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if GoPlus Security announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. GoPlus Security has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For GoPlus Security, the first reconciliation job is identity. The token symbol is GPS, the category is DeFi infrastructure, chains are Base, Binance Smart Chain, exchanges are BINANCE, BITHUMB, BYBIT, BITGET, and tags are Developer Tooling, Security & Audit. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If GPS trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $42.39M, FDV is $96.14M, 24h volume is $4.04M, FDV / market cap is 2.27x, 7d price change is 6.13%, and 30d price change is 37.10%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | GoPlus Security / GPS | Match official docs, CG, CMC, Surf, contracts |
...
For GoPlus Security, the current liquidity read is watchlist first. The market cap is $42.39M, FDV is $96.14M, reported 24h volume is $4.04M, and FDV / market cap is 2.27x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. GoPlus Security can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.

For GoPlus Security, the live monitoring stack should include contract events, TVL, volume, governance, documentation changes, social announcements, security disclosures, and exchange-market quality. A single metric is not enough. TVL may stay high while liquidity thins. Volume may rise because of volatility rather than adoption. Social attention may rise around controversy rather than progress. Governance may pass proposals that improve short-term price but add long-term risk. The strongest monitoring dashboard combines on-chain state, off-chain disclosure, and market quality.

Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If GoPlus Security suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | GoPlus Security gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://gopluslabs.io/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=GoPlus%20Security) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=GoPlus%20Security) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=GoPlus%20Security) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=GoPlus%20Security) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=GoPlus%20Security) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/GoPlusSecurity) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/GoPlusSecurity) | Identity, market, social, contract, or discovery cross-check |
| 9 | Discord | [link](https://discord.com/invite/goplus) | Identity, market, social, contract, or discovery cross-check |
| 10 | GoPlus Security Price Chart (GPS) | [link](https://www.coingecko.com/en/coins/goplus-security) | Identity, market, social, contract, or discovery cross-check |
| 11 | Tokenomics / GoPlus Security | [link](https://whitepaper.gopluslabs.io/goplus-network/ecosystem-and-token/tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 12 | GoPlus Security (GPS) / Tokenomics & Vesting Schedule | [link](https://tokenomist.ai/goplus-security) | Identity, market, social, contract, or discovery cross-check |
| 13 | GoPlus Security (GPS) Price Prediction For 2026 & Beyond | [link](https://coinmarketcap.com/cmc-ai/goplus-security/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 14 | GoPlus Security Price History: Download GPS Historical Data | [link](https://www.coingecko.com/en/coins/goplus-security/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 15 | bsc GPS | [link](https://bscscan.com/token/0x9a4a67721573f2c9209dfff972c52be4e3f6642e) | Identity, market, social, contract, or discovery cross-check |
| 16 | base GPS | [link](https://basescan.org/token/0x0c1dc73159e30c4b06170f2593d3118968a0dca5) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google project search | [link](https://www.google.com/search?q=GoPlus%20Security%20GPS%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google tokenomics search | [link](https://www.google.com/search?q=GoPlus%20Security%20GPS%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google docs search | [link](https://www.google.com/search?q=GoPlus%20Security%20docs) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google governance search | [link](https://www.google.com/search?q=GoPlus%20Security%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google audit search | [link](https://www.google.com/search?q=GoPlus%20Security%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/goplus-security/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/goplus-security/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/goplus-security/) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune discovery | [link](https://dune.com/browse/dashboards?q=GoPlus%20Security) | Identity, market, social, contract, or discovery cross-check |
| 26 | Dune token query | [link](https://dune.com/browse/queries?q=GoPlus%20Security) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=GoPlus%20Security) | Identity, market, social, contract, or discovery cross-check |
| 28 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=GPS) | Identity, market, social, contract, or discovery cross-check |
| 29 | TokenUnlocks search | [link](https://token.unlocks.app/?search=GoPlus%20Security) | Identity, market, social, contract, or discovery cross-check |
| 30 | CryptoRank search | [link](https://cryptorank.io/price/goplus-security) | Identity, market, social, contract, or discovery cross-check |
| 31 | RootData search | [link](https://www.rootdata.com/search?kw=GoPlus%20Security) | Identity, market, social, contract, or discovery cross-check |
| 32 | CoinDesk search | [link](https://www.coindesk.com/search?s=GoPlus%20Security) | Identity, market, social, contract, or discovery cross-check |
| 33 | The Block search | [link](https://www.theblock.co/search?query=GoPlus%20Security) | Identity, market, social, contract, or discovery cross-check |
| 34 | GitHub search | [link](https://github.com/search?q=GoPlus%20Security&type=repositories) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://gopluslabs.io/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=GoPlus%20Security)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=GoPlus%20Security)
- [DefiLlama protocol discovery](https://defillama.com/search?query=GoPlus%20Security)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=GoPlus%20Security)
- [Messari discovery](https://messari.io/search?q=GoPlus%20Security)
- [X profile](https://x.com/GoPlusSecurity)
- [Telegram](https://t.me/GoPlusSecurity)
- [Discord](https://discord.com/invite/goplus)
- [GoPlus Security Price Chart (GPS)](https://www.coingecko.com/en/coins/goplus-security)
- [Tokenomics | GoPlus Security](https://whitepaper.gopluslabs.io/goplus-network/ecosystem-and-token/tokenomics)
- [GoPlus Security (GPS) | Tokenomics & Vesting Schedule](https://tokenomist.ai/goplus-security)
- [GoPlus Security (GPS) Price Prediction For 2026 & Beyond](https://coinmarketcap.com/cmc-ai/goplus-security/price-prediction/)
- [GoPlus Security Price History: Download GPS Historical Data](https://www.coingecko.com/en/coins/goplus-security/historical_data)
- [bsc GPS](https://bscscan.com/token/0x9a4a67721573f2c9209dfff972c52be4e3f6642e)
- [base GPS](https://basescan.org/token/0x0c1dc73159e30c4b06170f2593d3118968a0dca5)
- [Google project search](https://www.google.com/search?q=GoPlus%20Security%20GPS%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=GoPlus%20Security%20GPS%20tokenomics)
- [Google docs search](https://www.google.com/search?q=GoPlus%20Security%20docs)
- [Google governance search](https://www.google.com/search?q=GoPlus%20Security%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=GoPlus%20Security%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/goplus-security/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/goplus-security/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/goplus-security/)

## Final Investment View

GoPlus Security should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
