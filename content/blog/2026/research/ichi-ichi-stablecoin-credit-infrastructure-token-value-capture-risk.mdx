---
title: 'ICHI / ICHI: stablecoin / credit infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'ICHI is treated as a stablecoin / credit infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'ichi-ichi-stablecoin-credit-infrastructure-token-value-capture-risk'
category: 'research'
---

# ICHI / ICHI: stablecoin / credit infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** ICHI appears in the local CoinGecko/CMC-oriented candidate pool through [CoinGecko](https://www.coingecko.com/en/coins/ichi) and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Arbitrum One, Binance Smart Chain, Ethereum, Polygon Pos; exchange exposure is not disclosed; category tags are Yield & Asset Mgmt, Stablecoins; current market cap is $827.50K; FDV is $882.25K; and 24h volume is $151.5800. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag ICHI before this file was created, and the existing candidate note says: DefiLlama category Liquidity Manager on Multi-Chain; TVL $9,917,582. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

ICHI is best understood as a **stablecoin / credit infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether ICHI captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| FDV | $882.25K |
| 24h volume | $151.5800 |
| Circulating supply | 9.38M |
| Total supply | 10.00M |
| 7d price change | 3.36% |
| 30d price change | -3.56% |
...
Base case: ICHI remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning ICHI into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

ICHI is a non-custodial DeFi protocol that automates single-token liquidity provision via Vaults, deploying deposits into concentrated AMM pools to generate yield and deepen on-chain liquidity. It also offers Branded Dollars (community stablecoins), and ICHI (ICHI) functions as the protocol’s governance and value token.

Surf's project profile maps ICHI to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://app.ichi.org/](https://app.ichi.org/) |
| X handle | [@ichifoundation](https://x.com/ichifoundation) |
| X followers | 17.39K |
| Tags | Yield & Asset Mgmt, Stablecoins |
| Chains | Arbitrum One, Binance Smart Chain, Ethereum, Polygon Pos |
| Exchanges | not disclosed |
| TGE status | not disclosed |
| CoinGecko ID | [ichi](https://www.coingecko.com/en/coins/ichi) |

The main product question is whether ICHI is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For stablecoin / credit infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does ICHI convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $151.5800 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 9.38M circulating vs 10.00M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that ICHI is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| ethereum | ICHI | 0x111111517e4929d3dcbdfa7cce55d30d4b6bc4d6 | [explorer](https://etherscan.io/token/0x111111517e4929d3dcbdfa7cce55d30d4b6bc4d6) |
| polygon | ICHI | 0x111111517e4929d3dcbdfa7cce55d30d4b6bc4d6 | [explorer](https://polygonscan.com/token/0x111111517e4929d3dcbdfa7cce55d30d4b6bc4d6) |
| arbitrum | ICHI | 0xadf5dd3e51bf28ab4f07e684ecf5d00691818790 | [explorer](https://arbiscan.io/token/0xadf5dd3e51bf28ab4f07e684ecf5d00691818790) |
| bsc | ICHI | 0x0ef4a107b48163ab4b57fca36e1352151a587be4 | [explorer](https://bscscan.com/token/0x0ef4a107b48163ab4b57fca36e1352151a587be4) |
| ethereum | ICHI | 0x903bef1736cddf2a537176cf3c64579c3867a881 | [explorer](https://etherscan.io/token/0x903bef1736cddf2a537176cf3c64579c3867a881) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For ICHI, the DeFi-oriented backlog note is the first signal: DefiLlama category Liquidity Manager on Multi-Chain; TVL $9,917,582. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $827.50K, FDV $882.25K, 24h volume $151.5800, and price $0.0882.
...
| 24h volume | $151.5800 | Position sizing and exit liquidity proxy |
| Market cap | $827.50K | Liquid valuation proxy |
| FDV | $882.25K | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.07x | Higher spread means more supply risk |
| X followers | 17.39K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for ICHI | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether ICHI is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. ICHI currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 3.36% and 30d -3.56% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that ICHI's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2022-02-02 | Seed | $3.50M | Fundamental Labs, GSR, Lattice, TRGC, Lightshift Capital |
...
ICHI competes in the stablecoin / credit infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether ICHI has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: ICHI is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | ICHI compounds usage, liquidity improves, and ICHI gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because ICHI has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If ICHI does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $151.5800 | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if ICHI announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. ICHI has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For ICHI, the first reconciliation job is identity. The token symbol is ICHI, the category is stablecoin / credit infrastructure, chains are Arbitrum One, Binance Smart Chain, Ethereum, Polygon Pos, exchanges are not disclosed, and tags are Yield & Asset Mgmt, Stablecoins. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If ICHI trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $827.50K, FDV is $882.25K, 24h volume is $151.5800, FDV / market cap is 1.07x, 7d price change is 3.36%, and 30d price change is -3.56%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | ICHI / ICHI | Match official docs, CG, CMC, Surf, contracts |
...
For ICHI, the current liquidity read is watchlist first. The market cap is $827.50K, FDV is $882.25K, reported 24h volume is $151.5800, and FDV / market cap is 1.07x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | ICHI gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://app.ichi.org/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko profile | [link](https://www.coingecko.com/en/coins/ichi) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=ICHI) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=ICHI) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=ICHI) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=ICHI) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/ichifoundation) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/ichifarm) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/ichifarm) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](https://discord.com/invite/QNdAtAPb) | Identity, market, social, contract, or discovery cross-check |
| 11 | How-To - CoinGecko API | [link](https://docs.coingecko.com/docs/how-to) | Identity, market, social, contract, or discovery cross-check |
| 12 | Coins List with Market Data | [link](https://docs.coingecko.com/reference/coins-markets) | Identity, market, social, contract, or discovery cross-check |
| 13 | Coin Historical Chart Data by ID | [link](https://docs.coingecko.com/reference/coins-id-market-chart) | Identity, market, social, contract, or discovery cross-check |
...
| 15 | ethereum ICHI | [link](https://etherscan.io/token/0x111111517e4929d3dcbdfa7cce55d30d4b6bc4d6) | Identity, market, social, contract, or discovery cross-check |
| 16 | polygon ICHI | [link](https://polygonscan.com/token/0x111111517e4929d3dcbdfa7cce55d30d4b6bc4d6) | Identity, market, social, contract, or discovery cross-check |
| 17 | arbitrum ICHI | [link](https://arbiscan.io/token/0xadf5dd3e51bf28ab4f07e684ecf5d00691818790) | Identity, market, social, contract, or discovery cross-check |
| 18 | bsc ICHI | [link](https://bscscan.com/token/0x0ef4a107b48163ab4b57fca36e1352151a587be4) | Identity, market, social, contract, or discovery cross-check |
| 19 | ethereum ICHI | [link](https://etherscan.io/token/0x903bef1736cddf2a537176cf3c64579c3867a881) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google project search | [link](https://www.google.com/search?q=ICHI%20ICHI%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google tokenomics search | [link](https://www.google.com/search?q=ICHI%20ICHI%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google docs search | [link](https://www.google.com/search?q=ICHI%20docs) | Identity, market, social, contract, or discovery cross-check |
| 23 | Google governance search | [link](https://www.google.com/search?q=ICHI%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 24 | Google audit search | [link](https://www.google.com/search?q=ICHI%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 25 | CoinGecko historical data | [link](https://www.coingecko.com/en/coins/ichi/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 26 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/ichi/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 27 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/ichi/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 28 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/ichi/) | Identity, market, social, contract, or discovery cross-check |
| 29 | Dune discovery | [link](https://dune.com/browse/dashboards?q=ICHI) | Identity, market, social, contract, or discovery cross-check |
| 30 | Dune token query | [link](https://dune.com/browse/queries?q=ICHI) | Identity, market, social, contract, or discovery cross-check |
| 31 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=ICHI) | Identity, market, social, contract, or discovery cross-check |
| 32 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=ICHI) | Identity, market, social, contract, or discovery cross-check |
| 33 | TokenUnlocks search | [link](https://token.unlocks.app/?search=ICHI) | Identity, market, social, contract, or discovery cross-check |
| 34 | CryptoRank search | [link](https://cryptorank.io/price/ichi) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://app.ichi.org/)
- [CoinGecko profile](https://www.coingecko.com/en/coins/ichi)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=ICHI)
- [DefiLlama protocol discovery](https://defillama.com/search?query=ICHI)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=ICHI)
- [Messari discovery](https://messari.io/search?q=ICHI)
- [X profile](https://x.com/ichifoundation)
- [GitHub](https://github.com/ichifarm)
- [Telegram](https://t.me/ichifarm)
- [Discord](https://discord.com/invite/QNdAtAPb)
- [How-To - CoinGecko API](https://docs.coingecko.com/docs/how-to)
...
- [ethereum ICHI](https://etherscan.io/token/0x111111517e4929d3dcbdfa7cce55d30d4b6bc4d6)
- [polygon ICHI](https://polygonscan.com/token/0x111111517e4929d3dcbdfa7cce55d30d4b6bc4d6)
- [arbitrum ICHI](https://arbiscan.io/token/0xadf5dd3e51bf28ab4f07e684ecf5d00691818790)
- [bsc ICHI](https://bscscan.com/token/0x0ef4a107b48163ab4b57fca36e1352151a587be4)
- [ethereum ICHI](https://etherscan.io/token/0x903bef1736cddf2a537176cf3c64579c3867a881)
- [Google project search](https://www.google.com/search?q=ICHI%20ICHI%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=ICHI%20ICHI%20tokenomics)
- [Google docs search](https://www.google.com/search?q=ICHI%20docs)
- [Google governance search](https://www.google.com/search?q=ICHI%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=ICHI%20audit%20report)

## Final Investment View

ICHI should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
