---
title: 'Renzo / REZ: Liquid restaking value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Renzo is treated as a Liquid restaking watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'renzo-rez-liquid-restaking-token-value-capture-risk'
category: 'research'
---

# Renzo / REZ: Liquid restaking value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Renzo appears in the local CoinGecko/CMC-oriented candidate pool through [CoinGecko](https://www.coingecko.com/en/coins/renzo) and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Arbitrum One, Base, Binance Smart Chain, Blast, Ethereum, Fraxtal, Ink, Linea, Mode, Movement, Solana, Unichain; exchange exposure is BINANCE, COINBASE, BITHUMB, BITGET; category tags are LSD & Restaking, Yield & Asset Mgmt; current market cap is $25.06M; FDV is $28.36M; and 24h volume is $5.49M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Renzo before this file was created, and the existing candidate note says: DefiLlama category Liquid Restaking on Multi-Chain; TVL $88,620,405. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Renzo is best understood as a **Liquid restaking** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether REZ captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Arbitrum One, Base, Binance Smart Chain, Blast, Ethereum, Fraxtal, Ink, Linea, Mode, Movement, Solana, Unichain |
| Exchanges | BINANCE, COINBASE, BITHUMB, BITGET |
| Price | $0.0029 |
| Market cap | $25.06M |
| FDV | $28.36M |
| 24h volume | $5.49M |
| Circulating supply | 8.67B |
| Total supply | 9.81B |
| 7d price change | -5.70% |
| 30d price change | -9.83% |
...
Base case: Renzo remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning REZ into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Renzo is a liquid restaking protocol that mints LRTs (e.g., ezETH, pzETH, ezSOL) and programmatically manages AVS strategies across EigenLayer, Symbiotic, and Jito to simplify and auto‑compound staking/restaking. It also issues the governance token REZ and provides products like restaking, bridges, flow vaults, and instant withdrawals.

Surf's project profile maps Renzo to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://earn.renzoprotocol.com/](https://earn.renzoprotocol.com/) |
| X handle | [@RenzoAI](https://x.com/RenzoAI) |
| X followers | 110.61K |
| Tags | LSD & Restaking, Yield & Asset Mgmt |
| Chains | Arbitrum One, Base, Binance Smart Chain, Blast, Ethereum, Fraxtal, Ink, Linea, Mode, Movement, Solana, Unichain |
| Exchanges | BINANCE, COINBASE, BITHUMB, BITGET |
| TGE status | post |
| CoinGecko ID | [renzo](https://www.coingecko.com/en/coins/renzo) |

The main product question is whether Renzo is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For Liquid restaking projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Renzo convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $5.49M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 8.67B circulating vs 9.81B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Renzo is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

Renzo's architecture should be evaluated through the job it performs. For a Liquid restaking project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
| Settlement layer | Chains and contracts that hold user value | Arbitrum One, Base, Binance Smart Chain, Blast, Ethereum, Fraxtal, Ink, Linea, Mode, Movement, Solana, Unichain |
...
|---|---|---|---|
| base | REZ | 0xf757c9804cf2ee8d8ed64e0a8936293fe43a7252 | [explorer](https://basescan.org/token/0xf757c9804cf2ee8d8ed64e0a8936293fe43a7252) |
| solana | REZ | 3DK98MXPz8TRuim7rfQnebSLpA7VSoc79Bgiee1m4Zw5 | [explorer](https://solscan.io/token/3DK98MXPz8TRuim7rfQnebSLpA7VSoc79Bgiee1m4Zw5) |
| ethereum | REZ | 0x3b50805453023a91a8bf641e279401a0b23fa6f9 | [explorer](https://etherscan.io/token/0x3b50805453023a91a8bf641e279401a0b23fa6f9) |
| ethereum | PZETH | 0x8c9532a60e0e7c6bbd2b2c1303f63ace1c3e9811 | [explorer](https://etherscan.io/token/0x8c9532a60e0e7c6bbd2b2c1303f63ace1c3e9811) |
| solana | EZJTO | ezjtoE6PYVnWkbJJYEsGHgR3rXgBydi9eeB1YHP8cVg | [explorer](https://solscan.io/token/ezjtoE6PYVnWkbJJYEsGHgR3rXgBydi9eeB1YHP8cVg) |
| solana | EZSOL | ezSoL6fY1PVdJcJsUpe5CM3xkfmy3zoVCABybm5WtiC | [explorer](https://solscan.io/token/ezSoL6fY1PVdJcJsUpe5CM3xkfmy3zoVCABybm5WtiC) |
| solana | BZSOL | bzSoL4Ehh4AsaPdVD2ndkeHy9nmKnP8YRwVt3TfaJQq | [explorer](https://solscan.io/token/bzSoL4Ehh4AsaPdVD2ndkeHy9nmKnP8YRwVt3TfaJQq) |
| Ink | EZETH | 0x2416092f143378750bb29b79ed961ab195cceea5 | not mapped |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Renzo, the DeFi-oriented backlog note is the first signal: DefiLlama category Liquid Restaking on Multi-Chain; TVL $88,620,405. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $25.06M, FDV $28.36M, 24h volume $5.49M, and price $0.0029.
...
| 24h volume | $5.49M | Position sizing and exit liquidity proxy |
| Market cap | $25.06M | Liquid valuation proxy |
| FDV | $28.36M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.13x | Higher spread means more supply risk |
| X followers | 110.61K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Renzo | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether REZ is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. Renzo should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that Renzo belongs to the Liquid restaking bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $25.06M, FDV $28.36M, 24h volume $5.49M, price $0.0029, circulating supply 8.67B, and total supply 9.81B. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For Renzo, the candidate backlog records DefiLlama category Liquid Restaking on Multi-Chain; TVL $88,620,405. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Arbitrum One, Base, Binance Smart Chain, Blast, Ethereum, Fraxtal, Ink, Linea, Mode, Movement, Solana, Unichain, exchanges as BINANCE, COINBASE, BITHUMB, BITGET, and tags as LSD & Restaking, Yield & Asset Mgmt. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.

The fourth frame is value capture. A Liquid restaking asset can be useful while the token remains weak. The memo therefore separates product-market fit from token-market fit. Product-market fit asks whether users need the protocol even when incentives fade. Token-market fit asks whether the token absorbs value through staking, fee routing, collateral, governance constraints, emissions control, slashing insurance, or privileged access. If the token is mostly governance, the base case assigns lower confidence. If the token is used for security or fee capture but the mechanism is not yet activated, the base case treats value capture as an option rather than as current cash flow.

The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Renzo currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d -5.70% and 30d -9.83% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Renzo's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2024-06-18 | Undisclosed | $17.00M | Galaxy, SevenX Ventures, Maven 11 Capital, Figment Capital, ConsenSys, Antalpha Global, Brevan Howard Digital, Protagonist |
| 2024-02-22 | Undisclosed | not disclosed | YZi Labs (Prev. Binance Labs) |
| 2024-01-15 | Seed | $3.20M | OKX Ventures, Bitscale Capital, SevenX Ventures, Maven 11 Capital, IOSG Ventures, Robot Ventures, Figment Capital, Protagonist |

| Team member | Role | Link |
|---|---|---|
| Lucas Kozinski | Co-Founder | [X](https://twitter.com/LucasKozinski) |
| James Poole | Co-Founder | [X](https://twitter.com/JamesCPoole) |
| Kratik Lodha | Co-Founder | [X](https://twitter.com/kratikl_) |
| Ron Gierlach | Contributor  | not disclosed |
| Jatin Jain | Contributor  | not disclosed |
...
Renzo competes in the Liquid restaking bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Renzo has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Renzo is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Renzo compounds usage, liquidity improves, and REZ gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Renzo has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
| 24h volume | $5.49M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Renzo announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Renzo has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Renzo, the first reconciliation job is identity. The token symbol is REZ, the category is Liquid restaking, chains are Arbitrum One, Base, Binance Smart Chain, Blast, Ethereum, Fraxtal, Ink, Linea, Mode, Movement, Solana, Unichain, exchanges are BINANCE, COINBASE, BITHUMB, BITGET, and tags are LSD & Restaking, Yield & Asset Mgmt. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If REZ trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $25.06M, FDV is $28.36M, 24h volume is $5.49M, FDV / market cap is 1.13x, 7d price change is -5.70%, and 30d price change is -9.83%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.

The third reconciliation job is usage data. The latest TVL sample is not disclosed, TVL sample change is not disclosed, and the sample contains 0 point(s). TVL is useful only when the underlying balances are real, withdrawable, and not double-counted. For Liquid restaking, the memo should identify whether TVL means deposits, staked assets, LP liquidity, stablecoin supply, collateral, locked governance assets, or a derived wrapper. A yield-bearing stablecoin and a validator middleware protocol can both show TVL, but the risk encoded by that TVL is completely different.

| Reconciliation item | Current value | Next check |
|---|---|---|
| Identity | Renzo / REZ | Match official docs, CG, CMC, Surf, contracts |
| Category | Liquid restaking | Confirm product mechanics and revenue model |
| Chains | Arbitrum One, Base, Binance Smart Chain, Blast, Ethereum, Fraxtal, Ink, Linea, Mode, Movement, Solana, Unichain | Confirm canonical deployments and bridge assumptions |
| Exchanges | BINANCE, COINBASE, BITHUMB, BITGET | Check live order books and inactive pairs |
| Market cap | $25.06M | Compare Surf, CG, CMC, exchange pages |
| FDV | $28.36M | Reconcile total supply and unlock schedule |
| Volume | $5.49M | Check exchange concentration and wash-trading risk |
| TVL | not disclosed | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Renzo does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Renzo, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does REZ sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
| Correlation check | Count exposure to related Liquid restaking assets | Category beta can hide concentration |
...
| Peer comparison | Monthly | Renzo gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://earn.renzoprotocol.com/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko profile | [link](https://www.coingecko.com/en/coins/renzo) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Renzo) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Renzo) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Renzo) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Renzo) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/RenzoAI) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/Renzo-Protocol) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/RenzoProtocolChat) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](https://discord.com/invite/renzoprotocol) | Identity, market, social, contract, or discovery cross-check |
| 11 | Renzo price today, REZ to USD live ... | [link](https://coinmarketcap.com/currencies/renzo/) | Identity, market, social, contract, or discovery cross-check |
| 12 | Renzo (REZ) / Tokenomics & Vesting Schedule | [link](https://tokenomist.ai/renzo) | Identity, market, social, contract, or discovery cross-check |
| 13 | Latest Renzo (REZ) News Update | [link](https://coinmarketcap.com/cmc-ai/renzo/latest-updates/) | Identity, market, social, contract, or discovery cross-check |
| 14 | REZ to BTC: Renzo Price in Bitcoin | [link](https://www.coingecko.com/en/coins/renzo/btc) | Identity, market, social, contract, or discovery cross-check |
| 15 | base REZ | [link](https://basescan.org/token/0xf757c9804cf2ee8d8ed64e0a8936293fe43a7252) | Identity, market, social, contract, or discovery cross-check |
| 16 | solana REZ | [link](https://solscan.io/token/3DK98MXPz8TRuim7rfQnebSLpA7VSoc79Bgiee1m4Zw5) | Identity, market, social, contract, or discovery cross-check |
| 17 | ethereum REZ | [link](https://etherscan.io/token/0x3b50805453023a91a8bf641e279401a0b23fa6f9) | Identity, market, social, contract, or discovery cross-check |
| 18 | ethereum PZETH | [link](https://etherscan.io/token/0x8c9532a60e0e7c6bbd2b2c1303f63ace1c3e9811) | Identity, market, social, contract, or discovery cross-check |
| 19 | solana EZJTO | [link](https://solscan.io/token/ezjtoE6PYVnWkbJJYEsGHgR3rXgBydi9eeB1YHP8cVg) | Identity, market, social, contract, or discovery cross-check |
| 20 | solana EZSOL | [link](https://solscan.io/token/ezSoL6fY1PVdJcJsUpe5CM3xkfmy3zoVCABybm5WtiC) | Identity, market, social, contract, or discovery cross-check |
| 21 | solana BZSOL | [link](https://solscan.io/token/bzSoL4Ehh4AsaPdVD2ndkeHy9nmKnP8YRwVt3TfaJQq) | Identity, market, social, contract, or discovery cross-check |
| 22 | base EZETH | [link](https://basescan.org/token/0x2416092f143378750bb29b79ed961ab195cceea5) | Identity, market, social, contract, or discovery cross-check |
| 23 | Google project search | [link](https://www.google.com/search?q=Renzo%20REZ%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 24 | Google tokenomics search | [link](https://www.google.com/search?q=Renzo%20REZ%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 25 | Google docs search | [link](https://www.google.com/search?q=Renzo%20docs) | Identity, market, social, contract, or discovery cross-check |
| 26 | Google governance search | [link](https://www.google.com/search?q=Renzo%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 27 | Google audit search | [link](https://www.google.com/search?q=Renzo%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 28 | CoinGecko historical data | [link](https://www.coingecko.com/en/coins/renzo/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 29 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/renzo/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 30 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/renzo/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 31 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Renzo) | Identity, market, social, contract, or discovery cross-check |
| 32 | Dune token query | [link](https://dune.com/browse/queries?q=Renzo) | Identity, market, social, contract, or discovery cross-check |
| 33 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Renzo) | Identity, market, social, contract, or discovery cross-check |
| 34 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=REZ) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://earn.renzoprotocol.com/)
- [CoinGecko profile](https://www.coingecko.com/en/coins/renzo)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Renzo)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Renzo)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Renzo)
- [Messari discovery](https://messari.io/search?q=Renzo)
- [X profile](https://x.com/RenzoAI)
- [GitHub](https://github.com/Renzo-Protocol)
- [Telegram](https://t.me/RenzoProtocolChat)
- [Discord](https://discord.com/invite/renzoprotocol)
- [Renzo price today, REZ to USD live ...](https://coinmarketcap.com/currencies/renzo/)
- [Renzo (REZ) | Tokenomics & Vesting Schedule](https://tokenomist.ai/renzo)
- [Latest Renzo (REZ) News Update](https://coinmarketcap.com/cmc-ai/renzo/latest-updates/)
- [REZ to BTC: Renzo Price in Bitcoin](https://www.coingecko.com/en/coins/renzo/btc)
- [base REZ](https://basescan.org/token/0xf757c9804cf2ee8d8ed64e0a8936293fe43a7252)
- [solana REZ](https://solscan.io/token/3DK98MXPz8TRuim7rfQnebSLpA7VSoc79Bgiee1m4Zw5)
- [ethereum REZ](https://etherscan.io/token/0x3b50805453023a91a8bf641e279401a0b23fa6f9)
- [ethereum PZETH](https://etherscan.io/token/0x8c9532a60e0e7c6bbd2b2c1303f63ace1c3e9811)
- [solana EZJTO](https://solscan.io/token/ezjtoE6PYVnWkbJJYEsGHgR3rXgBydi9eeB1YHP8cVg)
- [solana EZSOL](https://solscan.io/token/ezSoL6fY1PVdJcJsUpe5CM3xkfmy3zoVCABybm5WtiC)
- [solana BZSOL](https://solscan.io/token/bzSoL4Ehh4AsaPdVD2ndkeHy9nmKnP8YRwVt3TfaJQq)
- [base EZETH](https://basescan.org/token/0x2416092f143378750bb29b79ed961ab195cceea5)
- [Google project search](https://www.google.com/search?q=Renzo%20REZ%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Renzo%20REZ%20tokenomics)

## Final Investment View

Renzo should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
