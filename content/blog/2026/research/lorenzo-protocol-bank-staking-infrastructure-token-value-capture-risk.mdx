---
title: 'Lorenzo Protocol / BANK: staking infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Lorenzo Protocol is treated as a staking infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'lorenzo-protocol-bank-staking-infrastructure-token-value-capture-risk'
category: 'research'
---

# Lorenzo Protocol / BANK: staking infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Lorenzo Protocol appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Binance Smart Chain, Hemi, Sui; exchange exposure is BINANCE, BITGET, ASTER; category tags are LSD & Restaking, Yield & Asset Mgmt; current market cap is $15.14M; FDV is $15.14M; and 24h volume is $5.30M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Lorenzo Protocol before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 967, market cap $15,132,763, 24h volume $5,313,313. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Lorenzo Protocol is best understood as a **staking infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether BANK captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Binance Smart Chain, Hemi, Sui |
| Exchanges | BINANCE, BITGET, ASTER |
| Price | $0.0356 |
| Market cap | $15.14M |
| FDV | $15.14M |
| 24h volume | $5.30M |
| Circulating supply | 425.25M |
| Total supply | 425.25M |
...
Base case: Lorenzo Protocol remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning BANK into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

The first poker bankroll token on Solana, backed by professional poker players' tournament winnings.

Surf's project profile maps Lorenzo Protocol to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.lorenzo-protocol.xyz/](https://www.lorenzo-protocol.xyz/) |
| X handle | [@LorenzoProtocol](https://x.com/LorenzoProtocol) |
| X followers | 203.01K |
...
| Liquidity | $5.30M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 425.25M circulating vs 425.25M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Lorenzo Protocol is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Binance Smart Chain, Hemi, Sui |
...
|---|---|---|---|
| bsc | BANK | 0x3aee7602b612de36088f3ffed8c8f10e86ebf2bf | [explorer](https://bscscan.com/token/0x3aee7602b612de36088f3ffed8c8f10e86ebf2bf) |
| Hemi | ENZOBTC | 0x6a9a65b84843f5fd4ac9a0471c4fc11afffbce4a | not mapped |
| Sui | ENZOBTC | 0x8f2b5eb696ed88b71fea398d330bccfa52f6e2a5a8e1ac6180fcb25c6de42ebc::coin::COIN | not mapped |
| bsc | SUSD1+ | 0x4f2760b32720f013e900dc92f65480137391199b | [explorer](https://bscscan.com/token/0x4f2760b32720f013e900dc92f65480137391199b) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Lorenzo Protocol, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 967, market cap $15,132,763, 24h volume $5,313,313. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $15.14M, FDV $15.14M, 24h volume $5.30M, and price $0.0356.
...
| 24h volume | $5.30M | Position sizing and exit liquidity proxy |
| Market cap | $15.14M | Liquid valuation proxy |
| FDV | $15.14M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.00x | Higher spread means more supply risk |
| X followers | 203.01K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Lorenzo Protocol | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether BANK is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. Lorenzo Protocol should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that Lorenzo Protocol belongs to the staking infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $15.14M, FDV $15.14M, 24h volume $5.30M, price $0.0356, circulating supply 425.25M, and total supply 425.25M. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For Lorenzo Protocol, the candidate backlog records Seeded 2026-07-06 from Surf market-ranking; rank 967, market cap $15,132,763, 24h volume $5,313,313. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Binance Smart Chain, Hemi, Sui, exchanges as BINANCE, BITGET, ASTER, and tags as LSD & Restaking, Yield & Asset Mgmt. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Lorenzo Protocol currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 1.05% and 30d 15.80% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Lorenzo Protocol's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-04-18 | Ido | $200.00K | not disclosed |
| 2024-05-27 | Ido | $1.00K | not disclosed |
| not disclosed | Strategic | not disclosed | Sats Ventures |

| Team member | Role | Link |
|---|---|---|
| Matt Ye | Co-Founder, CEO | [X](https://twitter.com/MattYe_MEP) |
| Fan Sang | Co-Founder, CTO | not disclosed |
| Toby Yu | Co-Founder, CFO | not disclosed |
| Tad Tobar | COO | [X](https://twitter.com/TadTobar) |
| Lith Li | Head of Marketing | [X](https://x.com/lithli1004) |
...
Lorenzo Protocol competes in the staking infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Lorenzo Protocol has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Lorenzo Protocol is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Lorenzo Protocol compounds usage, liquidity improves, and BANK gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If BANK does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $5.30M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Lorenzo Protocol announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Lorenzo Protocol has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Lorenzo Protocol, the first reconciliation job is identity. The token symbol is BANK, the category is staking infrastructure, chains are Binance Smart Chain, Hemi, Sui, exchanges are BINANCE, BITGET, ASTER, and tags are LSD & Restaking, Yield & Asset Mgmt. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If BANK trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $15.14M, FDV is $15.14M, 24h volume is $5.30M, FDV / market cap is 1.00x, 7d price change is 1.05%, and 30d price change is 15.80%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Lorenzo Protocol / BANK | Match official docs, CG, CMC, Surf, contracts |
| Category | staking infrastructure | Confirm product mechanics and revenue model |
| Chains | Binance Smart Chain, Hemi, Sui | Confirm canonical deployments and bridge assumptions |
| Exchanges | BINANCE, BITGET, ASTER | Check live order books and inactive pairs |
| Market cap | $15.14M | Compare Surf, CG, CMC, exchange pages |
| FDV | $15.14M | Reconcile total supply and unlock schedule |
| Volume | $5.30M | Check exchange concentration and wash-trading risk |
| TVL | not disclosed | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Lorenzo Protocol does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Lorenzo Protocol, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does BANK sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
For Lorenzo Protocol, the current liquidity read is watchlist first. The market cap is $15.14M, FDV is $15.14M, reported 24h volume is $5.30M, and FDV / market cap is 1.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Lorenzo Protocol gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.lorenzo-protocol.xyz/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Lorenzo%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Lorenzo%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Lorenzo%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Lorenzo%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Lorenzo%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/LorenzoProtocol) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/lorenzoprotocol) | Identity, market, social, contract, or discovery cross-check |
| 9 | Discord | [link](https://discord.com/invite/fybK7C4gMH) | Identity, market, social, contract, or discovery cross-check |
| 10 | Lorenzo Protocol Price Chart (BANK) - CoinGecko | [link](https://www.coingecko.com/en/coins/lorenzo-protocol) | Identity, market, social, contract, or discovery cross-check |
| 11 | What is Lorenzo Protocol (BANK)? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/lorenzo-protocol/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 12 | Latest Lorenzo Protocol (BANK) News Update - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/lorenzo-protocol/latest-updates/) | Identity, market, social, contract, or discovery cross-check |
| 13 | Lorenzo Protocol (BANK) Price Prediction For 2026 & Beyond | [link](https://coinmarketcap.com/cmc-ai/lorenzo-protocol/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 14 | Lorenzo Protocol Price History: Download BANK Historical Data | [link](https://www.coingecko.com/en/coins/lorenzo-protocol/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 15 | bsc BANK | [link](https://bscscan.com/token/0x3aee7602b612de36088f3ffed8c8f10e86ebf2bf) | Identity, market, social, contract, or discovery cross-check |
| 16 | bsc SUSD1+ | [link](https://bscscan.com/token/0x4f2760b32720f013e900dc92f65480137391199b) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google project search | [link](https://www.google.com/search?q=Lorenzo%20Protocol%20BANK%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google tokenomics search | [link](https://www.google.com/search?q=Lorenzo%20Protocol%20BANK%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google docs search | [link](https://www.google.com/search?q=Lorenzo%20Protocol%20docs) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google governance search | [link](https://www.google.com/search?q=Lorenzo%20Protocol%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google audit search | [link](https://www.google.com/search?q=Lorenzo%20Protocol%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/lorenzo-protocol/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/lorenzo-protocol/) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Lorenzo%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune token query | [link](https://dune.com/browse/queries?q=Lorenzo%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Lorenzo%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=BANK) | Identity, market, social, contract, or discovery cross-check |
| 28 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Lorenzo%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 29 | CryptoRank search | [link](https://cryptorank.io/price/lorenzo-protocol) | Identity, market, social, contract, or discovery cross-check |
| 30 | RootData search | [link](https://www.rootdata.com/search?kw=Lorenzo%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 31 | CoinDesk search | [link](https://www.coindesk.com/search?s=Lorenzo%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 32 | The Block search | [link](https://www.theblock.co/search?query=Lorenzo%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 33 | GitHub search | [link](https://github.com/search?q=Lorenzo%20Protocol&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 34 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=BANK) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.lorenzo-protocol.xyz/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Lorenzo%20Protocol)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Lorenzo%20Protocol)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Lorenzo%20Protocol)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Lorenzo%20Protocol)
- [Messari discovery](https://messari.io/search?q=Lorenzo%20Protocol)
- [X profile](https://x.com/LorenzoProtocol)
- [Telegram](https://t.me/lorenzoprotocol)
- [Discord](https://discord.com/invite/fybK7C4gMH)
- [Lorenzo Protocol Price Chart (BANK) - CoinGecko](https://www.coingecko.com/en/coins/lorenzo-protocol)
- [What is Lorenzo Protocol (BANK)? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/lorenzo-protocol/what-is/)
- [Latest Lorenzo Protocol (BANK) News Update - CoinMarketCap](https://coinmarketcap.com/cmc-ai/lorenzo-protocol/latest-updates/)
- [Lorenzo Protocol (BANK) Price Prediction For 2026 & Beyond](https://coinmarketcap.com/cmc-ai/lorenzo-protocol/price-prediction/)
- [Lorenzo Protocol Price History: Download BANK Historical Data](https://www.coingecko.com/en/coins/lorenzo-protocol/historical_data)
- [bsc BANK](https://bscscan.com/token/0x3aee7602b612de36088f3ffed8c8f10e86ebf2bf)
- [bsc SUSD1+](https://bscscan.com/token/0x4f2760b32720f013e900dc92f65480137391199b)
- [Google project search](https://www.google.com/search?q=Lorenzo%20Protocol%20BANK%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Lorenzo%20Protocol%20BANK%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Lorenzo%20Protocol%20docs)
- [Google governance search](https://www.google.com/search?q=Lorenzo%20Protocol%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Lorenzo%20Protocol%20audit%20report)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/lorenzo-protocol/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/lorenzo-protocol/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Lorenzo%20Protocol)

## Final Investment View

Lorenzo Protocol should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
