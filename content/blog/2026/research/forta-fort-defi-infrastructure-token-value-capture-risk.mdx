---
title: 'Forta / FORT: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Forta is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'forta-fort-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# Forta / FORT: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Forta appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Arbitrum One, Base, Ethereum, Polygon Pos; exchange exposure is UPBIT, COINBASE, BITHUMB, BYBIT; category tags are Security & Audit, Data & Analytics; current market cap is $7.76M; FDV is $12.22M; and 24h volume is $193.92K. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Forta before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 1310, market cap $7,772,989, 24h volume $190,105. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Forta is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether FORT captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Arbitrum One, Base, Ethereum, Polygon Pos |
| Exchanges | UPBIT, COINBASE, BITHUMB, BYBIT |
| Price | $0.0122 |
| Market cap | $7.76M |
| FDV | $12.22M |
| 24h volume | $193.92K |
| Circulating supply | 635.58M |
| Total supply | 1.00B |
...
Base case: Forta remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning FORT into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Forta offers real-time transaction screening and prevention via Firewall, securing blockchains against exploits, scams, and compliance risks. It integrates into rollups for malicious transaction blocking and regulatory screening. The FORT token supports staking, rewards, and governance over revenue-generating treasury.

Surf's project profile maps Forta to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://forta.org/](https://forta.org/) |
| X handle | [@FortaNetwork](https://x.com/FortaNetwork) |
| X followers | 68.84K |
| Tags | Security & Audit, Data & Analytics |
| Chains | Arbitrum One, Base, Ethereum, Polygon Pos |
| Exchanges | UPBIT, COINBASE, BITHUMB, BYBIT |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Forta is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DeFi infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Forta convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $193.92K 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 635.58M circulating vs 1.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Forta is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Arbitrum One, Base, Ethereum, Polygon Pos |
...
|---|---|---|---|
| base | FORT | 0x7588310a7abf34dc608ac98a1c4432f85e194df5 | [explorer](https://basescan.org/token/0x7588310a7abf34dc608ac98a1c4432f85e194df5) |
| ethereum | FORT | 0x41545f8b9472d758bb669ed8eaeeecd7a9c4ec29 | [explorer](https://etherscan.io/token/0x41545f8b9472d758bb669ed8eaeeecd7a9c4ec29) |
| polygon | FORT | 0x9ff62d1fc52a907b6dcba8077c2ddca6e6a9d3e1 | [explorer](https://polygonscan.com/token/0x9ff62d1fc52a907b6dcba8077c2ddca6e6a9d3e1) |
| arbitrum | FORT | 0x3a1429d50e0cbbc45c997af600541fe1cc3d2923 | [explorer](https://arbiscan.io/token/0x3a1429d50e0cbbc45c997af600541fe1cc3d2923) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Forta, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 1310, market cap $7,772,989, 24h volume $190,105. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $7.76M, FDV $12.22M, 24h volume $193.92K, and price $0.0122.
...
| 24h volume | $193.92K | Position sizing and exit liquidity proxy |
| Market cap | $7.76M | Liquid valuation proxy |
| FDV | $12.22M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.57x | Higher spread means more supply risk |
| X followers | 68.84K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Forta | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether FORT is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Forta currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 4.14% and 30d 8.19% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Forta's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2021-09-30 | Seed | $23.00M | Coinbase Ventures, Placeholder Ventures, Andreessen Horowitz (a16z crypto), Blockchain Capital, Digital Currency Group (DCG), True Ventures, SCB 10X, BlueYard Capital |

| Team member | Role | Link |
|---|---|---|
| Andrew Beal | Head of Ecosystem | [X](https://x.com/ajbeal) |
...
Forta competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
Weighted conclusion: Forta is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Forta compounds usage, liquidity improves, and FORT gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Forta has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If FORT does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $193.92K | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Forta announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Forta has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Forta, the first reconciliation job is identity. The token symbol is FORT, the category is DeFi infrastructure, chains are Arbitrum One, Base, Ethereum, Polygon Pos, exchanges are UPBIT, COINBASE, BITHUMB, BYBIT, and tags are Security & Audit, Data & Analytics. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If FORT trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $7.76M, FDV is $12.22M, 24h volume is $193.92K, FDV / market cap is 1.57x, 7d price change is 4.14%, and 30d price change is 8.19%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Forta / FORT | Match official docs, CG, CMC, Surf, contracts |
| Category | DeFi infrastructure | Confirm product mechanics and revenue model |
| Chains | Arbitrum One, Base, Ethereum, Polygon Pos | Confirm canonical deployments and bridge assumptions |
| Exchanges | UPBIT, COINBASE, BITHUMB, BYBIT | Check live order books and inactive pairs |
| Market cap | $7.76M | Compare Surf, CG, CMC, exchange pages |
| FDV | $12.22M | Reconcile total supply and unlock schedule |
| Volume | $193.92K | Check exchange concentration and wash-trading risk |
| TVL | not disclosed | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Forta does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Forta, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does FORT sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
For Forta, the current liquidity read is watchlist first. The market cap is $7.76M, FDV is $12.22M, reported 24h volume is $193.92K, and FDV / market cap is 1.57x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. Forta can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.
...
| Peer comparison | Monthly | Forta gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://forta.org/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Forta) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Forta) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Forta) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Forta) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Forta) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/FortaNetwork) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/forta-network) | Identity, market, social, contract, or discovery cross-check |
| 9 | Discord | [link](https://discord.com/invite/KTwDUpAPqe) | Identity, market, social, contract, or discovery cross-check |
| 10 | Forta Price: FORT/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/forta) | Identity, market, social, contract, or discovery cross-check |
| 11 | Tokenomics - Forta Docs | [link](https://docs.forta.network/en/latest/fort-token/) | Identity, market, social, contract, or discovery cross-check |
| 12 | What Is Forta (FORT) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/forta/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 13 | Forta (FORT) Tokenomics & Price Analysis - MEXC Exchange | [link](https://www.mexc.com/price/FORT/tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 14 | Forta (FORT) Price Prediction For 2026 & Beyond - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/forta/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 15 | base FORT | [link](https://basescan.org/token/0x7588310a7abf34dc608ac98a1c4432f85e194df5) | Identity, market, social, contract, or discovery cross-check |
| 16 | ethereum FORT | [link](https://etherscan.io/token/0x41545f8b9472d758bb669ed8eaeeecd7a9c4ec29) | Identity, market, social, contract, or discovery cross-check |
| 17 | polygon FORT | [link](https://polygonscan.com/token/0x9ff62d1fc52a907b6dcba8077c2ddca6e6a9d3e1) | Identity, market, social, contract, or discovery cross-check |
| 18 | arbitrum FORT | [link](https://arbiscan.io/token/0x3a1429d50e0cbbc45c997af600541fe1cc3d2923) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google project search | [link](https://www.google.com/search?q=Forta%20FORT%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google tokenomics search | [link](https://www.google.com/search?q=Forta%20FORT%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google docs search | [link](https://www.google.com/search?q=Forta%20docs) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google governance search | [link](https://www.google.com/search?q=Forta%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 23 | Google audit search | [link](https://www.google.com/search?q=Forta%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/forta/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 25 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/forta/) | Identity, market, social, contract, or discovery cross-check |
| 26 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Forta) | Identity, market, social, contract, or discovery cross-check |
| 27 | Dune token query | [link](https://dune.com/browse/queries?q=Forta) | Identity, market, social, contract, or discovery cross-check |
| 28 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Forta) | Identity, market, social, contract, or discovery cross-check |
| 29 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=FORT) | Identity, market, social, contract, or discovery cross-check |
| 30 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Forta) | Identity, market, social, contract, or discovery cross-check |
| 31 | CryptoRank search | [link](https://cryptorank.io/price/forta) | Identity, market, social, contract, or discovery cross-check |
| 32 | RootData search | [link](https://www.rootdata.com/search?kw=Forta) | Identity, market, social, contract, or discovery cross-check |
| 33 | CoinDesk search | [link](https://www.coindesk.com/search?s=Forta) | Identity, market, social, contract, or discovery cross-check |
| 34 | The Block search | [link](https://www.theblock.co/search?query=Forta) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://forta.org/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Forta)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Forta)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Forta)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Forta)
- [Messari discovery](https://messari.io/search?q=Forta)
- [X profile](https://x.com/FortaNetwork)
- [GitHub](https://github.com/forta-network)
- [Discord](https://discord.com/invite/KTwDUpAPqe)
- [Forta Price: FORT/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/forta)
- [Tokenomics - Forta Docs](https://docs.forta.network/en/latest/fort-token/)
- [What Is Forta (FORT) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/forta/what-is/)
- [Forta (FORT) Tokenomics & Price Analysis - MEXC Exchange](https://www.mexc.com/price/FORT/tokenomics)
- [Forta (FORT) Price Prediction For 2026 & Beyond - CoinMarketCap](https://coinmarketcap.com/cmc-ai/forta/price-prediction/)
- [base FORT](https://basescan.org/token/0x7588310a7abf34dc608ac98a1c4432f85e194df5)
- [ethereum FORT](https://etherscan.io/token/0x41545f8b9472d758bb669ed8eaeeecd7a9c4ec29)
- [polygon FORT](https://polygonscan.com/token/0x9ff62d1fc52a907b6dcba8077c2ddca6e6a9d3e1)
- [arbitrum FORT](https://arbiscan.io/token/0x3a1429d50e0cbbc45c997af600541fe1cc3d2923)
- [Google project search](https://www.google.com/search?q=Forta%20FORT%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Forta%20FORT%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Forta%20docs)
- [Google governance search](https://www.google.com/search?q=Forta%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Forta%20audit%20report)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/forta/price-analysis/)

## Final Investment View

Forta should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
