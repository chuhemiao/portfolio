---
title: 'QANplatform / QANX: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'QANplatform is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'qanplatform-qanx-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# QANplatform / QANX: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** QANplatform appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Binance Smart Chain, Ethereum; exchange exposure is not disclosed; category tags are Layer1, Developer Tooling; current market cap is $22.19M; FDV is $27.40M; and 24h volume is $392.72K. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag QANplatform before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 761, market cap $22,909,465, 24h volume $597,348. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

QANplatform is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether QANX captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| FDV | $27.40M |
| 24h volume | $392.72K |
| Circulating supply | 1.70B |
| Total supply | 2.10B |
| 7d price change | 51.00% |
...
Base case: QANplatform remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning QANX into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

QANplatform is a quantum-resistant Layer-1 hybrid blockchain enabling developers and enterprises to build EVM-compatible smart contracts, DApps and tokens while coding in any programming language via the QAN Virtual Machine (QVM). Its native utility token is QANX and the project focuses on NIST-aligned post-quantum cryptography and enterprise integrations.

Surf's project profile maps QANplatform to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.qanplatform.com/](https://www.qanplatform.com/) |
| X handle | [@QANplatform](https://x.com/QANplatform) |
| X followers | 47.70K |
| Tags | Layer1, Developer Tooling |
| Chains | Binance Smart Chain, Ethereum |
| Exchanges | not disclosed |
| TGE status | not disclosed |
| CoinGecko ID | not disclosed |

The main product question is whether QANplatform is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DeFi infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does QANplatform convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $392.72K 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 1.70B circulating vs 2.10B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that QANplatform is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

QANplatform's architecture should be evaluated through the job it performs. For a DeFi infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
|---|---|---|---|
| ethereum | QANX | 0xaaa9214f675316182eaa21c85f0ca99160cc3aaa | [explorer](https://etherscan.io/token/0xaaa9214f675316182eaa21c85f0ca99160cc3aaa) |
| bsc | QANX | 0xaaa9214f675316182eaa21c85f0ca99160cc3aaa | [explorer](https://bscscan.com/token/0xaaa9214f675316182eaa21c85f0ca99160cc3aaa) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For QANplatform, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 761, market cap $22,909,465, 24h volume $597,348. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $22.19M, FDV $27.40M, 24h volume $392.72K, and price $0.0131.
...
| 24h volume | $392.72K | Position sizing and exit liquidity proxy |
| Market cap | $22.19M | Liquid valuation proxy |
| FDV | $27.40M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.24x | Higher spread means more supply risk |
| X followers | 47.70K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for QANplatform | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether QANX is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. QANplatform currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 51.00% and 30d 16.05% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that QANplatform's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2023-12-05 | Undisclosed | $15.00M | MBK Holding |
| 2021-06-01 | Ido | $200.00K | not disclosed |
| 2021-05-31 | Ido | $300.00K | not disclosed |

| Team member | Role | Link |
|---|---|---|
| Johann Polecsak | Co-Founder, CTO | [X](https://twitter.com/pmjohann) |
| Endre Abraham | Co-Founder, Head of Cryptology | not disclosed |
| Jevgenia Kim | CEO | not disclosed |
| Mihaly Jurin | Head of BD, Head of Strategic Partnerships | [X](https://x.com/mishajurin) |
| Gergo Szoke | Strategic Advisor | not disclosed |
...
QANplatform competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether QANplatform has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: QANplatform is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | QANplatform compounds usage, liquidity improves, and QANX gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If QANX does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $392.72K | Weekly | Volume below position-size threshold |
| FDV / market cap | 1.24x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if QANplatform announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. QANplatform has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For QANplatform, the first reconciliation job is identity. The token symbol is QANX, the category is DeFi infrastructure, chains are Binance Smart Chain, Ethereum, exchanges are not disclosed, and tags are Layer1, Developer Tooling. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If QANX trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $22.19M, FDV is $27.40M, 24h volume is $392.72K, FDV / market cap is 1.24x, 7d price change is 51.00%, and 30d price change is 16.05%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | QANplatform / QANX | Match official docs, CG, CMC, Surf, contracts |
...
For QANplatform, the current liquidity read is watchlist first. The market cap is $22.19M, FDV is $27.40M, reported 24h volume is $392.72K, and FDV / market cap is 1.24x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If QANplatform suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | QANplatform gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.qanplatform.com/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=QANplatform) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=QANplatform) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=QANplatform) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=QANplatform) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=QANplatform) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/QANplatform) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/qanplatform) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/QANplatform) | Identity, market, social, contract, or discovery cross-check |
| 10 | QANplatform Price Chart (QANX) - CoinGecko | [link](https://www.coingecko.com/en/coins/qanplatform) | Identity, market, social, contract, or discovery cross-check |
| 11 | QANX to BTC: QANplatform Price in Bitcoin - CoinGecko | [link](https://www.coingecko.com/en/coins/qanplatform/btc) | Identity, market, social, contract, or discovery cross-check |
| 12 | QANplatform | [link](https://qanplatform.com/en) | Identity, market, social, contract, or discovery cross-check |
| 13 | QANX to ETH: QANplatform Price in Ether - CoinGecko | [link](https://www.coingecko.com/en/coins/qanplatform/eth) | Identity, market, social, contract, or discovery cross-check |
| 14 | QANplatform Price History: Download QANX Historical Data | [link](https://www.coingecko.com/en/coins/qanplatform/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 15 | ethereum QANX | [link](https://etherscan.io/token/0xaaa9214f675316182eaa21c85f0ca99160cc3aaa) | Identity, market, social, contract, or discovery cross-check |
| 16 | bsc QANX | [link](https://bscscan.com/token/0xaaa9214f675316182eaa21c85f0ca99160cc3aaa) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google project search | [link](https://www.google.com/search?q=QANplatform%20QANX%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google tokenomics search | [link](https://www.google.com/search?q=QANplatform%20QANX%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google docs search | [link](https://www.google.com/search?q=QANplatform%20docs) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google governance search | [link](https://www.google.com/search?q=QANplatform%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google audit search | [link](https://www.google.com/search?q=QANplatform%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/qanplatform/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/qanplatform/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/qanplatform/) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune discovery | [link](https://dune.com/browse/dashboards?q=QANplatform) | Identity, market, social, contract, or discovery cross-check |
| 26 | Dune token query | [link](https://dune.com/browse/queries?q=QANplatform) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=QANplatform) | Identity, market, social, contract, or discovery cross-check |
| 28 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=QANX) | Identity, market, social, contract, or discovery cross-check |
| 29 | TokenUnlocks search | [link](https://token.unlocks.app/?search=QANplatform) | Identity, market, social, contract, or discovery cross-check |
| 30 | CryptoRank search | [link](https://cryptorank.io/price/qanplatform) | Identity, market, social, contract, or discovery cross-check |
| 31 | RootData search | [link](https://www.rootdata.com/search?kw=QANplatform) | Identity, market, social, contract, or discovery cross-check |
| 32 | CoinDesk search | [link](https://www.coindesk.com/search?s=QANplatform) | Identity, market, social, contract, or discovery cross-check |
| 33 | The Block search | [link](https://www.theblock.co/search?query=QANplatform) | Identity, market, social, contract, or discovery cross-check |
| 34 | GitHub search | [link](https://github.com/search?q=QANplatform&type=repositories) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.qanplatform.com/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=QANplatform)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=QANplatform)
- [DefiLlama protocol discovery](https://defillama.com/search?query=QANplatform)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=QANplatform)
- [Messari discovery](https://messari.io/search?q=QANplatform)
- [X profile](https://x.com/QANplatform)
- [GitHub](https://github.com/qanplatform)
- [Telegram](https://t.me/QANplatform)
- [QANplatform Price Chart (QANX) - CoinGecko](https://www.coingecko.com/en/coins/qanplatform)
- [QANX to BTC: QANplatform Price in Bitcoin - CoinGecko](https://www.coingecko.com/en/coins/qanplatform/btc)
- [QANplatform](https://qanplatform.com/en)
- [QANX to ETH: QANplatform Price in Ether - CoinGecko](https://www.coingecko.com/en/coins/qanplatform/eth)
- [QANplatform Price History: Download QANX Historical Data](https://www.coingecko.com/en/coins/qanplatform/historical_data)
- [ethereum QANX](https://etherscan.io/token/0xaaa9214f675316182eaa21c85f0ca99160cc3aaa)
- [bsc QANX](https://bscscan.com/token/0xaaa9214f675316182eaa21c85f0ca99160cc3aaa)
- [Google project search](https://www.google.com/search?q=QANplatform%20QANX%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=QANplatform%20QANX%20tokenomics)
- [Google docs search](https://www.google.com/search?q=QANplatform%20docs)
- [Google governance search](https://www.google.com/search?q=QANplatform%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=QANplatform%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/qanplatform/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/qanplatform/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/qanplatform/)

## Final Investment View

QANplatform should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
