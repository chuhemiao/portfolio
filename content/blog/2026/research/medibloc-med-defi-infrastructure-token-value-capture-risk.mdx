---
title: 'Medibloc / MED: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Medibloc is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'medibloc-med-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# Medibloc / MED: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Medibloc appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Osmosis; exchange exposure is UPBIT, BITHUMB; category tags are Layer1, DID & Identity; current market cap is $15.58M; FDV is $16.74M; and 24h volume is $185.57K. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Medibloc before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 953, market cap $15,494,376, 24h volume $182,160. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Medibloc is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether MED captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| FDV | $16.74M |
| 24h volume | $185.57K |
| Circulating supply | 10.64B |
| Total supply | 11.43B |
| 7d price change | 3.36% |
| 30d price change | -2.72% |
...
Base case: Medibloc remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning MED into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

MediBloc is a patient-centered health-data platform that developed Panacea, a Cosmos‑SDK-based blockchain enabling secure, interoperable health record exchange using decentralized identifiers (DID), encrypted off‑chain storage, and IBC. The MED token incentivizes ecosystem participants and supports staking, governance, and integrations via client SDKs and Panacea's PNFT features.

Surf's project profile maps Medibloc to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://medibloc.com](https://medibloc.com) |
| X handle | [@_MediBloc](https://x.com/_MediBloc) |
| X followers | 19.00K |
| Tags | Layer1, DID & Identity |
| Chains | Osmosis |
| Exchanges | UPBIT, BITHUMB |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Medibloc is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DeFi infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Medibloc convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $185.57K 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 10.64B circulating vs 11.43B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Medibloc is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

Medibloc's architecture should be evaluated through the job it performs. For a DeFi infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
|---|---|---|---|
| Osmosis | MED | ibc/3BCCC93AD5DF58D11A6F8A05FA8BC801CBA0BA61A981F57E91B8B598BF8061CB | not mapped |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Medibloc, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 953, market cap $15,494,376, 24h volume $182,160. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $15.58M, FDV $16.74M, 24h volume $185.57K, and price $0.0015.
...
| 24h volume | $185.57K | Position sizing and exit liquidity proxy |
| Market cap | $15.58M | Liquid valuation proxy |
| FDV | $16.74M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.07x | Higher spread means more supply risk |
| X followers | 19.00K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Medibloc | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether MED is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Medibloc currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 3.36% and 30d -2.72% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Medibloc's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2020-12-02 | Undisclosed | not disclosed | not disclosed |
| 2017-11-13 | Ico | $21.27M | not disclosed |
...
Medibloc competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Medibloc has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Medibloc is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Medibloc compounds usage, liquidity improves, and MED gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If MED does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $185.57K | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Medibloc announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Medibloc has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Medibloc, the first reconciliation job is identity. The token symbol is MED, the category is DeFi infrastructure, chains are Osmosis, exchanges are UPBIT, BITHUMB, and tags are Layer1, DID & Identity. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If MED trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $15.58M, FDV is $16.74M, 24h volume is $185.57K, FDV / market cap is 1.07x, 7d price change is 3.36%, and 30d price change is -2.72%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Medibloc / MED | Match official docs, CG, CMC, Surf, contracts |
...
For Medibloc, the current liquidity read is watchlist first. The market cap is $15.58M, FDV is $16.74M, reported 24h volume is $185.57K, and FDV / market cap is 1.07x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Medibloc gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://medibloc.com) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Medibloc) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Medibloc) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Medibloc) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Medibloc) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Medibloc) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/_MediBloc) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/Medibloc) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/medibloc) | Identity, market, social, contract, or discovery cross-check |
| 10 | Medibloc MED Price - CoinGecko | [link](https://www.coingecko.com/en/coins/medibloc) | Identity, market, social, contract, or discovery cross-check |
| 11 | MediBloc price today, MED to USD live price, marketcap and chart | [link](https://coinmarketcap.com/currencies/medibloc/) | Identity, market, social, contract, or discovery cross-check |
| 12 | Top Healthcare Coins by Market Cap - CoinGecko | [link](https://www.coingecko.com/en/categories/healthcare) | Identity, market, social, contract, or discovery cross-check |
| 13 | Latest MediBloc (MED) Price Analysis - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/medibloc/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
...
| 15 | Google project search | [link](https://www.google.com/search?q=Medibloc%20MED%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google tokenomics search | [link](https://www.google.com/search?q=Medibloc%20MED%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google docs search | [link](https://www.google.com/search?q=Medibloc%20docs) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google governance search | [link](https://www.google.com/search?q=Medibloc%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google audit search | [link](https://www.google.com/search?q=Medibloc%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 20 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/medibloc/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 21 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Medibloc) | Identity, market, social, contract, or discovery cross-check |
| 22 | Dune token query | [link](https://dune.com/browse/queries?q=Medibloc) | Identity, market, social, contract, or discovery cross-check |
| 23 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Medibloc) | Identity, market, social, contract, or discovery cross-check |
| 24 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=MED) | Identity, market, social, contract, or discovery cross-check |
| 25 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Medibloc) | Identity, market, social, contract, or discovery cross-check |
| 26 | CryptoRank search | [link](https://cryptorank.io/price/medibloc) | Identity, market, social, contract, or discovery cross-check |
| 27 | RootData search | [link](https://www.rootdata.com/search?kw=Medibloc) | Identity, market, social, contract, or discovery cross-check |
| 28 | CoinDesk search | [link](https://www.coindesk.com/search?s=Medibloc) | Identity, market, social, contract, or discovery cross-check |
| 29 | The Block search | [link](https://www.theblock.co/search?query=Medibloc) | Identity, market, social, contract, or discovery cross-check |
| 30 | GitHub search | [link](https://github.com/search?q=Medibloc&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 31 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=MED) | Identity, market, social, contract, or discovery cross-check |
| 32 | BscScan token search | [link](https://bscscan.com/search?f=0&q=MED) | Identity, market, social, contract, or discovery cross-check |
| 33 | Arbiscan token search | [link](https://arbiscan.io/search?f=0&q=MED) | Identity, market, social, contract, or discovery cross-check |
| 34 | BaseScan token search | [link](https://basescan.org/search?f=0&q=MED) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://medibloc.com)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Medibloc)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Medibloc)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Medibloc)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Medibloc)
- [Messari discovery](https://messari.io/search?q=Medibloc)
- [X profile](https://x.com/_MediBloc)
- [GitHub](https://github.com/Medibloc)
- [Telegram](https://t.me/medibloc)
- [Medibloc MED Price - CoinGecko](https://www.coingecko.com/en/coins/medibloc)
- [MediBloc price today, MED to USD live price, marketcap and chart](https://coinmarketcap.com/currencies/medibloc/)
- [Top Healthcare Coins by Market Cap - CoinGecko](https://www.coingecko.com/en/categories/healthcare)
- [Latest MediBloc (MED) Price Analysis - CoinMarketCap](https://coinmarketcap.com/cmc-ai/medibloc/price-analysis/)
- [CoinGecko: Cryptocurrency Prices, Charts, and Crypto Market Cap](https://www.coingecko.com/)
- [Google project search](https://www.google.com/search?q=Medibloc%20MED%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Medibloc%20MED%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Medibloc%20docs)
- [Google governance search](https://www.google.com/search?q=Medibloc%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Medibloc%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/medibloc/what-is/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Medibloc)
- [Dune token query](https://dune.com/browse/queries?q=Medibloc)
- [DeFiLlama fees discovery](https://defillama.com/fees?chain=All&search=Medibloc)
- [DeFiLlama yield discovery](https://defillama.com/yields?token=MED)

## Final Investment View

Medibloc should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
