---
title: 'Nym / NYM: privacy infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Nym is treated as a privacy infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'nym-nym-privacy-infrastructure-token-value-capture-risk'
category: 'research'
---

# Nym / NYM: privacy infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Nym appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Ethereum, Osmosis; exchange exposure is BYBIT, BITGET; category tags are DePIN (Physical Infra), Developer Tooling, Privacy & ZK; current market cap is $15.49M; FDV is $18.49M; and 24h volume is $443.09K. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Nym before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 960, market cap $15,234,782, 24h volume $473,600. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Nym is best understood as a **privacy infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether NYM captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| FDV | $18.49M |
| 24h volume | $443.09K |
| Circulating supply | 837.38M |
| Total supply | 1.00B |
| 7d price change | 2.94% |
...
Base case: Nym remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning NYM into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Nym is an open-source privacy platform running a decentralized mixnet and zero-knowledge credentials to protect metadata and enable unlinkable network-level privacy. It powers NymVPN and uses the NYM token to reward mixnet operators and support network operations.

Surf's project profile maps Nym to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://nym.com/](https://nym.com/) |
| X handle | [@nym](https://x.com/nym) |
| X followers | 160.31K |
| Tags | DePIN (Physical Infra), Developer Tooling, Privacy & ZK |
| Chains | Ethereum, Osmosis |
| Exchanges | BYBIT, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Nym is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For privacy infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Nym convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $443.09K 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 837.38M circulating vs 1.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Nym is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Ethereum, Osmosis |
...
|---|---|---|---|
| Osmosis | NYM | ibc/37CB3078432510EE57B9AFA8DBE028B33AE3280A144826FEAC5F2334CF2C5539 | not mapped |
| ethereum | NYM | 0x525a8f6f3ba4752868cde25164382bfbae3990e1 | [explorer](https://etherscan.io/token/0x525a8f6f3ba4752868cde25164382bfbae3990e1) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Nym, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 960, market cap $15,234,782, 24h volume $473,600. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $15.49M, FDV $18.49M, 24h volume $443.09K, and price $0.0185.
...
| 24h volume | $443.09K | Position sizing and exit liquidity proxy |
| Market cap | $15.49M | Liquid valuation proxy |
| FDV | $18.49M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.19x | Higher spread means more supply risk |
| X followers | 160.31K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Nym | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether NYM is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Nym currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 2.94% and 30d -3.54% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Nym's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2022-04-14 | Ieo | $30.00K | not disclosed |
| 2022-02-09 | Ico | $6.25M | not disclosed |
| 2022-02-09 | Ico | $25.00M | not disclosed |
| 2021-11-17 | Series A | $13.00M | HTX Ventures, Fenbushi Capital, Andreessen Horowitz (a16z crypto), Digital Currency Group (DCG), HashKey Capital, Tayssir Capital |
| 2021-06-16 | Series A | $6.00M | Polychain Capital, Maven 11 Capital, 1kx, Eden Block, Greenfield Capital, Tioga Capital Partners |
| 2019-05-13 | Pre Seed | $2.50M | NGC Ventures, YZi Labs (Prev. Binance Labs), Lemniscap, 1kx, Eden Block |

| Team member | Role | Link |
|---|---|---|
| Claudia Diaz | Co-Founder, Chief Scientist | [X](https://twitter.com/claudsdayz) |
| Ania Piotrowska | Co-Founder, Head of Research | [X](https://twitter.com/aniampiotrowska) |
| Jess H | Co-Founder, CPO | not disclosed |
| Dave Hrycyszyn | Co-Founder, Advisor | [X](https://twitter.com/futurechimp/) |
| Harry Halpin | CEO | [X](https://twitter.com/harryhalpin) |
| Mark Sinclair | CTO | not disclosed |
| Alexis Roussel | COO, Board Member | [X](https://twitter.com/alexis_roussel) |
| Shane Qiu | Advisor | [X](https://x.com/shaneqiu) |
...
Nym competes in the privacy infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Nym has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Nym is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Nym compounds usage, liquidity improves, and NYM gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Nym has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If NYM does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $443.09K | Weekly | Volume below position-size threshold |
| FDV / market cap | 1.19x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Nym announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Nym has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Nym, the first reconciliation job is identity. The token symbol is NYM, the category is privacy infrastructure, chains are Ethereum, Osmosis, exchanges are BYBIT, BITGET, and tags are DePIN (Physical Infra), Developer Tooling, Privacy & ZK. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If NYM trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $15.49M, FDV is $18.49M, 24h volume is $443.09K, FDV / market cap is 1.19x, 7d price change is 2.94%, and 30d price change is -3.54%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
For Nym, the current liquidity read is watchlist first. The market cap is $15.49M, FDV is $18.49M, reported 24h volume is $443.09K, and FDV / market cap is 1.19x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Nym gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://nym.com/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Nym) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Nym) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Nym) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Nym) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Nym) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/nym) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/nymtech) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/nymchan) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](https://discord.com/invite/nym) | Identity, market, social, contract, or discovery cross-check |
| 11 | NYM/USD Live Price Chart, Market Cap & News Today - CoinGecko | [link](https://www.coingecko.com/en/coins/nym) | Identity, market, social, contract, or discovery cross-check |
| 12 | NYM price today, NYM to USD live price, marketcap and chart | [link](https://coinmarketcap.com/currencies/nym/) | Identity, market, social, contract, or discovery cross-check |
| 13 | NYM — The Architecture of Recovery : A Unified Theory of Token ... | [link](https://forum.nym.com/t/nym-the-architecture-of-recovery-a-unified-theory-of-token-value-ecosystem-growth-and-failure-prevention-for-the-nym-privacy-network/2337) | Identity, market, social, contract, or discovery cross-check |
| 14 | What Is NYM (NYM) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/nym/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 15 | INR - Nym Price in Indian Rupee - CoinGecko | [link](https://www.coingecko.com/en/coins/nym/inr) | Identity, market, social, contract, or discovery cross-check |
| 16 | ethereum NYM | [link](https://etherscan.io/token/0x525a8f6f3ba4752868cde25164382bfbae3990e1) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google project search | [link](https://www.google.com/search?q=Nym%20NYM%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google tokenomics search | [link](https://www.google.com/search?q=Nym%20NYM%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google docs search | [link](https://www.google.com/search?q=Nym%20docs) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google governance search | [link](https://www.google.com/search?q=Nym%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google audit search | [link](https://www.google.com/search?q=Nym%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/nym/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Nym) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune token query | [link](https://dune.com/browse/queries?q=Nym) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Nym) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=NYM) | Identity, market, social, contract, or discovery cross-check |
| 27 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Nym) | Identity, market, social, contract, or discovery cross-check |
| 28 | CryptoRank search | [link](https://cryptorank.io/price/nym) | Identity, market, social, contract, or discovery cross-check |
| 29 | RootData search | [link](https://www.rootdata.com/search?kw=Nym) | Identity, market, social, contract, or discovery cross-check |
| 30 | CoinDesk search | [link](https://www.coindesk.com/search?s=Nym) | Identity, market, social, contract, or discovery cross-check |
| 31 | The Block search | [link](https://www.theblock.co/search?query=Nym) | Identity, market, social, contract, or discovery cross-check |
| 32 | GitHub search | [link](https://github.com/search?q=Nym&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 33 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=NYM) | Identity, market, social, contract, or discovery cross-check |
| 34 | BscScan token search | [link](https://bscscan.com/search?f=0&q=NYM) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://nym.com/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Nym)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Nym)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Nym)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Nym)
- [Messari discovery](https://messari.io/search?q=Nym)
- [X profile](https://x.com/nym)
- [GitHub](https://github.com/nymtech)
- [Telegram](https://t.me/nymchan)
- [Discord](https://discord.com/invite/nym)
- [NYM/USD Live Price Chart, Market Cap & News Today - CoinGecko](https://www.coingecko.com/en/coins/nym)
- [NYM price today, NYM to USD live price, marketcap and chart](https://coinmarketcap.com/currencies/nym/)
- [NYM — The Architecture of Recovery : A Unified Theory of Token ...](https://forum.nym.com/t/nym-the-architecture-of-recovery-a-unified-theory-of-token-value-ecosystem-growth-and-failure-prevention-for-the-nym-privacy-network/2337)
- [What Is NYM (NYM) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/nym/what-is/)
- [INR - Nym Price in Indian Rupee - CoinGecko](https://www.coingecko.com/en/coins/nym/inr)
- [ethereum NYM](https://etherscan.io/token/0x525a8f6f3ba4752868cde25164382bfbae3990e1)
- [Google project search](https://www.google.com/search?q=Nym%20NYM%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Nym%20NYM%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Nym%20docs)
- [Google governance search](https://www.google.com/search?q=Nym%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Nym%20audit%20report)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/nym/price-analysis/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Nym)
- [Dune token query](https://dune.com/browse/queries?q=Nym)

## Final Investment View

Nym should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
