---
title: 'CYBER / CYBER: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'CYBER is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'cyber-cyber-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# CYBER / CYBER: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** CYBER appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Base, Binance Smart Chain, Cyber, Ethereum, Optimistic Ethereum, Solana; exchange exposure is BINANCE, UPBIT, BITHUMB, BYBIT, BITGET; category tags are Layer2 & Scaling, SocialFi, Web3 AI (Agent/AIGC); current market cap is $24.98M; FDV is $37.15M; and 24h volume is $8.43M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag CYBER before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 730, market cap $24,543,582, 24h volume $8,064,305. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

CYBER is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether CYBER captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Base, Binance Smart Chain, Cyber, Ethereum, Optimistic Ethereum, Solana |
| Exchanges | BINANCE, UPBIT, BITHUMB, BYBIT, BITGET |
| Price | $0.3715 |
| Market cap | $24.98M |
| FDV | $37.15M |
| 24h volume | $8.43M |
| Circulating supply | 67.24M |
...
Base case: CYBER remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning CYBER into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Cyber is an Ethereum Layer 2 network bringing social primitives onchain, optimized for AI to enable new ways for people and AI to connect, create, and experience crypto. It offers restaking, staking incentives, and developer tools for building social dApps.

Surf's project profile maps CYBER to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://cyber.co/](https://cyber.co/) |
| X handle | [@BuildOnCyber](https://x.com/BuildOnCyber) |
| X followers | 585.05K |
| Tags | Layer2 & Scaling, SocialFi, Web3 AI (Agent/AIGC) |
| Chains | Base, Binance Smart Chain, Cyber, Ethereum, Optimistic Ethereum, Solana |
| Exchanges | BINANCE, UPBIT, BITHUMB, BYBIT, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether CYBER is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DeFi infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does CYBER convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $8.43M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 67.24M circulating vs 100.00M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that CYBER is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Base, Binance Smart Chain, Cyber, Ethereum, Optimistic Ethereum, Solana |
...
|---|---|---|---|
| cyber | CYBER | 0x14778860e937f509e651192a90589de711fb88a9 | not mapped |
| ethereum | CYBER | 0x14778860e937f509e651192a90589de711fb88a9 | [explorer](https://etherscan.io/token/0x14778860e937f509e651192a90589de711fb88a9) |
| base | CYBER | 0x14778860E937f509e651192a90589dE711Fb88a9 | [explorer](https://basescan.org/token/0x14778860E937f509e651192a90589dE711Fb88a9) |
| solana | CYBER | 5gzCDgVf5Nk6gosa5dpYZV6P6RiAytvCyQUaMwAVyf8z | [explorer](https://solscan.io/token/5gzCDgVf5Nk6gosa5dpYZV6P6RiAytvCyQUaMwAVyf8z) |
| bsc | CYBER | 0x14778860e937f509e651192a90589de711fb88a9 | [explorer](https://bscscan.com/token/0x14778860e937f509e651192a90589de711fb88a9) |
| optimism | CYBER | 0x14778860e937f509e651192a90589de711fb88a9 | [explorer](https://optimistic.etherscan.io/token/0x14778860e937f509e651192a90589de711fb88a9) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For CYBER, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 730, market cap $24,543,582, 24h volume $8,064,305. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $24.98M, FDV $37.15M, 24h volume $8.43M, and price $0.3715.
...
| 24h volume | $8.43M | Position sizing and exit liquidity proxy |
| Market cap | $24.98M | Liquid valuation proxy |
| FDV | $37.15M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.49x | Higher spread means more supply risk |
| X followers | 585.05K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for CYBER | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether CYBER is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. CYBER currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 7.45% and 30d 5.38% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that CYBER's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2023-10-25 | Undisclosed | not disclosed | YZi Labs (Prev. Binance Labs) |
| 2023-05-19 | Ieo | $10.80K | not disclosed |
| 2023-05-18 | Ico | $5.40M | not disclosed |
| 2022-05-18 | Series A | $15.00M | Animoca Brands, The Spartan Group, Delphi Ventures, SevenX Ventures, Tribe Capital, IOSG Ventures, Polygon Studios, Amber Group |
| 2021-11-16 | Seed | $10.10M | Hashed, Animoca Brands, Multicoin Capital, Draper Dragon, Sky9 Capital, Smrti Lab, Mask Network, Zoo Capital |

| Team member | Role | Link |
|---|---|---|
| Wilson Wei | Co-Founder, CEO | [X](https://twitter.com/WilsonWei777) |
| Shiyu Zhang | Co-Founder | [X](https://twitter.com/shiyuSQ) |
| Ryan Li | Co-Founder | [X](https://twitter.com/ryanli_me) |
| Zhimao Liu | Co-Founder | [X](https://twitter.com/zhimao_liu) |
...
CYBER competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether CYBER has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: CYBER is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | CYBER compounds usage, liquidity improves, and CYBER gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because CYBER has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If CYBER does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $8.43M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if CYBER announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. CYBER has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For CYBER, the first reconciliation job is identity. The token symbol is CYBER, the category is DeFi infrastructure, chains are Base, Binance Smart Chain, Cyber, Ethereum, Optimistic Ethereum, Solana, exchanges are BINANCE, UPBIT, BITHUMB, BYBIT, BITGET, and tags are Layer2 & Scaling, SocialFi, Web3 AI (Agent/AIGC). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If CYBER trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $24.98M, FDV is $37.15M, 24h volume is $8.43M, FDV / market cap is 1.49x, 7d price change is 7.45%, and 30d price change is 5.38%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
For CYBER, the current liquidity read is watchlist first. The market cap is $24.98M, FDV is $37.15M, reported 24h volume is $8.43M, and FDV / market cap is 1.49x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | CYBER gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://cyber.co/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=CYBER) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=CYBER) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=CYBER) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=CYBER) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=CYBER) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/BuildOnCyber) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/cyberconnecthq) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/buildoncyber) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](https://discord.com/invite/uyaKW9EMV5) | Identity, market, social, contract, or discovery cross-check |
| 11 | CYBER/USD Live Price Chart, Market Cap & News Today - CoinGecko | [link](https://www.coingecko.com/en/coins/cyberconnect) | Identity, market, social, contract, or discovery cross-check |
| 12 | Top Cyber Ecosystem Coins by Market Cap - CoinGecko | [link](https://www.coingecko.com/en/categories/cyber-ecosystem) | Identity, market, social, contract, or discovery cross-check |
| 13 | Top Cybersecurity Coins by Market Cap - CoinGecko | [link](https://www.coingecko.com/en/categories/cybersecurity) | Identity, market, social, contract, or discovery cross-check |
| 14 | cyber - Tokenomics & Vesting Schedule - Token Unlocks | [link](https://tokenomist.ai/cyberconnect) | Identity, market, social, contract, or discovery cross-check |
| 15 | Latest Cyber (CYBER) News Update - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/cyberconnect/latest-updates/) | Identity, market, social, contract, or discovery cross-check |
| 16 | ethereum CYBER | [link](https://etherscan.io/token/0x14778860e937f509e651192a90589de711fb88a9) | Identity, market, social, contract, or discovery cross-check |
| 17 | base CYBER | [link](https://basescan.org/token/0x14778860E937f509e651192a90589dE711Fb88a9) | Identity, market, social, contract, or discovery cross-check |
| 18 | solana CYBER | [link](https://solscan.io/token/5gzCDgVf5Nk6gosa5dpYZV6P6RiAytvCyQUaMwAVyf8z) | Identity, market, social, contract, or discovery cross-check |
| 19 | bsc CYBER | [link](https://bscscan.com/token/0x14778860e937f509e651192a90589de711fb88a9) | Identity, market, social, contract, or discovery cross-check |
| 20 | optimism CYBER | [link](https://optimistic.etherscan.io/token/0x14778860e937f509e651192a90589de711fb88a9) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google project search | [link](https://www.google.com/search?q=CYBER%20CYBER%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google tokenomics search | [link](https://www.google.com/search?q=CYBER%20CYBER%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 23 | Google docs search | [link](https://www.google.com/search?q=CYBER%20docs) | Identity, market, social, contract, or discovery cross-check |
| 24 | Google governance search | [link](https://www.google.com/search?q=CYBER%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 25 | Google audit search | [link](https://www.google.com/search?q=CYBER%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 26 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/cyber/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 27 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/cyber/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 28 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/cyber/) | Identity, market, social, contract, or discovery cross-check |
| 29 | Dune discovery | [link](https://dune.com/browse/dashboards?q=CYBER) | Identity, market, social, contract, or discovery cross-check |
| 30 | Dune token query | [link](https://dune.com/browse/queries?q=CYBER) | Identity, market, social, contract, or discovery cross-check |
| 31 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=CYBER) | Identity, market, social, contract, or discovery cross-check |
| 32 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=CYBER) | Identity, market, social, contract, or discovery cross-check |
| 33 | TokenUnlocks search | [link](https://token.unlocks.app/?search=CYBER) | Identity, market, social, contract, or discovery cross-check |
| 34 | CryptoRank search | [link](https://cryptorank.io/price/cyber) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://cyber.co/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=CYBER)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=CYBER)
- [DefiLlama protocol discovery](https://defillama.com/search?query=CYBER)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=CYBER)
- [Messari discovery](https://messari.io/search?q=CYBER)
- [X profile](https://x.com/BuildOnCyber)
- [GitHub](https://github.com/cyberconnecthq)
- [Telegram](https://t.me/buildoncyber)
- [Discord](https://discord.com/invite/uyaKW9EMV5)
- [CYBER/USD Live Price Chart, Market Cap & News Today - CoinGecko](https://www.coingecko.com/en/coins/cyberconnect)
- [Top Cyber Ecosystem Coins by Market Cap - CoinGecko](https://www.coingecko.com/en/categories/cyber-ecosystem)
- [Top Cybersecurity Coins by Market Cap - CoinGecko](https://www.coingecko.com/en/categories/cybersecurity)
- [cyber - Tokenomics & Vesting Schedule - Token Unlocks](https://tokenomist.ai/cyberconnect)
- [Latest Cyber (CYBER) News Update - CoinMarketCap](https://coinmarketcap.com/cmc-ai/cyberconnect/latest-updates/)
- [ethereum CYBER](https://etherscan.io/token/0x14778860e937f509e651192a90589de711fb88a9)
- [base CYBER](https://basescan.org/token/0x14778860E937f509e651192a90589dE711Fb88a9)
- [solana CYBER](https://solscan.io/token/5gzCDgVf5Nk6gosa5dpYZV6P6RiAytvCyQUaMwAVyf8z)
- [bsc CYBER](https://bscscan.com/token/0x14778860e937f509e651192a90589de711fb88a9)
- [optimism CYBER](https://optimistic.etherscan.io/token/0x14778860e937f509e651192a90589de711fb88a9)
- [Google project search](https://www.google.com/search?q=CYBER%20CYBER%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=CYBER%20CYBER%20tokenomics)
- [Google docs search](https://www.google.com/search?q=CYBER%20docs)
- [Google governance search](https://www.google.com/search?q=CYBER%20governance%20proposal)

## Final Investment View

CYBER should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
