---
title: 'Spacecoin / SPACE: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Spacecoin is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'spacecoin-space-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# Spacecoin / SPACE: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Spacecoin appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Binance Smart Chain, Ethereum; exchange exposure is OKX, BITHUMB, BINANCE ALPHA, BITGET; category tags are DePIN (Physical Infra); current market cap is $34.41M; FDV is $130.11M; and 24h volume is $4.07M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Spacecoin before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 583, market cap $34,158,792, 24h volume $4,779,983. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Spacecoin is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether SPACE captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Exchanges | OKX, BITHUMB, BINANCE ALPHA, BITGET |
| Price | $0.0062 |
| Market cap | $34.41M |
| FDV | $130.11M |
| 24h volume | $4.07M |
| Circulating supply | 5.55B |
| Total supply | 21.00B |
| 7d price change | -1.17% |
| 30d price change | 6.44% |
...
Base case: Spacecoin remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning SPACE into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Spacecoin™ is a decentralized physical infrastructure network (DePIN) building a blockchain-enabled low-Earth-orbit nanosatellite constellation to provide permissionless, resilient global internet connectivity. It integrates on-chain payments (leveraging Creditcoin’s smart-contract layer), community incentives (cadet/airdrop programs), and aims to let participants add satellites and join the network.

Surf's project profile maps Spacecoin to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://spacecoin.org/](https://spacecoin.org/) |
| X handle | [@spacecoin](https://x.com/spacecoin) |
| X followers | 543.16K |
...
| Liquidity | $4.07M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 5.55B circulating vs 21.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Spacecoin is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| bsc | SPACE | 0x87acfa3fd7a6e0d48677d070644d76905c2bdc00 | [explorer](https://bscscan.com/token/0x87acfa3fd7a6e0d48677d070644d76905c2bdc00) |
| ethereum | SPACE | 0x87acfa3fd7a6e0d48677d070644d76905c2bdc00 | [explorer](https://etherscan.io/token/0x87acfa3fd7a6e0d48677d070644d76905c2bdc00) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Spacecoin, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 583, market cap $34,158,792, 24h volume $4,779,983. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $34.41M, FDV $130.11M, 24h volume $4.07M, and price $0.0062.
...
| 24h volume | $4.07M | Position sizing and exit liquidity proxy |
| Market cap | $34.41M | Liquid valuation proxy |
| FDV | $130.11M | Dilution-aware valuation proxy |
| FDV / market cap spread | 3.78x | Higher spread means more supply risk |
| X followers | 543.16K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Spacecoin | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether SPACE is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Spacecoin currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d -1.17% and 30d 6.44% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Spacecoin's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|
| Scott Hasbrouck | CTO | not disclosed |
...
Spacecoin competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Spacecoin has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Spacecoin is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Spacecoin compounds usage, liquidity improves, and SPACE gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If SPACE does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $4.07M | Weekly | Volume below position-size threshold |
| FDV / market cap | 3.78x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Spacecoin announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Spacecoin has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Spacecoin, the first reconciliation job is identity. The token symbol is SPACE, the category is DeFi infrastructure, chains are Binance Smart Chain, Ethereum, exchanges are OKX, BITHUMB, BINANCE ALPHA, BITGET, and tags are DePIN (Physical Infra). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If SPACE trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $34.41M, FDV is $130.11M, 24h volume is $4.07M, FDV / market cap is 3.78x, 7d price change is -1.17%, and 30d price change is 6.44%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Spacecoin / SPACE | Match official docs, CG, CMC, Surf, contracts |
...
| Exchanges | OKX, BITHUMB, BINANCE ALPHA, BITGET | Check live order books and inactive pairs |
| Market cap | $34.41M | Compare Surf, CG, CMC, exchange pages |
| FDV | $130.11M | Reconcile total supply and unlock schedule |
| Volume | $4.07M | Check exchange concentration and wash-trading risk |
| TVL | not disclosed | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Spacecoin does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Spacecoin, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does SPACE sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
For Spacecoin, the current liquidity read is watchlist first. The market cap is $34.41M, FDV is $130.11M, reported 24h volume is $4.07M, and FDV / market cap is 3.78x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Spacecoin gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://spacecoin.org/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Spacecoin) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Spacecoin) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Spacecoin) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Spacecoin) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Spacecoin) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/spacecoin) | Identity, market, social, contract, or discovery cross-check |
| 8 | Spacecoin Price Chart (SPACE) - CoinGecko | [link](https://www.coingecko.com/en/coins/spacecoin) | Identity, market, social, contract, or discovery cross-check |
| 9 | Spacecoin (SPACE) Price Prediction For 2026 & Beyond | [link](https://coinmarketcap.com/cmc-ai/spacecoin-org/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 10 | MicrovisionChain Price Chart (SPACE) - CoinGecko | [link](https://www.coingecko.com/en/coins/microvisionchain) | Identity, market, social, contract, or discovery cross-check |
| 11 | Space Token Price Chart (SPACE) - CoinGecko | [link](https://www.coingecko.com/en/coins/space-token-2) | Identity, market, social, contract, or discovery cross-check |
| 12 | What Is Spacecoin (SPACE) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/spacecoin-org/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 13 | bsc SPACE | [link](https://bscscan.com/token/0x87acfa3fd7a6e0d48677d070644d76905c2bdc00) | Identity, market, social, contract, or discovery cross-check |
| 14 | ethereum SPACE | [link](https://etherscan.io/token/0x87acfa3fd7a6e0d48677d070644d76905c2bdc00) | Identity, market, social, contract, or discovery cross-check |
| 15 | Google project search | [link](https://www.google.com/search?q=Spacecoin%20SPACE%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google tokenomics search | [link](https://www.google.com/search?q=Spacecoin%20SPACE%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google docs search | [link](https://www.google.com/search?q=Spacecoin%20docs) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google governance search | [link](https://www.google.com/search?q=Spacecoin%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google audit search | [link](https://www.google.com/search?q=Spacecoin%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 20 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/spacecoin/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/spacecoin/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/spacecoin/) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Spacecoin) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune token query | [link](https://dune.com/browse/queries?q=Spacecoin) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Spacecoin) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=SPACE) | Identity, market, social, contract, or discovery cross-check |
| 27 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Spacecoin) | Identity, market, social, contract, or discovery cross-check |
| 28 | CryptoRank search | [link](https://cryptorank.io/price/spacecoin) | Identity, market, social, contract, or discovery cross-check |
| 29 | RootData search | [link](https://www.rootdata.com/search?kw=Spacecoin) | Identity, market, social, contract, or discovery cross-check |
| 30 | CoinDesk search | [link](https://www.coindesk.com/search?s=Spacecoin) | Identity, market, social, contract, or discovery cross-check |
| 31 | The Block search | [link](https://www.theblock.co/search?query=Spacecoin) | Identity, market, social, contract, or discovery cross-check |
| 32 | GitHub search | [link](https://github.com/search?q=Spacecoin&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 33 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=SPACE) | Identity, market, social, contract, or discovery cross-check |
| 34 | BscScan token search | [link](https://bscscan.com/search?f=0&q=SPACE) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://spacecoin.org/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Spacecoin)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Spacecoin)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Spacecoin)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Spacecoin)
- [Messari discovery](https://messari.io/search?q=Spacecoin)
- [X profile](https://x.com/spacecoin)
- [Spacecoin Price Chart (SPACE) - CoinGecko](https://www.coingecko.com/en/coins/spacecoin)
- [Spacecoin (SPACE) Price Prediction For 2026 & Beyond](https://coinmarketcap.com/cmc-ai/spacecoin-org/price-prediction/)
- [MicrovisionChain Price Chart (SPACE) - CoinGecko](https://www.coingecko.com/en/coins/microvisionchain)
- [Space Token Price Chart (SPACE) - CoinGecko](https://www.coingecko.com/en/coins/space-token-2)
- [What Is Spacecoin (SPACE) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/spacecoin-org/what-is/)
- [bsc SPACE](https://bscscan.com/token/0x87acfa3fd7a6e0d48677d070644d76905c2bdc00)
- [ethereum SPACE](https://etherscan.io/token/0x87acfa3fd7a6e0d48677d070644d76905c2bdc00)
- [Google project search](https://www.google.com/search?q=Spacecoin%20SPACE%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Spacecoin%20SPACE%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Spacecoin%20docs)
- [Google governance search](https://www.google.com/search?q=Spacecoin%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Spacecoin%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/spacecoin/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/spacecoin/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/spacecoin/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Spacecoin)
- [Dune token query](https://dune.com/browse/queries?q=Spacecoin)

## Final Investment View

Spacecoin should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
