---
title: 'Siacoin / SC: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Siacoin is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'siacoin-sc-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# Siacoin / SC: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Siacoin appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is not disclosed; exchange exposure is BINANCE, UPBIT, OKX, BITHUMB, BYBIT; category tags are Layer1, DePIN (Physical Infra); current market cap is $32.12M; FDV is $32.12M; and 24h volume is $2.75M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Siacoin before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 614, market cap $31,672,803, 24h volume $2,429,718. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Siacoin is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether SC captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Exchanges | BINANCE, UPBIT, OKX, BITHUMB, BYBIT |
| Price | $0.0006 |
| Market cap | $32.12M |
| FDV | $32.12M |
| 24h volume | $2.75M |
| Circulating supply | 49.75B |
| Total supply | 49.75B |
...
Base case: Siacoin remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning SC into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Sia is an open-source, decentralized cloud-storage platform that creates a marketplace for renters and hosts, using encryption, erasure-coding, and blockchain contracts to secure and pay for storage. Siacoin (SC) is the native utility token used to pay for storage and incentivize hosts, and the Sia Foundation stewards ongoing protocol development and ecosystem work.

Surf's project profile maps Siacoin to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://sia.tech/](https://sia.tech/) |
| X handle | [@Sia__Foundation](https://x.com/Sia__Foundation) |
| X followers | 0 |
| Tags | Layer1, DePIN (Physical Infra) |
| Chains | not disclosed |
| Exchanges | BINANCE, UPBIT, OKX, BITHUMB, BYBIT |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Siacoin is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DeFi infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Siacoin convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Supply quality | 49.75B circulating vs 49.75B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Siacoin is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

Siacoin's architecture should be evaluated through the job it performs. For a DeFi infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Siacoin, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 614, market cap $31,672,803, 24h volume $2,429,718. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $32.12M, FDV $32.12M, 24h volume $2.75M, and price $0.0006.
...
| Market cap | $32.12M | Liquid valuation proxy |
| FDV | $32.12M | Dilution-aware valuation proxy |
...
| Accrual channel | Strength for Siacoin | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether SC is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Siacoin currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 1.22% and 30d -6.24% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Siacoin's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2020-09-23 | Seed | $3.00M | Paradigm, Dragonfly, a_capital, Hack VC, Bain Capital Crypto, Collaborative Fund, Bessemer Venture Partners, INBlockchain |

| Team member | Role | Link |
|---|---|---|
| Luke Champine | Founder, Blockchain Researcher | [X](https://twitter.com/lukechampine) |
| David Vorick | Co-Founder | [X](https://twitter.com/DavidVorick) |
| Nate Maninger | President | not disclosed |
| Josh Cincinatti | Advisor | [X](https://twitter.com/acityinohio) |
| James Prestwich | Advisor | [X](https://twitter.com/_prestwich) |
| Eddie Wang | Chairman | [X](https://twitter.com/eddiepluswang) |
...
Siacoin competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Siacoin has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Siacoin is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Siacoin compounds usage, liquidity improves, and SC gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Siacoin has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If SC does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
1. Re-run Surf project-detail and project-defi-metrics if Siacoin announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Siacoin has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Siacoin, the first reconciliation job is identity. The token symbol is SC, the category is DeFi infrastructure, chains are not disclosed, exchanges are BINANCE, UPBIT, OKX, BITHUMB, BYBIT, and tags are Layer1, DePIN (Physical Infra). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If SC trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $32.12M, FDV is $32.12M, 24h volume is $2.75M, FDV / market cap is 1.00x, 7d price change is 1.22%, and 30d price change is -6.24%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Siacoin / SC | Match official docs, CG, CMC, Surf, contracts |
...
| Exchanges | BINANCE, UPBIT, OKX, BITHUMB, BYBIT | Check live order books and inactive pairs |
| Market cap | $32.12M | Compare Surf, CG, CMC, exchange pages |
| FDV | $32.12M | Reconcile total supply and unlock schedule |
...
For Siacoin, the current liquidity read is watchlist first. The market cap is $32.12M, FDV is $32.12M, reported 24h volume is $2.75M, and FDV / market cap is 1.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Siacoin gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://sia.tech/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Siacoin) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Siacoin) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Siacoin) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Siacoin) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Siacoin) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/Sia__Foundation) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/NebulousLabs/Sia) | Identity, market, social, contract, or discovery cross-check |
| 9 | Siacoin Price: SC/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/siacoin) | Identity, market, social, contract, or discovery cross-check |
| 10 | Siacoin price today, SC to USD live price, marketcap and chart | [link](https://coinmarketcap.com/currencies/siacoin/) | Identity, market, social, contract, or discovery cross-check |
...
| 12 | What Is Siacoin? Features and Tokenomics - CoinMarketCap | [link](https://coinmarketcap.com/academy/article/what-is-siacoin-features-and-tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 13 | What Is Siacoin (SC) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/siacoin/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 14 | Google project search | [link](https://www.google.com/search?q=Siacoin%20SC%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 15 | Google tokenomics search | [link](https://www.google.com/search?q=Siacoin%20SC%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google docs search | [link](https://www.google.com/search?q=Siacoin%20docs) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google governance search | [link](https://www.google.com/search?q=Siacoin%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google audit search | [link](https://www.google.com/search?q=Siacoin%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 19 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/siacoin/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 20 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Siacoin) | Identity, market, social, contract, or discovery cross-check |
| 21 | Dune token query | [link](https://dune.com/browse/queries?q=Siacoin) | Identity, market, social, contract, or discovery cross-check |
| 22 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Siacoin) | Identity, market, social, contract, or discovery cross-check |
| 23 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=SC) | Identity, market, social, contract, or discovery cross-check |
| 24 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Siacoin) | Identity, market, social, contract, or discovery cross-check |
| 25 | CryptoRank search | [link](https://cryptorank.io/price/siacoin) | Identity, market, social, contract, or discovery cross-check |
| 26 | RootData search | [link](https://www.rootdata.com/search?kw=Siacoin) | Identity, market, social, contract, or discovery cross-check |
| 27 | CoinDesk search | [link](https://www.coindesk.com/search?s=Siacoin) | Identity, market, social, contract, or discovery cross-check |
| 28 | The Block search | [link](https://www.theblock.co/search?query=Siacoin) | Identity, market, social, contract, or discovery cross-check |
| 29 | GitHub search | [link](https://github.com/search?q=Siacoin&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 30 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=SC) | Identity, market, social, contract, or discovery cross-check |
| 31 | BscScan token search | [link](https://bscscan.com/search?f=0&q=SC) | Identity, market, social, contract, or discovery cross-check |
| 32 | Arbiscan token search | [link](https://arbiscan.io/search?f=0&q=SC) | Identity, market, social, contract, or discovery cross-check |
| 33 | BaseScan token search | [link](https://basescan.org/search?f=0&q=SC) | Identity, market, social, contract, or discovery cross-check |
| 34 | RWA.xyz search | [link](https://app.rwa.xyz/search?query=Siacoin) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://sia.tech/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Siacoin)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Siacoin)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Siacoin)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Siacoin)
- [Messari discovery](https://messari.io/search?q=Siacoin)
- [X profile](https://x.com/Sia__Foundation)
- [GitHub](https://github.com/NebulousLabs/Sia)
- [Siacoin Price: SC/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/siacoin)
- [Siacoin price today, SC to USD live price, marketcap and chart](https://coinmarketcap.com/currencies/siacoin/)
- [CoinGecko: Cryptocurrency Prices, Charts, and Crypto Market Cap](https://www.coingecko.com/)
- [What Is Siacoin? Features and Tokenomics - CoinMarketCap](https://coinmarketcap.com/academy/article/what-is-siacoin-features-and-tokenomics)
- [What Is Siacoin (SC) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/siacoin/what-is/)
- [Google project search](https://www.google.com/search?q=Siacoin%20SC%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Siacoin%20SC%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Siacoin%20docs)
- [Google governance search](https://www.google.com/search?q=Siacoin%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Siacoin%20audit%20report)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/siacoin/price-analysis/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Siacoin)
- [Dune token query](https://dune.com/browse/queries?q=Siacoin)
- [DeFiLlama fees discovery](https://defillama.com/fees?chain=All&search=Siacoin)
- [DeFiLlama yield discovery](https://defillama.com/yields?token=SC)
- [TokenUnlocks search](https://token.unlocks.app/?search=Siacoin)

## Final Investment View

Siacoin should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
