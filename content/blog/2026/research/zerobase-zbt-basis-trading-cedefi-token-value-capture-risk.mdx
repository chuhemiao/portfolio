---
title: 'ZEROBASE / ZBT: Basis trading / CeDeFi value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'ZEROBASE is treated as a Basis trading / CeDeFi watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'zerobase-zbt-basis-trading-cedefi-token-value-capture-risk'
category: 'research'
---

# ZEROBASE / ZBT: Basis trading / CeDeFi value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** ZEROBASE appears in the local CoinGecko/CMC-oriented candidate pool through [CoinGecko](https://www.coingecko.com/en/coins/zerobase) and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Base, Binance Smart Chain, Ethereum; exchange exposure is BINANCE, UPBIT, BITHUMB, BYBIT, BITGET; category tags are DePIN (Physical Infra), Privacy & ZK; current market cap is $31.44M; FDV is $128.31M; and 24h volume is $6.99M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag ZEROBASE before this file was created, and the existing candidate note says: DefiLlama category Basis Trading on Multi-Chain; TVL $56,044,391. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

ZEROBASE is best understood as a **Basis trading / CeDeFi** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether ZBT captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.

| Metric | Current read |
|---|---|
| Project | ZEROBASE |
| Token | ZBT |
| Category | Basis trading / CeDeFi |
...
Base case: ZEROBASE remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning ZBT into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

ZEROBASE is a real-time, decentralized zero-knowledge (ZK) prover network that generates proofs in milliseconds to enable privacy-preserving, low-latency applications such as zkLogin and zkDarkPool. It employs a HUB–Prover architecture with TEE support for compliance and a ZBT-based economic model for staking and prover incentives.

Surf's project profile maps ZEROBASE to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://zerobase.pro/](https://zerobase.pro/) |
| X handle | [@zerobasezk](https://x.com/zerobasezk) |
| X followers | 68.65K |
| Tags | DePIN (Physical Infra), Privacy & ZK |
| Chains | Base, Binance Smart Chain, Ethereum |
| Exchanges | BINANCE, UPBIT, BITHUMB, BYBIT, BITGET |
| TGE status | post |
| CoinGecko ID | [zerobase](https://www.coingecko.com/en/coins/zerobase) |

The main product question is whether ZEROBASE is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For Basis trading / CeDeFi projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does ZEROBASE convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $6.99M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 244.99M circulating vs 1.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that ZEROBASE is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

ZEROBASE's architecture should be evaluated through the job it performs. For a Basis trading / CeDeFi project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
|---|---|---|---|
| base | ZBT | 0xfab99fcf605fd8f4593edb70a43ba56542777777 | [explorer](https://basescan.org/token/0xfab99fcf605fd8f4593edb70a43ba56542777777) |
| ethereum | ZBT | 0xfab99fcf605fd8f4593edb70a43ba56542777777 | [explorer](https://etherscan.io/token/0xfab99fcf605fd8f4593edb70a43ba56542777777) |
| bsc | ZBT | 0xfab99fcf605fd8f4593edb70a43ba56542777777 | [explorer](https://bscscan.com/token/0xfab99fcf605fd8f4593edb70a43ba56542777777) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For ZEROBASE, the DeFi-oriented backlog note is the first signal: DefiLlama category Basis Trading on Multi-Chain; TVL $56,044,391. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $31.44M, FDV $128.31M, 24h volume $6.99M, and price $0.1283.
...
| 24h volume | $6.99M | Position sizing and exit liquidity proxy |
| Market cap | $31.44M | Liquid valuation proxy |
| FDV | $128.31M | Dilution-aware valuation proxy |
| FDV / market cap spread | 4.08x | Higher spread means more supply risk |
| X followers | 68.65K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for ZEROBASE | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether ZBT is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. ZEROBASE should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that ZEROBASE belongs to the Basis trading / CeDeFi bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $31.44M, FDV $128.31M, 24h volume $6.99M, price $0.1283, circulating supply 244.99M, and total supply 1.00B. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For ZEROBASE, the candidate backlog records DefiLlama category Basis Trading on Multi-Chain; TVL $56,044,391. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Base, Binance Smart Chain, Ethereum, exchanges as BINANCE, UPBIT, BITHUMB, BYBIT, BITGET, and tags as DePIN (Physical Infra), Privacy & ZK. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.

The fourth frame is value capture. A Basis trading / CeDeFi asset can be useful while the token remains weak. The memo therefore separates product-market fit from token-market fit. Product-market fit asks whether users need the protocol even when incentives fade. Token-market fit asks whether the token absorbs value through staking, fee routing, collateral, governance constraints, emissions control, slashing insurance, or privileged access. If the token is mostly governance, the base case assigns lower confidence. If the token is used for security or fee capture but the mechanism is not yet activated, the base case treats value capture as an option rather than as current cash flow.

The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. ZEROBASE currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 21.99% and 30d 16.24% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that ZEROBASE's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2024-10-19 | Undisclosed | $5.00M | YZi Labs (Prev. Binance Labs), PAKA, MH Ventures, Faction, dao5, K24 Ventures, IDG Capital, 10K VC |

| Team member | Role | Link |
|---|---|---|
| Mirror Tang | CEO, Co-Founder | [X](https://x.com/mirrorzk) |
| Koppany Smith | COO | not disclosed |
| Gink Chen | CMO | [X](https://x.com/Gink5814) |
| Jacky Cao | VP, Ecosystem Strategy | [X](https://x.com/JackyONLINE9) |
...
ZEROBASE competes in the Basis trading / CeDeFi bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether ZEROBASE has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: ZEROBASE is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | ZEROBASE compounds usage, liquidity improves, and ZBT gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If ZBT does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $6.99M | Weekly | Volume below position-size threshold |
| FDV / market cap | 4.08x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if ZEROBASE announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. ZEROBASE has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For ZEROBASE, the first reconciliation job is identity. The token symbol is ZBT, the category is Basis trading / CeDeFi, chains are Base, Binance Smart Chain, Ethereum, exchanges are BINANCE, UPBIT, BITHUMB, BYBIT, BITGET, and tags are DePIN (Physical Infra), Privacy & ZK. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If ZBT trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $31.44M, FDV is $128.31M, 24h volume is $6.99M, FDV / market cap is 4.08x, 7d price change is 21.99%, and 30d price change is 16.24%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.

The third reconciliation job is usage data. The latest TVL sample is not disclosed, TVL sample change is not disclosed, and the sample contains 0 point(s). TVL is useful only when the underlying balances are real, withdrawable, and not double-counted. For Basis trading / CeDeFi, the memo should identify whether TVL means deposits, staked assets, LP liquidity, stablecoin supply, collateral, locked governance assets, or a derived wrapper. A yield-bearing stablecoin and a validator middleware protocol can both show TVL, but the risk encoded by that TVL is completely different.

| Reconciliation item | Current value | Next check |
|---|---|---|
| Identity | ZEROBASE / ZBT | Match official docs, CG, CMC, Surf, contracts |
| Category | Basis trading / CeDeFi | Confirm product mechanics and revenue model |
...
| Correlation check | Count exposure to related Basis trading / CeDeFi assets | Category beta can hide concentration |
...
The failure-mode view is deliberately harsher than the base case. ZEROBASE can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.
...
Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If ZEROBASE suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | ZEROBASE gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://zerobase.pro/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko profile | [link](https://www.coingecko.com/en/coins/zerobase) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=ZEROBASE) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=ZEROBASE) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=ZEROBASE) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=ZEROBASE) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/zerobasezk) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/zerobasezk) | Identity, market, social, contract, or discovery cross-check |
| 9 | Discord | [link](https://discord.com/invite/zerobasezk) | Identity, market, social, contract, or discovery cross-check |
| 10 | ZEROBASE price today, ZBT to USD live price, marketcap and chart | [link](https://coinmarketcap.com/currencies/zerobase/) | Identity, market, social, contract, or discovery cross-check |
| 11 | ZEROBASE Price History: Download ZBT Historical Data / CoinGecko | [link](https://www.coingecko.com/en/coins/zerobase/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 12 | What Is ZEROBASE (ZBT) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/zerobase/what-is/) | Identity, market, social, contract, or discovery cross-check |
...
| 14 | base ZBT | [link](https://basescan.org/token/0xfab99fcf605fd8f4593edb70a43ba56542777777) | Identity, market, social, contract, or discovery cross-check |
| 15 | ethereum ZBT | [link](https://etherscan.io/token/0xfab99fcf605fd8f4593edb70a43ba56542777777) | Identity, market, social, contract, or discovery cross-check |
| 16 | bsc ZBT | [link](https://bscscan.com/token/0xfab99fcf605fd8f4593edb70a43ba56542777777) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google project search | [link](https://www.google.com/search?q=ZEROBASE%20ZBT%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google tokenomics search | [link](https://www.google.com/search?q=ZEROBASE%20ZBT%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google docs search | [link](https://www.google.com/search?q=ZEROBASE%20docs) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google governance search | [link](https://www.google.com/search?q=ZEROBASE%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google audit search | [link](https://www.google.com/search?q=ZEROBASE%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/zerobase/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune discovery | [link](https://dune.com/browse/dashboards?q=ZEROBASE) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune token query | [link](https://dune.com/browse/queries?q=ZEROBASE) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=ZEROBASE) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=ZBT) | Identity, market, social, contract, or discovery cross-check |
| 27 | TokenUnlocks search | [link](https://token.unlocks.app/?search=ZEROBASE) | Identity, market, social, contract, or discovery cross-check |
| 28 | CryptoRank search | [link](https://cryptorank.io/price/zerobase) | Identity, market, social, contract, or discovery cross-check |
| 29 | RootData search | [link](https://www.rootdata.com/search?kw=ZEROBASE) | Identity, market, social, contract, or discovery cross-check |
| 30 | CoinDesk search | [link](https://www.coindesk.com/search?s=ZEROBASE) | Identity, market, social, contract, or discovery cross-check |
| 31 | The Block search | [link](https://www.theblock.co/search?query=ZEROBASE) | Identity, market, social, contract, or discovery cross-check |
| 32 | GitHub search | [link](https://github.com/search?q=ZEROBASE&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 33 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=ZBT) | Identity, market, social, contract, or discovery cross-check |
| 34 | BscScan token search | [link](https://bscscan.com/search?f=0&q=ZBT) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://zerobase.pro/)
- [CoinGecko profile](https://www.coingecko.com/en/coins/zerobase)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=ZEROBASE)
- [DefiLlama protocol discovery](https://defillama.com/search?query=ZEROBASE)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=ZEROBASE)
- [Messari discovery](https://messari.io/search?q=ZEROBASE)
- [X profile](https://x.com/zerobasezk)
- [Telegram](https://t.me/zerobasezk)
- [Discord](https://discord.com/invite/zerobasezk)
- [ZEROBASE price today, ZBT to USD live price, marketcap and chart](https://coinmarketcap.com/currencies/zerobase/)
- [ZEROBASE Price History: Download ZBT Historical Data | CoinGecko](https://www.coingecko.com/en/coins/zerobase/historical_data)
- [What Is ZEROBASE (ZBT) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/zerobase/what-is/)
- [CoinGecko: Cryptocurrency Prices, Charts, and Crypto Market Cap](https://www.coingecko.com/)
- [base ZBT](https://basescan.org/token/0xfab99fcf605fd8f4593edb70a43ba56542777777)
- [ethereum ZBT](https://etherscan.io/token/0xfab99fcf605fd8f4593edb70a43ba56542777777)
- [bsc ZBT](https://bscscan.com/token/0xfab99fcf605fd8f4593edb70a43ba56542777777)
- [Google project search](https://www.google.com/search?q=ZEROBASE%20ZBT%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=ZEROBASE%20ZBT%20tokenomics)
- [Google docs search](https://www.google.com/search?q=ZEROBASE%20docs)
- [Google governance search](https://www.google.com/search?q=ZEROBASE%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=ZEROBASE%20audit%20report)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/zerobase/price-analysis/)
- [Dune discovery](https://dune.com/browse/dashboards?q=ZEROBASE)
- [Dune token query](https://dune.com/browse/queries?q=ZEROBASE)

## Final Investment View

ZEROBASE should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
