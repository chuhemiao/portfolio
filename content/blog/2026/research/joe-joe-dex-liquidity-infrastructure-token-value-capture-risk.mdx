---
title: 'JOE / JOE: DEX / liquidity infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'JOE is treated as a DEX / liquidity infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'joe-joe-dex-liquidity-infrastructure-token-value-capture-risk'
category: 'research'
---

# JOE / JOE: DEX / liquidity infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** JOE appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Arbitrum One, Avalanche, Binance Smart Chain, Mantle; exchange exposure is BINANCE, OKX, BITHUMB, BITGET; category tags are DEX & AMM, Cross-Chain & Bridge, ICO/IDO/IEO launchpad; current market cap is $13.67M; FDV is $14.94M; and 24h volume is $4.84M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag JOE before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 1011, market cap $13,690,531, 24h volume $4,878,925. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

JOE is best understood as a **DEX / liquidity infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether JOE captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Arbitrum One, Avalanche, Binance Smart Chain, Mantle |
| Exchanges | BINANCE, OKX, BITHUMB, BITGET |
| Price | $0.0299 |
| Market cap | $13.67M |
| FDV | $14.94M |
| 24h volume | $4.84M |
| Circulating supply | 457.18M |
| Total supply | 499.71M |
| 7d price change | 3.98% |
| 30d price change | -0.19% |
| Latest TVL sample | $127.36M |
| TVL sample change | 32.64% |
| Primary source posture | Surf + CoinGecko + CMC discovery + official/project links |

Base case: JOE remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning JOE into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

LFJ (LFJ.gg — sometimes labeled “Trader Joe” on data sites) is an on‑chain trading platform and DEX/aggregator offering token discovery, a cross‑chain meta‑aggregator, and advanced orders (limit, DCA, stop‑loss). It also runs Token Mill, a launchpad (“King of the Mill”) that channels trading fees into concentrated token rounds.

Surf's project profile maps JOE to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://lfj.gg/](https://lfj.gg/) |
| X handle | [@LFJ_gg](https://x.com/LFJ_gg) |
| X followers | 375.93K |
| Tags | DEX & AMM, Cross-Chain & Bridge, ICO/IDO/IEO launchpad |
| Chains | Arbitrum One, Avalanche, Binance Smart Chain, Mantle |
| Exchanges | BINANCE, OKX, BITHUMB, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether JOE is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DEX / liquidity infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does JOE convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $4.84M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 457.18M circulating vs 499.71M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that JOE is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Arbitrum One, Avalanche, Binance Smart Chain, Mantle |
...
|---|---|---|---|
| Mantle | JOE | 0x371c7ec6d8039ff7933a2aa28eb827ffe1f52f07 | not mapped |
| avalanche | JOE | 0x6e84a6216ea6dacc71ee8e6b0a5b7322eebc0fdd | [explorer](https://snowtrace.io/token/0x6e84a6216ea6dacc71ee8e6b0a5b7322eebc0fdd) |
| arbitrum | JOE | 0x371c7ec6d8039ff7933a2aa28eb827ffe1f52f07 | [explorer](https://arbiscan.io/token/0x371c7ec6d8039ff7933a2aa28eb827ffe1f52f07) |
| bsc | JOE | 0x371c7ec6d8039ff7933a2aa28eb827ffe1f52f07 | [explorer](https://bscscan.com/token/0x371c7ec6d8039ff7933a2aa28eb827ffe1f52f07) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For JOE, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 1011, market cap $13,690,531, 24h volume $4,878,925. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $13.67M, FDV $14.94M, 24h volume $4.84M, and price $0.0299.

| Traction metric | Value | Interpretation |
|---|---|---|
| Latest TVL sample | $127.36M | Useful only if sustained without aggressive incentives |
| TVL sample points | 90 | More points improve trend confidence |
| TVL sample change | 32.64% | Positive trend supports usage; negative trend requires explanation |
| 24h volume | $4.84M | Position sizing and exit liquidity proxy |
| Market cap | $13.67M | Liquid valuation proxy |
| FDV | $14.94M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.09x | Higher spread means more supply risk |
| X followers | 375.93K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for JOE | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether JOE is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. JOE should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that JOE belongs to the DEX / liquidity infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $13.67M, FDV $14.94M, 24h volume $4.84M, price $0.0299, circulating supply 457.18M, and total supply 499.71M. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For JOE, the candidate backlog records Seeded 2026-07-06 from Surf market-ranking; rank 1011, market cap $13,690,531, 24h volume $4,878,925. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Arbitrum One, Avalanche, Binance Smart Chain, Mantle, exchanges as BINANCE, OKX, BITHUMB, BITGET, and tags as DEX & AMM, Cross-Chain & Bridge, ICO/IDO/IEO launchpad. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. JOE currently needs to be monitored through that sequence. The available 90-day TVL series has 90 point(s), latest TVL $127.36M, and approximate change 32.64%. Market-price movement shows 7d 3.98% and 30d -0.19% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that JOE's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
| 2021-09-02 | Seed | $5.00M | Mechanism Capital, GBV Capital, Three Arrows Capital, Delphi Ventures, DeFiance Capital, Arche Fund, Avalaunch, Stani Kulechov |

| Team member | Role | Link |
|---|---|---|
| Kirill Suslov | Co-founder, CEO | not disclosed |
...
JOE competes in the DEX / liquidity infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether JOE has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: JOE is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | JOE compounds usage, liquidity improves, and JOE gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because JOE has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If JOE does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
|---|---|---|---|
| TVL | $127.36M | Weekly | >20% drawdown or sustained growth |
| 24h volume | $4.84M | Weekly | Volume below position-size threshold |
| FDV / market cap | 1.09x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if JOE announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. JOE has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For JOE, the first reconciliation job is identity. The token symbol is JOE, the category is DEX / liquidity infrastructure, chains are Arbitrum One, Avalanche, Binance Smart Chain, Mantle, exchanges are BINANCE, OKX, BITHUMB, BITGET, and tags are DEX & AMM, Cross-Chain & Bridge, ICO/IDO/IEO launchpad. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If JOE trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $13.67M, FDV is $14.94M, 24h volume is $4.84M, FDV / market cap is 1.09x, 7d price change is 3.98%, and 30d price change is -0.19%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.

The third reconciliation job is usage data. The latest TVL sample is $127.36M, TVL sample change is 32.64%, and the sample contains 90 point(s). TVL is useful only when the underlying balances are real, withdrawable, and not double-counted. For DEX / liquidity infrastructure, the memo should identify whether TVL means deposits, staked assets, LP liquidity, stablecoin supply, collateral, locked governance assets, or a derived wrapper. A yield-bearing stablecoin and a validator middleware protocol can both show TVL, but the risk encoded by that TVL is completely different.

| Reconciliation item | Current value | Next check |
|---|---|---|
| Identity | JOE / JOE | Match official docs, CG, CMC, Surf, contracts |
| Category | DEX / liquidity infrastructure | Confirm product mechanics and revenue model |
| Chains | Arbitrum One, Avalanche, Binance Smart Chain, Mantle | Confirm canonical deployments and bridge assumptions |
| Exchanges | BINANCE, OKX, BITHUMB, BITGET | Check live order books and inactive pairs |
| Market cap | $13.67M | Compare Surf, CG, CMC, exchange pages |
| FDV | $14.94M | Reconcile total supply and unlock schedule |
| Volume | $4.84M | Check exchange concentration and wash-trading risk |
| TVL | $127.36M | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what JOE does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For JOE, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does JOE sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
For JOE, the current liquidity read is watchlist first. The market cap is $13.67M, FDV is $14.94M, reported 24h volume is $4.84M, and FDV / market cap is 1.09x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | JOE gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://lfj.gg/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=JOE) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=JOE) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=JOE) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=JOE) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=JOE) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/LFJ_gg) | Identity, market, social, contract, or discovery cross-check |
| 8 | Discord | [link](https://discord.com/invite/lfj) | Identity, market, social, contract, or discovery cross-check |
| 9 | JOE/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/joe) | Identity, market, social, contract, or discovery cross-check |
| 10 | Joe Coin Price: JOE/USD Live Price Chart, Market Cap ... | [link](https://www.coingecko.com/en/coins/joe-coin) | Identity, market, social, contract, or discovery cross-check |
| 11 | Joe coin price today, JOE to USD live price, marketcap ... | [link](https://coinmarketcap.com/currencies/joe-coin/) | Identity, market, social, contract, or discovery cross-check |
| 12 | JOE/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/joe-2) | Identity, market, social, contract, or discovery cross-check |
| 13 | What Is JOE (JOE) And How Does It Work? | [link](https://coinmarketcap.com/cmc-ai/joe/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 14 | avalanche JOE | [link](https://snowtrace.io/token/0x6e84a6216ea6dacc71ee8e6b0a5b7322eebc0fdd) | Identity, market, social, contract, or discovery cross-check |
| 15 | arbitrum JOE | [link](https://arbiscan.io/token/0x371c7ec6d8039ff7933a2aa28eb827ffe1f52f07) | Identity, market, social, contract, or discovery cross-check |
| 16 | bsc JOE | [link](https://bscscan.com/token/0x371c7ec6d8039ff7933a2aa28eb827ffe1f52f07) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google project search | [link](https://www.google.com/search?q=JOE%20JOE%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google tokenomics search | [link](https://www.google.com/search?q=JOE%20JOE%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google docs search | [link](https://www.google.com/search?q=JOE%20docs) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google governance search | [link](https://www.google.com/search?q=JOE%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google audit search | [link](https://www.google.com/search?q=JOE%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/joe/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/joe/) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune discovery | [link](https://dune.com/browse/dashboards?q=JOE) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune token query | [link](https://dune.com/browse/queries?q=JOE) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=JOE) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=JOE) | Identity, market, social, contract, or discovery cross-check |
| 28 | TokenUnlocks search | [link](https://token.unlocks.app/?search=JOE) | Identity, market, social, contract, or discovery cross-check |
| 29 | CryptoRank search | [link](https://cryptorank.io/price/joe) | Identity, market, social, contract, or discovery cross-check |
| 30 | RootData search | [link](https://www.rootdata.com/search?kw=JOE) | Identity, market, social, contract, or discovery cross-check |
| 31 | CoinDesk search | [link](https://www.coindesk.com/search?s=JOE) | Identity, market, social, contract, or discovery cross-check |
| 32 | The Block search | [link](https://www.theblock.co/search?query=JOE) | Identity, market, social, contract, or discovery cross-check |
| 33 | GitHub search | [link](https://github.com/search?q=JOE&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 34 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=JOE) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://lfj.gg/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=JOE)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=JOE)
- [DefiLlama protocol discovery](https://defillama.com/search?query=JOE)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=JOE)
- [Messari discovery](https://messari.io/search?q=JOE)
- [X profile](https://x.com/LFJ_gg)
- [Discord](https://discord.com/invite/lfj)
- [JOE/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/joe)
- [Joe Coin Price: JOE/USD Live Price Chart, Market Cap ...](https://www.coingecko.com/en/coins/joe-coin)
- [Joe coin price today, JOE to USD live price, marketcap ...](https://coinmarketcap.com/currencies/joe-coin/)
- [JOE/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/joe-2)
- [What Is JOE (JOE) And How Does It Work?](https://coinmarketcap.com/cmc-ai/joe/what-is/)
- [avalanche JOE](https://snowtrace.io/token/0x6e84a6216ea6dacc71ee8e6b0a5b7322eebc0fdd)
- [arbitrum JOE](https://arbiscan.io/token/0x371c7ec6d8039ff7933a2aa28eb827ffe1f52f07)
- [bsc JOE](https://bscscan.com/token/0x371c7ec6d8039ff7933a2aa28eb827ffe1f52f07)
- [Google project search](https://www.google.com/search?q=JOE%20JOE%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=JOE%20JOE%20tokenomics)
- [Google docs search](https://www.google.com/search?q=JOE%20docs)
- [Google governance search](https://www.google.com/search?q=JOE%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=JOE%20audit%20report)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/joe/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/joe/)
- [Dune discovery](https://dune.com/browse/dashboards?q=JOE)

## Final Investment View

JOE should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
