---
title: 'Axelar / AXL: staking infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Axelar is treated as a staking infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'axelar-axl-staking-infrastructure-token-value-capture-risk'
category: 'research'
---

# Axelar / AXL: staking infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Axelar appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Arbitrum One, Archway, Avalanche, Base, Binance Smart Chain, Blast, Celo, Ethereum, Evmos, Fantom, Filecoin, Fraxtal, Immutable, Kava, Linea, Mantle, Moonbeam, Optimistic Ethereum, Osmosis, Polygon Pos, Scroll, Secret, Terra 2; exchange exposure is BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET; category tags are Cross-Chain & Bridge, Node & Validator; current market cap is $50.79M; FDV is $52.81M; and 24h volume is $4.55M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Axelar before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 430, market cap $50,986,919, 24h volume $1,931,086. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Axelar is best understood as a **staking infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether AXL captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Arbitrum One, Archway, Avalanche, Base, Binance Smart Chain, Blast, Celo, Ethereum, Evmos, Fantom, Filecoin, Fraxtal, Immutable, Kava, Linea, Mantle, Moonbeam, Optimistic Ethereum, Osmosis, Polygon Pos, Scroll, Secret, Terra 2 |
...
Base case: Axelar remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning AXL into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Axelar is a decentralized interoperability network and development platform that securely connects blockchains, enabling programmable cross-chain communication, token transfers, and smart contracts via General Message Passing and the Axelar Virtual Machine. AXL is the native token used for staking, governance, and fees; Axelar provides SDKs, APIs and tooling for building interchain dApps.

Surf's project profile maps Axelar to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://axelar.network](https://axelar.network) |
| X handle | [@axelar](https://x.com/axelar) |
| X followers | 215.71K |
...
| Liquidity | $4.55M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 1.20B circulating vs 1.25B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Axelar is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Arbitrum One, Archway, Avalanche, Base, Binance Smart Chain, Blast, Celo, Ethereum, Evmos, Fantom, Filecoin, Fraxtal, Immutable, Kava, Linea, Mantle, Moonbeam, Optimistic Ethereum, Osmosis, Polygon Pos, Scroll, Secret, Terra 2 |
...
|---|---|---|---|
| base | AXL | 0x23ee2343b892b1bb63503a4fabc840e0e2c6810f | [explorer](https://basescan.org/token/0x23ee2343b892b1bb63503a4fabc840e0e2c6810f) |
| fantom | AXL | 0x8b1f4432f943c465a973fedc6d7aa50fc96f1f65 | not mapped |
| Secret | AXL | secret1vcau4rkn7mvfwl8hf0dqa9p0jr59983e3qqe3z | not mapped |
| Osmosis | AXL | ibc/903A61A498756EA560B85A85132D3AEE21B5DEDD41213725D22ABF276EA6945E | not mapped |
| ethereum | AXL | 0x467719ad09025fcc6cf6f8311755809d45a5e5f3 | [explorer](https://etherscan.io/token/0x467719ad09025fcc6cf6f8311755809d45a5e5f3) |
| Moonbeam | AXL | 0x467719ad09025fcc6cf6f8311755809d45a5e5f3 | not mapped |
| avalanche | AXL | 0x44c784266cf024a60e8acf2427b9857ace194c5d | [explorer](https://snowtrace.io/token/0x44c784266cf024a60e8acf2427b9857ace194c5d) |
| polygon | AXL | 0x6e4e624106cb12e168e6533f8ec7c82263358940 | [explorer](https://polygonscan.com/token/0x6e4e624106cb12e168e6533f8ec7c82263358940) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Axelar, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 430, market cap $50,986,919, 24h volume $1,931,086. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $50.79M, FDV $52.81M, 24h volume $4.55M, and price $0.0424.
...
| 24h volume | $4.55M | Position sizing and exit liquidity proxy |
| Market cap | $50.79M | Liquid valuation proxy |
| FDV | $52.81M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.04x | Higher spread means more supply risk |
| X followers | 215.71K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Axelar | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether AXL is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. Axelar should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that Axelar belongs to the staking infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $50.79M, FDV $52.81M, 24h volume $4.55M, price $0.0424, circulating supply 1.20B, and total supply 1.25B. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For Axelar, the candidate backlog records Seeded 2026-07-06 from Surf market-ranking; rank 430, market cap $50,986,919, 24h volume $1,931,086. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Arbitrum One, Archway, Avalanche, Base, Binance Smart Chain, Blast, Celo, Ethereum, Evmos, Fantom, Filecoin, Fraxtal, Immutable, Kava, Linea, Mantle, Moonbeam, Optimistic Ethereum, Osmosis, Polygon Pos, Scroll, Secret, Terra 2, exchanges as BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET, and tags as Cross-Chain & Bridge, Node & Validator. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Axelar currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 3.82% and 30d 1.77% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Axelar's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-03-11 | Otc Purchase | $30.00M | Arrington Capital, Electric Capital, Distributed Global, C² Ventures, Plassa Capital, Laser Digital, WAGMI Ventures |
| 2022-09-26 | Ieo | $80.00K | not disclosed |
| 2022-03-03 | Ico | $50.00M | not disclosed |
| 2022-02-15 | Series B | $35.00M | Dragonfly, Polychain Capital, Lemniscap, RockawayX, Blockchange Ventures |
| 2021-11-22 | Strategic | not disclosed | Morningstar Ventures, Crypto.com Capital |
| 2021-07-15 | Series A | $25.00M | Divergence Ventures, Dragonfly, Polychain Capital, Galaxy, Lemniscap, Hypersphere Ventures, Robot Ventures, Collab+Currency |

| Team member | Role | Link |
|---|---|---|
| Sergey Gorbunov | Co-Founder, CEO | [X](https://twitter.com/sergey_nog) |
| Georgios Vlachos | Co-Founder | [X](https://twitter.com/yorgosv_) |
| Monica Puchner | Advisor | [X](https://twitter.com/monicapuchner) |
| Stefan Mueller | Advisor | not disclosed |
...
Axelar competes in the staking infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Axelar has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Axelar is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Axelar compounds usage, liquidity improves, and AXL gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Axelar has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If AXL does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $4.55M | Weekly | Volume below position-size threshold |
| FDV / market cap | 1.04x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Axelar announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Axelar has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Axelar, the first reconciliation job is identity. The token symbol is AXL, the category is staking infrastructure, chains are Arbitrum One, Archway, Avalanche, Base, Binance Smart Chain, Blast, Celo, Ethereum, Evmos, Fantom, Filecoin, Fraxtal, Immutable, Kava, Linea, Mantle, Moonbeam, Optimistic Ethereum, Osmosis, Polygon Pos, Scroll, Secret, Terra 2, exchanges are BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET, and tags are Cross-Chain & Bridge, Node & Validator. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If AXL trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $50.79M, FDV is $52.81M, 24h volume is $4.55M, FDV / market cap is 1.04x, 7d price change is 3.82%, and 30d price change is 1.77%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Axelar / AXL | Match official docs, CG, CMC, Surf, contracts |
| Category | staking infrastructure | Confirm product mechanics and revenue model |
| Chains | Arbitrum One, Archway, Avalanche, Base, Binance Smart Chain, Blast, Celo, Ethereum, Evmos, Fantom, Filecoin, Fraxtal, Immutable, Kava, Linea, Mantle, Moonbeam, Optimistic Ethereum, Osmosis, Polygon Pos, Scroll, Secret, Terra 2 | Confirm canonical deployments and bridge assumptions |
| Exchanges | BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET | Check live order books and inactive pairs |
| Market cap | $50.79M | Compare Surf, CG, CMC, exchange pages |
| FDV | $52.81M | Reconcile total supply and unlock schedule |
| Volume | $4.55M | Check exchange concentration and wash-trading risk |
| TVL | not disclosed | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Axelar does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Axelar, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does AXL sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
For Axelar, the current liquidity read is watchlist first. The market cap is $50.79M, FDV is $52.81M, reported 24h volume is $4.55M, and FDV / market cap is 1.04x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Axelar gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://axelar.network) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Axelar) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Axelar) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Axelar) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Axelar) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Axelar) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/axelar) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/axelarnetwork) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/axelarcommunity) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](https://discord.gg/axelar) | Identity, market, social, contract, or discovery cross-check |
| 11 | Axelar Price: AXL/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/axelar) | Identity, market, social, contract, or discovery cross-check |
| 12 | Axelar Price History: Download AXL Historical Data / CoinGecko | [link](https://www.coingecko.com/en/coins/axelar/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 13 | What Is Axelar (AXL) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/axelar/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 14 | The AXL Token & the Interchain Future / Axelar Blog | [link](https://www.axelar.network/blog/axl-token-post-summary) | Identity, market, social, contract, or discovery cross-check |
| 15 | Axelar Bridged USDC Price Chart (AXLUSDC) - CoinGecko | [link](https://www.coingecko.com/en/coins/axelar-bridged-usdc) | Identity, market, social, contract, or discovery cross-check |
| 16 | base AXL | [link](https://basescan.org/token/0x23ee2343b892b1bb63503a4fabc840e0e2c6810f) | Identity, market, social, contract, or discovery cross-check |
| 17 | ethereum AXL | [link](https://etherscan.io/token/0x467719ad09025fcc6cf6f8311755809d45a5e5f3) | Identity, market, social, contract, or discovery cross-check |
| 18 | avalanche AXL | [link](https://snowtrace.io/token/0x44c784266cf024a60e8acf2427b9857ace194c5d) | Identity, market, social, contract, or discovery cross-check |
| 19 | polygon AXL | [link](https://polygonscan.com/token/0x6e4e624106cb12e168e6533f8ec7c82263358940) | Identity, market, social, contract, or discovery cross-check |
| 20 | bsc AXL | [link](https://bscscan.com/token/0x8b1f4432f943c465a973fedc6d7aa50fc96f1f65) | Identity, market, social, contract, or discovery cross-check |
| 21 | optimism AXL | [link](https://optimistic.etherscan.io/token/0x23ee2343b892b1bb63503a4fabc840e0e2c6810f) | Identity, market, social, contract, or discovery cross-check |
| 22 | arbitrum AXL | [link](https://arbiscan.io/token/0x23ee2343b892b1bb63503a4fabc840e0e2c6810f) | Identity, market, social, contract, or discovery cross-check |
| 23 | Google project search | [link](https://www.google.com/search?q=Axelar%20AXL%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 24 | Google tokenomics search | [link](https://www.google.com/search?q=Axelar%20AXL%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 25 | Google docs search | [link](https://www.google.com/search?q=Axelar%20docs) | Identity, market, social, contract, or discovery cross-check |
| 26 | Google governance search | [link](https://www.google.com/search?q=Axelar%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 27 | Google audit search | [link](https://www.google.com/search?q=Axelar%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 28 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/axelar/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 29 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/axelar/) | Identity, market, social, contract, or discovery cross-check |
| 30 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Axelar) | Identity, market, social, contract, or discovery cross-check |
| 31 | Dune token query | [link](https://dune.com/browse/queries?q=Axelar) | Identity, market, social, contract, or discovery cross-check |
| 32 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Axelar) | Identity, market, social, contract, or discovery cross-check |
| 33 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=AXL) | Identity, market, social, contract, or discovery cross-check |
| 34 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Axelar) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://axelar.network)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Axelar)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Axelar)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Axelar)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Axelar)
- [Messari discovery](https://messari.io/search?q=Axelar)
- [X profile](https://x.com/axelar)
- [GitHub](https://github.com/axelarnetwork)
- [Telegram](https://t.me/axelarcommunity)
- [Discord](https://discord.gg/axelar)
- [Axelar Price: AXL/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/axelar)
- [Axelar Price History: Download AXL Historical Data | CoinGecko](https://www.coingecko.com/en/coins/axelar/historical_data)
- [What Is Axelar (AXL) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/axelar/what-is/)
- [The AXL Token & the Interchain Future | Axelar Blog](https://www.axelar.network/blog/axl-token-post-summary)
- [Axelar Bridged USDC Price Chart (AXLUSDC) - CoinGecko](https://www.coingecko.com/en/coins/axelar-bridged-usdc)
- [base AXL](https://basescan.org/token/0x23ee2343b892b1bb63503a4fabc840e0e2c6810f)
- [ethereum AXL](https://etherscan.io/token/0x467719ad09025fcc6cf6f8311755809d45a5e5f3)
- [avalanche AXL](https://snowtrace.io/token/0x44c784266cf024a60e8acf2427b9857ace194c5d)
- [polygon AXL](https://polygonscan.com/token/0x6e4e624106cb12e168e6533f8ec7c82263358940)
- [bsc AXL](https://bscscan.com/token/0x8b1f4432f943c465a973fedc6d7aa50fc96f1f65)
- [optimism AXL](https://optimistic.etherscan.io/token/0x23ee2343b892b1bb63503a4fabc840e0e2c6810f)
- [arbitrum AXL](https://arbiscan.io/token/0x23ee2343b892b1bb63503a4fabc840e0e2c6810f)
- [Google project search](https://www.google.com/search?q=Axelar%20AXL%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Axelar%20AXL%20tokenomics)

## Final Investment View

Axelar should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
