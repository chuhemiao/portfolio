---
title: 'ChainGPT / CGPT: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'ChainGPT is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'chaingpt-cgpt-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# ChainGPT / CGPT: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** ChainGPT appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Binance Smart Chain, Ethereum, Solana; exchange exposure is BINANCE, BYBIT, BITGET; category tags are Web3 AI (Agent/AIGC), Developer Tooling, ICO/IDO/IEO launchpad; current market cap is $18.14M; FDV is $19.40M; and 24h volume is $2.82M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag ChainGPT before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 869, market cap $18,305,203, 24h volume $1,427,226. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

ChainGPT is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether CGPT captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| FDV | $19.40M |
| 24h volume | $2.82M |
| Circulating supply | 932.91M |
| Total supply | 997.77M |
...
Base case: ChainGPT remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning CGPT into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

ChainGPT is a Web3-AI infrastructure platform offering AI-powered tools—Web3 chatbot, smart-contract generator and auditor, NFT generator, trading assistant, and APIs/SDKs for builders. Its CGPT utility token powers access, staking, and governance, and the team is developing AIVM, an AI-native blockchain for running autonomous agents and AI workloads.

Surf's project profile maps ChainGPT to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.chaingpt.org/](https://www.chaingpt.org/) |
| X handle | [@Chain_GPT](https://x.com/Chain_GPT) |
| X followers | 824.73K |
| Tags | Web3 AI (Agent/AIGC), Developer Tooling, ICO/IDO/IEO launchpad |
| Chains | Binance Smart Chain, Ethereum, Solana |
| Exchanges | BINANCE, BYBIT, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether ChainGPT is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DeFi infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does ChainGPT convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Supply quality | 932.91M circulating vs 997.77M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that ChainGPT is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| solana | CGPT | CCDfDXZxzZtkZLuhY48gyKdXc5KywqpR8xEVHHh8ck1G | [explorer](https://solscan.io/token/CCDfDXZxzZtkZLuhY48gyKdXc5KywqpR8xEVHHh8ck1G) |
| ethereum | CGPT | 0x25931894a86d47441213199621f1f2994e1c39aa | [explorer](https://etherscan.io/token/0x25931894a86d47441213199621f1f2994e1c39aa) |
| bsc | CGPT | 0x9840652dc04fb9db2c43853633f0f62be6f00f98 | [explorer](https://bscscan.com/token/0x9840652dc04fb9db2c43853633f0f62be6f00f98) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For ChainGPT, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 869, market cap $18,305,203, 24h volume $1,427,226. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $18.14M, FDV $19.40M, 24h volume $2.82M, and price $0.0194.
...
| Market cap | $18.14M | Liquid valuation proxy |
| FDV | $19.40M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.07x | Higher spread means more supply risk |
| X followers | 824.73K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for ChainGPT | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether CGPT is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. ChainGPT should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that ChainGPT belongs to the DeFi infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $18.14M, FDV $19.40M, 24h volume $2.82M, price $0.0194, circulating supply 932.91M, and total supply 997.77M. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For ChainGPT, the candidate backlog records Seeded 2026-07-06 from Surf market-ranking; rank 869, market cap $18,305,203, 24h volume $1,427,226. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Binance Smart Chain, Ethereum, Solana, exchanges as BINANCE, BYBIT, BITGET, and tags as Web3 AI (Agent/AIGC), Developer Tooling, ICO/IDO/IEO launchpad. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. ChainGPT currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 2.33% and 30d -1.00% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that ChainGPT's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2023-04-07 | Ido | $384.99K | not disclosed |
| 2023-04-06 | Ido | $100.00K | not disclosed |
| 2023-04-06 | Private | $50.00K | not disclosed |
| 2023-04-06 | Ido | $100.00K | not disclosed |
| 2023-04-05 | Ido | $120.00K | not disclosed |
| not disclosed | Seed | $405.00K | not disclosed |

| Team member | Role | Link |
|---|---|---|
| Ilan Rakhmanov | Founder, CEO, CMO | not disclosed |
| Adnan Tariq | Fractional CTO, Head of AI | not disclosed |
| Ariel Asafov | COO | not disclosed |
| Vlad Filatov | CAO | not disclosed |
| Mitchell Girao | CCO | [X](https://twitter.com/MitchellCGPT_) |
...
ChainGPT competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether ChainGPT has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: ChainGPT is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | ChainGPT compounds usage, liquidity improves, and CGPT gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If CGPT does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
1. Re-run Surf project-detail and project-defi-metrics if ChainGPT announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. ChainGPT has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For ChainGPT, the first reconciliation job is identity. The token symbol is CGPT, the category is DeFi infrastructure, chains are Binance Smart Chain, Ethereum, Solana, exchanges are BINANCE, BYBIT, BITGET, and tags are Web3 AI (Agent/AIGC), Developer Tooling, ICO/IDO/IEO launchpad. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If CGPT trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $18.14M, FDV is $19.40M, 24h volume is $2.82M, FDV / market cap is 1.07x, 7d price change is 2.33%, and 30d price change is -1.00%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | ChainGPT / CGPT | Match official docs, CG, CMC, Surf, contracts |
...
| Market cap | $18.14M | Compare Surf, CG, CMC, exchange pages |
| FDV | $19.40M | Reconcile total supply and unlock schedule |
...
For ChainGPT, the current liquidity read is watchlist first. The market cap is $18.14M, FDV is $19.40M, reported 24h volume is $2.82M, and FDV / market cap is 1.07x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | ChainGPT gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.chaingpt.org/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=ChainGPT) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=ChainGPT) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=ChainGPT) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=ChainGPT) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=ChainGPT) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/Chain_GPT) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/chaingpt) | Identity, market, social, contract, or discovery cross-check |
| 9 | Discord | [link](https://discord.gg/chaingpt) | Identity, market, social, contract, or discovery cross-check |
| 10 | ChainGPT Price Chart (CGPT) - CoinGecko | [link](https://www.coingecko.com/en/coins/chaingpt) | Identity, market, social, contract, or discovery cross-check |
| 11 | Tokenomics / ChainGPT Documentation | [link](https://docs.chaingpt.org/our-ecosystem/cgpt-utility-token/tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 12 | Top ChainGPT Launchpad Coins by Market Cap - CoinGecko | [link](https://www.coingecko.com/en/categories/chaingpt-pad) | Identity, market, social, contract, or discovery cross-check |
| 13 | ChainGPT (CGPT) Price Prediction For 2026 & Beyond | [link](https://coinmarketcap.com/cmc-ai/chaingpt/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 14 | CGPT to BTC: ChainGPT Price in Bitcoin - CoinGecko | [link](https://www.coingecko.com/en/coins/chaingpt/btc) | Identity, market, social, contract, or discovery cross-check |
| 15 | solana CGPT | [link](https://solscan.io/token/CCDfDXZxzZtkZLuhY48gyKdXc5KywqpR8xEVHHh8ck1G) | Identity, market, social, contract, or discovery cross-check |
| 16 | ethereum CGPT | [link](https://etherscan.io/token/0x25931894a86d47441213199621f1f2994e1c39aa) | Identity, market, social, contract, or discovery cross-check |
| 17 | bsc CGPT | [link](https://bscscan.com/token/0x9840652dc04fb9db2c43853633f0f62be6f00f98) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google project search | [link](https://www.google.com/search?q=ChainGPT%20CGPT%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google tokenomics search | [link](https://www.google.com/search?q=ChainGPT%20CGPT%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google docs search | [link](https://www.google.com/search?q=ChainGPT%20docs) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google governance search | [link](https://www.google.com/search?q=ChainGPT%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google audit search | [link](https://www.google.com/search?q=ChainGPT%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/chaingpt/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/chaingpt/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 25 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/chaingpt/) | Identity, market, social, contract, or discovery cross-check |
| 26 | Dune discovery | [link](https://dune.com/browse/dashboards?q=ChainGPT) | Identity, market, social, contract, or discovery cross-check |
| 27 | Dune token query | [link](https://dune.com/browse/queries?q=ChainGPT) | Identity, market, social, contract, or discovery cross-check |
| 28 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=ChainGPT) | Identity, market, social, contract, or discovery cross-check |
| 29 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=CGPT) | Identity, market, social, contract, or discovery cross-check |
| 30 | TokenUnlocks search | [link](https://token.unlocks.app/?search=ChainGPT) | Identity, market, social, contract, or discovery cross-check |
| 31 | CryptoRank search | [link](https://cryptorank.io/price/chaingpt) | Identity, market, social, contract, or discovery cross-check |
| 32 | RootData search | [link](https://www.rootdata.com/search?kw=ChainGPT) | Identity, market, social, contract, or discovery cross-check |
| 33 | CoinDesk search | [link](https://www.coindesk.com/search?s=ChainGPT) | Identity, market, social, contract, or discovery cross-check |
| 34 | The Block search | [link](https://www.theblock.co/search?query=ChainGPT) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.chaingpt.org/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=ChainGPT)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=ChainGPT)
- [DefiLlama protocol discovery](https://defillama.com/search?query=ChainGPT)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=ChainGPT)
- [Messari discovery](https://messari.io/search?q=ChainGPT)
- [X profile](https://x.com/Chain_GPT)
- [Telegram](https://t.me/chaingpt)
- [Discord](https://discord.gg/chaingpt)
- [ChainGPT Price Chart (CGPT) - CoinGecko](https://www.coingecko.com/en/coins/chaingpt)
- [Tokenomics | ChainGPT Documentation](https://docs.chaingpt.org/our-ecosystem/cgpt-utility-token/tokenomics)
- [Top ChainGPT Launchpad Coins by Market Cap - CoinGecko](https://www.coingecko.com/en/categories/chaingpt-pad)
- [ChainGPT (CGPT) Price Prediction For 2026 & Beyond](https://coinmarketcap.com/cmc-ai/chaingpt/price-prediction/)
- [CGPT to BTC: ChainGPT Price in Bitcoin - CoinGecko](https://www.coingecko.com/en/coins/chaingpt/btc)
- [solana CGPT](https://solscan.io/token/CCDfDXZxzZtkZLuhY48gyKdXc5KywqpR8xEVHHh8ck1G)
- [ethereum CGPT](https://etherscan.io/token/0x25931894a86d47441213199621f1f2994e1c39aa)
- [bsc CGPT](https://bscscan.com/token/0x9840652dc04fb9db2c43853633f0f62be6f00f98)
- [Google project search](https://www.google.com/search?q=ChainGPT%20CGPT%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=ChainGPT%20CGPT%20tokenomics)
- [Google docs search](https://www.google.com/search?q=ChainGPT%20docs)
- [Google governance search](https://www.google.com/search?q=ChainGPT%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=ChainGPT%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/chaingpt/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/chaingpt/price-analysis/)

## Final Investment View

ChainGPT should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
