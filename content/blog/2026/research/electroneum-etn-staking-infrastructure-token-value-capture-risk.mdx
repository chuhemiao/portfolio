...
Base case: Electroneum remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning ETN into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Electroneum is a mobile-first, EVM-compatible Layer-1 blockchain and cryptocurrency focused on accessible mobile payments, microtransactions, and developer-friendly smart contracts to drive mass adoption in emerging markets. The project provides a mobile wallet, merchant tools, and developer resources for building on the Electroneum Smart Chain with low fees and fast finality.

Surf's project profile maps Electroneum to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [http://electroneum.com/](http://electroneum.com/) |
| X handle | [@electroneum](https://x.com/electroneum) |
| X followers | 125.52K |
| Tags | Layer1, Node & Validator, Payments & Gateway |
| Chains | not disclosed |
| Exchanges | not disclosed |
| TGE status | not disclosed |
| CoinGecko ID | not disclosed |

The main product question is whether Electroneum is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For staking infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Electroneum convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $376.46K 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 17.98B circulating vs 17.98B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Electroneum is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Electroneum, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 900, market cap $17,314,989, 24h volume $375,290. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $17.30M, FDV $17.30M, 24h volume $376.46K, and price $0.0010.
...
| 24h volume | $376.46K | Position sizing and exit liquidity proxy |
| Market cap | $17.30M | Liquid valuation proxy |
| FDV | $17.30M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.00x | Higher spread means more supply risk |
| X followers | 125.52K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Electroneum | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether ETN is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Electroneum currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 1.87% and 30d 8.93% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Electroneum's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2017-09-14 | Ico | $39.93M | not disclosed |
...
Electroneum competes in the staking infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Electroneum has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Electroneum is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Electroneum compounds usage, liquidity improves, and ETN gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If ETN does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $376.46K | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Electroneum announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Electroneum has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Electroneum, the first reconciliation job is identity. The token symbol is ETN, the category is staking infrastructure, chains are not disclosed, exchanges are not disclosed, and tags are Layer1, Node & Validator, Payments & Gateway. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If ETN trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $17.30M, FDV is $17.30M, 24h volume is $376.46K, FDV / market cap is 1.00x, 7d price change is 1.87%, and 30d price change is 8.93%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Electroneum / ETN | Match official docs, CG, CMC, Surf, contracts |
...
For Electroneum, the current liquidity read is watchlist first. The market cap is $17.30M, FDV is $17.30M, reported 24h volume is $376.46K, and FDV / market cap is 1.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. Electroneum can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.

For Electroneum, the live monitoring stack should include contract events, TVL, volume, governance, documentation changes, social announcements, security disclosures, and exchange-market quality. A single metric is not enough. TVL may stay high while liquidity thins. Volume may rise because of volatility rather than adoption. Social attention may rise around controversy rather than progress. Governance may pass proposals that improve short-term price but add long-term risk. The strongest monitoring dashboard combines on-chain state, off-chain disclosure, and market quality.

Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If Electroneum suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | Electroneum gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](http://electroneum.com/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Electroneum) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Electroneum) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Electroneum) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Electroneum) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Electroneum) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/electroneum) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/electroneum) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/officialelectroneum) | Identity, market, social, contract, or discovery cross-check |
| 10 | Electroneum Price Chart (ETN) - CoinGecko | [link](https://www.coingecko.com/en/coins/electroneum) | Identity, market, social, contract, or discovery cross-check |
| 11 | ETN to BTC: Electroneum Price in Bitcoin - CoinGecko | [link](https://www.coingecko.com/en/coins/electroneum/btc) | Identity, market, social, contract, or discovery cross-check |
| 12 | ETN to INR: Electroneum Price in Indian Rupee - CoinGecko | [link](https://www.coingecko.com/en/coins/electroneum/inr) | Identity, market, social, contract, or discovery cross-check |
| 13 | Electroneum Price History: Download ETN Historical Data / CoinGecko | [link](https://www.coingecko.com/en/coins/electroneum/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 14 | Electroneum Price History and Historical Data / CoinMarketCap | [link](https://coinmarketcap.com/currencies/electroneum/historical-data/) | Identity, market, social, contract, or discovery cross-check |
| 15 | Google project search | [link](https://www.google.com/search?q=Electroneum%20ETN%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google tokenomics search | [link](https://www.google.com/search?q=Electroneum%20ETN%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google docs search | [link](https://www.google.com/search?q=Electroneum%20docs) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google governance search | [link](https://www.google.com/search?q=Electroneum%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google audit search | [link](https://www.google.com/search?q=Electroneum%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 20 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/electroneum/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/electroneum/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/electroneum/) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Electroneum) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune token query | [link](https://dune.com/browse/queries?q=Electroneum) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Electroneum) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=ETN) | Identity, market, social, contract, or discovery cross-check |
| 27 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Electroneum) | Identity, market, social, contract, or discovery cross-check |
| 28 | CryptoRank search | [link](https://cryptorank.io/price/electroneum) | Identity, market, social, contract, or discovery cross-check |
| 29 | RootData search | [link](https://www.rootdata.com/search?kw=Electroneum) | Identity, market, social, contract, or discovery cross-check |
| 30 | CoinDesk search | [link](https://www.coindesk.com/search?s=Electroneum) | Identity, market, social, contract, or discovery cross-check |
| 31 | The Block search | [link](https://www.theblock.co/search?query=Electroneum) | Identity, market, social, contract, or discovery cross-check |
| 32 | GitHub search | [link](https://github.com/search?q=Electroneum&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 33 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=ETN) | Identity, market, social, contract, or discovery cross-check |
| 34 | BscScan token search | [link](https://bscscan.com/search?f=0&q=ETN) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](http://electroneum.com/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Electroneum)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Electroneum)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Electroneum)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Electroneum)
- [Messari discovery](https://messari.io/search?q=Electroneum)
- [X profile](https://x.com/electroneum)
- [GitHub](https://github.com/electroneum)
- [Telegram](https://t.me/officialelectroneum)
- [Electroneum Price Chart (ETN) - CoinGecko](https://www.coingecko.com/en/coins/electroneum)
- [ETN to BTC: Electroneum Price in Bitcoin - CoinGecko](https://www.coingecko.com/en/coins/electroneum/btc)
- [ETN to INR: Electroneum Price in Indian Rupee - CoinGecko](https://www.coingecko.com/en/coins/electroneum/inr)
- [Electroneum Price History: Download ETN Historical Data | CoinGecko](https://www.coingecko.com/en/coins/electroneum/historical_data)
- [Electroneum Price History and Historical Data | CoinMarketCap](https://coinmarketcap.com/currencies/electroneum/historical-data/)
- [Google project search](https://www.google.com/search?q=Electroneum%20ETN%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Electroneum%20ETN%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Electroneum%20docs)
- [Google governance search](https://www.google.com/search?q=Electroneum%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Electroneum%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/electroneum/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/electroneum/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/electroneum/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Electroneum)
- [Dune token query](https://dune.com/browse/queries?q=Electroneum)
