---
title: 'Nosana / NOS: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Nosana is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'nosana-nos-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# Nosana / NOS: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Nosana appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Solana; exchange exposure is not disclosed; category tags are DePIN (Physical Infra), Web3 AI (Agent/AIGC); current market cap is $30.07M; FDV is $30.07M; and 24h volume is $157.35K. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Nosana before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 638, market cap $29,975,264, 24h volume $150,109. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Nosana is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether NOS captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
Base case: Nosana remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning NOS into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Nosana is a decentralized GPU marketplace and compute network that enables on‑demand AI inference and fast model deployment by connecting clients with hosts who monetize idle GPUs. Its native NOS token (Solana-based) powers staking, payments, and governance to coordinate jobs, rewards, and network operations.

Surf's project profile maps Nosana to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://nosana.com](https://nosana.com) |
| X handle | [@nosana_ai](https://x.com/nosana_ai) |
| X followers | 59.55K |
...
| Liquidity | $157.35K 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
...
The strongest reason to add the report is not that Nosana is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| solana | NOS | nosXBVoaCTtYdLvKY6Csb4AC8JCdQKKAaWYtx2ZMoo7 | [explorer](https://solscan.io/token/nosXBVoaCTtYdLvKY6Csb4AC8JCdQKKAaWYtx2ZMoo7) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Nosana, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 638, market cap $29,975,264, 24h volume $150,109. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $30.07M, FDV $30.07M, 24h volume $157.35K, and price $0.3007.
...
| 24h volume | $157.35K | Position sizing and exit liquidity proxy |
| Market cap | $30.07M | Liquid valuation proxy |
| FDV | $30.07M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.00x | Higher spread means more supply risk |
| X followers | 59.55K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Nosana | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether NOS is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Nosana currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 8.69% and 30d 1.21% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Nosana's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2022-01-16 | Ieo | $80.00K | not disclosed |
| 2022-01-10 | Ido | $300.00K | not disclosed |
| not disclosed | Private | $1.36M | not disclosed |
...
Nosana competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Nosana has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Nosana is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Nosana compounds usage, liquidity improves, and NOS gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Nosana has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If NOS does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $157.35K | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Nosana announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Nosana has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Nosana, the first reconciliation job is identity. The token symbol is NOS, the category is DeFi infrastructure, chains are Solana, exchanges are not disclosed, and tags are DePIN (Physical Infra), Web3 AI (Agent/AIGC). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If NOS trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $30.07M, FDV is $30.07M, 24h volume is $157.35K, FDV / market cap is 1.00x, 7d price change is 8.69%, and 30d price change is 1.21%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Nosana / NOS | Match official docs, CG, CMC, Surf, contracts |
...
For Nosana, the current liquidity read is watchlist first. The market cap is $30.07M, FDV is $30.07M, reported 24h volume is $157.35K, and FDV / market cap is 1.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Nosana gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://nosana.com) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Nosana) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Nosana) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Nosana) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Nosana) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Nosana) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/nosana_ai) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/nosana-ci) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/Nosanacompute) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](https://discord.com/invite/nosana-ai) | Identity, market, social, contract, or discovery cross-check |
| 11 | solana NOS | [link](https://solscan.io/token/nosXBVoaCTtYdLvKY6Csb4AC8JCdQKKAaWYtx2ZMoo7) | Identity, market, social, contract, or discovery cross-check |
| 12 | Google project search | [link](https://www.google.com/search?q=Nosana%20NOS%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 13 | Google tokenomics search | [link](https://www.google.com/search?q=Nosana%20NOS%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 14 | Google docs search | [link](https://www.google.com/search?q=Nosana%20docs) | Identity, market, social, contract, or discovery cross-check |
| 15 | Google governance search | [link](https://www.google.com/search?q=Nosana%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google audit search | [link](https://www.google.com/search?q=Nosana%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 17 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/nosana/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 18 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/nosana/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 19 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/nosana/) | Identity, market, social, contract, or discovery cross-check |
| 20 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Nosana) | Identity, market, social, contract, or discovery cross-check |
| 21 | Dune token query | [link](https://dune.com/browse/queries?q=Nosana) | Identity, market, social, contract, or discovery cross-check |
| 22 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Nosana) | Identity, market, social, contract, or discovery cross-check |
| 23 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=NOS) | Identity, market, social, contract, or discovery cross-check |
| 24 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Nosana) | Identity, market, social, contract, or discovery cross-check |
| 25 | CryptoRank search | [link](https://cryptorank.io/price/nosana) | Identity, market, social, contract, or discovery cross-check |
| 26 | RootData search | [link](https://www.rootdata.com/search?kw=Nosana) | Identity, market, social, contract, or discovery cross-check |
| 27 | CoinDesk search | [link](https://www.coindesk.com/search?s=Nosana) | Identity, market, social, contract, or discovery cross-check |
| 28 | The Block search | [link](https://www.theblock.co/search?query=Nosana) | Identity, market, social, contract, or discovery cross-check |
| 29 | GitHub search | [link](https://github.com/search?q=Nosana&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 30 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=NOS) | Identity, market, social, contract, or discovery cross-check |
| 31 | BscScan token search | [link](https://bscscan.com/search?f=0&q=NOS) | Identity, market, social, contract, or discovery cross-check |
| 32 | Arbiscan token search | [link](https://arbiscan.io/search?f=0&q=NOS) | Identity, market, social, contract, or discovery cross-check |
| 33 | BaseScan token search | [link](https://basescan.org/search?f=0&q=NOS) | Identity, market, social, contract, or discovery cross-check |
| 34 | RWA.xyz search | [link](https://app.rwa.xyz/search?query=Nosana) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://nosana.com)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Nosana)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Nosana)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Nosana)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Nosana)
- [Messari discovery](https://messari.io/search?q=Nosana)
- [X profile](https://x.com/nosana_ai)
- [GitHub](https://github.com/nosana-ci)
- [Telegram](https://t.me/Nosanacompute)
- [Discord](https://discord.com/invite/nosana-ai)
- [solana NOS](https://solscan.io/token/nosXBVoaCTtYdLvKY6Csb4AC8JCdQKKAaWYtx2ZMoo7)
- [Google project search](https://www.google.com/search?q=Nosana%20NOS%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Nosana%20NOS%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Nosana%20docs)
- [Google governance search](https://www.google.com/search?q=Nosana%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Nosana%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/nosana/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/nosana/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/nosana/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Nosana)
- [Dune token query](https://dune.com/browse/queries?q=Nosana)
- [DeFiLlama fees discovery](https://defillama.com/fees?chain=All&search=Nosana)
- [DeFiLlama yield discovery](https://defillama.com/yields?token=NOS)
- [TokenUnlocks search](https://token.unlocks.app/?search=Nosana)
