---
title: 'My Neighbor Alice / ALICE: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'My Neighbor Alice is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'my-neighbor-alice-alice-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# My Neighbor Alice / ALICE: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** My Neighbor Alice appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Binance Smart Chain, Ethereum; exchange exposure is BINANCE, COINBASE, BITHUMB, BITGET; category tags are GameFi, Metaverse; current market cap is $13.56M; FDV is $13.56M; and 24h volume is $54.02M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag My Neighbor Alice before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 1012, market cap $13,675,029, 24h volume $56,283,214. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

My Neighbor Alice is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether ALICE captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| FDV | $13.56M |
| 24h volume | $54.02M |
| Circulating supply | 99.46M |
| Total supply | 99.46M |
| 7d price change | 7.97% |
| 30d price change | 36.67% |
...
Base case: My Neighbor Alice remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning ALICE into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

My Neighbor Alice is a fully on-chain multiplayer builder game on Chromia where players buy and own virtual land NFTs, craft items, and socialize in a shared world. ALICE is the project's ERC-20 utility/governance token used for in-game purchases, staking and DAO voting. The game is developed by Antler Interactive and maintains official docs and active community channels.

Surf's project profile maps My Neighbor Alice to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.myneighboralice.com/](https://www.myneighboralice.com/) |
| X handle | [@MyNeighborAlice](https://x.com/MyNeighborAlice) |
| X followers | 168.38K |
| Tags | GameFi, Metaverse |
| Chains | Binance Smart Chain, Ethereum |
| Exchanges | BINANCE, COINBASE, BITHUMB, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether My Neighbor Alice is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DeFi infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does My Neighbor Alice convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $54.02M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 99.46M circulating vs 99.46M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that My Neighbor Alice is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

My Neighbor Alice's architecture should be evaluated through the job it performs. For a DeFi infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
|---|---|---|---|
| ethereum | ALICE | 0xac51066d7bec65dc4589368da368b212745d63e8 | [explorer](https://etherscan.io/token/0xac51066d7bec65dc4589368da368b212745d63e8) |
| bsc | ALICE | 0xac51066d7bec65dc4589368da368b212745d63e8 | [explorer](https://bscscan.com/token/0xac51066d7bec65dc4589368da368b212745d63e8) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For My Neighbor Alice, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 1012, market cap $13,675,029, 24h volume $56,283,214. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $13.56M, FDV $13.56M, 24h volume $54.02M, and price $0.1362.
...
| 24h volume | $54.02M | Position sizing and exit liquidity proxy |
| Market cap | $13.56M | Liquid valuation proxy |
| FDV | $13.56M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.00x | Higher spread means more supply risk |
| X followers | 168.38K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for My Neighbor Alice | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether ALICE is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. My Neighbor Alice should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that My Neighbor Alice belongs to the DeFi infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $13.56M, FDV $13.56M, 24h volume $54.02M, price $0.1362, circulating supply 99.46M, and total supply 99.46M. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For My Neighbor Alice, the candidate backlog records Seeded 2026-07-06 from Surf market-ranking; rank 1012, market cap $13,675,029, 24h volume $56,283,214. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Binance Smart Chain, Ethereum, exchanges as BINANCE, COINBASE, BITHUMB, BITGET, and tags as GameFi, Metaverse. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. My Neighbor Alice currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 7.97% and 30d 36.67% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that My Neighbor Alice's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2021-02-02 | Ido | $100.00K | not disclosed |
| not disclosed | Private | $2.10M | not disclosed |

| Team member | Role | Link |
|---|---|---|
| Hampus Granberg | Co-Founder, 3D Graphics Artist | [X](https://twitter.com/HampusGranberg) |
| Faviana Vangelius | Co-Founder | [X](https://twitter.com/FVangelius) |
| Lenny Pettersson | CEO | [X](https://twitter.com/LennyPettersson) |
| Pelle Sunnerot | CMO | [X](https://twitter.com/albintyko) |
| Riccardo Sibani | CPO | [X](https://twitter.com/bloody_bit) |
...
My Neighbor Alice competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
Weighted conclusion: My Neighbor Alice is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | My Neighbor Alice compounds usage, liquidity improves, and ALICE gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If ALICE does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $54.02M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if My Neighbor Alice announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. My Neighbor Alice has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For My Neighbor Alice, the first reconciliation job is identity. The token symbol is ALICE, the category is DeFi infrastructure, chains are Binance Smart Chain, Ethereum, exchanges are BINANCE, COINBASE, BITHUMB, BITGET, and tags are GameFi, Metaverse. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If ALICE trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $13.56M, FDV is $13.56M, 24h volume is $54.02M, FDV / market cap is 1.00x, 7d price change is 7.97%, and 30d price change is 36.67%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | My Neighbor Alice / ALICE | Match official docs, CG, CMC, Surf, contracts |
...
For My Neighbor Alice, the current liquidity read is watchlist first. The market cap is $13.56M, FDV is $13.56M, reported 24h volume is $54.02M, and FDV / market cap is 1.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. My Neighbor Alice can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.

For My Neighbor Alice, the live monitoring stack should include contract events, TVL, volume, governance, documentation changes, social announcements, security disclosures, and exchange-market quality. A single metric is not enough. TVL may stay high while liquidity thins. Volume may rise because of volatility rather than adoption. Social attention may rise around controversy rather than progress. Governance may pass proposals that improve short-term price but add long-term risk. The strongest monitoring dashboard combines on-chain state, off-chain disclosure, and market quality.

Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If My Neighbor Alice suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | My Neighbor Alice gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.myneighboralice.com/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=My%20Neighbor%20Alice) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=My%20Neighbor%20Alice) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=My%20Neighbor%20Alice) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=My%20Neighbor%20Alice) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=My%20Neighbor%20Alice) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/MyNeighborAlice) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/MyNeighborAlice) | Identity, market, social, contract, or discovery cross-check |
| 9 | Discord | [link](https://discord.com/invite/myneighboralice) | Identity, market, social, contract, or discovery cross-check |
| 10 | My Neighbor Alice Price Chart (ALICE) - CoinGecko | [link](https://www.coingecko.com/en/coins/my-neighbor-alice) | Identity, market, social, contract, or discovery cross-check |
| 11 | ALICE to ETH: My Neighbor Alice Price in Ether / CoinGecko | [link](https://www.coingecko.com/en/coins/my-neighbor-alice/eth) | Identity, market, social, contract, or discovery cross-check |
| 12 | Latest MyNeighborAlice (ALICE) Price Analysis - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/myneighboralice/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 13 | ALICE to TRY: My Neighbor Alice Price in Turkish Lira / CoinGecko | [link](https://www.coingecko.com/en/coins/my-neighbor-alice/try) | Identity, market, social, contract, or discovery cross-check |
| 14 | My Neighbor Alice Price History: Download ALICE Historical Data | [link](https://www.coingecko.com/en/coins/my-neighbor-alice/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 15 | ethereum ALICE | [link](https://etherscan.io/token/0xac51066d7bec65dc4589368da368b212745d63e8) | Identity, market, social, contract, or discovery cross-check |
| 16 | bsc ALICE | [link](https://bscscan.com/token/0xac51066d7bec65dc4589368da368b212745d63e8) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google project search | [link](https://www.google.com/search?q=My%20Neighbor%20Alice%20ALICE%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google tokenomics search | [link](https://www.google.com/search?q=My%20Neighbor%20Alice%20ALICE%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google docs search | [link](https://www.google.com/search?q=My%20Neighbor%20Alice%20docs) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google governance search | [link](https://www.google.com/search?q=My%20Neighbor%20Alice%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google audit search | [link](https://www.google.com/search?q=My%20Neighbor%20Alice%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/my-neighbor-alice/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/my-neighbor-alice/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/my-neighbor-alice/) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune discovery | [link](https://dune.com/browse/dashboards?q=My%20Neighbor%20Alice) | Identity, market, social, contract, or discovery cross-check |
| 26 | Dune token query | [link](https://dune.com/browse/queries?q=My%20Neighbor%20Alice) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=My%20Neighbor%20Alice) | Identity, market, social, contract, or discovery cross-check |
| 28 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=ALICE) | Identity, market, social, contract, or discovery cross-check |
| 29 | TokenUnlocks search | [link](https://token.unlocks.app/?search=My%20Neighbor%20Alice) | Identity, market, social, contract, or discovery cross-check |
| 30 | CryptoRank search | [link](https://cryptorank.io/price/my-neighbor-alice) | Identity, market, social, contract, or discovery cross-check |
| 31 | RootData search | [link](https://www.rootdata.com/search?kw=My%20Neighbor%20Alice) | Identity, market, social, contract, or discovery cross-check |
| 32 | CoinDesk search | [link](https://www.coindesk.com/search?s=My%20Neighbor%20Alice) | Identity, market, social, contract, or discovery cross-check |
| 33 | The Block search | [link](https://www.theblock.co/search?query=My%20Neighbor%20Alice) | Identity, market, social, contract, or discovery cross-check |
| 34 | GitHub search | [link](https://github.com/search?q=My%20Neighbor%20Alice&type=repositories) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.myneighboralice.com/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=My%20Neighbor%20Alice)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=My%20Neighbor%20Alice)
- [DefiLlama protocol discovery](https://defillama.com/search?query=My%20Neighbor%20Alice)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=My%20Neighbor%20Alice)
- [Messari discovery](https://messari.io/search?q=My%20Neighbor%20Alice)
- [X profile](https://x.com/MyNeighborAlice)
- [Telegram](https://t.me/MyNeighborAlice)
- [Discord](https://discord.com/invite/myneighboralice)
- [My Neighbor Alice Price Chart (ALICE) - CoinGecko](https://www.coingecko.com/en/coins/my-neighbor-alice)
- [ALICE to ETH: My Neighbor Alice Price in Ether | CoinGecko](https://www.coingecko.com/en/coins/my-neighbor-alice/eth)
- [Latest MyNeighborAlice (ALICE) Price Analysis - CoinMarketCap](https://coinmarketcap.com/cmc-ai/myneighboralice/price-analysis/)
- [ALICE to TRY: My Neighbor Alice Price in Turkish Lira | CoinGecko](https://www.coingecko.com/en/coins/my-neighbor-alice/try)
- [My Neighbor Alice Price History: Download ALICE Historical Data](https://www.coingecko.com/en/coins/my-neighbor-alice/historical_data)
- [ethereum ALICE](https://etherscan.io/token/0xac51066d7bec65dc4589368da368b212745d63e8)
- [bsc ALICE](https://bscscan.com/token/0xac51066d7bec65dc4589368da368b212745d63e8)
- [Google project search](https://www.google.com/search?q=My%20Neighbor%20Alice%20ALICE%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=My%20Neighbor%20Alice%20ALICE%20tokenomics)
- [Google docs search](https://www.google.com/search?q=My%20Neighbor%20Alice%20docs)
- [Google governance search](https://www.google.com/search?q=My%20Neighbor%20Alice%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=My%20Neighbor%20Alice%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/my-neighbor-alice/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/my-neighbor-alice/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/my-neighbor-alice/)

## Final Investment View

My Neighbor Alice should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
