---
title: 'Aztec / AZTEC: privacy infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Aztec is treated as a privacy infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'aztec-aztec-privacy-infrastructure-token-value-capture-risk'
category: 'research'
---

# Aztec / AZTEC: privacy infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Aztec appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is not disclosed; exchange exposure is UPBIT, COINBASE, BITHUMB, BYBIT, KRAKEN, GATE, HYPERLIQUID, UNISWAP; category tags are Developer Tooling, Layer2 & Scaling, Privacy & ZK; current market cap is $42.87M; FDV is $150.15M; and 24h volume is $8.18M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Aztec before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 509, market cap $41,840,280, 24h volume $7,666,845. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Aztec is best understood as a **privacy infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether AZTEC captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Exchanges | UPBIT, COINBASE, BITHUMB, BYBIT, KRAKEN, GATE, HYPERLIQUID, UNISWAP |
| Price | $0.0146 |
| Market cap | $42.87M |
| FDV | $150.15M |
| 24h volume | $8.18M |
| Circulating supply | 2.96B |
| Total supply | 10.35B |
| 7d price change | 15.84% |
...
Base case: Aztec remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning AZTEC into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Privacy-focused L2 on Ethereum using zero-knowledge proofs for encrypted smart contracts; token used for staking, fees, governance.

Surf's project profile maps Aztec to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://aztec.network/index.html](https://aztec.network/index.html) |
| X handle | [@aztecnetwork](https://x.com/aztecnetwork) |
| X followers | 209.77K |
| Tags | Developer Tooling, Layer2 & Scaling, Privacy & ZK |
| Chains | not disclosed |
| Exchanges | UPBIT, COINBASE, BITHUMB, BYBIT, KRAKEN, GATE, HYPERLIQUID, UNISWAP |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Aztec is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For privacy infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Aztec convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $8.18M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 2.96B circulating vs 10.35B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Aztec is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Aztec, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 509, market cap $41,840,280, 24h volume $7,666,845. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $42.87M, FDV $150.15M, 24h volume $8.18M, and price $0.0146.
...
| 24h volume | $8.18M | Position sizing and exit liquidity proxy |
| Market cap | $42.87M | Liquid valuation proxy |
| FDV | $150.15M | Dilution-aware valuation proxy |
| FDV / market cap spread | 3.50x | Higher spread means more supply risk |
| X followers | 209.77K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Aztec | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether AZTEC is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Aztec currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 15.84% and 30d -5.67% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Aztec's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2026-02-09 | Ieo | $3.75K | not disclosed |
| 2026-02-09 | Ieo | $5.00K | not disclosed |
| 2025-12-08 | Ieo | $10.00K | not disclosed |
| 2025-12-02 | Ico | $32.31M | not disclosed |
| 2025-11-13 | Ido | $32.33M | not disclosed |
| 2022-12-15 | Series B | $100.00M | Fenbushi Capital, Andreessen Horowitz (a16z crypto), HashKey Capital, Variant, a_capital, SV Angel, Alumni Ventures, King River Capital |

| Team member | Role | Link |
|---|---|---|
| Zachary Williamson | Co-Founder, CEO | [X](https://twitter.com/Zac_Aztec) |
| Joe Andrews | Co-Founder, Head of Product | [X](https://twitter.com/jaosef) |
...
Aztec competes in the privacy infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Aztec has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Aztec is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Aztec compounds usage, liquidity improves, and AZTEC gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Aztec has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If AZTEC does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $8.18M | Weekly | Volume below position-size threshold |
| FDV / market cap | 3.50x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Aztec announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Aztec has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Aztec, the first reconciliation job is identity. The token symbol is AZTEC, the category is privacy infrastructure, chains are not disclosed, exchanges are UPBIT, COINBASE, BITHUMB, BYBIT, KRAKEN, GATE, HYPERLIQUID, UNISWAP, and tags are Developer Tooling, Layer2 & Scaling, Privacy & ZK. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If AZTEC trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $42.87M, FDV is $150.15M, 24h volume is $8.18M, FDV / market cap is 3.50x, 7d price change is 15.84%, and 30d price change is -5.67%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Aztec / AZTEC | Match official docs, CG, CMC, Surf, contracts |
...
| Exchanges | UPBIT, COINBASE, BITHUMB, BYBIT, KRAKEN, GATE, HYPERLIQUID, UNISWAP | Check live order books and inactive pairs |
| Market cap | $42.87M | Compare Surf, CG, CMC, exchange pages |
| FDV | $150.15M | Reconcile total supply and unlock schedule |
| Volume | $8.18M | Check exchange concentration and wash-trading risk |
| TVL | not disclosed | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Aztec does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Aztec, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does AZTEC sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
For Aztec, the current liquidity read is watchlist first. The market cap is $42.87M, FDV is $150.15M, reported 24h volume is $8.18M, and FDV / market cap is 3.50x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Aztec gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://aztec.network/index.html) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Aztec) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Aztec) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Aztec) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Aztec) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Aztec) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/aztecnetwork) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/AztecProtocol) | Identity, market, social, contract, or discovery cross-check |
| 9 | Discord | [link](https://discord.gg/aztec) | Identity, market, social, contract, or discovery cross-check |
| 10 | AZTEC/USD Live Price Chart, Market Cap & News Today - CoinGecko | [link](https://www.coingecko.com/en/coins/aztec) | Identity, market, social, contract, or discovery cross-check |
| 11 | Aztec price today, AZTEC to USD live price, marketcap and chart | [link](https://coinmarketcap.com/currencies/aztec/) | Identity, market, social, contract, or discovery cross-check |
| 12 | JUST IN: #AZTEC, a privacy L2 on Ethereum, launches its TGE. It's ... | [link](https://www.facebook.com/coingecko/posts/just-in-aztec-a-privacy-l2-on-ethereum-launches-its-tge-its-now-ranked-426-by-ma/1340344951471116/) | Identity, market, social, contract, or discovery cross-check |
| 13 | Overview / Privacy-first zkRollup - Aztec Documentation | [link](https://docs.aztec.network/participate/token) | Identity, market, social, contract, or discovery cross-check |
| 14 | What Is Aztec (AZTEC) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/aztec/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 15 | Google project search | [link](https://www.google.com/search?q=Aztec%20AZTEC%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google tokenomics search | [link](https://www.google.com/search?q=Aztec%20AZTEC%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google docs search | [link](https://www.google.com/search?q=Aztec%20docs) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google governance search | [link](https://www.google.com/search?q=Aztec%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google audit search | [link](https://www.google.com/search?q=Aztec%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 20 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/aztec/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 21 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Aztec) | Identity, market, social, contract, or discovery cross-check |
| 22 | Dune token query | [link](https://dune.com/browse/queries?q=Aztec) | Identity, market, social, contract, or discovery cross-check |
| 23 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Aztec) | Identity, market, social, contract, or discovery cross-check |
| 24 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=AZTEC) | Identity, market, social, contract, or discovery cross-check |
| 25 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Aztec) | Identity, market, social, contract, or discovery cross-check |
| 26 | CryptoRank search | [link](https://cryptorank.io/price/aztec) | Identity, market, social, contract, or discovery cross-check |
| 27 | RootData search | [link](https://www.rootdata.com/search?kw=Aztec) | Identity, market, social, contract, or discovery cross-check |
| 28 | CoinDesk search | [link](https://www.coindesk.com/search?s=Aztec) | Identity, market, social, contract, or discovery cross-check |
| 29 | The Block search | [link](https://www.theblock.co/search?query=Aztec) | Identity, market, social, contract, or discovery cross-check |
| 30 | GitHub search | [link](https://github.com/search?q=Aztec&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 31 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=AZTEC) | Identity, market, social, contract, or discovery cross-check |
| 32 | BscScan token search | [link](https://bscscan.com/search?f=0&q=AZTEC) | Identity, market, social, contract, or discovery cross-check |
| 33 | Arbiscan token search | [link](https://arbiscan.io/search?f=0&q=AZTEC) | Identity, market, social, contract, or discovery cross-check |
| 34 | BaseScan token search | [link](https://basescan.org/search?f=0&q=AZTEC) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://aztec.network/index.html)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Aztec)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Aztec)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Aztec)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Aztec)
- [Messari discovery](https://messari.io/search?q=Aztec)
- [X profile](https://x.com/aztecnetwork)
- [GitHub](https://github.com/AztecProtocol)
- [Discord](https://discord.gg/aztec)
- [AZTEC/USD Live Price Chart, Market Cap & News Today - CoinGecko](https://www.coingecko.com/en/coins/aztec)
- [Aztec price today, AZTEC to USD live price, marketcap and chart](https://coinmarketcap.com/currencies/aztec/)
- [JUST IN: #AZTEC, a privacy L2 on Ethereum, launches its TGE. It's ...](https://www.facebook.com/coingecko/posts/just-in-aztec-a-privacy-l2-on-ethereum-launches-its-tge-its-now-ranked-426-by-ma/1340344951471116/)
- [Overview | Privacy-first zkRollup - Aztec Documentation](https://docs.aztec.network/participate/token)
- [What Is Aztec (AZTEC) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/aztec/what-is/)
- [Google project search](https://www.google.com/search?q=Aztec%20AZTEC%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Aztec%20AZTEC%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Aztec%20docs)
- [Google governance search](https://www.google.com/search?q=Aztec%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Aztec%20audit%20report)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/aztec/price-analysis/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Aztec)
- [Dune token query](https://dune.com/browse/queries?q=Aztec)
- [DeFiLlama fees discovery](https://defillama.com/fees?chain=All&search=Aztec)
- [DeFiLlama yield discovery](https://defillama.com/yields?token=AZTEC)

## Final Investment View

Aztec should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
