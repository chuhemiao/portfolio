---
title: 'Peanut / PEANUT: yield infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Peanut is treated as a yield infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'peanut-peanut-yield-infrastructure-token-value-capture-risk'
category: 'research'
---

# Peanut / PEANUT: yield infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Peanut appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Binance Smart Chain, Ethereum; exchange exposure is not disclosed; category tags are MEV & Intent, Quant & Trading Tools, Yield & Asset Mgmt; current market cap is $423.09K; FDV is $423.09K; and 24h volume is $1.11K. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Peanut before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 153, market cap $228,181,317, 24h volume $0. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Peanut is best understood as a **yield infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether PEANUT captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
Base case: Peanut remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning PEANUT into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Peanut (Peanut Trade) is a crypto market‑making and TGE services firm providing full‑cycle token generation event management, MEV‑aware market‑making, arbitrage, and liquidity solutions across multiple blockchains. It is a B2B launch and liquidity provider distinct from the Solana memecoin “Peanut the Squirrel” (PNUT).

Surf's project profile maps Peanut to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://peanut.trade/](https://peanut.trade/) |
| X handle | [@PeanutTrade](https://x.com/PeanutTrade) |
| X followers | 33.57K |
| Tags | MEV & Intent, Quant & Trading Tools, Yield & Asset Mgmt |
| Chains | Binance Smart Chain, Ethereum |
| Exchanges | not disclosed |
| TGE status | not disclosed |
| CoinGecko ID | not disclosed |

The main product question is whether Peanut is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For yield infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Peanut convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $1.11K 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 50.00M circulating vs 50.00M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Peanut is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| ethereum | NUX | 0x89bd2e7e388fab44ae88bef4e1ad12b4f1e0911c | [explorer](https://etherscan.io/token/0x89bd2e7e388fab44ae88bef4e1ad12b4f1e0911c) |
| bsc | NUX | 0x6d8734002fbffe1c86495e32c95f732fc77f6f2a | [explorer](https://bscscan.com/token/0x6d8734002fbffe1c86495e32c95f732fc77f6f2a) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Peanut, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 153, market cap $228,181,317, 24h volume $0. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $423.09K, FDV $423.09K, 24h volume $1.11K, and price $0.0085.
...
| 24h volume | $1.11K | Position sizing and exit liquidity proxy |
| Market cap | $423.09K | Liquid valuation proxy |
| FDV | $423.09K | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.00x | Higher spread means more supply risk |
| X followers | 33.57K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Peanut | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether PEANUT is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Peanut currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 6.57% and 30d 55.64% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Peanut's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2021-02-10 | Ieo | $1.46M | not disclosed |
...
Peanut competes in the yield infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Peanut has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Peanut is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Peanut compounds usage, liquidity improves, and PEANUT gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Peanut has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If PEANUT does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $1.11K | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Peanut announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Peanut has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Peanut, the first reconciliation job is identity. The token symbol is PEANUT, the category is yield infrastructure, chains are Binance Smart Chain, Ethereum, exchanges are not disclosed, and tags are MEV & Intent, Quant & Trading Tools, Yield & Asset Mgmt. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If PEANUT trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $423.09K, FDV is $423.09K, 24h volume is $1.11K, FDV / market cap is 1.00x, 7d price change is 6.57%, and 30d price change is 55.64%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Peanut / PEANUT | Match official docs, CG, CMC, Surf, contracts |
...
For Peanut, the current liquidity read is watchlist first. The market cap is $423.09K, FDV is $423.09K, reported 24h volume is $1.11K, and FDV / market cap is 1.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Peanut gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://peanut.trade/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Peanut) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Peanut) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Peanut) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Peanut) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Peanut) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/PeanutTrade) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/peanuttrade) | Identity, market, social, contract, or discovery cross-check |
| 9 | Peanut the Squirrel PNUT Price - CoinGecko | [link](https://www.coingecko.com/en/coins/peanut-the-squirrel) | Identity, market, social, contract, or discovery cross-check |
| 10 | Peanut Price Chart (PEANUT) - Market Cap - CoinGecko | [link](https://www.coingecko.com/en/coins/peanut-2) | Identity, market, social, contract, or discovery cross-check |
| 11 | Peanut Price Chart (PNUT) - CoinGecko | [link](https://www.coingecko.com/en/coins/peanut-3) | Identity, market, social, contract, or discovery cross-check |
| 12 | Peanut Price: NUX/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/peanut) | Identity, market, social, contract, or discovery cross-check |
| 13 | Peanut the Squirrel Price History and Historical Data / CoinMarketCap | [link](https://coinmarketcap.com/currencies/peanut-the-squirrel/historical-data/) | Identity, market, social, contract, or discovery cross-check |
| 14 | ethereum NUX | [link](https://etherscan.io/token/0x89bd2e7e388fab44ae88bef4e1ad12b4f1e0911c) | Identity, market, social, contract, or discovery cross-check |
| 15 | bsc NUX | [link](https://bscscan.com/token/0x6d8734002fbffe1c86495e32c95f732fc77f6f2a) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google project search | [link](https://www.google.com/search?q=Peanut%20PEANUT%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google tokenomics search | [link](https://www.google.com/search?q=Peanut%20PEANUT%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google docs search | [link](https://www.google.com/search?q=Peanut%20docs) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google governance search | [link](https://www.google.com/search?q=Peanut%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google audit search | [link](https://www.google.com/search?q=Peanut%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/peanut/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/peanut/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/peanut/) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Peanut) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune token query | [link](https://dune.com/browse/queries?q=Peanut) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Peanut) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=PEANUT) | Identity, market, social, contract, or discovery cross-check |
| 28 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Peanut) | Identity, market, social, contract, or discovery cross-check |
| 29 | CryptoRank search | [link](https://cryptorank.io/price/peanut) | Identity, market, social, contract, or discovery cross-check |
| 30 | RootData search | [link](https://www.rootdata.com/search?kw=Peanut) | Identity, market, social, contract, or discovery cross-check |
| 31 | CoinDesk search | [link](https://www.coindesk.com/search?s=Peanut) | Identity, market, social, contract, or discovery cross-check |
| 32 | The Block search | [link](https://www.theblock.co/search?query=Peanut) | Identity, market, social, contract, or discovery cross-check |
| 33 | GitHub search | [link](https://github.com/search?q=Peanut&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 34 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=PEANUT) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://peanut.trade/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Peanut)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Peanut)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Peanut)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Peanut)
- [Messari discovery](https://messari.io/search?q=Peanut)
- [X profile](https://x.com/PeanutTrade)
- [Telegram](https://t.me/peanuttrade)
- [Peanut the Squirrel PNUT Price - CoinGecko](https://www.coingecko.com/en/coins/peanut-the-squirrel)
- [Peanut Price Chart (PEANUT) - Market Cap - CoinGecko](https://www.coingecko.com/en/coins/peanut-2)
- [Peanut Price Chart (PNUT) - CoinGecko](https://www.coingecko.com/en/coins/peanut-3)
- [Peanut Price: NUX/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/peanut)
- [Peanut the Squirrel Price History and Historical Data | CoinMarketCap](https://coinmarketcap.com/currencies/peanut-the-squirrel/historical-data/)
- [ethereum NUX](https://etherscan.io/token/0x89bd2e7e388fab44ae88bef4e1ad12b4f1e0911c)
- [bsc NUX](https://bscscan.com/token/0x6d8734002fbffe1c86495e32c95f732fc77f6f2a)
- [Google project search](https://www.google.com/search?q=Peanut%20PEANUT%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Peanut%20PEANUT%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Peanut%20docs)
- [Google governance search](https://www.google.com/search?q=Peanut%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Peanut%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/peanut/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/peanut/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/peanut/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Peanut)
