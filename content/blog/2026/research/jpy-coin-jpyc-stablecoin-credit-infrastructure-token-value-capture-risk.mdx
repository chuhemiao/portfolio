---
title: 'JPY Coin / JPYC: stablecoin / credit infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'JPY Coin is treated as a stablecoin / credit infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'jpy-coin-jpyc-stablecoin-credit-infrastructure-token-value-capture-risk'
category: 'research'
---

# JPY Coin / JPYC: stablecoin / credit infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** JPY Coin appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Astar, Avalanche, Ethereum, Polygon Pos, Shiden Network, Xdai; exchange exposure is not disclosed; category tags are Stablecoins, Payments & Gateway; current market cap is $20.24M; FDV is $20.24M; and 24h volume is $63.9400. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag JPY Coin before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 693, market cap $26,841,691, 24h volume $76,401. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

JPY Coin is best understood as a **stablecoin / credit infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether JPYC captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Astar, Avalanche, Ethereum, Polygon Pos, Shiden Network, Xdai |
...
| Latest TVL sample | $11.67M |
| TVL sample change | -56.71% |
| Primary source posture | Surf + CoinGecko + CMC discovery + official/project links |

Base case: JPY Coin remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning JPYC into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

JPYC is Japan's first regulated yen-pegged stablecoin, launched October 2025 by JPYC Inc. It is fully backed by yen deposits and Japanese government bonds, convertible 1:1 with JPY, and aims for domestic and cross-border payments.

Surf's project profile maps JPY Coin to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://jpyc.jp/](https://jpyc.jp/) |
| X handle | [@jpy_coin](https://x.com/jpy_coin) |
| X followers | 9.30K |
| Tags | Stablecoins, Payments & Gateway |
| Chains | Astar, Avalanche, Ethereum, Polygon Pos, Shiden Network, Xdai |
| Exchanges | not disclosed |
| TGE status | not disclosed |
| CoinGecko ID | not disclosed |

The main product question is whether JPY Coin is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For stablecoin / credit infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does JPY Coin convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $63.9400 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 2.63B circulating vs 2.63B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that JPY Coin is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

JPY Coin's architecture should be evaluated through the job it performs. For a stablecoin / credit infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
| Settlement layer | Chains and contracts that hold user value | Astar, Avalanche, Ethereum, Polygon Pos, Shiden Network, Xdai |
...
|---|---|---|---|
| Xdai | JPYC | 0x431d5dff03120afa4bdf332c61a6e1766ef37bdb | not mapped |
| Astar | JPYC | 0x431d5dff03120afa4bdf332c61a6e1766ef37bdb | not mapped |
| ethereum | JPYC | 0x431d5dff03120afa4bdf332c61a6e1766ef37bdb | [explorer](https://etherscan.io/token/0x431d5dff03120afa4bdf332c61a6e1766ef37bdb) |
| avalanche | JPYC | 0x431d5dff03120afa4bdf332c61a6e1766ef37bdb | [explorer](https://snowtrace.io/token/0x431d5dff03120afa4bdf332c61a6e1766ef37bdb) |
| polygon | JPYC | 0x431d5dff03120afa4bdf332c61a6e1766ef37bdb | [explorer](https://polygonscan.com/token/0x431d5dff03120afa4bdf332c61a6e1766ef37bdb) |
| Shiden Network | JPYC | 0x431d5dff03120afa4bdf332c61a6e1766ef37bdb | not mapped |
| Xdai | JPYC | 0x417602f4fbdd471a431ae29fb5fe0a681964c11b | not mapped |
| ethereum | JPYC | 0x2370f9d504c7a6e775bf6e14b3f12846b594cd53 | [explorer](https://etherscan.io/token/0x2370f9d504c7a6e775bf6e14b3f12846b594cd53) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For JPY Coin, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 693, market cap $26,841,691, 24h volume $76,401. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $20.24M, FDV $20.24M, 24h volume $63.9400, and price $0.0077.

| Traction metric | Value | Interpretation |
|---|---|---|
| Latest TVL sample | $11.67M | Useful only if sustained without aggressive incentives |
| TVL sample points | 90 | More points improve trend confidence |
| TVL sample change | -56.71% | Positive trend supports usage; negative trend requires explanation |
| 24h volume | $63.9400 | Position sizing and exit liquidity proxy |
| Market cap | $20.24M | Liquid valuation proxy |
| FDV | $20.24M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.00x | Higher spread means more supply risk |
| X followers | 9.30K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for JPY Coin | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether JPYC is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. JPY Coin currently needs to be monitored through that sequence. The available 90-day TVL series has 90 point(s), latest TVL $11.67M, and approximate change -56.71%. Market-price movement shows 7d 5.14% and 30d 8.97% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that JPY Coin's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2026-05-23 | Extended Series B | $19.50M | Meiji Yasuda Life Insurance |
| 2026-02-27 | Series B | $11.90M | Asteria Japan |
| 2024-05-23 | Undisclosed | not disclosed | not disclosed |
| 2024-01-02 | Undisclosed | not disclosed | Hyperithm |
| 2022-01-18 | Undisclosed | not disclosed | Infinity Ventures Crypto (IVC) |
| 2021-11-10 | Series A | $4.40M | Circle, Headline |

| Team member | Role | Link |
|---|---|---|
| Noritaka Okabe | Founder, CEO | [X](https://x.com/noritaka_okabe) |
...
JPY Coin competes in the stablecoin / credit infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether JPY Coin has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: JPY Coin is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | JPY Coin compounds usage, liquidity improves, and JPYC gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If JPYC does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
|---|---|---|---|
| TVL | $11.67M | Weekly | >20% drawdown or sustained growth |
| 24h volume | $63.9400 | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if JPY Coin announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. JPY Coin has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For JPY Coin, the first reconciliation job is identity. The token symbol is JPYC, the category is stablecoin / credit infrastructure, chains are Astar, Avalanche, Ethereum, Polygon Pos, Shiden Network, Xdai, exchanges are not disclosed, and tags are Stablecoins, Payments & Gateway. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If JPYC trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $20.24M, FDV is $20.24M, 24h volume is $63.9400, FDV / market cap is 1.00x, 7d price change is 5.14%, and 30d price change is 8.97%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.

The third reconciliation job is usage data. The latest TVL sample is $11.67M, TVL sample change is -56.71%, and the sample contains 90 point(s). TVL is useful only when the underlying balances are real, withdrawable, and not double-counted. For stablecoin / credit infrastructure, the memo should identify whether TVL means deposits, staked assets, LP liquidity, stablecoin supply, collateral, locked governance assets, or a derived wrapper. A yield-bearing stablecoin and a validator middleware protocol can both show TVL, but the risk encoded by that TVL is completely different.

| Reconciliation item | Current value | Next check |
|---|---|---|
| Identity | JPY Coin / JPYC | Match official docs, CG, CMC, Surf, contracts |
| Category | stablecoin / credit infrastructure | Confirm product mechanics and revenue model |
| Chains | Astar, Avalanche, Ethereum, Polygon Pos, Shiden Network, Xdai | Confirm canonical deployments and bridge assumptions |
| Exchanges | not disclosed | Check live order books and inactive pairs |
| Market cap | $20.24M | Compare Surf, CG, CMC, exchange pages |
| FDV | $20.24M | Reconcile total supply and unlock schedule |
| Volume | $63.9400 | Check exchange concentration and wash-trading risk |
| TVL | $11.67M | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what JPY Coin does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For JPY Coin, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does JPYC sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
For JPY Coin, the current liquidity read is watchlist first. The market cap is $20.24M, FDV is $20.24M, reported 24h volume is $63.9400, and FDV / market cap is 1.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | JPY Coin gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://jpyc.jp/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=JPY%20Coin) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=JPY%20Coin) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=JPY%20Coin) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=JPY%20Coin) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=JPY%20Coin) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/jpy_coin) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/jcam1/JPYCv2) | Identity, market, social, contract, or discovery cross-check |
| 9 | JPY Coin Price Chart (JPYC) - CoinGecko | [link](https://www.coingecko.com/en/coins/jpycoin) | Identity, market, social, contract, or discovery cross-check |
| 10 | JPY Coin Price Chart (JPYC) - CoinGecko | [link](https://www.coingecko.com/en/coins/jpy-coin) | Identity, market, social, contract, or discovery cross-check |
| 11 | JPY Coin v1 price JPYC #5678 - CoinMarketCap | [link](https://coinmarketcap.com/currencies/jpycoin/) | Identity, market, social, contract, or discovery cross-check |
| 12 | JPY Coin v1 Price Chart (JPYC) - CoinGecko | [link](https://www.coingecko.com/en/coins/jpy-coin-v1) | Identity, market, social, contract, or discovery cross-check |
| 13 | JPY Coin (jpyc.co.jp) to USD Chart - CoinMarketCap | [link](https://coinmarketcap.com/currencies/jpyc/) | Identity, market, social, contract, or discovery cross-check |
| 14 | ethereum JPYC | [link](https://etherscan.io/token/0x431d5dff03120afa4bdf332c61a6e1766ef37bdb) | Identity, market, social, contract, or discovery cross-check |
| 15 | avalanche JPYC | [link](https://snowtrace.io/token/0x431d5dff03120afa4bdf332c61a6e1766ef37bdb) | Identity, market, social, contract, or discovery cross-check |
| 16 | polygon JPYC | [link](https://polygonscan.com/token/0x431d5dff03120afa4bdf332c61a6e1766ef37bdb) | Identity, market, social, contract, or discovery cross-check |
| 17 | ethereum JPYC | [link](https://etherscan.io/token/0x2370f9d504c7a6e775bf6e14b3f12846b594cd53) | Identity, market, social, contract, or discovery cross-check |
| 18 | polygon JPYC | [link](https://polygonscan.com/token/0x6ae7dfc73e0dde2aa99ac063dcf7e8a63265108c) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google project search | [link](https://www.google.com/search?q=JPY%20Coin%20JPYC%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google tokenomics search | [link](https://www.google.com/search?q=JPY%20Coin%20JPYC%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google docs search | [link](https://www.google.com/search?q=JPY%20Coin%20docs) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google governance search | [link](https://www.google.com/search?q=JPY%20Coin%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 23 | Google audit search | [link](https://www.google.com/search?q=JPY%20Coin%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/jpy-coin/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 25 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/jpy-coin/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 26 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/jpy-coin/) | Identity, market, social, contract, or discovery cross-check |
| 27 | Dune discovery | [link](https://dune.com/browse/dashboards?q=JPY%20Coin) | Identity, market, social, contract, or discovery cross-check |
| 28 | Dune token query | [link](https://dune.com/browse/queries?q=JPY%20Coin) | Identity, market, social, contract, or discovery cross-check |
| 29 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=JPY%20Coin) | Identity, market, social, contract, or discovery cross-check |
| 30 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=JPYC) | Identity, market, social, contract, or discovery cross-check |
| 31 | TokenUnlocks search | [link](https://token.unlocks.app/?search=JPY%20Coin) | Identity, market, social, contract, or discovery cross-check |
| 32 | CryptoRank search | [link](https://cryptorank.io/price/jpy-coin) | Identity, market, social, contract, or discovery cross-check |
| 33 | RootData search | [link](https://www.rootdata.com/search?kw=JPY%20Coin) | Identity, market, social, contract, or discovery cross-check |
| 34 | CoinDesk search | [link](https://www.coindesk.com/search?s=JPY%20Coin) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://jpyc.jp/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=JPY%20Coin)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=JPY%20Coin)
- [DefiLlama protocol discovery](https://defillama.com/search?query=JPY%20Coin)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=JPY%20Coin)
- [Messari discovery](https://messari.io/search?q=JPY%20Coin)
- [X profile](https://x.com/jpy_coin)
- [GitHub](https://github.com/jcam1/JPYCv2)
- [JPY Coin Price Chart (JPYC) - CoinGecko](https://www.coingecko.com/en/coins/jpycoin)
- [JPY Coin Price Chart (JPYC) - CoinGecko](https://www.coingecko.com/en/coins/jpy-coin)
- [JPY Coin v1 price JPYC #5678 - CoinMarketCap](https://coinmarketcap.com/currencies/jpycoin/)
- [JPY Coin v1 Price Chart (JPYC) - CoinGecko](https://www.coingecko.com/en/coins/jpy-coin-v1)
- [JPY Coin (jpyc.co.jp) to USD Chart - CoinMarketCap](https://coinmarketcap.com/currencies/jpyc/)
- [ethereum JPYC](https://etherscan.io/token/0x431d5dff03120afa4bdf332c61a6e1766ef37bdb)
- [avalanche JPYC](https://snowtrace.io/token/0x431d5dff03120afa4bdf332c61a6e1766ef37bdb)
- [polygon JPYC](https://polygonscan.com/token/0x431d5dff03120afa4bdf332c61a6e1766ef37bdb)
- [ethereum JPYC](https://etherscan.io/token/0x2370f9d504c7a6e775bf6e14b3f12846b594cd53)
- [polygon JPYC](https://polygonscan.com/token/0x6ae7dfc73e0dde2aa99ac063dcf7e8a63265108c)
- [Google project search](https://www.google.com/search?q=JPY%20Coin%20JPYC%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=JPY%20Coin%20JPYC%20tokenomics)
- [Google docs search](https://www.google.com/search?q=JPY%20Coin%20docs)
- [Google governance search](https://www.google.com/search?q=JPY%20Coin%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=JPY%20Coin%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/jpy-coin/what-is/)

## Final Investment View

JPY Coin should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
