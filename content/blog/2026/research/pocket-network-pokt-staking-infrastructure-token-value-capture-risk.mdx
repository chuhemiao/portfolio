---
title: 'Pocket Network / POKT: staking infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Pocket Network is treated as a staking infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'pocket-network-pokt-staking-infrastructure-token-value-capture-risk'
category: 'research'
---

# Pocket Network / POKT: staking infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Pocket Network appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Arbitrum One, Base, Binance Smart Chain, Ethereum, Optimistic Ethereum, Polygon Pos, Solana; exchange exposure is UPBIT, BITHUMB, BINANCE ALPHA, BITGET; category tags are DePIN (Physical Infra), Node & Validator, Storage & Compute; current market cap is $15.71M; FDV is $18.36M; and 24h volume is $862.85K. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Pocket Network before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 948, market cap $15,639,675, 24h volume $842,955. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Pocket Network is best understood as a **staking infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether POKT captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Arbitrum One, Base, Binance Smart Chain, Ethereum, Optimistic Ethereum, Polygon Pos, Solana |
| Exchanges | UPBIT, BITHUMB, BINANCE ALPHA, BITGET |
| Price | $0.0078 |
| Market cap | $15.71M |
| FDV | $18.36M |
| 24h volume | $862.85K |
| Circulating supply | 2.01B |
| Total supply | 2.35B |
...
Base case: Pocket Network remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning POKT into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Pocket Network is a decentralized infrastructure protocol enabling permissionless access to any public data through a global network of nodes. The Shannon upgrade supports AI, blockchains, and beyond with incentives and quality scoring. POKT token is used for staking nodes, paying for access, and governance.

Surf's project profile maps Pocket Network to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.pokt.network/](https://www.pokt.network/) |
| X handle | [@POKTnetwork](https://x.com/POKTnetwork) |
| X followers | 45.17K |
...
| Liquidity | $862.85K 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 2.01B circulating vs 2.35B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Pocket Network is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Arbitrum One, Base, Binance Smart Chain, Ethereum, Optimistic Ethereum, Polygon Pos, Solana |
...
|---|---|---|---|
| base | POKT | 0x764a726d9ced0433a8d7643335919deb03a9a935 | [explorer](https://basescan.org/token/0x764a726d9ced0433a8d7643335919deb03a9a935) |
| solana | POKT | 6CAsXfiCXZfP8APCG6Vma2DFMindopxiqYQN4LSQfhoC | [explorer](https://solscan.io/token/6CAsXfiCXZfP8APCG6Vma2DFMindopxiqYQN4LSQfhoC) |
| ethereum | POKT | 0x764a726d9ced0433a8d7643335919deb03a9a935 | [explorer](https://etherscan.io/token/0x764a726d9ced0433a8d7643335919deb03a9a935) |
| polygon | POKT | 0x764a726d9ced0433a8d7643335919deb03a9a935 | [explorer](https://polygonscan.com/token/0x764a726d9ced0433a8d7643335919deb03a9a935) |
| arbitrum | POKT | 0x764a726d9ced0433a8d7643335919deb03a9a935 | [explorer](https://arbiscan.io/token/0x764a726d9ced0433a8d7643335919deb03a9a935) |
| bsc | POKT | 0x764a726d9ced0433a8d7643335919deb03a9a935 | [explorer](https://bscscan.com/token/0x764a726d9ced0433a8d7643335919deb03a9a935) |
| optimism | POKT | 0x764a726d9ced0433a8d7643335919deb03a9a935 | [explorer](https://optimistic.etherscan.io/token/0x764a726d9ced0433a8d7643335919deb03a9a935) |
| ethereum | WPOKT | 0x67f4c72a50f8df6487720261e188f2abe83f57d7 | [explorer](https://etherscan.io/token/0x67f4c72a50f8df6487720261e188f2abe83f57d7) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Pocket Network, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 948, market cap $15,639,675, 24h volume $842,955. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $15.71M, FDV $18.36M, 24h volume $862.85K, and price $0.0078.
...
| 24h volume | $862.85K | Position sizing and exit liquidity proxy |
| Market cap | $15.71M | Liquid valuation proxy |
| FDV | $18.36M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.17x | Higher spread means more supply risk |
| X followers | 45.17K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Pocket Network | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether POKT is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. Pocket Network should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that Pocket Network belongs to the staking infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $15.71M, FDV $18.36M, 24h volume $862.85K, price $0.0078, circulating supply 2.01B, and total supply 2.35B. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For Pocket Network, the candidate backlog records Seeded 2026-07-06 from Surf market-ranking; rank 948, market cap $15,639,675, 24h volume $842,955. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Arbitrum One, Base, Binance Smart Chain, Ethereum, Optimistic Ethereum, Polygon Pos, Solana, exchanges as UPBIT, BITHUMB, BINANCE ALPHA, BITGET, and tags as DePIN (Physical Infra), Node & Validator, Storage & Compute. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Pocket Network currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d -2.83% and 30d 0.69% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Pocket Network's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2022-01-06 | Strategic | $10.00M | Arrington Capital, C² Ventures, Republic Capital, Decentral Park Capital, CoinShares, Dominance Ventures, RockTree Capital |
| 2020-07-07 | Seed | $130.00K | Eden Block, Borderless Capital |
| 2019-02-27 | Angel | $750.00K | not disclosed |

| Team member | Role | Link |
|---|---|---|
| Arthur Sabintsev | COO | [X](https://x.com/ArtSabintsev) |
...
Pocket Network competes in the staking infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Pocket Network has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Pocket Network is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Pocket Network compounds usage, liquidity improves, and POKT gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If POKT does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $862.85K | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Pocket Network announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Pocket Network has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Pocket Network, the first reconciliation job is identity. The token symbol is POKT, the category is staking infrastructure, chains are Arbitrum One, Base, Binance Smart Chain, Ethereum, Optimistic Ethereum, Polygon Pos, Solana, exchanges are UPBIT, BITHUMB, BINANCE ALPHA, BITGET, and tags are DePIN (Physical Infra), Node & Validator, Storage & Compute. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If POKT trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $15.71M, FDV is $18.36M, 24h volume is $862.85K, FDV / market cap is 1.17x, 7d price change is -2.83%, and 30d price change is 0.69%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
For Pocket Network, the current liquidity read is watchlist first. The market cap is $15.71M, FDV is $18.36M, reported 24h volume is $862.85K, and FDV / market cap is 1.17x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. Pocket Network can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.

For Pocket Network, the live monitoring stack should include contract events, TVL, volume, governance, documentation changes, social announcements, security disclosures, and exchange-market quality. A single metric is not enough. TVL may stay high while liquidity thins. Volume may rise because of volatility rather than adoption. Social attention may rise around controversy rather than progress. Governance may pass proposals that improve short-term price but add long-term risk. The strongest monitoring dashboard combines on-chain state, off-chain disclosure, and market quality.

Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If Pocket Network suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | Pocket Network gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.pokt.network/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Pocket%20Network) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Pocket%20Network) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Pocket%20Network) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Pocket%20Network) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Pocket%20Network) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/POKTnetwork) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/pokt-network) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/POKTnetwork) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](https://discord.com/invite/uYs6Esum3r) | Identity, market, social, contract, or discovery cross-check |
| 11 | Pocket Network Price Chart (POKT) | [link](https://www.coingecko.com/en/coins/pocket-network) | Identity, market, social, contract, or discovery cross-check |
| 12 | What Is Pocket Network (POKT) And How Does It Work? | [link](https://coinmarketcap.com/cmc-ai/pocket-network/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 13 | Pocket Network (POKT) Price Prediction For 2026 & Beyond | [link](https://coinmarketcap.com/cmc-ai/pocket-network/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 14 | POKT to EUR: Pocket Network Price in Euro | [link](https://www.coingecko.com/en/coins/pocket-network/eur) | Identity, market, social, contract, or discovery cross-check |
| 15 | Pocket Network Price (POKT) | [link](https://www.coinbase.com/price/pocket-network) | Identity, market, social, contract, or discovery cross-check |
| 16 | base POKT | [link](https://basescan.org/token/0x764a726d9ced0433a8d7643335919deb03a9a935) | Identity, market, social, contract, or discovery cross-check |
| 17 | solana POKT | [link](https://solscan.io/token/6CAsXfiCXZfP8APCG6Vma2DFMindopxiqYQN4LSQfhoC) | Identity, market, social, contract, or discovery cross-check |
| 18 | ethereum POKT | [link](https://etherscan.io/token/0x764a726d9ced0433a8d7643335919deb03a9a935) | Identity, market, social, contract, or discovery cross-check |
| 19 | polygon POKT | [link](https://polygonscan.com/token/0x764a726d9ced0433a8d7643335919deb03a9a935) | Identity, market, social, contract, or discovery cross-check |
| 20 | arbitrum POKT | [link](https://arbiscan.io/token/0x764a726d9ced0433a8d7643335919deb03a9a935) | Identity, market, social, contract, or discovery cross-check |
| 21 | bsc POKT | [link](https://bscscan.com/token/0x764a726d9ced0433a8d7643335919deb03a9a935) | Identity, market, social, contract, or discovery cross-check |
| 22 | optimism POKT | [link](https://optimistic.etherscan.io/token/0x764a726d9ced0433a8d7643335919deb03a9a935) | Identity, market, social, contract, or discovery cross-check |
| 23 | ethereum WPOKT | [link](https://etherscan.io/token/0x67f4c72a50f8df6487720261e188f2abe83f57d7) | Identity, market, social, contract, or discovery cross-check |
| 24 | Google project search | [link](https://www.google.com/search?q=Pocket%20Network%20POKT%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 25 | Google tokenomics search | [link](https://www.google.com/search?q=Pocket%20Network%20POKT%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 26 | Google docs search | [link](https://www.google.com/search?q=Pocket%20Network%20docs) | Identity, market, social, contract, or discovery cross-check |
| 27 | Google governance search | [link](https://www.google.com/search?q=Pocket%20Network%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 28 | Google audit search | [link](https://www.google.com/search?q=Pocket%20Network%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 29 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/pocket-network/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 30 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/pocket-network/) | Identity, market, social, contract, or discovery cross-check |
| 31 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Pocket%20Network) | Identity, market, social, contract, or discovery cross-check |
| 32 | Dune token query | [link](https://dune.com/browse/queries?q=Pocket%20Network) | Identity, market, social, contract, or discovery cross-check |
| 33 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Pocket%20Network) | Identity, market, social, contract, or discovery cross-check |
| 34 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=POKT) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.pokt.network/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Pocket%20Network)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Pocket%20Network)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Pocket%20Network)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Pocket%20Network)
- [Messari discovery](https://messari.io/search?q=Pocket%20Network)
- [X profile](https://x.com/POKTnetwork)
- [GitHub](https://github.com/pokt-network)
- [Telegram](https://t.me/POKTnetwork)
- [Discord](https://discord.com/invite/uYs6Esum3r)
- [Pocket Network Price Chart (POKT)](https://www.coingecko.com/en/coins/pocket-network)
- [What Is Pocket Network (POKT) And How Does It Work?](https://coinmarketcap.com/cmc-ai/pocket-network/what-is/)
- [Pocket Network (POKT) Price Prediction For 2026 & Beyond](https://coinmarketcap.com/cmc-ai/pocket-network/price-prediction/)
- [POKT to EUR: Pocket Network Price in Euro](https://www.coingecko.com/en/coins/pocket-network/eur)
- [Pocket Network Price (POKT)](https://www.coinbase.com/price/pocket-network)
- [base POKT](https://basescan.org/token/0x764a726d9ced0433a8d7643335919deb03a9a935)
- [solana POKT](https://solscan.io/token/6CAsXfiCXZfP8APCG6Vma2DFMindopxiqYQN4LSQfhoC)
- [ethereum POKT](https://etherscan.io/token/0x764a726d9ced0433a8d7643335919deb03a9a935)
- [polygon POKT](https://polygonscan.com/token/0x764a726d9ced0433a8d7643335919deb03a9a935)
- [arbitrum POKT](https://arbiscan.io/token/0x764a726d9ced0433a8d7643335919deb03a9a935)
- [bsc POKT](https://bscscan.com/token/0x764a726d9ced0433a8d7643335919deb03a9a935)
- [optimism POKT](https://optimistic.etherscan.io/token/0x764a726d9ced0433a8d7643335919deb03a9a935)
- [ethereum WPOKT](https://etherscan.io/token/0x67f4c72a50f8df6487720261e188f2abe83f57d7)
- [Google project search](https://www.google.com/search?q=Pocket%20Network%20POKT%20crypto)

## Final Investment View

Pocket Network should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
