---
title: 'Yield Basis / YB: Leveraged LP / yield strategy value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Yield Basis is treated as a Leveraged LP / yield strategy watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'yield-basis-yb-leveraged-lp-yield-strategy-token-value-capture-risk'
category: 'research'
---

# Yield Basis / YB: Leveraged LP / yield strategy value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Yield Basis appears in the local CoinGecko/CMC-oriented candidate pool through [CoinGecko](https://www.coingecko.com/en/coins/yield-basis) and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Binance Smart Chain, Ethereum; exchange exposure is BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BYBIT, BITGET; category tags are DEX & AMM, Yield & Asset Mgmt; current market cap is $16.88M; FDV is $55.31M; and 24h volume is $7.54M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Yield Basis before this file was created, and the existing candidate note says: DefiLlama category Leveraged Farming on Ethereum; TVL $111,480,963. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Yield Basis is best understood as a **Leveraged LP / yield strategy** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether YB captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.

| Metric | Current read |
|---|---|
| Project | Yield Basis |
| Token | YB |
| Category | Leveraged LP / yield strategy |
...
| FDV | $55.31M |
| 24h volume | $7.54M |
| Circulating supply | 226.11M |
| Total supply | 741.13M |
...
Base case: Yield Basis remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning YB into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Yield Basis is a DeFi protocol eliminating impermanent loss via auto-leveraging AMMs, enabling yield on BTC and ETH by converting volatility into returns. It features the YB token for governance and emissions, integrated with Curve's crvUSD. Focused on sustainable yields for crypto holders.

Surf's project profile maps Yield Basis to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://yieldbasis.com/](https://yieldbasis.com/) |
| X handle | [@yieldbasis](https://x.com/yieldbasis) |
| X followers | 35.69K |
| Tags | DEX & AMM, Yield & Asset Mgmt |
| Chains | Binance Smart Chain, Ethereum |
| Exchanges | BINANCE, UPBIT, COINBASE, OKX, BITHUMB, BYBIT, BITGET |
| TGE status | post |
| CoinGecko ID | [yield-basis](https://www.coingecko.com/en/coins/yield-basis) |

The main product question is whether Yield Basis is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For Leveraged LP / yield strategy projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Yield Basis convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $7.54M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 226.11M circulating vs 741.13M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Yield Basis is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

Yield Basis's architecture should be evaluated through the job it performs. For a Leveraged LP / yield strategy project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
|---|---|---|---|
| ethereum | YB | 0x01791f726b4103694969820be083196cc7c045ff | [explorer](https://etherscan.io/token/0x01791f726b4103694969820be083196cc7c045ff) |
| bsc | YB | 0xfb93ee8152dd0a0e6f4b49c66c06d800cf1db72d | [explorer](https://bscscan.com/token/0xfb93ee8152dd0a0e6f4b49c66c06d800cf1db72d) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Yield Basis, the DeFi-oriented backlog note is the first signal: DefiLlama category Leveraged Farming on Ethereum; TVL $111,480,963. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $16.88M, FDV $55.31M, 24h volume $7.54M, and price $0.0746.
...
| 24h volume | $7.54M | Position sizing and exit liquidity proxy |
| Market cap | $16.88M | Liquid valuation proxy |
| FDV | $55.31M | Dilution-aware valuation proxy |
| FDV / market cap spread | 3.28x | Higher spread means more supply risk |
| X followers | 35.69K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Yield Basis | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether YB is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
|---|---|---|---|
| 2025-10-13 | Ieo | $2.50M | not disclosed |
| 2025-10-01 | Ieo | $2.50M | not disclosed |
| 2025-10-01 | Ico | $7.19M | not disclosed |
| 2025-02-19 | Undisclosed | $6.05M | not disclosed |

| Team member | Role | Link |
|---|---|---|
| Michael Egorov | Founder | [X](https://x.com/newmichwill) |
...
Yield Basis competes in the Leveraged LP / yield strategy bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Yield Basis has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Yield Basis is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Yield Basis compounds usage, liquidity improves, and YB gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If YB does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $7.54M | Weekly | Volume below position-size threshold |
| FDV / market cap | 3.28x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Yield Basis announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
| Correlation check | Count exposure to related Leveraged LP / yield strategy assets | Category beta can hide concentration |
...
The failure-mode view is deliberately harsher than the base case. Yield Basis can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.

For Yield Basis, the live monitoring stack should include contract events, TVL, volume, governance, documentation changes, social announcements, security disclosures, and exchange-market quality. A single metric is not enough. TVL may stay high while liquidity thins. Volume may rise because of volatility rather than adoption. Social attention may rise around controversy rather than progress. Governance may pass proposals that improve short-term price but add long-term risk. The strongest monitoring dashboard combines on-chain state, off-chain disclosure, and market quality.

Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If Yield Basis suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | Yield Basis gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://yieldbasis.com/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko profile | [link](https://www.coingecko.com/en/coins/yield-basis) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Yield%20Basis) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Yield%20Basis) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Yield%20Basis) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Yield%20Basis) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/yieldbasis) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/yield-basis) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/yieldbasis_chat) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](https://discord.com/invite/yieldbasis) | Identity, market, social, contract, or discovery cross-check |
| 11 | What Is YieldBasis (YB) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/yieldbasis/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 12 | Yield Basis (YB) / Tokenomics & Vesting Schedule - Token Unlocks | [link](https://tokenomist.ai/yield-basis) | Identity, market, social, contract, or discovery cross-check |
| 13 | YB to BTC: Yield Basis Price in Bitcoin - CoinGecko | [link](https://www.coingecko.com/en/coins/yield-basis/btc) | Identity, market, social, contract, or discovery cross-check |
| 14 | Yield Basis (YB) - ICO Drops | [link](https://icodrops.com/yield-basis/) | Identity, market, social, contract, or discovery cross-check |
| 15 | ethereum YB | [link](https://etherscan.io/token/0x01791f726b4103694969820be083196cc7c045ff) | Identity, market, social, contract, or discovery cross-check |
| 16 | bsc YB | [link](https://bscscan.com/token/0xfb93ee8152dd0a0e6f4b49c66c06d800cf1db72d) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google project search | [link](https://www.google.com/search?q=Yield%20Basis%20YB%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google tokenomics search | [link](https://www.google.com/search?q=Yield%20Basis%20YB%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google docs search | [link](https://www.google.com/search?q=Yield%20Basis%20docs) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google governance search | [link](https://www.google.com/search?q=Yield%20Basis%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google audit search | [link](https://www.google.com/search?q=Yield%20Basis%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinGecko historical data | [link](https://www.coingecko.com/en/coins/yield-basis/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/yield-basis/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/yield-basis/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 25 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/yield-basis/) | Identity, market, social, contract, or discovery cross-check |
| 26 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Yield%20Basis) | Identity, market, social, contract, or discovery cross-check |
| 27 | Dune token query | [link](https://dune.com/browse/queries?q=Yield%20Basis) | Identity, market, social, contract, or discovery cross-check |
| 28 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Yield%20Basis) | Identity, market, social, contract, or discovery cross-check |
| 29 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=YB) | Identity, market, social, contract, or discovery cross-check |
| 30 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Yield%20Basis) | Identity, market, social, contract, or discovery cross-check |
| 31 | CryptoRank search | [link](https://cryptorank.io/price/yield-basis) | Identity, market, social, contract, or discovery cross-check |
| 32 | RootData search | [link](https://www.rootdata.com/search?kw=Yield%20Basis) | Identity, market, social, contract, or discovery cross-check |
| 33 | CoinDesk search | [link](https://www.coindesk.com/search?s=Yield%20Basis) | Identity, market, social, contract, or discovery cross-check |
| 34 | The Block search | [link](https://www.theblock.co/search?query=Yield%20Basis) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://yieldbasis.com/)
- [CoinGecko profile](https://www.coingecko.com/en/coins/yield-basis)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Yield%20Basis)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Yield%20Basis)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Yield%20Basis)
- [Messari discovery](https://messari.io/search?q=Yield%20Basis)
- [X profile](https://x.com/yieldbasis)
- [GitHub](https://github.com/yield-basis)
- [Telegram](https://t.me/yieldbasis_chat)
- [Discord](https://discord.com/invite/yieldbasis)
- [What Is YieldBasis (YB) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/yieldbasis/what-is/)
- [Yield Basis (YB) | Tokenomics & Vesting Schedule - Token Unlocks](https://tokenomist.ai/yield-basis)
- [YB to BTC: Yield Basis Price in Bitcoin - CoinGecko](https://www.coingecko.com/en/coins/yield-basis/btc)
- [Yield Basis (YB) - ICO Drops](https://icodrops.com/yield-basis/)
- [ethereum YB](https://etherscan.io/token/0x01791f726b4103694969820be083196cc7c045ff)
- [bsc YB](https://bscscan.com/token/0xfb93ee8152dd0a0e6f4b49c66c06d800cf1db72d)
- [Google project search](https://www.google.com/search?q=Yield%20Basis%20YB%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Yield%20Basis%20YB%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Yield%20Basis%20docs)
- [Google governance search](https://www.google.com/search?q=Yield%20Basis%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Yield%20Basis%20audit%20report)
- [CoinGecko historical data](https://www.coingecko.com/en/coins/yield-basis/historical_data)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/yield-basis/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/yield-basis/price-analysis/)

## Final Investment View

Yield Basis should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
