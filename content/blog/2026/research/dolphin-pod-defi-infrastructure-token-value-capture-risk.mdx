---
title: 'Dolphin / POD: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Dolphin is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'dolphin-pod-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# Dolphin / POD: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Dolphin appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Base; exchange exposure is not disclosed; category tags are DePIN (Physical Infra), Web3 AI (Agent/AIGC); current market cap is $18.00M; FDV is $206.38M; and 24h volume is $1.30M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Dolphin before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 868, market cap $18,311,088, 24h volume $1,314,191. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Dolphin is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether POD captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| FDV | $206.38M |
| 24h volume | $1.30M |
| Circulating supply | 43.60M |
| Total supply | 500.00M |
| 7d price change | -2.60% |
| 30d price change | 131.37% |
...
Base case: Dolphin remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning POD into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Dolphin (dphnAI) is an AI lab that develops steerable, distributed-inference large language models and tools (notably Dolphin Mistral 24B Venice Edition) and offers hosted access and a Telegram bot. The project publishes models and code on Hugging Face and maintains an ecosystem token ($DPHN) on Base.

Surf's project profile maps Dolphin to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | not disclosed |
| X handle | [@dphnAI](https://x.com/dphnAI) |
| X followers | 7.01K |
...
| Liquidity | $1.30M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 43.60M circulating vs 500.00M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Dolphin is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| base | DPHN | 0xe2b76f98fb5dc10a9aed0f4a4a854de325740019 | [explorer](https://basescan.org/token/0xe2b76f98fb5dc10a9aed0f4a4a854de325740019) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Dolphin, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 868, market cap $18,311,088, 24h volume $1,314,191. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $18.00M, FDV $206.38M, 24h volume $1.30M, and price $0.4128.
...
| 24h volume | $1.30M | Position sizing and exit liquidity proxy |
| Market cap | $18.00M | Liquid valuation proxy |
| FDV | $206.38M | Dilution-aware valuation proxy |
| FDV / market cap spread | 11.47x | Higher spread means more supply risk |
| X followers | 7.01K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Dolphin | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether POD is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Dolphin currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d -2.60% and 30d 131.37% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Dolphin's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
Dolphin competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Dolphin has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Dolphin is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Dolphin compounds usage, liquidity improves, and POD gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Dolphin has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If POD does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $1.30M | Weekly | Volume below position-size threshold |
| FDV / market cap | 11.47x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Dolphin announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Dolphin has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Dolphin, the first reconciliation job is identity. The token symbol is POD, the category is DeFi infrastructure, chains are Base, exchanges are not disclosed, and tags are DePIN (Physical Infra), Web3 AI (Agent/AIGC). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If POD trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $18.00M, FDV is $206.38M, 24h volume is $1.30M, FDV / market cap is 11.47x, 7d price change is -2.60%, and 30d price change is 131.37%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Dolphin / POD | Match official docs, CG, CMC, Surf, contracts |
...
For Dolphin, the current liquidity read is watchlist first. The market cap is $18.00M, FDV is $206.38M, reported 24h volume is $1.30M, and FDV / market cap is 11.47x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Dolphin gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Dolphin) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Dolphin) | Identity, market, social, contract, or discovery cross-check |
| 3 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Dolphin) | Identity, market, social, contract, or discovery cross-check |
| 4 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Dolphin) | Identity, market, social, contract, or discovery cross-check |
| 5 | Messari discovery | [link](https://messari.io/search?q=Dolphin) | Identity, market, social, contract, or discovery cross-check |
| 6 | X profile | [link](https://x.com/dphnAI) | Identity, market, social, contract, or discovery cross-check |
| 7 | Dolphin Price: POD/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/dolphin-2) | Identity, market, social, contract, or discovery cross-check |
| 8 | Dolphin Price Chart (DOLPHIN) - Market Cap - CoinGecko | [link](https://www.coingecko.com/en/coins/dolphin) | Identity, market, social, contract, or discovery cross-check |
| 9 | POD to BTC: Dolphin Price in Bitcoin - CoinGecko | [link](https://www.coingecko.com/en/coins/dolphin-2/btc) | Identity, market, social, contract, or discovery cross-check |
| 10 | Dolphin (POD) Price Prediction 2026, 2027-2030 / CoinEx Academy | [link](https://www.coinex.com/en/academy/detail/5807-dolphin-pod-price-prediction) | Identity, market, social, contract, or discovery cross-check |
...
| 12 | base DPHN | [link](https://basescan.org/token/0xe2b76f98fb5dc10a9aed0f4a4a854de325740019) | Identity, market, social, contract, or discovery cross-check |
| 13 | Google project search | [link](https://www.google.com/search?q=Dolphin%20POD%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 14 | Google tokenomics search | [link](https://www.google.com/search?q=Dolphin%20POD%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 15 | Google docs search | [link](https://www.google.com/search?q=Dolphin%20docs) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google governance search | [link](https://www.google.com/search?q=Dolphin%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google audit search | [link](https://www.google.com/search?q=Dolphin%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 18 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/dolphin/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 19 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/dolphin/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 20 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/dolphin/) | Identity, market, social, contract, or discovery cross-check |
| 21 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Dolphin) | Identity, market, social, contract, or discovery cross-check |
| 22 | Dune token query | [link](https://dune.com/browse/queries?q=Dolphin) | Identity, market, social, contract, or discovery cross-check |
| 23 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Dolphin) | Identity, market, social, contract, or discovery cross-check |
| 24 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=POD) | Identity, market, social, contract, or discovery cross-check |
| 25 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Dolphin) | Identity, market, social, contract, or discovery cross-check |
| 26 | CryptoRank search | [link](https://cryptorank.io/price/dolphin) | Identity, market, social, contract, or discovery cross-check |
| 27 | RootData search | [link](https://www.rootdata.com/search?kw=Dolphin) | Identity, market, social, contract, or discovery cross-check |
| 28 | CoinDesk search | [link](https://www.coindesk.com/search?s=Dolphin) | Identity, market, social, contract, or discovery cross-check |
| 29 | The Block search | [link](https://www.theblock.co/search?query=Dolphin) | Identity, market, social, contract, or discovery cross-check |
| 30 | GitHub search | [link](https://github.com/search?q=Dolphin&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 31 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=POD) | Identity, market, social, contract, or discovery cross-check |
| 32 | BscScan token search | [link](https://bscscan.com/search?f=0&q=POD) | Identity, market, social, contract, or discovery cross-check |
| 33 | Arbiscan token search | [link](https://arbiscan.io/search?f=0&q=POD) | Identity, market, social, contract, or discovery cross-check |
| 34 | BaseScan token search | [link](https://basescan.org/search?f=0&q=POD) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [CoinGecko search](https://www.coingecko.com/en/search?query=Dolphin)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Dolphin)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Dolphin)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Dolphin)
- [Messari discovery](https://messari.io/search?q=Dolphin)
- [X profile](https://x.com/dphnAI)
- [Dolphin Price: POD/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/dolphin-2)
- [Dolphin Price Chart (DOLPHIN) - Market Cap - CoinGecko](https://www.coingecko.com/en/coins/dolphin)
- [POD to BTC: Dolphin Price in Bitcoin - CoinGecko](https://www.coingecko.com/en/coins/dolphin-2/btc)
- [Dolphin (POD) Price Prediction 2026, 2027-2030 | CoinEx Academy](https://www.coinex.com/en/academy/detail/5807-dolphin-pod-price-prediction)
- [CoinGecko: Cryptocurrency Prices, Charts, and Crypto Market Cap](https://www.coingecko.com/)
- [base DPHN](https://basescan.org/token/0xe2b76f98fb5dc10a9aed0f4a4a854de325740019)
- [Google project search](https://www.google.com/search?q=Dolphin%20POD%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Dolphin%20POD%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Dolphin%20docs)
- [Google governance search](https://www.google.com/search?q=Dolphin%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Dolphin%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/dolphin/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/dolphin/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/dolphin/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Dolphin)
- [Dune token query](https://dune.com/browse/queries?q=Dolphin)
- [DeFiLlama fees discovery](https://defillama.com/fees?chain=All&search=Dolphin)
- [DeFiLlama yield discovery](https://defillama.com/yields?token=POD)

## Final Investment View

Dolphin should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
