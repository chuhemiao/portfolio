---
title: 'Heima / HEI: bridge / cross-chain infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Heima is treated as a bridge / cross-chain infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'heima-hei-bridge-cross-chain-infrastructure-token-value-capture-risk'
category: 'research'
---

# Heima / HEI: bridge / cross-chain infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Heima appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Ethereum; exchange exposure is BINANCE, BITGET; category tags are Cross-Chain & Bridge, Layer1, Wallets & AA (Account Abstraction); current market cap is $11.87M; FDV is $12.14M; and 24h volume is $10.90M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Heima before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 1089, market cap $11,644,325, 24h volume $8,026,876. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Heima is best understood as a **bridge / cross-chain infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether HEI captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| FDV | $12.14M |
| 24h volume | $10.90M |
| Circulating supply | 97.76M |
| Total supply | 100.00M |
| 7d price change | -20.20% |
...
Base case: Heima remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning HEI into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Heima Network (formerly Litentry) is an EVM‑compatible Layer‑1 chain‑abstraction protocol that provides unified omni‑accounts and account/chain abstraction to enable seamless cross‑chain DEX trading and on‑chain UX. Its stack (Heima L1, account abstraction, Agent Hub and TEE‑secured workers) routes user intents, automates cross‑chain execution and powers the HEI token for utility, staking and governance.

Surf's project profile maps Heima to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.heima.network/](https://www.heima.network/) |
| X handle | [@heimaNetwork](https://x.com/heimaNetwork) |
| X followers | 98.27K |
...
| Liquidity | $10.90M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 97.76M circulating vs 100.00M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Heima is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| ethereum | HEI | 0xf8f173e20e15f3b6cb686fb64724d370689de083 | [explorer](https://etherscan.io/token/0xf8f173e20e15f3b6cb686fb64724d370689de083) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Heima, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 1089, market cap $11,644,325, 24h volume $8,026,876. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $11.87M, FDV $12.14M, 24h volume $10.90M, and price $0.1211.
...
| 24h volume | $10.90M | Position sizing and exit liquidity proxy |
| Market cap | $11.87M | Liquid valuation proxy |
| FDV | $12.14M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.02x | Higher spread means more supply risk |
| X followers | 98.27K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Heima | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether HEI is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Heima currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d -20.20% and 30d 8.00% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Heima's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|
| Hanwen Cheng (Kevin) | Founder, CEO | [X](https://x.com/c_kevin75) |
| Fei Liu | CTO | [X](https://x.com/c_kevin75) |
...
Heima competes in the bridge / cross-chain infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Heima has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Heima is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Heima compounds usage, liquidity improves, and HEI gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Heima has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If HEI does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $10.90M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Heima announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Heima has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Heima, the first reconciliation job is identity. The token symbol is HEI, the category is bridge / cross-chain infrastructure, chains are Ethereum, exchanges are BINANCE, BITGET, and tags are Cross-Chain & Bridge, Layer1, Wallets & AA (Account Abstraction). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If HEI trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $11.87M, FDV is $12.14M, 24h volume is $10.90M, FDV / market cap is 1.02x, 7d price change is -20.20%, and 30d price change is 8.00%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Heima / HEI | Match official docs, CG, CMC, Surf, contracts |
...
For Heima, the current liquidity read is watchlist first. The market cap is $11.87M, FDV is $12.14M, reported 24h volume is $10.90M, and FDV / market cap is 1.02x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Heima gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.heima.network/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Heima) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Heima) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Heima) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Heima) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Heima) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/heimaNetwork) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/litentry/heima) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/heima_network) | Identity, market, social, contract, or discovery cross-check |
| 10 | Heima Price: HEI/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/heima) | Identity, market, social, contract, or discovery cross-check |
| 11 | Heima Price History: Download HEI Historical Data / CoinGecko | [link](https://www.coingecko.com/en/coins/heima/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 12 | What Is Heima (HEI) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/heima/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 13 | HEI to TRY: Heima Price in Turkish Lira - CoinGecko | [link](https://www.coingecko.com/en/coins/heima/try) | Identity, market, social, contract, or discovery cross-check |
| 14 | Heima price today, HEI to USD live price, marketcap and chart | [link](https://coinmarketcap.com/currencies/heima/) | Identity, market, social, contract, or discovery cross-check |
| 15 | ethereum HEI | [link](https://etherscan.io/token/0xf8f173e20e15f3b6cb686fb64724d370689de083) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google project search | [link](https://www.google.com/search?q=Heima%20HEI%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google tokenomics search | [link](https://www.google.com/search?q=Heima%20HEI%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google docs search | [link](https://www.google.com/search?q=Heima%20docs) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google governance search | [link](https://www.google.com/search?q=Heima%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google audit search | [link](https://www.google.com/search?q=Heima%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/heima/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 22 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Heima) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune token query | [link](https://dune.com/browse/queries?q=Heima) | Identity, market, social, contract, or discovery cross-check |
| 24 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Heima) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=HEI) | Identity, market, social, contract, or discovery cross-check |
| 26 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Heima) | Identity, market, social, contract, or discovery cross-check |
| 27 | CryptoRank search | [link](https://cryptorank.io/price/heima) | Identity, market, social, contract, or discovery cross-check |
| 28 | RootData search | [link](https://www.rootdata.com/search?kw=Heima) | Identity, market, social, contract, or discovery cross-check |
| 29 | CoinDesk search | [link](https://www.coindesk.com/search?s=Heima) | Identity, market, social, contract, or discovery cross-check |
| 30 | The Block search | [link](https://www.theblock.co/search?query=Heima) | Identity, market, social, contract, or discovery cross-check |
| 31 | GitHub search | [link](https://github.com/search?q=Heima&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 32 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=HEI) | Identity, market, social, contract, or discovery cross-check |
| 33 | BscScan token search | [link](https://bscscan.com/search?f=0&q=HEI) | Identity, market, social, contract, or discovery cross-check |
| 34 | Arbiscan token search | [link](https://arbiscan.io/search?f=0&q=HEI) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.heima.network/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Heima)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Heima)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Heima)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Heima)
- [Messari discovery](https://messari.io/search?q=Heima)
- [X profile](https://x.com/heimaNetwork)
- [GitHub](https://github.com/litentry/heima)
- [Telegram](https://t.me/heima_network)
- [Heima Price: HEI/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/heima)
- [Heima Price History: Download HEI Historical Data | CoinGecko](https://www.coingecko.com/en/coins/heima/historical_data)
- [What Is Heima (HEI) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/heima/what-is/)
- [HEI to TRY: Heima Price in Turkish Lira - CoinGecko](https://www.coingecko.com/en/coins/heima/try)
- [Heima price today, HEI to USD live price, marketcap and chart](https://coinmarketcap.com/currencies/heima/)
- [ethereum HEI](https://etherscan.io/token/0xf8f173e20e15f3b6cb686fb64724d370689de083)
- [Google project search](https://www.google.com/search?q=Heima%20HEI%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Heima%20HEI%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Heima%20docs)
- [Google governance search](https://www.google.com/search?q=Heima%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Heima%20audit%20report)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/heima/price-analysis/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Heima)
- [Dune token query](https://dune.com/browse/queries?q=Heima)
- [DeFiLlama fees discovery](https://defillama.com/fees?chain=All&search=Heima)

## Final Investment View

Heima should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
