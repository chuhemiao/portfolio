---
title: 'Stargate Finance / STG: bridge / cross-chain infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Stargate Finance is treated as a bridge / cross-chain infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'stargate-finance-stg-bridge-cross-chain-infrastructure-token-value-capture-risk'
category: 'research'
---

# Stargate Finance / STG: bridge / cross-chain infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Stargate Finance appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Arbitrum One, Avalanche, Base, Berachain, Binance Smart Chain, Ethereum, Fantom, Fuse, Gravity Alpha, Kaia, Kava, Linea, Mantle, Metis Andromeda, Optimistic Ethereum, Polygon Pos, Rari, Scroll, Taiko; exchange exposure is BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET; category tags are Cross-Chain & Bridge; current market cap is $22.10M; FDV is $22.10M; and 24h volume is $13.74M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Stargate Finance before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 776, market cap $21,982,979, 24h volume $13,764,154. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Stargate Finance is best understood as a **bridge / cross-chain infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether STG captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Arbitrum One, Avalanche, Base, Berachain, Binance Smart Chain, Ethereum, Fantom, Fuse, Gravity Alpha, Kaia, Kava, Linea, Mantle, Metis Andromeda, Optimistic Ethereum, Polygon Pos, Rari, Scroll, Taiko |
| Exchanges | BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET |
| Price | $0.1568 |
| Market cap | $22.10M |
| FDV | $22.10M |
| 24h volume | $13.74M |
| Circulating supply | 140.91M |
| Total supply | 140.91M |
| 7d price change | -5.18% |
| 30d price change | -23.10% |
...
Base case: Stargate Finance remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning STG into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Stargate Finance is a composable omnichain liquidity transport protocol that enables instant, guaranteed cross-chain transfers of native assets via unified liquidity pools, powering swaps, bridges and developer integrations. STG is the protocol token used for governance, staking (veSTG), and incentives for liquidity providers across supported networks.

Surf's project profile maps Stargate Finance to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://stargate.finance/](https://stargate.finance/) |
| X handle | [@StargateFinance](https://x.com/StargateFinance) |
| X followers | 325.37K |
...
| Liquidity | $13.74M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 140.91M circulating vs 140.91M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Stargate Finance is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

Stargate Finance's architecture should be evaluated through the job it performs. For a bridge / cross-chain infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
| Settlement layer | Chains and contracts that hold user value | Arbitrum One, Avalanche, Base, Berachain, Binance Smart Chain, Ethereum, Fantom, Fuse, Gravity Alpha, Kaia, Kava, Linea, Mantle, Metis Andromeda, Optimistic Ethereum, Polygon Pos, Rari, Scroll, Taiko |
...
|---|---|---|---|
| base | STG | 0xe3b53af74a4bf62ae5511055290838050bf764df | [explorer](https://basescan.org/token/0xe3b53af74a4bf62ae5511055290838050bf764df) |
| Kava | STG | 0x83c30eb8bc9ad7c56532895840039e62659896ea | not mapped |
| linea | STG | 0x808d7c71ad2ba3fa531b068a2417c63106bc0949 | not mapped |
| fantom | STG | 0x2f6f07cdcf3588944bf4c42ac74ff24bf56e7590 | not mapped |
| Mantle | STG | 0x8731d54e9d02c286767d56ac03e8037c07e01e98 | not mapped |
| Scroll | STG | 0x8731d54e9d02c286767d56ac03e8037c07e01e98 | not mapped |
| ethereum | STG | 0xaf5191b0de278c7286d6c7cc6ab6bb8a73ba2cd6 | [explorer](https://etherscan.io/token/0xaf5191b0de278c7286d6c7cc6ab6bb8a73ba2cd6) |
| avalanche | STG | 0x2f6f07cdcf3588944bf4c42ac74ff24bf56e7590 | [explorer](https://snowtrace.io/token/0x2f6f07cdcf3588944bf4c42ac74ff24bf56e7590) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Stargate Finance, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 776, market cap $21,982,979, 24h volume $13,764,154. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $22.10M, FDV $22.10M, 24h volume $13.74M, and price $0.1568.
...
| 24h volume | $13.74M | Position sizing and exit liquidity proxy |
| Market cap | $22.10M | Liquid valuation proxy |
| FDV | $22.10M | Dilution-aware valuation proxy |
| FDV / market cap spread | 1.00x | Higher spread means more supply risk |
| X followers | 325.37K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Stargate Finance | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether STG is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. Stargate Finance should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that Stargate Finance belongs to the bridge / cross-chain infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $22.10M, FDV $22.10M, 24h volume $13.74M, price $0.1568, circulating supply 140.91M, and total supply 140.91M. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For Stargate Finance, the candidate backlog records Seeded 2026-07-06 from Surf market-ranking; rank 776, market cap $21,982,979, 24h volume $13,764,154. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Arbitrum One, Avalanche, Base, Berachain, Binance Smart Chain, Ethereum, Fantom, Fuse, Gravity Alpha, Kaia, Kava, Linea, Mantle, Metis Andromeda, Optimistic Ethereum, Polygon Pos, Rari, Scroll, Taiko, exchanges as BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET, and tags as Cross-Chain & Bridge. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Stargate Finance currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d -5.18% and 30d -23.10% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Stargate Finance's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-08-26 | M&A | not disclosed | LayerZero |
| 2022-03-18 | Ico | $30.00M | not disclosed |
...
Stargate Finance competes in the bridge / cross-chain infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Stargate Finance has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Stargate Finance is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Stargate Finance compounds usage, liquidity improves, and STG gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If STG does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $13.74M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Stargate Finance announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Stargate Finance has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Stargate Finance, the first reconciliation job is identity. The token symbol is STG, the category is bridge / cross-chain infrastructure, chains are Arbitrum One, Avalanche, Base, Berachain, Binance Smart Chain, Ethereum, Fantom, Fuse, Gravity Alpha, Kaia, Kava, Linea, Mantle, Metis Andromeda, Optimistic Ethereum, Polygon Pos, Rari, Scroll, Taiko, exchanges are BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET, and tags are Cross-Chain & Bridge. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If STG trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $22.10M, FDV is $22.10M, 24h volume is $13.74M, FDV / market cap is 1.00x, 7d price change is -5.18%, and 30d price change is -23.10%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Stargate Finance / STG | Match official docs, CG, CMC, Surf, contracts |
| Category | bridge / cross-chain infrastructure | Confirm product mechanics and revenue model |
| Chains | Arbitrum One, Avalanche, Base, Berachain, Binance Smart Chain, Ethereum, Fantom, Fuse, Gravity Alpha, Kaia, Kava, Linea, Mantle, Metis Andromeda, Optimistic Ethereum, Polygon Pos, Rari, Scroll, Taiko | Confirm canonical deployments and bridge assumptions |
| Exchanges | BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET | Check live order books and inactive pairs |
| Market cap | $22.10M | Compare Surf, CG, CMC, exchange pages |
| FDV | $22.10M | Reconcile total supply and unlock schedule |
| Volume | $13.74M | Check exchange concentration and wash-trading risk |
| TVL | not disclosed | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Stargate Finance does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Stargate Finance, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does STG sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
For Stargate Finance, the current liquidity read is watchlist first. The market cap is $22.10M, FDV is $22.10M, reported 24h volume is $13.74M, and FDV / market cap is 1.00x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. Stargate Finance can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.

For Stargate Finance, the live monitoring stack should include contract events, TVL, volume, governance, documentation changes, social announcements, security disclosures, and exchange-market quality. A single metric is not enough. TVL may stay high while liquidity thins. Volume may rise because of volatility rather than adoption. Social attention may rise around controversy rather than progress. Governance may pass proposals that improve short-term price but add long-term risk. The strongest monitoring dashboard combines on-chain state, off-chain disclosure, and market quality.

Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If Stargate Finance suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | Stargate Finance gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://stargate.finance/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Stargate%20Finance) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Stargate%20Finance) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Stargate%20Finance) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Stargate%20Finance) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Stargate%20Finance) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/StargateFinance) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/stargate-protocol) | Identity, market, social, contract, or discovery cross-check |
| 9 | Discord | [link](https://discord.gg/4t8CJXufWM) | Identity, market, social, contract, or discovery cross-check |
| 10 | Stargate Finance Price Chart (STG) | [link](https://www.coingecko.com/en/coins/stargate-finance) | Identity, market, social, contract, or discovery cross-check |
| 11 | Latest Stargate Finance (STG) Price Analysis | [link](https://coinmarketcap.com/cmc-ai/stargate-finance/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 12 | Stargate Finance (STG): Enabling Cross-Chain Asset ... | [link](https://www.coingecko.com/learn/stargate-finance-stg) | Identity, market, social, contract, or discovery cross-check |
| 13 | Stargate Finance (STG) / Tokenomics & Vesting Schedule | [link](https://tokenomist.ai/stargate-finance) | Identity, market, social, contract, or discovery cross-check |
| 14 | What Is Stargate Finance (STG) And How Does It Work? | [link](https://coinmarketcap.com/cmc-ai/stargate-finance/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 15 | base STG | [link](https://basescan.org/token/0xe3b53af74a4bf62ae5511055290838050bf764df) | Identity, market, social, contract, or discovery cross-check |
| 16 | ethereum STG | [link](https://etherscan.io/token/0xaf5191b0de278c7286d6c7cc6ab6bb8a73ba2cd6) | Identity, market, social, contract, or discovery cross-check |
| 17 | avalanche STG | [link](https://snowtrace.io/token/0x2f6f07cdcf3588944bf4c42ac74ff24bf56e7590) | Identity, market, social, contract, or discovery cross-check |
| 18 | polygon STG | [link](https://polygonscan.com/token/0x2f6f07cdcf3588944bf4c42ac74ff24bf56e7590) | Identity, market, social, contract, or discovery cross-check |
| 19 | arbitrum STG | [link](https://arbiscan.io/token/0x6694340fc020c5e6b96567843da2df01b2ce1eb6) | Identity, market, social, contract, or discovery cross-check |
| 20 | bsc STG | [link](https://bscscan.com/token/0xb0d502e938ed5f4df2e681fe6e419ff29631d62b) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google project search | [link](https://www.google.com/search?q=Stargate%20Finance%20STG%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google tokenomics search | [link](https://www.google.com/search?q=Stargate%20Finance%20STG%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 23 | Google docs search | [link](https://www.google.com/search?q=Stargate%20Finance%20docs) | Identity, market, social, contract, or discovery cross-check |
| 24 | Google governance search | [link](https://www.google.com/search?q=Stargate%20Finance%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 25 | Google audit search | [link](https://www.google.com/search?q=Stargate%20Finance%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 26 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/stargate-finance/) | Identity, market, social, contract, or discovery cross-check |
| 27 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Stargate%20Finance) | Identity, market, social, contract, or discovery cross-check |
| 28 | Dune token query | [link](https://dune.com/browse/queries?q=Stargate%20Finance) | Identity, market, social, contract, or discovery cross-check |
| 29 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Stargate%20Finance) | Identity, market, social, contract, or discovery cross-check |
| 30 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=STG) | Identity, market, social, contract, or discovery cross-check |
| 31 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Stargate%20Finance) | Identity, market, social, contract, or discovery cross-check |
| 32 | CryptoRank search | [link](https://cryptorank.io/price/stargate-finance) | Identity, market, social, contract, or discovery cross-check |
| 33 | RootData search | [link](https://www.rootdata.com/search?kw=Stargate%20Finance) | Identity, market, social, contract, or discovery cross-check |
| 34 | CoinDesk search | [link](https://www.coindesk.com/search?s=Stargate%20Finance) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://stargate.finance/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Stargate%20Finance)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Stargate%20Finance)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Stargate%20Finance)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Stargate%20Finance)
- [Messari discovery](https://messari.io/search?q=Stargate%20Finance)
- [X profile](https://x.com/StargateFinance)
- [GitHub](https://github.com/stargate-protocol)
- [Discord](https://discord.gg/4t8CJXufWM)
- [Stargate Finance Price Chart (STG)](https://www.coingecko.com/en/coins/stargate-finance)
- [Latest Stargate Finance (STG) Price Analysis](https://coinmarketcap.com/cmc-ai/stargate-finance/price-analysis/)
- [Stargate Finance (STG): Enabling Cross-Chain Asset ...](https://www.coingecko.com/learn/stargate-finance-stg)
- [Stargate Finance (STG) | Tokenomics & Vesting Schedule](https://tokenomist.ai/stargate-finance)
- [What Is Stargate Finance (STG) And How Does It Work?](https://coinmarketcap.com/cmc-ai/stargate-finance/what-is/)
- [base STG](https://basescan.org/token/0xe3b53af74a4bf62ae5511055290838050bf764df)
- [ethereum STG](https://etherscan.io/token/0xaf5191b0de278c7286d6c7cc6ab6bb8a73ba2cd6)
- [avalanche STG](https://snowtrace.io/token/0x2f6f07cdcf3588944bf4c42ac74ff24bf56e7590)
- [polygon STG](https://polygonscan.com/token/0x2f6f07cdcf3588944bf4c42ac74ff24bf56e7590)
- [arbitrum STG](https://arbiscan.io/token/0x6694340fc020c5e6b96567843da2df01b2ce1eb6)
- [bsc STG](https://bscscan.com/token/0xb0d502e938ed5f4df2e681fe6e419ff29631d62b)
- [Google project search](https://www.google.com/search?q=Stargate%20Finance%20STG%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Stargate%20Finance%20STG%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Stargate%20Finance%20docs)
- [Google governance search](https://www.google.com/search?q=Stargate%20Finance%20governance%20proposal)

## Final Investment View

Stargate Finance should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
