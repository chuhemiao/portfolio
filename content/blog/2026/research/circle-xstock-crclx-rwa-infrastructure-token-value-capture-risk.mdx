---
title: 'Circle xStock / CRCLX: RWA infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Circle xStock is treated as a RWA infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'circle-xstock-crclx-rwa-infrastructure-token-value-capture-risk'
category: 'research'
---

# Circle xStock / CRCLX: RWA infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Circle xStock appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Arbitrum One, Binance Smart Chain, Ethereum, Ink, Mantle, Solana, Ton, Tron; exchange exposure is not disclosed; category tags are RWA (Real World Assets); current market cap is $43.39M; FDV is $208.52M; and 24h volume is $9.28M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Circle xStock before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 501, market cap $42,855,507, 24h volume $8,767,251. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Circle xStock is best understood as a **RWA infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether CRCLX captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Arbitrum One, Binance Smart Chain, Ethereum, Ink, Mantle, Solana, Ton, Tron |
| Exchanges | not disclosed |
| Price | $69.2300 |
| Market cap | $43.39M |
| FDV | $208.52M |
| 24h volume | $9.28M |
| Circulating supply | 626.75K |
| Total supply | 3.01M |
| 7d price change | -6.54% |
...
Base case: Circle xStock remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning CRCLX into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

xStocks are tokenized U.S. stocks and ETFs issued by Backed (Backed Finance), minted 1:1 against underlying assets and designed to be DeFi‑compatible and freely transferable. They enable 24/7 on‑chain trading and composability across exchanges and chains (e.g., Kraken, Solana) for global access to equities.

Surf's project profile maps Circle xStock to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://assets.backed.fi/products/coinbase-xstock](https://assets.backed.fi/products/coinbase-xstock) |
| X handle | [@xStocksFi](https://x.com/xStocksFi) |
| X followers | 77.26K |
| Tags | RWA (Real World Assets) |
| Chains | Arbitrum One, Binance Smart Chain, Ethereum, Ink, Mantle, Solana, Ton, Tron |
| Exchanges | not disclosed |
| TGE status | not disclosed |
| CoinGecko ID | not disclosed |

The main product question is whether Circle xStock is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For RWA infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Circle xStock convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $9.28M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 626.75K circulating vs 3.01M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Circle xStock is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

Circle xStock's architecture should be evaluated through the job it performs. For a RWA infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
| Settlement layer | Chains and contracts that hold user value | Arbitrum One, Binance Smart Chain, Ethereum, Ink, Mantle, Solana, Ton, Tron |
...
|---|---|---|---|
| arbitrum | CRCLX | 0xfebded1b0986a8ee107f5ab1a1c5a813491deceb | [explorer](https://arbiscan.io/token/0xfebded1b0986a8ee107f5ab1a1c5a813491deceb) |
| solana | CRCLX | XsueG8BtpquVJX9LVLLEGuViXUungE6WmK5YZ3p3bd1 | [explorer](https://solscan.io/token/XsueG8BtpquVJX9LVLLEGuViXUungE6WmK5YZ3p3bd1) |
| ethereum | CRCLX | 0xfebded1b0986a8ee107f5ab1a1c5a813491deceb | [explorer](https://etherscan.io/token/0xfebded1b0986a8ee107f5ab1a1c5a813491deceb) |
| bsc | CRCLX | 0xfebded1b0986a8ee107f5ab1a1c5a813491deceb | [explorer](https://bscscan.com/token/0xfebded1b0986a8ee107f5ab1a1c5a813491deceb) |
| arbitrum | TSLAX | 0x8ad3c73f833d3f9a523ab01476625f269aeb7cf0 | [explorer](https://arbiscan.io/token/0x8ad3c73f833d3f9a523ab01476625f269aeb7cf0) |
| Mantle | TSLAX | 0x8ad3c73f833d3f9a523ab01476625f269aeb7cf0 | not mapped |
| solana | TSLAX | XsDoVfqeBukxuZHWhdvWHBhgEHjGNst4MLodqsJHzoB | [explorer](https://solscan.io/token/XsDoVfqeBukxuZHWhdvWHBhgEHjGNst4MLodqsJHzoB) |
| ethereum | TSLAX | 0x8ad3c73f833d3f9a523ab01476625f269aeb7cf0 | [explorer](https://etherscan.io/token/0x8ad3c73f833d3f9a523ab01476625f269aeb7cf0) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Circle xStock, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 501, market cap $42,855,507, 24h volume $8,767,251. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $43.39M, FDV $208.52M, 24h volume $9.28M, and price $69.2300.
...
| 24h volume | $9.28M | Position sizing and exit liquidity proxy |
| Market cap | $43.39M | Liquid valuation proxy |
| FDV | $208.52M | Dilution-aware valuation proxy |
| FDV / market cap spread | 4.81x | Higher spread means more supply risk |
| X followers | 77.26K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Circle xStock | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether CRCLX is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Circle xStock currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d -6.54% and 30d -11.30% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Circle xStock's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-07-29 | Ipo | $2.47B | not disclosed |
| 2025-03-21 | Post-Ipo | $711.00M | not disclosed |
| 2024-11-21 | Post-Ipo Debt | $3.00B | not disclosed |
| 2024-09-20 | Post-Ipo Debt | $1.01B | not disclosed |
| 2023-02-17 | Post-Ipo | $46.60M | not disclosed |
| 2022-03-29 | Debt Financing | $205.00M | Silvergate Bank |
...
Circle xStock competes in the RWA infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Circle xStock has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Circle xStock is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Circle xStock compounds usage, liquidity improves, and CRCLX gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If CRCLX does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $9.28M | Weekly | Volume below position-size threshold |
| FDV / market cap | 4.81x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Circle xStock announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Circle xStock has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Circle xStock, the first reconciliation job is identity. The token symbol is CRCLX, the category is RWA infrastructure, chains are Arbitrum One, Binance Smart Chain, Ethereum, Ink, Mantle, Solana, Ton, Tron, exchanges are not disclosed, and tags are RWA (Real World Assets). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If CRCLX trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $43.39M, FDV is $208.52M, 24h volume is $9.28M, FDV / market cap is 4.81x, 7d price change is -6.54%, and 30d price change is -11.30%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Circle xStock / CRCLX | Match official docs, CG, CMC, Surf, contracts |
| Category | RWA infrastructure | Confirm product mechanics and revenue model |
| Chains | Arbitrum One, Binance Smart Chain, Ethereum, Ink, Mantle, Solana, Ton, Tron | Confirm canonical deployments and bridge assumptions |
| Exchanges | not disclosed | Check live order books and inactive pairs |
| Market cap | $43.39M | Compare Surf, CG, CMC, exchange pages |
| FDV | $208.52M | Reconcile total supply and unlock schedule |
| Volume | $9.28M | Check exchange concentration and wash-trading risk |
| TVL | not disclosed | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Circle xStock does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Circle xStock, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does CRCLX sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
For Circle xStock, the current liquidity read is watchlist first. The market cap is $43.39M, FDV is $208.52M, reported 24h volume is $9.28M, and FDV / market cap is 4.81x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. Circle xStock can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.

For Circle xStock, the live monitoring stack should include contract events, TVL, volume, governance, documentation changes, social announcements, security disclosures, and exchange-market quality. A single metric is not enough. TVL may stay high while liquidity thins. Volume may rise because of volatility rather than adoption. Social attention may rise around controversy rather than progress. Governance may pass proposals that improve short-term price but add long-term risk. The strongest monitoring dashboard combines on-chain state, off-chain disclosure, and market quality.

Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If Circle xStock suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | Circle xStock gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://assets.backed.fi/products/coinbase-xstock) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Circle%20xStock) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Circle%20xStock) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Circle%20xStock) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Circle%20xStock) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Circle%20xStock) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/xStocksFi) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/xstocksfi) | Identity, market, social, contract, or discovery cross-check |
| 9 | Circle xStock Price Chart (CRCLX) - CoinGecko | [link](https://www.coingecko.com/en/coins/circle-xstock) | Identity, market, social, contract, or discovery cross-check |
| 10 | Latest Circle tokenized stock (xStock) (CRCLX) Price Analysis | [link](https://coinmarketcap.com/cmc-ai/circle-tokenized-stock-xstock/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 11 | What is Circle tokenized stock (xStock) (CRCLX)? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/circle-tokenized-stock-xstock/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 12 | CRCLX to INR: Circle xStock Price in Indian Rupee - CoinGecko | [link](https://www.coingecko.com/en/coins/circle-xstock/inr) | Identity, market, social, contract, or discovery cross-check |
| 13 | Circle xStock - CRCLx - Backed Assets - Backed Finance | [link](https://assets.backed.fi/products/circle-xstock) | Identity, market, social, contract, or discovery cross-check |
| 14 | arbitrum CRCLX | [link](https://arbiscan.io/token/0xfebded1b0986a8ee107f5ab1a1c5a813491deceb) | Identity, market, social, contract, or discovery cross-check |
| 15 | solana CRCLX | [link](https://solscan.io/token/XsueG8BtpquVJX9LVLLEGuViXUungE6WmK5YZ3p3bd1) | Identity, market, social, contract, or discovery cross-check |
| 16 | ethereum CRCLX | [link](https://etherscan.io/token/0xfebded1b0986a8ee107f5ab1a1c5a813491deceb) | Identity, market, social, contract, or discovery cross-check |
| 17 | bsc CRCLX | [link](https://bscscan.com/token/0xfebded1b0986a8ee107f5ab1a1c5a813491deceb) | Identity, market, social, contract, or discovery cross-check |
| 18 | arbitrum TSLAX | [link](https://arbiscan.io/token/0x8ad3c73f833d3f9a523ab01476625f269aeb7cf0) | Identity, market, social, contract, or discovery cross-check |
| 19 | solana TSLAX | [link](https://solscan.io/token/XsDoVfqeBukxuZHWhdvWHBhgEHjGNst4MLodqsJHzoB) | Identity, market, social, contract, or discovery cross-check |
| 20 | ethereum TSLAX | [link](https://etherscan.io/token/0x8ad3c73f833d3f9a523ab01476625f269aeb7cf0) | Identity, market, social, contract, or discovery cross-check |
| 21 | bsc TSLAX | [link](https://bscscan.com/token/0x8ad3c73f833d3f9a523ab01476625f269aeb7cf0) | Identity, market, social, contract, or discovery cross-check |
| 22 | arbitrum MSTRX | [link](https://arbiscan.io/token/0xae2f842ef90c0d5213259ab82639d5bbf649b08e) | Identity, market, social, contract, or discovery cross-check |
| 23 | solana MSTRX | [link](https://solscan.io/token/XsP7xzNPvEHS1m6qfanPUGjNmdnmsLKEoNAnHjdxxyZ) | Identity, market, social, contract, or discovery cross-check |
| 24 | Google project search | [link](https://www.google.com/search?q=Circle%20xStock%20CRCLX%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 25 | Google tokenomics search | [link](https://www.google.com/search?q=Circle%20xStock%20CRCLX%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 26 | Google docs search | [link](https://www.google.com/search?q=Circle%20xStock%20docs) | Identity, market, social, contract, or discovery cross-check |
| 27 | Google governance search | [link](https://www.google.com/search?q=Circle%20xStock%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 28 | Google audit search | [link](https://www.google.com/search?q=Circle%20xStock%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 29 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/circle-xstock/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 30 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/circle-xstock/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 31 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/circle-xstock/) | Identity, market, social, contract, or discovery cross-check |
| 32 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Circle%20xStock) | Identity, market, social, contract, or discovery cross-check |
| 33 | Dune token query | [link](https://dune.com/browse/queries?q=Circle%20xStock) | Identity, market, social, contract, or discovery cross-check |
| 34 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Circle%20xStock) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://assets.backed.fi/products/coinbase-xstock)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Circle%20xStock)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Circle%20xStock)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Circle%20xStock)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Circle%20xStock)
- [Messari discovery](https://messari.io/search?q=Circle%20xStock)
- [X profile](https://x.com/xStocksFi)
- [Telegram](https://t.me/xstocksfi)
- [Circle xStock Price Chart (CRCLX) - CoinGecko](https://www.coingecko.com/en/coins/circle-xstock)
- [Latest Circle tokenized stock (xStock) (CRCLX) Price Analysis](https://coinmarketcap.com/cmc-ai/circle-tokenized-stock-xstock/price-analysis/)
- [What is Circle tokenized stock (xStock) (CRCLX)? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/circle-tokenized-stock-xstock/what-is/)
- [CRCLX to INR: Circle xStock Price in Indian Rupee - CoinGecko](https://www.coingecko.com/en/coins/circle-xstock/inr)
- [Circle xStock - CRCLx - Backed Assets - Backed Finance](https://assets.backed.fi/products/circle-xstock)
- [arbitrum CRCLX](https://arbiscan.io/token/0xfebded1b0986a8ee107f5ab1a1c5a813491deceb)
- [solana CRCLX](https://solscan.io/token/XsueG8BtpquVJX9LVLLEGuViXUungE6WmK5YZ3p3bd1)
- [ethereum CRCLX](https://etherscan.io/token/0xfebded1b0986a8ee107f5ab1a1c5a813491deceb)
- [bsc CRCLX](https://bscscan.com/token/0xfebded1b0986a8ee107f5ab1a1c5a813491deceb)
- [arbitrum TSLAX](https://arbiscan.io/token/0x8ad3c73f833d3f9a523ab01476625f269aeb7cf0)
- [solana TSLAX](https://solscan.io/token/XsDoVfqeBukxuZHWhdvWHBhgEHjGNst4MLodqsJHzoB)
- [ethereum TSLAX](https://etherscan.io/token/0x8ad3c73f833d3f9a523ab01476625f269aeb7cf0)
- [bsc TSLAX](https://bscscan.com/token/0x8ad3c73f833d3f9a523ab01476625f269aeb7cf0)
- [arbitrum MSTRX](https://arbiscan.io/token/0xae2f842ef90c0d5213259ab82639d5bbf649b08e)
- [solana MSTRX](https://solscan.io/token/XsP7xzNPvEHS1m6qfanPUGjNmdnmsLKEoNAnHjdxxyZ)
- [Google project search](https://www.google.com/search?q=Circle%20xStock%20CRCLX%20crypto)

## Final Investment View

Circle xStock should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
