---
title: 'Manta Pacific / MANTA: privacy infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Manta Pacific is treated as a privacy infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'manta-pacific-manta-privacy-infrastructure-token-value-capture-risk'
category: 'research'
---

# Manta Pacific / MANTA: privacy infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Manta Pacific appears in the local CoinGecko/CMC-oriented candidate pool through [CoinGecko](https://www.coingecko.com/en/coins/manta-network) and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Manta Pacific; exchange exposure is BINANCE, BITHUMB, BYBIT, BITGET; category tags are Layer2 & Scaling, Modular & DA, Privacy & ZK; current market cap is $28.98M; FDV is $60.87M; and 24h volume is $10.58M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Manta Pacific before this file was created, and the existing candidate note says: DefiLlama category Canonical Bridge on Ethereum; TVL $19,600,534. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Manta Pacific is best understood as a **privacy infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether MANTA captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Exchanges | BINANCE, BITHUMB, BYBIT, BITGET |
| Price | $0.0609 |
| Market cap | $28.98M |
| FDV | $60.87M |
| 24h volume | $10.58M |
| Circulating supply | 476.08M |
| Total supply | 1.00B |
| 7d price change | -26.76% |
...
Base case: Manta Pacific remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning MANTA into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Manta Network is a multi-modular ecosystem that builds zero-knowledge (ZK) application infrastructure, operating Manta Pacific (an EVM-equivalent ZK L2 on Ethereum) and Manta Atlantic (a high-speed ZK L1 on Polkadot for programmable privacy and zkSBTs). It enables scalable, low-cost, privacy-preserving dApps with developer-friendly EVM compatibility.

Surf's project profile maps Manta Pacific to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://manta.network/](https://manta.network/) |
| X handle | [@MantaNetwork](https://x.com/MantaNetwork) |
| X followers | 746.06K |
| Tags | Layer2 & Scaling, Modular & DA, Privacy & ZK |
| Chains | Manta Pacific |
| Exchanges | BINANCE, BITHUMB, BYBIT, BITGET |
| TGE status | post |
| CoinGecko ID | [manta-network](https://www.coingecko.com/en/coins/manta-network) |

The main product question is whether Manta Pacific is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For privacy infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Manta Pacific convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $10.58M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 476.08M circulating vs 1.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Manta Pacific is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

Manta Pacific's architecture should be evaluated through the job it performs. For a privacy infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
| Settlement layer | Chains and contracts that hold user value | Manta Pacific |
...
|---|---|---|---|
| Manta Pacific | MANTA | 0x95cef13441be50d20ca4558cc0a27b601ac544e5 | not mapped |
| Manta Pacific | WETH | 0x0dc808adce2099a9f62aa87d9670745aba741746 | not mapped |
| Manta Pacific | WSTETH | 0x2fe3ad97a60eb7c79a976fc18bb5ffd07dd94ba5 | not mapped |
| Manta Pacific | USDT | 0xf417f5a458ec102b90352f697d6e2ac3a3d2851f | not mapped |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Manta Pacific, the DeFi-oriented backlog note is the first signal: DefiLlama category Canonical Bridge on Ethereum; TVL $19,600,534. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $28.98M, FDV $60.87M, 24h volume $10.58M, and price $0.0609.
...
| 24h volume | $10.58M | Position sizing and exit liquidity proxy |
| Market cap | $28.98M | Liquid valuation proxy |
| FDV | $60.87M | Dilution-aware valuation proxy |
| FDV / market cap spread | 2.10x | Higher spread means more supply risk |
| X followers | 746.06K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Manta Pacific | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether MANTA is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. Manta Pacific should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that Manta Pacific belongs to the privacy infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $28.98M, FDV $60.87M, 24h volume $10.58M, price $0.0609, circulating supply 476.08M, and total supply 1.00B. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For Manta Pacific, the candidate backlog records DefiLlama category Canonical Bridge on Ethereum; TVL $19,600,534. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Manta Pacific, exchanges as BINANCE, BITHUMB, BYBIT, BITGET, and tags as Layer2 & Scaling, Modular & DA, Privacy & ZK. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Manta Pacific currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d -26.76% and 30d -25.21% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Manta Pacific's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2023-07-19 | Series A | $25.00M | Polychain Capital, CoinFund, SevenX Ventures, Alliance DAO, Qiming Venture Partners |
| 2021-11-10 | Ico | $28.71M | not disclosed |
| 2021-10-19 | Undisclosed | $5.50M | Divergence Ventures, CMS Holdings, The Spartan Group, CoinFund, LongHash Ventures, SkyVision Capital, Zee Prime Capital, ParaFi Capital |
| 2021-02-02 | Seed | $1.10M | Alameda Research, Three Arrows Capital, Multicoin Capital, Polychain Capital, AU21 Capital, DeFiance Capital, Genblock Capital, Hypersphere Ventures |

| Team member | Role | Link |
|---|---|---|
| Victor Ji | Co-Founder, Core Contributor | [X](https://twitter.com/victorji15) |
| Kenny Li | Co-Founder, Core Contributor, COO | [X](https://twitter.com/superanonymousk) |
| Shumo Chu | Co-Founder, Core Contributor | [X](https://twitter.com/shumochu) |
...
Manta Pacific competes in the privacy infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Manta Pacific has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Manta Pacific is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Manta Pacific compounds usage, liquidity improves, and MANTA gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If MANTA does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $10.58M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Manta Pacific announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Manta Pacific has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Manta Pacific, the first reconciliation job is identity. The token symbol is MANTA, the category is privacy infrastructure, chains are Manta Pacific, exchanges are BINANCE, BITHUMB, BYBIT, BITGET, and tags are Layer2 & Scaling, Modular & DA, Privacy & ZK. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If MANTA trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $28.98M, FDV is $60.87M, 24h volume is $10.58M, FDV / market cap is 2.10x, 7d price change is -26.76%, and 30d price change is -25.21%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
The failure-mode view is deliberately harsher than the base case. Manta Pacific can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.

For Manta Pacific, the live monitoring stack should include contract events, TVL, volume, governance, documentation changes, social announcements, security disclosures, and exchange-market quality. A single metric is not enough. TVL may stay high while liquidity thins. Volume may rise because of volatility rather than adoption. Social attention may rise around controversy rather than progress. Governance may pass proposals that improve short-term price but add long-term risk. The strongest monitoring dashboard combines on-chain state, off-chain disclosure, and market quality.

Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If Manta Pacific suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | Manta Pacific gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://manta.network/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko profile | [link](https://www.coingecko.com/en/coins/manta-network) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Manta%20Pacific) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Manta%20Pacific) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Manta%20Pacific) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Manta%20Pacific) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/MantaNetwork) | Identity, market, social, contract, or discovery cross-check |
| 8 | Telegram | [link](https://t.me/mantanetworkofficial) | Identity, market, social, contract, or discovery cross-check |
| 9 | Manta Network: The Multi-Modular Ecosystem for ZK Apps | [link](https://www.coingecko.com/learn/what-is-manta-crypto) | Identity, market, social, contract, or discovery cross-check |
| 10 | Manta Pacific Bridged USDT (Manta Pacific) USDT Price - CoinGecko | [link](https://www.coingecko.com/en/coins/manta-pacific-bridged-usdt-manta-pacific) | Identity, market, social, contract, or discovery cross-check |
| 11 | Manta Pacific Blockchain: Coins, NFTs, Exchanges & More | [link](https://www.coingecko.com/en/chains/manta) | Identity, market, social, contract, or discovery cross-check |
| 12 | $MANTA Tokenomics - Manta Network - Medium | [link](https://mantanetwork.medium.com/manta-tokenomics-b226f911c84c) | Identity, market, social, contract, or discovery cross-check |
| 13 | Google project search | [link](https://www.google.com/search?q=Manta%20Pacific%20MANTA%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 14 | Google tokenomics search | [link](https://www.google.com/search?q=Manta%20Pacific%20MANTA%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 15 | Google docs search | [link](https://www.google.com/search?q=Manta%20Pacific%20docs) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google governance search | [link](https://www.google.com/search?q=Manta%20Pacific%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google audit search | [link](https://www.google.com/search?q=Manta%20Pacific%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 18 | CoinGecko historical data | [link](https://www.coingecko.com/en/coins/manta-network/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 19 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/manta-pacific/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 20 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/manta-pacific/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/manta-pacific/) | Identity, market, social, contract, or discovery cross-check |
| 22 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Manta%20Pacific) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune token query | [link](https://dune.com/browse/queries?q=Manta%20Pacific) | Identity, market, social, contract, or discovery cross-check |
| 24 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Manta%20Pacific) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=MANTA) | Identity, market, social, contract, or discovery cross-check |
| 26 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Manta%20Pacific) | Identity, market, social, contract, or discovery cross-check |
| 27 | CryptoRank search | [link](https://cryptorank.io/price/manta-pacific) | Identity, market, social, contract, or discovery cross-check |
| 28 | RootData search | [link](https://www.rootdata.com/search?kw=Manta%20Pacific) | Identity, market, social, contract, or discovery cross-check |
| 29 | CoinDesk search | [link](https://www.coindesk.com/search?s=Manta%20Pacific) | Identity, market, social, contract, or discovery cross-check |
| 30 | The Block search | [link](https://www.theblock.co/search?query=Manta%20Pacific) | Identity, market, social, contract, or discovery cross-check |
| 31 | GitHub search | [link](https://github.com/search?q=Manta%20Pacific&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 32 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=MANTA) | Identity, market, social, contract, or discovery cross-check |
| 33 | BscScan token search | [link](https://bscscan.com/search?f=0&q=MANTA) | Identity, market, social, contract, or discovery cross-check |
| 34 | Arbiscan token search | [link](https://arbiscan.io/search?f=0&q=MANTA) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://manta.network/)
- [CoinGecko profile](https://www.coingecko.com/en/coins/manta-network)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Manta%20Pacific)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Manta%20Pacific)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Manta%20Pacific)
- [Messari discovery](https://messari.io/search?q=Manta%20Pacific)
- [X profile](https://x.com/MantaNetwork)
- [Telegram](https://t.me/mantanetworkofficial)
- [Manta Network: The Multi-Modular Ecosystem for ZK Apps](https://www.coingecko.com/learn/what-is-manta-crypto)
- [Manta Pacific Bridged USDT (Manta Pacific) USDT Price - CoinGecko](https://www.coingecko.com/en/coins/manta-pacific-bridged-usdt-manta-pacific)
- [Manta Pacific Blockchain: Coins, NFTs, Exchanges & More](https://www.coingecko.com/en/chains/manta)
- [$MANTA Tokenomics - Manta Network - Medium](https://mantanetwork.medium.com/manta-tokenomics-b226f911c84c)
- [Google project search](https://www.google.com/search?q=Manta%20Pacific%20MANTA%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Manta%20Pacific%20MANTA%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Manta%20Pacific%20docs)
- [Google governance search](https://www.google.com/search?q=Manta%20Pacific%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Manta%20Pacific%20audit%20report)
- [CoinGecko historical data](https://www.coingecko.com/en/coins/manta-network/historical_data)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/manta-pacific/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/manta-pacific/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/manta-pacific/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Manta%20Pacific)
- [Dune token query](https://dune.com/browse/queries?q=Manta%20Pacific)
- [DeFiLlama fees discovery](https://defillama.com/fees?chain=All&search=Manta%20Pacific)

## Final Investment View

Manta Pacific should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
