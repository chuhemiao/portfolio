---
title: 'Treehouse Protocol / TREE: Fixed income / DOR infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Treehouse Protocol is treated as a Fixed income / DOR infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'treehouse-protocol-tree-fixed-income-dor-infrastructure-token-value-capture-risk'
category: 'research'
---

# Treehouse Protocol / TREE: Fixed income / DOR infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Treehouse Protocol appears in the local CoinGecko/CMC-oriented candidate pool through [CoinGecko](https://www.coingecko.com/en/coins/treehouse) and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Arbitrum One, Avalanche, Base, Binance Smart Chain, Ethereum; exchange exposure is BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET; category tags are Yield & Asset Mgmt, LSD & Restaking, Oracle; current market cap is $6.61M; FDV is $42.33M; and 24h volume is $3.59M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Treehouse Protocol before this file was created, and the existing candidate note says: DefiLlama category DOR on Multi-Chain; TVL $52,436,115. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Treehouse Protocol is best understood as a **Fixed income / DOR infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether TREE captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.

| Metric | Current read |
|---|---|
| Project | Treehouse Protocol |
| Token | TREE |
| Category | Fixed income / DOR infrastructure |
| Chains | Arbitrum One, Avalanche, Base, Binance Smart Chain, Ethereum |
| Exchanges | BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET |
| Price | $0.0423 |
| Market cap | $6.61M |
| FDV | $42.33M |
| 24h volume | $3.59M |
| Circulating supply | 156.12M |
| Total supply | 1.00B |
...
Base case: Treehouse Protocol remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning TREE into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Decentralized fixed income layer building tAssets (liquid staking yield products) and DOR (on-chain benchmark rates) for DeFi.

Surf's project profile maps Treehouse Protocol to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.treehouse.finance/](https://www.treehouse.finance/) |
| X handle | [@TreehouseFi](https://x.com/TreehouseFi) |
| X followers | 185.76K |
| Tags | Yield & Asset Mgmt, LSD & Restaking, Oracle |
| Chains | Arbitrum One, Avalanche, Base, Binance Smart Chain, Ethereum |
| Exchanges | BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET |
| TGE status | post |
| CoinGecko ID | [treehouse](https://www.coingecko.com/en/coins/treehouse) |

The main product question is whether Treehouse Protocol is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For Fixed income / DOR infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Treehouse Protocol convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $3.59M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 156.12M circulating vs 1.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Treehouse Protocol is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

Treehouse Protocol's architecture should be evaluated through the job it performs. For a Fixed income / DOR infrastructure project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
| Settlement layer | Chains and contracts that hold user value | Arbitrum One, Avalanche, Base, Binance Smart Chain, Ethereum |
...
|---|---|---|---|
| ethereum | TREE | 0x77146784315ba81904d654466968e3a7c196d1f3 | [explorer](https://etherscan.io/token/0x77146784315ba81904d654466968e3a7c196d1f3) |
| bsc | TREE | 0x77146784315ba81904d654466968e3a7c196d1f3 | [explorer](https://bscscan.com/token/0x77146784315ba81904d654466968e3a7c196d1f3) |
| avalanche | TAVAX | 0x14a84f1a61ccd7d1be596a6cc11fe33a36bc1646 | [explorer](https://snowtrace.io/token/0x14a84f1a61ccd7d1be596a6cc11fe33a36bc1646) |
| base | TETH | 0xd09acb80c1e8f2291862c4978a008791c9167003 | [explorer](https://basescan.org/token/0xd09acb80c1e8f2291862c4978a008791c9167003) |
| ethereum | TETH | 0xd11c452fc99cf405034ee446803b6f6c1f6d5ed8 | [explorer](https://etherscan.io/token/0xd11c452fc99cf405034ee446803b6f6c1f6d5ed8) |
| arbitrum | TETH | 0xd09acb80c1e8f2291862c4978a008791c9167003 | [explorer](https://arbiscan.io/token/0xd09acb80c1e8f2291862c4978a008791c9167003) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Treehouse Protocol, the DeFi-oriented backlog note is the first signal: DefiLlama category DOR on Multi-Chain; TVL $52,436,115. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $6.61M, FDV $42.33M, 24h volume $3.59M, and price $0.0423.
...
| 24h volume | $3.59M | Position sizing and exit liquidity proxy |
| Market cap | $6.61M | Liquid valuation proxy |
| FDV | $42.33M | Dilution-aware valuation proxy |
| FDV / market cap spread | 6.41x | Higher spread means more supply risk |
| X followers | 185.76K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Treehouse Protocol | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether TREE is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. Treehouse Protocol should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that Treehouse Protocol belongs to the Fixed income / DOR infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $6.61M, FDV $42.33M, 24h volume $3.59M, price $0.0423, circulating supply 156.12M, and total supply 1.00B. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For Treehouse Protocol, the candidate backlog records DefiLlama category DOR on Multi-Chain; TVL $52,436,115. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Arbitrum One, Avalanche, Base, Binance Smart Chain, Ethereum, exchanges as BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET, and tags as Yield & Asset Mgmt, LSD & Restaking, Oracle. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.

The fourth frame is value capture. A Fixed income / DOR infrastructure asset can be useful while the token remains weak. The memo therefore separates product-market fit from token-market fit. Product-market fit asks whether users need the protocol even when incentives fade. Token-market fit asks whether the token absorbs value through staking, fee routing, collateral, governance constraints, emissions control, slashing insurance, or privileged access. If the token is mostly governance, the base case assigns lower confidence. If the token is used for security or fee capture but the mechanism is not yet activated, the base case treats value capture as an option rather than as current cash flow.

The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Treehouse Protocol currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 0.91% and 30d -4.29% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Treehouse Protocol's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-04-15 | Undisclosed | not disclosed | Matthew Tan, Richmond Teo, Jordi Alexander, Guy Young, Michael Ashby, Darius Sit |
| 2024-12-18 | Grant | not disclosed | Compound Labs |
| 2022-03-16 | Seed | $18.00M | Jump Capital, YZi Labs (Prev. Binance Labs), Mirana Ventures, Fundamental Labs, GSR, Lightspeed, Wintermute, Bybit |
| 2021-09-15 | Pre Seed | $2.40M | Global Founders Capital, Coinhako, The Brooker Group, CRC Capital |

| Team member | Role | Link |
|---|---|---|
| Brandon Goh | Co-Founder, CEO | not disclosed |
| Kang Loh | Co-Founder, CSO | not disclosed |
| Bryan Goh | Co-Founder | [X](https://x.com/bgohroo) |
...
Treehouse Protocol competes in the Fixed income / DOR infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Treehouse Protocol has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Treehouse Protocol is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Treehouse Protocol compounds usage, liquidity improves, and TREE gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If TREE does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $3.59M | Weekly | Volume below position-size threshold |
| FDV / market cap | 6.41x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Treehouse Protocol announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Treehouse Protocol has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Treehouse Protocol, the first reconciliation job is identity. The token symbol is TREE, the category is Fixed income / DOR infrastructure, chains are Arbitrum One, Avalanche, Base, Binance Smart Chain, Ethereum, exchanges are BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET, and tags are Yield & Asset Mgmt, LSD & Restaking, Oracle. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If TREE trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $6.61M, FDV is $42.33M, 24h volume is $3.59M, FDV / market cap is 6.41x, 7d price change is 0.91%, and 30d price change is -4.29%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.

The third reconciliation job is usage data. The latest TVL sample is not disclosed, TVL sample change is not disclosed, and the sample contains 0 point(s). TVL is useful only when the underlying balances are real, withdrawable, and not double-counted. For Fixed income / DOR infrastructure, the memo should identify whether TVL means deposits, staked assets, LP liquidity, stablecoin supply, collateral, locked governance assets, or a derived wrapper. A yield-bearing stablecoin and a validator middleware protocol can both show TVL, but the risk encoded by that TVL is completely different.

| Reconciliation item | Current value | Next check |
|---|---|---|
| Identity | Treehouse Protocol / TREE | Match official docs, CG, CMC, Surf, contracts |
| Category | Fixed income / DOR infrastructure | Confirm product mechanics and revenue model |
| Chains | Arbitrum One, Avalanche, Base, Binance Smart Chain, Ethereum | Confirm canonical deployments and bridge assumptions |
| Exchanges | BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET | Check live order books and inactive pairs |
| Market cap | $6.61M | Compare Surf, CG, CMC, exchange pages |
| FDV | $42.33M | Reconcile total supply and unlock schedule |
| Volume | $3.59M | Check exchange concentration and wash-trading risk |
| TVL | not disclosed | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Treehouse Protocol does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Treehouse Protocol, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does TREE sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
| Correlation check | Count exposure to related Fixed income / DOR infrastructure assets | Category beta can hide concentration |
...
| Peer comparison | Monthly | Treehouse Protocol gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.treehouse.finance/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko profile | [link](https://www.coingecko.com/en/coins/treehouse) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Treehouse%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Treehouse%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Treehouse%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Treehouse%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/TreehouseFi) | Identity, market, social, contract, or discovery cross-check |
| 8 | Discord | [link](https://discord.com/invite/treehousefi) | Identity, market, social, contract, or discovery cross-check |
| 9 | Treehouse price today, TREE to USD live price, marketcap and chart | [link](https://coinmarketcap.com/currencies/treehouse/) | Identity, market, social, contract, or discovery cross-check |
| 10 | Treehouse ETH Price Chart (TETH) - CoinGecko | [link](https://www.coingecko.com/en/coins/treehouse-eth) | Identity, market, social, contract, or discovery cross-check |
| 11 | TREE/USD Live Price Chart, Market Cap & News Today - CoinGecko | [link](https://www.coingecko.com/en/coins/tree) | Identity, market, social, contract, or discovery cross-check |
| 12 | What Is Treehouse (TREE) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/treehouse/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 13 | ethereum TREE | [link](https://etherscan.io/token/0x77146784315ba81904d654466968e3a7c196d1f3) | Identity, market, social, contract, or discovery cross-check |
| 14 | bsc TREE | [link](https://bscscan.com/token/0x77146784315ba81904d654466968e3a7c196d1f3) | Identity, market, social, contract, or discovery cross-check |
| 15 | avalanche TAVAX | [link](https://snowtrace.io/token/0x14a84f1a61ccd7d1be596a6cc11fe33a36bc1646) | Identity, market, social, contract, or discovery cross-check |
| 16 | base TETH | [link](https://basescan.org/token/0xd09acb80c1e8f2291862c4978a008791c9167003) | Identity, market, social, contract, or discovery cross-check |
| 17 | ethereum TETH | [link](https://etherscan.io/token/0xd11c452fc99cf405034ee446803b6f6c1f6d5ed8) | Identity, market, social, contract, or discovery cross-check |
| 18 | arbitrum TETH | [link](https://arbiscan.io/token/0xd09acb80c1e8f2291862c4978a008791c9167003) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google project search | [link](https://www.google.com/search?q=Treehouse%20Protocol%20TREE%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google tokenomics search | [link](https://www.google.com/search?q=Treehouse%20Protocol%20TREE%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google docs search | [link](https://www.google.com/search?q=Treehouse%20Protocol%20docs) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google governance search | [link](https://www.google.com/search?q=Treehouse%20Protocol%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 23 | Google audit search | [link](https://www.google.com/search?q=Treehouse%20Protocol%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinGecko historical data | [link](https://www.coingecko.com/en/coins/treehouse/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 25 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/treehouse-protocol/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 26 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/treehouse-protocol/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 27 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/treehouse-protocol/) | Identity, market, social, contract, or discovery cross-check |
| 28 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Treehouse%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 29 | Dune token query | [link](https://dune.com/browse/queries?q=Treehouse%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 30 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Treehouse%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 31 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=TREE) | Identity, market, social, contract, or discovery cross-check |
| 32 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Treehouse%20Protocol) | Identity, market, social, contract, or discovery cross-check |
| 33 | CryptoRank search | [link](https://cryptorank.io/price/treehouse-protocol) | Identity, market, social, contract, or discovery cross-check |
| 34 | RootData search | [link](https://www.rootdata.com/search?kw=Treehouse%20Protocol) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.treehouse.finance/)
- [CoinGecko profile](https://www.coingecko.com/en/coins/treehouse)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Treehouse%20Protocol)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Treehouse%20Protocol)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Treehouse%20Protocol)
- [Messari discovery](https://messari.io/search?q=Treehouse%20Protocol)
- [X profile](https://x.com/TreehouseFi)
- [Discord](https://discord.com/invite/treehousefi)
- [Treehouse price today, TREE to USD live price, marketcap and chart](https://coinmarketcap.com/currencies/treehouse/)
- [Treehouse ETH Price Chart (TETH) - CoinGecko](https://www.coingecko.com/en/coins/treehouse-eth)
- [TREE/USD Live Price Chart, Market Cap & News Today - CoinGecko](https://www.coingecko.com/en/coins/tree)
- [What Is Treehouse (TREE) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/treehouse/what-is/)
- [ethereum TREE](https://etherscan.io/token/0x77146784315ba81904d654466968e3a7c196d1f3)
- [bsc TREE](https://bscscan.com/token/0x77146784315ba81904d654466968e3a7c196d1f3)
- [avalanche TAVAX](https://snowtrace.io/token/0x14a84f1a61ccd7d1be596a6cc11fe33a36bc1646)
- [base TETH](https://basescan.org/token/0xd09acb80c1e8f2291862c4978a008791c9167003)
- [ethereum TETH](https://etherscan.io/token/0xd11c452fc99cf405034ee446803b6f6c1f6d5ed8)
- [arbitrum TETH](https://arbiscan.io/token/0xd09acb80c1e8f2291862c4978a008791c9167003)
- [Google project search](https://www.google.com/search?q=Treehouse%20Protocol%20TREE%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Treehouse%20Protocol%20TREE%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Treehouse%20Protocol%20docs)
- [Google governance search](https://www.google.com/search?q=Treehouse%20Protocol%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Treehouse%20Protocol%20audit%20report)
- [CoinGecko historical data](https://www.coingecko.com/en/coins/treehouse/historical_data)

## Final Investment View

Treehouse Protocol should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
