---
title: 'Dolomite / DOLO: Money market / margin lending value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Dolomite is treated as a Money market / margin lending watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'dolomite-dolo-money-market-margin-lending-token-value-capture-risk'
category: 'research'
---

# Dolomite / DOLO: Money market / margin lending value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Dolomite appears in the local CoinGecko/CMC-oriented candidate pool through [CoinGecko](https://www.coingecko.com/en/coins/dolomite) and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Berachain, Ethereum; exchange exposure is BINANCE, COINBASE, BYBIT, BITGET; category tags are DEX & AMM, Lending & Borrowing, Yield & Asset Mgmt; current market cap is $11.24M; FDV is $22.92M; and 24h volume is $3.68M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Dolomite before this file was created, and the existing candidate note says: DefiLlama category Lending on Multi-Chain; TVL $191,759,971. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Dolomite is best understood as a **Money market / margin lending** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether DOLO captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.

| Metric | Current read |
|---|---|
| Project | Dolomite |
| Token | DOLO |
| Category | Money market / margin lending |
| Chains | Berachain, Ethereum |
| Exchanges | BINANCE, COINBASE, BYBIT, BITGET |
| Price | $0.0230 |
| Market cap | $11.24M |
| FDV | $22.92M |
| 24h volume | $3.68M |
| Circulating supply | 489.76M |
| Total supply | 998.45M |
| 7d price change | -2.09% |
| 30d price change | 3.59% |
| Latest TVL sample | $932.71M |
| TVL sample change | 19.09% |
| Primary source posture | Surf + CoinGecko + CMC discovery + official/project links |

Base case: Dolomite remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning DOLO into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Dolomite is a next-generation decentralized money market and DEX that combines lending, borrowing, and capital-efficient margin trading via a virtual liquidity architecture. It supports broad token coverage across multiple chains (e.g., Arbitrum, Mantle, Polygon zkEVM) and offers a modular immutable core with upgradeable modules for strategies and integrations.

Surf's project profile maps Dolomite to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://dolomite.io/](https://dolomite.io/) |
| X handle | [@Dolomite_io](https://x.com/Dolomite_io) |
| X followers | 66.85K |
| Tags | DEX & AMM, Lending & Borrowing, Yield & Asset Mgmt |
| Chains | Berachain, Ethereum |
| Exchanges | BINANCE, COINBASE, BYBIT, BITGET |
| TGE status | post |
| CoinGecko ID | [dolomite](https://www.coingecko.com/en/coins/dolomite) |

The main product question is whether Dolomite is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For Money market / margin lending projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Dolomite convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $3.68M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 489.76M circulating vs 998.45M total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Dolomite is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.

## Architecture / Mechanism

Dolomite's architecture should be evaluated through the job it performs. For a Money market / margin lending project, architecture is not merely code shape. It includes the contracts, governance process, integration path, oracle and risk dependencies, bridge or staking assumptions, collateral policies, and the way users enter and exit positions.
...
| Settlement layer | Chains and contracts that hold user value | Berachain, Ethereum |
...
|---|---|---|---|
| ethereum | DOLO | 0x0f81001ef0a83ecce5ccebf63eb302c70a39a654 | [explorer](https://etherscan.io/token/0x0f81001ef0a83ecce5ccebf63eb302c70a39a654) |
| Berachain | DOLO | 0x0f81001ef0a83ecce5ccebf63eb302c70a39a654 | not mapped |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Dolomite, the DeFi-oriented backlog note is the first signal: DefiLlama category Lending on Multi-Chain; TVL $191,759,971. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $11.24M, FDV $22.92M, 24h volume $3.68M, and price $0.0230.

| Traction metric | Value | Interpretation |
|---|---|---|
| Latest TVL sample | $932.71M | Useful only if sustained without aggressive incentives |
| TVL sample points | 90 | More points improve trend confidence |
| TVL sample change | 19.09% | Positive trend supports usage; negative trend requires explanation |
| 24h volume | $3.68M | Position sizing and exit liquidity proxy |
| Market cap | $11.24M | Liquid valuation proxy |
| FDV | $22.92M | Dilution-aware valuation proxy |
| FDV / market cap spread | 2.04x | Higher spread means more supply risk |
| X followers | 66.85K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Dolomite | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether DOLO is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. Dolomite should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that Dolomite belongs to the Money market / margin lending bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $11.24M, FDV $22.92M, 24h volume $3.68M, price $0.0230, circulating supply 489.76M, and total supply 998.45M. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For Dolomite, the candidate backlog records DefiLlama category Lending on Multi-Chain; TVL $191,759,971. Local registry and current candidates duplicate check passed on 2026-06-28; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Berachain, Ethereum, exchanges as BINANCE, COINBASE, BYBIT, BITGET, and tags as DEX & AMM, Lending & Borrowing, Yield & Asset Mgmt. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.

The fourth frame is value capture. A Money market / margin lending asset can be useful while the token remains weak. The memo therefore separates product-market fit from token-market fit. Product-market fit asks whether users need the protocol even when incentives fade. Token-market fit asks whether the token absorbs value through staking, fee routing, collateral, governance constraints, emissions control, slashing insurance, or privileged access. If the token is mostly governance, the base case assigns lower confidence. If the token is used for security or fee capture but the mechanism is not yet activated, the base case treats value capture as an option rather than as current cash flow.

The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Dolomite currently needs to be monitored through that sequence. The available 90-day TVL series has 90 point(s), latest TVL $932.71M, and approximate change 19.09%. Market-price movement shows 7d -2.09% and 30d 3.59% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Dolomite's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2024-03-19 | Strategic | $900.00K | Sandeep Nailwal, DCF God, DeFi Dad, SobyLife, Marc Boiron, Pentoshi, Optic Capital, Altcoin Sherpa |
| 2023-10-13 | Grant | $804.00K | Arbitrum Foundation |
| 2023-05-12 | Undisclosed | $2.50M | Coinbase Ventures, NGC Ventures, 6th Man Ventures, Draper Goren Holm, WWVentures |

| Team member | Role | Link |
|---|---|---|
| Adam Knuckey | COO, Co-Founder | [X](https://x.com/adam_knuckey) |
| Corey Caplan | Co-Founder, President | [X](https://x.com/coreycaplan3) |
...
Dolomite competes in the Money market / margin lending bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Dolomite has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Dolomite is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Dolomite compounds usage, liquidity improves, and DOLO gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If DOLO does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
|---|---|---|---|
| TVL | $932.71M | Weekly | >20% drawdown or sustained growth |
| 24h volume | $3.68M | Weekly | Volume below position-size threshold |
| FDV / market cap | 2.04x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Dolomite announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Dolomite has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Dolomite, the first reconciliation job is identity. The token symbol is DOLO, the category is Money market / margin lending, chains are Berachain, Ethereum, exchanges are BINANCE, COINBASE, BYBIT, BITGET, and tags are DEX & AMM, Lending & Borrowing, Yield & Asset Mgmt. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If DOLO trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $11.24M, FDV is $22.92M, 24h volume is $3.68M, FDV / market cap is 2.04x, 7d price change is -2.09%, and 30d price change is 3.59%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.

The third reconciliation job is usage data. The latest TVL sample is $932.71M, TVL sample change is 19.09%, and the sample contains 90 point(s). TVL is useful only when the underlying balances are real, withdrawable, and not double-counted. For Money market / margin lending, the memo should identify whether TVL means deposits, staked assets, LP liquidity, stablecoin supply, collateral, locked governance assets, or a derived wrapper. A yield-bearing stablecoin and a validator middleware protocol can both show TVL, but the risk encoded by that TVL is completely different.

| Reconciliation item | Current value | Next check |
|---|---|---|
| Identity | Dolomite / DOLO | Match official docs, CG, CMC, Surf, contracts |
| Category | Money market / margin lending | Confirm product mechanics and revenue model |
| Chains | Berachain, Ethereum | Confirm canonical deployments and bridge assumptions |
| Exchanges | BINANCE, COINBASE, BYBIT, BITGET | Check live order books and inactive pairs |
| Market cap | $11.24M | Compare Surf, CG, CMC, exchange pages |
| FDV | $22.92M | Reconcile total supply and unlock schedule |
| Volume | $3.68M | Check exchange concentration and wash-trading risk |
| TVL | $932.71M | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Dolomite does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Dolomite, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does DOLO sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
| Correlation check | Count exposure to related Money market / margin lending assets | Category beta can hide concentration |
...
| Peer comparison | Monthly | Dolomite gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://dolomite.io/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko profile | [link](https://www.coingecko.com/en/coins/dolomite) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Dolomite) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Dolomite) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Dolomite) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Dolomite) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/Dolomite_io) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/dolomite-exchange) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/dolomite_official) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](https://discord.gg/uDRzrB2YgP) | Identity, market, social, contract, or discovery cross-check |
| 11 | What Is Dolomite (DOLO) And How Does It Work? - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/dolomite/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 12 | Token Mechanics - Dolomite | [link](https://docs.dolomite.io/dolo/token-mechanics) | Identity, market, social, contract, or discovery cross-check |
| 13 | DOLO to ETH: Dolomite Price in Ether - CoinGecko | [link](https://www.coingecko.com/en/coins/dolomite/eth) | Identity, market, social, contract, or discovery cross-check |
| 14 | Dolomite (DOLO) / Tokenomics & Vesting Schedule - Token Unlocks | [link](https://tokenomist.ai/dolomite) | Identity, market, social, contract, or discovery cross-check |
| 15 | ethereum DOLO | [link](https://etherscan.io/token/0x0f81001ef0a83ecce5ccebf63eb302c70a39a654) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google project search | [link](https://www.google.com/search?q=Dolomite%20DOLO%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google tokenomics search | [link](https://www.google.com/search?q=Dolomite%20DOLO%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google docs search | [link](https://www.google.com/search?q=Dolomite%20docs) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google governance search | [link](https://www.google.com/search?q=Dolomite%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google audit search | [link](https://www.google.com/search?q=Dolomite%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinGecko historical data | [link](https://www.coingecko.com/en/coins/dolomite/historical_data) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/dolomite/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/dolomite/) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Dolomite) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune token query | [link](https://dune.com/browse/queries?q=Dolomite) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Dolomite) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=DOLO) | Identity, market, social, contract, or discovery cross-check |
| 28 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Dolomite) | Identity, market, social, contract, or discovery cross-check |
| 29 | CryptoRank search | [link](https://cryptorank.io/price/dolomite) | Identity, market, social, contract, or discovery cross-check |
| 30 | RootData search | [link](https://www.rootdata.com/search?kw=Dolomite) | Identity, market, social, contract, or discovery cross-check |
| 31 | CoinDesk search | [link](https://www.coindesk.com/search?s=Dolomite) | Identity, market, social, contract, or discovery cross-check |
| 32 | The Block search | [link](https://www.theblock.co/search?query=Dolomite) | Identity, market, social, contract, or discovery cross-check |
| 33 | GitHub search | [link](https://github.com/search?q=Dolomite&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 34 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=DOLO) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://dolomite.io/)
- [CoinGecko profile](https://www.coingecko.com/en/coins/dolomite)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Dolomite)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Dolomite)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Dolomite)
- [Messari discovery](https://messari.io/search?q=Dolomite)
- [X profile](https://x.com/Dolomite_io)
- [GitHub](https://github.com/dolomite-exchange)
- [Telegram](https://t.me/dolomite_official)
- [Discord](https://discord.gg/uDRzrB2YgP)
- [What Is Dolomite (DOLO) And How Does It Work? - CoinMarketCap](https://coinmarketcap.com/cmc-ai/dolomite/what-is/)
- [Token Mechanics - Dolomite](https://docs.dolomite.io/dolo/token-mechanics)
- [DOLO to ETH: Dolomite Price in Ether - CoinGecko](https://www.coingecko.com/en/coins/dolomite/eth)
- [Dolomite (DOLO) | Tokenomics & Vesting Schedule - Token Unlocks](https://tokenomist.ai/dolomite)
- [ethereum DOLO](https://etherscan.io/token/0x0f81001ef0a83ecce5ccebf63eb302c70a39a654)
- [Google project search](https://www.google.com/search?q=Dolomite%20DOLO%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Dolomite%20DOLO%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Dolomite%20docs)
- [Google governance search](https://www.google.com/search?q=Dolomite%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Dolomite%20audit%20report)
- [CoinGecko historical data](https://www.coingecko.com/en/coins/dolomite/historical_data)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/dolomite/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/dolomite/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Dolomite)

## Final Investment View

Dolomite should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
