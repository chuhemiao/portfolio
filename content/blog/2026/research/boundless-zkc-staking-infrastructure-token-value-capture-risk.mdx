---
title: 'Boundless / ZKC: staking infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Boundless is treated as a staking infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'boundless-zkc-staking-infrastructure-token-value-capture-risk'
category: 'research'
---

# Boundless / ZKC: staking infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Boundless appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Binance Smart Chain, Ethereum; exchange exposure is BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET; category tags are Privacy & ZK, DePIN (Physical Infra), Node & Validator; current market cap is $13.30M; FDV is $48.37M; and 24h volume is $3.05M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Boundless before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 1021, market cap $13,259,716, 24h volume $3,123,978. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Boundless is best understood as a **staking infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether ZKC captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
Base case: Boundless remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning ZKC into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Boundless is a universal ZK protocol that provides verifiable compute to any blockchain, letting developers offload execution and verify computations with zero-knowledge proofs. Independent prover nodes compete to fulfill proof requests and earn rewards via a Proof-of-Verifiable-Work model, enabling scalable, cross-chain ZK proving for rollups and apps.

Surf's project profile maps Boundless to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://boundless.network/](https://boundless.network/) |
| X handle | [@boundless_xyz](https://x.com/boundless_xyz) |
| X followers | 265.57K |
| Tags | Privacy & ZK, DePIN (Physical Infra), Node & Validator |
| Chains | Binance Smart Chain, Ethereum |
| Exchanges | BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Boundless is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For staking infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Boundless convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $3.05M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 290.64M circulating vs 1.06B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Boundless is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| ethereum | ZKC | 0x000006c2a22ff4a44ff1f5d0f2ed65f781f55555 | [explorer](https://etherscan.io/token/0x000006c2a22ff4a44ff1f5d0f2ed65f781f55555) |
| bsc | ZKC | 0x15247e6e23d3923a853ccf15940a20ccdf16e94a | [explorer](https://bscscan.com/token/0x15247e6e23d3923a853ccf15940a20ccdf16e94a) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Boundless, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 1021, market cap $13,259,716, 24h volume $3,123,978. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $13.30M, FDV $48.37M, 24h volume $3.05M, and price $0.0458.
...
| 24h volume | $3.05M | Position sizing and exit liquidity proxy |
| Market cap | $13.30M | Liquid valuation proxy |
| FDV | $48.37M | Dilution-aware valuation proxy |
| FDV / market cap spread | 3.64x | Higher spread means more supply risk |
| X followers | 265.57K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Boundless | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether ZKC is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. Boundless should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that Boundless belongs to the staking infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $13.30M, FDV $48.37M, 24h volume $3.05M, price $0.0458, circulating supply 290.64M, and total supply 1.06B. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For Boundless, the candidate backlog records Seeded 2026-07-06 from Surf market-ranking; rank 1021, market cap $13,259,716, 24h volume $3,123,978. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Binance Smart Chain, Ethereum, exchanges as BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET, and tags as Privacy & ZK, DePIN (Physical Infra), Node & Validator. If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Boundless currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 4.37% and 30d -4.22% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Boundless's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-08-25 | Ido | $4.00M | not disclosed |

| Team member | Role | Link |
|---|---|---|
| Shiv Shankar | CEO | [X](https://x.com/sshankar) |
| Joe | COO | [X](https://x.com/writtenterribly) |
| Lucian Stroie | Head of Integrations | [X](https://x.com/societalruin) |
| Mashiat Mutmainnah | Head of Ecosystem | [X](https://x.com/marvelgirl_eth) |
| Santi | Head of Product Marketing | [X](https://x.com/zksanti) |
| Emiliano Bonassi | VP, Engineering | [X](https://x.com/emilianobonassi) |
| Brett Carter | VP, Product | not disclosed |
...
Boundless competes in the staking infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Boundless has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Boundless is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Boundless compounds usage, liquidity improves, and ZKC gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If ZKC does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $3.05M | Weekly | Volume below position-size threshold |
| FDV / market cap | 3.64x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Boundless announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Boundless has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Boundless, the first reconciliation job is identity. The token symbol is ZKC, the category is staking infrastructure, chains are Binance Smart Chain, Ethereum, exchanges are BINANCE, UPBIT, COINBASE, BITHUMB, BYBIT, BITGET, and tags are Privacy & ZK, DePIN (Physical Infra), Node & Validator. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If ZKC trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $13.30M, FDV is $48.37M, 24h volume is $3.05M, FDV / market cap is 3.64x, 7d price change is 4.37%, and 30d price change is -4.22%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Boundless / ZKC | Match official docs, CG, CMC, Surf, contracts |
...
For Boundless, the current liquidity read is watchlist first. The market cap is $13.30M, FDV is $48.37M, reported 24h volume is $3.05M, and FDV / market cap is 3.64x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Boundless gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://boundless.network/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Boundless) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Boundless) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Boundless) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Boundless) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Boundless) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/boundless_xyz) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/boundless-xyz) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/boundlessannouncements) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](https://discord.com/invite/boundlessxyz) | Identity, market, social, contract, or discovery cross-check |
| 11 | Boundless Price Chart (ZKC) - CoinGecko | [link](https://www.coingecko.com/en/coins/boundless) | Identity, market, social, contract, or discovery cross-check |
| 12 | Boundless price today, ZKC to USD live price, marketcap and chart | [link](https://coinmarketcap.com/currencies/boundless/) | Identity, market, social, contract, or discovery cross-check |
| 13 | Boundless (ZKC) Price Prediction For 2026 & Beyond | [link](https://coinmarketcap.com/cmc-ai/boundless/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
| 14 | Latest Boundless (ZKC) Price Analysis - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/boundless/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 15 | ZKC to BTC: Boundless Price in Bitcoin - CoinGecko | [link](https://www.coingecko.com/en/coins/boundless/btc) | Identity, market, social, contract, or discovery cross-check |
| 16 | ethereum ZKC | [link](https://etherscan.io/token/0x000006c2a22ff4a44ff1f5d0f2ed65f781f55555) | Identity, market, social, contract, or discovery cross-check |
| 17 | bsc ZKC | [link](https://bscscan.com/token/0x15247e6e23d3923a853ccf15940a20ccdf16e94a) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google project search | [link](https://www.google.com/search?q=Boundless%20ZKC%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google tokenomics search | [link](https://www.google.com/search?q=Boundless%20ZKC%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google docs search | [link](https://www.google.com/search?q=Boundless%20docs) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google governance search | [link](https://www.google.com/search?q=Boundless%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google audit search | [link](https://www.google.com/search?q=Boundless%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/boundless/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Boundless) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune token query | [link](https://dune.com/browse/queries?q=Boundless) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Boundless) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=ZKC) | Identity, market, social, contract, or discovery cross-check |
| 28 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Boundless) | Identity, market, social, contract, or discovery cross-check |
| 29 | CryptoRank search | [link](https://cryptorank.io/price/boundless) | Identity, market, social, contract, or discovery cross-check |
| 30 | RootData search | [link](https://www.rootdata.com/search?kw=Boundless) | Identity, market, social, contract, or discovery cross-check |
| 31 | CoinDesk search | [link](https://www.coindesk.com/search?s=Boundless) | Identity, market, social, contract, or discovery cross-check |
| 32 | The Block search | [link](https://www.theblock.co/search?query=Boundless) | Identity, market, social, contract, or discovery cross-check |
| 33 | GitHub search | [link](https://github.com/search?q=Boundless&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 34 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=ZKC) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://boundless.network/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Boundless)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Boundless)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Boundless)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Boundless)
- [Messari discovery](https://messari.io/search?q=Boundless)
- [X profile](https://x.com/boundless_xyz)
- [GitHub](https://github.com/boundless-xyz)
- [Telegram](https://t.me/boundlessannouncements)
- [Discord](https://discord.com/invite/boundlessxyz)
- [Boundless Price Chart (ZKC) - CoinGecko](https://www.coingecko.com/en/coins/boundless)
- [Boundless price today, ZKC to USD live price, marketcap and chart](https://coinmarketcap.com/currencies/boundless/)
- [Boundless (ZKC) Price Prediction For 2026 & Beyond](https://coinmarketcap.com/cmc-ai/boundless/price-prediction/)
- [Latest Boundless (ZKC) Price Analysis - CoinMarketCap](https://coinmarketcap.com/cmc-ai/boundless/price-analysis/)
- [ZKC to BTC: Boundless Price in Bitcoin - CoinGecko](https://www.coingecko.com/en/coins/boundless/btc)
- [ethereum ZKC](https://etherscan.io/token/0x000006c2a22ff4a44ff1f5d0f2ed65f781f55555)
- [bsc ZKC](https://bscscan.com/token/0x15247e6e23d3923a853ccf15940a20ccdf16e94a)
- [Google project search](https://www.google.com/search?q=Boundless%20ZKC%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Boundless%20ZKC%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Boundless%20docs)
- [Google governance search](https://www.google.com/search?q=Boundless%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Boundless%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/boundless/what-is/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Boundless)

## Final Investment View

Boundless should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
