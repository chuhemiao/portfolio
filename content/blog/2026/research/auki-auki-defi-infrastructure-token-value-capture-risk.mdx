---
title: 'Auki / AUKI: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Auki is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'auki-auki-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# Auki / AUKI: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Auki appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Base, peaq; exchange exposure is not disclosed; category tags are not disclosed; current market cap is $23.51M; FDV is $52.69M; and 24h volume is $54.40K. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Auki before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 758, market cap $23,300,501, 24h volume $50,749. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Auki is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether AUKI captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
Base case: Auki remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning AUKI into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Auki is a decentralized machine perception network designed to facilitate collaborative spatial computing. The core of Auki is its Posemesh protocol, which enables devices to securely share spatial data and computing power, allowing them to form a shared understanding of physical environments. This technology is aimed at advancing spatial computing, crucial for applications like augmented reality, AI, robotics, and smart cities. Auki's $AUKI token serves as the backbone of its economy, where tokens are used for staking and accessing network services. The protocol is built around a burn-credit-mint economy. Participants burn $AUKI tokens to access network services, which are then credited in dollar-denominated values, allowing for more stable and predictable service costs. Meanwhile, the network rewards participants who provide computational resources with newly minted tokens. Over time, the supply of $AUKI will deflate until it reaches a set limit, encouraging early participation.

Surf's project profile maps Auki to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://www.aukilabs.com/](https://www.aukilabs.com/) |
| X handle | [@AukiNetwork](https://x.com/AukiNetwork) |
| X followers | 1.30K |
| Tags | not disclosed |
| Chains | Base, peaq |
| Exchanges | not disclosed |
...
| Liquidity | $54.40K 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 4.46B circulating vs 9.99B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Auki is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| base | AUKI | 0xf9569cfb8fd265e91aa478d86ae8c78b8af55df4 | [explorer](https://basescan.org/token/0xf9569cfb8fd265e91aa478d86ae8c78b8af55df4) |
| peaq | AUKI | 0xf67db9d00401d9e883208882f5c100d7482b083d | not mapped |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Auki, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 758, market cap $23,300,501, 24h volume $50,749. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $23.51M, FDV $52.69M, 24h volume $54.40K, and price $0.0053.
...
| 24h volume | $54.40K | Position sizing and exit liquidity proxy |
| Market cap | $23.51M | Liquid valuation proxy |
| FDV | $52.69M | Dilution-aware valuation proxy |
| FDV / market cap spread | 2.24x | Higher spread means more supply risk |
| X followers | 1.30K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Auki | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether AUKI is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Auki currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 9.34% and 30d -11.48% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Auki's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
Auki competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Auki has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Auki is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Auki compounds usage, liquidity improves, and AUKI gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Auki has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If AUKI does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $54.40K | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Auki announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Auki has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Auki, the first reconciliation job is identity. The token symbol is AUKI, the category is DeFi infrastructure, chains are Base, peaq, exchanges are not disclosed, and tags are not disclosed. These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If AUKI trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $23.51M, FDV is $52.69M, 24h volume is $54.40K, FDV / market cap is 2.24x, 7d price change is 9.34%, and 30d price change is -11.48%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Auki / AUKI | Match official docs, CG, CMC, Surf, contracts |
...
For Auki, the current liquidity read is watchlist first. The market cap is $23.51M, FDV is $52.69M, reported 24h volume is $54.40K, and FDV / market cap is 2.24x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Auki gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://www.aukilabs.com/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Auki) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Auki) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Auki) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Auki) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Auki) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/AukiNetwork) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/aukilabs) | Identity, market, social, contract, or discovery cross-check |
| 9 | Telegram | [link](https://t.me/AukiNetwork) | Identity, market, social, contract, or discovery cross-check |
| 10 | Discord | [link](https://discord.gg/aukiverse) | Identity, market, social, contract, or discovery cross-check |
| 11 | AUKI/USD Live Price Chart, Market Cap & News Today - CoinGecko | [link](https://www.coingecko.com/en/coins/auki-labs) | Identity, market, social, contract, or discovery cross-check |
| 12 | AUKI to ETH: Auki Price in Ether - CoinGecko | [link](https://www.coingecko.com/en/coins/auki-labs/eth) | Identity, market, social, contract, or discovery cross-check |
...
| 15 | Auki Price, AUKI Price, Live Charts, and Marketcap - Coinbase | [link](https://www.coinbase.com/price/auki-labs) | Identity, market, social, contract, or discovery cross-check |
| 16 | base AUKI | [link](https://basescan.org/token/0xf9569cfb8fd265e91aa478d86ae8c78b8af55df4) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google project search | [link](https://www.google.com/search?q=Auki%20AUKI%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google tokenomics search | [link](https://www.google.com/search?q=Auki%20AUKI%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google docs search | [link](https://www.google.com/search?q=Auki%20docs) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google governance search | [link](https://www.google.com/search?q=Auki%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google audit search | [link](https://www.google.com/search?q=Auki%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/auki/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/auki/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/auki/) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Auki) | Identity, market, social, contract, or discovery cross-check |
| 26 | Dune token query | [link](https://dune.com/browse/queries?q=Auki) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Auki) | Identity, market, social, contract, or discovery cross-check |
| 28 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=AUKI) | Identity, market, social, contract, or discovery cross-check |
| 29 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Auki) | Identity, market, social, contract, or discovery cross-check |
| 30 | CryptoRank search | [link](https://cryptorank.io/price/auki) | Identity, market, social, contract, or discovery cross-check |
| 31 | RootData search | [link](https://www.rootdata.com/search?kw=Auki) | Identity, market, social, contract, or discovery cross-check |
| 32 | CoinDesk search | [link](https://www.coindesk.com/search?s=Auki) | Identity, market, social, contract, or discovery cross-check |
| 33 | The Block search | [link](https://www.theblock.co/search?query=Auki) | Identity, market, social, contract, or discovery cross-check |
| 34 | GitHub search | [link](https://github.com/search?q=Auki&type=repositories) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://www.aukilabs.com/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Auki)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Auki)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Auki)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Auki)
- [Messari discovery](https://messari.io/search?q=Auki)
- [X profile](https://x.com/AukiNetwork)
- [GitHub](https://github.com/aukilabs)
- [Telegram](https://t.me/AukiNetwork)
- [Discord](https://discord.gg/aukiverse)
- [AUKI/USD Live Price Chart, Market Cap & News Today - CoinGecko](https://www.coingecko.com/en/coins/auki-labs)
- [AUKI to ETH: Auki Price in Ether - CoinGecko](https://www.coingecko.com/en/coins/auki-labs/eth)
...
- [Auki Price, AUKI Price, Live Charts, and Marketcap - Coinbase](https://www.coinbase.com/price/auki-labs)
- [base AUKI](https://basescan.org/token/0xf9569cfb8fd265e91aa478d86ae8c78b8af55df4)
- [Google project search](https://www.google.com/search?q=Auki%20AUKI%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Auki%20AUKI%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Auki%20docs)
- [Google governance search](https://www.google.com/search?q=Auki%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Auki%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/auki/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/auki/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/auki/)

## Final Investment View

Auki should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
