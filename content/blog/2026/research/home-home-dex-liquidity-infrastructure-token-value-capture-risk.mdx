---
title: 'HOME / HOME: DEX / liquidity infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'HOME is treated as a DEX / liquidity infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'home-home-dex-liquidity-infrastructure-token-value-capture-risk'
category: 'research'
---

# HOME / HOME: DEX / liquidity infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** HOME appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Base, Binance Smart Chain, Solana; exchange exposure is BINANCE, COINBASE, BITHUMB, BYBIT, BITGET; category tags are DEX & AMM, Lending & Borrowing, Wallets & AA (Account Abstraction); current market cap is $60.32M; FDV is $155.27M; and 24h volume is $9.49M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag HOME before this file was created, and the existing candidate note says: Seeded 2026-06-28 from Surf market-ranking; rank 319, market cap $73,345,832, 24h volume $10,384,715. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

HOME is best understood as a **DEX / liquidity infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether HOME captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| FDV | $155.27M |
| 24h volume | $9.49M |
| Circulating supply | 3.88B |
| Total supply | 10.00B |
| 7d price change | -19.79% |
| 30d price change | -64.00% |
...
Base case: HOME remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning HOME into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Defi App is an all-in-one DeFi “Everything App” providing self-custodial wallets, instant cross-chain swaps, gasless transactions, lending, leverage/perps and yield-earning products. It has a native HOME token used for rewards, staking and governance, with docs and app access published on the project’s official site.

Surf's project profile maps HOME to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://defi.app/](https://defi.app/) |
| X handle | [@defiapp](https://x.com/defiapp) |
| X followers | 124.30K |
| Tags | DEX & AMM, Lending & Borrowing, Wallets & AA (Account Abstraction) |
| Chains | Base, Binance Smart Chain, Solana |
| Exchanges | BINANCE, COINBASE, BITHUMB, BYBIT, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether HOME is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DEX / liquidity infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does HOME convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $9.49M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 3.88B circulating vs 10.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that HOME is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| bsc | HOME | 0x4bfaa776991e85e5f8b1255461cbbd216cfc714f | [explorer](https://bscscan.com/token/0x4bfaa776991e85e5f8b1255461cbbd216cfc714f) |
| base | HOME | 0x4bfaa776991e85e5f8b1255461cbbd216cfc714f | [explorer](https://basescan.org/token/0x4bfaa776991e85e5f8b1255461cbbd216cfc714f) |
| solana | HOME | J3umBWqhSjd13sag1E1aUojViWvPYA5dFNyqpKuX3WXj | [explorer](https://solscan.io/token/J3umBWqhSjd13sag1E1aUojViWvPYA5dFNyqpKuX3WXj) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For HOME, the DeFi-oriented backlog note is the first signal: Seeded 2026-06-28 from Surf market-ranking; rank 319, market cap $73,345,832, 24h volume $10,384,715. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $60.32M, FDV $155.27M, 24h volume $9.49M, and price $0.0155.
...
| 24h volume | $9.49M | Position sizing and exit liquidity proxy |
| Market cap | $60.32M | Liquid valuation proxy |
| FDV | $155.27M | Dilution-aware valuation proxy |
| FDV / market cap spread | 2.57x | Higher spread means more supply risk |
| X followers | 124.30K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for HOME | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether HOME is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. HOME currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d -19.79% and 30d -64.00% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that HOME's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-02-13 | Ico | $2.00M | not disclosed |
| 2025-02-13 | Extended Seed | $2.00M | Mechanism Capital, Balaji Srinivasan, Pentoshi, DCF Capital Partners |
| 2024-12-04 | Seed | $2.00M | Mechanism Capital, Selini Capital, North Rock Capital |

| Team member | Role | Link |
|---|---|---|
| Dan Greer | Co-Founder | [X](https://x.com/0xUxDesign/) |
...
HOME competes in the DEX / liquidity infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether HOME has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: HOME is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | HOME compounds usage, liquidity improves, and HOME gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because HOME has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If HOME does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $9.49M | Weekly | Volume below position-size threshold |
| FDV / market cap | 2.57x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if HOME announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. HOME has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For HOME, the first reconciliation job is identity. The token symbol is HOME, the category is DEX / liquidity infrastructure, chains are Base, Binance Smart Chain, Solana, exchanges are BINANCE, COINBASE, BITHUMB, BYBIT, BITGET, and tags are DEX & AMM, Lending & Borrowing, Wallets & AA (Account Abstraction). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If HOME trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $60.32M, FDV is $155.27M, 24h volume is $9.49M, FDV / market cap is 2.57x, 7d price change is -19.79%, and 30d price change is -64.00%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | HOME / HOME | Match official docs, CG, CMC, Surf, contracts |
...
For HOME, the current liquidity read is watchlist first. The market cap is $60.32M, FDV is $155.27M, reported 24h volume is $9.49M, and FDV / market cap is 2.57x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | HOME gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://defi.app/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=HOME) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=HOME) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=HOME) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=HOME) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=HOME) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/defiapp) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/defi-app) | Identity, market, social, contract, or discovery cross-check |
| 9 | Discord | [link](https://discord.com/invite/defiapp) | Identity, market, social, contract, or discovery cross-check |
| 10 | HOME/USD Live Price Chart, Market Cap & News Today - CoinGecko | [link](https://www.coingecko.com/en/coins/home) | Identity, market, social, contract, or discovery cross-check |
...
| 13 | CoinMarketCap: Cryptocurrency Prices, Charts And Market ... | [link](https://coinmarketcap.com/) | Identity, market, social, contract, or discovery cross-check |
| 14 | HOME Coin price today, HOME to USD live price, marketcap and chart | [link](https://coinmarketcap.com/currencies/baconcoin-token/) | Identity, market, social, contract, or discovery cross-check |
| 15 | bsc HOME | [link](https://bscscan.com/token/0x4bfaa776991e85e5f8b1255461cbbd216cfc714f) | Identity, market, social, contract, or discovery cross-check |
| 16 | base HOME | [link](https://basescan.org/token/0x4bfaa776991e85e5f8b1255461cbbd216cfc714f) | Identity, market, social, contract, or discovery cross-check |
| 17 | solana HOME | [link](https://solscan.io/token/J3umBWqhSjd13sag1E1aUojViWvPYA5dFNyqpKuX3WXj) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google project search | [link](https://www.google.com/search?q=HOME%20HOME%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google tokenomics search | [link](https://www.google.com/search?q=HOME%20HOME%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google docs search | [link](https://www.google.com/search?q=HOME%20docs) | Identity, market, social, contract, or discovery cross-check |
| 21 | Google governance search | [link](https://www.google.com/search?q=HOME%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 22 | Google audit search | [link](https://www.google.com/search?q=HOME%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/home/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 24 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/home/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 25 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/home/) | Identity, market, social, contract, or discovery cross-check |
| 26 | Dune discovery | [link](https://dune.com/browse/dashboards?q=HOME) | Identity, market, social, contract, or discovery cross-check |
| 27 | Dune token query | [link](https://dune.com/browse/queries?q=HOME) | Identity, market, social, contract, or discovery cross-check |
| 28 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=HOME) | Identity, market, social, contract, or discovery cross-check |
| 29 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=HOME) | Identity, market, social, contract, or discovery cross-check |
| 30 | TokenUnlocks search | [link](https://token.unlocks.app/?search=HOME) | Identity, market, social, contract, or discovery cross-check |
| 31 | CryptoRank search | [link](https://cryptorank.io/price/home) | Identity, market, social, contract, or discovery cross-check |
| 32 | RootData search | [link](https://www.rootdata.com/search?kw=HOME) | Identity, market, social, contract, or discovery cross-check |
| 33 | CoinDesk search | [link](https://www.coindesk.com/search?s=HOME) | Identity, market, social, contract, or discovery cross-check |
| 34 | The Block search | [link](https://www.theblock.co/search?query=HOME) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://defi.app/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=HOME)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=HOME)
- [DefiLlama protocol discovery](https://defillama.com/search?query=HOME)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=HOME)
- [Messari discovery](https://messari.io/search?q=HOME)
- [X profile](https://x.com/defiapp)
- [GitHub](https://github.com/defi-app)
- [Discord](https://discord.com/invite/defiapp)
- [HOME/USD Live Price Chart, Market Cap & News Today - CoinGecko](https://www.coingecko.com/en/coins/home)
...
- [HOME Coin price today, HOME to USD live price, marketcap and chart](https://coinmarketcap.com/currencies/baconcoin-token/)
- [bsc HOME](https://bscscan.com/token/0x4bfaa776991e85e5f8b1255461cbbd216cfc714f)
- [base HOME](https://basescan.org/token/0x4bfaa776991e85e5f8b1255461cbbd216cfc714f)
- [solana HOME](https://solscan.io/token/J3umBWqhSjd13sag1E1aUojViWvPYA5dFNyqpKuX3WXj)
- [Google project search](https://www.google.com/search?q=HOME%20HOME%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=HOME%20HOME%20tokenomics)
- [Google docs search](https://www.google.com/search?q=HOME%20docs)
- [Google governance search](https://www.google.com/search?q=HOME%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=HOME%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/home/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/home/price-analysis/)
