---
title: 'Recall / RECALL: DeFi infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Recall is treated as a DeFi infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'recall-recall-defi-infrastructure-token-value-capture-risk'
category: 'research'
---

# Recall / RECALL: DeFi infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Recall appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Base; exchange exposure is COINBASE, BITHUMB, BYBIT, BINANCE ALPHA, BITGET; category tags are Prediction Market, Web3 AI (Agent/AIGC); current market cap is $8.40M; FDV is $33.11M; and 24h volume is $10.66M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Recall before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 1268, market cap $8,373,232, 24h volume $10,833,885. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Recall is best understood as a **DeFi infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether RECALL captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Base |
| Exchanges | COINBASE, BITHUMB, BYBIT, BINANCE ALPHA, BITGET |
| Price | $0.0331 |
| Market cap | $8.40M |
| FDV | $33.11M |
| 24h volume | $10.66M |
| Circulating supply | 253.56M |
| Total supply | 1.00B |
...
Base case: Recall remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning RECALL into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Recall is a decentralized skill market for AI where communities fund, rank, and discover the AI solutions they need. By combining tokenized crowdfunding, curation, and competitions, Recall gives humanity the power to shape the future of AI by coordinating resources to generate high-quality AI that serves their diverse and ever-changing needs.

Surf's project profile maps Recall to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://recall.network/](https://recall.network/) |
| X handle | [@recallnet](https://x.com/recallnet) |
| X followers | 254.48K |
| Tags | Prediction Market, Web3 AI (Agent/AIGC) |
| Chains | Base |
| Exchanges | COINBASE, BITHUMB, BYBIT, BINANCE ALPHA, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Recall is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For DeFi infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Recall convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $10.66M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 253.56M circulating vs 1.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Recall is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
|---|---|---|---|
| base | RECALL | 0x1f16e03c1a5908818f47f6ee7bb16690b40d0671 | [explorer](https://basescan.org/token/0x1f16e03c1a5908818f47f6ee7bb16690b40d0671) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Recall, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 1268, market cap $8,373,232, 24h volume $10,833,885. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $8.40M, FDV $33.11M, 24h volume $10.66M, and price $0.0331.
...
| 24h volume | $10.66M | Position sizing and exit liquidity proxy |
| Market cap | $8.40M | Liquid valuation proxy |
| FDV | $33.11M | Dilution-aware valuation proxy |
| FDV / market cap spread | 3.94x | Higher spread means more supply risk |
| X followers | 254.48K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Recall | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether RECALL is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Recall currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 5.21% and 30d -11.96% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Recall's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2023-04-20 | Series A | $8.00M | Multicoin Capital, CoinFund, A.Capital Ventures, Protocol Labs, BlueYard Capital, Betaworks |
| 2022-02-16 | Series A | $30.00M | Coinbase Ventures, Placeholder Ventures, Fenbushi Capital, Hashed, Animoca Brands, Multicoin Capital, CoinFund, Digital Currency Group (DCG) |
| 2019-07-11 | Seed | $1.50M | Coinbase Ventures, Multicoin Capital, Henri Stern, Collaborative Fund, BlueYard Capital, Jesse Clayburgh, System.One |

| Team member | Role | Link |
|---|---|---|
| Andrew Hill | Co-Founder, CEO | [X](https://x.com/andrewxhill) |
| Carson Farmer | Co-Founder, CTO | [X](https://x.com/carsonfarmer) |
| Michael Sena | Co-Founder, CMO | [X](https://x.com/dataliquidity) |
| Sander Pick | Co-Founder | [X](https://x.com/sanderpick) |
| Danny Zuckerman | COO | [X](https://x.com/dazuck) |
...
Recall competes in the DeFi infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Recall has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Recall is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Recall compounds usage, liquidity improves, and RECALL gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
**Confidence: 6.2 / 10.** The score is above neutral because Recall has enough source coverage and category relevance to justify a full report. It is below high conviction because several valuation-critical fields remain unresolved: durable fees, tokenholder capture, precise unlocks, and stress-tested liquidity.
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If RECALL does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $10.66M | Weekly | Volume below position-size threshold |
...
1. Re-run Surf project-detail and project-defi-metrics if Recall announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Recall has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Recall, the first reconciliation job is identity. The token symbol is RECALL, the category is DeFi infrastructure, chains are Base, exchanges are COINBASE, BITHUMB, BYBIT, BINANCE ALPHA, BITGET, and tags are Prediction Market, Web3 AI (Agent/AIGC). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If RECALL trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $8.40M, FDV is $33.11M, 24h volume is $10.66M, FDV / market cap is 3.94x, 7d price change is 5.21%, and 30d price change is -11.96%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
|---|---|---|
| Identity | Recall / RECALL | Match official docs, CG, CMC, Surf, contracts |
...
| Exchanges | COINBASE, BITHUMB, BYBIT, BINANCE ALPHA, BITGET | Check live order books and inactive pairs |
| Market cap | $8.40M | Compare Surf, CG, CMC, exchange pages |
| FDV | $33.11M | Reconcile total supply and unlock schedule |
| Volume | $10.66M | Check exchange concentration and wash-trading risk |
| TVL | not disclosed | Confirm TVL meaning, source, and double-counting risk |

## Primary-source Diligence Work Plan

The next diligence pass should start from primary sources, not from the token chart. The first document to find is an official docs or whitepaper page that explains what Recall does and where users interact with it. The second is tokenomics: allocation, emissions, unlocks, utility, fee routing, treasury policy, and governance rights. The third is security material: audits, bug bounty scope, incident post-mortems, admin-key disclosures, or upgrade timelocks. The fourth is usage evidence: live app metrics, dashboards, contract balances, fee dashboards, validator counts, loan books, LP depth, or stablecoin collateral reports. Without those four documents, any price target is mostly narrative.

For Recall, the primary-source pass should answer a narrow sequence of questions. What user action creates protocol value? Who pays? Who receives the fee or spread? Does RECALL sit inside that flow, or does it only vote on parameters? Can users exit without trusting a multisig, bridge, centralized custodian, or opaque off-chain book? What part of the system can be paused or upgraded? Which parameters would cause losses if changed incorrectly? If the project is young, which metrics prove organic adoption rather than incentives? If the project is mature, which metrics prove durability rather than slow decline?
...
For Recall, the current liquidity read is watchlist first. The market cap is $8.40M, FDV is $33.11M, reported 24h volume is $10.66M, and FDV / market cap is 3.94x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
| Peer comparison | Monthly | Recall gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://recall.network/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Recall) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Recall) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Recall) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Recall) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Recall) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/recallnet) | Identity, market, social, contract, or discovery cross-check |
| 8 | GitHub | [link](https://github.com/recallnet) | Identity, market, social, contract, or discovery cross-check |
| 9 | Discord | [link](https://discord.com/invite/recallnet) | Identity, market, social, contract, or discovery cross-check |
| 10 | RECALL/USD Live Price Chart, Market Cap & News Today | [link](https://www.coingecko.com/en/coins/recall) | Identity, market, social, contract, or discovery cross-check |
| 11 | What Is Recall (RECALL) And How Does It Work? | [link](https://coinmarketcap.com/cmc-ai/recall-network/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 12 | Recall (RECALL) Price Prediction For 2026 & Beyond | [link](https://coinmarketcap.com/cmc-ai/recall-network/price-prediction/) | Identity, market, social, contract, or discovery cross-check |
...
| 14 | Latest Recall (RECALL) Price Analysis | [link](https://coinmarketcap.com/cmc-ai/recall-network/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 15 | base RECALL | [link](https://basescan.org/token/0x1f16e03c1a5908818f47f6ee7bb16690b40d0671) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google project search | [link](https://www.google.com/search?q=Recall%20RECALL%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google tokenomics search | [link](https://www.google.com/search?q=Recall%20RECALL%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google docs search | [link](https://www.google.com/search?q=Recall%20docs) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google governance search | [link](https://www.google.com/search?q=Recall%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 20 | Google audit search | [link](https://www.google.com/search?q=Recall%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/recall/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 22 | CoinMarketCap price analysis guess | [link](https://coinmarketcap.com/cmc-ai/recall/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 23 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/recall/) | Identity, market, social, contract, or discovery cross-check |
| 24 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Recall) | Identity, market, social, contract, or discovery cross-check |
| 25 | Dune token query | [link](https://dune.com/browse/queries?q=Recall) | Identity, market, social, contract, or discovery cross-check |
| 26 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Recall) | Identity, market, social, contract, or discovery cross-check |
| 27 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=RECALL) | Identity, market, social, contract, or discovery cross-check |
| 28 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Recall) | Identity, market, social, contract, or discovery cross-check |
| 29 | CryptoRank search | [link](https://cryptorank.io/price/recall) | Identity, market, social, contract, or discovery cross-check |
| 30 | RootData search | [link](https://www.rootdata.com/search?kw=Recall) | Identity, market, social, contract, or discovery cross-check |
| 31 | CoinDesk search | [link](https://www.coindesk.com/search?s=Recall) | Identity, market, social, contract, or discovery cross-check |
| 32 | The Block search | [link](https://www.theblock.co/search?query=Recall) | Identity, market, social, contract, or discovery cross-check |
| 33 | GitHub search | [link](https://github.com/search?q=Recall&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 34 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=RECALL) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://recall.network/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Recall)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Recall)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Recall)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Recall)
- [Messari discovery](https://messari.io/search?q=Recall)
- [X profile](https://x.com/recallnet)
- [GitHub](https://github.com/recallnet)
- [Discord](https://discord.com/invite/recallnet)
- [RECALL/USD Live Price Chart, Market Cap & News Today](https://www.coingecko.com/en/coins/recall)
- [What Is Recall (RECALL) And How Does It Work?](https://coinmarketcap.com/cmc-ai/recall-network/what-is/)
- [Recall (RECALL) Price Prediction For 2026 & Beyond](https://coinmarketcap.com/cmc-ai/recall-network/price-prediction/)
- [CoinGecko: Cryptocurrency Prices, Charts, and Crypto Market ...](https://www.coingecko.com/)
- [Latest Recall (RECALL) Price Analysis](https://coinmarketcap.com/cmc-ai/recall-network/price-analysis/)
- [base RECALL](https://basescan.org/token/0x1f16e03c1a5908818f47f6ee7bb16690b40d0671)
- [Google project search](https://www.google.com/search?q=Recall%20RECALL%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Recall%20RECALL%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Recall%20docs)
- [Google governance search](https://www.google.com/search?q=Recall%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Recall%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/recall/what-is/)
- [CoinMarketCap price analysis guess](https://coinmarketcap.com/cmc-ai/recall/price-analysis/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/recall/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Recall)

## Final Investment View

Recall should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
