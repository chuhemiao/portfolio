---
title: 'Particle Network / PARTI: bridge / cross-chain infrastructure value capture, liquidity, and token risk'
publishedAt: '2026-07-06'
summary: 'Particle Network is treated as a bridge / cross-chain infrastructure watchlist candidate sourced from CoinGecko/CMC discovery and refreshed with Surf, with emphasis on product durability, token value capture, liquidity, supply, and execution risk.'
slug: 'particle-network-parti-bridge-cross-chain-infrastructure-token-value-capture-risk'
category: 'research'
---

# Particle Network / PARTI: bridge / cross-chain infrastructure value capture, liquidity, and token risk

## Pre-screen Decision

**Decision: add to Research Map as a full-depth watchlist report, not as an automatic accumulation call.** Particle Network appears in the local CoinGecko/CMC-oriented candidate pool through market discovery and was refreshed with Surf on 2026-07-06. The project has enough protocol surface for a real memo: chain exposure is Base, Binance Smart Chain; exchange exposure is BINANCE, OKX, BITHUMB, BYBIT, BITGET; category tags are Cross-Chain & Bridge, Layer1, Wallets & AA (Account Abstraction); current market cap is $11.27M; FDV is $48.38M; and 24h volume is $11.69M. That combination makes it worth researching, but not enough to override the main risks: token value capture may be indirect, liquidity can be fragile, supply methodology may differ across data vendors, and protocol usage can be incentive-sensitive.

The report uses a conservative watchlist standard. A pass means the project deserves monitoring and comparison against peers. It does not mean the token is cheap. A fail would require one of three red flags: no identifiable official/project surface, no meaningful market or DeFi data, or a high-confidence duplicate in the Research Map. Local duplicate checks did not flag Particle Network before this file was created, and the existing candidate note says: Seeded 2026-07-06 from Surf market-ranking; rank 1100, market cap $11,416,036, 24h volume $11,833,426. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.

## TL;DR / Executive Summary

Particle Network is best understood as a **bridge / cross-chain infrastructure** asset where the product thesis and the token thesis must be separated. The product thesis asks whether users, validators, LPs, borrowers, or asset issuers have a durable reason to use the protocol. The token thesis asks whether PARTI captures that activity through fees, staking, collateral, governance, liquidity routing, protocol-owned assets, or emissions control. The available evidence supports a watchlist entry, especially because it has a visible data footprint across Surf, CoinGecko, CMC discovery, and DeFi-oriented candidate notes. The evidence does not yet support a high-conviction valuation without better fee/revenue, unlock, treasury, and user-retention disclosure.
...
| Chains | Base, Binance Smart Chain |
| Exchanges | BINANCE, OKX, BITHUMB, BYBIT, BITGET |
| Price | $0.0484 |
| Market cap | $11.27M |
| FDV | $48.38M |
| 24h volume | $11.69M |
| Circulating supply | 233.00M |
| Total supply | 1.00B |
...
Base case: Particle Network remains a selective watchlist asset until token value capture is cleaner. Bull case: protocol usage and token mechanics converge, turning PARTI into a security, collateral, or fee-routing asset with real demand. Bear case: the protocol remains useful but token holders absorb dilution, thin liquidity, governance optionality, or regulatory/technical tail risk.

## Project Overview

Particle Network is a modular Layer‑1 chain‑abstraction infrastructure that provides a single Universal Account and unified balance across multiple chains, enabling cross‑chain payments, Universal Gas, and aggregated liquidity. It offers developer SDKs for social login and ERC‑4337 account abstraction, cross‑chain execution and operator/staking services, and issues the PARTI token for protocol economics.

Surf's project profile maps Particle Network to the following high-level surface:

| Area | Evidence |
|---|---|
| Website | [https://particle.network/](https://particle.network/) |
| X handle | [@ParticleNtwrk](https://x.com/ParticleNtwrk) |
| X followers | 791.99K |
| Tags | Cross-Chain & Bridge, Layer1, Wallets & AA (Account Abstraction) |
| Chains | Base, Binance Smart Chain |
| Exchanges | BINANCE, OKX, BITHUMB, BYBIT, BITGET |
| TGE status | post |
| CoinGecko ID | not disclosed |

The main product question is whether Particle Network is a standalone protocol with defensible demand or a token wrapper riding a larger ecosystem wave. For bridge / cross-chain infrastructure projects, the strongest evidence is usually not the token chart. It is repeated usage across cycles, measurable TVL or volume, sticky integrations, credible counterparties, and a token design that matters even after incentive budgets normalize.

## Research Question / Investment Relevance

The core research question is: **does Particle Network convert protocol relevance into tokenholder relevance?** This is the gap that breaks many DeFi, infrastructure, and RWA tokens. A protocol can have meaningful TVL, a strong founder story, integrations, and real users while the token remains a weak claim on that value. Conversely, a token can appreciate because of listings or narrative momentum while the underlying protocol economics are not yet strong enough to justify the move.
...
| Liquidity | $11.69M 24h volume | Deeper CEX/DEX books and less fragmented contract liquidity |
| Supply quality | 233.00M circulating vs 1.00B total | Official unlock schedule and reconciled circulating methodology |
| Downside containment | Watchlist only | Transparent treasury, audits, risk controls, and stress-test history |

The strongest reason to add the report is not that Particle Network is obviously cheap. It is that the project is large or specific enough to track in the Research Map, and the current candidate backlog indicates it had not yet been covered locally.
...
| Settlement layer | Chains and contracts that hold user value | Base, Binance Smart Chain |
...
|---|---|---|---|
| base | PARTI | 0x59264f02d301281f3393e1385c0aefd446eb0f00 | [explorer](https://basescan.org/token/0x59264f02d301281f3393e1385c0aefd446eb0f00) |
| bsc | PARTI | 0x59264f02d301281f3393e1385c0aefd446eb0f00 | [explorer](https://bscscan.com/token/0x59264f02d301281f3393e1385c0aefd446eb0f00) |
...
The market intelligence layer has three parts: DeFi usage, market liquidity, and attention. For Particle Network, the DeFi-oriented backlog note is the first signal: Seeded 2026-07-06 from Surf market-ranking; rank 1100, market cap $11,416,036, 24h volume $11,833,426. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf/CG data then adds market cap $11.27M, FDV $48.38M, 24h volume $11.69M, and price $0.0484.
...
| 24h volume | $11.69M | Position sizing and exit liquidity proxy |
| Market cap | $11.27M | Liquid valuation proxy |
| FDV | $48.38M | Dilution-aware valuation proxy |
| FDV / market cap spread | 4.29x | Higher spread means more supply risk |
| X followers | 791.99K | Attention proxy, not adoption proof |
...
| Accrual channel | Strength for Particle Network | Evidence needed |
|---|---|---|
...
| Staking / security | Depends on whether PARTI is bonded or slashable | Staking contracts, validator docs, reward schedule |
...
The first analytical frame is utility durability. Particle Network should not be treated as investable only because it appears in CoinGecko or CoinMarketCap discovery. The useful question is whether the protocol controls a repeatable workflow: validators needing distributed operation, lenders needing collateralized liquidity, LPs needing routing, tokenized assets needing compliance rails, or depositors needing yield. In this memo the base assumption is that Particle Network belongs to the bridge / cross-chain infrastructure bucket. That means the token, if any, has to be judged by how directly it captures fees, security demand, collateral demand, risk-management demand, governance power, or distribution access. A high TVL number without fee conversion is weaker than a modest TVL base with sticky users, clear revenue, and controllable emissions.

The second frame is liquidity quality. Current tracked market data points to market cap $11.27M, FDV $48.38M, 24h volume $11.69M, price $0.0484, circulating supply 233.00M, and total supply 1.00B. These numbers are not a buy signal by themselves. They are a risk surface. When FDV materially exceeds market cap, the memo discounts upside because later unlocks can absorb demand. When volume is low relative to market cap, the memo discounts tradability because position sizing becomes difficult. When price performance is strong but project usage is flat, the memo treats the move as narrative beta rather than fundamental repricing. When usage is durable but price is weak, the memo treats the asset as a watchlist candidate rather than an immediate long.

The third frame is source triangulation. Surf project-detail gives an aggregated project view, CoinGecko provides a market-data and community-data surface, CoinMarketCap discovery is used to cross-check symbol identity, and DefiLlama style TVL notes provide a protocol-usage lens. For Particle Network, the candidate backlog records Seeded 2026-07-06 from Surf market-ranking; rank 1100, market cap $11,416,036, 24h volume $11,833,426. Local registry duplicate check passed; refresh CG/CMC/Surf and primary sources before writing.. Surf identifies chains as Base, Binance Smart Chain, exchanges as BINANCE, OKX, BITHUMB, BYBIT, BITGET, and tags as Cross-Chain & Bridge, Layer1, Wallets & AA (Account Abstraction). If these labels diverge from official documentation, official documentation wins. If CMC, CG, and Surf disagree on supply or market cap, this memo treats the supply field as unresolved until a token contract, official tokenomics page, and exchange-circulating methodology can be reconciled.
...
The fifth frame is reflexivity. Many DeFi and infrastructure assets reprice in waves: first through TVL and listings, then through fee visibility, then through tokenomics cleanup, and finally through real cash-flow or strategic acquisition narratives. Particle Network currently needs to be monitored through that sequence. The available 90-day TVL series has 0 point(s), latest TVL not disclosed, and approximate change not disclosed. Market-price movement shows 7d 5.95% and 30d -14.74% when Surf or CoinGecko exposed the field. If TVL and price move together, the thesis may be simple beta. If TVL rises while price lags, the thesis becomes value-capture skepticism. If price rises while TVL falls, the thesis becomes momentum without fundamental confirmation.

The conservative conclusion is that Particle Network's product relevance may be ahead of its token value capture. That is common in this sector. The report should therefore watch for activation events: fee switch, staking redesign, revenue dashboard, cap increases, major integrations, or emissions reductions.
...
|---|---|---|---|
| 2025-03-25 | Ido | $1.25M | not disclosed |
| 2024-08-01 | Undisclosed | not disclosed | YZi Labs (Prev. Binance Labs) |
| 2024-06-20 | Series A | $15.00M | Morningstar Ventures, The Spartan Group, HashKey Capital, LongHash Ventures, SevenX Ventures, gumi Cryptos, SNZ Holding, Flow Traders |
| 2023-04-14 | Strategic | not disclosed | Cobo |
| 2023-03-24 | Seed | $7.00M | Animoca Brands, HashKey Capital, LongHash Ventures, ArkStream Capital, Inception capital, FunPlus, GSR Ventures, Arcane Group |
| 2022-05-05 | Pre Seed | $1.80M | LongHash Ventures, 7 O'Clock Capital, CyberConnect, Insignia Ventures Partners, BitCoke, FSC Ventures, Monad Labs, Source Code Capital |

| Team member | Role | Link |
|---|---|---|
| Pengyu Wang | Founder | [X](https://x.com/0xpengyu) |
| Peter Pan | CTO | [X](https://x.com/0xpeterpan_) |
| Kerf Chang | Head of Ecosystem | not disclosed |
| DongXi Huang | Backend Lead | not disclosed |
...
Particle Network competes in the bridge / cross-chain infrastructure bucket, where category peers vary by exact design. The memo uses functional competitors rather than only ticker competitors:
...
The key competitive question is whether Particle Network has a reason to exist beyond incentives. If switching costs are low and users are primarily chasing emissions, any valuation premium should be temporary. If the protocol controls a scarce integration point, compliance rail, validator layer, or liquidity venue, it can deserve a persistent watchlist premium.
...
Weighted conclusion: Particle Network is important enough to track, but not clean enough to rank as a high-conviction token until economic capture is clearer.

## Bull / Base / Bear

| Scenario | Probability | Thesis | Monitoring signal |
|---|---:|---|---|
| Bull | 25% | Particle Network compounds usage, liquidity improves, and PARTI gains measurable value capture through staking, fees, collateral, or governance-controlled economics. | TVL up, fees visible, volume deepens, supply overhang falls |
...
The strongest bearish critique is simple: this could be a useful protocol with an average token. If PARTI does not capture fees or security demand, the market will eventually value it as governance optionality plus liquidity beta. The second critique is data inconsistency. CG, CMC, Surf, and official docs can disagree on circulating supply, FDV, and market cap. The third critique is reflexive TVL: deposits can leave quickly when incentives, yields, or narratives change.
...
| 24h volume | $11.69M | Weekly | Volume below position-size threshold |
| FDV / market cap | 4.29x | Monthly | Rising dilution gap |
...
1. Re-run Surf project-detail and project-defi-metrics if Particle Network announces a major integration, listing, tokenomics change, exploit, or governance proposal.
...
This report intentionally separates source availability from source quality. Particle Network has a visible market footprint, but a visible footprint is not the same as a reconciled investment record. The Research Map standard treats CoinGecko and CoinMarketCap as discovery and market-data surfaces, Surf as an aggregation and refresh layer, DefiLlama style records as a protocol-usage lens, and official docs as the final arbiter for product mechanics. When these sources disagree, the disagreement itself is part of the thesis. A token with unclear supply methodology, mismatched contract labels, stale app docs, or broken dashboard coverage is usually harder to underwrite even when the narrative looks attractive.

For Particle Network, the first reconciliation job is identity. The token symbol is PARTI, the category is bridge / cross-chain infrastructure, chains are Base, Binance Smart Chain, exchanges are BINANCE, OKX, BITHUMB, BYBIT, BITGET, and tags are Cross-Chain & Bridge, Layer1, Wallets & AA (Account Abstraction). These fields should be checked against official docs, token contracts, exchange market tabs, and community channels before the report graduates from watchlist to allocation candidate. If PARTI trades on multiple chains, each contract should be mapped to a canonical issuer statement. If the same symbol is reused by another asset, exchange tickers and contract addresses must be treated as separate facts rather than as interchangeable labels. This matters because a fuzzy ticker match can make a good memo wrong at the first line.

The second reconciliation job is market data. Market cap is $11.27M, FDV is $48.38M, 24h volume is $11.69M, FDV / market cap is 4.29x, 7d price change is 5.95%, and 30d price change is -14.74%. These numbers need a vendor-drift check. If CMC reports a different circulating supply from CoinGecko or Surf, the memo should prefer the official unlock and circulating-supply definition, then keep the aggregator spread as a risk note. If volume is dominated by one venue, the liquidity score should be lower than the headline volume implies. If FDV is not disclosed or total supply is missing, the base case should assume worse dilution until proven otherwise.
...
For Particle Network, the current liquidity read is watchlist first. The market cap is $11.27M, FDV is $48.38M, reported 24h volume is $11.69M, and FDV / market cap is 4.29x. These values can support a research card, but they do not automatically support an investment. A cleaner setup would show stable volume across multiple venues, narrow spreads, less reliance on a single pair, and a supply schedule that does not create a near-term wall of unlock pressure.
...
The failure-mode view is deliberately harsher than the base case. Particle Network can fail for reasons that do not show up in a simple market-data table. Smart contracts can be exploited. Oracles can misprice collateral. Bridges can be paused. Staking systems can experience slashing or validator concentration. RWA systems can face issuer, custodian, or redemption risk. Stablecoin systems can lose pegs. Privacy systems can face sanctions, delistings, or legal restrictions. DEXs and yield systems can lose users when incentives rotate. A full-depth memo should make these risks visible before they become headlines.

For Particle Network, the live monitoring stack should include contract events, TVL, volume, governance, documentation changes, social announcements, security disclosures, and exchange-market quality. A single metric is not enough. TVL may stay high while liquidity thins. Volume may rise because of volatility rather than adoption. Social attention may rise around controversy rather than progress. Governance may pass proposals that improve short-term price but add long-term risk. The strongest monitoring dashboard combines on-chain state, off-chain disclosure, and market quality.

Incident response matters because crypto risks are discontinuous. A protocol can look normal until a bridge halt, oracle issue, admin pause, market-maker withdrawal, regulatory action, or treasury sale changes the entire thesis. The Research Map should therefore attach explicit downgrade triggers. If Particle Network suffers a material exploit, downgrade until a post-mortem, reimbursement plan, and contract patch are published. If supply expands faster than disclosed, downgrade tokenomics confidence. If the main app loses integrations or TVL without explanation, downgrade product durability. If market depth collapses while price holds, downgrade liquidity quality.
...
| Peer comparison | Monthly | Particle Network gains share with better economics | Peers grow faster with cleaner capture |

## Evidence Ledger

| # | Source | Link | Why it matters |
|---:|---|---|---|
| 1 | Official website | [link](https://particle.network/) | Identity, market, social, contract, or discovery cross-check |
| 2 | CoinGecko search | [link](https://www.coingecko.com/en/search?query=Particle%20Network) | Identity, market, social, contract, or discovery cross-check |
| 3 | CoinMarketCap discovery | [link](https://coinmarketcap.com/search/?q=Particle%20Network) | Identity, market, social, contract, or discovery cross-check |
| 4 | DefiLlama protocol discovery | [link](https://defillama.com/search?query=Particle%20Network) | Identity, market, social, contract, or discovery cross-check |
| 5 | Token Terminal discovery | [link](https://tokenterminal.com/explorer?search=Particle%20Network) | Identity, market, social, contract, or discovery cross-check |
| 6 | Messari discovery | [link](https://messari.io/search?q=Particle%20Network) | Identity, market, social, contract, or discovery cross-check |
| 7 | X profile | [link](https://x.com/ParticleNtwrk) | Identity, market, social, contract, or discovery cross-check |
| 8 | Particle Network Price Chart (PARTI) - CoinGecko | [link](https://www.coingecko.com/en/coins/particle-network) | Identity, market, social, contract, or discovery cross-check |
| 9 | Latest Particle Network (PARTI) Price Analysis - CoinMarketCap | [link](https://coinmarketcap.com/cmc-ai/particle-network/price-analysis/) | Identity, market, social, contract, or discovery cross-check |
| 10 | PARTI to PKR: Particle Network Price in Pakistani Rupee / CoinGecko | [link](https://www.coingecko.com/en/coins/particle-network/pkr) | Identity, market, social, contract, or discovery cross-check |
| 11 | Particle Network (PARTI) / Tokenomics & Vesting Schedule | [link](https://tokenomist.ai/particle-network) | Identity, market, social, contract, or discovery cross-check |
| 12 | PARTI to BTC: Particle Network Price in Bitcoin - CoinGecko | [link](https://www.coingecko.com/en/coins/particle-network/btc) | Identity, market, social, contract, or discovery cross-check |
| 13 | base PARTI | [link](https://basescan.org/token/0x59264f02d301281f3393e1385c0aefd446eb0f00) | Identity, market, social, contract, or discovery cross-check |
| 14 | bsc PARTI | [link](https://bscscan.com/token/0x59264f02d301281f3393e1385c0aefd446eb0f00) | Identity, market, social, contract, or discovery cross-check |
| 15 | Google project search | [link](https://www.google.com/search?q=Particle%20Network%20PARTI%20crypto) | Identity, market, social, contract, or discovery cross-check |
| 16 | Google tokenomics search | [link](https://www.google.com/search?q=Particle%20Network%20PARTI%20tokenomics) | Identity, market, social, contract, or discovery cross-check |
| 17 | Google docs search | [link](https://www.google.com/search?q=Particle%20Network%20docs) | Identity, market, social, contract, or discovery cross-check |
| 18 | Google governance search | [link](https://www.google.com/search?q=Particle%20Network%20governance%20proposal) | Identity, market, social, contract, or discovery cross-check |
| 19 | Google audit search | [link](https://www.google.com/search?q=Particle%20Network%20audit%20report) | Identity, market, social, contract, or discovery cross-check |
| 20 | CoinMarketCap what-is guess | [link](https://coinmarketcap.com/cmc-ai/particle-network/what-is/) | Identity, market, social, contract, or discovery cross-check |
| 21 | CoinMarketCap currency guess | [link](https://coinmarketcap.com/currencies/particle-network/) | Identity, market, social, contract, or discovery cross-check |
| 22 | Dune discovery | [link](https://dune.com/browse/dashboards?q=Particle%20Network) | Identity, market, social, contract, or discovery cross-check |
| 23 | Dune token query | [link](https://dune.com/browse/queries?q=Particle%20Network) | Identity, market, social, contract, or discovery cross-check |
| 24 | DeFiLlama fees discovery | [link](https://defillama.com/fees?chain=All&search=Particle%20Network) | Identity, market, social, contract, or discovery cross-check |
| 25 | DeFiLlama yield discovery | [link](https://defillama.com/yields?token=PARTI) | Identity, market, social, contract, or discovery cross-check |
| 26 | TokenUnlocks search | [link](https://token.unlocks.app/?search=Particle%20Network) | Identity, market, social, contract, or discovery cross-check |
| 27 | CryptoRank search | [link](https://cryptorank.io/price/particle-network) | Identity, market, social, contract, or discovery cross-check |
| 28 | RootData search | [link](https://www.rootdata.com/search?kw=Particle%20Network) | Identity, market, social, contract, or discovery cross-check |
| 29 | CoinDesk search | [link](https://www.coindesk.com/search?s=Particle%20Network) | Identity, market, social, contract, or discovery cross-check |
| 30 | The Block search | [link](https://www.theblock.co/search?query=Particle%20Network) | Identity, market, social, contract, or discovery cross-check |
| 31 | GitHub search | [link](https://github.com/search?q=Particle%20Network&type=repositories) | Identity, market, social, contract, or discovery cross-check |
| 32 | Etherscan token search | [link](https://etherscan.io/search?f=0&q=PARTI) | Identity, market, social, contract, or discovery cross-check |
| 33 | BscScan token search | [link](https://bscscan.com/search?f=0&q=PARTI) | Identity, market, social, contract, or discovery cross-check |
| 34 | Arbiscan token search | [link](https://arbiscan.io/search?f=0&q=PARTI) | Identity, market, social, contract, or discovery cross-check |

Additional direct source list:

- [Official website](https://particle.network/)
- [CoinGecko search](https://www.coingecko.com/en/search?query=Particle%20Network)
- [CoinMarketCap discovery](https://coinmarketcap.com/search/?q=Particle%20Network)
- [DefiLlama protocol discovery](https://defillama.com/search?query=Particle%20Network)
- [Token Terminal discovery](https://tokenterminal.com/explorer?search=Particle%20Network)
- [Messari discovery](https://messari.io/search?q=Particle%20Network)
- [X profile](https://x.com/ParticleNtwrk)
- [Particle Network Price Chart (PARTI) - CoinGecko](https://www.coingecko.com/en/coins/particle-network)
- [Latest Particle Network (PARTI) Price Analysis - CoinMarketCap](https://coinmarketcap.com/cmc-ai/particle-network/price-analysis/)
- [PARTI to PKR: Particle Network Price in Pakistani Rupee | CoinGecko](https://www.coingecko.com/en/coins/particle-network/pkr)
- [Particle Network (PARTI) | Tokenomics & Vesting Schedule](https://tokenomist.ai/particle-network)
- [PARTI to BTC: Particle Network Price in Bitcoin - CoinGecko](https://www.coingecko.com/en/coins/particle-network/btc)
- [base PARTI](https://basescan.org/token/0x59264f02d301281f3393e1385c0aefd446eb0f00)
- [bsc PARTI](https://bscscan.com/token/0x59264f02d301281f3393e1385c0aefd446eb0f00)
- [Google project search](https://www.google.com/search?q=Particle%20Network%20PARTI%20crypto)
- [Google tokenomics search](https://www.google.com/search?q=Particle%20Network%20PARTI%20tokenomics)
- [Google docs search](https://www.google.com/search?q=Particle%20Network%20docs)
- [Google governance search](https://www.google.com/search?q=Particle%20Network%20governance%20proposal)
- [Google audit search](https://www.google.com/search?q=Particle%20Network%20audit%20report)
- [CoinMarketCap what-is guess](https://coinmarketcap.com/cmc-ai/particle-network/what-is/)
- [CoinMarketCap currency guess](https://coinmarketcap.com/currencies/particle-network/)
- [Dune discovery](https://dune.com/browse/dashboards?q=Particle%20Network)
- [Dune token query](https://dune.com/browse/queries?q=Particle%20Network)
- [DeFiLlama fees discovery](https://defillama.com/fees?chain=All&search=Particle%20Network)

## Final Investment View

Particle Network should be in the Research Map as a **watchlist / selective diligence** asset. The project has enough category relevance and data footprint to merit ongoing coverage, especially because it came from the CG/CMC-oriented candidate workflow and has no high-confidence local duplicate. The current evidence does not yet support a high-conviction allocation thesis. The next level of confidence requires cleaner token economics: fee capture, staking or collateral utility, transparent unlocks, and liquidity that can support realistic position sizing.
