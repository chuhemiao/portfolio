...
- **2026-06-18**：基于当前 `/research` Research Map 和 160+ 篇 research MDX，创建可自动发现的 Codex skill：`~/.codex/skills/research-map-builder`。该 skill 用于后续 Web3 项目深度研究与生成新的 portfolio Research，覆盖“研究问题 → source-backed fact base → 投资级分析 → MDX 写作 → `pnpm sync:research --add` 同步 Research Map → logo/content 校验”的完整流程；附带 `source-playbook.md`、`report-blueprint.md`、`portfolio-workflow.md` 三份 references。已用临时 `/tmp` venv + PyYAML 跑过 skill-creator `quick_validate.py`，结果 `Skill is valid!`。
